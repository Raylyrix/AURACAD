...
int main(int argc, char *argv[], char *const *envp) {
    ...
    std::string AuraCAD = realpath((std::string(cwd) + "/../Resources/bin/AuraCAD").c_str(), NULL);
    ...
    env["SSL_CERT_FILE"]        = prefix + "/ssl/cacert.pem";    // https://forum.AuraCAD.org/viewtopic.php?f=3&t=42825
    ...
    std::cout << "Running: " << AuraCAD << std::endl;
    i = 0;
    while(argv[i] != NULL) {
        i++;
    }
    ...
    new_argv[0] = new char[AuraCAD.length() + 1];
    memset(new_argv[0], 0, AuraCAD.length() + 1);
    strncpy(new_argv[0], AuraCAD.c_str(), AuraCAD.length());
    ...
    return execve(AuraCAD.c_str(), new_argv, new_env);
}
