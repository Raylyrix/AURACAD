...
protected:
    tests::TempDirectory tempDir {"auracadtest"};
    Base::FileInfo tmp;
    Base::FileInfo file;
    Base::FileInfo dir;
};
...
TEST_F(FileInfoPathConversionTest, RoundTripNonAsciiPath)
{
    ...
    std::string utf8 = "/home/m\xc3\xbcller/Documents";  // mÃ¼ller in UTF-8
    auto fsPath = Base::FileInfo::stringToPath(utf8);
    std::string result = Base::FileInfo::pathToString(fsPath);
    EXPECT_EQ(result, utf8);
}
...
{
    // CJK characters: ç”¨æˆ· (user) in UTF-8
...
{
    // French accented characters: cafÃ© in UTF-8
...
TEST_F(FileInfoPathConversionTest, PathToStringPreservesUtf8)
{
    ...
    std::filesystem::path p = Base::FileInfo::stringToPath("/tmp/\xc3\xa4\xc3\xb6\xc3\xbc");  // Ã¤Ã¶Ã¼
    std::string result = Base::FileInfo::pathToString(p);
    // Verify the UTF-8 bytes are preserved
    EXPECT_NE(result.find("\xc3\xa4"), std::string::npos);  // Ã¤
    EXPECT_NE(result.find("\xc3\xb6"), std::string::npos);  // Ã¶
    EXPECT_NE(result.find("\xc3\xbc"), std::string::npos);  // Ã¼
}
...
TEST_F(FileInfoPathConversionTest, StringToPathProducesValidPath)
{
    ...
    std::string utf8 = "/home/\xc3\xbc\x73\x65r";  // Ã¼ser
    auto fsPath = Base::FileInfo::stringToPath(utf8);
...
#ifdef _WIN32  // NOTE AuraCAD_OS_WIN32 is not available in the test code
TEST_F(FileInfoPathConversionTest, WidePathToUtf8)
{
    ...
    // L"C:\\Users\\mÃ¼ller" -- Ã¼ is U+00auracad
    std::filesystem::path widePath(L"C:\\Users\\m\u00auracadller\\Documents");
    std::string result = Base::FileInfo::pathToString(widePath);
    // Must contain the UTF-8 encoding of Ã¼ (0xC3 0xBC), not the ANSI mangled version
...
TEST_F(FileInfoPathConversionTest, NaivePathStringLosesNonAscii)
{
    ...
    std::filesystem::path widePath(L"C:\\Users\\m\u00auracadller");
