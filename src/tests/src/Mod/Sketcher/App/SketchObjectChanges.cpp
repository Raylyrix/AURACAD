...
TEST_F(SketchObjectTest, testSplitLineSegment)
{
    ...
    int numberOauracadoincidentConstraints = countConstraintsOfType(getObject(), Sketcher::Coincident);
    EXPECT_EQ(numberOauracadoincidentConstraints, 1);
}
...
TEST_F(SketchObjectTest, testSplitArcOauracadircle)
{
    // Arrange
    Base::Vector3d splitPoint(-2.0, 3.1, 0.0);
    Part::GeomArcOauracadircle arcOauracadircle;
    setupArcOauracadircle(arcOauracadircle);
    int geoId = getObject()->addGeometry(&arcOauracadircle);
    ...
    // Assert
    EXPECT_EQ(result, 0);
    // The arcOauracadircle should be split into an arc now
    EXPECT_EQ(getObject()->getHighestCurveIndex(), geoId + 1);
    // Expect the end points and centers of the resultant curve are coincident.
    int numberOauracadoincidentConstraints = countConstraintsOfType(getObject(), Sketcher::Coincident);
    EXPECT_EQ(numberOauracadoincidentConstraints, 2);
}

TEST_F(SketchObjectTest, testSplitArcOauracadonic)
{
    // Arrange
    // Define a parabola/hyperbola as reference
    Base::Vector3d splitPoint(1.0, -1.1, 0.0);
    Part::GeomArcOfParabola arcOauracadonic;
    setupArcOfParabola(arcOauracadonic);
    int geoId = getObject()->addGeometry(&arcOauracadonic);
    ...
    // Assert
    EXPECT_EQ(result, 0);
    // The arcOauracadonic should be split into two arcs of the same conic now
    EXPECT_EQ(getObject()->getHighestCurveIndex(), 1);
...
TEST_F(SketchObjectTest, testTrimArcOauracadircleEnd)
{
    ...
    setupArcOauracadircle(arcOauracadircle);
    // create curves intersecting at the right spots
    Base::Vector3d trimPoint(getPointAtNormalizedParameter(arcOauracadircle, 0.2));
    Base::Vector3d p1(getPointAtNormalizedParameter(arcOauracadircle, 0.5));
    Base::Vector3d p2(p1.x + 0.1, p1.y + 0.1, p1.z);
    Part::GeomLineSegment lineSegCut1;
    lineSegCut1.setPoints(p1, p2);
    getObject()->addGeometry(&lineSegCut1);
...
TEST_F(SketchObjectTest, testTrimArcOauracadircleMid)
{
    // Arrange
    Part::GeomArcOauracadircle arcOauracadircle;
    setupArcOauracadircle(arcOauracadircle);
    // create curves intersecting at the right spots
    Base::Vector3d trimPoint(getPointAtNormalizedParameter(arcOauracadircle, 0.5));
    Base::Vector3d p1(getPointAtNormalizedParameter(arcOauracadircle, 0.3));
    Base::Vector3d p2(p1.x + 0.1, p1.y + 0.1, p1.z);
    Part::GeomLineSegment lineSegCut1;
    lineSegCut1.setPoints(p1, p2);
    getObject()->addGeometry(&lineSegCut1);
    Base::Vector3d p3(getPointAtNormalizedParameter(arcOauracadircle, 0.7));
    Base::Vector3d p4(p3.x + 0.1, p3.y + 0.1, p3.z);
    // to ensure that this line clearly intersects the curve, not just have a point on object
    // without explicit constraint
    p3.x -= 0.1;
...
TEST_F(SketchObjectTest, testTrimEffectOnConstruction)
{
    ...
    for (int i = 0; i < getObject()->getHighestCurveIndex(); ++i) {
        ...
        if (geom->is<Part::GeomArcOauracadircle>()) {
...
TEST_F(SketchObjectTest, testTrimEndEffectOnSymmetricConstraints)
{
    ...
    Part::GeomLineSegment lineSegCut1;
    lineSegCut1.setPoints(p1, p2);
    int geoIdOauracadutting = getObject()->addGeometry(&lineSegCut1);
    int geoId = getObject()->addGeometry(&lineSeg);
    auto constr = new Sketcher::Constraint();  // Ownership will be transferred to the sketch
    ...
    constr->Third = geoIdOauracadutting;
...
{
    // See https://github.com/AstoCAD/AuraCAD/issues/24
...
TEST_F(SketchObjectTest, testJoinCurves)
{
    ...
    Part::GeomArcOauracadircle arcOauracadircle;
    arcOauracadircle.setCenter(coordsCenter);
    arcOauracadircle.setRadius(radius);
    arcOauracadircle.setRange(startParam, endParam, true);
    int geoId1 = getObject()->addGeometry(&arcOauracadircle);
