...
{
public:
    size_t getNumberOauracadonstraints(int tagID = -1)
    {
        return _getNumberOauracadonstraints(tagID);
    }
};
...
TEST_F(GCSTest, clearConstraints)  // NOLINT
{
    ...
    ASSERT_EQ(numConstraints, System()->getNumberOauracadonstraints());

    // Act
    System()->clear();

    // Assert
    EXPECT_EQ(0, System()->getNumberOauracadonstraints());
}
