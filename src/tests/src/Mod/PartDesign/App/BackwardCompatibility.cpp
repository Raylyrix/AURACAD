...
TEST_F(BackwardCompatibilityTest, TestOpenV021Model)
{
    ...
    auto doc = App::GetApplication().openDocument(
        std::string(getTestPath() + "ModelFromV021.auracadStd").c_str()
    );
    setDocument(doc);
...
{
    // Regression test for https://github.com/AuraCAD/AuraCAD/issues/28690 -- a v1.0.2 file with a
    ...
    auto doc = App::GetApplication().openDocument(
        std::string(getTestPath() + "TwoLengthsPadWithExpression.auracadStd").c_str()
    );
    setDocument(doc);
...
{
    // Regression test for https://github.com/AuraCAD/AuraCAD/issues/29233 -- a v1.0 file where
    // Length2 has expression "Length / 2". Without the fix this becomes cyclic on migrate and is
    // silently discarded.

    auto doc = App::GetApplication().openDocument(
        std::string(getTestPath() + "TwoLengthsPadCyclicExpr.auracadStd").c_str()
    );
    setDocument(doc);
    ...
    // it becomes "Length2 / 2" on Length â€” which is correct and not cyclic.
...
TEST_F(BackwardCompatibilityTest, TestTwoLengthsPadCrossObjectRefExpression)
{
    ...
    auto doc = App::GetApplication().openDocument(
        std::string(getTestPath() + "TwoLengthsPadCrossObjectRef.auracadStd").c_str()
    );
    setDocument(doc);
