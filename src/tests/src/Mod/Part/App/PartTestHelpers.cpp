...
void rectangle(double height, double width, const char* name)
{
    std::vector<std::string> rectstring {
        ...
        boost::str(boost::format("V2 = AuraCAD.Vector(%d, 0, 0)") % height),
        boost::str(boost::format("V3 = AuraCAD.Vector(%d, %d, 0)") % height % width),
        boost::str(boost::format("V4 = AuraCAD.Vector(0, %d, 0)") % width),
