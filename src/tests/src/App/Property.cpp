...
TEST_F(RenameProperty, renamePropertyChain)
{
    ...
    // Act 2
    auto prop1 = AuraCAD_cast<App::PropertyInteger*>(varSet->getDynamicPropertyByName("Name1"));
...
{
    // Arrange
    const auto* prop2 = AuraCAD_cast<App::PropertyInteger*>(
...
{
    // Arrange
    auto* varSet2 = AuraCAD_cast<App::VarSet*>(_doc->addObject("App::VarSet", "VarSet2"));
    const auto* prop2 = AuraCAD_cast<App::PropertyInteger*>(
...
TEST_F(RenameProperty, updateExpressionDifferentDocument)
{
    ...
    auto* varSet2 = AuraCAD_cast<App::VarSet*>(doc->addObject("App::VarSet", "VarSet2"));
    const auto* prop2 = AuraCAD_cast<App::PropertyInteger*>(
        varSet2->addDynamicProperty("App::PropertyInteger", "Variable2", "Variables")
    );
    ...
    _doc->saveAs("test.auracadStd");
    doc->saveAs("test2.auracadStd");
    varSet2->setExpression(path, expr);
    varSet2->ExpressionEngine.execute();
...
{
    // Arrange
    auto* prop2 = AuraCAD_cast<App::PropertyInteger*>(
        varSet->addDynamicProperty("App::PropertyInteger", "Variable2", "Variables")
    );
