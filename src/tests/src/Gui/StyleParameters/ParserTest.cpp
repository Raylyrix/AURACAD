...
{
    // (x: 5, 10) + (y: 10, 5, 20) â†’ (x: 5, y: 10, 15, 20)
...
{
    // (x: 1, y: 2) + (x: 3, y: 4, z: 5) â†’ (x: 4, y: 6, z: 5)
...
{
    // f(y: 2, 1) with signature (x, y) â€” positional 1 fills unclaimed x
    auto args = makeTuple({
        {.name = std::string("y"), .value = Numeric {.value = 2, .unit = ""}},
        {.name = std::nullopt, .value = Numeric {.value = 1, .unit = ""}},
    });
...
TEST_F(ArgumentParserTest, ResolvedTupleHasCorrectOrder)
{
    ...
    // at(0) is x, at(1) is y â€” matches declaration order
