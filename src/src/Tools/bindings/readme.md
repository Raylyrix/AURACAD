# AuraCAD C++ to Python Binding System Manual

Welcome to the new Python-based binding system for exposing AuraCAD C++ APIs to Python. This system replaces the previous XML-based approach with a more direct and flexible Python interface, allowing C++ developers to define Python bindings using native Python syntax, type annotations, and decorators.
...
The new Python-based binding system allows you to create bindings between AuraCADâ€™s C++ APIs and Python directly within Python source files. By leveraging Pythonâ€™s native featuresâ€”such as type annotations, decorators, and overloadsâ€”you can produce well-documented, type-safe Python interfaces that closely mirror your C++ classes.
...
Attributes defined as read-only are annotated with `Final` from Pythonâ€™s `typing` module to indicate immutability.
...
The `sequence_protocol` decorator is used to declare that a class implements Pythonâ€™s sequence protocol. This includes support for operations like indexing, slicing, iteration, and length retrieval. By attaching protocol metadata, you can control how the binding system exposes these behaviors.
...
1. **`sq_length` â†’ `static Py_ssize_t sequence_length(PyObject *)`**

    * **Purpose:**
        Implements the â€œlengthâ€ function for the object.
    * **Usage in Python:**
...
2. **`sq_concat` â†’ `static PyObject* sequence_concat(PyObject *, PyObject *)`**
...
3. **`sq_repeat` â†’ `static PyObject * sequence_repeat(PyObject *, Py_ssize_t)`**
...
4. **`sq_item` â†’ `static PyObject * sequence_item(PyObject *, Py_ssize_t)`**
...
5. **`sq_ass_item` â†’ `static int sequence_ass_item(PyObject *, Py_ssize_t, PyObject *)`**
...
6. **`sq_contains` â†’ `static int sequence_contains(PyObject *, PyObject *)`**
...
7. **`sq_inplace_concat` â†’ `static PyObject* sequence_inplace_concat(PyObject *, PyObject *)`**
...
8. **`sq_inplace_repeat` â†’ `static PyObject * sequence_inplace_repeat(PyObject *, Py_ssize_t)`**
...
1. **`mp_subscript` â†’ `static PyObject * mapping_subscript(PyObject *, PyObject *)`**
...
2. **`mp_ass_subscript` â†’ `static int mapping_ass_subscript(PyObject *, PyObject *, PyObject *)`**
