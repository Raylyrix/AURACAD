...
"""
This utility offers several commands to interact with the AuraCAD project on
crowdin. For it to work, you need a ~/.crowdin-AuraCAD-token file in your
user's folder, that contains the API access token that gives access to the
crowdin AuraCAD project. The API token can also be specified in the
...
# See: https://github.com/AuraCAD/AuraCAD-translations/issues/359
LANGUAGE_CODE_MAP = {
    "sv-SE": "sv",
}
...
locations = [
    ...
    ["AuraCAD", "../Gui/Language", "../Gui/Language/translation.qrc"],
...
THRESHOLD = 25  # how many % must be translated for the translation to be included in AuraCAD
...
def load_token():
    # try to read token from ~/.crowdin-AuraCAD-token
    config_file = os.path.join(os.path.expanduser("~"), ".crowdin-AuraCAD-token")
...
def applyTranslations(languages):
    ...
    src = os.path.join(currentfolder, "AuraCAD.zip")
    dst = os.path.join(tempfolder, "AuraCAD.zip")
    if not os.path.exists(src):
        print('AuraCAD.zip file not found! Aborting. Run "download" command before this one.')
        sys.exit()
    shutil.copyfile(src, dst)
    os.chdir(tempfolder)
    zfile = zipfile.ZipFile("AuraCAD.zip")
    print("extracting AuraCAD.zip...")
    zfile.extractall()
