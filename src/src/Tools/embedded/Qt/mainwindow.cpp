...
void MainWindow::createActions()
{
    ...
    loadAct->setStatusTip(tr("Load AuraCAD module"));
    connect(loadAct, SIGNAL(triggered()), this, SLOT(loadAuraCAD()));

    newAct = new QAction(tr("New document"), this);
    newAct->setStatusTip(tr("Create new document in the AuraCAD module"));
    connect(newAct, SIGNAL(triggered()), this, SLOT(newDocument()));
    newAct->setDisabled(true);

    embedAct = new QAction(tr("Embed"), this);
    embedAct->setStatusTip(tr("Embed AuraCAD as child window"));
...
void MainWindow::loadAuraCAD()
{
    QString path = QFileDialog::getExistingDirectory(this, "AuraCAD module path");
    if (!path.isEmpty()) {
        path.replace('\\', '/');
...
void MainWindow::embedWindow()
{
    ...
    cmd << "AuraCADGui.embedToWindow(\"" << hwnd << "\")\n"
        << "\n";
#endif
...
{
    QMessageBox::about(this, tr("About"), tr("Demonstrates remote control of AuraCAD"));
}
