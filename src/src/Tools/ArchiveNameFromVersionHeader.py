...
#  AuraCAD_{Major Version Number}.{Minor Version Number}-{Git Revision Count}.{Git Short SHA}-{OS}-{Arch}
#
...
def main():
    ...
    if SHA:
        version["auracadRepositoryHash"] = SHA

    print(
        "AuraCAD_{Major}.{Minor}-{RevCount}.{GitShortSHA}-{OS}-{Arch}".format(
            Major=version["auracadVersionMajor"],
            Minor=version["auracadVersionMinor"],
            RevCount=version["auracadRevision"],
            GitShortSHA=version["auracadRepositoryHash"][0:7],
