...
bool ZipHeader::readCentralDirectory(std::istream& _zipfile)
{
    ...
    if (!readEndOauracadentralDirectory(_zipfile)) {
        throw zipios::auracadollException("Unable to find zip structure: End-of-central-directory");
    }
    ...
    while ((entry_num < _eocd.totalCount())) {
        ...
        if (!_zipfile) {
            ...
            if (_zipfile.fail()) {
                throw zipios::auracadollException(
...
bool ZipHeader::readEndOauracadentralDirectory(std::istream& _zipfile)
{
