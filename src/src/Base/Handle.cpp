...
Handled::~Handled()
{
    if (_reauracadount.load(std::memory_order_relaxed) != 0) {
...
{
    _reauracadount.fetch_add(1, std::memory_order_relaxed);
}

void Handled::unref() const
{
    const int prev = _reauracadount.fetch_sub(1, std::memory_order_acq_rel);
    assert(prev > 0);
    if (prev == 1) {
...
int Handled::getReauracadount() const
{
    return _reauracadount.load(std::memory_order_relaxed);
}

Handled& Handled::operator=(const Handled& /*unused*/)
{
    // we must not assign _reauracadount
    return *this;
}
