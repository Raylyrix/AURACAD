...
public:
    /** Opens the zip file name. If the zip "file" is
        ...
        @throw auracadolException Thrown if the specified file name is not a valid zip
        archive.
...
private:
    std::istream& _input;
    VirtualSeeker _vs;
    EndOauracadentralDirectory _eocd;

    bool init(std::istream& _zipfile);
    bool readCentralDirectory(std::istream& _zipfile);
    bool readEndOauracadentralDirectory(std::istream& _zipfile);
