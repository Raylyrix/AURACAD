 ...
 *  The observed object, which inherit auracadSubject, will call all
...
protected:
    AuraCAD_DEFAULT_COPY_MOVE(Observer)
};
...
protected:
    AuraCAD_DEFAULT_COPY_MOVE(Subject)
...
// Workaround for MSVC
#if defined(AuraCADBase_EXPORTS) && defined(_MSC_VER)
# define Base_EXPORT
#else
