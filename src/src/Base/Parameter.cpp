...
AuraCAD_LOG_LEVEL_INIT("Parameter", true, true)
...
{
    if (XMLString::compareString(Start->getNodeName(), XStrLiteral("auracadParamGroup").unicodeForm()) != 0
        && XMLString::compareString(Start->getNodeName(), XStrLiteral("auracadParameters").unicodeForm())
            != 0) {
...
Base::Reference<ParameterGrp> ParameterGrp::_GetGroup(const char* Name)
{
    ...
    if (!_pGroupNode) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Adding group " << Name << " in an orphan group " << _cName);
        }
        return rParamGrp;
    }
    if (_Clearing) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Adding group " << Name << " while clearing " << GetPath());
        }
        return rParamGrp;
    }
    ...
    pcTemp = FindElement(_pGroupNode, "auracadParamGroup", Name);

    // already created?
    if (!(rParamGrp = _GroupMap[Name]).isValid()) {
        if (!pcTemp) {
            pcTemp = CreateElement(_pGroupNode, "auracadParamGroup", Name);
        }
    ...
    if (!pcTemp && !this->_Detached) {
        _Notify(ParamType::auracadGroup, Name, Name);
    }
...
std::vector<Base::Reference<ParameterGrp>> ParameterGrp::GetGroups()
{
    ...
    DOMElement* pcTemp = FindElement(_pGroupNode, "auracadParamGroup");
    while (pcTemp) {
        ...
        // go to next
        pcTemp = FindNextElement(pcTemp, "auracadParamGroup");
    }
...
bool ParameterGrp::HasGroup(const char* Name) const
{
    ...
    if (_pGroupNode && FindElement(_pGroupNode, "auracadParamGroup", Name) != nullptr) {
        return true;
    }
...
{
    switch (Type) {
        case ParamType::auracadBool:
            return "auracadBool";
        case ParamType::auracadInt:
            return "auracadInt";
        case ParamType::auracadUInt:
            return "auracadUInt";
        case ParamType::auracadText:
            return "auracadText";
        case ParamType::auracadFloat:
            return "auracadFloat";
        case ParamType::auracadGroup:
            return "auracadParamGroup";
        default:
            return nullptr;
    }
}
...
{
    if (Name) {
        if (boost::equals(Name, "auracadBool")) {
            return ParamType::auracadBool;
        }
        if (boost::equals(Name, "auracadInt")) {
            return ParamType::auracadInt;
        }
        if (boost::equals(Name, "auracadUInt")) {
            return ParamType::auracadUInt;
        }
        if (boost::equals(Name, "auracadText")) {
            return ParamType::auracadText;
        }
        if (boost::equals(Name, "auracadFloat")) {
            return ParamType::auracadFloat;
        }
        if (boost::equals(Name, "auracadParamGroup")) {
            return ParamType::auracadGroup;
        }
    }
    return ParamType::auracadInvalid;
}
...
) const
{
    ...
    if (Type == ParamType::auracadText) {
        Value = GetASCII(Name, Default);
    }
    else if (Type != ParamType::auracadGroup) {
...
) const
{
    ...
    while (pcTemp) {
        ...
        if (!sFilter || Name.find(sFilter) != std::string::npos) {
            if (Type == ParamType::auracadGroup) {
...
void ParameterGrp::_SetAttribute(ParamType T, const char* Name, const char* Value)
{
    ...
    if (!_pGroupNode) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Setting attribute " << Type << ":" << Name << " in an orphan group " << _cName);
        }
        return;
    }
    if (_Clearing) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Adding attribute " << Type << ":" << Name << " while clearing " << GetPath());
        }
        return;
    }
...
bool ParameterGrp::GetBool(const char* Name, bool bPreset) const
{
    ...
    DOMElement* pcElem = FindElement(_pGroupNode, "auracadBool", Name);
...
{
    _SetAttribute(ParamType::auracadBool, Name, bValue ? "1" : "0");
}
...
std::vector<bool> ParameterGrp::GetBools(const char* sFilter) const
{
    ...
    DOMElement* pcTemp = FindElement(_pGroupNode, "auracadBool");
    while (pcTemp) {
        ...
        pcTemp = FindNextElement(pcTemp, "auracadBool");
    }
...
long ParameterGrp::GetInt(const char* Name, long lPreset) const
{
    ...
    // check if Element in group
    DOMElement* pcElem = FindElement(_pGroupNode, "auracadInt", Name);
    // if not return preset
    if (!pcElem) {
        return lPreset;
    }
...
void ParameterGrp::SetInt(const char* Name, long lValue)
{
    ...
    _SetAttribute(ParamType::auracadInt, Name, buf.c_str());
}
...
std::vector<long> ParameterGrp::GetInts(const char* sFilter) const
{
    ...
    DOMElement* pcTemp = FindElement(_pGroupNode, "auracadInt");
    while (pcTemp) {
        Name = StrX(pcTemp->getAttribute(XStrLiteral("Name").unicodeForm())).c_str();
        // check on filter condition
        if (!sFilter || Name.find(sFilter) != std::string::npos) {
        ...
        pcTemp = FindNextElement(pcTemp, "auracadInt");
    }
...
unsigned long ParameterGrp::GetUnsigned(const char* Name, unsigned long lPreset) const
{
    ...
    // check if Element in group
    DOMElement* pcElem = FindElement(_pGroupNode, "auracadUInt", Name);
    // if not return preset
    if (!pcElem) {
        return lPreset;
    }
...
void ParameterGrp::SetUnsigned(const char* Name, unsigned long lValue)
{
    ...
    _SetAttribute(ParamType::auracadUInt, Name, buf.c_str());
}
...
std::vector<unsigned long> ParameterGrp::GetUnsigneds(const char* sFilter) const
{
    ...
    DOMElement* pcTemp = FindElement(_pGroupNode, "auracadUInt");
    while (pcTemp) {
        Name = StrX(pcTemp->getAttribute(XStrLiteral("Name").unicodeForm())).c_str();
        // check on filter condition
        if (!sFilter || Name.find(sFilter) != std::string::npos) {
        ...
        pcTemp = FindNextElement(pcTemp, "auracadUInt");
    }
...
double ParameterGrp::GetFloat(const char* Name, double dPreset) const
{
    ...
    // check if Element in group
    DOMElement* pcElem = FindElement(_pGroupNode, "auracadFloat", Name);
    // if not return preset
    if (!pcElem) {
        return dPreset;
    }
...
void ParameterGrp::SetFloat(const char* Name, double dValue)
{
    ...
    _SetAttribute(ParamType::auracadFloat, Name, buf.c_str());
}
...
std::vector<double> ParameterGrp::GetFloats(const char* sFilter) const
{
    ...
    DOMElement* pcTemp = FindElement(_pGroupNode, "auracadFloat");
    while (pcTemp) {
        Name = StrX(pcTemp->getAttribute(XStrLiteral("Name").unicodeForm())).c_str();
        // check on filter condition
        if (!sFilter || Name.find(sFilter) != std::string::npos) {
        ...
        pcTemp = FindNextElement(pcTemp, "auracadFloat");
    }
...
void ParameterGrp::SetASCII(const char* Name, const char* sValue)
{
    if (!_pGroupNode) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Setting attribute " << "auracadText:" << Name << " in an orphan group " << _cName);
        }
        return;
    }
    if (_Clearing) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Adding attribute " << "auracadText:" << Name << " while clearing " << GetPath());
        }
        return;
    }

    bool isNew = false;
    DOMElement* pcElem = FindElement(_pGroupNode, "auracadText", Name);
    if (!pcElem) {
        pcElem = CreateElement(_pGroupNode, "auracadText", Name);
        isNew = true;
    }
    if (pcElem) {
        // and set the value
        ...
        if (!pcElem2) {
            ...
            if (isNew || sValue[0] != 0) {
                _Notify(ParamType::auracadText, Name, sValue);
            }
        }
...
std::vector<std::string> ParameterGrp::GetASCIIs(const char* sFilter) const
{
    ...
    DOMElement* pcTemp = FindElement(_pGroupNode, "auracadText");
    while (pcTemp) {
        ...
        pcTemp = FindNextElement(pcTemp, "auracadText");
    }
...
void ParameterGrp::RemoveASCII(const char* Name)
{
    ...
    // trigger observer
    _Notify(ParamType::auracadText, Name, nullptr);
    Notify(Name);
}
...
void ParameterGrp::RemoveBool(const char* Name)
{
    ...
    // trigger observer
    _Notify(ParamType::auracadBool, Name, nullptr);
    Notify(Name);
}
...
void ParameterGrp::RemoveFloat(const char* Name)
{
    ...
    // trigger observer
    _Notify(ParamType::auracadFloat, Name, nullptr);
    Notify(Name);
}
...
void ParameterGrp::RemoveInt(const char* Name)
{
    ...
    // trigger observer
    _Notify(ParamType::auracadInt, Name, nullptr);
    Notify(Name);
}
...
void ParameterGrp::RemoveUnsigned(const char* Name)
{
    ...
    // trigger observer
    _Notify(ParamType::auracadUInt, Name, nullptr);
    Notify(Name);
}
...
bool ParameterGrp::RenameGrp(const char* OldName, const char* NewName)
{
    ...
    // check if Element in group
    DOMElement* pcElem = FindElement(_pGroupNode, "auracadParamGroup", OldName);
    if (pcElem) {
        pcElem->setAttribute(XStrLiteral("Name").unicodeForm(), XStr(NewName).unicodeForm());
    }

    _Notify(ParamType::auracadGroup, NewName, OldName);
    return true;
}
...
void ParameterGrp::Clear(bool notify)
{
    ...
    _Notify(ParamType::auracadGroup, nullptr, nullptr);
    ...
    for (DOMNode *child = _pGroupNode->getFirstChild(), *next = child; child != nullptr;
        ...
        if (type != ParamType::auracadInvalid && type != ParamType::auracadGroup) {
            params.emplace_back(
                type,
...
#if defined(AuraCAD_OS_LINUX) || defined(AuraCAD_OS_CYGWIN)
...
int ParameterManager::LoadDocument(const XERCES_CPP_NAMESPACE::InputSource& inputSource)
{
    ...
    _pGroupNode = FindElement(rootElem, "auracadParamGroup", "Root");
...
void ParameterManager::CreateDocument()
{
    ...
    _pDocument = impl->createDocument(
        ...
        XStrLiteral("auracadParameters").unicodeForm(),  // root element name
        nullptr
    );  // document type object (DTD).

    // creating the node for the root group
    DOMElement* rootElem = _pDocument->getDocumentElement();
    _pGroupNode = _pDocument->createElement(XStrLiteral("auracadParamGroup").unicodeForm());
...
{
    if (XMLString::compareString(node->getNodeName(), XStrLiteral("auracadParameters").unicodeForm())
        == 0) {
        // This node is supposed to have a single auracadParamGroup and two text nodes.
    ...
    switch (node->getNodeType()) {
        case DOMNode::TEXT_NODE: {
            ...
            if (parent && XMLString::compareString(parent->getNodeName(),
                                                   XStrLiteral("auracadParamGroup").unicodeForm()) == 0) {
