...
T AuraCAD_cast(Base::BaseClass* type)
{
...
template<typename T, typename U = std::remove_pointer_t<T>>
    requires(std::is_pointer_v<T>)
const U* AuraCAD_cast(const Base::BaseClass* type)
{
    static_assert(std::is_base_of_v<Base::BaseClass, U>, "T must be derived from Base::BaseClass");
...
// We define global alias for AuraCAD_cast to be used by all AuraCAD files that include
// BaseClass.h. While doing using on header level is non-ideal it allows for much easier use
// of the important AuraCAD_cast. In that case the name is prefixed with AuraCAD so there is no
// chance of symbols collision.
using Base::AuraCAD_cast;
