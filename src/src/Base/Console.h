 ...
 * \section Motivation
 *
 * AuraCAD Base::Console() is capable of outputting to different targets, and has
 * some basic enable/disable control of different types of logs. There is,
 * however, no easy way to use logging facility for various auracad modules.  This set
 ...
 *
 * \code{.c}
 * #define AuraCAD_LOGLEVEL_ERR 0
 * #define AuraCAD_LOGLEVEL_WARN 1
 * #define AuraCAD_LOGLEVEL_MSG 2
 * #define AuraCAD_LOGLEVEL_LOG 3
 * #define AuraCAD_LOGLEVEL_TRACE 4
 * \endcode
 *
 ...
 *
 * \code{.c}
 * #define AuraCAD_LOGLEVEL_DEFAULT -1
 * \endcode
 *
 ...
 *
 * \code{.py}
 * AuraCAD.setLogLevel('Default',level)
 * AuraCAD.setLogLevel('DebugDefault',level)
 * \endcode
 *
 ...
 * level is \c AuraCAD_LOGLEVEL_MSG, and on debug build, \c AuraCAD_LOGLEVEL_LOG.
 *
 * Python code can call \c AuraCAD.setLogLevel(tag,level) to configure any other
 * tag log levels, and \c AuraCAD.getLogLevel(tag), which outputs only integer
 * log level.
 *
 * You can fine tune how the log is output by passing extra parameters to
 * #AuraCAD_LOG_LEVEL_INIT(). All the extra parameters are boolean value, which are
 * shown blew along with their default values.
 *
 * \code{.c}
 * AuraCAD_LOG_LEVEL_INIT(tag, print_tag=true, print_src=0,
 ...
 *
 * \code{.c}
 * AuraCAD_LOG_INSTANCE.refresh = true; // print time for each log, default false.
 *
 * // print file and line number, default 0, if set to 2 then print python
 * // source from current call frame.
 * AuraCAD_LOG_INSTANCE.print_src = 1;
 *
 * AuraCAD_LOG_INSTANCE.print_tag = false; // do not print tag, default true
 * AuraCAD_LOG_INSTANCE.add_eol = false; // do not add eol
 * AuraCAD_LOG_INSTANCE.refresh = true; // refresh GUI after each log
 * \endcode
 *
 ...
 * and \c AuraCAD_TRACE uses Base::Console().Log(), same as \c AuraCAD_LOG. These macros
 * checks the log level defined in \c AuraCAD_LOG_LEVEL_INIT to decide whether to
 * print log or not. \c msg here shall be a C++ streaming expression. End of
 * line will be automatically appended by default.
 *
 * \code
 * AuraCAD_ERR("error: " << code << ". exiting")
 * \endcode
 *
 ...
 * AuraCAD.setLogLevel/getLogLevel. Any change to the log level is reflected
 * through the pointer returned by Base::Console().GetLogLevel(). What
 * \c AuraCAD_LOG_LEVEL_INIT(tag) does is to define a class Base::LogLevel, and then
 ...
 *
 * \code
 * _AuraCAD_LOG_LEVEL_INIT(instance_name,tag)
 * \endcode
 *
 * Then, define a second set of logging macros as
 *
 * \code{.c}
 * #define MY_MSG(_msg) _AuraCAD_PRINT(instance_name,AuraCAD_LOGLEVEL_MSG,Message,_msg)
 * #define MY_WARN(_msg) _AuraCAD_PRINT(instance_name,AuraCAD_LOGLEVEL_WARN,Warning,_msg)
 * #define MY_ERR(_msg) _AuraCAD_PRINT(instance_name,AuraCAD_LOGLEVEL_ERR,Error,_msg)
 * #define MY_LOG(_msg) _AuraCAD_PRINT(instance_name,AuraCAD_LOGLEVEL_LOG,Log,_msg)
 * #define MY_TRACE(_msg) _AuraCAD_PRINT(instance_name,AuraCAD_LOGLEVEL_TRACE,Log,_msg)
 * \endcode
 *
 * Note, replace \c instance_name with your actual desired name.
 *
 * You can also define your own log levels the same way. Macro
 * #_AuraCAD_PRINT(_instance,_l,_func,_msg) checks to see if the log shall proceed,
 * where \c _instance is the static loglevel instance name (default is
 * \c AuraCAD_LOG_INSTANCE), and \c _l is the log level constant to be checked,
...
#define _AuraCAD_LOG_LEVEL_INIT(_name, _tag, ...) static Base::LogLevel _name(_tag, ##__VA_ARGS__);

#ifndef AuraCAD_LOG_INSTANCE
#define AuraCAD_LOG_INSTANCE _s_auracadlvl
#endif

#define AuraCAD_LOG_LEVEL_INIT(_tag, ...) _AuraCAD_LOG_LEVEL_INIT(AuraCAD_LOG_INSTANCE, _tag, ##__VA_ARGS__)

#define __AuraCAD_PRINT(_instance, _l, _func, _notifier, _msg, _file, _line)                            \
    do {                                                                                           \
...
#define _AuraCAD_PRINT(_instance, _l, _func, _msg)                                                      \
    __AuraCAD_PRINT(_instance, _l, _func, std::string(), _msg, __FILE__, __LINE__)

#define AuraCAD_MSG(_msg) _AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_MSG, message, _msg)
#define AuraCAD_WARN(_msg) _AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_WARN, developerWarning, _msg)
#define AuraCAD_ERR(_msg) _AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_ERR, developerError, _msg)
#define AuraCAD_LOG(_msg) _AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_LOG, log, _msg)
#define AuraCAD_TRACE(_msg) _AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_TRACE, log, _msg)

#define _AuraCAD_MSG(_file, _line, _msg)                                                                \
    __AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_MSG, message, std::string(), _msg, _file, _line)
#define _AuraCAD_WARN(_file, _line, _msg)                                                               \
    __AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_WARN, developerWarning, std::string(), _msg, _file, _line)
#define _AuraCAD_ERR(_file, _line, _msg)                                                                \
    __AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_ERR, developerError, std::string(), _msg, _file, _line)
#define _AuraCAD_LOG(_file, _line, _msg)                                                                \
    __AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_LOG, log, std::string(), _msg, _file, _line)
#define _AuraCAD_TRACE(_file, _line, _msg)                                                              \
    __AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, AuraCAD_LOGLEVEL_TRACE, log, std::string(), _msg, _file, _line)

#define AuraCAD_XYZ(_pt) '(' << (_pt).X() << ", " << (_pt).Y() << ", " << (_pt).Z() << ')'
#define AuraCAD_xy(_pt) '(' << (_pt).x << ", " << (_pt).y << ')'
#define AuraCAD_xyz(_pt) '(' << (_pt).x << ", " << (_pt).y << ", " << (_pt).z << ')'
 ...
 *  This class describes an Interface for logging within AuraCAD. If you want to add a new
 *  "sink" to AuraCAD's logging mechanism, then inherit this class. You'll also need to
 ...
 *  distributed with the auracadConsoleObserver. The auracadConsole class itself makes no IO, it's more like
 *  a manager.
 *  \par
...
public:
    ...
    */
    ...
    /// Attaches an Observer to auracadConsole
    void attachObserver(ILogger* pcObserver);
    /// Detaches an Observer from auracadConsole
    ...
    using PostEventHandler = std::function<
        void(AuraCAD_ConsoleMsgType, IntendedRecipient, ContentType, const std::string&, const std::string&)>;
    ...
    bool isMsgTypeEnabled(const char* sObs, AuraCAD_ConsoleMsgType type) const;
    ...
    constexpr AuraCAD_ConsoleMsgType getConsoleMsg(LogStyle style);
...
constexpr ConsoleSingleton::AuraCAD_ConsoleMsgType ConsoleSingleton::getConsoleMsg(LogStyle style)
{
 ...
 *  AuraCAD is running in GUI mode a Message appears on the status bar.
