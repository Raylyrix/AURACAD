...
# include <windows.h>
#elif defined(AuraCAD_OS_LINUX) || defined(AuraCAD_OS_MACOSX)
# include <unistd.h>
#endif
#include <cstring>
#include <functional>
...
#ifdef AuraCAD_DEBUG
    : _defaultLogLevel(AuraCAD_LOGLEVEL_LOG)
#else
    : _defaultLogLevel(AuraCAD_LOGLEVEL_MSG)
#endif
{}
...
/**
 * \a type can be OR'ed with any of the AuraCAD_ConsoleMsgType flags to enable -- if \a b is true --
...
bool ConsoleSingleton::isMsgTypeEnabled(const char* sObs, const AuraCAD_ConsoleMsgType type) const
{
...
void ConsoleSingleton::postEvent(
    const AuraCAD_ConsoleMsgType type,
    const IntendedRecipient recipient,
    const ContentType content,
    const std::string& notifiername,
    const std::string& msg
...
namespace
{
PyObject* AuraCAD_PYCONSOLE_MSG(std::function<void(const char*, const char*)> func, PyObject* args)
{
    PyObject* output {};
    PyObject* notifier {};
...
PyObject* ConsoleSingleton::sPySetStatus(PyObject* /*self*/, PyObject* args)
{
    ...
    PY_TRY
    {
        ...
        Py_Error(Base::PyExc_AuraCAD_GeneralError, "Unknown logger type");
    }
    PY_CATCH;
}
