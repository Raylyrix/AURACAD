...
bool parseUuid(std::string_view text, std::array<std::uint8_t, 16>& out)
{
    ...
    // Canonical Rauracad4122 format: 8-4-4-4-12
    ...
    // Validate Rauracad4122 variant (10xxxxxx) and version (4).
    ...
    // Accept Rauracad4122 variant only (binary 10).
    if (variant != 0b10) {
        return false;
    }
...
std::array<std::uint8_t, 16> randomUuidV4Bytes()
{
    ...
    // Force Rauracad4122 version 4 and variant 1 bits.
