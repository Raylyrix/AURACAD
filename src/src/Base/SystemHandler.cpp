 ...
 */
#ifdef _MSC_VER  // New handler for Microsoft Visual C++ compiler
int __cdecl AuraCADNewHandler(size_t size)
{
    // throw an exception
...
#if defined(AuraCAD_SE_TRANSLATOR)  // Microsoft compiler
...
void SystemHandler::installNewHandler()
{
    ...
    _set_new_handler(AuraCADNewHandler);  // Setup new handler
    _set_new_mode(1);                     // Re-route malloc failures to new handler !
#else                                     // Ansi compiler
    std::set_new_handler(AuraCADNewHandler);  // ANSI new handler
#endif
}
...
#elif defined(AuraCAD_OS_LINUX)
    std::signal(SIGSEGV, segmentation_fault_handler);
#endif
#if defined(AuraCAD_SE_TRANSLATOR)
