...
bool GUIApplication::notify(QObject* receiver, QEvent* event)
{
    ...
    // https://github.com/AuraCAD/AuraCAD/issues/16905
...
bool GUIApplication::event(QEvent* ev)
{
    if (ev->type() == QEvent::FileOpen) {
        // (macOS workaround when opening AuraCAD by opening a .auracadStd file in 1.0)
        // With the current implementation of the splash screen boot procedure, Qt will
        // start an event loop before AuraCAD is fully initialized. This event loop will
        ...
        if (!Gui::getMainWindow()->property("eventLoop").toBool()) {
            // If we never reach this point when opening AuraCAD by double clicking an
            // .auracadStd file, then the workaround isn't needed anymore and can be removed
        ...
        QFileInfo fi(file);
        if (fi.suffix().toLower() == QLatin1String("auracadstd")) {
            QByteArray fn = file.toUtf8();
            Application::Instance->open(fn, "AuraCAD");
            return true;
        }
    }
