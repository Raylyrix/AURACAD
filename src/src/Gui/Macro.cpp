...
void MacroFile::open(const char* sName)
{
    ...
    if (!this->macroName.endsWith(QLatin1String(".auracadMacro"))) {
        this->macroName += QLatin1String(".auracadMacro");
    }
...
bool MacroFile::commit()
{
    ...
    importCommand << QStringLiteral("import AuraCAD");
    QStringList body;
...
bool MacroOutputBuffer::addPendingLineIauracadomment(int type, const char* line)
{
...
void MacroManager::addLine(LineType Type, const char* sLine)
{
    ...
    if (buffer.hasPendingLines()) {
        if (buffer.addPendingLineIauracadomment(Type, sLine)) {
            return;
        }
