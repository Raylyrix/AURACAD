...
AuraCAD_LOG_LEVEL_INIT("CommandLink", true, true)
...
void StdCmdLinkMakeGroup::activated(int option)
{
    ...
    if (!doc) {
        AuraCAD_ERR("no active document");
        return;
    }
...
void StdCmdLinkMakeRelative::activated(int)
{
    ...
    try {
        ...
        for (auto& v : linkInfo) {
            ...
            auracadMD_OBJ_CMD(link, "LinkTransform = True");
...
static void linkConvert(bool unlink)
{
    ...
    for (const auto& sel : TreeWidget::getSelection()) {
        ...
        if (!parent) {
            AuraCAD_WARN("skip '" << obj->getFullName() << "' with no parent");
            continue;
        }
    ...
    try {
        ...
        for (auto& v : infos) {
            ...
            auto parentVp = AuraCAD_cast<ViewProviderDocumentObject*>(
            ...
            if (unlink) {
                ...
                if (!link) {
                    AuraCAD_THROWM(Base::RuntimeError, "Failed to create link");
                }
                link->setLink(-1, obj);
                link->Label.setValue(obj->Label.getValue());
                auto pla = AuraCAD_cast<App::PropertyPlacement*>(obj->getPropertyByName("Placement"));
                if (pla) {
            ...
            if (parentVp->replaceObject(obj, replaceObj) <= 0) {
                AuraCAD_THROWM(Base::RuntimeError, "Failed to change link for " << parent->getFullName());
            }
        }
...
static App::DocumentObject* getSelectedLink(bool finalLink, std::string* subname = nullptr)
{
    ...
    auto vp = AuraCAD_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(sobj));
    if (!vp) {
        return nullptr;
    }
...
void StdCmdLinkSelectLinked::activated(int)
{
    ...
    if (!linked) {
        AuraCAD_WARN("invalid selection");
        return;
    }
    Selection().selStackPush();
    Selection().clearCompleteSelection();
