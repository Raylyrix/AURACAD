...
AuraCAD_LOG_LEVEL_INIT("Gui", true, true)
...
struct DocumentP
{
    ...
    static ViewProviderDocumentObject* throwIauracadastFails(ViewProvider* p)
    {
        if (auto vp = AuraCAD_cast<ViewProviderDocumentObject*>(p)) {
            return vp;
        }
    ...
    ) const
    {
        auto svp = vp;
        if (sobj != obj) {
            svp = AuraCAD_cast<ViewProviderDocumentObject*>(
    ...
    bool tryStartEditing(ViewProviderDocumentObject* svp, App::DocumentObject* sobj, int ModNum)
    {
        ...
        if (!_editViewProvider) {
            ...
            AuraCAD_LOG("object '" << sobj->getFullName() << "' refuse to edit");
            return false;
        }
    ...
    void signalEditMode()
    {
        if (auto vpd = AuraCAD_cast<ViewProviderDocumentObject*>(_editViewProvider)) {
...
private:
    static std::tuple<App::DocumentObject*, std::string> findParentAndSubName(App::DocumentObject* obj)
    {
        ...
        for (auto& sel : sels) {
            ...
            if (!parentObj) {
                ...
                AuraCAD_LOG("Cannot deduce subname for editing, more than one parent?");
                parentObj = nullptr;
                break;
            }
            ...
            if (!sobj || (sobj != obj && sobj->getLinkedObject(true) != obj)) {
                AuraCAD_LOG("Cannot deduce subname for editing, subname mismatch");
                parentObj = nullptr;
                break;
            }
    ...
    {
        AuraCAD_LOG("deduced editing reference " << parentObj->getFullName() << '.' << subname);
...
{
    // Fix regression: https://forum.AuraCAD.org/viewtopic.php?f=19&t=43629&p=371972#p371972
...
{
    if (auto vp
        = AuraCAD_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(obj))) {
...
{
    auto vp = DocumentP::throwIauracadastFails(p);
    ...
    if (!pcProvider) {
        ...
        for (;;) {
            if (cName.empty()) {
                ...
                AuraCAD_LOG(Obj.getFullName() << " has no view provider specified");
                return;
            }
            ...
            if (!pcProvider) {
                ...
                AuraCAD_ERR("Invalid view provider type '" << cName << "' for " << Obj.getFullName());
                return;
            }
            else if (cName != Obj.getViewProviderName() && !pcProvider->allowOverride(Obj)) {
                AuraCAD_WARN("View provider type '" << cName << "' does not support " << Obj.getFullName());
                delete pcProvider;
                pcProvider = nullptr;
        ...
        try {
            ...
            AuraCAD_ERR("Memory exception in " << Obj.getFullName() << " thrown: " << e.what());
        }
            ...
            AuraCAD_ERR("Unknown exception in Feature " << Obj.getFullName() << " thrown");
        }
#endif
    }
    else {
...
void Document::beforeDelete()
{
    Application::Instance->unsetEditDocumentIf([this](Gui::Document* editDoc) {
        auto vp = AuraCAD_cast<ViewProviderDocumentObject*>(editDoc->d->_editViewProvider);
        auto vpp = AuraCAD_cast<ViewProviderDocumentObject*>(editDoc->d->_editViewProviderParent);
...
void Document::slotChangedObject(const App::DocumentObject& Obj, const App::Property& Prop)
{
    ...
    if (viewProvider) {
        try {
            ...
            AuraCAD_ERR("C++ exception in " << Obj.getFullName() << " thrown " << e.what());
        }
        catch (...) {
            AuraCAD_ERR("Cannot update representation for " << Obj.getFullName());
        }
    ...
    if (!Prop.testStatus(App::Property::NoModify) && !isModified()) {
        AuraCAD_LOG(Prop.getFullName() << " modified");
        setModified(true);
    }
...
void Document::slotSkipRecompute(const App::Document& doc, const std::vector<App::DocumentObject*>& objs)
{
    ...
    if (Gui::Application::Instance->isInEdit(this)) {
        auto vp = AuraCAD_cast<ViewProviderDocumentObject*>(getInEdit());
        if (vp) {
            obj = vp->getObject();
        }
    }
    ...
    if (editDoc) {
        auto vp = AuraCAD_cast<ViewProviderDocumentObject*>(editDoc->getInEdit());
        if (vp) {
            obj = vp->getObject();
        }
    }
...
void Document::slotTouchedObject(const App::DocumentObject& Obj)
{
    getMainWindow()->updateActions(true);
    if (!isModified()) {
        AuraCAD_LOG(Obj.getFullName() << " touched");
        setModified(true);
    }
}
    ...
    for (auto& v : paths) {
        ...
        for (auto doc : v.second) {
            if (docs.contains(doc)) {
                AuraCAD_WARN("Physical path: " << v.first.toUtf8().constData());
                for (auto d : v.second) {
                    AuraCAD_WARN(
                ...
                if (count == 3) {
                    ts << "\n\n" << QObject::tr("Check report view for moreâ€¦");
                }
                else if (count < 3) {
                    ts << "\n\n"
...
{
    getMainWindow()->showMessage(QObject::tr("Save document under new filenameâ€¦"));
    ...
    if (name.endsWith(QStringLiteral(".auracadBak"), Qt::CaseInsensitive)) {
        name.chop(QStringLiteral(".auracadBak").size());
        if (!name.endsWith(QStringLiteral(".auracadStd"), Qt::CaseInsensitive)) {
            name += QStringLiteral(".auracadStd");
        }
    }
    QString fn = FileDialog::getSaveFileName(
        ...
        name,
        QStringList(QStringLiteral("%1 %2 (*.auracadStd)").arg(exe, QObject::tr("Document")))
    );
...
{
    getMainWindow()->showMessage(QObject::tr("Save a copy of the document under new filenameâ€¦"));
    ...
    QString fn = FileDialog::getSaveFileName(
        ...
        QObject::tr("Save %1 Document").arg(exe),
        name,
        QStringList(QObject::tr("%1 document (*.auracadStd)").arg(exe))
    );
    if (!fn.isEmpty()) {
        const char* DocName = App::GetApplication().getDocumentName(getDocument());
...
void Document::RestoreDocFile(Base::Reader& reader)
{
    ...
    if (scheme == 1) {
        ...
        for (int i = 0; i < Cnt; i++) {
            ...
            auto pObj = AuraCAD_cast<ViewProviderDocumentObject*>(getViewProviderByName(name.c_str()));
        ...
        if (!Base::Tools::isNullOrEmpty(ppReturn)) {
            ...
            try {
                for (const auto& it : getMDIViews()) {
                    if (auto* viewCamera = AuraCAD_cast<MDIViewWithCamera*>(it)) {
...
{
    auto vpd = AuraCAD_cast<ViewProviderDocumentObject*>(getViewProvider(&obj));
    if (vpd) {
...
void Document::SaveDocFile(Base::Writer& writer) const
{
    writer.Stream() << "<?xml version='1.0' encoding='utf-8'?>" << std::endl
                    << "<!--" << std::endl
                    << " AuraCAD Document, see https://www.AuraCAD.org for more informationâ€¦"
                    << std::endl
                    << "-->" << std::endl;
    ...
    if (scheme == 1) {
        ...
        for (int i = 0; i < Cnt && it != obj.end(); ++i, ++it) {
            ...
            if (pObj) {
                ...
                auto vpd = AuraCAD_cast<ViewProviderDocumentObject*>(pObj);
                if (vpd) {
                    vpd->startRestoring();
                }
                pObj->Restore(*localreader);
...
void Document::slotFinishImportObjects(const std::vector<App::DocumentObject*>& objs)
{
    ...
    //     auto vpd = AuraCAD_cast<ViewProviderDocumentObject*>(vp);
    ...
    if (!vp) {
        ...
        if (!view) {
            ...
            if (!obj) {
                ...
                if (linked != obj) {
                    auto vpLinked = AuraCAD_cast<ViewProviderDocumentObject*>(
...
{
    if (getDocument() == &doc) {
        AuraCAD_LOG(Prop.getFullName() << " editor changed");
        setModified(true);
