 ...
 */
class GuiExport SoauracadUnifiedSelection: public SoSeparator
{
    using inherited = SoSeparator;

    SO_NODE_HEADER(Gui::SoauracadUnifiedSelection);

public:
    static void initClass();
    static void finish();
    SoauracadUnifiedSelection();
    void applySettings();
...
protected:
    ~SoauracadUnifiedSelection() override;
...
class GuiExport SoauracadPathAnnotation: public SoSeparator
{
    using inherited = SoSeparator;

    SO_NODE_HEADER(Gui::SoauracadPathAnnotation);

public:
    static void initClass();
    static void finish();
    SoauracadPathAnnotation();
...
protected:
    ~SoauracadPathAnnotation() override;
...
class GuiExport SoauracadSeparator: public SoSeparator
{
    using inherited = SoSeparator;

    SO_NODE_HEADER(Gui::SoauracadSeparator);

public:
    static void initClass();
    static void finish();
    explicit SoauracadSeparator(bool trackCacheMode = true);
...
class GuiExport SoauracadSelectionRoot: public SoauracadSeparator
{
    using inherited = SoauracadSeparator;

    SO_NODE_HEADER(Gui::SoauracadSelectionRoot);

public:
    static void initClass();
    static void finish();
    explicit SoauracadSelectionRoot(bool trackCacheMode = false);
     ...
     * always stored in the first encountered SoauracadSelectionRoot in the path. It
     * is keyed using the entire sequence of SoauracadSelectionRoot along the path
     * to \c node, replacing the first SoauracadSelectionRoot with the given node.
     *
     ...
     * last encountered SoauracadSelectionRoot, and is keyed using the querying
     * \c node and (if there are more than one SoauracadSelectionRoot along the
     * path) the first SoauracadSelectionRoot. The reason is so that any link to a
     * node (new links means additional SoauracadSelectionRoot added in front) with
     ...
     * See SoauracadSelectionContext::merge() for an implementation of merging multiple
     * context.
     *
     * @note For simplicity reason, currently secondary context is only freed
     * when the storage SoauracadSSelectionRoot node is freed.
     */
    template<class T>
    static std::shared_ptr<T> getRenderContext(
        SoNode* node,
     ...
     *
     * @return If no SoauracadSelectionRoot is found in the current path of action,
...
protected:
    ~SoauracadSelectionRoot() override;
    ...
    static SoauracadSelectionContextBasePtr getNodeContext(
        Stack& stack,
        SoNode* node,
        SoauracadSelectionContextBasePtr def
    );
    static SoauracadSelectionContextBasePtr getNodeContext2(
        Stack& stack,
        SoNode* node,
        SoauracadSelectionContextBase::MergeFunc* merge
    );
    static std::pair<bool, SoauracadSelectionContextBasePtr*> findActionContext(
        SoAction* action,
        SoNode* node,
        bool create,
        bool erase
    ...
    using ContextMap = std::map<Stack, SoauracadSelectionContextBasePtr, StackComp>;
    ContextMap contextMap;
    ContextMap contextMap2;  // holding secondary context

    struct SelContext: SoauracadSelectionContextBase
    {
    public:
        SbColor selColor;
        SbColor hlColor;
    ...
    static SoauracadSelectionRoot* ShapeColorNode;
