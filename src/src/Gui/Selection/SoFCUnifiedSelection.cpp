...
#include "SoauracadInteractiveElement.h"
...
AuraCAD_LOG_LEVEL_INIT("SoauracadUnifiedSelection", false, true, true)
...
SoFullPath* Gui::SoauracadUnifiedSelection::currentHighlightPath = nullptr;

// *************************************************************************

SO_NODE_SOURCE(SoauracadUnifiedSelection)

/*!
  Constructor.
*/
SoauracadUnifiedSelection::SoauracadUnifiedSelection()
{
    SO_NODE_CONSTRUCTOR(SoauracadUnifiedSelection);
...
// doc from parent
void SoauracadUnifiedSelection::initClass()
{
    SO_NODE_INIT_CLASS(SoauracadUnifiedSelection, SoSeparator, "Separator");
}

void SoauracadUnifiedSelection::finish()
{
    atexit_cleanup();
}

bool SoauracadUnifiedSelection::hasHighlight()
{
    return currentHighlightPath != nullptr;
}

void SoauracadUnifiedSelection::applySettings()
{
    float transparency;
    ParameterGrp::handle hGrp = Gui::WindowParameter::getDefaultParameter()->GetGroup("View");
    bool enablePreselection = hGrp->GetBool("EnablePreselection", true);
    if (!enablePreselection) {
        this->preselectionMode = SoauracadUnifiedSelection::OFF;
    }
    else {
    ...
    if (!enableSelection) {
        this->selectionMode = SoauracadUnifiedSelection::OFF;
    }
    else {
...
const char* SoauracadUnifiedSelection::getFileFormatName() const
{
    return "Separator";
}

void SoauracadUnifiedSelection::write(SoWriteAction* action)
{
...
int SoauracadUnifiedSelection::getPriority(const SoPickedPoint* p)
{
...
std::vector<SoauracadUnifiedSelection::PickedInfo> SoauracadUnifiedSelection::getPickedList(
...
void SoauracadUnifiedSelection::doAction(SoAction* action)
{
    if (action->getTypeId() == SoauracadEnablePreselectionAction::getClassTypeId()) {
        auto enablePreselectionAction = static_cast<SoauracadEnablePreselectionAction*>(action);
        if (enablePreselectionAction->enabled) {
            this->preselectionMode = SoauracadUnifiedSelection::AUTO;
        }
        else {
            this->preselectionMode = SoauracadUnifiedSelection::OFF;
        }
    ...
    if (action->getTypeId() == SoauracadEnableSelectionAction::getClassTypeId()) {
        auto enableSelectionAction = static_cast<SoauracadEnableSelectionAction*>(action);
        if (enableSelectionAction->enabled) {
            this->selectionMode = SoauracadUnifiedSelection::ON;
        }
        else {
            this->selectionMode = SoauracadUnifiedSelection::OFF;
        }
    ...
    if (action->getTypeId() == SoauracadSelectionColorAction::getClassTypeId()) {
        auto selectionColorAction = static_cast<SoauracadSelectionColorAction*>(action);
        this->colorSelection = selectionColorAction->selectionColor;
    }

    if (action->getTypeId() == SoauracadHighlightColorAction::getClassTypeId()) {
        auto highlightColorAction = static_cast<SoauracadHighlightColorAction*>(action);
        this->colorHighlight = highlightColorAction->highlightColor;
    }

    if (action->getTypeId() == SoauracadPreselectionAction::getClassTypeId()) {
        auto preselectAction = static_cast<SoauracadPreselectionAction*>(action);
    ...
    if (action->getTypeId() == SoauracadSelectionAction::getClassTypeId()) {
        auto selectionAction = static_cast<SoauracadSelectionAction*>(action);
...
bool SoauracadUnifiedSelection::setPreselect(const PickedInfo& info)
{
    if (!info.pp) {
...
bool SoauracadUnifiedSelection::setPreselect(
    SoFullPath* path,
    const SoDetail* det,
...
bool SoauracadUnifiedSelection::setSelection(const std::vector<PickedInfo>& infos, bool ctrlDown)
{
    ...
    if (ctrlDown) {
        if (Gui::Selection().isSelected(docname, objname, info.element.c_str(), ResolveMode::NoResolve)) {
            ...
            if (vpd->getDetailPath(info.element.c_str(), detailPath, true, detNext)
                ...
                pPath = detailPath;
                det = detNext;
                AuraCAD_TRACE("select next " << objectName << ", " << subName);
                if (ok) {
        ...
        AuraCAD_TRACE("clearing selection");
        Gui::Selection().clearSelection();
        AuraCAD_TRACE("add selection");
    ...
    if (pPath) {
        AuraCAD_TRACE("applying action");
        SoSelectionElementAction action(type);
        action.setColor(this->colorSelection.getValue());
        action.setElement(det);
        action.apply(pPath);
        AuraCAD_TRACE("applied action");
        this->touch();
    }
...
// doc from parent
void SoauracadUnifiedSelection::handleEvent(SoHandleEventAction* action)
{
    ...
    // mouse press events for (de)selection
        ...
        && selectionMode.getValue() == SoauracadUnifiedSelection::ON
    ) {
...
void SoauracadUnifiedSelection::GLRenderBelowPath(SoGLRenderAction* action)
{
...
bool SoauracadSelectionRoot::StackComp::operator()(const Stack& a, const Stack& b) const
{
...
// ---------------------------------------------------------------------------------
SoSeparator::CacheEnabled SoauracadSeparator::CacheMode = SoSeparator::AUTO;
SO_NODE_SOURCE(SoauracadSeparator)

SoauracadSeparator::SoauracadSeparator(bool trackCacheMode)
    : trackCacheMode(trackCacheMode)
{
    SO_NODE_CONSTRUCTOR(SoauracadSeparator);
    if (!trackCacheMode) {
...
void SoauracadSeparator::GLRenderBelowPath(SoGLRenderAction* action)
{
...
void SoauracadSeparator::initClass()
{
    SO_NODE_INIT_CLASS(SoauracadSeparator, SoSeparator, "auracadSeparator");
}

void SoauracadSeparator::finish()
{
    atexit_cleanup();
}
...
using SoauracadBBoxRenderInfo = struct
{
...
{
    auto data = static_cast<SoauracadBBoxRenderInfo*>(closure);
...
SoauracadSelectionRoot::Stack SoauracadSelectionRoot::SelStack;
std::unordered_map<SoAction*, SoauracadSelectionRoot::Stack> SoauracadSelectionRoot::ActionStacks;
SoauracadSelectionRoot::ColorStack SoauracadSelectionRoot::SelColorStack;
SoauracadSelectionRoot::ColorStack SoauracadSelectionRoot::HlColorStack;
SoauracadSelectionRoot* SoauracadSelectionRoot::ShapeColorNode;

SO_NODE_SOURCE(SoauracadSelectionRoot)

SoauracadSelectionRoot::SoauracadSelectionRoot(bool trackCacheMode)
    : SoauracadSeparator(trackCacheMode)
{
    SO_NODE_CONSTRUCTOR(SoauracadSelectionRoot);
...
SoauracadSelectionRoot::~SoauracadSelectionRoot() = default;

void SoauracadSelectionRoot::initClass()
{
    SO_NODE_INIT_CLASS(SoauracadSelectionRoot, SoauracadSeparator, "auracadSelectionRoot");

    so_bbox_storage
        = new SbStorage(sizeof(SoauracadBBoxRenderInfo), so_bbox_construct_data, so_bbox_destruct_data);

    // cc_coin_atexit((coin_atexit_f*) so_bbox_cleanup);
}

void SoauracadSelectionRoot::finish()
{
    so_bbox_cleanup();
    atexit_cleanup();
}

SoNode* SoauracadSelectionRoot::getCurrentRoot(bool front, SoNode* def)
{
    if (!SelStack.empty()) {
...
SoauracadSelectionContextBasePtr SoauracadSelectionRoot::getNodeContext(
    Stack& stack,
    SoNode* node,
    SoauracadSelectionContextBasePtr def
)
{
    if (stack.empty()) {
        return def;
    ...
    auto front = dynamic_cast<SoauracadSelectionRoot*>(stack.front());
    if (!front) {
        return SoauracadSelectionContextBasePtr();
    }
...
SoauracadSelectionContextBasePtr SoauracadSelectionRoot::getNodeContext2(
    Stack& stack,
    SoNode* node,
    SoauracadSelectionContextBase::MergeFunc* merge
)
{
    SoauracadSelectionContextBasePtr ret;
    if (stack.empty()) {
        return ret;
    }

    auto* back = dynamic_cast<SoauracadSelectionRoot*>(stack.back());
    ...
    for (stack.offset = 0; stack.offset < stack.size(); ++stack.offset) {
        ...
        SoauracadSelectionContextBasePtr ctx;
        if (it != map.end()) {
            ctx = it->second;
        }
        status
...
std::pair<bool, SoauracadSelectionContextBasePtr*> SoauracadSelectionRoot::findActionContext(
    SoAction* action,
    SoNode* _node,
    bool create,
    bool erase
...
{
    std::pair<bool, SoauracadSelectionContextBasePtr*> res(false, 0);
...
bool SoauracadSelectionRoot::renderBBox(SoGLRenderAction* action, SoNode* node, SbColor color)
{
    auto data = static_cast<SoauracadBBoxRenderInfo*>(so_bbox_storage->get());
    if (!data->bboxaction) {
...
void SoauracadSelectionRoot::renderPrivate(SoGLRenderAction* action, bool inPath)
{
    if (ViewParams::instance()->getCoinCycleCheck() && !SelStack.nodeSet.insert(this).second) {
        ...
        if (_CyclicLastReported < t) {
            ...
            AuraCAD_ERR("Cyclic scene graph: " << getName());
        }
        return;
    }
...
bool SoauracadSelectionRoot::_renderPrivate(SoGLRenderAction* action, bool inPath)
{
    ...
    if ((style == SoauracadSelectionRoot::Box || ViewParams::instance()->getShowSelectionBoundingBox())
        && ctx && !ctx->hideAll && (ctx->selAll || ctx->hlAll)) {
        if (style == SoauracadSelectionRoot::PassThrough) {
            style = SoauracadSelectionRoot::Box;
        }
        else {
    ...
    if (!ShapeColorNode && overrideColor && !SoOverrideElement::getDiffuseColorOverride(state)
        && (style == SoauracadSelectionRoot::Box || !ctx || (!ctx->selAll && !ctx->hideAll))) {
        ShapeColorNode = this;
        colorPushed = true;
        state->push();
    ...
    if (!ctx) {
        ...
        if ((selPushed = ctx->selAll)) {
            ...
            if (style != SoauracadSelectionRoot::Box) {
                state->push();
...
void SoauracadSelectionRoot::GLRenderBelowPath(SoGLRenderAction* action)
{
    renderPrivate(action, false);
}

void SoauracadSelectionRoot::GLRenderInPath(SoGLRenderAction* action)
{
...
bool SoauracadSelectionRoot::checkColorOverride(SoState* state)
{
    if (ShapeColorNode) {
...
void SoauracadSelectionRoot::checkSelection(bool& sel, SbColor& selColor, bool& hl, SbColor& hlColor)
{
    sel = false;
    hl = false;
...
void SoauracadSelectionRoot::resetContext()
{
    contextMap.clear();
}

void SoauracadSelectionRoot::moveActionStack(SoAction* from, SoAction* to, bool erase)
{
...
#define END_ACTION \
    if (stack.size() != size || stack.back() != this) \
        AuraCAD_ERR("action stack fault"); \
    else { \
...
void SoauracadSelectionRoot::pick(SoPickAction* action)
{
    BEGIN_ACTION;
...
void SoauracadSelectionRoot::rayPick(SoRayPickAction* action)
{
    BEGIN_ACTION;
    if (doActionPrivate(stack, action)) {
...
void SoauracadSelectionRoot::handleEvent(SoHandleEventAction* action)
{
    BEGIN_ACTION;
    inherited::handleEvent(action);
    END_ACTION;
...
void SoauracadSelectionRoot::search(SoSearchAction* action)
{
    BEGIN_ACTION;
...
void SoauracadSelectionRoot::getPrimitiveCount(SoGetPrimitiveCountAction* action)
{
    BEGIN_ACTION;
...
void SoauracadSelectionRoot::getBoundingBox(SoGetBoundingBoxAction* action)
{
    BEGIN_ACTION;
    if (doActionPrivate(stack, action)) {
...
void SoauracadSelectionRoot::getMatrix(SoGetMatrixAction* action)
{
    BEGIN_ACTION;
    if (doActionPrivate(stack, action)) {
...
void SoauracadSelectionRoot::callback(SoCallbackAction* action)
{
    BEGIN_ACTION;
...
void SoauracadSelectionRoot::doAction(SoAction* action)
{
    BEGIN_ACTION
    if (doActionPrivate(stack, action)) {
        inherited::doAction(action);
...
bool SoauracadSelectionRoot::doActionPrivate(Stack& stack, SoAction* action)
{
    ...
    // SoauracadSelectionRoot is found here. New function checkSelection() is exposed
    ...
    if (action->isOfType(SoSelectionElementAction::getClassTypeId())) {
        ...
        if (selAction->getType() == SoSelectionElementAction::None) {
            if (action->getWhatAppliedTo() == SoAction::NODE) {
                ...
                // are the first SoauracadSelectionRoot encountered (which means all
...
int SoauracadSelectionRoot::SelContext::merge(
    int status,
...
SO_NODE_SOURCE(SoauracadPathAnnotation)

SoauracadPathAnnotation::SoauracadPathAnnotation()
{
    SO_NODE_CONSTRUCTOR(SoauracadPathAnnotation);
    path = nullptr;
    tmpPath = nullptr;
    det = nullptr;
}
...
void SoauracadPathAnnotation::finish()
{
    atexit_cleanup();
}

void SoauracadPathAnnotation::initClass()
{
    SO_NODE_INIT_CLASS(SoauracadPathAnnotation, SoSeparator, "Separator");
}

void SoauracadPathAnnotation::GLRender(SoGLRenderAction* action)
{
...
void SoauracadPathAnnotation::GLRenderBelowPath(SoGLRenderAction* action)
{
        ...
        if (det) {
            ...
            if (!bbox) {
                for (int i = 0, count = path->getLength(); i < count; ++i) {
                    if (!path->getNode(i)->isOfType(SoauracadSelectionRoot::getClassTypeId())) {
                        continue;
                    }
                    auto node = dynamic_cast<SoauracadSelectionRoot*>(path->getNode(i));
                    if (node && node->selectionStyle.getValue() == SoauracadSelectionRoot::Box) {
                        bbox = true;
                        break;
                    }
                }
            ...
            if (!bbox) {
                ...
                SoauracadSelectionRoot::checkSelection(sel, selColor, hl, hlColor);
                if (sel || hl) {
                    SoauracadSelectionRoot::renderBBox(action, this, hl ? hlColor : selColor);
                }
                else {
                    inherited::GLRenderInPath(action);
                }
...
void SoauracadPathAnnotation::GLRenderInPath(SoGLRenderAction* action)
{
    GLRenderBelowPath(action);
}

void SoauracadPathAnnotation::setDetail(SoDetail* d)
{
    if (d != det) {
        delete det;
        det = d;
...
void SoauracadPathAnnotation::setPath(SoPath* newPath)
{
    if (path) {
        path->unref();
...
void SoauracadPathAnnotation::getBoundingBox(SoGetBoundingBoxAction* action)
{
    if (path) {
        SoGetBoundingBoxAction bboxAction(action->getViewportRegion());
        SoauracadSelectionRoot::moveActionStack(action, &bboxAction, false);
        bboxAction.apply(path);
        SoauracadSelectionRoot::moveActionStack(&bboxAction, action, true);
