...
AuraCAD_LOG_LEVEL_INIT("Selection", false, true, true)
...
void SelectionObserver::_onSelectionChanged(const SelectionChanges& msg)
{
    try {
        ...
        e.reportException();
        AuraCAD_ERR("Unhandled Base::Exception caught in selection observer: ");
    }
    catch (std::exception& e) {
        AuraCAD_ERR("Unhandled std::exception caught in selection observer: " << e.what());
    }
    catch (...) {
        AuraCAD_ERR("Unhandled unknown exception caught in selection observer");
    }
}
...
)
{
    ...
    if (Chng.Type == SelectionChanges::SetPreselect) {
        ...
        AuraCAD_TRACE("preselect " << DocName << '#' << FeatName << '.' << SubName);
    }
    else {
        AuraCAD_TRACE("preselect signal " << DocName << '#' << FeatName << '.' << SubName);
    }
...
)
{
    ...
    for (const auto& pSubName : pSubNames) {
        ...
        AuraCAD_LOG("Add Selection " << Chng.pDocName << '#' << Chng.pObjectName << '.' << Chng.pSubName);
...
)
{
    ...
    if (DocName == pDocName && FeatName == pObjectName && SubName == pSubName) {
        if (show) {
            AuraCAD_TRACE("preselect signal");
            notify(
    ...
    AuraCAD_LOG("Update Selection " << Chng.pDocName << '#' << Chng.pObjectName << '.' << Chng.pSubName);
...
) const
{
    ...
    if (!sel.pDoc) {
        if (!selList) {
            AuraCAD_ERR("Cannot find document");
        }
        return -1;
    }
...
SelectionSingleton::SelectionContext SelectionSingleton::getSelectionContext(const char* pDocName)
{
    ...
    // operations on AuraCAD are meant to act on the current active
    // document anyway
    if (pDocName && strcmp(pDocName, "*") == 0) {
        pDocName = nullptr;
    }
...
PyObject* SelectionSingleton::sAddSelection(PyObject* /*self*/, PyObject* args)
{
    ...
    if (PyArg_ParseTuple(
        ...
        if (!docObj || !docObj->isAttachedToDocument()) {
            PyErr_SetString(Base::PyExc_AuraCAD_GeneralError, "Cannot check invalid object");
            return nullptr;
        }
