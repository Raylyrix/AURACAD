...
SoFullPath* Gui::SoauracadSelection::currenthighlight = nullptr;


// *************************************************************************

SO_NODE_SOURCE(SoauracadSelection)

/*!
  Constructor.
*/
SoauracadSelection::SoauracadSelection()
{
    SO_NODE_CONSTRUCTOR(SoauracadSelection);
...
SoauracadSelection::~SoauracadSelection()
{
    ...
    if (currenthighlight
        && (!currenthighlight->getTail()->isOfType(SoauracadSelection::getClassTypeId()))) {
...
// doc from parent
void SoauracadSelection::initClass()
{
    SO_NODE_INIT_CLASS(SoauracadSelection, SoGroup, "Group");
}

void SoauracadSelection::finish()
{
    atexit_cleanup();
}
...
*/
void SoauracadSelection::turnOfauracadurrentHighlight(SoGLRenderAction* action)
{
    SoauracadSelection::turnofauracadurrent(action);
}

void SoauracadSelection::doAction(SoAction* action)
{
    if (useNewSelection.getValue() && action->getCurPathCode() != SoAction::OFF_PATH) {
        if (action->getTypeId() == Gui::SoHighlightElementAction::getClassTypeId()) {
            ...
            if (!hlaction->isHighlighted()) {
                auto ctx = Gui::SoauracadSelectionRoot::getActionContext(action, this, selContext, false);
                ...
                auto ctx = Gui::SoauracadSelectionRoot::getActionContext(action, this, selContext);
                if (ctx) {
            ...
            if (selaction->getType() == Gui::SoSelectionElementAction::All
                ...
                SelContextPtr ctx = Gui::SoauracadSelectionRoot::getActionContext(action, this, selContext);
                if (ctx) {
                ...
                SelContextPtr ctx
                    = Gui::SoauracadSelectionRoot::getActionContext(action, this, selContext, false);
    ...
    if (action->getTypeId() == SoauracadDocumentAction::getClassTypeId()) {
        auto docaction = static_cast<SoauracadDocumentAction*>(action);
        this->documentName = docaction->documentName;
    }

    if (action->getTypeId() == SoauracadDocumentObjectAction::getClassTypeId()) {
        auto objaction = static_cast<SoauracadDocumentObjectAction*>(action);
    ...
    if (!useNewSelection.getValue()) {
        ...
        if (action->getTypeId() == SoauracadEnablePreselectionAction::getClassTypeId()) {
            auto preaction = static_cast<SoauracadEnablePreselectionAction*>(action);
            if (preaction->enabled) {
                this->preselectionMode = SoauracadSelection::AUTO;
            }
            else {
                this->preselectionMode = SoauracadSelection::OFF;
            }
        }

        if (action->getTypeId() == SoauracadEnableSelectionAction::getClassTypeId()) {
            auto selaction = static_cast<SoauracadEnableSelectionAction*>(action);
            if (selaction->enabled) {
                this->selectionMode = SoauracadSelection::SEL_ON;
            }
            else {
                this->selectionMode = SoauracadSelection::SEL_OFF;
        ...
        if (action->getTypeId() == SoauracadSelectionColorAction::getClassTypeId()) {
            auto colaction = static_cast<SoauracadSelectionColorAction*>(action);
            this->colorSelection = colaction->selectionColor;
        }

        if (action->getTypeId() == SoauracadHighlightColorAction::getClassTypeId()) {
            auto colaction = static_cast<SoauracadHighlightColorAction*>(action);
            this->colorHighlight = colaction->highlightColor;
        }

        if (selectionMode.getValue() == SEL_ON
            && action->getTypeId() == SoauracadSelectionAction::getClassTypeId()) {
            auto selaction = static_cast<SoauracadSelectionAction*>(action);
...
int SoauracadSelection::getPriority(const SoPickedPoint* p)
{
...
const SoPickedPoint* SoauracadSelection::getPickedPoint(SoHandleEventAction* action) const
{
...
// doc from parent
void SoauracadSelection::handleEvent(SoHandleEventAction* action)
{
    if (useNewSelection.getValue()) {
            ...
            if (pp && pp->getPath()->containsPath(action->getCurPath())) {
                if (!highlighted) {
                    if (Gui::Selection().setPreselect(
                        ...
                        SoauracadSelection::turnofauracadurrent(action);
                        SoauracadSelection::currenthighlight = Gui::toFullPath(action->getCurPath()->copy());
                        SoauracadSelection::currenthighlight->ref();
                        highlighted = true;
...
// doc from parent
void SoauracadSelection::GLRenderBelowPath(SoGLRenderAction* action)
{
    SoState* state = action->getState();
    SelContextPtr ctx = Gui::SoauracadSelectionRoot::getRenderContext<SelContext>(this, selContext);
...
void SoauracadSelection::GLRender(SoGLRenderAction* action)
{
    SoState* state = action->getState();
    SelContextPtr ctx = Gui::SoauracadSelectionRoot::getRenderContext<SelContext>(this, selContext);
    if (selContext2->checkGlobal(ctx)) {
...
// doc from parent
void SoauracadSelection::GLRenderInPath(SoGLRenderAction* action)
{
    SelContextPtr ctx = Gui::SoauracadSelectionRoot::getRenderContext<SelContext>(this, selContext);
    if (selContext2->checkGlobal(ctx)) {
        ctx = selContext2;
...
SbBool SoauracadSelection::preRender(SoGLRenderAction* action, GLint& oldDepthFunc)
//
////////////////////////////////////////////////////////////////////////
{
...
/*!
  ...
  when status change.
*/
void SoauracadSelection::redrawHighlighted(SoAction* action, SbBool doHighlight)
{
    Q_UNUSED(action);
    Q_UNUSED(doHighlight);
}

SbBool SoauracadSelection::readInstance(SoInput* in, unsigned short flags)
{
...
//
bool SoauracadSelection::setOverride(SoGLRenderAction* action, SelContextPtr ctx)
{
    auto mymode = static_cast<PreselectionModes>(this->preselectionMode.getValue());
    ...
    if (mystyle == SoauracadSelection::BOX) {
        if (ctx) {
            SoauracadSelectionRoot::renderBBox(
                action,
                this,
    ...
    if (SoLazyElement::getLightModel(state) == SoLazyElement::BASE_COLOR
        || mystyle == SoauracadSelection::EMISSIVE_DIFFUSE) {
        if (!preselected && ctx) {
...
void SoauracadSelection::turnofauracadurrent(SoAction* action)
{
    if (SoauracadSelection::currenthighlight && SoauracadSelection::currenthighlight->getLength()) {
        SoNode* tail = SoauracadSelection::currenthighlight->getTail();
        if (tail->isOfType(SoauracadSelection::getClassTypeId())) {
            static_cast<SoauracadSelection*>(tail)->highlighted = false;
            static_cast<SoauracadSelection*>(tail)->touch();  // force scene redraw
            if (action) {
                static_cast<SoauracadSelection*>(tail)->redrawHighlighted(action, false);
            }
        }
    }
    if (SoauracadSelection::currenthighlight) {
        SoauracadSelection::currenthighlight->unref();
        SoauracadSelection::currenthighlight = nullptr;
    }
}

SbBool SoauracadSelection::isHighlighted(SoAction* action)
//
////////////////////////////////////////////////////////////////////////
{
...
void SoauracadSelection::applySettings()
{
    // TODO Some view providers got copy of this code: make them use this (2015-09-03, Fat-Zer)
    // Note: SoauracadUnifiedSelection got the same code, keep in sync or think about a way to share it
    float transparency;
    ParameterGrp::handle hGrp = Gui::WindowParameter::getDefaultParameter()->GetGroup("View");
    bool enablePre = hGrp->GetBool("EnablePreselection", true);
    bool enableSel = hGrp->GetBool("EnableSelection", true);
    if (!enablePre) {
        this->preselectionMode = Gui::SoauracadSelection::OFF;
    }
    else {
    ...
    if (!enableSel) {
        this->selectionMode = Gui::SoauracadSelection::SEL_OFF;
    }
    else {
