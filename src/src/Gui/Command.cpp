...
AuraCAD_LOG_LEVEL_INIT("Command", true, true)
...
/** \defgroup commands Command Framework
    \ingroup GUI
    \brief Structure for registering commands to the AuraCAD system
 * \section Overview
...
void Command::_invoke(int id, bool disablelog)
{
    try {
        ...
        e.reportException();
        // Pop-up a dialog for AuraCAD-specific exceptions
...
const char* Command::beginCmdHelp()
{
    return "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Transitional//EN\">\n"
           ...
           "<title>AuraCAD Main Index</title>\n"
           "</head>\n"
           "<body bgcolor=\"#ffffff\">\n\n";
}
...
const char* Command::keySequenceToAccel(int sk) const
{
    ...
    // In case AuraCAD is loaded without GUI (issue 16407)
...
void CommandManager::addCommand(Command* pCom)
{
    ...
    if (cmd) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_ERR("duplicate command " << pCom->getName());
        }
        return;
    }
    ++_revision;
