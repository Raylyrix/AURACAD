...
AuraCAD_LOG_LEVEL_INIT("ViewProviderFeaturePython", true, true)
...
#undef AuraCAD_PY_ELEMENT
#define AuraCAD_PY_ELEMENT(_name) py_##_name = Py::None();

    try {
        AuraCAD_PY_VIEW_OBJECT
    }
...
#undef AuraCAD_PY_ELEMENT
#define AuraCAD_PY_ELEMENT(_name) AuraCAD_PY_ELEMENT_INIT(_name)
...
#define AuraCAD_PY_CALL_CHECK(_name) _AuraCAD_PY_CALL_CHECK(_name, return (NotImplemented))

QIcon ViewProviderFeaturePythonImp::getIcon() const
{
    _AuraCAD_PY_CALL_CHECK(getIcon, return (QIcon()));
...
std::map<BitmapFactoryInst::Position, std::string> ViewProviderFeaturePythonImp::getOverlayIcons() const
{
    ...
    _AuraCAD_PY_CALL_CHECK(getOverlayIcons, return overlays);
...
{
    _AuraCAD_PY_CALL_CHECK(claimChildren, return (false));
...
{
    AuraCAD_PY_CALL_CHECK(useNewSelectionModel);
...
{
    _AuraCAD_PY_CALL_CHECK(getElement, return (false));
...
{
    AuraCAD_PY_CALL_CHECK(getElementPicked);
...
{
    _AuraCAD_PY_CALL_CHECK(getDetail, return (false));
...
{
    AuraCAD_PY_CALL_CHECK(getDetailPath);
...
{
    AuraCAD_PY_CALL_CHECK(unsetEdit)
...
{
    AuraCAD_PY_CALL_CHECK(setEditViewer)
...
{
    AuraCAD_PY_CALL_CHECK(unsetEditViewer)
...
{
    AuraCAD_PY_CALL_CHECK(doubleClicked)
...
{
    _AuraCAD_PY_CALL_CHECK(setupContextMenu, return (false));
...
{
    _AuraCAD_PY_CALL_CHECK(attach, return);
...
void ViewProviderFeaturePythonImp::finishRestoring()
{
    ...
    try {
        ...
        if (vp.isNone()) {
            ...
            _AuraCAD_PY_CALL_CHECK(finishRestoring, return);
...
{
    _AuraCAD_PY_CALL_CHECK(beforeDelete, return);
...
{
    AuraCAD_PY_CALL_CHECK(onDelete);
...
{
    AuraCAD_PY_CALL_CHECK(canDelete);
...
{
    AuraCAD_PY_CALL_CHECK(canAddToSceneGraph);
...
{
    _AuraCAD_PY_CALL_CHECK(getDefaultDisplayMode, return (0));
...
std::vector<std::string> ViewProviderFeaturePythonImp::getDisplayModes() const
{
    ...
    _AuraCAD_PY_CALL_CHECK(getDisplayModes, return (modes));
...
{
    _AuraCAD_PY_CALL_CHECK(setDisplayMode, return (ModeName));
...
{
    AuraCAD_PY_CALL_CHECK(canDragObjects);
...
{
    AuraCAD_PY_CALL_CHECK(canDragObject);
...
{
    AuraCAD_PY_CALL_CHECK(dragObject);
...
{
    AuraCAD_PY_CALL_CHECK(canDropObjects);
...
{
    AuraCAD_PY_CALL_CHECK(canDropObject);
...
{
    AuraCAD_PY_CALL_CHECK(dropObject);
...
{
    AuraCAD_PY_CALL_CHECK(canDragAndDropObject);
...
{
    AuraCAD_PY_CALL_CHECK(canDropObjectEx);
...
{
    _AuraCAD_PY_CALL_CHECK(dropObjectEx, return (false));
...
ViewProviderFeaturePythonImp::ValueT ViewProviderFeaturePythonImp::canRemoveChildrenFromRoot() const
{
    ...
    AuraCAD_PY_CALL_CHECK(canRemoveChildrenFromRoot);
...
bool ViewProviderFeaturePythonImp::getDropPrefix(std::string& prefix) const
{
    ...
    _AuraCAD_PY_CALL_CHECK(getDropPrefix, return (false));
...
)
{
    ...
    AuraCAD_PY_CALL_CHECK(replaceObject);
...
{
    _AuraCAD_PY_CALL_CHECK(getLinkedViewProvider, return (false));
    ...
    try {
        ...
        AuraCAD_ERR("getLinkedViewProvider(): invalid return type, expects ViewObject or (ViewObject, subname)");
        return true;
    }
    catch (Py::Exception&) {
        if (PyErr_ExceptionMatches(PyExc_NotImplementedError)) {
...
{
    _AuraCAD_PY_CALL_CHECK(editProperty, return false);
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::String(name));
