...
void DlgPropertyLink::init(const App::DocumentObjectT& prop, bool tryFilter)
{
    ...
    auto propLink = AuraCAD_cast<App::PropertyLinkBase*>(objProp.getProperty());
    if (!propLink) {
        return;
    }
...
void DlgPropertyLink::onItemSelectionChanged()
{
    ...
    if (!TreeParams::getSyncView()) {
        ...
        if (doc) {
            auto vp = AuraCAD_cast<Gui::ViewProviderDocumentObject*>(doc->getViewProvider(obj));
            if (vp) {
...
QTreeWidgetItem* DlgPropertyLink::createItem(App::DocumentObject* obj, QTreeWidgetItem* parent)
{
    ...
    auto prop = AuraCAD_cast<App::PropertyPythonObject*>(obj->getPropertyByName("Proxy"));
    if (prop) {
