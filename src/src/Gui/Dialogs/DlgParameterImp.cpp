    ...
    , ui(new Ui_DlgParameter)
{
    ...
    // we don't know what style sheet the user uses for auracad
...
void ParameterGroup::onDeleteSelectedItem()
{
    ...
    if (sel && sel->isSelected() && sel->parent()) {
        if (QMessageBox::question(
            ...
            int index = parent->indexOauracadhild(sel);
...
void ParameterGroup::onExportToFile()
{
    QString file = FileDialog::getSaveFileName(
        ...
        QString(),
        QStringList(QStringLiteral("XML (*.auracadParam)"))
    );
    if (!file.isEmpty()) {
        QTreeWidgetItem* item = currentItem();
        if (item && item->isSelected()) {
