...
{
    QDir dir(folder, QLatin1String("*.auracadMacro *.py"));
    QStringList unfiltered;  // all .auracadmacro and .py files
    QDirIterator it(
        dir.path(),
        QStringList() << "*.auracadMacro" << "*.py",
        QDir::Files,
...
/**
 * Specify the location of your macro files. The default location is AuraCAD's home path.
 */
...
void DlgMacroExecuteImp::onCreateButtonClicked()
{
    ...
    if (!fn.isEmpty()) {
        ...
        if (suffix != QLatin1String("auracadmacro") && suffix != QLatin1String("py")) {
            fn += QLatin1String(".auracadMacro");
        }
 ...
 * "MyMacro.auracadMacro" becomes "MyMacro@001.auracadMacro"
 * "MyMacro@002.auracadMacro.py" becomes "MyMacro@003.auracadMacro.py" unless there is
 * no already existing "MyMacro@001.auracadMacro.py"
 */
void DlgMacroExecuteImp::onDuplicateButtonClicked()
{
    ...
    // example: mymacro@005.fix_bug_in_dialog.auracadMacro
    // and then when duplicating to have the extra note removed so the suggested new name is:
    // mymacro@006.auracadMacro instead of mymacro@006.fix_bug_in_dialog.auracadMacro since the new duplicate
    ...
    // completeSuffix = auracadMacro or py or auracadMacro.py or else suffix will become auracadMacro below
    // if ignoreExtra any extra notes added between @NN. and .auracadMacro will be ignored
