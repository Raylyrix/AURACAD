...
AuraCAD_LOG_LEVEL_INIT("DlgExpressionInput", true, true)
...
void DlgExpressionInput::checkExpression(const QString& text)
{
    ...
    if (expr) {
        ...
        auto* n = AuraCAD_cast<NumberExpression*>(result.get());
        if (n) {
...
{
    return AuraCAD_cast<const App::NumberExpression*>(expr);
}

static const App::StringExpression* toStringExpr(const App::Expression* expr)
{
    return AuraCAD_cast<const App::StringExpression*>(expr);
}

static const App::OperatorExpression* toUnitNumberExpr(const App::Expression* expr)
{
    auto* opExpr = AuraCAD_cast<const App::OperatorExpression*>(expr);
...
void DlgExpressionInput::preselectVarSet()
{
    ...
    if (!doc) {
        AuraCAD_ERR("No active document found");
    }
...
)
{
    for (auto* varSet : varSets) {
        auto* vp = AuraCAD_cast<Gui::ViewProviderDocumentObject*>(
            Gui::Application::Instance->getViewProvider(varSet)
        );
        if (!vp) {
            AuraCAD_ERR("No ViewProvider found for VarSet: " << varSet->getNameInDocument());
            continue;
        }
...
void DlgExpressionInput::onVarSetSelected(int /*index*/)
{
    ...
    if (docName.isEmpty() || varSetName.isEmpty()) {
        AuraCAD_ERR("No document or variable set selected");
        return;
    }

    App::Document* doc = App::GetApplication().getDocument(docName.toUtf8());
    if (!doc) {
        AuraCAD_ERR("Document not found: " << docName.toStdString());
        return;
    }

    App::DocumentObject* varSet = doc->getObject(varSetName.toUtf8());
    if (!varSet) {
        AuraCAD_ERR("Variable set not found: " << varSetName.toStdString());
        return;
    }
