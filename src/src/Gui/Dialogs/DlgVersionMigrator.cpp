    ...
    , ui(std::make_unique<Ui_DlgVersionMigrator>())
{
    ...
    auto calculatingSizeString = QStringLiteral("<b>") + tr("Calculating sizeâ€¦")
        + QStringLiteral("</b>");
...
void PathMigrationWorker::writeMigrationLog(const std::vector<std::filesystem::path>& skippedPaths)
{
    ...
    try {
        ...
        log << "\nSee the AuraCAD Report View for additional details.\n";
    }
    catch (const std::exception& e) {
...
void DlgVersionMigrator::migrate()
{
    ...
    migrationRunning->setText(QObject::tr("Migrating configuration data and addonsâ€¦"));
        ...
        if (!skippedFiles.isEmpty()) {
            ...
            for (const auto& path : skippedFiles) {
                if (path.length() > maxDisplayLength) {
                    truncated.append(
                        path.left(prefixLength) + QStringLiteral("â€¦") + path.right(suffixLength)
                    );
                }
                else {
...
{
    auto helpPage = QStringLiteral("https://wiki.AuraCAD.org/Version_migration");
...
void DlgVersionMigrator::restart(const QString& message)
{
    ...
    restarting->setText(message + QObject::tr(" â†’ Restartingâ€¦"));
