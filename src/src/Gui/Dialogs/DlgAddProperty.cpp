...
AuraCAD_LOG_LEVEL_INIT("DlgAddProperty", true, true)
...
void DlgAddProperty::setWidgetForLabel(const char* labelName, QWidget* widget, QLayout* layout)
{
    ...
    if (!formLayout) {
        AuraCAD_ERR("Form layout not found");
        return;
    }

    int labelRow = findLabelRow(labelName, formLayout);
    if (labelRow < 0) {
        AuraCAD_ERR("Could not find row for '" << labelName << "'");
        return;
    }
...
void DlgAddProperty::initializeTypes()
{
    ...
    const auto addSeparator = [this]() {
        ui->comboBoxType->addItem(QStringLiteral("â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€"));
...
void DlgAddProperty::createSupportDataForType(const Base::Type& type)
{
    ...
    if (!propInstance) {
        AuraCAD_THROWM(Base::RuntimeError, "Failed to create a property of type " << type.getName());
    }
...
) const
{
    ...
    const App::Document* document = AuraCAD_cast<App::Document*>(container);
    const App::DocumentObject* object = AuraCAD_cast<App::DocumentObject*>(container);
    if (document) {
        ...
        AuraCAD_ERR("Cannot record macro for container of type " << container->getTypeId().getName());
        return;
    }
...
{
    addDocumentation();
    auto* object = AuraCAD_cast<App::DocumentObject*>(container);
    if (object) {
