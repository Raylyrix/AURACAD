...
void ViewProviderInventorObject::updateData(const App::Property* prop)
{
    ...
    if (prop == &ivObj->Buffer) {
        ...
        // In AuraCAD a placement only has a rotation and a translation part but
...
{
    if (child->getTypeId().isDerivedFrom(SoauracadSelection::getClassTypeId())) {
        static_cast<SoauracadSelection*>(child)->documentName = docname;
        static_cast<SoauracadSelection*>(child)->objectName = objname;
    }
