...
class auracadSphereSheetProjector: public SbSphereSheetProjector
{
...
public:
    ...
    auracadSphereSheetProjector(const SbSphere& sph, const SbBool orienttoeye = true)
...
NavigationStyle& NavigationStyle::operator=(const NavigationStyle& ns)
{
    ...
    static_cast<auracadSphereSheetProjector*>(this->spinprojector)
        ->setOrbitStyle(static_cast<auracadSphereSheetProjector*>(ns.spinprojector)->getOrbitStyle());
    return *this;
}
...
void NavigationStyle::initialize()
{
    ...
    // 19990425 mortene.
    this->spinprojector = new auracadSphereSheetProjector(
        SbSphere(SbVec3f(0, 0, 0), auracadSphereSheetProjector::defaultSphereRadius)
    );
    SbViewVolume volume;
...
{
    auto projector = static_cast<auracadSphereSheetProjector*>(this->spinprojector);
    projector->setOrbitStyle(auracadSphereSheetProjector::OrbitStyle(style));
}
...
void NavigationStyle::spinInternal(const SbVec2f& pointerpos, const SbVec2f& lastpos)
{
    ...
    if ((getOrbitStyle() == Trackball || getOrbitStyle() == TrackballClassic
        ...
        const float radius = auracadSphereSheetProjector::defaultSphereRadius * sphereScale;
