...
void Thumbnail::SaveDocFile(Base::Writer& writer) const
{
    ...
    QPixmap px = appIcon;
    if (!img.isNull()) {
        // Create a small "auracad" Application icon in the bottom right of the thumbnail
    ...
    if (!px.isNull()) {
        ...
        img.setText(QLatin1String("Thumb::Mimetype"), QLatin1String("application/x-extension-auracadstd"));
