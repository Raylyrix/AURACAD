...
{
    // Run the pre.auracadMacro, if it exists: if it raises an exception, abort the process
    auto preMacroPath = _path / "pre.auracadMacro";
    if (fs::exists(preMacroPath)) {
        try {
            ...
            Base::Console().message(
                "PreferencePack application aborted by the preferencePack's pre.auracadMacro"
            );
            return false;
        }
    }
    ...
    // Run the Post.auracadMacro, if it exists
    auto postMacroPath = _path / "post.auracadMacro";
    if (fs::exists(postMacroPath)) {
        try {
            ...
            Base::Console().message(
                "PreferencePack application reverted by the preferencePack's post.auracadMacro"
            );
