...
void StartupProcess::setThemePaths()
{
    ...
    QIcon::setThemeSearchPaths(QIcon::themeSearchPaths() << QStringLiteral(":/icons/AuraCAD-default"));
#endif
...
void StartupPostProcess::migrateOldTheme(const std::string& style)
{
    ...
    if (style == "AuraCAD Light.qss") {
        prefPackManager->apply("AuraCAD Light");
    }
    else if (style == "AuraCAD Dark.qss") {
    ...
    if (context.create()) {
        ...
        if (major < 2) {
            auto message = QObject::tr(
                               ...
                               "AuraCAD requires OpenGL 2.0 or above. "
...
void StartupPostProcess::setImportImageFormats()
{
    ...
    App::GetApplication().addImportType(filter.c_str(), "AuraCADGui");
}
...
void StartupPostProcess::showMainWindow()
{
    ...
    try {
        ...
        Base::Console().error("Error in AuraCADGuiInit.py: %s\n", e.what());
