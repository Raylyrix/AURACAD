...
AuraCAD_LOG_LEVEL_INIT("PropertyView", true, true)
...
#ifdef Q_OS_MACOS
        // Brute-force workaround for https://github.com/AuraCAD/AuraCAD/issues/14350
        int currentIndex = 0;
...
void PropertyEditor::openEditor(const QModelIndex& index)
{
    ...
    if (items.empty()) {
        AuraCAD_LOG("editor no item");
        return;
    }
    auto prop = items[0];
    auto parent = prop->getContainer();
    auto obj = AuraCAD_cast<App::DocumentObject*>(parent);
    if (!obj) {
        auto view = AuraCAD_cast<ViewProviderDocumentObject*>(parent);
        if (view) {
            obj = view->getObject();
        }
    }
    if (!obj || !obj->getDocument()) {
        AuraCAD_LOG("invalid object");
        return;
    }
    if (obj->getDocument()->hasPendingTransaction()) {
        AuraCAD_LOG("pending transaction");
        return;
    }
    ...
    AuraCAD_LOG("editor transaction " << App::GetApplication().getTransactionName(transactionID));
}
