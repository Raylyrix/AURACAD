...
#define CREATE_AXIS(_type, _key, _count, _offset, _mat) \
    do { \
        ...
        auto pAxis = new SoauracadSelection; \
        pAxis->applySettings(); \
        pAxis->style = SoauracadSelection::EMISSIVE_DIFFUSE; \
...
bool AxisOrigin::getElementPicked(const SoPickedPoint* pp, std::string& subname) const
{
    ...
    for (int i = 0; i < length; ++i) {
        ...
        if (node->isOfType(SoauracadSelection::getClassTypeId())) {
            subname = static_cast<SoauracadSelection*>(node)->subElementName.getValue().getString();
            return true;
        }
        else if (node->isOfType(SoauracadSelectionRoot::getClassTypeId())) {
            break;
        }
    }
    return false;
