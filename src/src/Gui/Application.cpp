...
#include "Inventor/SoauracadPlacementIndicatorKit.h"
#include "QtWidgets.h"

#include <AuraCADStyle.h>
...
public:
    ...
    {
        auto vpd = AuraCAD_cast<ViewProviderDocumentObject*>(const_cast<ViewProvider*>(&vp));
...
static PyObject* AuraCADGui_subgraphFromObject(PyObject* /*self*/, PyObject* args)
{
    PyObject* o;
    ...
    try {
        ...
        if (base && base->isDerivedFrom<Gui::ViewProviderDocumentObject>()) {
            ...
            else if (type == "SoauracadSelectionRoot") {
                type = "SoSeparator";
            }
...
static PyObject* AuraCADGui_exportSubgraph(PyObject* /*self*/, PyObject* args)
{
    ...
    try {
        ...
        if (node) {
            ...
            if (formatStr == "VRML") {
                SoauracadDB::writeToVRML(node, buffer);
            }
            else if (formatStr == "IV") {
                buffer = SoauracadDB::writeNodesToString(node);
            }
            else {
...
static PyObject* AuraCADGui_getSoDBVersion(PyObject* /*self*/, PyObject* args)
{
...
struct PyMethodDef AuraCADGui_methods[] = {
    {"subgraphFromObject",
     AuraCADGui_subgraphFromObject,
     METH_VARARGS,
     "subgraphFromObject(object) -> Node\n\n"
     "Return the Inventor subgraph to an object"},
    {"exportSubgraph",
     AuraCADGui_exportSubgraph,
     METH_VARARGS,
...
Application::Application(bool GUIenabled)
{
    ...
    if (GUIenabled) {
        ...
        PyDoc_STRVAR(
            AuraCADGui_doc,
            "The functions in the AuraCADGui module allow working with GUI documents,\n"
            "view providers, views, workbenches and much more.\n\n"
            "The AuraCADGui instance provides a list of references of GUI documents which\n"
            ...
            "The AuraCADGui module also provides a set of functions to work with so called\n"
            "workbenches.");

        // if this returns a valid pointer then the 'AuraCADGui' Python module was loaded,
        // otherwise the executable was launched
        PyObject* modules = PyImport_GetModuleDict();
        PyObject* module = PyDict_GetItemString(modules, "AuraCADGui");
        if (!module) {
            static struct PyModuleDef AuraCADGuiModuleDef = {PyModuleDef_HEAD_INIT,
                                                             "AuraCADGui",
                                                             AuraCADGui_doc,
                                                             -1,
                                                             ...
                                                             nullptr,
                                                             nullptr};
            module = PyModule_Create(&AuraCADGuiModuleDef);

            PyDict_SetItemString(modules, "AuraCADGui", module);
        }
        else {
        ...
        Base::PyRegisterEnum<SoauracadPlacementIndicatorKit::Part>(module, "PlacementIndicatorParts", {
            {"Axes", SoauracadPlacementIndicatorKit::Axes},
            {"ArrowHeads", SoauracadPlacementIndicatorKit::ArrowHeads},
            {"Labels", SoauracadPlacementIndicatorKit::Labels},
            {"PlaneIndicator", SoauracadPlacementIndicatorKit::PlaneIndicator},
            {"OriginIndicator", SoauracadPlacementIndicatorKit::OriginIndicator},

            // common configurations
            {"AllParts", SoauracadPlacementIndicatorKit::AllParts},
            {"AxisCross", SoauracadPlacementIndicatorKit::AxisCross},
        });
    ...
    Base::PyGILStateLocker lock;
    PyObject* module = PyImport_AddModule("AuraCADGui");
    PyMethodDef* meth = AuraCADGui_methods;
...
void Application::open(const char* FileName, const char* Module)
{
    ...
    if (Module) {
        try {
            if (File.hasExtension("auracadStd")) {
                bool handled = false;
                ...
                for (auto& v : d->documents) {
                    ...
                    if (filepath == fi) {
                        handled = true;
                        Command::doCommand(Command::App, "AuraCADGui.reload('%s')", doc->getName());
                        break;
                    }
                }

                if (!handled) {
                    Command::doCommand(Command::App, "AuraCAD.openDocument('%s')", unicodepath.c_str());
                    Gui::Application::checkForRecomputes();
                }
            }
            else {
                std::string code = fmt::format(
                    "from AuraCAD import module_io\n"
...
void Application::exportTo(const char* FileName, const char* DocName, const char* Module)
{
    ...
    if (strcmp(Module, "Part") == 0) {
        AuraCAD_WARN("Exporting with 'Part' is deprecated, use 'ImportGui' instead");
    }
        ...
        try {
            ...
            for (auto it : sel) {
                if (unique_objs.insert(it).second) {
                    str << "__objs__.append(AuraCAD.getDocument(\"" << DocName << "\").getObject(\""
...
void Application::checkForRecomputes()
{
    ...
    // Certain tests want to use very old .auracadStd files.  We should not prompt during those tests, so
    // this allows them to 'AuraCAD.ConfigSet("SuppressRecomputeRequiredDialog", "True")`
    ...
    if (doc != d->documents.end()) {
        ...
        if (d->activeDocument != doc->second) {
            ...
            if (d->activeDocument) {
                ...
                Py::Module("AuraCADGui").setAttr(std::string("ActiveDocument"), active);
                ...
                Base::PyGILStateLocker lock;
                Py::Module("AuraCADGui").setAttr(std::string("ActiveDocument"), Py::None());
            }
        }
...
#ifdef AuraCAD_DEBUG  // redirect Coin messages to AuraCAD
...
static void init_resources()
{
    ...
    Q_INIT_RESOURCE(AuraCAD_translation);
}
...
void Application::initApplication()
{
    ...
    try {
        initTypes();
        new Base::ScriptProducer("AuraCADGuiInit", AuraCADGuiInit);
        new Base::ScriptProducer("AuraCADGuiTest", AuraCADGuiTest);
        init_resources();
...
{
    Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("AuraCADGuiInit"));
}
...
void tryRunEventLoop(GUISingleApplication& mainApp)
{
...
#if !defined(AuraCAD_OS_WIN32) || (BOOST_VERSION < 107600)
    ...
    try {
        ...
        if (flock.try_lock()) {
            Base::Console().log("Init: Executing event loopâ€¦\n");
            QApplication::exec();
...
void Application::runApplication()
{
...
#if defined(AuraCAD_OS_LINUX) || defined(AuraCAD_OS_BSD)
    ...
    && (defined(AuraCAD_OS_LINUX) || defined(AuraCAD_OS_BSD))
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=3&t=15540
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=10&t=21665
...
void Application::init3DMouse(MainWindow* mainWindow, QApplication* qtApp)
{
    ...
    if (!Instance->pNavlibInterface) {
        ...
        if (auto auracadApp = qobject_cast<GUIApplicationNativeEventAware*>(qtApp)) {
            Base::Console().log("Init: Initializing 3D mouse event handling\n");
            auracadApp->initSpaceball(mainWindow);
        }
    }
}
...
void Application::setStyleSheet(const QString& qssFile, bool tiledBackground)
{
    ...
    // and https://forum.AuraCAD.org/viewtopic.php?f=34&t=50744
    static bool init = true;
    if (init) {
        init = false;
        mw->setProperty("AuraCAD_originalLinkCoor", qApp->palette().color(QPalette::Link));
    }
    else {
        QPalette newPal(qApp->palette());
        newPal.setColor(QPalette::Link, mw->property("AuraCAD_originalLinkCoor").value<QColor>());
        qApp->setPalette(newPal);
    }

    mw->setProperty("AuraCAD_currentStyleSheet", qssFile);
    mw->setProperty("AuraCAD_tiledBackground", tiledBackground);
    ...
    // See https://forum.AuraCAD.org/viewtopic.php?f=17&t=50783
    if (!d->startingUp) {
        if (mdi->style()) {
...
void Application::reloadStyleSheet()
{
    ...
    const QString qssFile = mw->property("AuraCAD_currentStyleSheet").toString();
    const bool tiledBackground = mw->property("AuraCAD_tiledBackground").toBool();
...
void Application::setStyle(const QString& name)
{
    const auto createStyleFromName = [](const QString& name) -> QStyle* {
        if (name == QStringLiteral("AuraCAD")) {
            return new AuraCADStyle();
        }
    ...
    const auto requiresEventFilter = [](QStyle* style) {
        // for now only AuraCAD style requires additional event processing
        return qobject_cast<AuraCADStyle*>(style) != nullptr;
    };
...
{
    // From 0.21, `auracadBak` will be the intended default backup format
    bool makeBackups = App::GetApplication()
                           .GetParameterGroupByPath("User parameter:BaseApp/Preferences/Document")
                           ->GetBool("CreateBackupFiles", true);
    if (makeBackups) {
        bool useauracadBakExtension = App::GetApplication()
        ...
        if (!useauracadBakExtension) {
            ...
            Base::Console().warning(
                "The `.auracadStd#` backup format is deprecated and may "
                "be removed in future versions.\n"
                "To update, check the 'Preferences->General->Document->Use "
                "date and auracadBak extension' option.\n"
            );
        }
    }
}
