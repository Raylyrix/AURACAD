    ...
    , _viewerPy(nullptr)
{
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=3&t=6055&sid=150ed90cbefba50f1e2ad4b4e6684eba
    // describes a minor error but trying to fix it leads to a major issue
    // https://forum.AuraCAD.org/viewtopic.php?f=3&t=6085&sid=3f4bcab8007b96aaf31928b564190fd7
...
View3DInventor::~View3DInventor()
{
    if (_pcDocument) {
        ...
        if (Cam) {
            _pcDocument->saveCameraSettings(SoauracadDB::writeNodesToString(Cam).c_str());
        }
    }
...
void View3DInventor::onRename(Gui::Document* pDoc)
{
    ...
    SoauracadDocumentAction cAct(name);
...
const std::string& View3DInventor::getCamera() const
{
    ...
    return SoauracadDB::writeNodesToString(Cam);
}
...
RayPickInfo View3DInventor::getObjInfoRay(Base::Vector3d* startvec, Base::Vector3d* dirvec)
{
    ...
    if (vp && vp->isDerivedFrom<ViewProviderDocumentObject>()) {
        ...
        if (vp->useNewSelectionModel()) {
            ...
            // search for a SoauracadSelection node
            SoauracadDocumentObjectAction objaction;
        ...
        // custom nodes not in a VP: search for a SoauracadSelection node
        SoauracadDocumentObjectAction objaction;
        objaction.apply(Point->getPath());
        if (objaction.isHandled()) {
