...
AuraCAD_LOG_LEVEL_INIT("3DViewerSelection", true, true)

using namespace Gui;

View3DInventorSelection::View3DInventorSelection(SoauracadUnifiedSelection* root)
    : selectionRoot(root)
{
    selectionRoot->ref();
    ...
    {
        auto selRoot = new SoauracadSelectionRoot;
        selRoot->selectionStyle = SoauracadSelectionRoot::PassThrough;
...
void View3DInventorSelection::checkGroupOnTop(const SelectionChanges& Reason)
{
    ...
    if (Reason.Type == SelectionChanges::RmvSelection) {
        ...
        if (index >= 0) {
            auto node = static_cast<SoauracadPathAnnotation*>(it->second);
            ...
            AuraCAD_LOG("remove annotation " << Reason.Type << " " << key);
        }
        else {
            AuraCAD_LOG("remove annotation object " << Reason.Type << " " << key);
        }
        objs.erase(it);
        return;
    }
    ...
    auto vp = AuraCAD_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(obj));
    ...
    for (auto childVp = vp;; childVp = grpVp) {
        ...
        grpVp = AuraCAD_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(grp));
        if (!grpVp) {
            break;
        }
    ...
    if (vp->getDetailPath(subname, &path, true, det) && path.getLength()) {
        auto node = new SoauracadPathAnnotation;
        node->setPath(&path);
        ...
        AuraCAD_LOG("add annotation " << Reason.Type << " " << key);
        objs[key.c_str()] = node;
    }
    delete det;
    path.unrefNoDelete();
