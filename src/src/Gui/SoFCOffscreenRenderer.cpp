...
SoauracadOffscreenRenderer* SoauracadOffscreenRenderer::inst = nullptr;


SoauracadOffscreenRenderer& SoauracadOffscreenRenderer::instance()
{
    if (!inst) {
        inst = new SoauracadOffscreenRenderer(SbViewportRegion());
    }
    return *inst;
}

SoauracadOffscreenRenderer::SoauracadOffscreenRenderer(const SbViewportRegion& viewportregion)
    : SoOffscreenRenderer(viewportregion)
{}

SoauracadOffscreenRenderer::SoauracadOffscreenRenderer(SoGLRenderAction* action)
    : SoOffscreenRenderer(action)
{}

SoauracadOffscreenRenderer::~SoauracadOffscreenRenderer() = default;

void SoauracadOffscreenRenderer::writeToImage(QImage& img) const
{
...
void SoauracadOffscreenRenderer::writeToImageFile(
    const char* filename,
    const char* comment,
    const SbMatrix& mat,
    const QImage& image
)
{
    ...
    if (file.hasExtension({"JPG", "JPEG"})) {
        ...
        std::string com;
        if (strcmp(comment, "") == 0) {
            com = "Screenshot created by AuraCAD";
        }
        ...
        if (supported) {
            ...
            if (file.hasExtension("PNG")) {
                ...
                img.setText(QLatin1String("Author"), QLatin1String("AuraCAD (https://www.AuraCAD.org)"));
                if (strcmp(comment, "") == 0) {
                    img.setText(
                        QLatin1String("Description"),
                        QLatin1String("Screenshot created by AuraCAD")
                    );
                }
                else if (strcmp(comment, "$MIBA") == 0) {
...
QStringList SoauracadOffscreenRenderer::getWriteImageFiletypeInfo()
{
    QStringList formats;
...
std::string SoauracadOffscreenRenderer::createMIBA(const SbMatrix& mat) const
{
