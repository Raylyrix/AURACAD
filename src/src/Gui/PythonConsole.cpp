    ...
    , _sourceDrain(nullptr)
{
    ...
    // Don't override stdin when running AuraCAD as Python module
...
void PythonConsole::runSource(const QString& line)
{
    ...
    try {
        ...
        QMessageBox::critical(this, tr("Python Console"), tr("Unhandled AuraCAD exception."));
    }
    ...
    if (source->hasUrls()) {
        ...
        for (const auto& it : uri) {
            ...
            if (info.exists() && info.isFile()) {
                ...
                if (ext == QLatin1String("py") || ext == QLatin1String("auracadmacro")) {
                    return true;
                }
            }
        }
...
void PythonConsole::insertFromMimeData(const QMimeData* source)
{
    ...
    if (source->hasUrls()) {
        ...
        for (const auto& it : uri) {
            ...
            if (info.exists()) {
                existingFile = true;
                if (info.isFile() && (ext == QLatin1String("py") || ext == QLatin1String("auracadmacro"))) {
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=3&t=34618
...
void PythonConsole::contextMenuEvent(QContextMenuEvent* e)
{
    ...
    a = menu.addAction(tr("Save History Asâ€¦"), this, &PythonConsole::onSaveHistoryAs);
    a->setEnabled(!d->history.isEmpty());
    ...
    menu.addSeparator();
    menu.addAction(tr("Insert File Nameâ€¦"), this, &PythonConsole::onInsertFileName);
    menu.addSeparator();
...
void PythonConsole::onSaveHistoryAs()
{
    ...
    QString fn = FileDialog::getSaveFileName(
        ...
        tr("Save History"),
        cMacroPath,
        QStringList(QStringLiteral("%1 (*.auracadMacro *.py)").arg(tr("Macro Files")))
    );
    if (!fn.isEmpty()) {
