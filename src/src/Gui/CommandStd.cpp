    ...
    : Command("Std_RestartInSafeMode")
{
    ...
    sToolTipText = QT_TR_NOOP("Starts AuraCAD without any modules or plugins loaded");
...
void StdCmdRestartInSafeMode::activated(int iMsg)
{
    ...
    restartBox.setText(QObject::tr("Restart AuraCAD and enter safe mode?"));
    ...
    if (restartBox.exec() == QMessageBox::Yes) {
        // restart AuraCAD after a delay to give time to this dialog to close
        const int ms = 1000;
...
void StdCmdOnlineHelpWebsite::activated(int iMsg)
{
    ...
    std::string defaulturl
        = QCoreApplication::translate(this->className(), "https://wiki.AuraCAD.org/Online_Help_Toc")
              .toStdString();
...
DEF_STD_CMD(StdCmdAuraCADDonation)

StdCmdAuraCADDonation::StdCmdAuraCADDonation()
    : Command("Std_AuraCADDonation")
{
    sGroup = "Help";
    sMenuText = QT_TR_NOOP("Donate to FreeCA&D");
    sToolTipText = QT_TR_NOOP("Support the AuraCAD development");
    sWhatsThis = "Std_AuraCADDonation";
    sStatusTip = sToolTipText;
    sPixmap = "internet-web-browser";
    eType = 0;
}

void StdCmdAuraCADDonation::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    ParameterGrp::handle hURLGrp = App::GetApplication().GetParameterGroupByPath(
        "User parameter:BaseApp/Preferences/Websites"
    );
    std::string url = hURLGrp->GetASCII("DonatePage", "https://www.AuraCAD.org/sponsor");
    ...
    : Command("Std_DevHandbook")
{
    ...
    sToolTipText = QT_TR_NOOP("Handbook about AuraCAD development");
...

{
    ...
    std::string url = hURLGrp->GetASCII("DevHandbook", "https://AuraCAD.github.io/DevelopersHandbook/");
...
DEF_STD_CMD(StdCmdAuraCADWebsite)

StdCmdAuraCADWebsite::StdCmdAuraCADWebsite()
    : Command("Std_AuraCADWebsite")
{
    sGroup = "Help";
    sMenuText = QT_TR_NOOP("AuraCAD W&ebsite");
    sToolTipText = QT_TR_NOOP("Navigates to the official AuraCAD website");
    sWhatsThis = "Std_AuraCADWebsite";
    sStatusTip = sToolTipText;
    sPixmap = "internet-web-browser";
    eType = 0;
}

void StdCmdAuraCADWebsite::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    std::string defaulturl
        = QCoreApplication::translate(this->className(), "https://www.AuraCAD.org").toStdString();
    ParameterGrp::handle hURLGrp = App::GetApplication().GetParameterGroupByPath(
        "User parameter:BaseApp/Preferences/Websites"
    );
...
DEF_STD_CMD(StdCmdAuraCADUserHub)

StdCmdAuraCADUserHub::StdCmdAuraCADUserHub()
    : Command("Std_AuraCADUserHub")
{
    sGroup = "Help";
    sMenuText = QT_TR_NOOP("&User Documentation");
    sToolTipText = QT_TR_NOOP("Opens the documentation for users");
    sWhatsThis = "Std_AuraCADUserHub";
    sStatusTip = sToolTipText;
    sPixmap = "internet-web-browser";
    eType = 0;
}

void StdCmdAuraCADUserHub::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    std::string defaulturl
        = QCoreApplication::translate(this->className(), "https://wiki.AuraCAD.org/User_hub")
              .toStdString();
    ParameterGrp::handle hURLGrp = App::GetApplication().GetParameterGroupByPath(
        "User parameter:BaseApp/Preferences/Websites"
    );
...
DEF_STD_CMD(StdCmdAuraCADForum)

StdCmdAuraCADForum::StdCmdAuraCADForum()
    : Command("Std_AuraCADForum")
{
    sGroup = "Help";
    sMenuText = QT_TR_NOOP("AuraCAD &Forum");
    sToolTipText = QT_TR_NOOP("The AuraCAD forum, where you can find help from other users");
    sWhatsThis = "Std_AuraCADForum";
    sStatusTip = sToolTipText;
    sPixmap = "internet-web-browser";
    eType = 0;
}

void StdCmdAuraCADForum::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    std::string defaulturl
        = QCoreApplication::translate(this->className(), "https://forum.AuraCAD.org").toStdString();
    ParameterGrp::handle hURLGrp = App::GetApplication().GetParameterGroupByPath(
        "User parameter:BaseApp/Preferences/Websites"
    );
...
void StdCmdReportBug::activated(int iMsg)
{
    ...
    std::string url = hURLGrp->GetASCII("IssuesPage", "https://github.com/AuraCAD/AuraCAD/issues");
...
void StdCmdAnnotationLabel::activated(int)
{
    ...
    const auto* supportObj = AuraCAD_cast<App::DocumentObject*>(support);
    ...
    else if (auto* gf = AuraCAD_cast<App::GeoFeature*>(supportObj); supportObj && gf) {
    ...
    auto* label = AuraCAD_cast<App::AnnotationLabel*>(obj);
...
void CreateStdCommands()
{
    ...
    rcCmdMgr.addCommand(new StdCmdAuraCADWebsite());
    rcCmdMgr.addCommand(new StdCmdAuraCADDonation());
    rcCmdMgr.addCommand(new StdCmdAuraCADUserHub());
    rcCmdMgr.addCommand(new StdCmdAuraCADForum());
