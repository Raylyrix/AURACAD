...
PythonDebugModule::PythonDebugModule()
    : Py::ExtensionModule<PythonDebugModule>("AuraCADDbg")
{
    add_varargs_method(
    ...
    initialize("The AuraCAD Python debug module");
...
void PythonDebugStdout::init_type()
{
    ...
    behaviors().doc("Redirection of stdout to AuraCAD's Python debugger window");
...
void PythonDebugStderr::init_type()
{
    ...
    behaviors().doc("Redirection of stderr to AuraCAD's Python debugger window");
    // you must have overwritten the virtual functions
    behaviors().supportRepr();
...
void PythonDebugExcept::init_type()
{
    ...
    // you must have overwritten the virtual functions
    behaviors().supportRepr();
    add_varargs_method("AuraCAD_excepthook", &PythonDebugExcept::excepthook, "Custom exception handler");
}
...
struct PythonDebuggerP
{
    ...
    explicit PythonDebuggerP(PythonDebugger* that)
    {
        ...
        Py::Object func = pypde->getattr("AuraCAD_excepthook");
