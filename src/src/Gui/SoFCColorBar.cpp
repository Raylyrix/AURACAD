...
SO_NODE_ABSTRACT_SOURCE(SoauracadColorBarBase)

/*!
  Constructor.
*/
SoauracadColorBarBase::SoauracadColorBarBase()
    : _windowSize(0, 0)
{
    SO_NODE_CONSTRUCTOR(SoauracadColorBarBase);
}

/*!
  Destructor.
*/
SoauracadColorBarBase::~SoauracadColorBarBase() = default;

// doc from parent
void SoauracadColorBarBase::initClass()
{
    SO_NODE_INIT_ABSTRACT_CLASS(SoauracadColorBarBase, SoSeparator, "Separator");
}

void SoauracadColorBarBase::finish()
{
    atexit_cleanup();
}

void SoauracadColorBarBase::GLRenderBelowPath(SoGLRenderAction* action)
{
...
void SoauracadColorBarBase::setModified()
{
    _boxWidth = -1.0f;
}

void SoauracadColorBarBase::setFormat(const SoLabelTextFormat& fmt)
{
    format = fmt;
    applyFormat(fmt);
}

SoLabelTextFormat SoauracadColorBarBase::getFormat() const
{
    return format;
}

void SoauracadColorBarBase::applyFormat(const SoLabelTextFormat& fmt)
{
    boost::ignore_unused(fmt);
}

float SoauracadColorBarBase::getBoundingWidth(const SbVec2s& size)
{
...
float SoauracadColorBarBase::getBounds(const SbVec2s& size, float& fMinX, float& fMinY, float& fMaxX, float& fMaxY)
{
    // ratio of window width / height
    float fRatio = static_cast<float>(size[0]) / static_cast<float>(size[1]);

    // The cam height is set in SoauracadColorBarBase::getBoundingWidth to 10.
...
class SoauracadColorBarProxyObject: public QObject
{
public:
    explicit SoauracadColorBarProxyObject(SoauracadColorBar* b)
        : QObject(nullptr)
        , bar(b)
    {}
    ~SoauracadColorBarProxyObject() override = default;
...
private:
    SoauracadColorBar* bar;
};
}  // namespace Gui

SO_NODE_SOURCE(SoauracadColorBar)

/*!
  Constructor.
*/
SoauracadColorBar::SoauracadColorBar()
{
    SO_NODE_CONSTRUCTOR(SoauracadColorBar);
    ...
    _colorBars.push_back(new SoauracadColorGradient);
    _colorBars.push_back(new SoauracadColorLegend);
...
/*!
  Destructor.
*/
SoauracadColorBar::~SoauracadColorBar() = default;

// doc from parent
void SoauracadColorBar::initClass()
{
    SO_NODE_INIT_CLASS(SoauracadColorBar, SoauracadColorBarBase, "Separator");
}

void SoauracadColorBar::finish()
{
    atexit_cleanup();
}

SoauracadColorBarBase* SoauracadColorBar::getActiveBar() const
{
    int child = pColorMode->whichChild.getValue();
    return _colorBars[child];
}

void SoauracadColorBar::setFormat(const SoLabelTextFormat& fmt)
{
    for (auto it : _colorBars) {
        it->setFormat(fmt);
    }
...
void SoauracadColorBar::setViewportSize(const SbVec2s& size)
{
    boost::ignore_unused(size);
}

void SoauracadColorBar::setRange(float fMin, float fMax, int prec)
{
    for (auto it : _colorBars) {
...
void SoauracadColorBar::setOutsideGrayed(bool bVal)
{
    for (auto it : _colorBars) {
...
bool SoauracadColorBar::isVisible(float fVal) const
{
    return this->getActiveBar()->isVisible(fVal);
}

float SoauracadColorBar::getMinValue() const
{
    return this->getActiveBar()->getMinValue();
}

float SoauracadColorBar::getMaxValue() const
{
    return this->getActiveBar()->getMaxValue();
}

void SoauracadColorBar::triggerChange(SoauracadColorBarBase*)
{
    Notify(0);
}

void SoauracadColorBar::customize(SoauracadColorBarBase* child)
{
    try {
...
Base::Color SoauracadColorBar::getColor(float fVal) const
{
    return this->getActiveBar()->getColor(fVal);
}

void SoauracadColorBar::eventCallback(void* /*userdata*/, SoEventCallback* node)
{
...
void SoauracadColorBar::handleEvent(SoHandleEventAction* action)
{
        ...
        if ((e->getButton() == SoMouseButtonEvent::BUTTON1)) {
            if (e->getState() == SoButtonEvent::DOWN) {
                ...
                if (!_timer.isValid()) {
                    ...
                    QApplication::postEvent(new SoauracadColorBarProxyObject(this), new QEvent(QEvent::User));
                }
            }
        }
        // right mouse pressed
            ...
            if (e->getState() == SoButtonEvent::UP) {
                SoauracadColorBarBase* current = getActiveBar();
                QMenu menu;
                int i = 0;
                for (auto it : _colorBars) {
