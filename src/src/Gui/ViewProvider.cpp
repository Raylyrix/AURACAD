...
AuraCAD_LOG_LEVEL_INIT("ViewProvider", true, true)
...
void coinRemoveAllChildren(SoGroup* group)
{
    ...
    AuraCAD_TRACE("coin remove all children " << count);
    ...
    , toggleVisibilityMode(ToggleVisibilityMode::CanToggleVisibility)
{
    ...
    // SoauracadSeparator and SoauracadSelectionRoot can both track render cache setting.
    // We change to SoauracadSelectionRoot so that we can dynamically change full
    // selection mode (full highlight vs. boundbox). Note that comparing to
    // SoauracadSeparator, there are some small overhead with SoauracadSelectionRoot for
    // selection context tracking.
    //
    // pcRoot = new SoauracadSeparator(true);
    pcRoot = new SoauracadSelectionRoot(true);
    pcRoot->ref();
    pcModeSwitch = new SoSwitch();
    pcModeSwitch->ref();
    pcModeSwitch->setName("ModeSwitch");
    pcTransform = new SoauracadTransform();
    pcTransform->ref();
            ...
            switch (ke->getKey()) {
                case SoKeyboardEvent::ESCAPE:
                    if (self->keyPressed(press, ke->getKey())) {
                        ...
                        // (https://bitbucket.org/Coin3D/coin/pull-requests/119), auracad may crash if
                        ...
                        AuraCAD_WARN("Release all mouse buttons before exiting editing");
                    }
                    break;
                default:
...
{
    return SoauracadDB::writeNodesToString(pcRoot);
}
...
int ViewProvider::partialRender(const std::vector<std::string>& elements, bool clear)
{
    if (elements.empty()) {
        ...
        if (node) {
            AuraCAD_LOG("partial render clear");
    ...
    for (auto element : elements) {
        ...
        if (getDetailPath(element.c_str(), path, false, det)) {
            if (!hidden && !det) {
                AuraCAD_LOG("partial render element not found: " << element);
                continue;
            }
            AuraCAD_LOG("partial render (" << path->getLength() << "): " << element);
            if (!hidden) {
                action.setType(
