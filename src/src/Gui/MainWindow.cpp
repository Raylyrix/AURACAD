...
AuraCAD_LOG_LEVEL_INIT("MainWindow", false, true, true)
    ...
    : QMainWindow(parent, f /*WDestructiveClose*/)
{
    ...
    // Starts checked because AuraCAD shows bottom panels by default on first launch. On subsequent
...
void MainWindow::removeWindow(Gui::MDIView* view, bool close)
{
    if (view->currentViewMode() != MDIView::Child) {
        AuraCAD_WARN("tried to remove an MDIView that is not currently in child mode");
        return;
    }
...
void MainWindow::closeEvent(QCloseEvent* e)
{
    ...
    if (e->isAccepted()) {
        ...
        // https://forum.AuraCAD.org/viewtopic.php?f=8&t=67748
...
void MainWindow::delayedStartup()
{
    ...
    if (Gui::isInternalGuiTestRun()) {
        try {
            ...
            // delayedStartup().
            Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("AuraCADGuiTest"));
    ...
    if (SafeMode::SafeModeEnabled()) {
        auto safeModePopup = QMessageBox(
            ...
            tr("AuraCAD is now running in safe mode."),
            QMessageBox::Ok
        );
...
void MainWindow::registerQuickLookExtensions()
{
    ...
    // Get the path to AuraCAD.app/Contents/PlugIns
    QString appPath = QApplication::applicationDirPath();
    QString plugInsPath = appPath + "/../PlugIns";

    QString thumbnailExt = plugInsPath + "/AuraCADThumbnailExtension.appex";
    QString previewExt = plugInsPath + "/AuraCADPreviewExtension.appex";
    ...
    const QString thumbnailId = QStringLiteral("org.AuraCAD.AuraCAD.quicklook.thumbnail");
    const QString previewId = QStringLiteral("org.AuraCAD.AuraCAD.quicklook.preview");
...
void MainWindow::setWindowTitle(const QString& string)
{
    ...
    if (showVersion) {
        // set main window title with AuraCAD Version
