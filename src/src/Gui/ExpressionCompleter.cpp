...
AuraCAD_LOG_LEVEL_INIT("Completer", true, true)
...
QStringList ExpressionCompleter::splitPath(const QString& input) const
{
    ...
    while (true) {
        try {
            ...
            AuraCAD_TRACE("split path " << path << " error: " << except.what());
            if (retry == 0) {
...
{
    AuraCAD_TRACE("SlotUpdate:" << prefix.toUtf8().constData());
    ...
    AuraCAD_TRACE("Completion Prefix:" << completionPrefix.toUtf8().constData());
    // Set completion prefix
    setCompletionPrefix(completionPrefix);

    if (widget()->hasFocus()) {
        AuraCAD_TRACE("Complete on Prefix" << completionPrefix.toUtf8().constData());
        complete();
        AuraCAD_TRACE("Complete Done");
    }
