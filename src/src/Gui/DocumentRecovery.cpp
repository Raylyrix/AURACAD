...
QString DocumentRecovery::createProjectFile(const QString& documentXml)
{
    ...
    QFileInfo fi(source);
    QString dest = fi.dir().absoluteFilePath(QStringLiteral("AuraCAD_recovery_file.auracadstd"));
    ...
    if (!d->recovered) {
        ...
        for (size_t i = 0; i < docs.size(); ++i) {
            ...
            if (!docs[i] || !errs[i].empty()) {
                ...
                if (fi.fileName() == QLatin1String("AuraCAD_recovery_file.auracadstd")) {
...
DocumentRecoveryPrivate::Info DocumentRecoveryPrivate::getRecoveryInfo(const QFileInfo& fi) const
{
    ...
    QDir rec_dir(doc_dir.absoluteFilePath(QLatin1String("AuraCAD_recovery_files")));

    // compressed recovery file
    if (doc_dir.exists(QLatin1String("AuraCAD_recovery_file.auracadstd"))) {
        file = doc_dir.absoluteFilePath(QLatin1String("AuraCAD_recovery_file.auracadstd"));
    }
    ...
    info.xmlFile = doc_dir.absoluteFilePath(QLatin1String("AuraCAD_recovery_file.xml"));
    if (doc_dir.exists(QLatin1String("AuraCAD_recovery_file.xml"))) {
...
bool xmlFilesAreValid(const QString& auracadstdFile)
{
    try {
        zipios::ZipFile zf(auracadstdFile.toStdString());
...
void DocumentRecoveryFinder::checkDocumentDirs(QDir& tmp, const QList<QFileInfo>& dirs, const QString& fn)
{
    if (dirs.isEmpty()) {
        ...
        QString recovery_files = QStringLiteral("AuraCAD_recovery_files");
        for (QList<QFileInfo>::const_iterator it = dirs.cbegin(); it != dirs.cend(); ++it) {
            ...
            else if (doc_dir.exists(QLatin1String("AuraCAD_recovery_file.xml"))) {
                // store the transient directory in case it's not empty
                restoreDocFiles << *it;
            }
            // search for the 'AuraCAD_recovery_files' sub-directory and check that it's the only entry
