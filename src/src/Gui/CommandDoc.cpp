...
AuraCAD_LOG_LEVEL_INIT("Command", false)
...
void StdCmdOpen::activated(int iMsg)
{
    ...
    // Cram all formats AuraCAD can import under one label
    ...
    allSupportedFormats += QLatin1String(" *.auracadBak)");
    formatList += allSupportedFormats;

    const auto importFilters = App::GetApplication().getImportFilters();
    // Make sure auracadStd is the second entry in the format list
    auto auracadstdIt = importFilters.cend();
    for (auto it = importFilters.cbegin(); it != importFilters.cend(); ++it) {
        if (const auto auracad = it->first.find("*.auracadStd"); auracad != std::string::npos) {
            auracadstdIt = it;
            QString auracadstdFilter = QString::fromStdString(it->first);
            if (!auracadstdFilter.contains(QStringLiteral("*.auracadBak"), Qt::CaseInsensitive)) {
                auracadstdFilter.replace(")", QStringLiteral(" *.auracadBak)"));
            }
            formatList += auracadstdFilter;
            break;
        }
    }
    for (auto it = importFilters.cbegin(); it != importFilters.cend(); ++it) {
    ...
    // Open backup files as native documents (same data format as auracadStd).
    for (const QString& file : fileList) {
        if (!file.endsWith(QStringLiteral(".auracadBak"), Qt::CaseInsensitive)) {
            continue;
        }

        getGuiApplication()->setStatus(Gui::Application::UserInitiatedOpenDocument, true);
        getGuiApplication()->open(file.toUtf8(), "AuraCAD");
    ...
    fileList.erase(
        std::remove_if(
            ...
            fileList.end(),
            [](const QString& file) {
                return file.endsWith(QStringLiteral(".auracadBak"), Qt::CaseInsensitive);
            }
        ),
        fileList.end()
    );
...
void StdCmdImport::activated(int iMsg)
{
    ...
    const auto importFilters = App::GetApplication().getImportFilters();
    for (auto it = importFilters.cbegin(); it != importFilters.cend(); ++it) {
        if (it->first.find("*.auracadStd") == std::string::npos) {
            formatList += QString::fromStdString(it->first);
        }
    }
...
Format options are:
%F - the basename of the .auracadStd file (or the label, if it is not saved yet)
...
QString createDefaultExportBasename()
{
    ...
    QString auracadstdBasename = fi.completeBaseName();
    if (auracadstdBasename.isEmpty()) {
        auracadstdBasename = QString::fromStdString(
        ...
        if (c != QLatin1Char('%')) {
            ...
            if (i < exportFormatString.size() - 1) {
                ...
                if (formatChar == QLatin1Char('F')) {
                    defaultFilename.append(auracadstdBasename);
                }
...
void StdCmdExport::activated(int iMsg)
{
    ...
    for (const auto& filter : filterMap) {
        ...
        if (filter.first.find("(*.auracadStd)") == std::string::npos) {
    ...
    if (exportInfo.filename.empty() || exportInfo.generatedName || exportInfo.object != toExport) {
        // First, get the name and path of the current .auracadStd file, if there is one:
    ...
    : Command("Std_MergeProjects")
{
    ...
    sToolTipText = QT_TR_NOOP("Merges another AuraCAD document into the active one");
...
void StdCmdMergeProjects::activated(int iMsg)
{
    ...
    QString project = FileDialog::getOpenFileName(
        ...
        QStringList(QString::fromUtf8(QT_TR_NOOP("%1 document (*.auracadStd)")).arg(exe))
    );
    if (!project.isEmpty()) {
...
{
    sGroup = "Tools";
    sMenuText = QT_TR_NOOP("Export Dependency &Graphâ€¦");
...
void StdCmdSave::activated(int iMsg)
{
    ...
    if (App::Document* doc = App::GetApplication().getActiveDocument()) {
        ...
        if (filename.hasExtension("auracadbak")) {
...
{
    sGroup = "File";
    sMenuText = QT_TR_NOOP("Save a Cop&yâ€¦");
...
void StdCmdPrint::activated(int iMsg)
{
    ...
    if (getMainWindow()->activeWindow()) {
        getMainWindow()->showMessage(QObject::tr("Printingâ€¦"));
...
void StdCmdPrintPdf::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    if (getMainWindow()->activeWindow()) {
        getMainWindow()->showMessage(QObject::tr("Exporting PDFâ€¦"));
...
void StdCmdDelete::activated(int iMsg)
{
    ...
    try {
        ...
        for (auto& editDoc : editDocs) {
            auto vpedit = AuraCAD_cast<ViewProviderDocumentObject*>(editDoc->getInEdit());
                ...
                for (auto& sel : sels) {
                    ...
                    if (vp) {
                        ...
                        if (forceDeletion || vp->onDelete(sel.getSubNames())) {
                            ...
                            auracadMD_OBJ_DOC_CMD(obj, "removeObject('" << obj->getNameInDocument() << "')");
                        }
                    }
                }
            }
        ...
        if (!docs.empty()) {
            ...
            for (auto doc : docs) {
                auracadMD_DOC_CMD(doc, "recompute()");
            }
        }
    }
...
{
    sGroup = "Edit";
    sMenuText = QT_TR_NOOP("Ali&gn Toâ€¦");
...
protected:
    ...
    void pasteExpressions()
    {
        ...
        if (!boost::regex_search(tstart, m, rule)) {
            AuraCAD_WARN("No expression header found");
            return;
        }
        boost::cmatch m2;
        bool found = true;
        for (; found; m = m2) {
            ...
            if (!doc) {
                AuraCAD_WARN("Cannot find document '" << docName << "'");
                continue;
            }

            auto obj = doc->getObject(objName.c_str());
            if (!obj) {
                AuraCAD_WARN("Cannot find object '" << docName << '#' << objName << "'");
                continue;
            }
            ...
            if (!prop) {
                AuraCAD_WARN("Invalid property '" << docName << '#' << objName << '.' << propName << "'");
                continue;
            }
            ...
            try {
                ...
                AuraCAD_ERR(e.what() << std::endl << m[0].str());
                failed = true;
            }
        }
        if (failed) {
