...
ViewProviderGeometryObject::ViewProviderGeometryObject()
{
    ...
    pcBoundingBox = new Gui::SoauracadBoundingBox;
    pcBoundingBox->ref();
...
void ViewProviderGeometryObject::setSelectable(bool selectable)
{
    ...
    sa.setType(Gui::SoauracadSelection::getClassTypeId());
    sa.apply(pcRoot);

    SoPathList& pathList = sa.getPaths();

    for (int i = 0; i < pathList.getLength(); i++) {
        auto selNode = dynamic_cast<SoauracadSelection*>(pathList[i]->getTail());
        if (selectable) {
            if (selNode) {
                selNode->selectionMode = SoauracadSelection::SEL_ON;
                selNode->preselectionMode = SoauracadSelection::AUTO;
            }
        }
        else {
            if (selNode) {
                selNode->selectionMode = SoauracadSelection::SEL_OFF;
                selNode->preselectionMode = SoauracadSelection::OFF;
                selNode->selected = SoauracadSelection::NOTSELECTED;
            }
        }
    }
