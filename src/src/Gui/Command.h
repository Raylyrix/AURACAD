 ...
 * Example:
 * @code{.cpp}
 *      _auracadMD_DOC_CMD(Gui,doc,"getObject('" << objName << "')");
 * @endcode
 *
 ...
 * @endcode
 */
#define _auracadMD_DOC_CMD(_type, _doc, _cmd) \
    do { \
        auto __doc = _doc; \
 ...
 * @param _doc: pointer to a document
 * @param _cmd: command string, streamable
 * @sa _auracadMD_DOC_CMD()
 */
#define auracadMD_DOC_CMD(_doc, _cmd) _auracadMD_DOC_CMD(App, _doc, _cmd)
 ...
 * @param _cmd: command string, streamable
 */
#define _auracadMD_OBJ_DOC_CMD(_type, _obj, _cmd) \
    do { \
        auto __obj = _obj; \
        if (__obj) \
            _auracadMD_DOC_CMD(_type, __obj->getDocument(), _cmd); \
    } while (0)
 ...
 * @param _cmd: command string, streamable
 */
#define auracadMD_OBJ_DOC_CMD(_obj, _cmd) _auracadMD_OBJ_DOC_CMD(App, _obj, _cmd)

/** Runs a command for accessing an object's Gui::Document attribute or method
 * @param _obj: pointer to a DocumentObject
 * @param _cmd: command string, streamable
 */
#define auracadMD_VOBJ_DOC_CMD(_obj, _cmd) _auracadMD_OBJ_DOC_CMD(Gui, _obj, _cmd)
 ...
 * Example:
 * @code{.cpp}
 *      _auracadMD_OBJ_CMD(Gui,obj,"Visibility = " << (visible?"True":"False"));
 * @endcode
 *
 ...
 * @endcode
 */
#define _auracadMD_OBJ_CMD(_type, _cmd_type, _obj, _cmd) \
    do { \
        auto __obj = _obj; \
 ...
 * @param _obj: pointer to a DocumentObject
 * @param _cmd: command string, streamable
 * @sa _auracadMD_OBJ_CMD()
 */
#define auracadMD_OBJ_CMD(_obj, _cmd) _auracadMD_OBJ_CMD(App, Doc, _obj, _cmd)
 ...
 * @sa _auracadMD_OBJ_CMD()
 */
#define auracadMD_VOBJ_CMD(_obj, _cmd) _auracadMD_OBJ_CMD(Gui, Gui, _obj, _cmd)
 ...
 * Example:
 * @code{.cpp}
 *      auracadMD_OBJ_CMD2("Visibility = %s", obj, visible?"True":"False");
 * @endcode
 *
 * Translates to command (assuming obj's document name is 'DocName', obj's name
 * is 'ObjName', and visible is true):
 ...
 * @endcode
 */
#define auracadMD_OBJ_CMD2(_cmd, _obj, ...) \
    do { \
        auto __obj = _obj; \
        if (__obj && __obj->isAttachedToDocument()) { \
 ...
 * @param _cmd: command string, supporting printf like formatter
 * @param _obj: pointer to a DocumentObject
 * @sa auracadMD_OBJ_CMD2()
 */
#define auracadMD_VOBJ_CMD2(_cmd, _obj, ...) \
    do { \
        auto __obj = _obj; \
        if (__obj && __obj->isAttachedToDocument()) { \
            Gui::Command::doCommand( \
 ...
 * @param _obj: pointer to a DocumentObject
 *
 * Unlike other auracadMD macros, this macro editing the object using the current
 ...
 * external group.
 */
#define auracadMD_SET_EDIT(_obj) \
    do { \
        auto __obj = _obj; \
        if (__obj && __obj->isAttachedToDocument()) { \
            Gui::Command::doCommand( \
...
/// Hides an object
#define auracadMD_OBJ_HIDE(_obj) auracadMD_OBJ_CMD(_obj, "Visibility = False")

/// Shows an object
#define auracadMD_OBJ_SHOW(_obj) auracadMD_OBJ_CMD(_obj, "Visibility = True")
 ...
 * This lightweight class is the base class of all commands in AuraCAD. It represents the link
 * between the AuraCAD command framework and the QAction world of Qt.
 * @author Werner Mayer
 */
 ...
 * It contains also a lot of helper methods to make implementing commands for AuraCAD as easy as
 * possible.
 *
 ...
 *  This class manage all available commands in AuraCAD. All
