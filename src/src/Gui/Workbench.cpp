...
/** \defgroup workbench Workbench Framework
    ...
    AuraCAD provides the possibility to have one or more workbenches for a module.
 ...
 * Once you have implemented your workbench class you have to register it to make it known to the
 AuraCAD core system. You must make sure that the step
 ...
 * your workbench, if not, the default AuraCAD icon is taken, and finally the most important method
 ...
 * \note You must make sure to choose a unique name for your workbench (in this example "My
 workbench"). Since AuraCAD doesn't provide a mechanism for
 ...
 * AuraCAD and saves memory.
 *
 * At startup AuraCAD scans all module directories and invokes InitGui.py. So an item for a
 ...
 *
 * AuraCAD provides also the possibility to define pure Python workbenches. Such workbenches are
 temporarily only and are lost after exiting
 * the AuraCAD session. But if you want to keep your Python workbench you can write a macro and
 attach it with a user defined button or just
 * perform the macro during the next AuraCAD session.
 ...
 * dir(w)                                                     # lists all available function of the
 object
 * AuraCADGui.addWorkbench(w)                                 # Creates an item for our workbench
 now
...
MenuItem* StdWorkbench::setupMenuBar() const
{
    ...
    *help << "Std_WhatsThis"
          ...
          << "Std_AuraCADUserHub" << "Std_AuraCADForum" << "Std_ReportBug" << "Separator"
          << "Std_RestartInSafeMode" << "Separator"
          << "Std_DevHandbook" << "Std_PythonHelp" << "Separator"
          << "Std_AuraCADWebsite" << "Std_AuraCADDonation" << "Std_About";
