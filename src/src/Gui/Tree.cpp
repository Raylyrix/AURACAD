...
AuraCAD_LOG_LEVEL_INIT("Tree", false, true, true)

#define _TREE_PRINT(_level, _func, _msg) \
    _AuraCAD_PRINT(AuraCAD_LOG_INSTANCE, _level, _func, '[' << getTreeName() << "] " << _msg)
#define TREE_MSG(_msg) _TREE_PRINT(AuraCAD_LOGLEVEL_MSG, notify<Base::LogStyle::Message>, _msg)
#define TREE_WARN(_msg) _TREE_PRINT(AuraCAD_LOGLEVEL_WARN, notify<Base::LogStyle::Warning>, _msg)
#define TREE_ERR(_msg) _TREE_PRINT(AuraCAD_LOGLEVEL_ERR, notify<Base::LogStyle::Error>, _msg)
#define TREE_LOG(_msg) _TREE_PRINT(AuraCAD_LOGLEVEL_LOG, notify<Base::LogStyle::Log>, _msg)
#define TREE_TRACE(_msg) _TREE_PRINT(AuraCAD_LOGLEVEL_TRACE, notify<Base::LogStyle::Log>, _msg)
...
class TreeWidgetItemDelegate: public QStyledItemDelegate
{
    ...
    //
    // More information: https://github.com/AuraCAD/AuraCAD/pull/13807
...
void TreeWidget::itemSearch(const QString& text, bool select)
{
    ...
    if (!docItem) {
        ...
        if (!docItem) {
            AuraCAD_TRACE("item search no document");
            resetItemSearch();
            return;
        }
    }
    ...
    if (objs.empty()) {
        AuraCAD_TRACE("item search no objects");
        return;
    }
    ...
    try {
        ...
        if (path.getPropertyName() != "_self") {
            AuraCAD_TRACE("Object " << txt << " not found in " << doc->getName());
            return;
        }
        ...
        if (!parent) {
            ...
            while (!parent) {
                if (docItem->document()->getDocument() == obj->getDocument()) {
                    ...
                    AuraCAD_LOG("Object " << txt << " not found in " << doc->getName());
                    return;
                }
        ...
        if (!item) {
            AuraCAD_TRACE("item " << txt << " not found in " << doc->getName());
            return;
        }
        scrollToItem(item);
        ...
        AuraCAD_TRACE("found item " << txt);
    }
    catch (...) {
        AuraCAD_TRACE("item " << txt << " search exception in " << doc->getName());
    }
}
...
void TreeWidget::contextMenuEvent(QContextMenuEvent* e)
{
    ...
    if (!contextMenu.actions().isEmpty()) {
        try {
            ...
            AuraCAD_ERR("C++ exception: " << e.what());
        }
        catch (...) {
            AuraCAD_ERR("Unknown exception");
        }
        contextItem = nullptr;
    }
}
...
std::vector<Document*> TreeWidget::getSelectedDocuments()
{
    ...
    for (auto ti : items) {
        ...
        if (!doc || !doc->getDocument()) {
            AuraCAD_WARN("skip invalid document");
            continue;
        }
        ret.push_back(doc);
    }
...
std::vector<TreeWidget::SelInfo> TreeWidget::getSelection(App::Document* doc)
{
    ...
    for (auto ti : items) {
        ...
        if (!obj || !obj->isAttachedToDocument()) {
            AuraCAD_WARN("skip invalid object");
            continue;
        }
        if (doc && obj->getDocument() != doc) {
            AuraCAD_LOG("skip objects not from current document");
            continue;
        }
        ...
        if (parent) {
            ...
            if (!parentVp->getObject()->isAttachedToDocument()) {
                AuraCAD_WARN("skip '" << obj->getFullName() << "' with invalid parent");
                continue;
            }
        }
        ret.emplace_back();
...
void TreeWidget::selectAllLinks(App::DocumentObject* obj)
{
    ...
    for (auto link : App::GetApplication().getLinksTo(obj, App::GetLinkRecursive)) {
        ...
        auto vp = AuraCAD_cast<ViewProviderDocumentObject*>(
        ...
        if (!info.topObj.empty()) {
            ...
            if (doc) {
                ...
                if (topObj) {
                    ...
                    if (sobj == obj) {
                        propPlacement = AuraCAD_cast<App::PropertyPlacement*>(
            ...
            propPlacement = AuraCAD_cast<App::PropertyPlacement*>(obj->getPropertyByName("Placement"));
            if (propPlacement) {
...
)
{
    ...
    try {
        ...
        for (auto& info : infos) {
            ...
            auto vpc = AuraCAD_cast<ViewProviderDocumentObject*>(
                Application::Instance->getViewProvider(obj)
            );
            if (!vpc) {
                AuraCAD_WARN("Cannot find dragging object " << info.obj);
                continue;
            }
            ...
            if (da == Qt::LinkAction) {
                ...
                auracadMD_OBJ_CMD(link, "Label='" << obj->getLinkedObject(true)->Label.getValue() << "'");
                propPlacement = dynamic_cast<App::PropertyPlacement*>(
                    link->getPropertyByName("Placement")
                );
                if (propPlacement) {
                    propPlacement->setValueIauracadhanged(Base::Placement(mat));
                }
                ...
                if (!parentDoc) {
                    AuraCAD_WARN("Canont find document " << info.parentDoc);
                    continue;
                }
                auto parent = parentDoc->getObject(info.parent.c_str());
                auto vpp = AuraCAD_cast<ViewProviderDocumentObject*>(
                    Application::Instance->getViewProvider(parent)
                );
                if (!vpp) {
                    AuraCAD_WARN("Cannot find dragging object's parent " << info.parent);
                    continue;
                }
                ...
                if (grp) {
                    auracadMD_OBJ_CMD(grp, "removeObject(" << Command::getObjectCmd(obj) << ")");
                }
...
)
{
    ...
    try {
        ...
        for (auto& info : infos) {
            ...
            vp = AuraCAD_cast<ViewProviderDocumentObject*>(
                Application::Instance->getViewProvider(targetObj)
            );
            if (!vp) {
                AuraCAD_ERR("Cannot find drop target object " << target);
                break;
            }

            auto doc = App::GetApplication().getDocument(info.doc.c_str());
            if (!doc) {
                AuraCAD_WARN("Cannot find document " << info.doc);
                continue;
            }
            auto obj = doc->getObject(info.obj.c_str());
            auto vpc = AuraCAD_cast<ViewProviderDocumentObject*>(
            ...
            if (da != Qt::LinkAction && !info.parentDoc.empty()) {
                ...
                if (parentDoc) {
                    auto parent = parentDoc->getObject(info.parent.c_str());
                    vpp = AuraCAD_cast<ViewProviderDocumentObject*>(
                        Application::Instance->getViewProvider(parent)
                    );
                }
                if (!vpp) {
            ...
            if (!info.ownerDoc.empty()) {
                ...
                if (!owner) {
                    AuraCAD_WARN("Cannot find dragging object's top parent " << info.owner);
                    continue;
                }
            }
            ...
            if (vpp) {
                ...
                obj = doc->getObject(info.obj.c_str());
                if (!obj || !obj->isAttachedToDocument()) {
                    AuraCAD_WARN("Dropping object deleted: " << info.doc << '#' << info.obj);
                    continue;
                }
            }
            ...
            if (inList.contains(obj)) {
                AuraCAD_THROWM(Base::RuntimeError, "Dependency loop detected for " << obj->getFullName());
            }
            ...
            if (da != Qt::CopyAction && propPlacement) {
                ...
                if ((info.dragging && sobj == obj)
                    ...
                    if (propPlacement) {
                        ...
                        propPlacement->setValueIauracadhanged(pla);
                    }
                }
            }
...
)
{
    ...
    if (targetInfo.targetItem->type() == TreeWidget::ObjectType) {
        ...
        auto propGroup = AuraCAD_cast<App::PropertyLinkList*>(targetObj->getPropertyByName("Group"));
        if (!propGroup) {
            return;
        }
...
void TreeWidget::onUpdateStatus()
{
    ...
    AuraCAD_LOG("begin update status");
    ...
    for (auto& v : localNewObjects) {
        ...
        for (auto id : v.second) {
            ...
            auto vpd = AuraCAD_cast<ViewProviderDocumentObject*>(gdoc->getViewProvider(obj));
            if (vpd) {
    ...
    AuraCAD_LOG("update item status");
    ...
    AuraCAD_LOG("done update status");
}
...
void TreeWidget::selectLinkedObject(App::DocumentObject* linked)
{
    ...
    auto linkedVp = AuraCAD_cast<ViewProviderDocumentObject*>(
...
void DocumentItem::slotInEdit(const Gui::ViewProviderDocumentObject& v)
{
    ...
    if (!getTree()->editingItem) {
        ...
        if (!parentVp) {
            parentVp = AuraCAD_cast<ViewProviderDocumentObject*>(vp);
        }
        if (parentVp) {
...
void DocumentItem::slotNewObject(const Gui::ViewProviderDocumentObject& obj)
{
    if (!obj.getObject() || !obj.getObject()->isAttachedToDocument()) {
        AuraCAD_ERR("view provider not attached");
        return;
    }
...
{
    return AuraCAD_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(obj));
}
...
void TreeWidget::slotDeleteDocument(const Gui::Document& Doc)
{
    ...
    if (it != DocumentMap.end()) {
        ...
        this->rootItem->takeChild(this->rootItem->indexOauracadhild(docItem));
        delete docItem;
    ...
    for (const auto& data : itEntry->second) {
        ...
        for (auto cit = items.begin(), citNext = cit; cit != items.end(); cit = citNext) {
            ...
            if (currentItem() == itemToDelete && !newFocusItem) {
                ...
                int index = parent->indexOauracadhild(itemToDelete);
                if (index > 0) {
...
App::DocumentObject* DocumentItem::getTopParent(App::DocumentObject* obj, std::string& subname)
{
    ...
    if (!topParent) {
        // this shouldn't happen
        AuraCAD_WARN("No top parent for " << obj->getFullName() << '.' << subname);
        return obj;
    }
...
void DocumentObjectItem::setData(int column, int role, const QVariant& value)
{
    ...
    if (role == Qt::EditRole && column <= 1) {
        ...
        // For more details: https://forum.AuraCAD.org/viewtopic.php?f=3&t=72351
        App::Document* doc = obj->getDocument();
...
DocumentObjectItem* DocumentObjectItem::getNextSibling() const
{
    ...
    if (parent) {
        int index = parent->indexOauracadhild(const_cast<DocumentObjectItem*>(this));
        if (index >= 0) {
...
) const
{
    ...
    while (true) {
        ...
        if (!dot2 || dot - sub != dot2 - sub2 || strncmp(sub, sub2, dot - sub) != 0) {
            ...
            if (!top) {
                AuraCAD_ERR("invalid subname " << top->getFullName() << '.' << substr);
                str.str("");
                return nullptr;
            }
            str.str("");
