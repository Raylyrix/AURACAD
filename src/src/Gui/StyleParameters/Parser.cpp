...
std::unique_ptr<Expr> Parser::parseFactor()
{
    ...
    if (match('(')) {
        ...
        // 1. If we see `identifier:` pattern â†’ definitely a tuple
        if (peekNamedElement()) {
            expr = parseTuple();
        }
        else {
            ...
            // If followed by `,` â†’ reinterpret as tuple with this as first element
            if (pos < input.size() && input[pos] == ',') {
                ...
                // If followed by `)` â†’ grouped expression (backward compatible)
                if (!match(')')) {
