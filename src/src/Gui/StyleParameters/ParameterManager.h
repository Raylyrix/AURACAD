...
public:
    ...
    AuraCAD_DEFAULT_MOVE(ParameterSource);
    AuraCAD_DISABLE_COPY(ParameterSource);
...
/**
 * @brief Built-in parameter source that reads from AuraCAD's parameter system.
 *
 * This source provides access to predefined parameters that are stored in
 * AuraCAD's global parameter system. These parameters are typically defined
