...
void ViewProviderDocumentObject::onChanged(const App::Property* prop)
{
    if (prop == &DisplayMode) {
        ...
        if (getRoot()->isOfType(SoauracadSelectionRoot::getClassTypeId())) {
            static_cast<SoauracadSelectionRoot*>(getRoot())->selectionStyle = SelectionStyle.getValue()
                ? SoauracadSelectionRoot::Box
                : SoauracadSelectionRoot::Full;
        }
    }
    ...
    if (prop && !prop->testStatus(App::Property::NoModify) && pcDocument
        ...
        if (prop) {
            AuraCAD_LOG(prop->getFullName() << " changed");
        }
...
int ViewProviderDocumentObject::replaceObject(App::DocumentObject* oldObj, App::DocumentObject* newObj)
{
    ...
    if (!obj || !obj->isAttachedToDocument()) {
        AuraCAD_THROWM(Base::RuntimeError, "View provider not attached");
    }
