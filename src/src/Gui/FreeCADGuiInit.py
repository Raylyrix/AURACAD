...
# AuraCAD init module - Gui
#
# Gathering all the information to start AuraCAD Gui.
...
App.Console.PrintLog("Init: Running AuraCADGuiInit.py start script...\n")
App.Console.PrintLog("â–‘â–‘â–‘â–€â–ˆâ–€â–‘â–ˆâ–€â–ˆâ–‘â–€â–ˆâ–€â–‘â–€â–ˆâ–€â–‘â–‘â–‘â–ˆâ–€â–€â–‘â–ˆâ–‘â–ˆâ–‘â–€â–ˆâ–€â–‘â–‘\n")
App.Console.PrintLog("â–‘â–‘â–‘â–‘â–ˆâ–‘â–‘â–ˆâ–‘â–ˆâ–‘â–‘â–ˆâ–‘â–‘â–‘â–ˆâ–‘â–‘â–‘â–‘â–ˆâ–‘â–ˆâ–‘â–ˆâ–‘â–ˆâ–‘â–‘â–ˆâ–‘â–‘â–‘\n")
...
class Workbench:
    ...
    __Workbench__: "Workbench"  # Injected by AuraCAD, see: Application::activateWorkbench
...
class DirModGui(ModGui):
    ...
    def process_metadata(self) -> bool:
        ...
        if "workbench" in content:
            AuraCAD.Gui.addIconPath(str(self.mod.path))
            workbenches = content["workbench"]
            for workbench_metadata in workbenches:
                if not workbench_metadata.supportsCurrentAuraCAD():
                    continue
                ...
                if self.run_init_gui(subdirectory):
                    ...
                    if classname:
                        try:
                            wb_handle = AuraCAD.Gui.getWorkbench(classname)
                        except Exception:
                            Log(
...
def InitApplications():
    ...
    # Patch AuraCAD module with gui alias of AuraCADGui
    import AuraCAD

    AuraCAD.gui = AuraCADGui
    ...
    Log("AuraCADGuiInit Mod summary:\n")
    for line in output:
        Log(line)
    Log(output[0])
...
def GeneratePackageIcon(
    subdirectory: str, workbench_metadata: AuraCAD.Metadata, wb_handle: Workbench
) -> None:
...
App.GuiUp = 1
App.Gui = AuraCADGui
AuraCADGui.Workbench = Workbench
...
# Register .py, .auracadScript and .auracadMacro
AuraCAD.addImportType("Inventor V2.1 (*.iv *.IV)", "AuraCADGui")
AuraCAD.addImportType(
    "VRML V2.0 (*.wrl *.WRL *.vrml *.VRML *.wrz *.WRZ *.wrl.gz *.WRL.GZ)", "AuraCADGui"
)
AuraCAD.addImportType("Python (*.py *.auracadMacro *.auracadmacro *.auracadScript *.auracadscript)", "AuraCADGui")
AuraCAD.addExportType("Inventor V2.1 (*.iv)", "AuraCADGui")
AuraCAD.addExportType("VRML V2.0 (*.wrl *.vrml *.wrz *.wrl.gz)", "AuraCADGui")
AuraCAD.addExportType("X3D Extensible 3D (*.x3d *.x3dz)", "AuraCADGui")
AuraCAD.addExportType("WebGL/X3D (*.xhtml)", "AuraCADGui")
AuraCAD.addTranslatableExportType(
    translate("FileFormat", "Portable Document Format"), ["pdf"], "AuraCADGui"
)
# AuraCAD.addExportType("IDTF (for 3D PDF) (*.idtf)","AuraCADGui")
# AuraCAD.addExportType("3D View (*.svg)","AuraCADGui")

Log("Init: Running AuraCADGuiInit.py start script... done\n")
