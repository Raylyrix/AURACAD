...
#include "SoauracadOffscreenRenderer.h"
...
AuraCAD_LOG_LEVEL_INIT("3DViewer", true, true)

// #define AuraCAD_LOGGING_CB
 ...
 * Therefore an overview and some remarks to the AuraCAD 3D viewing system.
 *
...
void View3DInventorViewer::init()
{
    ...
    // Background stuff
    pcBackGround = new SoauracadBackgroundGradient;
    pcBackGround->ref();
    ...
    // NOTE: For every mouse click event the SoauracadUnifiedSelection searches for the picked
    ...
    selectionRoot = new Gui::SoauracadUnifiedSelection();
    ...
    // increase reauracadount before passing it to setScenegraph(), to avoid
...
#if defined(AuraCAD_DEBUG) && defined(AuraCAD_LOGGING_CB)
    ...
    // the SoauracadSelection::BOX style is set
    //
    // Important note:
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=10&t=7486&start=120#p74398 and for
    // the fix and some details what happens behind the scene have a look at this
    // https://forum.AuraCAD.org/viewtopic.php?f=10&t=7486&p=74777#p74736
...
View3DInventorViewer::~View3DInventorViewer()
{
    ...
    // See https://forum.AuraCAD.org/viewtopic.php?f=3&t=39720
...
void View3DInventorViewer::onSelectionChanged(const SelectionChanges& reason)
{
    ...
    if (Reason.Type == SelectionChanges::RmvPreselect
        ...
        SoauracadPreselectionAction preselectionAction(Reason);
        preselectionAction.apply(pcViewProviderRoot);
    }
    else {
        SoauracadSelectionAction selectionAction(Reason);
...
void View3DInventorViewer::resetEditingRoot(bool updateLinks)
{
    ...
    if (root->getNumChildren()) {
        AuraCAD_ERR("WARNING!!! Editing view provider root node is tampered");
    }
...
void View3DInventorViewer::setViewportCB(void* ud, SoAction* action)
{
    ...
    if (action->isOfType(SoGLRenderAction::getClassTypeId())) {
        SoauracadOffscreenRenderer& renderer = SoauracadOffscreenRenderer::instance();
...
void View3DInventorViewer::setGradientBackground(View3DInventorViewer::Background grad)
{
    switch (grad) {
        ...
        case Background::LinearGradient:
            pcBackGround->setGradient(SoauracadBackgroundGradient::LINEAR);
            if (backgroundroot->findChild(pcBackGround) == -1) {
                backgroundroot->addChild(pcBackGround);
            }
            break;
        case Background::RadialGradient:
            pcBackGround->setGradient(SoauracadBackgroundGradient::RADIAL);
            if (backgroundroot->findChild(pcBackGround) == -1) {
                backgroundroot->addChild(pcBackGround);
            }
            break;
...
View3DInventorViewer::Background View3DInventorViewer::getGradientBackground() const
{
    ...
    if (pcBackGround->getGradient() == SoauracadBackgroundGradient::LINEAR) {
...
void View3DInventorViewer::setRenderCache(int mode)
{
    ...
    if (mode < 0) {
        ...
        // https://forum.AuraCAD.org/viewtopic.php?f=18&t=43305&start=10#p412537
    ...
    SoauracadSeparator::setCacheMode(caching);
}
...
void View3DInventorViewer::setAxisCross(bool on)
{
    ...
    if (on) {
        if (!axisGroup) {
            using enum SoauracadPlacementIndicatorKit::Part;

            constexpr float axisCrossLength = 2.0F;

            auto axisCrossKit = new Gui::SoauracadPlacementIndicatorKit();
...
bool View3DInventorViewer::dumpToFile(SoNode* node, const char* filename, bool binary) const
{
    ...
    if (fi.hasExtension({"idtf", "svg"})) {
        ...
        if (fi.hasExtension("svg")) {
            vo = std::unique_ptr<SoVectorizeAction>(new SoauracadVectorizeSVGAction());
        }
        else if (fi.hasExtension("idtf")) {
            vo = std::unique_ptr<SoVectorizeAction>(new SoauracadVectorizeU3DAction());
        }
        ...
        ret = SoauracadDB::writeToFile(node, filename, binary);
    }
...
/**
 * Sets the SoauracadInteractiveElement to \a true.
 */
void View3DInventorViewer::interactionStartCB(void* ud, SoQTQuarterAdaptor* viewer)
{
    ...
    SoauracadInteractiveElement::set(glra->getState(), viewer->getSceneGraph(), true);
}

/**
 * Sets the SoauracadInteractiveElement to \a false and forces a redraw.
 */
void View3DInventorViewer::interactionFinishCB(void* ud, SoQTQuarterAdaptor* viewer)
{
    Q_UNUSED(ud)
    SoGLRenderAction* glra = viewer->getSoRenderManager()->getGLRenderAction();
    SoauracadInteractiveElement::set(glra->getState(), viewer->getSceneGraph(), false);
    viewer->redraw();
}
...
// https://forum.AuraCAD.org/viewtopic.php?f=3&t=6037&hilit=transparency
...
void View3DInventorViewer::renderScene()
{
    ...
    // https://github.com/AuraCAD/AuraCAD/issues/8341
    // https://github.com/AuraCAD/AuraCAD/issues/6177
...
bool View3DInventorViewer::setCamera(const char* pCamera)
{
    ...
    if (!CamViewer) {
        throw Base::RuntimeError("No camera set so farâ€¦");
    }
...
void View3DInventorViewer::drawSingleBackground(const QColor& col)
{
    ...
    // See also: https://github.com/AuraCAD/AuraCAD/pull/9356#issuecomment-1529521654
    glMatrixMode(GL_PROJECTION);
    glPushMatrix();
    glLoadIdentity();
    glOrtho(-1, 1, -1, 1, -1, 1);
