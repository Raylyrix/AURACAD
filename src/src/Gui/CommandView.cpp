...
void StdCmdFreezeViews::activated(int iMsg)
{
    ...
    if (iMsg == 0) {
        ...
        // Create a new view
        auto* view = AuraCAD_cast<MDIViewWithCamera*>(getGuiApplication()->activeView());
        if (!view) {
            return;
        }
...
{
    sGroup = "Standard-View";
    sMenuText = QT_TR_NOOP("AuraCAD VR");
    sToolTipText = QT_TR_NOOP("Extends the AuraCAD 3D Window to a VR device");
...
{
    return AuraCAD_cast<MDIViewWithCamera*>(getGuiApplication()->activeView());
}
...
)
{
    ...
    for (auto& sub : subs) {
        ...
        auto svp = AuraCAD_cast<ViewProviderDocumentObject*>(
...
void StdCmdTreeSelectAllInstances::activated(int iMsg)
{
    ...
    auto vpd = AuraCAD_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(obj));
    if (!vpd) {
        return;
    }
...
void StdCmdClarifySelection::activated(int iMsg)
{
    ...
    // Get the active view
    auto view3d = AuraCAD_cast<View3DInventor*>(Application::Instance->activeView());
    if (!view3d) {
        return;
    }
    ...
    for (int i = 0; i < pplist.getLength(); ++i) {
        ...
        auto vpDoc = AuraCAD_cast<Gui::ViewProviderDocumentObject*>(vp);
        if (!vpDoc) {
            continue;
        }
