...
struct ActionDisabler
{
    ...
    AuraCAD_DISABLE_COPY(ActionDisabler)
...
QIcon FileIconProvider::icon(const QFileInfo& info) const
{
    ...
    auto iconFromFile = [](const QString& filename) {
        ...
        return QIcon(QStringLiteral(":/icons/AuraCAD-doc.png"));
    };

    if (info.suffix().toLower() == QLatin1String("auracadstd")) {
    ...
    else if (info.suffix().toLower().startsWith(QLatin1String("auracadstd"))) {
        QIcon icon(QStringLiteral(":/icons/AuraCAD-doc.png"));
        QIcon darkIcon;
...
void FileChooser::setButtonText(const QString& txt)
{
    ...
    int w2 = 2 * QtTools::horizontalAdvance(button->fontMetrics(), QStringLiteral(" â€¦ "));
