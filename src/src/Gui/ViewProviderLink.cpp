...
AuraCAD_LOG_LEVEL_INIT("App::Link", true, true)
...
# define appendPath(_path, _node) \
     do { \
         if (!appendPathSafe(_path, _node)) \
             AuraCAD_ERR("LinkView: coin path error"); \
     } while (0)
#else
...
public:
    ...
    explicit LinkInfo(ViewProviderDocumentObject* vp)
        ...
        , pcLinked(vp)
    {
        AuraCAD_LOG("new link to " << pcLinked->getObject()->getFullName());
        // NOLINTBEGIN
    ...
    {
        AuraCAD_LOG("link detach " << getLinkedNameSafe());
    ...
    inline void release()
    {
        ...
        if (r == 0) {
            ...
            if (pcLinked) {
                AuraCAD_LOG("link release " << getLinkedNameSafe());
    ...
    SoSeparator* getSnapshot(int type, bool update = false)
    {
        ...
        if (pcSnapshot) {
            ...
            if (ViewParams::instance()->getUseSelectionRoot()) {
                pcSnapshot = new SoauracadSelectionRoot;
            }
            else {
...
#define CHECK_NAME(_name, _end) \
    ...
    bool getDetail(bool checkname, int type, const char* subname, SoDetail*& det, SoFullPath* path) const
    {
        ...
        while (true) {
            ...
            if (!ssobj) {
                AuraCAD_ERR("invalid sub name " << nextsub << " of object " << sobj->getFullName());
                return false;
            }
            sobj = ssobj;
            auto vp = Application::Instance->getViewProvider(sobj);
            if (!vp) {
                AuraCAD_ERR("cannot find view provider of " << sobj->getFullName());
                return false;
            }
            if (vp->getChildRoot()) {
...
{
    if (linkInfo) {
        linkInfo->pcLinked = AuraCAD_cast<ViewProviderDocumentObject*>(getExtendedContainer());
        linkInfo->update();
    }
}
...
{
    auto owner = AuraCAD_cast<ViewProviderDocumentObject*>(getExtendedContainer());
    if (owner && linkInfo) {
...
{
    if (linkInfo) {
        AuraCAD_TRACE("linked finish restoing");
        linkInfo->update();
    }
}
...
public:
    ...
    SubInfo(LinkView& handle)
        : handle(handle)
    {
        pcNode = new SoauracadSelectionRoot(true);
...
public:
    ...
    CoinPtr<SoauracadSelectionRoot> pcRoot;
    CoinPtr<SoTransform> pcTransform;
    int groupIndex = -1;
    bool isGroup = false;
    ...
    {
        pcTransform = new SoTransform;
        pcRoot = new SoauracadSelectionRoot(true);
...
LinkView::LinkView()
    ...
    , autoSubLink(true)
{
    pcLinkRoot = new SoauracadSelectionRoot;
}
...
void LinkView::setNodeType(SnapshotType type, bool sublink)
{
    ...
    if (nodeType >= 0 && type < 0) {
        ...
        replaceLinkedRoot(CoinPtr<SoSeparator>(new SoauracadSelectionRoot));
    }
...
void LinkView::updateLink()
{
    ...
    if (linkOwner && linkOwner->isLinked() && linkOwner->pcLinked->isRestoring()) {
        AuraCAD_TRACE("restoring '" << linkOwner->pcLinked->getObject()->getFullName() << "'");
        return;
    }
    ...
    if (!linkedRoot) {
        linkedRoot = new SoauracadSelectionRoot;
    }
    else {
        SoSelectionElementAction action(SoSelectionElementAction::None, true);
...
void ViewProviderLink::onChanged(const App::Property* prop)
{
    ...
    if (prop == &ChildViewProvider) {
        childVp = AuraCAD_cast<ViewProviderDocumentObject*>(ChildViewProvider.getObject().get());
...
void ViewProviderLink::updateDataPrivate(App::LinkBaseExtension* ext, const App::Property* prop)
{
    ...
    if (prop == &ext->_ChildCache) {
        ...
        if (!ext->_getShowElementValue()) {
            ...
            auto linked = AuraCAD_cast<ViewProviderDocumentObject*>(getLinkedView(true, ext));
            ...
            if (!elements.empty()) {
                ...
                for (size_t i = 0; i < elements.size(); ++i) {
                    auto element = AuraCAD_cast<App::LinkElement*>(elements[i]);
                    if (!element) {
                        continue;
                    }
                    auto vp = AuraCAD_cast<ViewProviderLink*>(
...
void ViewProviderLink::updateElementList(App::LinkBaseExtension* ext)
{
    ...
    if (OverrideMaterialList.getSize() || MaterialList.getSize()) {
        ...
        for (auto obj : elements) {
            ++i;
            auto vp = AuraCAD_cast<ViewProviderLink*>(Application::Instance->getViewProvider(obj));
            if (!vp) {
                continue;
            }
...
{
    AuraCAD_TRACE("finish restoring");
    auto ext = getLinkExtension();
    if (!ext) {
        return;
    }
...
) const
{
    ...
    if (!hasSubName && linkView->isLinked()) {
        auto linked = getLinkedView(false, ext);
        if (linked) {
            auto linkedVdp = AuraCAD_cast<ViewProviderDocumentObject*>(linked);
            if (linkedVdp) {
    ...
    if (obj->getDocument() != getObject()->getDocument()
        && !AuraCAD_cast<App::PropertyXLink*>(ext->getLinkedObjectProperty())) {
        return false;
    }
...
{
    if (linkEdit(ext)) {
        if (auto linkvp = AuraCAD_cast<ViewProviderLink*>(linkView->getLinkedView())) {
...
bool ViewProviderLink::initDraggingPlacement()
{
            ...
            if (feature.hasAttr(fname)) {
                ...
                if (ret.isTuple()) {
                    ...
                    if (!PyArg_ParseTuple(
                        ...
                        AuraCAD_ERR("initDraggingPlacement() expects return of type tuple(matrix,placement,boundbox)");
                        return false;
                    }
    ...
    auto ext = getLinkExtension();
    if (!ext) {
        AuraCAD_ERR("no link extension");
        return false;
    }
    if (!ext->hasPlacement()) {
        AuraCAD_ERR("no placement");
        return false;
    }
    ...
    if (!Application::Instance->isInEdit(getDocument())) {
        AuraCAD_ERR("document is not in edit");
        return false;
    }
    ...
    if (ext->getLinkTransformValue() && ext->getLinkedObjectValue()) {
        propPla = AuraCAD_cast<App::PropertyPlacement*>(
...
ViewProvider* ViewProviderLink::startEditing(int mode)
{
    ...
    if (!linkEdit()) {
        AuraCAD_ERR("unsupported edit mode " << mode);
        return nullptr;
    }
    ...
    if (!doc) {
        AuraCAD_ERR("no editing document");
        return nullptr;
    }
    ...
    if (!linked || linked == getObject()) {
        AuraCAD_ERR("no linked object");
        return nullptr;
    }
    auto vpd = AuraCAD_cast<ViewProviderDocumentObject*>(
        Application::Instance->getViewProvider(linked)
    );
    if (!vpd) {
        AuraCAD_ERR("no linked viewprovider");
        return nullptr;
    }
...
std::map<std::string, Base::Color> ViewProviderLink::getElementColors(const char* subname) const
{
    ...
    if (!wildcard.empty()) {
        ...
        while (true) {
            ...
            auto next = AuraCAD_cast<ViewProviderLink*>(Application::Instance->getViewProvider(link));
            if (!next) {
                break;
            }
...
) const
{
    ...
    auto res = AuraCAD_cast<ViewProviderDocumentObject*>(
        Application::Instance->getViewProvider(linked)
    );
    if (res) {
        return res;
