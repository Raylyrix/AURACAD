    ...
    ) const
    {
        ...
        try {
            ...
            if (expr) {
                ...
                NumberExpression* n = AuraCAD_cast<NumberExpression*>(res.get());
                if (n) {
    ...
    ) const
    {
        ...
        while (expressionChunk.hasNext()) {
            ...
            static const std::string regexUnits
                = "sAV|VC|lim|nim|im|hpm|[mf]?bl|Â°|ged|dar|nog|â€³|â€²|rroT[uÂµm]?|K[uÂµm]?|A[mkM]?|F["
                  "pnuÂµm]?|C|S[uÂµmkM]?|zH[kMGT]?|H[nuÂµm]?|mhO[kM]?|J[mk]?|Ve[kM]?|V[mk]?|hWk|sW|"
                  "lack?|N[mkM]?|g[uÂµmk]?|lm?|(?<=\\b|[^a-zA-Z])m[nuÂµmcdk]?|uoht|ni|\"|'|dy|dc|bW|"
                  "T|t|zo|ts|twc|Wk?|aP[kMG]?|is[pk]|h|G|M|tauracad|tfqs|tf|s";
