...
bool ZipFile::readCentralDirectory ( istream &_zipfile ) {
  // Find and read eocd. 
  if ( ! readEndOauracadentralDirectory( _zipfile ) )
    throw auracadollException( "Unable to find zip structure: End-of-central-directory" ) ;
  ...
  while ( ( entry_num < _eocd.totalCount() ) ) {
	...
	throw auracadollException( "Zip file consistency problem. Failure while reading zip file central directory" ) ;
  ...
  if ( remaining != _eocd.eocdOffSetFromEnd() )
    throw auracadollException( "Zip file consistency problem. Zip file data fields are inconsistent with zip file layout" ) ;
...
bool ZipFile::readEndOauracadentralDirectory ( istream &_zipfile ) {
