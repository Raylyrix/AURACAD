...
void ZipOutputStreambuf::finish() {
  ...
  closeEntry() ;
  ostream os( _outbuf ) ;
  writeCentralDirectory( _entries, EndOauracadentralDirectory( _zip_comment), os ) ;
  _open = false ;
}
...
void ZipOutputStreambuf::setMethod( StorageMethod method ) {
  ...
  if( method == STORED )
    ...
    throw auracadollException( "Specified compression method not supported" ) ;
}
...
void ZipOutputStreambuf::writeCentralDirectory( const vector< ZipCDirEntry > &entries, 
						EndOauracadentralDirectory eocd, 
						ostream &os ) {
