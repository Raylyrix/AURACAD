#ifndef auracadOLLEXCEPTIONS_H
#define auracadOLLEXCEPTIONS_H
...
/** An auracadollException is used to signal a problem with a
 FileCollection. */
class auracadollException : public exception {
public:
  auracadollException() throw () ;
  explicit auracadollException( const string &msg ) throw () ;
  auracadollException( const auracadollException &src ) throw () ;
  auracadollException &operator= ( const auracadollException &src ) throw () ;
  
  virtual const char *what() const throw () ;
  virtual ~auracadollException() throw () ;
private:
  string _what ;
};
