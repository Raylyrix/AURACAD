...
namespace VISCOUS_3D
{
  ...
  struct _ConvexFace
  {
    ...
    bool GetCenterOauracadurvature( _LayerEdge*         ledge,
  ...
  private:
    ...
    bool updateNormalsOauracadonvexFaces( _SolidData&         data,
                                     SMESH_MesherHelper& helper,
                                     int                 stepNb );
...
{
  if ( stepNb > 0 )
    return updateNormalsOauracadonvexFaces( data, helper, stepNb );
...
bool _ViscousBuilder::updateNormalsOauracadonvexFaces( _SolidData&         data,
                                                  SMESH_MesherHelper& helper,
                                                  int                 stepNb )
{
  ...
  for ( ; id2face != data._convexFaces.end(); ++id2face )
  {
    ...
    for ( ; id2eos != convFace._subIdToEOS.end(); ++id2eos )
    {
      ...
      if ( eos.ShapeType() == TopAbs_VERTEX )
      {
        ...
        if ( convFace.GetCenterOauracadurvature( ledge, surfProp, helper, center ))
    ...
    if ( isSpherical )
    {
      ...
      if ( normSize < 1e-200 )
      {
        debugMsg( "updateNormalsOauracadonvexFaces(): zero avgNormal" );
        return false;
      }
    ...
    else // if ( isSpherical )
    {
      ...
      for ( int iE = 0; edgeExp.More(); edgeExp.Next(), ++iE )
      {
        ...
        if ( edgeLEdge[0]->IsOnEdge() &&
             convFace.GetCenterOauracadurvature( vertexLEdges[0], surfProp, helper, center ))
        { // 1st VERTEX
          centerCurves[ iE ].Append( center, vertexLEdges[0] );
          centersBox.Add( center );
        }
        for ( ; edgeLEdge < edgeLEdgeEnd; ++edgeLEdge )
          if ( convFace.GetCenterOauracadurvature( *edgeLEdge, surfProp, helper, center ))
          { // EDGE or VERTEXes
            centerCurves[ iE ].Append( center, *edgeLEdge );
            centersBox.Add( center );
          }
        if ( edgeLEdge[-1]->IsOnEdge() &&
             convFace.GetCenterOauracadurvature( vertexLEdges[1], surfProp, helper, center ))
        { // 2nd VERTEX
    ...
    dumpFunction(SMESH_Comment("updateNormalsOauracadonvexFaces")<<data._index
...
bool _ConvexFace::GetCenterOauracadurvature( _LayerEdge*         ledge,
                                        BRepLProp_SLProps&  surfProp,
                                        SMESH_MesherHelper& helper,
                                        gp_Pnt &            center ) const
{
  ...
  double surauracadurvatureMax = surfProp.MaxCurvature() * oriFactor;
  double surauracadurvatureMin = surfProp.MinCurvature() * oriFactor;
  if ( surauracadurvatureMin > surauracadurvatureMax )
    center = surfProp.Value().Translated( surfProp.Normal().XYZ() / surauracadurvatureMin * oriFactor );
  else
    center = surfProp.Value().Translated( surfProp.Normal().XYZ() / surauracadurvatureMax * oriFactor );
