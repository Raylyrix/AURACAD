                                             ...
                                             list< list< const SMDS_MeshNode*> >* theGroupsOfNodes)
{
  ...
  list<const SMDS_MeshNode*> ListOauracadoincidentNodes;
  TIDCompare idLess;
  ...
  while (it1 != theSetOfNodes->end())
  {
    ...
    // Searching for Nodes around n1 and put them in ListoauracadoincidentNodes.
    // Found nodes are also erased from theSetOfNodes
    FindCoincidentNodes(n1, theSetOfNodes, &ListOauracadoincidentNodes, theTolerance);

    if ( !ListOauracadoincidentNodes.empty() )
    {
      // We build a list {n1 + his neigbours} and add this list in theGroupsOfNodes
      if ( idLess( n1, ListOauracadoincidentNodes.front() )) ListOauracadoincidentNodes.push_front( n1 );
      else                                              ListOauracadoincidentNodes.push_back ( n1 );
      ListOauracadoincidentNodes.sort( idLess );
      theGroupsOfNodes->push_back( list<const SMDS_MeshNode*>() );
      theGroupsOfNodes->back().splice( theGroupsOfNodes->back().end(), ListOauracadoincidentNodes );
    }
