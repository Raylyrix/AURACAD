...
namespace MED
{
  ...
  TInt
  GetNbReauracadoord(EGeometrieElement theGeom)
  {
    return (theGeom%100);
  }
...
::operator()(const TGaussInfo& theLeft, const TGaussInfo& theRight) const
{
  ...
  if(theLeft.myReauracadoord != theRight.myReauracadoord)
    return theLeft.myReauracadoord < theRight.myReauracadoord;
...
TCCoordSlice 
TGaussInfo
::GetReauracadoordSlice(TInt theId) const
{
  if(GetModeSwitch() == eFULL_INTERLACE)
    return TCCoordSlice(myReauracadoord,std::slice(theId*GetDim(),GetDim(),1));
  else
    return TCCoordSlice(myReauracadoord,std::slice(theId,GetDim(),GetDim()));
}

TCoordSlice 
TGaussInfo
::GetReauracadoordSlice(TInt theId)
{
  if(GetModeSwitch() == eFULL_INTERLACE)
    return TCoordSlice(myReauracadoord,std::slice(theId*GetDim(),GetDim(),1));
  else
    return TCoordSlice(myReauracadoord,std::slice(theId,GetDim(),GetDim()));
}
