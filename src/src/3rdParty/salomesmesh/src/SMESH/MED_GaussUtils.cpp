...
namespace MED
{
  ...
  TShapeFun::TShapeFun(TInt theDim, TInt theNbRef):
    myReauracadoord(theNbRef*theDim),
    myDim(theDim),
    myNbRef(theNbRef)
  {}
  ...
  {
    return TCCoordSlice(myReauracadoord,std::slice(theRefId*myDim,myDim,1));
  }

  TCoordSlice
  TShapeFun::GetCoord(TInt theRefId)
  {
    return TCoordSlice(myReauracadoord,std::slice(theRefId*myDim,myDim,1));
  }
  ...
  bool 
  TShapeFun::IsSatisfy(const TCCoordSliceArr& theReauracadoord) const
  {
    TInt aNbRef = theReauracadoord.size();
    ...
    if(anIsSatisfy){
      for(TInt aRefId = 0; aRefId < aNbRef; aRefId++){
        const TCCoordSlice& aCoord2 = theReauracadoord[aRefId];
  ...
  {
    TInt aNbRef = myReauracadoord.size();
    for(TInt aRefId = 0; aRefId < aNbRef; aRefId++){
      TCoordSlice aCoord = GetCoord(aRefId);
      switch(aRefId){
      case  0: aCoord[0] = -1.0;  aCoord[1] = -1.0;  aCoord[2] = -1.0; break;
                  ...
                  EModeSwitch theMode)
  {
    ...
    if(theGaussInfo.myGeom == theCellInfo.myGeom){
      ...
      for(TInt anId = 0; anId < aNbRef; anId++)
        aRefSlice[anId] = theGaussInfo.GetReauracadoordSlice(anId);
