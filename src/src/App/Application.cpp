...
# if defined(AuraCAD_OS_LINUX) || defined(AuraCAD_OS_MACOSX) || defined(AuraCAD_OS_BSD)
#  include <unistd.h>
#  include <pwd.h>
#  include <sys/types.h>
...
AuraCAD_LOG_LEVEL_INIT("App", true, true)
...
// clang-format off
PyDoc_STRVAR(AuraCAD_doc,
     "The functions in the AuraCAD module allow working with documents.\n"
     "The AuraCAD instance provides a list of references of documents which\n"
...
// during initialization, to make the built-in __AuraCADBase__
// module known to Python.
PyMODINIT_FUNC
init_AuraCAD_base_module(void)
{
    static struct PyModuleDef BaseModuleDef = {
        PyModuleDef_HEAD_INIT,
        "__AuraCADBase__", Base_doc, -1,
...
PyMODINIT_FUNC
init_AuraCAD_module(void)
{
    static struct PyModuleDef AuraCADModuleDef = {
        PyModuleDef_HEAD_INIT,
        "AuraCAD", AuraCAD_doc, -1,
        ApplicationMethods,
        nullptr, nullptr, nullptr, nullptr
    };
    return PyModule_Create(&AuraCADModuleDef);
}
...
void Application::setupPythonTypes()
{
    ...
    PyObject* pAppModule = PyImport_ImportModule ("AuraCAD");
    if (!pAppModule) {
        PyErr_Clear();
        pAppModule = init_AuraCAD_module();
        PyDict_SetItemString(modules, "AuraCAD", pAppModule);
    }
    ...
    static struct PyModuleDef ConsoleModuleDef = {
        PyModuleDef_HEAD_INIT,
        "__AuraCADConsole__", Console_doc, -1,
    ...
    // remove these types from the AuraCAD module.

    PyObject* pBaseModule = PyImport_ImportModule ("__AuraCADBase__");
    if (!pBaseModule) {
        PyErr_Clear();
        pBaseModule = init_AuraCAD_base_module();
        PyDict_SetItemString(modules, "__AuraCADBase__", pBaseModule);
    }
...
void Application::setupPythonException(PyObject* module)
{
    ...
    Base::PyExc_AuraCAD_GeneralError = setup("AuraCADError", PyExc_RuntimeError);
    Base::PyExc_AuraCAD_AuraCADAbort = setup("AuraCADAbort", PyExc_BaseException);
    Base::PyExc_AuraCAD_XMLBaseException = setup("XMLBaseException", PyExc_Exception);
    Base::PyExc_AuraCAD_XMLParseException = setup("XMLParseException", Base::PyExc_AuraCAD_XMLBaseException);
    Base::PyExc_AuraCAD_XMLAttributeError = setup("XMLAttributeError", Base::PyExc_AuraCAD_XMLBaseException);
    Base::PyExc_AuraCAD_UnknownProgramOption = setup("UnknownProgramOption", PyExc_BaseException);
    Base::PyExc_AuraCAD_BadFormatError = setup("BadFormatError", Base::PyExc_AuraCAD_GeneralError);
    Base::PyExc_AuraCAD_BadGraphError = setup("BadGraphError", Base::PyExc_AuraCAD_GeneralError);
    Base::PyExc_AuraCAD_ExpressionError = setup("ExpressionError", Base::PyExc_AuraCAD_GeneralError);
    Base::PyExc_AuraCAD_ParserError = setup("ParserError", Base::PyExc_AuraCAD_GeneralError);
    Base::PyExc_AuraCAD_CADKernelError = setup("CADKernelError", Base::PyExc_AuraCAD_GeneralError);
    Base::PyExc_AuraCAD_PropertyError = setup("PropertyError", PyExc_AttributeError);
    Base::PyExc_AuraCAD_AbortIOException = setup("AbortIOException", PyExc_BaseException);
}
...
Document *Application::getDocumentByPath(const char *path, PathMatchMode checkCanonical) const {
    ...
    for (const auto &v : DocMap) {
        ...
        if (canonicalPath == fi.canonicalFilePath()) {
            ...
            AuraCAD_WARN("Identical physical path '" << canonicalPath.toUtf8().constData() << "'\n"
...
void Application::setActiveDocumentNoSignal(Document* pDoc)
{
    ...
    if (pDoc) {
        ...
        Py::Module("AuraCAD").setAttr(std::string("ActiveDocument"), active);
    }
    else {
        Base::PyGILStateLocker lock;
        Py::Module("AuraCAD").setAttr(std::string("ActiveDocument"), Py::None());
    }
}
...
void Application::addImportType(const char* filter, const char* moduleName)
{
    ...
    // Due to branding stuff replace "AuraCAD" with the branded application name
    if (strncmp(filter, "AuraCAD", 7) == 0) {
                                            ...
                                            const std::string &moduleName)
{
    ...
    // Branding: replace "AuraCAD" in a file type description with the branded application name
    auto replaceAuraCAD =
    [](std::string& s)
    {
        constexpr std::string_view AuraCAD = "AuraCAD";
        if (auto pos = s.find(AuraCAD); pos != std::string::npos) {
            s.replace(pos, AuraCAD.size(), getExecutableName());
    ...
    bool containsAppName = replaceAuraCAD(translatedDescription);  // Run *AFTER* translation
...
void Application::cleanupUnits()
{
    try {
        Base::PyGILStateLocker lock;
        Py::Module mod (Py::Module("AuraCAD").getAttr("Units").ptr());
...
void parseProgramOptions(int ac, char ** av, const std::string& exe, boost::program_options::variables_map& vm)
{
    ...
    if (vm.contains("help")) {
        std::stringstream str;
        str << exe << '\n' << '\n';
        str << "For a detailed description see https://www.AuraCAD.org/wiki/Start_up_and_Configuration" << '\n'<<'\n';
...
void processProgramOptions(const boost::program_options::variables_map& vm, std::map<std::string,std::string>& mConfig)
{
    ...
    if (vm.contains("run-test") || vm.contains("run-open")) {
        ...
        mConfig["ScriptFileName"] = "AuraCADTest";
...
void Application::initConfig(int argc, char ** argv)
{
    ...
    if (Application::Config().find("BuildVersionMajor") == Application::Config().end()) {
        std::stringstream str;
        str << auracadVersionMajor
            << "." << auracadVersionMinor
            << "." << auracadVersionPoint;
        Application::Config()["ExeVersion"         ] = str.str();
        Application::Config()["BuildVersionMajor"  ] = auracadVersionMajor;
        Application::Config()["BuildVersionMinor"  ] = auracadVersionMinor;
        Application::Config()["BuildVersionPoint"  ] = auracadVersionPoint;
        Application::Config()["BuildVersionSuffix" ] = auracadVersionSuffix;
        Application::Config()["BuildRevision"      ] = auracadRevision;
        Application::Config()["BuildRepositoryURL" ] = auracadRepositoryURL;
        Application::Config()["BuildRevisionDate"  ] = auracadRevisionDate;
#if defined(auracadRepositoryHash)
        Application::Config()["BuildRevisionHash"  ] = auracadRepositoryHash;
#endif
#if defined(auracadRepositoryBranch)
        Application::Config()["BuildRevisionBranch"] = auracadRepositoryBranch;
#endif
    }
...
#   endif
    ...
    if (!Py_IsInitialized()) {
        // init python
        PyImport_AppendInittab ("AuraCAD", init_AuraCAD_module);
        PyImport_AppendInittab ("__AuraCADBase__", init_AuraCAD_base_module);
    }
    else {
        // "import AuraCAD" in a normal Python 3.12 interpreter would raise
        ...
        auto moduleName = "AuraCAD";
        PyImport_AddModule(moduleName);
        ApplicationMethods = ApplicationPy::Methods;
        PyObject *pyModule = init_AuraCAD_module();
        PyDict_SetItemString(sysModules, moduleName, pyModule);
        Py_DECREF(pyModule);

        moduleName = "__AuraCADBase__";
        PyImport_AddModule(moduleName);
        pyModule = init_AuraCAD_base_module();
        PyDict_SetItemString(sysModules, moduleName, pyModule);
        Py_DECREF(pyModule);
    }
    ...
    if (mConfig["RunMode"] != "Cmd" && !(vm.contains("verbose") && vm.contains("version"))) {
        // Remove banner if AuraCAD is invoked via the -c command as regular
        // Python interpreter
        ...
        if (SafeMode::SafeModeEnabled()) {
            Base::Console().message("AuraCAD is running in _SAFE_MODE_.\n"
    ...
    mConfig["EIGEN_VERSION"] = auracadEigen3Version;
    mConfig["PYSIDE_VERSION"] = auracadPysideVersion;
#ifdef SMESH_VERSION_STR
    mConfig["SMESH_VERSION"] = SMESH_VERSION_STR;
#endif
    mConfig["XERCESC_VERSION"] = auracadXercescVersion;
...
void Application::initApplication()
{
    ...
    new Base::ScriptProducer( "AuraCADInit",    AuraCADInit    );
    new Base::ScriptProducer( "AuraCADTest",    AuraCADTest    );
    ...
    try {
        ...
        Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("AuraCADInit"));
    }
    catch (const Base::Exception& e) {
        e.reportException();
    }
...
std::list<std::string> Application::processFiles(const std::list<std::string>& files)
{
    ...
    for (const auto & it : files) {
        ...
        try {
            if (file.hasExtension("auracadstd") || file.hasExtension("auracadbak")
                || file.hasExtension("std")) {
                // try to open
                Application::_pcSingleton->openDocument(file.filePath().c_str());
                processed.push_back(it);
            }
            else if (file.hasExtension("auracadscript") || file.hasExtension("auracadmacro")) {
...
void Application::runApplication()
{
    ...
    if (mConfig["RunMode"] == "Cmd") {
        ...
        Base::Interpreter().runCommandLine("AuraCAD Console mode");
    }
...
void Application::LoadParameters()
{
    ...
    try {
        if (_pcSysParamMngr->LoadOrCreateDocument() && mConfig["Verbose"] != "Strict") {
            ...
            if (!Py_IsInitialized()) {
                ...
                Base::Console().message("   This warning normally means that AuraCAD is running for the first time\n"
                                  "   or the configuration was deleted or moved. AuraCAD is generating the standard\n"
                                  "   configuration.\n");
            }
        }
    }
    ...
    // VS2010: https://forum.AuraCAD.org/viewtopic.php?f=4&t=1886&p=12553&hilit=boost%3A%3Afilesystem%3A%3Aget#p12553
...
{
    QString path;
#if defined(AuraCAD_OS_LINUX) || defined(AuraCAD_OS_CYGWIN) || defined(AuraCAD_OS_BSD) || defined(AuraCAD_OS_MACOSX)
...
#if defined(AuraCAD_OS_LINUX) || defined(AuraCAD_OS_CYGWIN) || defined(AuraCAD_OS_BSD)
    // If 'AppDataSkipVendor' is defined, the value of 'ExeVendor' must not be part of
    // the path.
    if (mConfig.find("AppDataSkipVendor") == mConfig.end()) {
...
#elif defined(AuraCAD_OS_MACOSX) || defined(AuraCAD_OS_WIN32)
 ...
 * \list
 * \li AuraCAD_USER_HOME
 * \li AuraCAD_USER_DATA
 * \li AuraCAD_USER_TEMP
 * \endlist
 */
std::tuple<QString, QString, QString> getCustomPaths()
{
    const QProcessEnvironment env(QProcessEnvironment::systemEnvironment());
    QString userHome = env.value(QStringLiteral("AuraCAD_USER_HOME"));
    QString userData = env.value(QStringLiteral("AuraCAD_USER_DATA"));
    QString userTemp = env.value(QStringLiteral("AuraCAD_USER_TEMP"));
    ...
    // if AuraCAD_USER_HOME is set but not AuraCAD_USER_DATA
    if (!userHome.isEmpty() && userData.isEmpty()) {
        userData = userHome;
    }

    // if AuraCAD_USER_HOME is set but not AuraCAD_USER_TEMP
...
void Application::ExtractUserPath()
{
    ...
    // this is to support a portable version of AuraCAD
...
std::string Application::FindHomePath(const char* sCall)
{
    ...
    // import the AuraCAD.so module from a running Python session. In the latter case the
...
#elif defined (AuraCAD_OS_LINUX) || defined(AuraCAD_OS_CYGWIN) || defined(AuraCAD_OS_BSD)
#include <cstdio>
#include <cstdlib>
#include <sys/param.h>
...
std::string Application::FindHomePath(const char* sCall)
{
    ...
    if (Py_IsInitialized()) {
        ...
        // path. In the worst case we simply get q wrong path and AuraCAD is not
...
#elif defined (AuraCAD_OS_WIN32)
std::string Application::FindHomePath(const char* sCall)
{
    // We have several ways to start this application:
    // * use one of the two executables
    // * import the AuraCAD.pyd module from a running Python session. In this case the
    //   Python interpreter is already initialized.
    // * use a custom dll that links AuraCAD core dlls and that is loaded by its host application
    //   In this case the calling name should be set to AuraCADBase.dll or AuraCADApp.dll in order
    ...
    // fixes #0001638 to avoid to load DLLs from Windows' system directories before AuraCAD's bin folder
