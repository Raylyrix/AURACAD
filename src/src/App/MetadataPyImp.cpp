...
/**************************************************************************
 *                                                                         *
 *   Copyright (c) 2022 AuraCAD Project Association                        *
 *                                                                         *
...
int MetadataPy::PyInit(PyObject* args, PyObject* /*kwd*/)
{
    ...
    if (PyArg_ParseTuple(args, "et", "utf-8", &filename)) {
        try {
            ...
            PyErr_SetString(Base::PyExc_AuraCAD_XMLBaseException, what.c_str());
            return -1;
        }
            ...
            PyErr_SetString(Base::PyExc_AuraCAD_GeneralError, "Failed to create Metadata object");
            return -1;
        }
    }
    ...
    PyErr_SetString(Base::PyExc_AuraCAD_GeneralError,
...
void MetadataPy::setUrls(Py::List args)
{
    ...
    for (const auto& url : urls) {
        ...
        if (typeAsString == "website") {
            ...
            PyErr_SetString(Base::PyExc_AuraCAD_GeneralError, "Unrecognized URL type");
            return;
        }
...
Py::String MetadataPy::getAuraCADMin() const
{
    return Py::String(getMetadataPtr()->AuraCADmin().str());
}

void MetadataPy::setAuraCADMin(Py::String args)
{
    ...
    if (version) {
        getMetadataPtr()->setAuraCADMin(App::Meta::Version(version));
    }
    else {
        getMetadataPtr()->setAuraCADMin(App::Meta::Version());
    }
}

Py::String MetadataPy::getAuraCADMax() const
{
    return Py::String(getMetadataPtr()->AuraCADmax().str());
}

void MetadataPy::setAuraCADMax(Py::String args)
{
    char* version = nullptr;
    PyObject* p = args.ptr();
    if (!PyArg_Parse(p, "z", &version)) {
    ...
    if (version) {
        getMetadataPtr()->setAuraCADMax(App::Meta::Version(version));
    }
    else {
        getMetadataPtr()->setAuraCADMax(App::Meta::Version());
    }
}
...
PyObject* MetadataPy::getFirstSupportedAuraCADVersion(PyObject* p)
{
    ...
    if (getMetadataPtr()->AuraCADmin() != App::Meta::Version()) {
        return Py::new_reference_to(Py::String(getMetadataPtr()->AuraCADmin().str()));
    }

    auto content = getMetadataPtr()->content();
    auto result = App::Meta::Version();
    for (const auto& item : content) {
        auto minVersion = item.second.AuraCADmin();
...
PyObject* MetadataPy::getLastSupportedAuraCADVersion(PyObject* p)
{
    if (!PyArg_ParseTuple(p, "")) {
        return nullptr;
    }
    ...
    // Short-circuit: if the toplevel sets a version, then the lower-levels are overridden
    if (getMetadataPtr()->AuraCADmax() != App::Meta::Version()) {
        return Py::new_reference_to(Py::String(getMetadataPtr()->AuraCADmax().str()));
    }

    auto content = getMetadataPtr()->content();
    auto result = App::Meta::Version();
    for (const auto& item : content) {
        auto maxVersion = item.second.AuraCADmax();
...
PyObject* MetadataPy::supportsCurrentAuraCAD(PyObject* p)
{
    if (!PyArg_ParseTuple(p, "")) {
        return nullptr;
    }

    bool result = getMetadataPtr()->supportsCurrentAuraCAD();
