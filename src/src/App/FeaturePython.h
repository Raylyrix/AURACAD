...
#define AuraCAD_PY_FEATURE_PYTHON                                                                       \
    AuraCAD_PY_ELEMENT(execute)                                                                         \
    AuraCAD_PY_ELEMENT(mustExecute)                                                                     \
    AuraCAD_PY_ELEMENT(onBeforeChange)                                                                  \
    AuraCAD_PY_ELEMENT(onBeforeChangeLabel)                                                             \
    AuraCAD_PY_ELEMENT(onChanged)                                                                       \
    AuraCAD_PY_ELEMENT(onDocumentRestored)                                                              \
    AuraCAD_PY_ELEMENT(unsetupObject)                                                                   \
    AuraCAD_PY_ELEMENT(getViewProviderName)                                                             \
    AuraCAD_PY_ELEMENT(getSubObject)                                                                    \
    AuraCAD_PY_ELEMENT(getSubObjects)                                                                   \
    AuraCAD_PY_ELEMENT(getLinkedObject)                                                                 \
    AuraCAD_PY_ELEMENT(canLinkProperties)                                                               \
    AuraCAD_PY_ELEMENT(allowDuplicateLabel)                                                             \
    AuraCAD_PY_ELEMENT(redirectSubName)                                                                 \
    AuraCAD_PY_ELEMENT(canLoadPartial)                                                                  \
    AuraCAD_PY_ELEMENT(supportsAsyncRecompute)                                                          \
    AuraCAD_PY_ELEMENT(hasChildElement)                                                                 \
    AuraCAD_PY_ELEMENT(isElementVisible)                                                                \
    AuraCAD_PY_ELEMENT(setElementVisible)                                                               \
    AuraCAD_PY_ELEMENT(editProperty)                                                                    \
    AuraCAD_PY_ELEMENT(isLink)                                                                          \
    AuraCAD_PY_ELEMENT(isLinkGroup)                                                                     \
    AuraCAD_PY_ELEMENT(getPlacementOf)

#define AuraCAD_PY_ELEMENT_DEFINE(_name) Py::Object py_##_name;

#define AuraCAD_PY_ELEMENT_INIT(_name)                                                                  \
    AuraCAD_PY_GetCallable(pyobj, #_name, py_##_name);                                                  \
    if (!py_##_name.isNone()) {                                                                    \
...
#define AuraCAD_PY_ELEMENT_FLAG(_name) FlagCalling_##_name, FlagAllowRecursive_##_name,

#define _AuraCAD_PY_CALL_CHECK(_name, _ret)                                                             \
...
#define AuraCAD_PY_ELEMENT(_name) AuraCAD_PY_ELEMENT_FLAG(_name)
    ...
    enum Flag
    {
        AuraCAD_PY_FEATURE_PYTHON FlagMax,
    };
