...
AuraCAD_LOG_LEVEL_INIT("ElementMap", true, 2);  // NOLINT
        ...
        for (auto& vv : indexedName.second.children) {
            ...
            if (child.elementMap) {
                ...
                if (it == childMapSet.end() || it->second == 0) {
                    AuraCAD_ERR("Invalid child element map");  // NOLINT
                }
                else {
                    mapIndex = it->second;
                }
...
ElementMapPtr ElementMap::restore(::App::StringHasherRef hasherRef, std::istream& stream)
{
    ...
    if (!(stream >> id >> tmp >> count) || tmp != "PostfixCount") {
        AuraCAD_THROWM(Base::RuntimeError, msg);  // NOLINT
    }
                                  ...
                                  const std::vector<std::string>& postfixes)
{
    ...
    if (typeCount < 0 || typeCount > maxTypeCount) {
        AuraCAD_THROWM(Base::RuntimeError, "Bad type count in element map, ignoring map");  // NOLINT
    }
    ...
    if (map) {
        while (tmp != "EndMap") {
            if (!std::getline(stream, tmp)) {
                AuraCAD_THROWM(Base::RuntimeError, "unexpected end of child element map");  // NOLINT
            }
        }
        return map;
    }
    ...
    for (int i = 0; i < typeCount; ++i) {
        int outerCount = 0;
        if (!(stream >> tmp)) {
            AuraCAD_THROWM(Base::RuntimeError, "missing element type");  // NOLINT
        }
        IndexedName idx(tmp.c_str(), 1);

        if (!(stream >> tmp >> outerCount) || tmp != "ChildCount") {
            AuraCAD_THROWM(Base::RuntimeError, "missing element child count");  // NOLINT
        }
        ...
        for (int j = 0; j < outerCount; ++j) {
            ...
            if (!(stream >> cIndex >> offset >> count >> tag >> mapIndex >> tmp)) {
                AuraCAD_THROWM(Base::RuntimeError, "Invalid element child");  // NOLINT
            }
            if (cIndex < 0) {
                AuraCAD_THROWM(Base::RuntimeError, "Invalid element child index");  // NOLINT
            }
            if (offset < 0) {
                AuraCAD_THROWM(Base::RuntimeError, "Invalid element child offset");  // NOLINT
            }
            if (mapIndex >= index || mapIndex < 0 || mapIndex > (int)childMaps.size()) {
                AuraCAD_THROWM(Base::RuntimeError, "Invalid element child map index");  // NOLINT
            }
            ...
            if (!(stream >> tmp)) {
                AuraCAD_THROWM(Base::RuntimeError, "Invalid element child string id");  // NOLINT
            }
        ...
        if (!(stream >> tmp >> outerCount) || tmp != "NameCount") {
            AuraCAD_THROWM(Base::RuntimeError, "missing element name outerCount");  // NOLINT
        }
        ...
        for (int j = 0; j < outerCount; ++j) {
            ...
            while (true) {
                if (!(stream >> tmp)) {
                    AuraCAD_THROWM(Base::RuntimeError, "Failed to read element name");  // NOLINT
                }
                if (tmp == "0") {
                    break;
                }
                ...
                boost::split(tokens, tmp, boost::is_any_of("."));
                if (tokens.size() < 2) {
                    AuraCAD_THROWM(Base::RuntimeError, "Invalid element entry");  // NOLINT
                }
                ...
                switch (tokens[0][0]) {
                    case ':': {
                        ...
                        if (elementNameIndex <= 0 || elementNameIndex > (int)postfixes.size()) {
                            AuraCAD_THROWM(Base::RuntimeError, "Invalid element name index");  // NOLINT
                        }
                    ...
                    case ';':
                        ...
                        break;
                    default:
                        AuraCAD_THROWM(Base::RuntimeError, "Invalid element name marker");  // NOLINT
                }
    ...
    if (hasherWarn) {
        AuraCAD_WARN(hasherWarn);  // NOLINT
    }
    if (hasherIDWarn) {
        AuraCAD_WARN(hasherIDWarn);  // NOLINT
    }
    if (postfixWarn) {
        AuraCAD_WARN(postfixWarn);  // NOLINT
    }
    if (childSIDWarn) {
        AuraCAD_WARN(childSIDWarn);  // NOLINT
    }
                               ...
                               IndexedName* existing)
{
    if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
        if (name.find("#") >= 0 && name.findTagInElementName() < 0) {
            AuraCAD_ERR("missing tag postfix " << name);  // NOLINT
        }
    }
    while (true) {
        if (overwrite) {
        ...
        if (ret.second) {                // element just inserted did not exist yet in the map
            ...
            AuraCAD_TRACE(idx << " -> " << name);  // NOLINT
            return ret.first->first;
        }
        if (ret.first->second == idx) {
            AuraCAD_TRACE("duplicate " << idx << " -> " << name);  // NOLINT
            return ret.first->first;
        }
        if (!overwrite) {
            if (existing) {
                                      ...
                                      bool overwrite)
{
    ...
    for (int i = 0, count = name.size(); i < count; ++i) {
        ...
        if (check == '.' || (std::isspace((int)check) != 0)) {
            AuraCAD_THROWM(Base::RuntimeError, "Illegal character in mapped name: " << name);  // NOLINT
        }
    }
    for (const char* readChar = element.getType(); *readChar != 0; ++readChar) {
        char check = *readChar;
        if (check == '.' || (std::isspace((int)check) != 0)) {
            AuraCAD_THROWM(Base::RuntimeError,  // NOLINT
    ...
    for (int i = 0;;) {
        ...
        if (++i == maxAttempts) {
            AuraCAD_ERR("unresolved duplicate element mapping '"  // NOLINT
...
MappedName ElementMap::dehashElementName(const MappedName& name) const
{
    ...
    if (!sid) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_TRACE)) {
            AuraCAD_WARN("failed to find hash id " << id);  // NOLINT
        }
        else {
            AuraCAD_LOG("failed to find hash id " << id);  // NOLINT
        }
        return name;
    }
    if (sid.isHashed()) {
        AuraCAD_LOG("cannot de-hash id " << id);  // NOLINT
        return name;
    }
    MappedName ret(sid);
    //        sid.toString());// FIXME .toString() was missing in original function. is this
    //        correct?
    AuraCAD_TRACE("de-hash " << name << " -> " << ret);  // NOLINT
    return ret;
}
                                              ...
                                              long masterTag) const
{
    ...
    if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
        AuraCAD_WARN("duplicate element mapping '"  // NOLINT
...
void ElementMap::addChildElements(long masterTag, const std::vector<MappedChildElements>& children)
{
    ...
    for (auto& child : expansion.empty() ? children : expansion) {
        if (!child.indexedName || (child.count == 0)) {
            if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                AuraCAD_ERR("invalid mapped child element");  // NOLINT
            }
            continue;
        }
            ...
            for (int i = 0; i < child.count; ++i, ++childIdx, ++idx) {
                ...
                if (!name) {
                    if ((child.tag == 0) || child.tag == masterTag) {
                        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                            AuraCAD_WARN("unmapped element");  // NOLINT
                        }
                        continue;
                    }
                    name = MappedName(childIdx);
        ...
        if (entry->index != 1) {
            ...
            entry = &childElements[tmp.toBytes()];
            if (entry->childMap) {
                AuraCAD_ERR("duplicate mapped child element");  // NOLINT
                continue;
            }
        }
                                   ...
                                   std::vector<MappedName>* history) const
{
    ...
    while (true) {
        if ((len == 0) || len > pos) {
            AuraCAD_WARN("invalid name length " << name);  // NOLINT
            return 0;
        }
        bool deHashed = false;
...
void ElementMap::traceElement(const MappedName& name, long masterTag, TraceCallback cb) const
{
    ...
    //
    // https://github.com/realthunder/AuraCAD_assembly3/issues/968
    //
    ...
    for (int index = 0; index < 50; ++index) {
        ...
        if (encodedTag && masterTag != std::abs(encodedTag)
            ...
            if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                AuraCAD_WARN("circular element mapping");
                if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_TRACE)) {
                    auto doc = App::GetApplication().getActiveDocument();
                    if (doc) {
                        auto obj = doc->getObjectByID(masterTag);
                        if (obj) {
                            AuraCAD_LOG("\t" << obj->getFullName() << obj->getFullName() << "." << name);
                        }
                    }
