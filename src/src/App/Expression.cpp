...
App::any pyObjectToAny(Py::Object value, bool check) {
    ...
    if (PyLong_Check(pyvalue))
        ...
        if (!utf8value) {
            AuraCAD_THROWM(Base::ValueError, "Invalid unicode string");
        }
...
void NumberExpression::_toString(std::ostream &ss, bool,int) const
{
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=3&t=44057&p=375882#p375882
    // See also:
    ...
    case OperatorExpression::ADD: {
        ...
        if (PyUnicode_CheckExact(*l) && PyUnicode_CheckExact(*r)) {
            if(inplace) {
                ...
                // Try to make sure ob_reauracadnt is 1, although unlike
...
ExpressionPtr OperatorExpression::simplify() const
{
    ...
    if (AuraCAD_cast<NumberExpression*>(v1.get()) && AuraCAD_cast<NumberExpression*>(v2.get())) {
        return eval();
    }
    else
...
void OperatorExpression::_toString(std::ostream &s, bool persistent,int) const
{
    ...
    needsParens = false;
    if (AuraCAD_cast<OperatorExpression*>(left))
    ...
    needsParens = false;
    if (AuraCAD_cast<OperatorExpression*>(right))
    ...
    for (auto &arg : args) {
        if (arg->isDerivedFrom<RangeExpression>()) {
            ...
            do {
                ...
                if ((qp = AuraCAD_cast<PropertyQuantity*>(p)))
                    c->collect(qp->getQuantityValue());
                else if ((fp = AuraCAD_cast<PropertyFloat*>(p)))
                    c->collect(Quantity(fp->getValue()));
                else if ((ip = AuraCAD_cast<PropertyInteger*>(p)))
...
ExpressionPtr FunctionExpression::simplify() const
{
    ...
    for (auto it : args) {
        ...
        if (AuraCAD_cast<NumberExpression*>(v.get())) {
            ++numerics;
        }
...
void VariableExpression::addComponent(Component *c) {
    do {
        ...
        long l1=0,l2=0,l3=1;
        if(c->e3) {
            auto n3 = AuraCAD_cast<NumberExpression*>(c->e3.get());
            if(!n3 || !essentiallyEqual(n3->getValue(),(double)l3))
                break;
        }
        if(c->e1) {
            auto n1 = AuraCAD_cast<NumberExpression*>(c->e1.get());
            if(!n1) {
                if(c->e2 || c->e3)
                    break;
                auto s = AuraCAD_cast<StringExpression*>(c->e1.get());
                if(!s)
                    break;
        ...
        auto n2 = AuraCAD_cast<NumberExpression*>(c->e2.get());
...
void VariableExpression::_offsetCells(int rowOffset, int colOffset, ExpressionVisitor &v) {
    ...
    if(!addr.isValid()) {
        AuraCAD_WARN("Not changing relative cell reference '"
...
ExpressionPtr ConditionalExpression::simplify() const
{
    ...
    NumberExpression * v = AuraCAD_cast<NumberExpression*>(e.get());
...
)
{
    ...
    if (!unitExpression) {
        auto* fraction = AuraCAD_cast<OperatorExpression*>(ScanResult.get());

        if (fraction && fraction->getOperator() == OperatorExpression::DIV) {
            NumberExpression * nom = AuraCAD_cast<NumberExpression*>(fraction->getLeft());
            UnitExpression * denom = AuraCAD_cast<UnitExpression*>(fraction->getRight());
    ...
    if (auto num = AuraCAD_cast<NumberExpression*>(simplified.get()); num) {
        return std::make_unique<UnitExpression>(num->getOwner(), num->getQuantity());
    }
    return std::unique_ptr<UnitExpression>(AuraCAD_cast<UnitExpression*>(simplified.release()));
}
