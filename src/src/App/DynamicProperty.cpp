...
AuraCAD_LOG_LEVEL_INIT("Property", true, true)
                                            ...
                                            const char* newName)
{
    ...
    if (propIt->property->testStatus(Property::LockDynamic)) {
        AuraCAD_THROWM(Base::RuntimeError, "Property " << prop->getName() << " is locked");
    }
    ...
    if (Base::Tools::getIdentifier(newName) != newName) {
        AuraCAD_THROWM(Base::NameError, "Invalid property name '" << newName << "'");
    }
