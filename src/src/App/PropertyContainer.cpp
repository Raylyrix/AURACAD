...
void PropertyContainer::Restore(Base::XMLReader &reader)
{
    ...
    for (int i=0;i<transientCount; ++i) {
        ...
        if(prop)
            AuraCAD_TRACE("restore transient '" << prop->getName() << "'");
    ...
    for (int i=0 ;i<Cnt ;i++) {
        ...
        try {
            ...
            if (prop && strcmp(prop->getTypeId().getName(), TypeName.c_str()) == 0) {
                ...
                {
                    AuraCAD_TRACE("restore property '" << prop->getName() << "'");
                    prop->Restore(reader);
                }else
                    AuraCAD_TRACE("skip transient '" << prop->getName() << "'");
            }
...
#ifdef AuraCAD_DEBUG
            if(it->Offset != offset) {
                AuraCAD_ERR("Duplicate property '" << PropName << "'");
            }
#endif
        }
    }
