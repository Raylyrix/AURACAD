...
AuraCAD_LOG_LEVEL_INIT("App", true, true, true)
...
int Document::openTransaction(TransactionName name, int tid) // NOLINT
{
    ...
    if (isTransactionLocked()) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Transaction locked, ignore new transaction '" << name.name << "'");
        }
        return 0;
    }
    if (isPerformingTransaction() || d->committing) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Cannot open transaction while transacting");
        }
        return 0;
    }
    if (name.name.empty()) {
...
int Document::_openTransaction(std::string name, int id)
{
    if (isTransactionLocked() && id != d->bookedTransaction) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Transaction locked, ignore new transaction '" << name << "'");
        }
    }
    if (isPerformingTransaction() || d->committing) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
    ...
    if (transactionInitiator && transactionInitiator != this && !transactionInitiator->hasPendingTransaction()) {
        ...
        AuraCAD_LOG("auto transaction " << getName() << " -> " << transactionInitiator->getName());
...
int Document::setActiveTransaction(TransactionName name, int tid)
{
    ...
    if (tid != NullTransaction) {
        if (!GetApplication().transactionIsActive(tid)) {
            AuraCAD_LOG("Could not set active transaction to inactive ID");
            return NullTransaction;
        }
        if (d->bookedTransaction != NullTransaction && d->bookedTransaction != tid && !_commitTransaction(true)) {
            AuraCAD_LOG("Could not book transaction for document");
            return NullTransaction;
        }
...
void Document::_checkTransaction(DocumentObject* pcDelObj, const Property* What, int line)
{
    ...
    if (!testStatus(Restoring) || testStatus(Importing)) {
        ...
        if (d->bookedTransaction != NullTransaction) {
            ...
            if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                if (What) {
                    AuraCAD_LOG((ignore ? "ignore" : "auto")
                            << " transaction (" << line << ") '" << What->getFullName());
                }
                else {
                    AuraCAD_LOG((ignore ? "ignore" : "auto") << " transaction (" << line << ") '"
                                                        << name << "' in " << getName());
                }
            }
            if (!ignore) {
...
{
    if (isPerformingTransaction() || d->committing) {
        AuraCAD_ERR("Cannot clear redo while transacting");
        return;
    }
...
void Document::commitTransaction() // NOLINT
{
    if (isPerformingTransaction() || d->committing) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Cannot commit transaction while transacting");
        }
        return;
    }
...
void Document::abortTransaction() const
{
    if (isPerformingTransaction() || d->committing) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Cannot abort transaction while transacting");
        }
        return;
    }
    if (d->activeUndoTransaction) {
...
{
    if (isPerformingTransaction() || d->committing) {
        AuraCAD_ERR("Cannot clear undos while transacting");
        return;
    }
...
void Document::onChanged(const Property* prop)
{
    ...
    if (prop == &Label) {
        ...
        for (auto obj : d->objectArray) {
            auto geofeature = AuraCAD_cast<GeoFeature*>(obj);
...
void Document::Save(Base::Writer& writer) const
{
    ...
    //       see: https://github.com/AuraCAD/AuraCAD/issues/27489
    //
    ...
    if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
        for (auto o : obj) {
            if (o && o->isAttachedToDocument()) {
                AuraCAD_LOG("exporting " << o->getFullName());
...
constexpr auto auracadAttrDependencies {"Dependencies"};
constexpr auto auracadElementObjectDeps {"ObjectDeps"};
constexpr auto auracadAttrDepCount {"Count"};
constexpr auto auracadAttrDepObjName {"Name"};
constexpr auto auracadAttrDepAllowPartial {"AllowPartial"};
constexpr auto auracadElementObjectDep {"Dep"};
                               ...
                               Base::Writer& writer) const
{
    for (auto o : objs) {
        ...
        writer.Stream() << writer.ind()
                        << "<" << auracadElementObjectDeps
                        << " " << auracadAttrDepObjName << "=\""
                        << (objName ? objName : "") << "\" "
                        << auracadAttrDepCount << "=\""
                        << outList.size();
        if (outList.empty()) {
        ...
        if (partial > 0) {
            writer.Stream() << "\" " << auracadAttrDepAllowPartial << "=\"" << partial;
        }
        writer.Stream() << "\">\n";
        writer.incInd();
        for (auto dep : outList) {
            ...
            writer.Stream() << writer.ind()
                            << "<" << auracadElementObjectDep
                            << " " << auracadAttrDepObjName << "=\""
                            << (depName ? depName : "") << "\"/>\n";
        }
        writer.decInd();
        writer.Stream() << writer.ind() << "</" << auracadElementObjectDeps << ">\n";
    }
}
...
std::vector<DocumentObject*> Document::readObjects(Base::XMLReader& reader)
{
    ...
    if (!reader.hasAttribute(auracadAttrDependencies)) {
        d->partialLoadObjects.clear();
    }
    else if (!d->partialLoadObjects.empty()) {
        std::unordered_map<std::string, DepInfo> deps;
        for (int i = 0; i < Cnt; i++) {
            reader.readElement(auracadElementObjectDeps);
            int dcount = static_cast<int>(reader.getAttribute<long>(auracadAttrDepCount));
            if (dcount == 0) {
                continue;
            }
            auto& info = deps[reader.getAttribute<const char*>(auracadAttrDepObjName)];
            if (reader.hasAttribute(auracadAttrDepAllowPartial)) {
                info.canLoadPartial =
                    static_cast<int>(reader.getAttribute<long>(auracadAttrDepAllowPartial));
            }
            for (int j = 0; j < dcount; ++j) {
                reader.readElement(auracadElementObjectDep);
                const char* name = reader.getAttribute<const char*>(auracadAttrDepObjName);
                if (!Base::Tools::isNullOrEmpty(name)) {
                    info.deps.insert(name);
                }
            }
            reader.readEndElement(auracadElementObjectDeps);
        }
    ...
    for (int i = 0; i < Cnt; i++) {
        ...
        if (DocumentObject* pObj = getObject(name.c_str()); pObj
            ...
            pObj->setStatus(ObjectStatus::Restore, true);
            try {
                AuraCAD_TRACE("restoring " << pObj->getFullName());
                pObj->Restore(reader);
            }
...
std::vector<DocumentObject*> Document::importObjects(Base::XMLReader& reader)
{
    ...
    for (const auto o : objs) {
        if (o && o->isAttachedToDocument()) {
            ...
            AuraCAD_LOG("importing " << o->getFullName());
            if (const auto propUUID =
                    AuraCAD_cast<PropertyUUID*>(o->getPropertyByName("_ObjectUUID"))) {
                auto propSource =
                    AuraCAD_cast<PropertyUUID*>(o->getPropertyByName("_SourceUUID"));
                if (!propSource) {
...
static std::string checkFileName(const char* file)
{
    ...
    if (GetApplication()
        ...
        constexpr std::size_t backupExtLen = sizeof(".auracadbak") - 1;
        constexpr std::size_t backupAndDocExtLen = sizeof(".auracadbak.auracadstd") - 1;
        if (boost::iends_with(fn, ".auracadbak.auracadstd")) {
            fn.erase(fn.size() - backupAndDocExtLen);
            fn += ".auracadStd";
            return fn;
        }
        if (boost::iends_with(fn, ".auracadbak")) {
        ...
        if ((ext == nullptr) || !boost::iequals(ext + 1, "auracadstd")) {
            if (ext && ext[1] == 0) {
                fn += "auracadStd";
            }
            else {
...
{
    if (testStatus(Document::PartialDoc)) {
        AuraCAD_ERR("Partial loaded document '" << Label.getValue() << "' cannot be saved");
...
bool Document::saveToFile(const char* filename) const
{
    ...
    // open extra scope to close ZipWriter properly
    {
        ...
        writer.setComment("AuraCAD Document");
        ...
        writer.Stream() << "<?xml version='1.0' encoding='utf-8'?>" << '\n'
                        << "<!--" << '\n'
                        << " AuraCAD Document, see https://www.AuraCAD.org for more information..."
                        << '\n'
                        << "-->" << '\n';
    ...
    if (policy) {
        ...
        bool useauracadBakExtension =
            GetApplication()
                .GetParameterGroupByPath("User parameter:BaseApp/Preferences/Document")
                ->GetBool("UseauracadBakExtension", true);
        ...
        if (useauracadBakExtension) {
            backupPolicy.setPolicy(BackupPolicy::TimeStamp);
            backupPolicy.useBackupExtension(useauracadBakExtension);
...
bool Document::afterRestore(const bool checkPartial)
{
    ...
    if (!afterRestore(d->objectArray, checkPartial)) {
        AuraCAD_WARN("Reload partial document " << getName());
    ...
    for (auto obj : objArray) {
        ...
        for (auto prop : props) {
            try {
                ...
                AuraCAD_ERR("Failed to restore " << obj->getFullName() << '.' << prop->getName() << ": "
                                            << e.what());
            }
        }
    }
    ...
    for (auto obj : objs) {
        ...
        try {
            ...
            if (returnCode != DocumentObject::StdReturn) {
                AuraCAD_ERR("Expression engine failed to restore " << obj->getFullName() << ": "
                                                              << returnCode->Why);
            ...
            AuraCAD_ERR("Failed to restore " << obj->getFullName() << ": " << e.what());
        }
            ...
            // Segfaults later when AuraCAD interacts with Python.
            if (PyErr_Occurred()) {
            ...
            AuraCAD_ERR("Failed to restore " << obj->getFullName() << ": " << "unknown exception");
        }
        if (obj->isValid()) {
            auto& props = propMap[obj];
            props.clear();
            ...
            obj->getPropertyList(props);
            for (auto prop : props) {
                auto link = AuraCAD_cast<PropertyLinkBase*>(prop);
                int res {0};
                std::string errMsg;
                if (link && ((res = link->checkRestore(&errMsg)) != 0)) {
                    d->touchedObjs.insert(obj);
                    if (res == 1 || checkPartial) {
                        AuraCAD_WARN(obj->getFullName() << '.' << prop->getName() << ": " << errMsg);
                        ...
                        AuraCAD_ERR(obj->getFullName() << '.' << prop->getName() << ": " << errMsg);
...
Document::getDependencyList(const std::vector<DocumentObject*>& objs, int options)
{
    ...
    try {
        ...
        if ((options & DepNoCycle) != 0) {
            ...
            AuraCAD_ERR(ss.str());
            AuraCAD_THROWM(Base::BadGraphError, e.what());
        }
        AuraCAD_ERR(e.what());
                        ...
                        int options)
{
    ...
    if (d->undoing || d->rollback) {
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Ignore document recompute on undo/redo");
        }
        return 0;
    }
    ...
    if (testStatus(Document::PartialDoc)) {
        if (mustExecute()) {
            AuraCAD_WARN("Please reload partial document '" << Label.getValue()
                                                       << "' for recomputation.");
        }
        return 0;
    }
    if (testStatus(Document::Recomputing)) {
        ...
        AuraCAD_ERR("Recursive calling of recompute for document " << getName());
        return 0;
    }
   ...
   */
    ...
    try {
        ...
        for (int passes = 0; passes < 2 && idx < topoSortedObjects.size(); ++passes) {
            ...
            AuraCAD_LOG("Recompute pass " << passes);
            ...
            for (size_t i = 0; i < topoSortedObjects.size(); ++i) {
                ...
                if (!filter.contains(obj) && obj->isTouched()) {
                    if (passes > 0) {
                        AuraCAD_ERR(obj->getFullName() << " still touched after recompute");
                    }
                    else {
                        AuraCAD_LOG(obj->getFullName() << " still touched after recompute");
    ...
    for (auto doc : GetApplication().getDocuments()) {
        ...
        for (auto& o : objects) {
            try {
                ...
                e.reportException();
                AuraCAD_ERR("error when removing object " << o.getDocumentName() << '#'
                                                     << o.getObjectName());
            }
        }
    }
...
{
    AuraCAD_LOG("Recomputing " << Feat->getFullName());
    ...
    try {
        ...
        e.reportException();
        AuraCAD_LOG("Failed to recompute " << Feat->getFullName() << ": " << e.what());
        d->addRecomputeLog("User abort", Feat);
        return -1;
    }
    catch (const Base::MemoryException& e) {
        AuraCAD_ERR("Memory exception in " << Feat->getFullName() << " thrown: " << e.what());
        ...
        AuraCAD_ERR("Exception in " << Feat->getFullName() << " thrown: " << e.what());
        d->addRecomputeLog(e.what(), Feat);
        return 1;
    }
#ifndef AuraCAD_DEBUG
    catch (...) {
        AuraCAD_ERR("Unknown exception in " << Feat->getFullName() << " thrown");
    ...
    if (returnCode == DocumentObject::StdReturn) {
        ...
        d->addRecomputeLog(returnCode);
        AuraCAD_LOG("Failed to recompute " << Feat->getFullName() << ": " << returnCode->Why);
        return 1;
    }
    return 0;
}
...
void Document::removeObject(const char* sName)
{
    ...
    if (pos == d->objectMap.end()){
        AuraCAD_MSG("Object " << sName << " already deleted in document " << getName());
        return;
    }

    if (pos->second->testStatus(ObjectStatus::PendingRecompute)) {
        // TODO: shall we allow removal if there is active undo transaction?
        AuraCAD_MSG("pending remove of " << sName << " after recomputing document " << getName());
...
void Document::_removeObject(DocumentObject* pcObject, RemoveObjectOptions options)
{
    if (!options.testFlag(RemoveObjectOption::MayRemoveWhileRecomputing) && testStatus(Document::Recomputing)) {
        AuraCAD_ERR("Cannot delete " << pcObject->getFullName() << " while recomputing");
        return;
    }
    ...
    if (pos == d->objectMap.end()) {
        AuraCAD_ERR("Internal error, could not find " << pcObject->getFullName() << " to remove");
    }
...
Document::importLinks(const std::vector<DocumentObject*>& objs)
{
    ...
    for (auto obj : links) {
        ...
        for (auto prop : propList) {
            auto linkProp = AuraCAD_cast<PropertyLinkBase*>(prop);
