...
void Transaction::apply(Document& Doc, bool forward)
{
    ...
    if (!errMsg.empty()) {
        AuraCAD_ERR("Exception on " << (forward ? "redo" : "undo") << " '" << Name << "':" << errMsg);
    }
}
...
void TransactionObject::applyChn(Document& /*Doc*/, TransactionalObject* pcObj, bool /* Forward */)
{
    if (status == New || status == Chn) {
        ...
        for (auto& v : _PropChangeMap) {
            ...
            //     AuraCAD_WARN("Cannot " << (Forward?"redo":"undo")
            ...
            try {
                ...
                e.reportException();
                AuraCAD_ERR("exception while restoring " << prop->getFullName() << ": " << e.what());
            }
            catch (std::exception& e) {
                AuraCAD_ERR("exception while restoring " << prop->getFullName() << ": " << e.what());
            }
