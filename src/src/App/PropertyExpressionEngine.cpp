...
AuraCAD_LOG_LEVEL_INIT("App", true);
...
void PropertyExpressionEngine::updateHiddenReference(const std::string& key)
{
    ...
    for (auto& var : it->second) {
        ...
        try {
            ...
            e.reportException();
            AuraCAD_ERR("Failed to evaluate property binding " << myProp->getFullName()
                                                          << " on change of " << key);
        }
        catch (std::bad_cast&) {
            AuraCAD_ERR("Invalid type '" << value.type().name() << "' in property binding "
...
{
    DocumentObject* docObj = AuraCAD_cast<DocumentObject*>(getContainer());
                                                    ...
                                                    const RestoredExpression& info)
{
    try {
        ...
        AuraCAD_ERR("Failed to restore " << docObj->getFullName()
                                    << '.'
                                    << getName()
                                    << ": "
                                    << e.what());
                                                                 ...
                                                                 bool* touched)
{
...
#ifdef AuraCAD_PROPERTYEXPRESSIONENGINE_LOG
    ...
    for (; it != evaluationOrder.end(); ++it) {
        ...
        DocumentObject* parent = AuraCAD_cast<DocumentObject*>(prop->getContainer());
...
{
    DocumentObject* owner = AuraCAD_cast<DocumentObject*>(getContainer());
