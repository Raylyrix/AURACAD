...
/**************************************************************************
 *                                                                         *
 *   Copyright (c) 2021-2023 AuraCAD Project Association                   *
 *                                                                         *
        ...
        switch (format) {
            ...
            default:
                throw Base::XMLBaseException(
                    "package.xml format version is not supported by this version of AuraCAD");
        }
    }
}
...
Meta::Version Metadata::AuraCADmin() const
{
    return _AuraCADmin;
}

Meta::Version Metadata::AuraCADmax() const
{
    return _AuraCADmax;
}
...
void Metadata::setAuraCADMin(const Meta::Version& version)
{
    _AuraCADmin = version;
}
...
void Metadata::setAuraCADMax(const Meta::Version& version)
{
    _AuraCADmax = version;
}
...
void Metadata::write(const fs::path& file) const
{
    ...
    root->setAttribute(XUTF8StrLiteral("xmlns").unicodeForm(),
                       XUTF8StrLiteral("https://wiki.AuraCAD.org/Package_Metadata").unicodeForm());
...
bool Metadata::satisfies(const Meta::Dependency& dep)
{
    ...
    // must contain a valid AuraCAD Expression. If it evaluates to false, this dependency is
...
bool Metadata::supportsCurrentAuraCAD() const
{
    static auto auracadVersion = Meta::Version();
    if (auracadVersion == Meta::Version()) {
        std::map<std::string, std::string>& config = App::Application::Config();
        std::stringstream ss;
        ...
        auracadVersion = Meta::Version(ss.str());
    }

    if (_AuraCADmin != Meta::Version() && _AuraCADmin > auracadVersion) {
        return false;
    }
    if (_AuraCADmax != Meta::Version() && _AuraCADmax < auracadVersion) {
        return false;
    }
    return true;
}
...
void Metadata::appendToElement(DOMElement* root) const
{
    ...
    if (_AuraCADmin != Meta::Version()) {
        appendSimpleXMLNode(root, "AuraCADmin", _AuraCADmin.str());
    }

    if (_AuraCADmax != Meta::Version()) {
        appendSimpleXMLNode(root, "AuraCADmax", _AuraCADmax.str());
    }
...
void Metadata::parseVersion1(const DOMNode* startNode)
{
    ...
    for (XMLSize_t i = 0; i < children->getLength(); ++i) {
        ...
        else if (tagString == "AuraCADmin") {
            _AuraCADmin = Meta::Version(StrXUTF8(element->getTextContent()).str);
        }
        else if (tagString == "AuraCADmax") {
            _AuraCADmax = Meta::Version(StrXUTF8(element->getTextContent()).str);
        }
