                ...
                for (const Base::FileInfo& it : files) {
                    if (it.isFile()) {
                        ...
                        if ((startsWith(file, filename) && (file.length() > filename.length())
                            ...
                            ||
                            // .auracadBak case : The bame starts with the base name of the project +
                            // "."
                            // + complement with no "." + ".auracadBak"
                            ((fextUp == "auracadBAK") && startsWith(file, pbn)
            ...
            // create a new backup file
            {
                ...
                if (useauracadBakExtension) {
                    ...
                    if ((fn.empty()) || (fn[fn.length() - 1] == ' ')
                        ...
                        if (!renameFileNoErase(fi, fn + ".auracadBak")) {
                            fn = fn + "-";
                        }
                        else {
                            done = true;
                    ...
                    if (!done) {
                        while (ext2 < numberOfFiles + 10) {
                            if (renameFileNoErase(fi, fn + std::to_string(ext2) + ".auracadBak")) {
                                break;
                            }
                            ext2++;
                        }
