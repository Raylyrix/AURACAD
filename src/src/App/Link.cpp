...
void LinkBaseExtension::setProperty(int idx, Property* prop)
{
    ...
    switch (idx) {
        ...
        case PropLinkCopyOnChangeGroup:
            if (auto linkProp = AuraCAD_cast<PropertyLinkBase*>(prop)) {
    ...
    if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_TRACE)) {
        ...
        AuraCAD_TRACE("set property " << infos[idx].name << ": " << propName);
    }
}
...
App::DocumentObjectExecReturn* LinkBaseExtension::extensionExecute()
{
    ...
    if (getLinkedObjectProperty()) {
        ...
        if (!linked) {
            ...
            ss << "Link broken!";
            auto xlink = AuraCAD_cast<PropertyXLink*>(getLinkedObjectProperty());
            if (xlink) {
        ...
        if (getLinkExecuteProperty() && !boost::iequals(getLinkExecuteValue(), "none")
            ...
            proxy = AuraCAD_cast<PropertyPythonObject*>(
                                          ...
                                          App::DocumentObject* src)
{
    ...
    for (auto it = res.begin(); it != res.end();) {
        ...
        auto prop =
            AuraCAD_cast<PropertyMap*>(obj->getPropertyByName("_CopyOnChangeControl"));
...
void LinkBaseExtension::setOnChangeCopyObject(App::DocumentObject* obj, OnChangeCopyOptions options)
{
    ...
    if (!prop) {
        ...
        if (!prop) {
            AuraCAD_ERR("Failed to setup copy on change object " << obj->getFullName());
            return;
        }
    }
...
void LinkBaseExtension::syncCopyOnChange()
{
    ...
    if (auto prop = getLinkCopyOnChangeGroupProperty()) {
        copyOnChangeGroup = AuraCAD_cast<LinkGroup*>(prop->getValue());
        if (!copyOnChangeGroup) {
            ...
            for (auto obj : objs) {
                ...
                auto prop =
                    AuraCAD_cast<PropertyUUID*>(obj->getPropertyByName("_SourceUUID"));
    ...
    for (auto obj : copiedObjs) {
        auto prop = AuraCAD_cast<PropertyUUID*>(obj->getPropertyByName("_SourceUUID"));
        if (prop) {
    ...
    for (const auto& objT : oldObjs) {
        auto prop = AuraCAD_cast<PropertyUUID*>(objT.getProperty());
        if (!prop) {
            continue;
        }
        ...
        for (auto doc : App::GetApplication().getDocuments()) {
            for (auto o : doc->getObjects()) {
                ...
                for (auto prop : propList) {
                    ...
                    auto linkProp = AuraCAD_cast<App::PropertyLinkBase*>(prop);
                    if (!linkProp) {
                        continue;
                    }
...
App::DocumentObject* LinkBaseExtension::makeCopyOnChange()
{
    ...
    for (auto obj : srcobjs) {
        if (obj->testStatus(App::PartialObject)) {
            AuraCAD_THROWM(Base::RuntimeError,
...
void LinkBaseExtension::parseSubName() const
{
    ...
    mySubElements.clear();
    mySubName.clear();
    auto xlink = AuraCAD_cast<const PropertyXLink*>(getLinkedObjectProperty());
...
void LinkBaseExtension::updateGroup()
{
    ...
    if (!groups.empty()) {
        ...
        for (auto ext : groups) {
            ...
            if (!conn.connected()) {
                AuraCAD_LOG("new group connection " << getExtendedObject()->getFullName() << " -> "
                                               << group->getFullName());
                // NOLINTBEGIN
...
void LinkBaseExtension::update(App::DocumentObject* parent, const Property* prop)
{
    ...
    if (prop == getLinkPlacementProperty() || prop == getPlacementProperty()) {
        ...
        if (_getShowElementValue()) {
            ...
            for (auto obj : objs) {
                auto element = AuraCAD_cast<LinkElement*>(obj);
                if (element) {
        ...
        if (!_getShowElementValue()) {
            ...
            if (elementCount > objs.size()) {
                ...
                for (size_t i = objs.size(); i < elementCount; ++i) {
                    ...
                    auto obj = AuraCAD_cast<LinkElement*>(doc->getObject(name.c_str()));
                    if (obj
                ...
                while (objs.size() > elementCount) {
                    auto element = AuraCAD_cast<LinkElement*>(objs.back());
...
void LinkBaseExtension::syncElementList()
{
    ...
    auto xlink = AuraCAD_cast<const PropertyXLink*>(link);

    auto owner = getContainer();
    auto ownerID = owner ? owner->getID() : 0;
    auto elements = getElementListValue();
    for (auto i : elements) {
        auto element = AuraCAD_cast<LinkElement*>(i);
        if (!element
...
void LinkBaseExtension::onExtendedDocumentRestored()
{
    ...
    if (hasOldSubElement) {
        ...
        auto xlink = AuraCAD_cast<PropertyXLink*>(getLinkedObjectProperty());
        if (!xlink) {
            AuraCAD_ERR("Failed to restore SubElements for " << parent->getFullName());
        }
        else if (!xlink->getValue()) {
            AuraCAD_ERR("Discard SubElements of " << parent->getFullName() << " due to null link");
        }
                                ...
                                const std::vector<std::string>& subElements)
{
    ...
    if (index >= 0) {
        ...
        if (!obj) {
            ...
            if (getLinkModeValue() >= LinkModeAutoLink || !Base::Tools::isNullOrEmpty(subname)
                ...
                auto pla =
                    AuraCAD_cast<PropertyPlacement*>(obj->getPropertyByName("Placement"));
                if (pla) {
    ...
    auto xlink = AuraCAD_cast<PropertyXLink*>(linkProp);
    if (obj) {
        if (!obj->isAttachedToDocument()) {
