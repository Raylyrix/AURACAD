...
AuraCAD_LOG_LEVEL_INIT("ComplexGeoData", true, true)  // NOLINT
...
size_t ComplexGeoData::getElementMapSize(bool flush) const
{
    if (flush) {
        flushElementMap();
#ifdef _AuraCAD_MEM_TRACE
        AuraCAD_MSG("memory size " << (_MemSize / 1024 / 1024) << "MB, " << (_MemMaxSize / 1024 / 1024));
        for (auto& unit : _MemUnits) {
            AuraCAD_MSG("unit " << unit.first << ": " << unit.second.count << ", "
...
void ComplexGeoData::readElements(Base::XMLReader& reader, size_t count)
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (reader.hasAttribute("sid")) {
            if (!Hasher) {
                if (!warned) {
                    warned = true;
                    AuraCAD_ERR("missing hasher");  // NOLINT
                }
            }
            else {
    ...
    if (invalid_count != 0) {
        AuraCAD_ERR("Found " << invalid_count << " invalid string id");  // NOLINT
    }
}
    ...
    try {
        for (size_t i = 0; i < count; ++i) {
            ...
            if (!(stream >> value >> key >> sCount)) {
                // NOLINTNEXTLINE
                AuraCAD_THROWM(Base::RuntimeError, "Failed to restore element map " << _persistenceName);
            }
            constexpr std::size_t oneGbOfInts {(1 << 30) / sizeof(int)};
            if (sCount > oneGbOfInts) {
                // NOLINTNEXTLINE
                AuraCAD_THROWM(Base::RuntimeError, "Failed to restore element map (>1GB) " << _persistenceName);
            }
...
void ComplexGeoData::RestoreDocFile(Base::Reader& reader)
{
    ...
    if (boost::equals(marker, "BeginElementMap")) {
        ...
        reader >> ver;
        if (ver != "v1") {
            AuraCAD_WARN("Unknown element map format");  // NOLINT
        }
        else {
            resetElementMap(std::make_shared<ElementMap>());
