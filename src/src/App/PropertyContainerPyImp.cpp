...
AuraCAD_LOG_LEVEL_INIT("Property", true, 2)
...
PyObject* PropertyContainerPy::getPropertyByName(PyObject* args)
{
    ...
    if (!prop) {
        PyErr_Format(Base::PyExc_AuraCAD_PropertyError, "Property container has no property '%s'", pstr);
        return nullptr;
    }
    ...
    for (const auto& item : items) {
        ...
        if (item.isString()) {
            ...
            if (it == statusMap.end()) {
                ...
                auto lengthProp = AuraCAD_cast<App::PropertyLength*>(prop);
...
PyObject* PropertyContainerPy::getEnumerationsOfProperty(PyObject* args)
{
    ...
    PropertyEnumeration* enumProp = AuraCAD_cast<PropertyEnumeration*>(prop);
    if (!enumProp) {
        Py_Return;
    }
...
PyObject* PropertyContainerPy::getCustomAttributes(const char* attr) const
{
    ...
    if (AuraCAD_LOG_INSTANCE.level() > AuraCAD_LOGLEVEL_TRACE) {
        AuraCAD_TRACE("Get property " << attr);
    }
...
int PropertyContainerPy::setCustomAttributes(const char* attr, PyObject* obj)
{
    ...
    if (prop) {
        ...
        AuraCAD_TRACE("Set property " << prop->getFullName());
