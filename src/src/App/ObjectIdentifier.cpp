...
AuraCAD_LOG_LEVEL_INIT("Expression", true, true)
...
{
    if (_owner) {
        const DocumentObject* docObj = AuraCAD_cast<const DocumentObject*>(_owner);
        if (!docObj) {
            AuraCAD_THROWM(Base::RuntimeError, "Property must be owned by a document object.");
        }
...
ObjectIdentifier::ObjectIdentifier(const Property& prop, int index)
    ...
    , _hash(0)
{
    DocumentObject* docObj = AuraCAD_cast<DocumentObject*>(prop.getContainer());

    if (!docObj) {
        AuraCAD_THROWM(Base::TypeError, "Property must be owned by a document object.");
    }
    if (!prop.hasName()) {
        AuraCAD_THROWM(Base::RuntimeError, "Property must have a name.");
    }
                                                                                    ...
                                                                                    int* idx) const
{
    ...
    if (i < 0 || i >= static_cast<int>(components.size())) {
        AuraCAD_THROWM(Base::ValueError, "Invalid property component index");
    }
...
void App::ObjectIdentifier::setComponent(int idx, Component&& comp)
{
    if (idx < 0 || idx >= static_cast<int>(components.size())) {
        AuraCAD_THROWM(Base::ValueError, "Invalid component index");
    }
...
bool ObjectIdentifier::verify(const App::Property& prop, bool silent) const
{
    ...
    if (components.size() - result.propertyIndex != 1) {
        ...
        AuraCAD_THROWM(Base::ValueError, "Invalid property path: single component expected");
    }
    if (!components[result.propertyIndex].isSimple()) {
        if (silent) {
            return false;
        }
        AuraCAD_THROWM(Base::ValueError, "Invalid property path: simple component expected");
    }
    ...
    if ((isAddress && addr.toString(CellAddress::Cell::ShowRowColumn) != prop.getName())
        ...
        AuraCAD_THROWM(Base::ValueError, "Invalid property path: name mismatch");
    }
    return true;
}
...
size_t ObjectIdentifier::Component::getIndex(size_t count) const
{
    ...
    AuraCAD_THROWM(Base::IndexError, "Array out of bound: " << begin << ", " << count);
}
...
Py::Object ObjectIdentifier::Component::get(const Py::Object& pyobj) const
{
    ...
    if (isSimple()) {
        if (!pyobj.hasAttr(getName())) {
            AuraCAD_THROWM(Base::AttributeError, "No attribute named '" << getName() << "'");
        }
    ...
    if (PyModule_Check(res.ptr()) && !ExpressionParser::isModuleImported(res.ptr())) {
        AuraCAD_THROWM(Base::RuntimeError, "Module '" << getName() << "' access denied.");
    }
    return res;
}
                                                         ...
                                                         std::bitset<32>& flags)
{
    ...
    for (auto docObject : docObjects) {
        if (strcmp(docObject->Label.getValue(), static_cast<const char*>(name)) == 0) {
            ...
            if (objectByLabel) {
                AuraCAD_WARN("duplicate object label " << doc->getName() << '#'
...
ObjectIdentifier ObjectIdentifier::parse(const DocumentObject* docObj, const std::string& str)
{
    ...
    VariableExpression* v = AuraCAD_cast<VariableExpression*>(expr.get());

    if (v) {
        return v->getPath();
    }
    else {
        AuraCAD_THROWM(Base::RuntimeError, "Invalid property specification.");
    }
}
                                             ...
                                             bool checkImport)
{
    if (!owner || !obj || !obj->isAttachedToDocument() || !obj->getDocument()) {
        AuraCAD_THROWM(Base::RuntimeError, "invalid object");
    }
        ...
        if (obj || objName) {
            ...
            if (restoreLabel) {
                if (!obj) {
                    ...
                    if (!obj) {
                        AuraCAD_ERR("Cannot find object " << objName->toString());
                    }
                }
            ...
            if (!objForMapped) {
                AuraCAD_ERR("Cannot find object " << str);
            }
            else {
                isString = true;
...
ObjectIdentifier::access(const ResolveResults& result, const Py::Object* value, Dependencies* deps) const
{
    if (!result.resolvedDocumentObject || !result.resolvedProperty
        ...
        AuraCAD_THROWM(Base::RuntimeError, result.resolveErrorString() << " in '" << toString() << "'");
    }
...
#define GET_MODULE(_name)                                                                          \
    ...
    switch (ptype) {
        case PseudoApp:
            ...
            break;
        case PseudoGui:
            GET_MODULE(AuraCADGui);
            break;
        case PseudoPart:
            GET_MODULE(Part);
            break;
        case PseudoCadquery:
            GET_MODULE(AuraCAD.AuraCAD_cadquery);
            break;
        case PseudoRegex:
            GET_MODULE(re);
            break;
                ...
                default: {
                    ...
                    if (container && container != result.resolvedDocumentObject
                        ...
                        if (!container->isDerivedFrom<DocumentObject>()) {
                            AuraCAD_WARN("Invalid property container");
                        }
                        else {
    ...
    auto setPropDep = [deps](DocumentObject* obj, Property* prop, const char* propName) {
        ...
        if (prop && prop->getContainer() != obj) {
            auto linkTouched =
                AuraCAD_cast<PropertyBool*>(obj->getPropertyByName("_LinkTouched"));
            if (linkTouched) {
                propName = linkTouched->getName();
            }
            else {
                auto propOwner = AuraCAD_cast<DocumentObject*>(prop->getContainer());
                if (propOwner) {
                    obj = propOwner;
                }
                else {
...
void ObjectIdentifier::setValue(const App::any& value) const
{
    ...
    ResolveResults rs(*this);
    if (rs.propertyType) {
        AuraCAD_THROWM(Base::RuntimeError, "Cannot set pseudo property");
    }
...
void ObjectIdentifier::importSubNames(const ObjectIdentifier::SubNameMap& subNameMap)
{
    ...
    if (it != subNameMap.end()) {
        ...
        if (!obj) {
            AuraCAD_ERR("Failed to find import object " << it->second << " from "
