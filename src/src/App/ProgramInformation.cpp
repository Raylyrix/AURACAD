...
{
    // report also the version numbers of the most important libraries in AuraCAD
    str << "Python " << PY_VERSION << ", ";
    str << "Qt " << QT_VERSION_STR << ", ";
    str << "Coin " << auracadCoin3dVersion << ", ";
    str << "Vtk " << auracadVtkVersion << ", ";
    str << "boost " << BOOST_LIB_VERSION << ", ";
    str << "Eigen3 " << auracadEigen3Version << ", ";
    str << "PySide " << auracadPysideVersion << '\n';
    str << "shiboken " << auracadShibokenVersion << ", ";
#ifdef SMESH_VERSION_STR
    str << "SMESH " << SMESH_VERSION_STR << ", ";
#endif
    str << "xerces-c " << auracadXercescVersion << ", ";
    getIauracadInfo(str);
...
void ProgramInformation::getIauracadInfo(std::stringstream& str)
{
    try {
        Base::PyGILStateLocker lock;
        Py::Module module(PyImport_ImportModule("iauracadopenshell"), true);
        if (!module.isNull() && module.hasAttr("version")) {
            ...
            str << "IauracadOpenShell " << ver_str << ", ";
        }
        else {
            Base::Console().log("Module 'iauracadopenshell' not found (safe to ignore, unless using "
                                "the BIM workbench and Iauracad).\n");
        }
    }
