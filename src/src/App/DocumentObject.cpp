...
{
    return AuraCAD_cast<DocumentObjectGroup*>(GroupExtension::getGroupOfObject(this));
}
...
void DocumentObject::onEarlyChange(const Property* prop)
{
    ...
    if (!GetApplication().isRestoring() && !prop->testStatus(Property::PartialTrigger)
        ...
        if (warnedDoc != getDocument()) {
            ...
            AuraCAD_WARN("Changes to partial loaded document will not be saved: " << getFullName() << '.'
                                                                             << prop->getName());
        }
    }
...
void DocumentObject::onChanged(const Property* prop)
{
    ...
    if (!testStatus(ObjectStatus::NoTouch) && !(prop->getType() & Prop_Output)
        ...
        if (!StatusBits.test(ObjectStatus::Touch)) {
            AuraCAD_TRACE("touch '" << getFullName() << "' on change of '" << prop->getName() << "'");
            StatusBits.set(ObjectStatus::Touch);
        }
                                             ...
                                             int depth) const
{
    ...
    // should change that.
    if (transform && mat) {
        auto pla = AuraCAD_cast<PropertyPlacement*>(getPropertyByName("Placement"));
        if (pla) {
...
{
    auto prop = AuraCAD_cast<const PropertyComplexGeoData*>(_prop);
    if (!prop) {
        return std::string();
    }
...
{
    Base::Placement plc;
    auto* propPlacement = AuraCAD_cast<App::PropertyPlacement*>(getPropertyByName("Placement"));
    if (propPlacement) {
