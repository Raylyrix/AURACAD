                                            ...
                                            const Data::MappedElement& mapped) const
{
    ...
    if (mapped.index && mapped.name) {
        ...
        //        AuraCAD_TRACE("element mapped name " << name << " not found in " << getFullName());
                                           ...
                                           GeoFeature** geoFeature)
{
    ...
    auto geo = AuraCAD_cast<GeoFeature*>(linked);
    if (!geo && linked) {
        auto ext = linked->getExtensionByType<LinkBaseExtension>(true);
        if (ext) {
            geo = AuraCAD_cast<GeoFeature*>(ext->getTrueLinkedObject(true));
        }
    }
    if (geoFeature) {
        *geoFeature = geo;
