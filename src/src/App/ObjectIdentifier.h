...
#define AuraCAD_DEFAULT_CTORS(_t)                                                                       \
    _t(const _t&) = default;                                                                       \
...
public:
    ...
    public:
        ...
        /**
         * @brief Explicit moveâ€construct a String object for object identifiers.
         *
        ...
        bool operator>(const String& other) const
        {
          ...
          *
          * Applies name mapping and, if â€œ@â€ markers are present, defers label
          * substitution to import-time via PropertyLinkBase::importSubName() and
          * PropertyLinkBase::restoreLabelReference().
          *
          * @param[in] owner   The importing documentâ€™s owner object.
    ...
    public:
        // Constructors
        AuraCAD_DEFAULT_CTORS(Component)
        {
    ...
    AuraCAD_DEFAULT_CTORS(ObjectIdentifier)
    {
        owner = other.owner;
     ...
     * identifier.
     *
     * All type of objects are supported. Some types are casted to auracad native
