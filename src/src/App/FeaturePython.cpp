...
#define AuraCAD_PY_CALL_CHECK(_name) _AuraCAD_PY_CALL_CHECK(_name, return (false))
...
{
    AuraCAD_PY_CALL_CHECK(mustExecute)
    Base::PyGILStateLocker lock;
    try {
        if (has__object__) {
...
{
    _AuraCAD_PY_CALL_CHECK(onDocumentRestored, return);
...
{
    _AuraCAD_PY_CALL_CHECK(unsetupObject, return);
...
bool FeaturePythonImp::getSubObject(DocumentObject*& ret,
                                    ...
                                    int depth) const
{
    AuraCAD_PY_CALL_CHECK(getSubObject);
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(6);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    AuraCAD_PY_CALL_CHECK(getSubObjects);
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(2);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
bool FeaturePythonImp::getLinkedObject(DocumentObject*& ret,
                                       ...
                                       int depth) const
{
    AuraCAD_PY_CALL_CHECK(getLinkedObject);
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(5);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(hasChildElement, return (NotImplemented));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(isElementVisible, return (-2));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(2);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(setElementVisible, return (-2));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(3);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(getViewProviderName, return (std::string()));
    Base::PyGILStateLocker lock;
    try {
...
{
    _AuraCAD_PY_CALL_CHECK(canLinkProperties, return (NotImplemented));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(allowDuplicateLabel, return (NotImplemented));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(canLoadPartial, return (-1));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(supportsAsyncRecompute, return (NotImplemented));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(redirectSubName, return (NotImplemented));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(4);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(isLink, return (NotImplemented));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    _AuraCAD_PY_CALL_CHECK(isLinkGroup, return (NotImplemented));
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::Object(object->getPyObject(), true));
...
{
    AuraCAD_PY_CALL_CHECK(getPlacementOf);  // Standard macro check
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(3);
