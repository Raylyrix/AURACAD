 ...
 *
 * Modules already in @c sys.modules are permitted -- they were loaded by AuraCAD core or addons
 ...
 * a AuraCAD module directory.  This prevents a crafted auracadStd from importing arbitrary modules
 * (whose <tt>__init__.py</tt> could run malicious code on import) while still allowing
 * legitimate lazy-loaded AuraCAD workbench modules to restore.
 *
...
bool isAllowedModule(const std::string& moduleName)
{
    ...
    // AuraCAD adds each workbench directory to sys.path individually (e.g. .../Mod/Assembly/),
    // so a module stored as "Assembly.JointObject" in the auracadStd is actually importable as just
    ...
    // Use AuraCAD.__ModDirs__ as the authoritative list of allowed module directories.
    // This is populated during startup by AuraCADInit.py and includes built-in workbenches,
    // user addons, and any additional configured module paths.
    Py::Module AuraCAD(PyImport_ImportModule("AuraCAD"), true);
    if (!AuraCAD.hasAttr("__ModDirs__")) {
        throw Py::RuntimeError("AuraCAD.__ModDirs__ not set -- AuraCADInit.py has not run yet");
    }
    Py::List modDirs(AuraCAD.getAttr("__ModDirs__"));

    auto isUnderAuraCAD = [&](const std::string& path) {
    ...
    if (!origin.isNone() && origin.isString()) {
        return isUnderAuraCAD(Py::String(origin).as_std_string());
    }

    // No origin -- this could be a built-in module (why is an auracadStd trying to load this? Very
    // suspicious, block it) or a synthetic module from a AuraCAD migration finder like
    // FemMigrateApp (which we will allow).  Check whether the spec's loader itself comes from a
    // AuraCAD module directory.
    ...
    return isUnderAuraCAD(Py::String(loaderMod.getAttr("__file__")).as_std_string());
}
            ...
            if (reader.hasAttribute("module") && reader.hasAttribute("class")) {
                ...
                if (!isAllowedModule(moduleName)) {
                    Base::Console().warning(
                        ...
                        " document restore. Only modules from AuraCAD or installed addons"
                        " are permitted.\n",
                        moduleName.c_str());
