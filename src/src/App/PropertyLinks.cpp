...
AuraCAD_LOG_LEVEL_INIT("PropertyLinks", true, true)
...
{
    auto owner = AuraCAD_cast<DocumentObject*>(getContainer());
    if (owner) {
...
static std::string propertyName(const Property* prop)
{
    ...
    if (!prop->getContainer() || !prop->hasName()) {
        auto xlink = AuraCAD_cast<const PropertyXLink*>(prop);
        if (xlink) {
    ...
    for (auto prop : props) {
        if (prop->getContainer()) {
            try {
                ...
                e.reportException();
                AuraCAD_ERR("Failed to update element reference of " << propertyName(prop));
            }
                                             ...
                                             ShadowSub* shadow)
{
    ...
    for (char* dot = strchr(next, '.'); dot; next = dot + 1, dot = strchr(next, '.')) {
        ...
        try {
            ...
            if (!sobj) {
                AuraCAD_ERR("Failed to restore label reference " << obj->getFullName() << '.'
                                                            << ss.str());
                return;
            }
        }
                                               ...
                                               bool notify)
{
    ...
    if (missing) {
        AuraCAD_WARN(propertyName(this)
                << " missing element reference " << ret->getFullName() << " "
                << (elementName.newName.size() ? elementName.newName : elementName.oldName));
        shadow.oldName.swap(elementName.oldName);
    }
    else {
        AuraCAD_TRACE(propertyName(this) << " element reference shadow update " << ret->getFullName()
    ...
    if (pos == pos2) {
        if (sub.compare(pos, sub.size() - pos, &shadow.newName[pos2]) != 0) {
            AuraCAD_LOG("element reference update " << sub << " -> " << shadow.newName);
        ...
        AuraCAD_LOG("element reference update " << sub << " -> " << shadow.oldName);
        updateSub(shadow.oldName);
    }
    return true;
}
...
void PropertyLinkList::setValues(const std::vector<DocumentObject*>& value)
{
    ...
    auto parent = AuraCAD_cast<App::DocumentObject*>(getContainer());
    for (auto obj : value) {
        if (!obj || !obj->isAttachedToDocument()) {
...
void PropertyLinkList::Restore(Base::XMLReader& reader)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (child) {
            ...
            AuraCAD_WARN("Lost link to " << (document ? document->getName() : "") << " " << name
                                ...
                                std::vector<PropertyLinkBase::ShadowSub>& shadows)
{
    ...
    auto owner = AuraCAD_cast<DocumentObject*>(prop->getContainer());
                                            ...
                                            bool first_obj)
{
    ...
    if (first_obj) {
        ...
        if (!doc) {
            AuraCAD_ERR("Failed to get document for the first object in " << sub);
            return res;
        }
    ...
    for (const char* dot = strchr(sub, '.'); dot; sub = dot + 1, dot = strchr(sub, '.')) {
        ...
        if (!obj || !obj->isAttachedToDocument()) {
            AuraCAD_WARN("missing sub object '" << name << "' in '" << sub << "'");
            break;
        }
        if (obj->isExporting()) {
            if (name[0] == '$') {
                                                 ...
                                                 const std::map<std::string, std::string>& nameMap)
{
    if (doc && obj && obj->isAttachedToDocument()) {
        ...
        if (it != nameMap.end()) {
            ...
            if (!obj) {
                AuraCAD_THROWM(Base::RuntimeError, "Cannot find import object " << it->second);
            }
        }
    }
                                               ...
                                               const std::map<std::string, std::string>& nameMap)
{
    ...
    for (char* dot = strchr(next, '.'); dot; next = dot + 1, dot = strchr(next, '.')) {
        ...
        if (!sobj) {
            AuraCAD_ERR("Failed to restore label reference " << obj->getFullName() << '.' << subname);
            return {};
        }
        dot[0] = 0;
        if (next[0] == '$') {
        ...
        if (!imported) {
            AuraCAD_THROWM(RuntimeError, "Failed to find imported object " << it->second);
        }
        ss.write(sub, next - sub);
        if (next[0] == '$') {
    ...
    for (unsigned int i = 0; i < _cSubList.size(); i++) {
        ...
        if (exporting) {
            ...
            if (!_cSubList[i].empty()) {
                if (sub != _cSubList[i]) {
                    // Stores the actual value that is shadowed. For new version auracad,
...
void PropertyLinkSubList::Restore(Base::XMLReader& reader)
{
    ...
    DocumentObject* father = AuraCAD_cast<DocumentObject*>(getContainer());
...
public:
    ...
    get(const char* filename, App::Document* pDoc, PropertyXLink* l, const char* objName)
    {
        ...
        AuraCAD_LOG("finding doc " << filename);
        ...
        if (info->pcDoc) {
            ...
            auto owner = AuraCAD_cast<DocumentObject*>(l->getContainer());
    ...
    void deinit()
    {
        AuraCAD_LOG("deinit " << (pcDoc ? pcDoc->getName() : filePath()));
        assert(links.empty());
    ...
    void init(DocInfoMap::iterator pos, const char* objName, PropertyXLink* l)
    {
        ...
        if (fullpath.isEmpty()) {
            AuraCAD_ERR("document not found " << filePath());
        }
        else {
            for (App::Document* doc : App::GetApplication().getDocuments()) {
                if (getFullPath(doc->getFileName()) == fullpath) {
            ...
            AuraCAD_LOG("document pending " << filePath());
    ...
    void attach(Document* doc)
    {
        assert(!pcDoc);
        pcDoc = doc;
        AuraCAD_LOG("attaching " << doc->getName() << ", " << doc->getFileName());
        ...
        for (auto it = links.begin(), itNext = it; it != links.end(); it = itNext) {
            ...
            if (obj) {
                ...
                App::GetApplication().addPendingDocument(doc->FileName.getValue(),
                                                         ...
                                                         false);
                AuraCAD_WARN("reloading partial document '" << doc->FileName.getValue()
                                                       << "' due to object " << link->objectName);
            }
            else {
                AuraCAD_WARN("object '" << link->objectName << "' not found in document '"
                                   << doc->getName() << "'");
            }
        }
            ...
            for (auto link : v.second) {
                ...
                if (obj) {
                    ...
                    App::GetApplication().addPendingDocument(doc->FileName.getValue(),
                                                             link->objectName.c_str(),
                                                             false);
                    AuraCAD_WARN("reloading partial document '"
    ...
    void slotSaveDocument(const App::Document& doc)
    {
        ...
        if (path.isEmpty() || path != docPath) {
            AuraCAD_LOG("document '" << doc.getName() << "' path changed");
            auto me = shared_from_this();
            auto ret = _DocInfoMap.insert(std::make_pair(docPath, me));
            if (!ret.second) {
                // is that even possible?
                AuraCAD_WARN("document '" << doc.getName() << "' path exists, detach");
        ...
        for (auto link : links) {
            ...
            if (ret.second) {
                ...
                AuraCAD_LOG("touch document " << linkdoc->getName() << " on time stamp change of "
                                         << link->getFullName());
    ...
    void slotDeleteDocument(const App::Document& doc)
    {
        for (auto it = links.begin(), itNext = it; it != links.end(); it = itNext) {
            ++itNext;
            auto link = *it;
            auto obj = AuraCAD_cast<DocumentObject*>(link->getContainer());
...
PropertyXLink::~PropertyXLink()
{
    try {
        ...
        AuraCAD_WARN("Bad pointer exception caught when destroying PropertyXLink\n");
    }
}
    ...
    if (lValue) {
        ...
        if (lValue->getDocument() != owner->getDocument()) {
            if (!docInfo || lValue->getDocument() != docInfo->pcDoc) {
                ...
                AuraCAD_LOG("xlink set to new document " << lValue->getDocument()->getName());
...
bool PropertyXLink::upgrade(Base::XMLReader& reader, const char* typeName)
{
    ...
    AuraCAD_ERR("Cannot upgrade from " << typeName);
    return false;
}
        ...
        if (exporting) {
            ...
            if (docInfo) {
                ...
                if (!Base::Tools::isNullOrEmpty(docPath)) {
                    ...
                    AuraCAD_WARN("PropertyXLink export without saving the document");
                }
            }
            if (!_path.empty()) {
                path = _path.c_str();
...
{
    auto owner = AuraCAD_cast<const DocumentObject*>(getContainer());
...
void PropertyXLink::Paste(const Property& from)
{
    ...
    if (!other.docName.empty()) {
        ...
        if (!doc) {
            AuraCAD_WARN("Document '" << other.docName << "' not found");
            return;
        }
        auto obj = doc->getObject(other.objectName.c_str());
        if (!obj) {
            AuraCAD_WARN("Object '" << other.docName << '#' << other.objectName << "' not found");
            return;
        }
        setValue(obj,
...
#define CHECK_SUB_SIZE(_l, _r)                                                                     \
    do {                                                                                           \
        if (_l.size() != _r.size())                                                                \
            AuraCAD_THROWM(Base::ValueError, "object and subname size mismatch");                       \
    } while (0)
    ...
    std::map<App::DocumentObject*, std::vector<std::string>>&& values)
{
    for (auto& v : values) {
        if (!v.first || !v.first->isAttachedToDocument()) {
            AuraCAD_THROWM(Base::ValueError, "invalid document object");
        }
    }
...
const std::vector<std::string>& PropertyXLinkSubList::getSubValues(App::DocumentObject* obj) const
{
    ...
    AuraCAD_THROWM(Base::RuntimeError, "object not found");
}
...
void PropertyXLinkContainer::Save(Base::Writer& writer) const
{
    ...
    auto owner = AuraCAD_cast<App::DocumentObject*>(getContainer());
...
void PropertyXLinkContainer::Restore(Base::XMLReader& reader)
{
    ...
    if (reader.hasAttribute("docs")) {
        ...
        for (unsigned i = 0; i < docCount; ++i) {
            ...
            if (index >= count) {
                AuraCAD_ERR(propertyName(this) << " invalid document map entry");
                continue;
            }
...
void PropertyXLinkContainer::_onBreakLink(DocumentObject* obj)
{
    try {
        ...
        e.reportException();
        AuraCAD_ERR("Exception on breaking link property " << getFullName());
    }
    catch (std::exception& e) {
        AuraCAD_ERR("Exception on breaking link property " << getFullName() << ": " << e.what());
    }
    catch (...) {
        AuraCAD_ERR("Exception on breaking link property " << getFullName());
    }
