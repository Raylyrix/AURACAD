...
App.Console.PrintLog("Init: starting App::AuraCADInit.py\n")
App.Console.PrintLog("â–‘â–‘â–‘â–€â–ˆâ–€â–‘â–ˆâ–€â–ˆâ–‘â–€â–ˆâ–€â–‘â–€â–ˆâ–€â–‘â–‘â–‘â–ˆâ–€â–ˆâ–‘â–ˆâ–€â–ˆâ–‘â–ˆâ–€â–ˆâ–‘â–‘\n")
App.Console.PrintLog("â–‘â–‘â–‘â–‘â–ˆâ–‘â–‘â–ˆâ–‘â–ˆâ–‘â–‘â–ˆâ–‘â–‘â–‘â–ˆâ–‘â–‘â–‘â–‘â–ˆâ–€â–ˆâ–‘â–ˆâ–€â–€â–‘â–ˆâ–€â–€â–‘â–‘\n")
App.Console.PrintLog("â–‘â–‘â–‘â–€â–€â–€â–‘â–€â–‘â–€â–‘â–€â–€â–€â–‘â–‘â–€â–‘â–‘â–‘â–‘â–€â–‘â–€â–‘â–€â–‘â–‘â–‘â–€â–‘â–‘â–‘â–‘\n")
...
class auracadADLogger:
    """
    Convenient class for tagged logging.

    Example usage:
        >>> logger = AuraCAD.Logger('MyModule')
    ...
    To set/get logger level:
        >>> AuraCAD.setLogLevel('MyModule','Trace')
        >>> AuraCAD.getLogLevel('MyModule')
        4
    ...
    AuraCAD.setLogLevel() supports both text and integer value, which allows
    ...
    def __init__(self, tag: str, **kwargs) -> None:
        ...
        * tag: a string tag for this logger. The log level of this logger can be
               accessed using AuraCAD.getLogLevel(tag)/setLogLevel(tag,level).
        ...
        * title ('AuraCAD'): message box title used by report()
        """
        self.tag = tag
    ...
    def report(self, msg: str, func: callable, *args, **kwargs) -> object | None:
        ...
        * kwargs: dictionary of keyword arguments to be passed to func.
        ...
        try:
            ...
            if App.GuiUp:
                import AuraCADGui, PySide
                PySide.QtGui.QMessageBox.critical(
                    AuraCADGui.getMainWindow(),
                    self.title,
                    str(e),
                )
        return None
...
App.addImportType("AuraCAD document (*.auracadStd)", "AuraCAD")
...
class Transient:
    ...
    def cleanup(cls) -> None:
        ...
        keep = set(("__builtins__", "AuraCAD", "App", "os", "sys", "traceback", "inspect"))
...
class WindowsPlatform:
    ...
    def __init__(self) -> None:
        ...
        if lib_pack := utils.env_to_path("AuraCAD_LIBPACK_BIN"):
...
class DarwinPlatform:
    ...
    def post(self) -> None:
        ...
        sys.path.append(os.path.expanduser("~/Library/Application Support/AuraCAD/Mod"))
...
class Mod:
    ...
    def supports_AuraCAD_version(self) -> bool:
        """
        Check if the Mod supports the current AuraCAD version.
        """
        if meta := self.metadata:
            return meta.supportsCurrentAuraCAD()
        return True
...
class ExtMod(Mod):
    ...
    extension modules must be defined in namespace AuraCAD.*, i.e. AuraCAD.MyAddon.
    """

    name: str  # full module name, i.e.: AuraCAD.MyAddon
    ...
    def process_metadata(self, _search_paths: SearchPaths) -> None:
        ...
        if not self.supports_AuraCAD_version():
            self.state = ModState.Unsupported
            Msg(f"NOTICE: {self.name} does not support this version of AuraCAD, so is being skipped")
...
class DirMod(Mod):
    ...
    def process_metadata(self, search_paths: SearchPaths):
        ...
        if not self.supports_AuraCAD_version():
            self.state = ModState.Unsupported
            Msg(f"NOTICE: {meta.Name} does not support this version of AuraCAD, so is being skipped")
            return
        ...
        if "workbench" in content:
            ...
            for workbench in workbenches:
                if not workbench.supportsCurrentAuraCAD():
                    Msg(f"NOTICE: {meta.Name} content item {workbench.Name} does not support this version of AuraCAD, so is being skipped")
                    continue
...
class ExtModScanner:
    ...
    def scan(self):
        import AuraCAD
        modules = (m[1] for m in pkgutil.iter_modules(AuraCAD.__path__, "AuraCAD.") if m[2])
...
class InitPipeline:
    ...
    def scan(self) -> None:
        ...
        legacy_user_mod = Path.home() / ".AuraCAD" / "Mod"
        ...
        # to have all the module-paths available in AuraCADGuiInit.py:
        App.__ModDirs__ = [str(d) for d in mods.dirs()]

        # this allows importing with:
        # from AuraCAD.Module import package
    ...
    def load_mods(self) -> None:
        ...
        # Save to use in AuraCADGuiInit.py
...
def init_applications() -> None:
    try:
        InitPipeline().run()
        Log('Init: App::AuraCADInit.py done')
    except Exception as ex:
