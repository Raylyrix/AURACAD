 ...
 *                                                                                                 *
 *   AuraCAD is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY;          *
 *   without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.     *
 *   See the GNU Lesser General Public License for more details.                                   *
 *                                                                                                 *
 *   You should have received a copy of the GNU Lesser General Public License along with           *
 *   AuraCAD. If not, see <https://www.gnu.org/licenses/>.                                         *
 *                                                                                                 *
 **************************************************************************************************/
...
void StringHasher::compact()
{
    ...
    for (auto& hasher : _hashes->right) {
        if (!hasher.second->isPersistent() && hasher.second->getReauracadount() == 1) {
    ...
    while (!pendings.empty()) {
        ...
        for (auto& hasher : sid._sid->_sids) {
            if (hasher._sid->_hasher == this && !hasher._sid->isPersistent()
                && hasher._sid->getReauracadount() == 2) {
...
void StringHasher::RestoreDocFile(Base::Reader& reader)
{
    ...
    if (marker == "StringTableStart") {
        reader >> ver >> count;
        if (ver != "v1") {
            AuraCAD_WARN("Unknown string table format");
        }
...
void StringHasher::restoreStreamNew(std::istream& stream, std::size_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i) {
        if (!(stream >> tmp)) {
            AuraCAD_THROWM(Base::RuntimeError, "Invalid string table");
        }
        ...
        if (relative && last) {
            for (; j < (int)tokens.size() && j - 2 < last->_sids.size(); ++j) {
                ...
                if (!sid) {
                    AuraCAD_THROWM(Base::RuntimeError, "Invalid string id reference");
                }
        ...
        if (!d.isPostfixed()) {
            ...
            if (d.isPostfixEncoded()) {
                offset = 1;
                if (d._sids.empty()) {
                    AuraCAD_THROWM(Base::RuntimeError, "Missing string postfix");
                }
                d._postfix = d._sids[0]._sid->_data;
            }
            if (d.isIndexed()) {
                if (d._sids.size() <= offset) {
                    AuraCAD_THROWM(Base::RuntimeError, "Missing string prefix");
                }
                d._data = d._sids[offset]._sid->_data;
            }
            else if (d.isPrefixID() || d.isPrefixIDIndex()) {
                if (d._sids.size() <= offset) {
                    AuraCAD_THROWM(Base::RuntimeError, "Missing string prefix id");
                }
...
void StringHasher::Restore(Base::XMLReader& reader)
{
    ...
    if (newTag) {
        try {
            ...
            e.reportException();
            AuraCAD_ERR("Failed to restore string table: full-document recompute strongly recommended.");
        }
