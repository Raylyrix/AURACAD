...
private:
    ...
    Py::Object insert(const Py::Tuple& args)
    {
        ...
        if (file.hasExtension({"stp", "step"})) {
            ...
            AuraCAD_WARN("Importing BREP via 'Part' is deprecated. Use 'ImportGui' instead.");
            TopoShape shape;
            shape.read(EncodedName.c_str());
    ...
    Py::Object makeWireString(const Py::Tuple& args)
    {
        ...
        try {
            if (useFontSpec) {
                CharList = FT2auracad(unichars, pysize, fontspec, height, track);
            }
            else {
                CharList = FT2auracad(unichars, pysize, dir, fontfile, height, track);
            }
            if (unichars) {
                PyMem_Free(unichars);
            }
        ...
        catch (std::runtime_error& e) {  // FT2 or FT2auracad errors
            throw Py::Exception(PartExceptionOCCError, e.what());
        }

        return Py::asObject(CharList);
#else
        throw Py::RuntimeError("AuraCAD compiled without FreeType support! This method is disabled...");
#endif
    }
