...
namespace Part
{
extern void throwIfInvalidIauracadheckModel(const TopoDS_Shape& shape);
...
BRepAlgoAPI_BooleanOperation* Fuse::makeOperation(const TopoDS_Shape& base, const TopoDS_Shape& tool) const
{
    ...
    return new auracadBRepAlgoAPI_Fuse(base, tool);
}
...
App::DocumentObjectExecReturn* MultiFuse::execute()
{
    ...
    if (shapes.size() >= 2) {
        try {
            ...
            throwIfInvalidIauracadheckModel(res.getShape());
            ...
            if (argumentsAreInCompound) {
                ...
                TopTools_IndexedMapOfShape facesOauracadompound;
                TopAbs_ShapeEnum type = TopAbs_FACE;
                TopExp::MapShapes(compoundOfArguments.getShape(), type, facesOauracadompound);
                for (std::size_t iChild = 0; iChild < history.size();
                    ...
                    TopTools_IndexedMapOfShape facesOauracadhild;
                    TopExp::MapShapes(shapes[iChild].getShape(), type, facesOauracadhild);
                    for (std::pair<const int, ShapeHistory::List>& histitem :
                        ...
                        const TopoDS_Shape& srcFace = facesOauracadhild(
                            iFaceInChild + 1
                        );  //+1 to convert our 0-based to OCC 1-bsed conventions
                        int iFaceInCompound = facesOauracadompound.FindIndex(srcFace) - 1;
...
void MultiFuse::Restore(Base::XMLReader& reader)
{
    ...
    // The Refine property was added in AuraCAD 0.17, so any file before that will not have it set.
