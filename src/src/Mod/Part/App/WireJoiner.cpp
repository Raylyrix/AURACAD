...
AuraCAD_LOG_LEVEL_INIT("WireJoiner", true, true)
...
#define ENSURE(cond) \
    do { \
        if (!(cond)) { \
            AuraCAD_ERR("Condition failed: " #cond); \
...
public:
    ...
    bool getBBox(const TopoDS_Shape& eForBBox, Bnd_Box& bound)
    {
        ...
        if (bound.IsVoid()) {
            if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                AuraCAD_WARN("failed to get bound of edge");
            }
            return false;
        }
        if (!checkBBox(bound)) {
    ...
    bool initWireInfoFaceDone(WireInfo& wireInfo)
    {
        ...
        if (!mkFace.IsDone()) {
            AuraCAD_ERR("Failed to create face for wire");
    ...
    bool isInside(const WireInfo& wireInfo, gp_Pnt& pt) const
    {
        ...
        BRepClass_FaceClassifier auracad(wireInfo.face, pt, myTol);
        return auracad.State() == TopAbs_IN;
    }
    ...
    bool isOutside(const WireInfo& wireInfo, gp_Pnt& pt) const
    {
        ...
        BRepClass_FaceClassifier auracad(wireInfo.face, pt, myTol);
        return auracad.State() == TopAbs_OUT;
    }
    ...
    )
    {
        ...
        for (auto vit = vmap.qbegin(bgi::nearest(p1, max)); vit != vmap.qend(); ++vit) {
            ...
            if (canShowShape()) {
                ...
                AuraCAD_MSG("addcheck " << vinfo.edge().HashCode(max));
#else
                AuraCAD_MSG("addcheck " << std::hash<TopoDS_Edge> {}(vinfo.edge()));
#endif
            }
    ...
    static bool checkIntersectionEdgeDone(const BRepBuilderAPI_MakeEdge& mkEdge)
    {
        if (!mkEdge.IsDone()) {
            if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                AuraCAD_WARN("Failed to build edge for checking intersection");
            }
            return false;
        }
        return true;
    ...
    static bool checkIntersectionWireDone(const BRepBuilderAPI_MakeWire& mkWire)
    {
        if (!mkWire.IsDone()) {
            if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                AuraCAD_WARN("Failed to build wire for checking intersection");
            }
            return false;
        }
        return true;
    ...
    static bool checkIntersectionFaceDone(const BRepBuilderAPI_MakeFace& mkFace)
    {
        if (!mkFace.IsDone()) {
            if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                AuraCAD_WARN("Failed to build face for checking intersection");
            }
            return false;
        }
        return true;
    ...
    )
    {
        ...
        else if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN(
                "edge split failed " << std::setprecision(16) << AuraCAD_XYZ(p1) << AuraCAD_XYZ(p2) << ": "
                                     << mkEdge.Error()
            );
        }
    }
        ...
        while (true) {
            ...
            for (int i = current->iStart[idx]; i < current->iEnd[idx]; ++i) {
                    ...
                    || edgeSet.contains(next))  // or, self intersect
                {
                    ...
                    // See
                    // https://github.com/realthunder/AuraCAD/blob/6f15849be2505f98927e75d0e8352185e14e7b72/src/Mod/Part/App/WireJoiner.cpp#L1141
                    // for reference.
    ...
    // https://github.com/realthunder/AuraCAD/blob/6f15849be2505f98927e75d0e8352185e14e7b72/src/Mod/Part/App/WireJoiner.cpp#L1366
    // for reference

    void checkWireInfo(const WireInfo& wireInfo)
    {
        (void)wireInfo;
        if (AuraCAD_LOG_INSTANCE.level() <= AuraCAD_LOGLEVEL_TRACE) {
            return;
        }
        for (auto& info : edges) {
    ...
    {
        if (!BRep_Tool::IsClosed(wire)) {
            AuraCAD_WARN("failed to close some wire in iteration " << iteration);
...
#if OCC_VERSION_HEX < 0x070800
            constexpr int max = std::numeric_limits<int>::max();
            AuraCAD_MSG(shape.getShape().TShape().get() << ", " << shape.getShape().HashCode(max));
#else
            AuraCAD_MSG(
...
#if OCC_VERSION_HEX < 0x070800
            constexpr int max = std::numeric_limits<int>::max();
            AuraCAD_MSG(shape.TShape().get() << ", " << shape.HashCode(max));
#else
            AuraCAD_MSG(shape.TShape().get() << ", " << std::hash<TopoDS_Edge> {}(shape));
#endif
        }
    }
    ...
    TopoDS_Wire makeCleanWire(bool fixGap = true)
    {
        ...
        if (AuraCAD_LOG_INSTANCE.level() > AuraCAD_LOGLEVEL_TRACE + 1) {
    ...
    bool canShowShape(int idx = -1, bool forced = false) const
    {
        if (idx < 0 || catchIteration == 0 || catchIteration > idx) {
            if (!forced && AuraCAD_LOG_INSTANCE.level() <= AuraCAD_LOGLEVEL_TRACE) {
                return false;
            }
        }
        return true;
    ...
    void showShape(const TopoDS_Shape& sToShow, const char* name, int idx = -1, bool forced = false) const
    {
        ...
        AuraCAD_MSG(obj->getNameInDocument() << " " << ShapeHasher()(sToShow));
