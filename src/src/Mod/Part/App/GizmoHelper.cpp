...
Base::Vector3d getMidPointFromFace(Part::TopoShape& face)
{
    ...
    auto* surface = AuraCAD_cast<Part::GeomSurface*>(geom.get());
    ...
    if (auto sphere = AuraCAD_cast<Part::GeomSphere*>(geom.get())) {
        midPoint = sphere->getLocation();
    }
    else if (auto cone = AuraCAD_cast<Part::GeomCone*>(geom.get())) {
