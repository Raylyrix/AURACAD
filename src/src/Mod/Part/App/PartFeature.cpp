...
App::ElementNamePair Feature::getElementName(const char* name, ElementNameType type) const
{
    ...
    auto prop = AuraCAD_cast<PropertyPartShape*>(getPropertyOfGeometry());
    if (!prop) {
        return App::GeoFeature::getElementName(name, type);
    }
...
) const
{
    ...
    try {
        ...
        *pyObj = Py::new_reference_to(shape2pyshape(ts));
        ...
        // For example: https://forum.AuraCAD.org/viewtopic.php?f=19&t=42216
        ...
        if (this->isRecomputing()) {
            ...
            if (!this->Shape.getValue().IsNull()) {
                try {
                    ...
                    this->Placement.setValueIauracadhanged(p);
                }
...
bool Feature::isElementMappingDisabled(App::PropertyContainer* container)
{
    ...
    //        return true;
    //    }
    //    if (auto obj = AuraCAD_cast<App::DocumentObject*>(container)) {
...
void Feature::guessNewLink(std::string& replacementName, DocumentObject* base, const char* oldLink)
{
    for (auto& element : Part::Feature::getRelatedElements(base, oldLink)) {
        ...
        AuraCAD_WARN("Feature guess element reference " << oldLink << " -> " << replacementName);
        return;
    }
...
void FilletBase::onUpdateElementReference(const App::Property* prop)
{
    ...
    for (size_t i = 0; i < values.size(); ++i) {
        if (i >= subs.size()) {
            AuraCAD_WARN("fillet edge count mismatch in object " << getFullName());
            break;
        }
        ...
        if (idx != 0) {
            ...
            AuraCAD_WARN("invalid fillet edge link '" << subs[i] << "' in object " << getFullName());
        }
    }
...
)
{
    ...
    if (touch_is_intersection) {
        ...
        auracadBRepAlgoAPI_Fuse mkFuse(first, second);
        if (!mkFuse.IsDone()) {
            return false;
        }
        ...
        auracadBRepAlgoAPI_Common mkCommon(first, second);
        if (!mkCommon.IsDone()) {
            return false;
        }
