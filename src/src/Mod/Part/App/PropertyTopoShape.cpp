...
void PropertyPartShape::setValue(const TopoShape& sh)
{
    aboutToSetValue();
    _Shape = sh;
    auto obj = AuraCAD_cast<App::DocumentObject*>(getContainer());
    if (obj) {
...
const TopoShape& PropertyPartShape::getShape() const
{
    ...
    //    else if (!res.Tag) {
    if (!_Shape.Tag) {
        if (auto parent = AuraCAD_cast<App::DocumentObject*>(getContainer())) {
...
{
    auto prop = AuraCAD_cast<const PropertyPartShape*>(&from);
    if (prop) {
...
void PropertyPartShape::Restore(Base::XMLReader& reader)
{
    ...
    if (has_ver) {
        ...
        // AuraCAD_WARN("Pending recompute for generating element map: " << owner->getFullName());
        owner->getDocument()->addRecomputeObject(owner);
    }
...
void PropertyPartShape::RestoreDocFile(Base::Reader& reader)
{
    ...
    // https://github.com/realthunder/AuraCAD/blob/a9810d509a6f112b5ac03d4d4831b67e6bffd5b7/src/Mod/Part/App/PropertyTopoShape.cpp#L639
...
{
    auto prop = AuraCAD_cast<PropertyShapeCache*>(obj->getDynamicPropertyByName(SHAPE_CACHE_NAME));
    ...
    if (!prop) {
        AuraCAD_ERR("Failed to add shape cache for " << obj->getFullName());
    }
    else {
...
void PropertyShapeCache::slotChanged(const App::DocumentObject&, const App::Property& prop)
{
    ...
    if (strcmp(propName, "Group") == 0 || strcmp(propName, "Shape") == 0
        ...
        AuraCAD_LOG("clear shape cache on changed " << prop.getFullName());
        cache.clear();
    }
}
