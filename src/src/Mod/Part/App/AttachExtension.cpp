...
{
    prop = AuraCAD_cast<T*>(owner->getDynamicPropertyByName(name));
    if (prop || !force) {
        return false;
    }
    prop = static_cast<T*>(owner->addDynamicProperty(type.getName(), name, "Attachment", doc));
    if (!prop) {
        AuraCAD_THROWM(Base::RuntimeError, "Failed to add property " << owner->getFullName() << name);
    }
...
void AttachExtension::handleLegacyTangentPlaneOrientation()
{
    ...
    for (auto obj : this->AttachmentSupport.linkedObjects()) {
        if (obj->isDerivedFrom<App::Plane>()) {
            geof = AuraCAD_cast<App::GeoFeature*>(obj);
            break;
        }
    }
    if (!geof) {
...
{
    auto pla = AuraCAD_cast<App::PropertyPlacement*>(
