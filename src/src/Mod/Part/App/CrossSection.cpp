...
{
    auracadBRepAlgoAPI_Section cs(shape, gp_Pln(a, b, c, -d));
    if (cs.IsDone()) {
...
void CrossSection::sliceSolid(double d, const TopoDS_Shape& shape, std::list<TopoDS_Wire>& wires) const
{
    ...
    auracadBRepAlgoAPI_Cut mkCut(shape, solid);
...
{
    auracadBRepAlgoAPI_Section cs(shape.getShape(), gp_Pln(a, b, c, -d));
    if (cs.IsDone()) {
...
) const
{
    ...
    auracadBRepAlgoAPI_Cut mkCut(shape.getShape(), solid.getShape());
