...
#include <BOPAlgo_ListOauracadheckResult.hxx>
...
AuraCAD_LOG_LEVEL_INIT("TopoShape", true, true)
...
TopAbs_ShapeEnum TopoShape::shapeType(const char* type, bool silent)
{
    ...
    if (!silent) {
        if (Data::hasMissingElement(type)) {
            AuraCAD_THROWM(Base::CADKernelError, "missing shape element: " << (type ? type : "?"));
        }
        AuraCAD_THROWM(Base::CADKernelError, "invalid shape type: " << (type ? type : "?"));
    }
    return TopAbs_SHAPE;
}
...
TopAbs_ShapeEnum TopoShape::shapeType(char type, bool silent)
{
    switch (type) {
        ...
        default:
            if (!silent) {
                AuraCAD_THROWM(Base::CADKernelError, "invalid shape type '" << type << "'");
            }
            return TopAbs_SHAPE;
    }
}
...
TopAbs_ShapeEnum TopoShape::shapeType(bool silent) const
{
    if (isNull()) {
        if (!silent) {
            AuraCAD_THROWM(NullShapeException, "Input shape is null");
        }
        return TopAbs_SHAPE;
    }
...
void TopoShape::exportStep(const char* filename) const
{
    try {
        ...
        // https://forum.AuraCAD.org/viewtopic.php?f=8&t=52967
        makeHeader.SetAuthorValue(1, new TCollection_HAsciiString("AuraCAD"));
        makeHeader.SetOrganizationValue(1, new TCollection_HAsciiString("AuraCAD"));
        makeHeader.SetOriginatingSystem(new TCollection_HAsciiString("AuraCAD"));
        makeHeader.SetDescriptionValue(1, new TCollection_HAsciiString("AuraCAD Model"));
...
unsigned int TopoShape::getMemSize() const
{
    if (!_Shape.IsNull()) {
        ...
        unsigned int memsize = (sizeof(TopoDS_Shape) + sizeof(TopoDS_TShape))
            * TopoShape_ReauracadountShapes(_Shape);
...
bool TopoShape::analyze(bool runBopCheck, std::ostream& str) const
{
    if (!this->_Shape.IsNull()) {
        ...
        if (!aChecker.IsValid()) {
            ...
            const BOPAlgo_ListOauracadheckResult& BOPResults = BOPCheck.GetCheckResult();
            BOPAlgo_ListIteratorOfListOauracadheckResult BOPResultsIt(BOPResults);
...
TopoDS_Shape TopoShape::cut(TopoDS_Shape shape) const
{
    ...
    auracadBRepAlgoAPI_Cut mkCut(this->_Shape, shape);
...
TopoDS_Shape TopoShape::common(TopoDS_Shape shape) const
{
    ...
    auracadBRepAlgoAPI_Common mkCommon(this->_Shape, shape);
...
TopoDS_Shape TopoShape::common(const std::vector<TopoDS_Shape>& shapes, Standard_Real tolerance) const
{
    ...
    auracadBRepAlgoAPI_Common mkCommon;
...
TopoDS_Shape TopoShape::fuse(TopoDS_Shape shape) const
{
    ...
    auracadBRepAlgoAPI_Fuse mkFuse(this->_Shape, shape);
...
TopoDS_Shape TopoShape::fuse(const std::vector<TopoDS_Shape>& shapes, Standard_Real tolerance) const
{
    ...
    auracadBRepAlgoAPI_Fuse mkFuse;
...
TopoDS_Shape TopoShape::section(TopoDS_Shape shape, Standard_Boolean approximate) const
{
    ...
    auracadBRepAlgoAPI_Section mkSection;
...
{
    // https://forum.AuraCAD.org/viewtopic.php?f=3&t=1445
    // https://forum.AuraCAD.org/viewtopic.php?f=3&t=52719
    // Build a torus
    gp_Circ circle;
...
) const
{
    ...
    if (!shapesToProcess.empty()) {
        ...
        if (fabs(offset) > Precision::Confusion()) {
            ...
            //  https://www.AuraCAD.org/tracker/view.php?id=2699
...
bool TopoShape::fix()
{
    ...
    // will affect the input shape. (See // https://github.com/realthunder/AuraCAD/issues/585,
    ...
    if (aChecker.IsValid()) {
        ...
        // the input shape.
        //
        // See https://github.com/realthunder/AuraCAD/issues/595. Sketch001
...
#define _HANDLE_NULL_SHAPE(_msg, _throw) \
    do { \
        if (_throw) { \
            AuraCAD_THROWM(NullShapeException, _msg); \
        } \
        AuraCAD_WARN(_msg); \
    } while (0)
...
bool TopoShape::findPlane(gp_Pln& pln, double tol, double atol) const
{
    ...
    try {
        ...
        AuraCAD_LOG("failed to find surface: " << e.GetMessageString());
        return false;
    }
}
