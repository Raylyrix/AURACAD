...
#include <GC_MakeArcOauracadircle.hxx>
...
bool Geometry::hasSameExtensions(const Geometry& other) const
{
    ...
    for (const auto& ext : extensions) {
        if (auto persistExt = AuraCAD_cast<const GeometryPersistenceExtension*>(ext.get())) {
            for (; index < other.extensions.size(); ++index) {
                if (auto extOther = AuraCAD_cast<const GeometryPersistenceExtension*>(
    ...
    for (; index < other.extensions.size(); ++index) {
        if (AuraCAD_cast<const GeometryPersistenceExtension*>(other.extensions[index].get())) {
            return false;
        }
    }
    return true;
...
{
    // https://forum.AuraCAD.org/viewtopic.php?f=10&t=31700
...
{
    return new BezierCurvePy(AuraCAD_cast<GeomBezierCurve*>(this->clone()));
}
...
{
    return new BSplineCurvePy(AuraCAD_cast<GeomBSplineCurve*>(this->clone()));
}
...
// -------------------------------------------------
TYPESYSTEM_SOURCE_ABSTRACT(Part::GeomArcOauracadonic, Part::GeomTrimmedCurve)

GeomArcOauracadonic::GeomArcOauracadonic() = default;

GeomArcOauracadonic::~GeomArcOauracadonic() = default;

/*!
 * \brief GeomArcOauracadonic::getStartPoint
 ...
 */
Base::Vector3d GeomArcOauracadonic::getStartPoint(bool emulateCCWXY) const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    gp_Pnt pnt = curve->StartPoint();
    if (emulateCCWXY) {
...
/*!
 * \brief GeomArcOauracadonic::getEndPoint
 * \param emulateCCWXY: if true, the arc will pretent to be a CCW arc in XY plane.
 * For this to work, the arc must lie in XY plane (i.e. Axis is either +Z or -Z).
 * \return
 */
Base::Vector3d GeomArcOauracadonic::getEndPoint(bool emulateCCWXY) const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    gp_Pnt pnt = curve->EndPoint();
    if (emulateCCWXY) {
...
Base::Vector3d GeomArcOauracadonic::getCenter() const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
...
Base::Vector3d GeomArcOauracadonic::getLocation() const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    Handle(Geom_Conic) conic = Handle(Geom_Conic)::DownCast(curve->BasisCurve());
    gp_Ax1 axis = conic->Axis();
...
void GeomArcOauracadonic::setCenter(const Base::Vector3d& Center)
{
    gp_Pnt p1(Center.x, Center.y, Center.z);
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    Handle(Geom_Conic) conic = Handle(Geom_Conic)::DownCast(curve->BasisCurve());
...
void GeomArcOauracadonic::setLocation(const Base::Vector3d& Center)
{
    gp_Pnt p1(Center.x, Center.y, Center.z);
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    Handle(Geom_Conic) conic = Handle(Geom_Conic)::DownCast(curve->BasisCurve());
...
Base::Vector3d GeomArcOauracadonic::getAxisDirection() const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    Handle(Geom_Conic) conic = Handle(Geom_Conic)::DownCast(curve->BasisCurve());
    gp_Ax1 axis = conic->Axis();
...
/*!
 * \brief GeomArcOauracadonic::isReversed
 ...
 */
bool GeomArcOauracadonic::isReversed() const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    Handle(Geom_Conic) conic = Handle(Geom_Conic)::DownCast(curve->BasisCurve());
    assert(!conic.IsNull());
...
bool GeomArcOauracadonic::reverseIfReversed()
{
...
/*!
 * \brief GeomArcOauracadonic::getAngleXU
 * \return The angle between ellipse's major axis (in direction to focus1) and
 * X axis of a default axis system in the plane of ellipse. The angle is
 * counted CCW as seen when looking at the ellipse so that ellipse's axis is
 * pointing at you. Note that this function may give unexpected results when
 ...
 * with ellipse's axis, fully defines the orientation of the ellipse.
 */
double GeomArcOauracadonic::getAngleXU() const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    Handle(Geom_Conic) conic = Handle(Geom_Conic)::DownCast(curve->BasisCurve());
...
/*!
 * \brief GeomArcOauracadonic::setAngleXU complements getAngleXU.
 */
void GeomArcOauracadonic::setAngleXU(double angle)
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    Handle(Geom_Conic) conic = Handle(Geom_Conic)::DownCast(curve->BasisCurve());
...
/*!
 * \brief GeomArcOauracadonic::getXAxisDir
 ...
 */
Base::Vector3d GeomArcOauracadonic::getXAxisDir() const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
...
/*!
 * \brief GeomArcOauracadonic::setXAxisDir Rotates the conic in its plane, so
 ...
 * thrown.
 */
void GeomArcOauracadonic::setXAxisDir(const Base::Vector3d& newdir)
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    Handle(Geom_Conic) c = Handle(Geom_Conic)::DownCast(curve->BasisCurve());
    assert(!c.IsNull());
...
{
    auto newArc = new GeomArcOauracadircle(Handle(Geom_Circle)::DownCast(this->handle()->Copy()));
...
TYPESYSTEM_SOURCE(Part::GeomArcOauracadircle, Part::GeomArcOauracadonic)

GeomArcOauracadircle::GeomArcOauracadircle()
{
    Handle(Geom_Circle) c = new Geom_Circle(gp_Circ());
    this->myCurve = new Geom_TrimmedCurve(c, c->FirstParameter(), c->LastParameter());
}

GeomArcOauracadircle::GeomArcOauracadircle(const Handle(Geom_Circle) & c)
{
    setHandle(c);
}

GeomArcOauracadircle::~GeomArcOauracadircle() = default;

void GeomArcOauracadircle::setHandle(const Handle(Geom_TrimmedCurve) & c)
{
...
void GeomArcOauracadircle::setHandle(const Handle(Geom_Circle) & c)
{
    this->myCurve = new Geom_TrimmedCurve(c, c->FirstParameter(), c->LastParameter());
}


const Handle(Geom_Geometry) & GeomArcOauracadircle::handle() const
{
    return myCurve;
}

Geometry* GeomArcOauracadircle::copy() const
{
    GeomArcOauracadircle* copy = new GeomArcOauracadircle();
    copy->setHandle(this->myCurve);
    copy->copyNonTag(this);
    return copy;
}

GeomBSplineCurve* GeomArcOauracadircle::toNurbs(double first, double last) const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
...
double GeomArcOauracadircle::getRadius() const
{
    Handle(Geom_Circle) circle = Handle(Geom_Circle)::DownCast(myCurve->BasisCurve());
    return circle->Radius();
}

void GeomArcOauracadircle::setRadius(double Radius)
{
    Handle(Geom_Circle) circle = Handle(Geom_Circle)::DownCast(myCurve->BasisCurve());
...
/*!
 * \brief GeomArcOauracadircle::getAngle
 * \param emulateCCWXY: if true, the arc will pretend to be a CCW arc in XY plane.
 * For this to work, the arc must lie in XY plane (i.e. Axis is either +Z or -Z).
 */
double GeomArcOauracadircle::getAngle(bool emulateCCWXY) const
{
...
/*!
 * \brief GeomArcOauracadircle::getRange
 ...
 * (ArcOauracadircle specific).
 */
void GeomArcOauracadircle::getRange(double& u, double& v, bool emulateCCWXY) const
{
    Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
    u = curve->FirstParameter();
    v = curve->LastParameter();
...
/*!
 * \brief GeomArcOauracadircle::setRange
 ...
 * (ArcOauracadircle specific).
 */
void GeomArcOauracadircle::setRange(double u, double v, bool emulateCCWXY)
{
    try {
        Handle(Geom_TrimmedCurve) curve = Handle(Geom_TrimmedCurve)::DownCast(handle());
        if (emulateCCWXY) {
...
// Persistence implementer
unsigned int GeomArcOauracadircle::getMemSize() const
{
    return sizeof(Geom_Circle) + 2 * sizeof(double);
}

void GeomArcOauracadircle::Save(Base::Writer& writer) const
{
    // save the attributes of the father class
    Geometry::Save(writer);
    ...
    writer.Stream() << writer.ind() << "<ArcOauracadircle "
                    << "CenterX=\"" << center.X() << "\" CenterY=\"" << center.Y() << "\" CenterZ=\""
                    << center.Z() << "\" NormalX=\"" << normal.X() << "\" NormalY=\"" << normal.Y()
                    << "\" NormalZ=\"" << normal.Z() << "\" AngleXU=\"" << AngleXU << "\" Radius=\""
...
void GeomArcOauracadircle::Restore(Base::XMLReader& reader)
{
    // read the attributes of the father class
    Geometry::Restore(reader);
    ...
    double AngleXU = 0;
    // read my Element
    reader.readElement("ArcOauracadircle");
    // get the value of my Attribute
    CenterX = reader.getAttribute<double>("CenterX");
    CenterY = reader.getAttribute<double>("CenterY");
    CenterZ = reader.getAttribute<double>("CenterZ");
    ...
    try {
        ...
        GC_MakeArcOauracadircle ma(mc.Value()->Circ(), StartAngle, EndAngle, Standard_True);
        if (!ma.IsDone()) {
...
PyObject* GeomArcOauracadircle::getPyObject()
{
    return new ArcOauracadirclePy(static_cast<GeomArcOauracadircle*>(this->clone()));
}
...
TYPESYSTEM_SOURCE(Part::GeomArcOfEllipse, Part::GeomArcOauracadonic)
...
TYPESYSTEM_SOURCE(Part::GeomArcOfHyperbola, Part::GeomArcOauracadonic)
...
TYPESYSTEM_SOURCE(Part::GeomArcOfParabola, Part::GeomArcOauracadonic)
...
{
    return new LineSegmentPy(AuraCAD_cast<GeomLineSegment*>(this->clone()));
}
...
{
    return new OffsetCurvePy(AuraCAD_cast<GeomOffsetCurve*>(this->clone()));
}
...
gp_Vec GeomCone::getDN(double u, double v, int Nu, int Nv) const
{
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=10&t=66677
...
GeomArcOauracadircle* create2LinesFilletGeometry(
    const GeomLineSegment* lineSeg1,
    const GeomLineSegment* lineSeg2,
...
)
{
    ...
    // Create Arc Segment
    GeomArcOauracadircle* arc = new GeomArcOauracadircle();
...
GeomArcOauracadircle* createFilletGeometry(
    const Geometry* geo1,
    const Geometry* geo2,
    const Base::Vector3d& refPnt1,
    const Base::Vector3d& refPnt2,
...
)
{
    if (geo1->is<GeomLineSegment>() && geo2->is<GeomLineSegment>()) {
        ...
        if (radius == .0f) {
            // guess a radius
            // https://forum.AuraCAD.org/viewtopic.php?f=3&t=31594&start=50#p266658
            //
        ...
        // Create Arc Segment
        auto* arc = new GeomArcOauracadircle();
...
std::unique_ptr<GeomCurve> makeFromTrimmedCurve(const Handle(Geom_Curve) & c, double f, double l, bool silent)
{
    ...
    if (c->IsKind(STANDARD_TYPE(Geom_Circle))) {
        Handle(Geom_Circle) circ = Handle(Geom_Circle)::DownCast(c);
        std::unique_ptr<GeomCurve> arc(new GeomArcOauracadircle());
...
// ===== TEXT TO EDGES (adapted from FT2auracad.cpp) =====
namespace
{
...
// The core logic, refactored from FT2auracad to be Python-independent
...
)
{
    ...
    if (fontFile.empty()) {
#if defined(AuraCAD_OS_LINUX)
        fontFile = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf";
#elif defined(AuraCAD_OS_WIN32)
        fontFile = "C:/Windows/Fonts/Arial.ttf";
#elif defined(AuraCAD_OS_MAC)
