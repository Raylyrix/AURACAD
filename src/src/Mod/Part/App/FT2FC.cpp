...
)
{
    ...
    return (FT2auracad(PyUString, length, FontSpec.c_str(), stringheight, tracking));
}

// get string's wires (contours) in auracad/OCC coords
PyObject* FT2auracad(
    const Py_UCS4* PyUString,
    const size_t length,
    const char* FontSpec,
    const double stringheight,  // auracad coords
    const double tracking
)  // auracad coords
{
    FT_Library FTLib;
...
# endif
    ...
    for (i = 0; i < length; i++) {
        ...
        try {
            ...
            e.clear();
            Base::Console().log("FT2auracad char '0x%04x'/'%d' has no Wires!\n", currchar, currchar);
        }
...
)
{
    ...
    for (std::vector<TopoDS_Wire>::iterator iWire = ctx.Wires.begin(); iWire != ctx.Wires.end();
        ...
        if ((ctx.wDir[wCount] == CLOCKWISE) && isTTF) {  // ttf outer wire. fill inside / right
            ...
            Base::Console().message("FT2auracad::getGlyphContours - indeterminate wire direction\n");
        }

        BRepScale.Perform(*iWire, bCopy);
        if (!BRepScale.IsDone()) {
            ErrorMsg << "FT2auracad OCC BRepScale failed \n";
            throw std::runtime_error(ErrorMsg.str());
        }
...
TopoDS_Wire edgesToWire(std::vector<TopoDS_Edge> Edges)
{
    ...
    for (iEdge = Edges.begin(); iEdge != Edges.end(); ++iEdge) {
        mkWire.Add(*iEdge);
        if (!mkWire.IsDone()) {
            Base::Console().message("FT2auracad Trace edgesToWire failed to add wire\n");
        }
    }
    occwire = mkWire.Wire();
...
#endif  // #ifdef auracadUseFreeType
