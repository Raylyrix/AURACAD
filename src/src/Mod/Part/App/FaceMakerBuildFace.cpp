...
/****************************************************************************
 *                                                                          *
 *   Copyright (c) 2026 AuraCAD contributors                               *
 *                                                                          *
...
AuraCAD_LOG_LEVEL_INIT("FaceMakerBuildFace", true, true)
...
// Records the mapping original â†’ fragments in myPreSplitHistory so that
    ...
    for (TopTools_ListIteratorOfListOfShape it(edges); it.More(); it.Next()) {
        ...
        try {
            ...
            if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                AuraCAD_WARN("splitSelfIntersecting: " << e.GetMessageString());
            }
            result.Append(edge);
        }
        catch (...) {
            AuraCAD_WARN("splitSelfIntersecting: unknown exception");
            result.Append(edge);
        }
    }
    if (!myPreSplitHistory.IsNull()) {
...
TopTools_ListOfShape Part::FaceMakerBuildFace::splitAtIntersections(const TopTools_ListOfShape& edges)
{
    ...
    if (!mySplitter.IsDone()) {
        AuraCAD_WARN("FaceMakerBuildFace: failed to split edges at intersections");
        return edges;
    }
    TopTools_ListOfShape result;
...
void Part::FaceMakerBuildFace::Build_Essence()
{
    ...
    if (!findPlane(edges, plane)) {
        AuraCAD_LOG("FaceMakerBuildFace: cannot determine plane from edges");
        return;
    }
    ...
    if (faceBuilder.HasErrors()) {
        AuraCAD_WARN("FaceMakerBuildFace: BOPAlgo_BuilderFace failed");
        return;
    }
