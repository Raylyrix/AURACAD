...
PyObject* BRepOffsetAPI_MakePipeShellPy::setAuxiliarySpine(PyObject* args)
{
    ...
    try {
        ...
        BRepFill_TypeOauracadontact typeOauracadantact;
        switch (PyLong_AsLong(keep)) {
            case 1:
                typeOauracadantact = BRepFill_Contact;
                break;
            case 2:
                typeOauracadantact = BRepFill_ContactOnBorder;
                break;
            default:
                typeOauracadantact = BRepFill_NoContact;
                break;
        }
        this->getBRepOffsetAPI_MakePipeShellPtr()
            ->SetMode(TopoDS::Wire(s), Base::asBoolean(curv), typeOauracadantact);
        Py_Return;
    }
    catch (Standard_Failure& e) {
        PyErr_SetString(PartExceptionOCCError, e.GetMessageString());
