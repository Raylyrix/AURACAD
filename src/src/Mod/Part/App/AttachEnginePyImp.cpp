...
int AttachEnginePy::PyInit(PyObject* args, PyObject* /*kwd*/)
{
    ...
    if (PyArg_ParseTuple(args, "s", &typeName)) {
        ...
        if (!pNewAttacher) {
            ...
            PyErr_SetString(Base::PyExc_AuraCAD_GeneralError, errMsg.str().c_str());
            return -1;
        }
        AttachEngine* oldAttacher = this->getAttachEnginePtr();
...
PyObject* AttachEnginePy::getModeInfo(PyObject* args)
{
    ...
    try {
        ...
        Py::List pyListOauracadombinations;
        Py::List pyCombination;
        refTypeStringList& listOauracadombinations = attacher.modeRefTypes.at(mmode);
        for (const refTypeString& combination : listOauracadombinations) {
            ...
            pyListOauracadombinations.append(pyCombination);
        }
        Py::Dict ret;
        ret["ReferenceCombinations"] = pyListOauracadombinations;
    ...
    try {
        ...
        {  // sugr.reachableModes
            ...
            for (std::pair<const eMapMode, refTypeStringList>& rm : sugr.reachableModes) {
                ...
                pyRM[AttachEngine::getModeName(rm.first)] = pyListOauracadombinations;
            }
