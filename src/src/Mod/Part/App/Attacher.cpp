...
)
{
    ...
    // 4) Build Y as Z Ã— X (so it is perpendicular)
...
std::vector<App::DocumentObject*> AttachEngine::getRefObjects() const
{
    ...
    if (!doc) {
        AuraCAD_THROWM(AttachEngineException, "AttachEngine: document '" << docName << "' not found");
    }
    ...
    switch (mmode) {
        ...
        case mmMidpoint: {
            ...
            switch (shape->shapeType()) {
                case TopAbs_VERTEX: {
                    if (auto point = AuraCAD_cast<GeomPoint*>(geom.get())) {
                        placement.setPosition(point->getPoint());
                    }
                } break;

                case TopAbs_EDGE: {
                    if (auto conic = AuraCAD_cast<GeomConic*>(geom.get())) {
                        placement.setPosition(conic->getLocation());
                        placement.setRotation(conic->getRotation().value_or(Base::Rotation {}));
                    }
                    else if (auto line = AuraCAD_cast<GeomCurve*>(geom.get())) {
                ...
                case TopAbs_FACE: {
                    auto surface = AuraCAD_cast<GeomSurface*>(geom.get());
                    ...
                    if (auto sphere = AuraCAD_cast<GeomSphere*>(geom.get())) {
                        placement.setPosition(sphere->getLocation());
                    }
                    else if (auto cone = AuraCAD_cast<GeomCone*>(geom.get())) {
...
double AttachEngine3D::calculateFoldAngle(gp_Vec axA, gp_Vec axB, gp_Vec edA, gp_Vec edB) const
{
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=8&t=10511&sid=007946a934530ff2a6c9259fb32624ec&start=40#p87584
    axA.Normalize();
    axB.Normalize();
    edA.Normalize();
    edB.Normalize();
...
AttachEnginePoint::AttachEnginePoint()
{
    ...
    modeRefTypes[mm0CenterOauracadurvature] = attacher3D.modeRefTypes[mmRevolutionSection];
...
) const
{
    ...
    switch (mmode) {
        ...
        case mm0Origin:
            mmode = mmObjectXY;
            break;
        case mm0CenterOauracadurvature:
            mmode = mmRevolutionSection;
            break;
        case mm0OnEdge:
