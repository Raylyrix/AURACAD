...
#include <Mod/Part/App/auracadBRepAlgoAPI_Common.h>
#include <Mod/Part/App/auracadBRepAlgoAPI_Cut.h>
#include <Mod/Part/App/auracadBRepAlgoAPI_Fuse.h>
#include <Mod/Part/App/auracadBRepAlgoAPI_Section.h>
...
AuraCAD_LOG_LEVEL_INIT("TopoShape", true, true)  // NOLINT
...
{
    if (shape.isNull()) {
        AuraCAD_THROWM(NullShapeException, "Null input shape");
    }
...
//     AuraCAD_THROWM(NullShapeException, "Null shape");
//     AuraCAD_THROWM(NullShapeException, "Null input shape");
//     AuraCAD_WARN("Null input shape");  // NOLINT
//
// The original macros:
...
)
{
    ...
    if (count != other) {
        AuraCAD_WARN("sub shape mismatch");  // NOLINT
        if (count > other) {
            count = other;
        }
    }
        ...
        for (auto& mappedName :
            ...
            if (!sids.empty()) {
                if (!Hasher) {
                    ...
                    if (!warned) {
                        warned = true;
                        AuraCAD_WARN("hasher mismatch");  // NOLINT
                    }
                    sids.clear();
                }
            }
        ...
        if (Hasher) {
            if (other.Hasher != Hasher) {
                if (!getElementMapSize(false)) {
                    if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                        AuraCAD_WARN("hasher mismatch");
                    }
                }
                else {
                    // AuraCAD_THROWM(Base::RuntimeError, "hasher mismatch");
                    AuraCAD_ERR("hasher mismatch");
                }
                Hasher = other.Hasher;
            }
        }
...
)
{
    ...
    if (canMap != shapes.size() && AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
        AuraCAD_WARN("Not all input shapes are mappable");  // NOLINT
    }
            ...
            if (names.size() > 1) {
                ...
                for (++it; it != names.end(); ++it) {
                    ...
                    if ((name_type == 1 && other_info.index < 0)
                        ...
                        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                            AuraCAD_WARN("element is both generated and modified");  // NOLINT
                        }
                        name_type = 0;
                    }
            ...
            for (int i = 1; i <= info.count(); ++i) {
                ...
                for (; xp.More(); xp.Next()) {
                    ...
                    if (!name) {
                        ...
                        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
                            AuraCAD_WARN("unnamed lower element " << prevElement);  // NOLINT
                        }
                        names.clear();
                        break;
                    }
...
TopoShape TopoShape::getSubTopoShape(const char* Type, bool silent) const
{
    ...
    if (!mapped.index && boost::starts_with(Type, elementMapPrefix())) {
        if (!silent) {
            AuraCAD_THROWM(Base::CADKernelError, "Mapped element not found: " << Type);
        }
        return TopoShape();
    }
    ...
    if (res.second <= 0) {
        if (!silent) {
            AuraCAD_THROWM(Base::ValueError, "Invalid shape name " << (Type ? Type : ""));
        }
        return TopoShape();
    }
...
TopoShape TopoShape::getSubTopoShape(TopAbs_ShapeEnum type, int idx, bool silent) const
{
    ...
    if (idx <= 0) {
        if (!silent) {
            AuraCAD_THROWM(Base::ValueError, "Invalid shape index " << idx);
        }
        return TopoShape();
    }
    if (type < 0 || type > TopAbs_SHAPE) {
        if (!silent) {
            AuraCAD_THROWM(Base::ValueError, "Invalid shape type " << type);
        }
        return TopoShape();
    }
    initCache();
    ...
    if (idx > shapeMap.count()) {
        if (!silent) {
            AuraCAD_THROWM(Base::IndexError, "Shape index " << idx << " out of bound " << shapeMap.count());
        }
        return TopoShape();
    }
...
)
{
    ...
    if (spineShape.IsNull() || !BRepBuilderAPI_FindPlane(spineShape).Found()) {
        AuraCAD_THROWM(Base::CADKernelError, "Expect the spine to be a planar wire or face");
    }
    ...
    if (profileShape.IsNull() || !BRepBuilderAPI_FindPlane(profileShape).Found()) {
        if (profileShape.IsNull() || profile.countSubShapes(TopAbs_EDGE) > 1
            ...
            AuraCAD_THROWM(Base::CADKernelError, "Expect the profile to be a planar wire or a face or a line");
        }
    }
...
)
{
    ...
    if (shapes.size() != 2) {
        AuraCAD_THROWM(Base::CADKernelError, "Wrong number of input shapes");
    }
    ...
    for (auto& s : shapes) {
        ...
        if (countOfEdges == 0) {
            AuraCAD_THROWM(Base::CADKernelError, "Input shape has no edge");
        }
        if (countOfEdges == 1) {
        ...
        if (curves[i].shapeType() != TopAbs_WIRE) {
            AuraCAD_THROWM(Base::CADKernelError, "Input shape forms more than one wire");
        }
        ++i;
    }
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=8&t=24052
    //
...
static std::vector<TopoShape> prepareProfiles(const std::vector<TopoShape>& shapes, size_t offset = 0)
{
    ...
    for (size_t i = offset; i < shapes.size(); ++i) {
        ...
        AuraCAD_THROWM(Base::CADKernelError, "Profile shape is not a single vertex, edge, wire nor face.");
    }
    if (ret.empty()) {
        AuraCAD_THROWM(Base::CADKernelError, "No profile");
    }
    return ret;
}
...
)
{
    ...
    if (shapes.size() < 2) {
        AuraCAD_THROWM(Base::CADKernelError, "Not enough input shapes");
    }
    ...
    if (spine.getShape().ShapeType() != TopAbs_WIRE) {
        AuraCAD_THROWM(Base::CADKernelError, "Spine shape cannot form a single wire");
    }
    ...
    if (!mkPipeShell.IsReady()) {
        AuraCAD_THROWM(Base::CADKernelError, "shape is not ready to build");
    }
    else {
#if OCC_VERSION_HEX >= 0x070600
...
)
{
    ...
    if (!mkOffset.IsDone()) {
        AuraCAD_THROWM(Base::CADKernelError, "BRepOffsetAPI_MakeOffsetShape not done");
    }
    ...
    if (shape.hasSubShape(TopAbs_SOLID) && !res.hasSubShape(TopAbs_SOLID)) {
        try {
            ...
            AuraCAD_WARN("failed to make solid: " << e.GetMessageString());
        }
    }
    ...
    if (freeCheck.NbClosedFreeBounds() < 1) {
        AuraCAD_THROWM(Base::CADKernelError, "no closed bounds");
    }
    ...
    for (int index = 1; index <= freeCheck.NbClosedFreeBounds(); ++index) {
        ...
        for (const auto& s : originalWire.getSubShapes(TopAbs_EDGE)) {
            if (!img.HasImage(s)) {
                AuraCAD_THROWM(Base::CADKernelError, "no image for shape");
            }
            ...
            if (edgeCount != 1) {
                ...
                AuraCAD_THROWM(Base::CADKernelError, stream.str().c_str());
            }
        ...
        if (!aGenerator.IsDone()) {
            AuraCAD_THROWM(Base::CADKernelError, "ThruSections failed");
        }
...
)
{
    ...
    if (shape.isNull()) {
        AuraCAD_THROWM(Base::ValueError, "makeOffsetFace: input shape is null!");
    }
    if (!shape.hasSubShape(TopAbs_FACE)) {
        AuraCAD_THROWM(Base::ValueError, "makeOffsetFace: no face found");
    }
    ...
    for (auto& face : shape.getSubTopoShapes(TopAbs_FACE)) {
        ...
        if (outerWire.isNull()) {
            AuraCAD_THROWM(Base::CADKernelError, "makeOffsetFace: missing outer wire!");
        }
...
)
{
    ...
    if (shape.isNull()) {
        AuraCAD_THROWM(Base::ValueError, "makeOffset2D: input shape is null!");
    }
    ...
    if (shapesToProcess.size() > 0) {
        ...
        if (!TopoShape()
            ...
            AuraCAD_THROWM(Base::CADKernelError, "makeOffset2D: wires are nonplanar or noncoplanar");
        }
        ...
        if (fabs(offset) > Precision::Confusion()) {
            ...
            try {
#if defined(__GNUC__) && defined(AuraCAD_OS_LINUX)
            ...
            if (mkOffset.Shape().IsNull()) {
                AuraCAD_THROWM(NullShapeException, "makeOffset2D: result of offsetting is null!");
            }

            // Copying shape to fix strange orientation behavior, OCC7.0.0. See bug #2699
            //  http://www.AuraCAD.org/tracker/view.php?id=2699
        ...
        if (offsetWires.empty()) {
            AuraCAD_THROWM(Base::CADKernelError, "makeOffset2D: offset result has no wires.");
        }
        ...
        if (fill == FillType::noFill) {
            ...
            // fill offset
            if (fabs(offset) < Precision::Confusion()) {
                AuraCAD_THROWM(Base::ValueError, "makeOffset2D: offset distance is zero. Can't fill offset.");
            }
            ...
            if (allowOpenResult == OpenResult::noOpenResult || openWires.size() == 0) {
                ...
                // check
                if (v1.IsNull()) {
                    AuraCAD_THROWM(NullShapeException, "v1 is null");
                }
                if (v2.IsNull()) {
                    AuraCAD_THROWM(NullShapeException, "v2 is null");
                }
                if (v3.IsNull()) {
                    AuraCAD_THROWM(NullShapeException, "v3 is null");
                }
                if (v4.IsNull()) {
                    AuraCAD_THROWM(NullShapeException, "v4 is null");
                }
...
)
{
    ...
    for (auto& face : faces) {
        ...
        if (!shape.findShape(face.getShape())) {
            AuraCAD_THROWM(Base::CADKernelError, "face does not belong to the shape");
        }
...
const std::vector<TopoDS_Shape>& MapperSewing::modified(const TopoDS_Shape& s) const
{
    ...
    try {
        ...
        if (AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            AuraCAD_WARN("Exception on shape mapper: " << e.GetMessageString());
        }
    }
    return _res;
}
...
)
{
    ...
    if (bound.isNull()) {
        AuraCAD_THROWM(Base::CADKernelError, "No boundary wire");
    }
...
#endif
    if (!maker.IsDone()) {
        AuraCAD_THROWM(Base::CADKernelError, "Failed to created face by filling edges");
    }
...
TopoShape& TopoShape::makeElementSolid(const TopoShape& shape, const char* op)
{
    ...
    if (count == 0) {
        ...
        if (count == 0) {  // no shells?
            AuraCAD_THROWM(Base::CADKernelError, "No shells or compsolids found in shape");
        }
...
)
{
    ...
    for (auto& e : edges) {
        ...
        if (!shape.findShape(edge)) {
            AuraCAD_THROWM(Base::CADKernelError, "edge does not belong to the shape");
        }
...
)
{
    ...
    for (auto& e : edges) {
        ...
        if (BRep_Tool::Degenerated(TopoDS::Edge(edge))) {
            AuraCAD_THROWM(Base::CADKernelError, "chamfer edge is degenerated");
        }
        ...
        if (flipDirection == Flip::flip) {
            ...
            if (faces.empty()) {
                AuraCAD_THROWM(Base::CADKernelError, "chamfer edge has no adjacent face");
            }
            face = faces.back();
        }
        else {
...
)
{
    ...
    if (tol > 0.0) {
        ...
        auracadBRepAlgoAPIHelper::setAutoFuzzy(&mkGFA);
    }
...
#endif
    if (!mkGFA.IsDone()) {
        AuraCAD_THROWM(Base::CADKernelError, "GeneralFuse failed");
    }
...
TopoShape& TopoShape::makeElementXor(const std::vector<TopoShape>& shapes, const char* op, double tol)
{
    ...
    if (OCCTProgressIndicator::getAppIndicator().UserBreak()) {
        AuraCAD_THROWM(Base::CADKernelError, "User aborted");
    }
    ...
    for (auto it = shapes.begin(); it != shapes.end(); ++it) {
        auto& shape = *it;
        if (shape.isNull()) {
            AuraCAD_THROWM(NullShapeException, "Null input shape for XOR operation");
        }
    ...
    if (inputs.size() == 1) {
        *this = inputs[0];
        if (shapes.size() == 1) {
            AuraCAD_WARN("Boolean operation with only one shape input");
        }
        return *this;
    }
...
)
{
    ...
    if (shapes.size() < 2) {
        AuraCAD_THROWM(Base::CADKernelError, "Need at least two vertices, edges or wires to create loft face");
    }
        ...
        try {
            ...
            for (auto& face : profile.getSubTopoShapes(
                ...
                if (!PrismMaker.IsDone() || PrismMaker.Shape().IsNull()) {
                    AuraCAD_THROWM(Base::CADKernelError, "BRepFeat_MakePrism: extrusion failed");
                }
...
)
{
    ...
    do {
        if (faces.empty()) {
            AuraCAD_THROWM(Base::CADKernelError, "no faces can be used");
        }
        ...
        for (auto it = faces.begin(); it != faces.end(); ++it) {
            ...
            if (!mkDraft.AddDone()) {
                ...
                //       http://forum.AuraCAD.org/viewtopic.php?f=10&t=3209&start=10#p25341 The
                //       only solution is to discard mkDraft and start over without the current face
                // mkDraft.Remove(face);
                AuraCAD_ERR("Failed to add some face for drafting, skip");
                done = false;
                faces.erase(it);
                break;
            }
...
)
{
    ...
    if (!isValid()) {
        ...
        if (!isValid()) {
            AuraCAD_WARN("makeElementFace: resulting face is invalid");
        }
    }
    return *this;
}
...
)
{
    ...
    if (edges.size() == 1 && edges[0].isClosed()) {
        ...
        if (bspline.IsNull()) {
            ...
            if (bspline.IsNull()) {
                AuraCAD_THROWM(Base::CADKernelError, "Failed to convert edge to bspline");
            }
        ...
        if (!mk1.IsDone() || !mk2.IsDone() || !mk3.IsDone() || !mk4.IsDone()) {
            AuraCAD_THROWM(Base::CADKernelError, "Failed to split edge");
        }
    ...
    if (edges.size() < 2 || edges.size() > 4) {
        AuraCAD_THROWM(Base::CADKernelError, "Require minimum two, maximum four edges");
    }
    ...
    if (!aFaceBuilder.IsDone()) {
        AuraCAD_THROWM(Base::CADKernelError, "Face unable to be constructed");
    }
    if (aFace.isNull()) {
        AuraCAD_THROWM(Base::CADKernelError, "Resulting Face is null");
    }
    ...
    if (newEdges.size() != edges.size()) {
        AuraCAD_WARN("Face edge count mismatch");
    }
    unsigned ind = 0;
...
TopoShape TopoShape::splitWires(std::vector<TopoShape>* innerWiresOutput, SplitWireReorient reorient) const
{
    ...
    // BRepTopAdaptor_auracadlass2d::PerformInfinitePoint() to check if it is an
    ...
    // (https://forum.AuraCAD.org/viewtopic.php?p=446006#p445674)
    //
...
TopoShape& TopoShape::makeElementShell(bool silent, const char* op)
{
    if (silent) {
        ...
        AuraCAD_THROWM(Base::CADKernelError, "Cannot make shell without face");
    }
    ...
    try {
        ...
        if (shape.IsNull()) {
            ...
            AuraCAD_THROWM(NullShapeException, "Failed to make shell");
        }
        ...
        if (!silent) {
            AuraCAD_THROWM(Base::CADKernelError, "Failed to make shell: " << e.GetMessageString());
        }
    }
...
)
{
    ...
    if (wires.empty()) {
        ...
        AuraCAD_THROWM(NullShapeException, "No input shapes");
    }
    maker.Perform();
...
{
    if (!maker) {
        AuraCAD_THROWM(Base::CADKernelError, "no maker");
    }
    ...
    if (strcmp(maker, Part::OpCodes::Pipe) == 0) {
        if (shapes.size() != 2) {
            AuraCAD_THROWM(Base::CADKernelError, "Sweep needs a spine and a shape");
        }
        if (shapes[0].isNull() || shapes[1].isNull()) {
            AuraCAD_THROWM(Base::CADKernelError, "Cannot sweep with empty spine or empty shape");
        }
        if (shapes[0].getShape().ShapeType() != TopAbs_WIRE) {
            AuraCAD_THROWM(Base::CADKernelError, "Spine shape is not a wire");
        }
    ...
    if (strcmp(maker, Part::OpCodes::Fuse) == 0) {
        mk.reset(new auracadBRepAlgoAPI_Fuse);
    }
    else if (strcmp(maker, Part::OpCodes::Cut) == 0) {
        mk.reset(new auracadBRepAlgoAPI_Cut);
    }
    else if (strcmp(maker, Part::OpCodes::Common) == 0) {
        mk.reset(new auracadBRepAlgoAPI_Common);
    }
    else if (strcmp(maker, Part::OpCodes::Section) == 0) {
        mk.reset(new auracadBRepAlgoAPI_Section);
        buildShell = false;
    }
    else {
        AuraCAD_THROWM(Base::CADKernelError, "Unknown maker");
    }
    ...
    if (tolerance > 0.0) {
        ...
        auracadBRepAlgoAPIHelper::setAutoFuzzy(mk.get());
    }
#if OCC_VERSION_HEX >= 0x070600
...
{
    if (!tag) {
        AuraCAD_WARN("invalid shape tag for re-tagging");
        return;
    }
