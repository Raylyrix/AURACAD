...
# ---------------------------------------------------------------------------
# define the test cases to test the AuraCAD Part module
# ---------------------------------------------------------------------------
...
class PartTestCases(unittest.TestCase):
    def setUp(self):
        self.Doc = AuraCAD.newDocument("PartTest")
    ...
    def tearDown(self):
        # closing doc
        AuraCAD.closeDocument("PartTest")
...
class PartTestBSplineCurve(unittest.TestCase):
    ...
    def testIssue2671(self):
        if "BUILD_SPREADSHEET" in AuraCAD.__cmake__:
...
class PartTestRuledSurface(unittest.TestCase):
    ...
    def testRuledSurfaceFromTwoObjects(self):
        line1 = Part.makeLine(AuraCAD.Vector(-70, -30, 0), AuraCAD.Vector(-50, 40, 0))
        line2 = Part.makeLine(AuraCAD.Vector(-40, -30, 0), AuraCAD.Vector(-40, 10, 0))
        plm1 = AuraCAD.Placement()
        plm1.Rotation = AuraCAD.Rotation(0.7071067811865476, 0.0, 0.0, 0.7071067811865475)
    ...
    def testRuledSurfaceFromTwoObjectsWithSharedVertexs(self):
        ...
        # Arrange
        line = Part.makeLine(AuraCAD.Vector(0, 0, 50), AuraCAD.Vector(0, -50, 0))
        line1 = Part.makeLine(AuraCAD.Vector(0, 0, 50), AuraCAD.Vector(50, 0, 0))
        line2 = Part.makeLine(AuraCAD.Vector(0, 0, 0), AuraCAD.Vector(0, -50, 0))
        line3 = Part.makeLine(AuraCAD.Vector(0, 0, 0), AuraCAD.Vector(50, 0, 0))
    ...
    def testRuledSurfaceFromOneObject(self):
        if "BUILD_SKETCHER" in AuraCAD.__cmake__:
            sketch = self.Doc.addObject("Sketcher::SketchObject", "Sketch")
            sketch.Placement = AuraCAD.Placement(
                AuraCAD.Vector(0.000000, 0.000000, 0.000000),
...
class GeometryCurve(unittest.TestCase):
    def testProject(self):
        line = Part.Line()
        line.projectPoint(AuraCAD.Vector())
...
class EmptyEdge(unittest.TestCase):
    ...
    def testCenterOauracadurvatureAt(self):
        with self.assertRaises(ValueError):
            edge = Part.Edge()
            edge.centerOauracadurvatureAt(0)
...
class PartExtrusionTests(unittest.TestCase):
    ...
    def setUp(self):
        self.Doc = AuraCAD.newDocument("PartExtrusionTest")
    ...
    def tearDown(self):
        AuraCAD.closeDocument("PartExtrusionTest")
