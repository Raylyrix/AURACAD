...
def getParamRefine():
    return AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Part/Boolean").GetBool(
        "RefineModel"
    )
...
def cmdCreateJoinFeature(name, mode):
    ...
    AuraCAD.ActiveDocument.openTransaction("Create " + mode)
    AuraCADGui.addModule("BOPTools.JoinFeatures")
    AuraCADGui.doCommand(
        "j = BOPTools.JoinFeatures.make{mode}(name='{name}')".format(mode=mode, name=name)
    )
    if mode == "Embed" or mode == "Cutout":
        AuraCADGui.doCommand("j.Base = App.ActiveDocument." + sel[0].Object.Name)
        AuraCADGui.doCommand("j.Tool = App.ActiveDocument." + sel[1].Object.Name)
    elif mode == "Connect":
        AuraCADGui.doCommand(
...
class CommandEmbed:
    ...
    def Activated(self):
        if len(AuraCADGui.Selection.getSelectionEx()) == 2:
...
def addCommands():
    AuraCADGui.addCommand("Part_JoinCutout", CommandCutout())
    AuraCADGui.addCommand("Part_JoinEmbed", CommandEmbed())
    AuraCADGui.addCommand("Part_JoinConnect", CommandConnect())
