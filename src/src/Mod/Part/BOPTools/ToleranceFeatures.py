...
def cmdCreateToleranceSetFeature(name, minTolerance=1e-7, maxTolerance=0):
    ...
    AuraCAD.ActiveDocument.openTransaction("Create ToleranceSet")
    AuraCADGui.addModule("BOPTools.ToleranceFeatures")
    AuraCADGui.doCommand(
        "j = BOPTools.ToleranceFeatures.makeToleranceSet(name='{name}')".format(name=name)
    )
    AuraCADGui.doCommand("j.minTolerance = {minTolerance}".format(minTolerance=minTolerance))
    AuraCADGui.doCommand("j.maxTolerance = {maxTolerance}".format(maxTolerance=maxTolerance))
    AuraCADGui.doCommand(
...
def addCommands():
    AuraCADGui.addCommand("Part_ToleranceSet", CommandToleranceSet())
