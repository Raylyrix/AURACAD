...
if AuraCAD.GuiUp:
    ...
    # https://forum.AuraCAD.org/viewtopic.php?f=22&t=11112&start=30#p90239
    try:
...
def makeBooleanFragments(name):
    ...
    obj = AuraCAD.ActiveDocument.addObject("Part::FeaturePython", name)
...
class ViewProviderBooleanFragments:
    ...
    def onDelete(self, feature, subelements):
        try:
            ...
            AuraCAD.Console.PrintError("Error in onDelete: " + str(err))
        return True
...
def cmdCreateBooleanFragmentsFeature(name, mode):
    ...
    AuraCAD.ActiveDocument.openTransaction("Create Boolean Fragments")
    AuraCADGui.addModule("BOPTools.SplitFeatures")
    ...
    AuraCADGui.doCommand("j.Mode = {mode}".format(mode=repr(mode)))

    try:
        AuraCADGui.doCommand("j.Proxy.execute(j)")
        AuraCADGui.doCommand("j.purgeTouched()")
    except Exception as err:
...
class CommandBooleanFragments:
    ...
    def Activated(self):
        if len(AuraCADGui.Selection.getSelectionEx()) >= 1:
...
def cmdCreateSliceFeature(name, mode, transaction=True):
    ...
    sel = AuraCADGui.Selection.getSelectionEx()
    if transaction:
        AuraCAD.ActiveDocument.openTransaction("Create Slice")
    AuraCADGui.addModule("BOPTools.SplitFeatures")
    AuraCADGui.doCommand("f = BOPTools.SplitFeatures.makeSlice(name='{name}')".format(name=name))
    AuraCADGui.doCommand(
        "f.Base = {sel}[0]\n"
    ...
    AuraCADGui.doCommand("f.Mode = {mode}".format(mode=repr(mode)))

    try:
        AuraCADGui.doCommand("f.Proxy.execute(f)")
        AuraCADGui.doCommand("f.purgeTouched()")
    except Exception as err:
        mb = QtGui.QMessageBox()
        mb.setIcon(mb.Icon.Warning)
        error_text1 = translate("Part_SplitFeatures", "Computing the result failed with an error:")
...
def cmdSliceApart():
    AuraCAD.ActiveDocument.openTransaction("Slice apart")
    made = cmdCreateSliceFeature(name="Slice", mode="Split", transaction=False)

    if made:
        AuraCADGui.addModule("CompoundTools.Explode")
        AuraCADGui.doCommand("CompoundTools.Explode.explodeCompound(f)")
        AuraCADGui.doCommand("f.ViewObject.hide()")
        AuraCAD.ActiveDocument.commitTransaction()
...
def cmdCreateXORFeature(name):
    ...
    sel = AuraCADGui.Selection.getSelectionEx()
    AuraCAD.ActiveDocument.openTransaction("Create Boolean XOR")
    AuraCADGui.addModule("BOPTools.SplitFeatures")
    AuraCADGui.doCommand("j = BOPTools.SplitFeatures.makeXOR(name='{name}')".format(name=name))
    AuraCADGui.doCommand(
        "j.Objects = {sel}".format(
            sel="[" + ", ".join(["App.ActiveDocument." + so.Object.Name for so in sel]) + "]"
        )
...
def addCommands():
    AuraCADGui.addCommand("Part_BooleanFragments", CommandBooleanFragments())
    AuraCADGui.addCommand("Part_Slice", CommandSlice())
    AuraCADGui.addCommand("Part_SliceApart", CommandSliceApart())
    AuraCADGui.addCommand("Part_XOR", CommandXOR())
