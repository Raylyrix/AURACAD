...
class _CommandCompoundFilter:
    ...
    def Activated(self):
        if (
            ...
            or len(AuraCADGui.Selection.getSelection()) == 2
        ):
...
if AuraCAD.GuiUp:
    AuraCADGui.addCommand("Part_CompoundFilter", _CommandCompoundFilter())


# helper
def cmdCreateCompoundFilter(name):
    sel = AuraCADGui.Selection.getSelection()
    AuraCAD.ActiveDocument.openTransaction("Create CompoundFilter")
    AuraCADGui.addModule("CompoundTools.CompoundFilter")
    AuraCADGui.doCommand(
        "f = CompoundTools.CompoundFilter.makeCompoundFilter(name = '" + name + "')"
    )
    AuraCADGui.doCommand("f.Base = App.ActiveDocument." + sel[0].Name)
    if len(sel) == 2:
        AuraCADGui.doCommand("f.Stencil = App.ActiveDocument." + sel[1].Name)
        AuraCADGui.doCommand("f.Stencil.ViewObject.hide()")
        AuraCADGui.doCommand("f.FilterType = 'collision-pass'")
    else:
        AuraCADGui.doCommand("f.FilterType = 'window-volume'")
    ...
    AuraCADGui.doCommand("f.Base.ViewObject.hide()")
    AuraCADGui.doCommand("f = None")
