...
void CmdPartPointsFromMesh::activated(int iMsg)
{
    ...
    auto found = std::find_if(geoms.begin(), geoms.end(), [](App::DocumentObject* obj) {
        return AuraCAD_cast<Part::Feature*>(obj);
    });

    if (found != geoms.end()) {

        double defaultDistance = getDefaultDistance(AuraCAD_cast<Part::Feature*>(*found));
    ...
    for (std::vector<Gui::SelectionObject>::iterator it = objs.begin(); it != objs.end(); ++it) {
        try {
            ...
            doCommand(
                ...
                "else:\n"
                "\t\tAuraCAD.Console.PrintError('Defeaturing failed\\n')",
                it->getDocName(),
                it->getFeatName(),
                faces.c_str(),
                it->getFeatName()
