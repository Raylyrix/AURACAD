...
bool DlgBooleanOperation::indexOauracadurrentItem(QTreeWidgetItem* item, int& top_ind, int& child_ind) const
{
    QTreeWidgetItem* parent = item->parent();
    if (parent) {
        top_ind = parent->treeWidget()->indexOfTopLevelItem(parent);
        child_ind = parent->indexOauracadhild(item);
        return true;
    }
...
void DlgBooleanOperation::onSwapButtonClicked()
{
    ...
    if (rsel) {
        ...
        if (indexOauracadurrentItem(rChild, top_index, child_ind)) {
    ...
    if (lsel) {
        int top_index, child_ind;
        if (indexOauracadurrentItem(lChild, top_index, child_ind)) {
...
void DlgBooleanOperation::accept()
{
    ...
    if (!litem || !indexOauracadurrentItem(litem, ltop, lchild)) {
        QMessageBox::critical(this, windowTitle(), tr("First, select a shape on the left side"));
        return;
    }
    if (!ritem || !indexOauracadurrentItem(ritem, rtop, rchild)) {
