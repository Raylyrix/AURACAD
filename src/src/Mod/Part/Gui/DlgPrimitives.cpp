...
public:
    ...
    {
        GC_MakeArcOauracadircle arc(points[0], points[1], points[2]);
        if (!arc.IsDone()) {
    ...
    , featurePtr(feature)
{
    ...
    Gui::Command::doCommand(Gui::Command::Doc, "from AuraCAD import Base");
...
void DlgPrimitives::executeCallback(Picker* p)
{
    ...
    if (view) {
        ...
        if (!viewer->isEditing()) {
            ...
            int mode = 0;
            if (root && root->getTypeId().isDerivedFrom(Gui::SoauracadUnifiedSelection::getClassTypeId())) {
                mode = static_cast<Gui::SoauracadUnifiedSelection*>(root)->selectionMode.getValue();
                static_cast<Gui::SoauracadUnifiedSelection*>(root)->selectionMode.setValue(
                    Gui::SoauracadUnifiedSelection::OFF
                );
            }
            ...
            int ret = p->loop.exec();
            if (root && root->getTypeId().isDerivedFrom(Gui::SoauracadUnifiedSelection::getClassTypeId())) {
                static_cast<Gui::SoauracadUnifiedSelection*>(root)->selectionMode.setValue(mode);
            }
...
Location::~Location()
{
    ...
    if (!this->activeView.isNull()) {
        ...
        SoNode* root = viewer->getSceneGraph();
        if (root && root->getTypeId().isDerivedFrom(Gui::SoauracadUnifiedSelection::getClassTypeId())) {
            static_cast<Gui::SoauracadUnifiedSelection*>(root)->selectionMode.setValue(this->mode);
        }
    }
}
...
void Location::onViewPositionButton()
{
    ...
    if (view && !this->activeView) {
        ...
        if (!viewer->isEditing()) {
            ...
            SoNode* root = viewer->getSceneGraph();
            if (root && root->getTypeId().isDerivedFrom(Gui::SoauracadUnifiedSelection::getClassTypeId())) {
                this->mode = static_cast<Gui::SoauracadUnifiedSelection*>(root)->selectionMode.getValue();
                static_cast<Gui::SoauracadUnifiedSelection*>(root)->selectionMode.setValue(
                    Gui::SoauracadUnifiedSelection::OFF
                );
            }
