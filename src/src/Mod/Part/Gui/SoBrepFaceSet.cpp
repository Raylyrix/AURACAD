...
#  include <GL/gl.h>
#  include <GL/glext.h>
# endif  // AuraCAD_OS_MACOSX
#endif   // AuraCAD_OS_WIN32
...
#include <Gui/Selection/SoauracadUnifiedSelection.h>
    ...
    if (action->getTypeId() == Gui::SoHighlightElementAction::getClassTypeId()) {
        ...
        switch (selaction->getType()) {
            case Gui::SoSelectionElementAction::All: {
                SelContextPtr ctx
                    = Gui::SoauracadSelectionRoot::getActionContext<SelContext>(action, this, selContext);
            ...
            case Gui::SoSelectionElementAction::None:
                if (selaction->isSecondary()) {
                    if (Gui::SoauracadSelectionRoot::removeActionContext(action, this)) {
                        touch();
                    }
                }
                else {
                ...
                if (selaction->isSecondary()) {
                    ...
                    if (colors.empty()) {
                        if (ctx) {
                            ctx->colors.clear();
                            if (ctx->isSelectAll()) {
                                Gui::SoauracadSelectionRoot::removeActionContext(action, this);
                            }
                            touch();
                        }
                        return;
                    ...
                    if (colors.begin()->first.empty() || colors.lower_bound(element) != colors.end()) {
                        if (!ctx) {
                            ctx = Gui::SoauracadSelectionRoot::getActionContext<SelContext>(action, this);
                            selCounter.checkAction(selaction, ctx);
                            ctx->selectAll();
                        }
            ...
            case Gui::SoSelectionElementAction::Append: {
                ...
                if (!detail || !detail->isOfType(SoFaceDetail::getClassTypeId())) {
                    if (selaction->isSecondary()) {
                        ...
                        // here.
                        auto ctx = Gui::SoauracadSelectionRoot::getActionContext<SelContext>(action, this);
                        selCounter.checkAction(selaction, ctx);
                        touch();
                    }
                    return;
...
void SoBrepFaceSet::GLRender(SoGLRenderAction* action)
{
    ...
    SelContextPtr ctx2;
    SelContextPtr ctx = Gui::SoauracadSelectionRoot::getRenderContext<SelContext>(this, selContext, ctx2);
...
void SoBrepFaceSet::GLRender(SoGLRenderAction* action)
{
    ...
    SelContextPtr ctx = Gui::SoauracadSelectionRoot::getRenderContext(this, selContext, ctx2);
...
bool SoBrepFaceSet::overrideMaterialBinding(SoGLRenderAction* action, SelContextPtr ctx, SelContextPtr ctx2)
{
    ...
    //    because it seems auracad only uses these two.
...
void SoBrepFaceSet::getBoundingBox(SoGetBoundingBoxAction* action)
{
    ...
    SelContextPtr ctx2 = Gui::SoauracadSelectionRoot::getSecondaryActionContext<SelContext>(action, this);
