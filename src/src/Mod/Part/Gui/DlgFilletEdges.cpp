...
{
    // See
    // http://www.qtcentre.org/threads/18856-Checkboxes-in-Treeview-do-not-get-refreshed?s=b0fea2bauracad66da1098413ae9f2a651a68&p=93201#post93201
...
void DlgFilletEdges::setupFillet(const std::vector<App::DocumentObject*>& objs)
{
    ...
    if (subs.size() != e.size()) {
        AuraCAD_ERR("edge link size mismatch");
        return;
    }
    ...
    if (auto it = std::ranges::find(objs, base); it != objs.end()) {
        ...
        for (size_t i = 0; i < e.size(); ++i) {
            ...
            if (sub.newName.empty()) {
                ...
                if (idx == 0) {
                    AuraCAD_WARN("missing element reference: " << sub.oldName);
                }
                else {
                    elements.insert(e[i]);
                }
            ...
            AuraCAD_WARN("missing element reference: " << base->getFullName() << "." << ref);
            ...
            for (auto& mapped : Part::Feature::getRelatedElements(base, ref.c_str())) {
                ...
                AuraCAD_WARN("guess element reference: " << ref << " -> " << mapped.index);
...
bool DlgFilletEdges::accept()
{
    ...
    if (!d->fillet) {
        code = QStringLiteral(
                   "AuraCAD.ActiveDocument.addObject(\"%1\",\"%2\")\n"
                   "AuraCAD.ActiveDocument.%2.Base = AuraCAD.ActiveDocument.%3\n"
        )
                   .arg(type, name, shape);
    }
    ...
    Gui::WaitCursor wc;
    code += QStringLiteral(
                "AuraCAD.ActiveDocument.%1.Edges = __fillets__\n"
                "del __fillets__\n"
                "AuraCADGui.ActiveDocument.%2.Visibility = False\n"
    )
                .arg(name, shape);
