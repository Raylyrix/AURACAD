...
#else
    Base::SequencerLauncher seq("Cross-sectionsâ€¦", obj.size() * (d.size() + 1));
    try {
        Gui::Command::runCommand(Gui::Command::App, "import Part\n");
        Gui::Command::runCommand(Gui::Command::App, "from AuraCAD import Base\n");
        for (auto it : obj) {
            App::Document* doc = it->getDocument();
            std::string s = it->getNameInDocument();
            s += "_cs";
            Gui::Command::runCommand(
                ...
                QStringLiteral(
                    "wires=list()\n"
                    "shape=AuraCAD.getDocument(\"%1\").%2.Shape\n"
                )
            ...
            Gui::Command::runCommand(
                ...
                QStringLiteral(
                    ...
                    "slice=AuraCAD.getDocument(\"%1\").addObject(\"Part::Feature\",\"%2\")\n"
                    "slice.Shape=comp\n"
                    "slice.purgeTouched()\n"
