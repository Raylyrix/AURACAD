...
struct SoBrepEdgeSet::SelContext: Gui::SoauracadSelectionContextEx
{
    ...
    if (hasColorOverride) {
        ...
        if (!highlights.empty()) {
            ...
            for (const auto& segment : highlights) {
                ...
                // Note: AuraCAD Base::Color transparency is 0.0 (opaque) to 1.0 (transparent)
        ...
        switch (selaction->getType()) {
            ...
            case Gui::SoSelectionElementAction::Remove: {
                ...
                if (selaction->getType() == Gui::SoSelectionElementAction::Append) {
                    ctx = Gui::SoauracadSelectionRoot::getActionContext(action, this, selContext);
                    selCounter.checkAction(selaction, ctx);
                    ctx->selectionColor = selaction->getColor();
                    if (ctx->isSelectAll()) {
                        ctx->selectionIndex.clear();
                    ...
                    ctx = Gui::SoauracadSelectionRoot::getActionContext(action, this, selContext, false);
