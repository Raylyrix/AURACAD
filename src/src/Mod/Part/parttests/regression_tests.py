...
import AuraCAD
from AuraCAD import Vector, Base, newDocument, closeDocument
import Part
...
class RegressionTests(unittest.TestCase):
    ...
    def test_TangentMode3_Issue24254(self):
        """In AuraCAD 1.1 we changed the behavior of the mmTangentPlane, but added code to handle old files
        that relied upon the original tangent plane behavior. This test ensures that old files open with the expected
        tangent plane setup."""

        location = os.path.dirname(os.path.realpath(__file__))
        AuraCAD.openDocument(os.path.join(location, "TestTangentMode3-0.21.auracadStd"), True)

        def check_plane(name, expected_pos, expected_normal, expected_xaxis):
            obj = AuraCAD.ActiveDocument.getObject(name)
            if not obj:
                AuraCAD.Console.PrintError(f"{name}: object not found\n")
                return

            pos = obj.Placement.Base
            rot = obj.Placement.Rotation
            normal = rot.multVec(AuraCAD.Vector(0, 0, 1))
            xaxis = rot.multVec(AuraCAD.Vector(1, 0, 0))
        ...
        expected_planes = [
            (
                "Sketch001",
                AuraCAD.Vector(-1 * r, 0, 0),  # position
                AuraCAD.Vector(1, 0, 0),  # normal
                AuraCAD.Vector(0, -1, 0),
            ),  # tangent x-axis
            (
                "Sketch002",
                AuraCAD.Vector(sin30 * r, cos30 * cos30 * (-r), cos30 * sin30 * r),
                AuraCAD.Vector(sin30, cos30 * cos30 * (-1), cos30 * sin30),
                AuraCAD.Vector(0, sin30, cos30),
            ),
            (
                "Sketch003",
                AuraCAD.Vector(cos30 * cos30 * r, sin30 * r, cos30 * sin30 * r),
                AuraCAD.Vector(cos30 * cos30, sin30, cos30 * sin30),
                AuraCAD.Vector(sin30, 0, -cos30),
            ),
        ]
    ...
    def test_transformShape_copy_returns_independent_shape(self):
        ...
        cloned = shp.transformShape(AuraCAD.Matrix(), True)
        ...
        moved = cloned.transformShape(
            AuraCAD.Placement(AuraCAD.Vector(5, 0, 0), AuraCAD.Rotation()).toMatrix(), False
        )
