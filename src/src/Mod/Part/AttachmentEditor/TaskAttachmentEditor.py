...
class AttachmentEditorTaskPanel(FrozenClass):
    ...
    def __init__(
        ...
        for i in range(len(self.refLines)):
            QtCore.QObject.connect(
                ...
                lambda txt, i=i: self.lineReauracadhanged(i, txt),
            )
    ...
    def addSelection(self, docname, objname, subname, pnt):
        ...
        if i > 0 and self.auto_next:
            ...
            if prevref[0].Name == objname and subname == "":
                ...
                # its subelement was already written to line[i-1], so we decrease i to overwrite the lineReauracadhanged
                i -= 1
    ...
    def lineReauracadhanged(self, index, value):
        if self.block:
            return
