...
constexpr std::array
    lengthUnitLabels {"nm", "Âµm", "mm", "cm", "dm", "m", "km", "in", "ft", "thou", "yd", "mi"};

constexpr std::array angleUnitLabels {"deg", "rad", "gon"};

constexpr std::array areaUnitLabels {"mmÂ²", "cmÂ²", "mÂ²", "kmÂ²", "inÂ²", "ftÂ²", "ydÂ²", "miÂ²"};
...
void TaskMeasure::tryUpdate()
{
    ...
    for (auto sel : Gui::Selection().getSelection()) {
        ...
        // Resolve App::Link
        if (auto link = AuraCAD_cast<App::Link*>(sub)) {
