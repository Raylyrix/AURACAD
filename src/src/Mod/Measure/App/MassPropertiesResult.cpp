...
)
{
    ...
    for (const auto& object : objects) {
        ...
        if (shape.IsNull()) {
            auto* part = AuraCAD_cast<Part::Feature*>(obj);
            if (!part) {
                continue;
            }
        ...
        auto materialFeature = [](App::DocumentObject* candidate) -> Part::Feature* {
            ...
            while (candidate && visited.insert(candidate).second) {
                if (auto* feature = AuraCAD_cast<Part::Feature*>(candidate)) {
                    return feature;
                }
    ...
    if (mode == MassPropertiesMode::CenterOfGravity) {
        ...
        if (auto referenceDatumElement = AuraCAD_cast<App::DatumElement const*>(referenceDatum)) {
            lcs = referenceDatumElement->getLCS();
        }


        if (auto const* line = AuraCAD_cast<App::Line const*>(referenceDatum)) {
        ...
        if (!lcs) {
            lcs = AuraCAD_cast<const App::LocalCoordinateSystem*>(referenceDatum);
            if (!lcs) {
                return data;
            }
        }
