...
{
    std::stringstream ss;
    ss << "AuraCAD v" << App::Application::Config()["BuildVersionMajor"] << "."
...
void CDxfRead::DoRead(const bool ignore_errors /* = false */)
{
    ...
    try {
        ...
        // This catches specific AuraCAD exceptions and re-throws them.
        throw;
    }
    catch (const std::exception& e) {
        // This catches all standard C++ exceptions and converts them
        // to a AuraCAD exception, which the binding layer can handle.
...
Base::Color CDxfRead::ObjectColor(ColorIndex_t index)
{
    ...
    // this is just a rendering feature, but AuraCAD does not support this so the user has the
