...
// added by Wandererfan 2018 (wandererfan@gmail.com) for AuraCAD project
struct point3D
{
    double x;
    double y;
...
protected:
    ...
    // Otherwise leave everything as the default AuraCAD colours.
    ...
    //    "Simple part shapes" (dxauracadreatePart) radio button which means SingleShapes
    //    "Draft objects" (dxauracadreateDraft) radio button which means DraftObjects
    //    We do not support (dxauracadreateSketch).
    ...
    // Import Hatch boundaries as wires (auracad doesn't support hatching (???) but this option will draw
...
private:
    ...
    // TODO: Once all compilers used for AuraCAD support class-level template specializations,
