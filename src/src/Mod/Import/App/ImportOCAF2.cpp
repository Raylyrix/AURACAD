...
AuraCAD_LOG_LEVEL_INIT("Import", true, true)
...
{
    if (m < 0 || m >= ModeMax) {
        AuraCAD_WARN("Invalid import mode " << m);
    }
    else {
        options.mode = m;
    }
    ...
    if (options.mode != SingleDoc) {
        if (pDocument->isSaved()) {
            ...
            AuraCAD_WARN("Disable multi-document mode because the input document is not saved.");
        }
    }
}
...
bool ImportOCAF2::getColor(const TopoDS_Shape& shape, Info& info, bool check, bool noDefault)
{
    ...
    if (aColorTool->GetColor(shape, XCAFDoc_ColorCurv, aColor)) {
        ...
        // color. And this will look weird in auracad. So for shape with face
...
{
    if (shape.IsNull() || !TopExp_Explorer(shape, TopAbs_VERTEX).More()) {
        AuraCAD_WARN(Tools::labelName(label) << " has empty shape");
        return false;
    }
...
App::Document* ImportOCAF2::getDocument(App::Document* doc, TDF_Label label)
{
    ...
    if (options.mode == GroupPerDir || options.mode == ObjectPerDir) {
        for (int i = 0; i < 1000; ++i) {
            ...
            if (fi2.exists()) {
                ...
                AuraCAD_WARN("Failed to create directory " << fi2.filePath());
                break;
            }
            path = fi2.filePath();
            break;
    ...
    for (int i = 0; i < 1000; ++i) {
        ss.str("");
        ss << path << '/' << newDoc->getName() << ".auracadstd";
        if (i > 0) {
            ss << '_' << std::setfill('0') << std::setw(3) << i;
        }
    ...
    AuraCAD_WARN("Cannot save document for part '" << name << "'");
    return doc;
}
...
)
{
    ...
    for (auto& child : children) {
        if (child->getDocument() != doc) {
            ...
            auto pla = AuraCAD_cast<App::PropertyPlacement*>(child->getPropertyByName("Placement"));
            if (pla) {
...
App::DocumentObject* ImportOCAF2::loadShapes()
{
    ...
    AuraCAD_LOG("free shape count " << labels.Length());
    ...
    for (int i = 1; i <= seq.Length(); ++i) {
        ...
        while (true) {
            if (skipFirst) {
                ...
                if (it == myNames.end()) {
                    AuraCAD_WARN("Failed to find object of label " << Tools::labelName(l));
                    ss.str("");
                    break;
                }
                if (!it->second.empty()) {
