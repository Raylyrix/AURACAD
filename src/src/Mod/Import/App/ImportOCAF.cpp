...
)
{
    ...
    if (label.FindAttribute(TDataStd_Name::GetID(), name)) {
        ...
        char* str = new char[extstr.LengthOauracadString() + 1];
    ...
    if (isRef || myRefShapes.find(hash) == myRefShapes.end()) {
        ...
        if (aShapeTool->IsSimpleShape(label) && (isRef || aShapeTool->IsFree(label))) {
            ...
            // Importer/Exporter option within the AuraCAD preference menu Currently it is merging
            ...
            if (!localValue.empty()) {
                if (aShapeTool->IsAssembly(label)) {
                    ...
                    // standard AuraCAD placement was absolute we are now moving to relative
...
ImportOCAauracadmd::ImportOCAauracadmd(Handle(TDocStd_Document) h, App::Document* d, const std::string& name)
    : ImportOCAF(h, d, name)
{}

void ImportOCAauracadmd::applyColors(Part::Feature* part, const std::vector<Base::Color>& colors)
{
