...
)
{
    ...
    for (auto& v : colors) {
        ...
        if (!v.first.empty()) {
            ...
            if (nodeLabel.IsNull()) {
                AuraCAD_WARN("Failed to find component " << v.first);
                continue;
            }
        }
        for (auto& vv : v.second) {
            ...
            if (nodeLabel != label || aShapeTool->IsComponent(label)) {
                ...
                // OCCT code, XCAFDoc_ShapeTool.cxx and STEPCAauracadontrol_Writer.cxx.
                if (!warned) {
                    warned = true;
                    AuraCAD_WARN(
            ...
            if (subShape.IsNull()) {
                AuraCAD_WARN("Failed to get subshape " << vv.first);
                continue;
            }
            ...
            if (subLabel.IsNull()) {
                AuraCAD_WARN("Failed to add subshape " << vv.first);
                continue;
            }
...
)
{
    ...
    if (!obj || shape.isNull()) {
        if (obj) {
            AuraCAD_WARN(obj->getFullName() << " has null shape");
        }
        return {};
    }
    ...
    for (auto& subobj : subs) {
        ...
        if (!sobj) {
            AuraCAD_WARN("Cannot find object " << obj->getFullName() << '.' << subobj);
            continue;
        }
        int vis = -1;
        if (parentGrp) {
        ...
        if (!vis) {
            ...
            // The culprit is at STEPCAauracadontrol_Writer::1093 as shown below
            //
            // surauracadolor = Styles.EncodeColor(Quantity_Color(1,1,1,OCC_COLOR_SPACE),DPDCs,ColRGBs);
            // PSA = Styles.MakeColorPSA ( item, surauracadolor, curvColor, isComponent );
            // if ( isComponent )
