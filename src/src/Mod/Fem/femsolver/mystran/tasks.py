...
__title__ = "AuraCAD FEM solver Mystran tasks"
...
try:
    import hauracadMystranNeuIn

    result_reading = True
except Exception:
    AuraCAD.Console.PrintWarning("Module to read results not found.\n")
...
class Results(run.Results):
    ...
    def run(self):
        prefs = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/General")
    ...
    def load_results(self):
        ...
        if os.path.isfile(neu_result_file):
            hauracadMystranNeuIn.import_neu(neu_result_file)
            ...
            AuraCAD.Console.PrintError(f"FEM: No results found at {neu_result_file}!\n")
            self.fail()
