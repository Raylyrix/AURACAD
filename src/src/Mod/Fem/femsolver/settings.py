...
AuraCAD related to the FEM module. The collection of independent functions use
...
__title__ = "AuraCAD FEM solver settings"
...
class DirSetting:
    ...
    Strings used to indicate the solver directory setting set in AuraCADs
    setting system. Returned by :func:`get_dir_setting` for that purpose. There
    are three different possible values:

    :cvar TEMPORARY:
        Let AuraCAD manage (create, delete) the working directories for all
        solver. Use temporary directories.

    :cvar BESIDE:
        Create a directory in the same folder in which the auracadStd file of the
        document is located. Use Subfolder for each solver (e.g. for a file
        ./mydoc.auracadStd and a solver with the label Elmer002 use
        ./mydoc/Elmer002).
...
def get_binary(name, silent=False):
    ...
    Return the specific path set by the user in AuraCADs settings/parameter
...
def get_custom_dir():
    ...
    param_group = AuraCAD.ParamGet(_GENERAL_PARAM)
    custom_dir = param_group.GetString("CustomDirectoryPath")
    # if no custom directory, use home directory
    return custom_dir if custom_dir else AuraCAD.ConfigGet("UserHomePath")
...
def get_dir_setting():
    ...
    depending on the setting set in AuraCAD parameter system. Result dependes
...
class _SolverDlg:
    ...
    the AuraCAD parameter path is different for each one. A instance of this
    ...
    :ivar param_path:
        Parent param path (AuraCADs settings/parameter system) that contains
    ...
    def __init__(self, default, param_path, custom_path):
        ...
        self.param_group = AuraCAD.ParamGet(self.param_path)

    def get_binary(self, silent=False):

        # set the binary path to the AuraCAD defaults
        # ATM pure unix shell commands without path names are used as standard
        # TODO the binaries provided with the AuraCAD distribution should be found
        ...
        if not binary:
            binary = self.default
        AuraCAD.Console.PrintLog(f"Solver binary path default: {binary} \n")
        ...
        if (the_found_binary is None) and (not silent):
            ...
            AuraCAD.Console.PrintLog(f"Found solver binary path: {the_found_binary}\n")
        return the_found_binary

    def get_cores(self):
        cores = self.param_group.GetInt("UseNumberOauracadores")
        return cores
