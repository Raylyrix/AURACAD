...
__title__ = "AuraCAD FEM solver run"
    ...
    if solver.Proxy.Type == "Fem::SolverCcxTools":
        ...
        try:
            ...
            error_message = (
                ...
                'directory is set to "Beside *.auracadStd File".'
            )
            App.Console.PrintError(error_message + "\n")
            if App.GuiUp:
                QtGui.QMessageBox.critical(
                    AuraCADGui.getMainWindow(), "Can't start Solver", error_message
                )
            return
...
def _getBesideBase(solver):
    ...
    # doc=App.newDocument()
    # doc.FileName
    # the above returns an empty string in AuraCAD 0.19
    # https://forum.AuraCAD.org/viewtopic.php?f=10&t=48842
    if path == "":
        error_message = (
            "Please save the file before executing the solver. "
            "This must be done because the location of the working "
