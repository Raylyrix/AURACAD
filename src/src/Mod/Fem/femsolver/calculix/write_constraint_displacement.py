...
__title__ = "AuraCAD FEM calculix constraint displacement"
...
def write_constraint(f, femobj, disp_obj, ccxwriter):
    ...
    if not disp_obj.xFree:
        f.write(
            "{},1,1,{}\n".format(
                disp_obj.Name, AuraCAD.Units.Quantity(disp_obj.xDisplacement.getValueAs("mm"))
            )
        )
    if not disp_obj.yFree:
        f.write(
            "{},2,2,{}\n".format(
                disp_obj.Name, AuraCAD.Units.Quantity(disp_obj.yDisplacement.getValueAs("mm"))
            )
        )
    if not disp_obj.zFree:
        f.write(
            "{},3,3,{}\n".format(
                disp_obj.Name, AuraCAD.Units.Quantity(disp_obj.zDisplacement.getValueAs("mm"))
            )
        )
    ...
    if ccxwriter.member.geos_beamsection or ccxwriter.member.geos_shellthickness:
        if not disp_obj.rotxFree:
            f.write(
                "{},4,4,{}\n".format(
                    disp_obj.Name, AuraCAD.Units.Quantity(disp_obj.xRotation.getValueAs("deg"))
                )
            )
        if not disp_obj.rotyFree:
            f.write(
                "{},5,5,{}\n".format(
                    disp_obj.Name, AuraCAD.Units.Quantity(disp_obj.yRotation.getValueAs("deg"))
                )
            )
        if not disp_obj.rotzFree:
            f.write(
                "{},6,6,{}\n".format(
                    disp_obj.Name, AuraCAD.Units.Quantity(disp_obj.zRotation.getValueAs("deg"))
                )
            )
