...
class CalculiXTools(ObjectTools):
    ...
    def _generate_disp_mesh(self, mb):
        try:
            ...
            AuraCAD.Console.PrintWarning("Displaced mesh not generated\n")
            return mb
    ...
    def _generate_derived_result(self, mb):
        try:
            ...
            # return original data
            AuraCAD.Console.PrintWarning("Derived result not generated\n")
            return mb

    def _set_time_info(self, pipeline):
        match self.obj.AnalysisType:
            case "frequency":
                pipeline.setTimeInfo("Frequency", AuraCAD.Units.Frequency)
            case "buckling":
                pipeline.setTimeInfo("Buckling factor", AuraCAD.Units.Unit())
            case _:
                pipeline.setTimeInfo("TimeStep", AuraCAD.Units.TimeSpan)
