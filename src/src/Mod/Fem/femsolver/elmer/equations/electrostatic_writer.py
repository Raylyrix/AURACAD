...
__title__ = "AuraCAD FEM Electrostatics Elmer writer"
...
class ESwriter:
        ...
        for obj in self.write.getMember("Fem::ConstraintElectromagnetic"):
            if obj.References:
                for name in obj.References[0][1]:
                    # output the AuraCAD label as comment
                    if obj.Label:
                        self.write.boundary(name, "! AuraCAD Name", obj.Label)
    ...
    def handleElectrostaticBodyForces(self):
        for obj in self.write.getMember("Fem::ConstraintElectricChargeDensity"):
            ...
            for feat, sub_elem in obj.References:
                for name in sub_elem:
                    self.write.bodyForce(name, "! AuraCAD Name", obj.Label)
