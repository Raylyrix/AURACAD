...
__title__ = "AuraCAD FEM solver Elmer writer"
...
class Writer:
    ...
    def _handleUnits(self):
        # Elmer solver writer no longer uses AuraCAD unit system
        # to retrieve units for writing the sif file
        #
        # ATM Elmer writer uses SI units only
        #
        # see forum topic: https://forum.AuraCAD.org/viewtopic.php?f=18&t=70150
        #
        # TODO: adapt method and comment
        # should be only one system for all solver and not in each solver
        # https://forum.AuraCAD.org/viewtopic.php?t=47895
        # https://forum.AuraCAD.org/viewtopic.php?t=48451
        # https://forum.AuraCAD.org/viewtopic.php?f=10&t=48642
        # The AuraCAD unit schema is only used to determine the schema number
        ...
        # TODO retrieve the seven base units from AuraCAD unit schema
        # instead of hard coding them here for a second once
        self.unit_schema = Units.Scheme.Internal
        self.unit_system = {  # standard AuraCAD Base units = unit schema 0
            "L": "m",
            "M": "kg",
            "T": "s",
            "I": "A",
        ...
        if self.unit_schema == Units.Scheme.Internal:
            Console.PrintMessage(
                "The AuraCAD standard unit schema mm/kg/s is used. "
            ...
            Console.PrintMessage(
                ...
                "The AuraCAD standard unit schema mm/kg/s is used. "
                "Elmer sif-file writing is done in Standard AuraCAD units.\n".format(
    ...
    def _handleRedifinedConstants(self):
        ...
        if len(objs) == 1:
            ...
            # since the base unit of auracad is in mm, we must scale it to get plain SI
    ...
    def _addOutputSolver(self):
        s = sifio.createSection(sifio.SOLVER)
        # Since AuraCAD meshes are in mm we let Elmer scale it
        ...
        s["Output File Name"] = sifio.FileAttr("AuraCAD")
        s["Vtu Format"] = True
