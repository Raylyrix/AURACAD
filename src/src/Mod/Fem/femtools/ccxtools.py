...
class FemToolsCcx(QtCore.QRunnable, QtCore.QObject):
    ...
    def find_analysis(self):
        ...
        for m in AuraCAD.activeDocument().Objects:
    ...
    def update_objects(self):
        ...
        if mesh is not None:
            ...
            AuraCAD.Console.PrintWarning(f"{message} The prerequisite check will fail.\n")
    ...
    def check_prerequisites(self):
        ...
        AuraCAD.Console.PrintMessage("Check prerequisites...\n")
        message = ""
        # analysis
        if not self.analysis:
    ...
    def setup_working_dir(self, param_working_dir=None, create=False):
        ...
        fem_general_prefs = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/General")
        if param_working_dir is not None:
            ...
            if femutils.check_working_dir(self.working_dir) is not True:
                ...
                self.working_dir = femutils.get_pref_working_dir(self.solver)
                AuraCAD.Console.PrintMessage(f"Dir '{self.working_dir}' will be used instead.\n")
        else:
            self.working_dir = femutils.get_pref_working_dir(self.solver)
    ...
    def setup_ccx(self, ccx_binary=None, ccx_binary_sig="CalculiX"):
        ...
        if self.ccx_binary is None:
            self.ccx_binary = AuraCAD.ParamGet(
        ...
        try:
            ...
            AuraCAD.Console.PrintError(f"{e}\n")
            if e.errno == 2:
            ...
            error_message = self.tr(
                ...
                "Check if ccx runs standalone without AuraCAD."
    ...
    def start_ccx(self):
        ...
        self.ccx_prefs = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Ccx")
    ...
    def ccx_run(self):
        ret_code = None
        AuraCAD.Console.PrintMessage("\n")  # because of time print in separate line
        AuraCAD.Console.PrintMessage("CalculiX solver run...\n")
        if self.test_mode:
            AuraCAD.Console.PrintError("CalculiX can not be run if test_mode is True.\n")
            return
        self.setup_ccx()
        if self.ccx_binary_present is False:
            ...
            return
        progress_bar = AuraCAD.Base.ProgressIndicator()
        ...
        if ret_code or self.ccx_stderr:
            if ret_code == 201 and self.solver.AnalysisType == "check":
                ...
                # https://forum.AuraCAD.org/viewtopic.php?f=18&t=31303&start=10#p260743
                ret_code = 0
            else:
                AuraCAD.Console.PrintError(f"CalculiX failed with exit code {ret_code}\n")
                AuraCAD.Console.PrintMessage("--------start of stderr-------\n")
                AuraCAD.Console.PrintMessage(self.ccx_stderr)
                AuraCAD.Console.PrintMessage("--------end of stderr---------\n")
                AuraCAD.Console.PrintMessage("--------start of stdout-------\n")
                AuraCAD.Console.PrintMessage(self.ccx_stdout)
                AuraCAD.Console.PrintMessage("\n--------end of stdout---------\n")
                AuraCAD.Console.PrintMessage("--------start problems---------\n")
            ...
            AuraCAD.Console.PrintMessage("CalculiX finished without error.\n")
        return ret_code
    ...
    def run(self):
        ...
        if message:
            ...
            error_gui = f"{text}\n{message}"
            AuraCAD.Console.PrintError(error_app)
            if AuraCAD.GuiUp:
            ...
            if self.inp_file_name == "":
                ...
                AuraCAD.Console.PrintLog("Writing CalculiX input file completed.\n")
                ...
                if ret_code != 0:
                    ...
                    AuraCAD.Console.PrintLog("Try to read result files\n")
                    self.load_results()
    ...
    def has_nonpositive_jacobians(self):
        if "*ERROR in e_c3d: nonpositive jacobian" in self.ccx_stdout:
            ...
            AuraCAD.Console.PrintMessage(command_for_nonposjacnodes + "\n")
            if AuraCAD.GuiUp:
                import AuraCADGui

                # with this the list nonpositive_jacobian_elenodes
                # will be available for further user interaction
                AuraCADGui.doCommand(command_for_nonposjacnodes)
                AuraCAD.Console.PrintMessage("\n")
                AuraCADGui.doCommand(command_to_highlight)
            AuraCAD.Console.PrintMessage(
                "\nFollowing some commands to copy. "
    ...
    def load_results(self):
        AuraCAD.Console.PrintMessage("\n")  # because of time print in separate line
        AuraCAD.Console.PrintMessage("CalculiX read results...\n")
        self.results_present = False
        ...
        if os.path.isfile(frd_result_file):
            ...
            for m in self.analysis.Group:
                ...
                if self.solver.AnalysisType == "check":
                    ...
                    AuraCAD.Console.PrintError("FEM: No result object in active Analysis.\n")
        else:
            AuraCAD.Console.PrintError(f"FEM: No frd result file found at {frd_result_file}\n")
    ...
    def load_results_ccxdat(self):
        ...
        if os.path.isfile(dat_result_file):
            ...
            AuraCAD.Console.PrintError(f"FEM: No dat result file found at {dat_result_file}\n")
