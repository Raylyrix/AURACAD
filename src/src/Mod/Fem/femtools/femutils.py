...
# *   Copyright (c) 2022 Uwe Sțhr <uwestoehr@lyx.org>                      *
# *                                                                         *
...
__author__ = "Markus Hovorka, Bernd Hahnebach, Uwe Sțhr"
...
def createObject(doc, name, proxy, viewProxy=None):
    ...
    proxy(obj)
    if AuraCAD.GuiUp and viewProxy is not None:
...
def is_derived_from(obj, t):
    ...
    :note:
     ...
     https://forum.AuraCAD.org/viewtopic.php?f=10&t=32625
    """
...
def get_temp_dir(obj=None):
    ...
    return mkdtemp(prefix="auracadfem_")
...
def get_beside_base(obj):
    auracadstdPath = obj.Document.FileName
    if auracadstdPath == "":
        ...
        error_message = (
            ...
            'is set to "Beside *.auracadStd File". For the moment the tmp dir {} is used.'.format(
                new_path
            )
        )
        AuraCAD.Console.PrintError(f"{error_message}\n")
        # from .errors import MustSaveError
        # raise MustSaveError()
        return new_path
    else:
        return os.path.splitext(auracadstdPath)[0]
...
def make_dir(specific_path):
    try:
        ...
        # beside dir fails on installed auracad examples from start wb
        error_message = (
...
def getBoundBoxOfAllDocumentShapes(doc):
    ...
    # https://forum.AuraCAD.org/viewtopic.php?f=18&t=52920
    for o in doc.Objects:
        ...
        try:
            ...
            AuraCAD.Console.PrintMessage(f"{bb}\n")  # debug only
        except Exception:
            AuraCAD.Console.PrintMessage("exception \n")  # debug only

        if bb is None:
            try:
                AuraCAD.Console.PrintMessage(f"trying: {o.Label}: FemMesh\n")  # debug only
        ...
        if bb:
            ...
            AuraCAD.Console.PrintMessage("no bb\n")  # debug only

    AuraCAD.Console.PrintMessage("overallBB:" + str(overallboundbox) + "\n")  # debug only
...
def getSelectedFace(selectionex):
    ...
    if len(selectionex) != 1:
        AuraCAD.Console.PrintMessage("No or more than one object selected.\n")
    else:
        sel = selectionex[0]
        if len(sel.SubObjects) != 1:
            AuraCAD.Console.PrintMessage("More than one element selected.\n")
        else:
            aFace = sel.SubObjects[0]
            if aFace.ShapeType != "Face":
                AuraCAD.Console.PrintMessage("Not a Face selected.\n")
            else:
                AuraCAD.Console.PrintMessage(":-)\n")
                return aFace
    return aFace
...
def expandParentObject():
    ...
    trees = AuraCADGui.getMainWindow().findChildren(QtGui.QTreeWidget)
    for tree in trees:
...
def getOutputWinColor(type):
    ...
    """
    col_int = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/OutputWindow").GetUnsigned(
        "color" + type
    )
