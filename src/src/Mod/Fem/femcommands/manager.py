...
__title__ = "AuraCAD FEM command base class"
__author__ = "Przemo Firszt, Bernd Hahnebach"
__url__ = "https://www.AuraCAD.org"

## @package manager
#  \ingroup FEM
#  \brief AuraCAD FEM command base class
...
class CommandManager:
    ...
    def IsActive(self):
        if not self.is_active:
            ...
            active = AuraCADGui.ActiveDocument is not None
            ...
            active = (
                ...
                AuraCADGui.ActiveDocument is not None
                and self.result_selected()
            )
        elif self.is_active == "with_vtk_selresult":
            active = self.vtk_result_selected()
        elif self.is_active == "with_part_feature":
            active = AuraCADGui.ActiveDocument is not None and self.part_feature_selected()
        elif self.is_active == "with_femmesh":
            active = AuraCADGui.ActiveDocument is not None and self.femmesh_selected()
        elif self.is_active == "with_gmsh_femmesh":
            active = AuraCADGui.ActiveDocument is not None and self.gmsh_femmesh_selected()
        elif self.is_active == "with_femmesh_andor_res":
            active = (
                AuraCADGui.ActiveDocument is not None and self.with_femmesh_andor_res_selected()
            )
    ...
    def active_analysis_in_active_doc(self):
        ...
        if analysis.Document is AuraCAD.ActiveDocument:
    ...
    # ****************************************************************************************
    # methods to add the objects to the document in AuraCADGui mode

    def add_obj_on_gui_set_edit(self, objtype):
        AuraCAD.ActiveDocument.openTransaction(f"Create Fem{objtype}")
        ...
        AuraCADGui.doCommand(
            ...
            "make{}(AuraCAD.ActiveDocument))".format(objtype)
        )
    ...
    def add_obj_on_gui_noset_edit(self, objtype):
        ...
        # AuraCAD.ActiveDocument.commitTransaction()  # solver command class had this line
    ...
    def add_obj_on_gui_expand_noset_edit(self, objtype):
        ...
        AuraCADGui.doCommand(
            ...
            "make{}(AuraCAD.ActiveDocument))[0]".format(objtype)
        )
        # select only added object
        AuraCADGui.Selection.clearSelection()
        AuraCADGui.doCommand("addedObjDocObj = AuraCAD.ActiveDocument.getObject(addedObj.Name)")
        AuraCADGui.doCommand("AuraCADGui.Selection.addSelection(addedObjDocObj)")
    ...
    def add_obj_on_gui_selobj_set_edit(self, objtype):
        ...
        AuraCADGui.doCommand(
            "ObjectsFem.make{}("
            "AuraCAD.ActiveDocument, AuraCAD.ActiveDocument.{})".format(objtype, self.selobj.Name)
        )
        AuraCADGui.Selection.clearSelection()
        AuraCADGui.doCommand(
            "AuraCADGui.ActiveDocument.setEdit(AuraCAD.ActiveDocument.ActiveObject.Name)"
    ...
    def add_filter_set_edit(self, filtertype):
        ...
        AuraCAD.ActiveDocument.openTransaction(f"Create Fem{filtertype}")
        AuraCADGui.addModule("ObjectsFem")
        AuraCADGui.doCommand(
            "ObjectsFem.make{}("
            "AuraCAD.ActiveDocument, AuraCAD.ActiveDocument.{})".format(filtertype, group.Name)
        )
        # set display and selection style to assure the user sees the new object
        AuraCADGui.doCommand(
            'AuraCAD.ActiveDocument.ActiveObject.ViewObject.DisplayMode = "Surface"'
        )
        AuraCADGui.doCommand(
            'AuraCAD.ActiveDocument.ActiveObject.ViewObject.SelectionStyle = "BoundBox"'
        )
        AuraCADGui.doCommand(
            f"AuraCAD.ActiveDocument.ActiveObject.ViewObject.NoneFieldColor = {self.selobj.ViewObject.NoneFieldColor}"
        )

        # hide selected filter
        AuraCADGui.doCommand(
            "AuraCAD.ActiveDocument.{}.ViewObject.Visibility = False".format(self.selobj.Name)
        )

        # recompute, expand selobj in tree view
        expandParentObject()
        AuraCADGui.doCommand("AuraCAD.ActiveDocument.ActiveObject.recompute()")
