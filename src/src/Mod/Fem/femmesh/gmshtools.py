...
class GmshTools(ObjectTools):
    ...
    def compute(self):
        log_level = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Gmsh").GetString(
            "LogVerbosity", "3"
        )
    ...
    def get_gmsh_command(self):
        self.gmsh_bin = AuraCAD.ParamGet(
    ...
    def get_group_data(self):
        ...
        analysis_group_meshing = AuraCAD.ParamGet(
    ...
    def get_region_data(self):
        ...
        if not mesh_region_list:
            ...
            # https://forum.AuraCAD.org/viewtopic.php?f=18&t=18780&start=40#p149467
            # https://forum.AuraCAD.org/viewtopic.php?f=18&t=18780&p=149520#p149520
            part = self.part_obj
            if (
                mesh_region_list
    ...
    def get_boundary_layer_data(self):
        ...
        if not mesh_boundary_list:
            ...
            if self.part_obj.Shape.ShapeType == "Compound":
                # see https://forum.AuraCAD.org/viewtopic.php?f=18&t=18780&start=40#p149467 and
                # https://forum.AuraCAD.org/viewtopic.php?f=18&t=18780&p=149520#p149520
                self.outputCompoundWarning()
        ...
        if not transfinite_surface_list:
            ...
            for mr_obj in transfinite_surface_list:
                ...
                if mr_obj.UseAutomation and mr_obj.References:
                    ...
                    definition = tft.TauracadurveDefinition.from_tauracadurve_obj(mr_obj)
                    try:
            ...
            for mr_obj in transfinite_volume_list:
                ...
                if mr_obj.References:
                    ...
                    if mr_obj.UseAutomation:
                        ...
                        curve_definition = tft.TauracadurveDefinition.from_tauracadurve_obj(mr_obj)
    ...
    def write_groups(self, geo):
        ...
        # we use the element index of AuraCAD which starts with 1 (example: "Face1"),
        ...
        if len(self.bl_setting_list):
            ...
            for item in self.bl_setting_list:
                ...
                for k in item:
                    v = item[k]
                    if k == "CurvesList":
                        # the element name of AuraCAD which starts
    ...
    def write_part_file(self):
        ...
        # get partner shape
        geom_trans = geom.transformed(AuraCAD.Placement().Matrix)
    ...
    def write_geo(self):
        ...
        geo.write("// geo file for meshing with Gmsh meshing software created by AuraCAD\n")
        geo.write("\n")
        ...
        if self.ParallelProcessing and self.algorithm2D != "7":
            cpu_count = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Gmsh").GetInt(
        ...
        # see https://forum.AuraCAD.org/viewtopic.php?f=18&t=21571&start=20#p179443
    ...
    def run_gmsh_with_geo(self):
        ...
        # https://forum.AuraCAD.org/viewtopic.php?f=18&t=33946
        useless_warning = (
...
for len in max_mesh_sizes:
    ...
    gm.get_tmp_file_paths("/tmp/auracadgm_" + str(len), True)
    gm.get_gmsh_command()
...
class GmshPreviewTools(GmshTools):
    ...
    def write_preview_geo(self):
        ...
        factor = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Gmsh").GetInt(
            "previewMeshFactor", 5
        )
