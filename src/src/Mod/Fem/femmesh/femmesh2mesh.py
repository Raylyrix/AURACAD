...
the_file = join(AuraCAD.getResourceDir(), "examples", "FEMExample.auracadStd")
AuraCAD_file = AuraCAD.openDocument(the_file)
fem_mesh = AuraCAD_file.getObject("FEMMeshGmsh").FemMesh
result = AuraCAD_file.getObject("CCX_Results")
...
# The key is the face number, used internally by AuraCAD.
...
def femmesh_2_mesh(myFemMesh, myResults=None, myDispScale=1):
    ...
    while actFaceIdx < allFaces:
        if actFaceIdx < (allFaces - 1):
            ...
            AuraCAD.Console.PrintMessage(f"Found a last Face: {faceCodeList[actFaceIdx]}\n")
            singleFaces.append(faceCodeList[actFaceIdx])
            actFaceIdx += 1

    output_mesh = []
    if myResults:
        AuraCAD.Console.PrintMessage(f"{myResults.Name}\n")
    ...
    AuraCAD.Console.PrintMessage(f"Mesh by surface search method: {end_time - start_time}\n")
...
def mesh_2_femmesh(myFemMesh, singleFaces, faceCodeDict):
    ...
    obj = AuraCAD.ActiveDocument.addObject("Fem::FemMeshObject", "Mesh2Fem")
    obj.FemMesh = femmesh
    end_time = time.process_time()
    AuraCAD.Console.PrintMessage(f"Convert to FemMesh: {end_time - start_time}\n")
    return obj
