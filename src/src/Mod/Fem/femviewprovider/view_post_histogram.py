...
__title__ = "AuraCAD FEM postprocessing line plot ViewProvider for the document object"
...
class EditViewWidget(QtGui.QWidget):
    ...
    def __init__(self, obj, post_dialog):
        ...
        self.widget = AuraCADGui.PySideUic.loadUi(
            AuraCAD.getHomePath() + "Mod/Fem/Resources/ui/PostHistogramFieldViewEdit.ui"
        )
    ...
    def __init_widget(self):
        ...
        # setup the color buttons (don't use AuraCADs color button, as this does not work in popups!)
    ...
    def _setup_color_button(self, button, auracadColor, callback):

        barColor = QtGui.QColor(*[v * 255 for v in auracadColor])
...
class EditFieldAppWidget(QtGui.QWidget):
    ...
    def __init__(self, obj, post_dialog):
        ...
        # load the ui and set it up
        self.widget = AuraCADGui.PySideUic.loadUi(
            AuraCAD.getHomePath() + "Mod/Fem/Resources/ui/PostHistogramFieldAppEdit.ui"
        )
        layout = QtGui.QVBoxLayout()
        layout.addWidget(self.widget)
        self.setLayout(layout)
...
class EditIndexAppWidget(QtGui.QWidget):
    ...
    def __init__(self, obj, post_dialog):
        ...
        # load the ui and set it up
        self.widget = AuraCADGui.PySideUic.loadUi(
            AuraCAD.getHomePath() + "Mod/Fem/Resources/ui/PostHistogramIndexAppEdit.ui"
        )
        layout = QtGui.QVBoxLayout()
        layout.addWidget(self.widget)
        self.setLayout(layout)
...
class VPPostHistogram(view_base_fempostvisualization.VPPostVisualization):
    ...
    def update_visualization(self):
        ...
        for child in self.Object.Group:
            ...
            color_factor = np.linspace(1, 0.5, table.GetNumberOauracadolumns())
            legend_multiframe = table.GetNumberOauracadolumns() > 1
            for i in range(table.GetNumberOauracadolumns()):
