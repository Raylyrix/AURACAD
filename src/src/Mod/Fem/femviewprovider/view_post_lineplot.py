...
class EditViewWidget(QtGui.QWidget):
    ...
    def __init__(self, obj, post_dialog):
        ...
        self.widget = AuraCADGui.PySideUic.loadUi(
            AuraCAD.getHomePath() + "Mod/Fem/Resources/ui/PostLineplotFieldViewEdit.ui"
        )
...
class EditFieldAppWidget(QtGui.QWidget):
    ...
    def __init__(self, obj, post_dialog):
        ...
        # load the ui and set it up
        self.widget = AuraCADGui.PySideUic.loadUi(
            AuraCAD.getHomePath() + "Mod/Fem/Resources/ui/PostLineplotFieldAppEdit.ui"
        )
        layout = QtGui.QVBoxLayout()
        layout.addWidget(self.widget)
        self.setLayout(layout)
...
class EditIndexAppWidget(QtGui.QWidget):
    ...
    def __init__(self, obj, post_dialog):
        ...
        # load the ui and set it up
        self.widget = AuraCADGui.PySideUic.loadUi(
            AuraCAD.getHomePath() + "Mod/Fem/Resources/ui/PostLineplotIndexAppEdit.ui"
        )
        layout = QtGui.QVBoxLayout()
        layout.addWidget(self.widget)
        self.setLayout(layout)
...
class VPPostLineplot(view_base_fempostvisualization.VPPostVisualization):
    ...
    def update_visualization(self):
        ...
        for child in self.Object.Group:
            ...
            color_factor = np.linspace(1, 0.5, int(table.GetNumberOauracadolumns() / 2))
            legend_multiframe = table.GetNumberOauracadolumns() > 2

            for i in range(0, table.GetNumberOauracadolumns(), 2):
