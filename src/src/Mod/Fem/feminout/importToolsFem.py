...
__title__ = "AuraCAD FEM import tools"
__author__ = "Bernd Hahnebach"
__url__ = "https://www.AuraCAD.org"

## @package importToolsFem
#  \ingroup FEM
#  \brief AuraCAD FEM import tools
...
def get_FemMeshObjectElementTypes(fem_mesh_obj, remove_zero_element_entries=True):
    ...
    elements_list_with_zero = [
        ...
        for (s, d) in AuraCAD_element_names_dims.items()
    ]
    # ugly but necessary
...
def make_femmesh(mesh_data):
    """makes an AuraCAD FEM Mesh object from FEM Mesh data"""
    import Fem

    mesh = Fem.FemMesh()
    m = mesh_data
    if ("Nodes" in m) and (len(m["Nodes"]) > 0):
        AuraCAD.Console.PrintLog("Found: nodes\n")
        if (
            ("Seg2Elem" in m)
            or ("Seg3Elem" in m)
            or ("Tria3Elem" in m)
            ...
            nds = m["Nodes"]
            AuraCAD.Console.PrintLog("Found: elements\n")
            for i in nds:
                n = nds[i]
...
def fill_femresult_mechanical(res_obj, result_set):
    """fills a AuraCAD FEM mechanical result object with result data"""
