...
__title__ = "AuraCAD Fenics XDMF mesh writer"
__author__ = "Johannes Hartung"
__url__ = "https://www.AuraCAD.org"

## @package exportFenicsXDMF
#  \ingroup FEM
#  \brief AuraCAD Fenics Mesh XDMF writer for FEM workbench
...
AuraCAD_Group_Dimensions = {"Vertex": 0, "Edge": 1, "Face": 2, "Volume": 3}

AuraCAD_to_Fenics_XDMF_dict = {
    ("Node", 1): ("Polyvertex", 1),
    ("Edge", 1): ("Polyline", 2),
    ("Edge", 2): ("Edge_3", 3),
...
def write_fenics_mesh_codim_xdmf(
    ...
    """
    nodeindices = [(
        nodes_dict[ind] for ind in fem_mesh_obj.FemMesh.getElementNodes(AuraCAD_volume_ind)
    ) for (fen_ind, AuraCAD_volume_ind) in enumerate(AuraCAD_cells)]
    """
    ...
    topology_type, nodes_per_element = AuraCAD_to_Fenics_XDMF_dict[(name_topo, element_order)]
    ...
    if dim_topo == 3:
        AuraCAD_topo = fem_mesh_obj.FemMesh.Volumes
    elif dim_topo == 2:
        AuraCAD_topo = fem_mesh_obj.FemMesh.Faces
    elif dim_topo == 1:
        AuraCAD_topo = fem_mesh_obj.FemMesh.Edges
    elif dim_topo == 0:
        AuraCAD_topo = fem_mesh_obj.FemMesh.Nodes
    else:
        AuraCAD_topo = []
        Console.PrintError(
    ...
    nodeindices = [
        (nodes_dict[ind] for ind in fem_mesh_obj.FemMesh.getElementNodes(AuraCAD_topo_ind))
        for (fen_ind, AuraCAD_topo_ind) in enumerate(AuraCAD_topo)
    ]
...
def write_fenics_mesh_xdmf(fem_mesh_obj, outputfile, group_values_dict={}, encoding=ENCODING_ASCII):
    ...
    num_cells, cellname_auracad, dim_cell = celltype_in_mesh
    ...
    for g in gmshgroups:
        ...
        mesh_function_codim = dim_cell - AuraCAD_Group_Dimensions[mesh_function_type]
