...
#  \ingroup FEM
#  \brief AuraCAD Calculix FRD Reader for FEM workbench
...
def setupPipeline(doc, analysis, results_name, result_data):
    ...
    if not "BUILD_FEM_VTK" in AuraCAD.__cmake__:
        return
        ...
        if len(m["Results"]) > 0:
            for result_set in m["Results"]:
                ...
                if not res_obj.MassFlowRate:
                    ...
                    # https://www.AuraCAD.org/tracker/view.php?id=2873
                    # information 2:
                    ...
                    # example frd file: https://forum.AuraCAD.org/viewtopic.php?t=32649#p274291
            ...
            if number_of_increments > 1:
                ...
                match result_analysis_type:
                    case "frequency":
                        unit = AuraCAD.Units.Frequency
                        description = "Eigenmode"
                    case "buckling":
                        unit = AuraCAD.Units.Unit()
                        description = "Buckling factor"
                    case "thermomech":
                        unit = AuraCAD.Units.TimeSpan
...
def read_frd_result(frd_input):
    ...
    for line in frd_file:
        ...
        if nodes_found and (line[1:3] == "-1"):
            ...
            nodes[elem] = AuraCAD.Vector(nodes_x, nodes_y, nodes_z)
        ...
        if elements_found and (line[1:3] == "-2"):
            ...
            if elemType == 1:
                ...
                # first line
                # C3D15 Calculix --> penta15 AuraCAD
                ...
                # C3D10 Calculix --> tetra10 AuraCAD
                ...
                # B31 CalculiX --> seg2 AuraCAD
                # N1, N2
                nd1 = int(line[3:13])
                nd2 = int(line[13:23])
                elements_seg2[elem] = (nd1, nd2)
            elif elemType == 12:
                # B32 CalculiX --> seg3 AuraCAD
        ...
        if mode_disp_found and (line[1:3] == "-1"):
            ...
            mode_disp[elem] = AuraCAD.Vector(mode_disp_x, mode_disp_y, mode_disp_z)
        ...
        if mode_stress_found and (line[1:3] == "-1"):
            ...
            # AuraCAD:            (Sxx, Syy, Szz, Sxy, Sxz, Syz)
        ...
        if mode_strain_found and (line[1:3] == "-1"):
            ...
            # AuraCAD:            (Exx, Eyy, Ezz, Exy, Exz, Eyz)
            # thus exchange the last two entries
        ...
        if mode_heatflux_found and (line[1:3] == "-1"):
            ...
            mode_heatflux[elem] = AuraCAD.Vector(mode_heatflux_x, mode_heatflux_y, mode_heatflux_z)
        ...
        if (
            ...
            mode_results = {}
            # https://forum.AuraCAD.org/viewtopic.php?f=18&t=32649&start=10#p274686
            mode_results["number"] = float("NaN")
            mode_results["time"] = float("NaN")
            end_of_section_found = False
