...
__title__ = "AuraCAD Fenics XML mesh writer"
__author__ = "Johannes Hartung"
__url__ = "https://www.AuraCAD.org"

## @package exportFenicsXML
#  \ingroup FEM
#  \brief AuraCAD Fenics Mesh XML writer for FEM workbench
...
def write_fenics_mesh_xml(fem_mesh_obj, outputfile):
    ...
    cellname_fenics = AuraCAD_to_Fenics_dict[cellname_auracad]
    ...
    for nodeind, AuraCAD_vec in list(fem_mesh_obj.FemMesh.Nodes.items()):
        ET.SubElement(
            vertices,
            "vertex",
            index=str(nodeind - 1),
            # auracad starts from 1, fenics starts from 0 to size-1
            x=str(AuraCAD_vec[0]),
            y=str(AuraCAD_vec[1]),
            z=str(AuraCAD_vec[2]),
        )
    ...
    if dim_cell == 3:
        AuraCAD_cells = fem_mesh_obj.FemMesh.Volumes
    elif dim_cell == 2:
        AuraCAD_cells = fem_mesh_obj.FemMesh.Faces
    elif dim_cell == 1:
        AuraCAD_cells = fem_mesh_obj.FemMesh.Edges
    else:
        AuraCAD_cells = ()

    for fen_ind, AuraCAD_volume_ind in enumerate(AuraCAD_cells):
        # auracad starts after all other entities, fenics start from 0 to size-1
        nodeindices = fem_mesh_obj.FemMesh.getElementNodes(AuraCAD_volume_ind)
