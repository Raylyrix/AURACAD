...
__title__ = "AuraCAD FEM postprocessing view for summarizing extractor links"
...
def build_new_visualization_tree_model():
    ...
    for vis_name in visualizations:
        vis_icon = AuraCADGui.getIcon(visualizations[vis_name].icon)
        ...
        for ext in visualizations[vis_name].extractions:
            icon = AuraCADGui.getIcon(ext.icon)
...
class _ElideToolButton(QtGui.QToolButton):
    ...
    def paintEvent(self, event):
        ...
        txt_min = fm.boundingRect("â€¦").width()
...
class _SummaryWidget(QtGui.QWidget):
    ...
    def __init__(self, st_object, extractor, post_dialog):
        ...
        self.rmButton.setIcon(AuraCADGui.getIcon("delete.svg"))
    ...
    def showVisualization(self):
        if vis.is_visualization_object(self._st_object):
            ...
            AuraCADGui.Selection.addSelection(self._st_object)
...
class ExtractLinkView(QtGui.QWidget):
    ...
    def newVisualization(self, vis_data, ext_data):
        ...
        AuraCADGui.addModule(vis_data.module)
        AuraCADGui.addModule(ext_data.module)
        AuraCADGui.addModule("FemGui")

        # create visualization
        AuraCADGui.doCommand(
            f"visualization = {vis_data.module}.{vis_data.factory}(AuraCAD.ActiveDocument)"
        )

        analysis = self._find_parent_analysis(self._object)
        if analysis:
            AuraCADGui.doCommand(f"AuraCAD.ActiveDocument.{analysis.Name}.addObject(visualization)")

        # create extraction and add it
        AuraCADGui.doCommand(
            f"extraction = {ext_data.module}.{ext_data.factory}(AuraCAD.ActiveDocument)"
        )
        AuraCADGui.doCommand(f"extraction.Source = AuraCAD.ActiveDocument.{self._object.Name}")
        # default values: color
        color_prop = AuraCADGui.ActiveDocument.ActiveObject.Proxy.get_default_color_property()
        if color_prop:
            AuraCADGui.doCommand(
                f"extraction.ViewObject.{color_prop} = visualization.ViewObject.Proxy.get_next_default_color()"
            )

        AuraCADGui.doCommand(f"visualization.addObject(extraction)")
    ...
    def addExtractionToVisualization(self, vis_obj, ext_data):
        ...
        AuraCADGui.doCommand(f"App.ActiveDocument.{vis_obj.Name}.addObject(extraction)")
    ...
    def addExtractionToPostObject(self, post_obj, ext_data):
        ...
        AuraCADGui.doCommand(f"extraction.Source = AuraCAD.ActiveDocument.{post_obj.Name}")
        ...
        AuraCADGui.doCommand(f"App.ActiveDocument.{self._object.Name}.addObject(extraction)")
