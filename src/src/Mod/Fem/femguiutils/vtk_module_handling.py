...
AuraCAD is linked with VTK libraries during its build process. To use the VTK
python module and pass objects between python and c++ the compiled module library
needs to be linked to the exact same vtk library as AuraCAD is. This is ensured by
...
set of VTK libs, and hence object passing in AuraCAD fails. (Note: import and
...
# Note: This file is imported from AuraCAD App files. Do not import any AuraCADGui
...
def vtk_module_compatible():
    # checks if the VTK library AuraCAD is build against is the one used by
    # the python module
...
# If AuraCAD is build with VTK python support this function checks if the
...
def vtk_module_handling():
    ...
    if not "BUILD_FEM_VTK_PYTHON" in AuraCAD.__cmake__:
        # no VTK python api support in AuraCAD
        return
    ...
    if not vtk_module_compatible():
        ...
        message = translate(
            "FEM",
            (
                "AuraCAD is linked to a different VTK library than the imported "
            ...
            if not loaded:
                ...
                message += translate(
                    ...
                    (
                        ...
                        "Restart AuraCAD to get the option for loading this module."
                    ),
                )
