...
__title__ = "AuraCAD visualization registry"
...
class _VisualizationCommand:
    ...
    def Activated(self):
        ...
        vis = _registry[self._visualization_type]
        AuraCAD.ActiveDocument.openTransaction(f"Create {vis.name}")

        AuraCADGui.addModule(vis.module)
        AuraCADGui.addModule("FemGui")

        AuraCADGui.doCommand(f"obj = {vis.module}.{vis.factory}(AuraCAD.ActiveDocument)")
        AuraCADGui.doCommand(f"FemGui.getActiveAnalysis().addObject(obj)")

        AuraCADGui.Selection.clearSelection()
        AuraCADGui.doCommand("AuraCADGui.ActiveDocument.setEdit(obj)")
...
def setup_commands(toplevel_name):
    ...
    for vis in _registry:
        AuraCADGui.addCommand(_to_command_name(vis), _VisualizationCommand(vis))

    # build the group command!
    AuraCADGui.addCommand("FEM_PostVisualization", _VisualizationGroupCommand())
