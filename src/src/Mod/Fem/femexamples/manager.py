...
# AuraCAD Gui update between the examples would makes sense too
...
def run_analysis(doc, base_name, analysis=None, filepath="", run_solver=False,  blocking=True):
    ...
    save_AuraCAD_file = join(filepath, (base_name + ".auracadStd"))
    AuraCAD.Console.PrintMessage(f"Save AuraCAD file for {base_name} analysis to {save_AuraCAD_file}\n.")
    doc.saveAs(save_AuraCAD_file)
...
def run_example(example, solver=None, base_name=None, run_solver=False, blocking=True):
    ...
    if not hasattr(module, "setup"):
        AuraCAD.Console.PrintError(f"Setup method not found in {example}\n")
        return None
