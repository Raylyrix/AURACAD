...
def reset_mesh_deformation(resultobj):
    ...
    resultobj : Fem::ResultMechanical
        AuraCAD FEM mechanical result object
    """
...
def show_result(resultobj, result_type="Sabs", limit=None):
    ...
    if resultobj:
        ...
        AuraCAD.Console.PrintError("Error, No result object given.\n")
...
def get_all_stats(res_obj):
    ...
    the methods _getAuraCADMechResultVectorProperties()
    and _getAuraCADMechResultScalarProperties()
    as well as forum topic
    https://forum.AuraCAD.org/viewtopic.php?f=18&t=33106&start=30#p277434
...
def fill_femresult_stats(res_obj):
    """Fills a AuraCAD FEM mechanical result object with stats data.
    ...
    AuraCAD.Console.PrintLog("Calculate stats list for result obj: " + res_obj.Name + "\n")
    ...
    AuraCAD.Console.PrintLog("Stats list for result obj: " + res_obj.Name + " calculated\n")
    return res_obj


def add_disp_apps(res_obj):
    res_obj.DisplacementLengths = calculate_disp_abs(res_obj.DisplacementVectors)
    AuraCAD.Console.PrintLog("Added DisplacementLengths.\n")
    return res_obj
...
def add_von_mises(res_obj):
    ...
    AuraCAD.Console.PrintLog("Added von Mises stress.\n")
    return res_obj
...
def add_principal_stress_std(res_obj):
    ...
    # https://forum.AuraCAD.org/viewtopic.php?f=18&t=33106&p=416006#p416006
    ...
    AuraCAD.Console.PrintLog("Added standard principal stresses and max shear values.\n")

    #
    # Add critical strain ratio using the Stress Modified Critical Strain (SMCS) criterion
    #   Forum Discussion: https://forum.AuraCAD.org/viewtopic.php?f=18&t=35893#p303392
    ...
    MatMechNon = AuraCAD.ActiveDocument.getObject("MaterialMechanicalNonlinear")
    if MatMechNon:
...
def get_concrete_nodes(res_obj):
    ...
    for obj in res_obj.getParentGroup().Group:
        if obj.isDerivedFrom("App::MaterialObjectPython") and is_of_type(
            ...
            AuraCAD.Console.PrintMessage("ReinforcedMaterial\n")
            if obj.References == []:
            ...
            AuraCAD.Console.PrintMessage("No ReinforcedMaterial\n")
            if obj.References == []:
                for iic in range(nsr):
                    if ic[iic] == 0:
                        ic[iic] = 2
...
def add_principal_stress_reinforced(res_obj):
    ...
    for obj in res_obj.getParentGroup().Group:
        if is_of_type(obj, "Fem::MaterialReinforced"):
            matrix_af = float(
                AuraCAD.Units.Quantity(obj.Material["AngleOfFriction"]).getValueAs("rad")
            )
            matrix_cs = float(
                AuraCAD.Units.Quantity(obj.Material["CompressiveStrength"]).getValueAs("MPa")
            )
            reinforce_yield = float(
                AuraCAD.Units.Quantity(obj.Reinforcement["YieldStrength"]).getValueAs("MPa")
            )
    # print(matrix_af)
    # print(matrix_cs)
...
def compact_result(res_obj):
    ...
    """
    # as workaround for https://www.AuraCAD.org/tracker/view.php?id=2873
    ...
    # AuraCAD result obj does not support elem results ATM
...
def calculate_von_mises(stress_tensor):
    ...
    Simplification: https://forum.AuraCAD.org/viewtopic.php?f=18&t=33974&p=296542#p296542
...
def calculate_principal_stress_std(stress_tensor):
    ...
    # https://forum.AuraCAD.org/viewtopic.php?f=22&t=33911&start=10#p284229
    # https://forum.AuraCAD.org/viewtopic.php?f=18&t=32649#p274291
    ...
    )  # https://forum.AuraCAD.org/viewtopic.php?f=18&t=24637&start=10#p240408
...
def calculate_principal_stress_reinforced(stress_tensor):
    ...
    https://forum.AuraCAD.org/viewtopic.php?f=18&t=33106&start=90#p296539
    """
...
def calculate_rho(stress_tensor, fy):
    ...
    See post:
    https://forum.AuraCAD.org/viewtopic.php?f=18&t=28821
    ...
    # Solution (9)
    if sxx != 0.0:
        auracad = sxz * sxy / sxx - syz
        fxy = sxy**2 / sxx
        fxz = sxz**2 / sxx

        # Solution (9+)
        rhoy[3] = syy - fxy + auracad
        rhoy[3] /= fy
        rhoz[3] = szz - fxz + auracad
        rhoz[3] /= fy

        # Solution (9-)
        rhoy[4] = syy - fxy - auracad
        rhoy[4] /= fy
        rhoz[4] = szz - fxz - auracad
        rhoz[4] /= fy

    # Solution (10)
    if syy != 0.0:
        auracad = syz * sxy / syy - sxz
        fxy = sxy**2 / syy
        fyz = syz**2 / syy

        # Solution (10+)
        rhox[5] = sxx - fxy + auracad
        rhox[5] /= fy
        rhoz[5] = szz - fyz + auracad
        rhoz[5] /= fy

        # Solution (10-)vm
        rhox[6] = sxx - fxy - auracad

        rhox[6] /= fy
        rhoz[6] = szz - fyz - auracad
        rhoz[6] /= fy

    # Solution (11)
    if szz != 0.0:
        auracad = sxz * syz / szz - sxy
        fxz = sxz**2 / szz
        fyz = syz**2 / szz

        # Solution (11+)
        rhox[7] = sxx - fxz + auracad
        rhox[7] /= fy
        rhoy[7] = syy - fyz + auracad
        rhoy[7] /= fy

        # Solution (11-)
        rhox[8] = sxx - fxz - auracad
        rhox[8] /= fy
        rhoy[8] = syy - fyz - auracad
        rhoy[8] /= fy
...
def calculate_mohr_coulomb(prin1, prin3, phi, auracadk):
    ...
    - auracadk: factored compressive strength of the matrix material (usually concrete)
    """

    coh = auracadk * (1 - np.sin(phi)) / 2 / np.cos(phi)
...
def calculate_disp_abs(displacements):
    # see https://forum.AuraCAD.org/viewtopic.php?f=18&t=33106&start=100#p296657
