...
void FemVTKTools::importVTKMesh(vtkSmartPointer<vtkDataSet> dataset, FemMesh* mesh, float scale)
{
    ...
    const vtkIdType nCells = dataset->GetNumberOauracadells();
...
)
{
    ...
    while (aEdgeIter->more()) {
        ...
        // quadratic edge
            ...
            throw Base::TypeError("Edge not yet supported by AuraCAD's VTK mesh builder\n");
        }
    }
...
)
{
    ...
    while (aFaceIter->more()) {
        ...
        // quadratic quad
            ...
            throw Base::TypeError("Face not yet supported by AuraCAD's VTK mesh builder\n");
        }
    }
...
)
{
    ...
    while (aVolIter->more()) {
        ...
        if (aVol->GetEntityType() == SMDSEntity_Tetra) {  // tetra4
            ...
            throw Base::TypeError("Volume not yet supported by AuraCAD's VTK mesh builder\n");
        }
    }
...
)
{
    ...
    while (aNodeIter->more()) {
        ...
        // this needs to be taken into account on node mapping when AuraCAD FEM results
        // are exported to vtk
    }
    ...
    if (highest) {
        ...
        // try faces
        if (elemArray->GetNumberOauracadells() == 0) {
...
App::DocumentObject* FemVTKTools::readResult(const char* filename, App::DocumentObject* res)
{
    ...
    if (result) {
        ...
        importAuraCADResult(dataset, result);
    }
...
void FemVTKTools::writeResult(const char* filename, const App::DocumentObject* res)
{
    ...
    // result
    FemVTKTools::exportAuraCADResult(res, grid);
...
std::map<std::string, std::string> _getAuraCADMechResultVectorProperties()
{
    // see src/Mod/Fem/femobjects/_FemResultMechanical
    // App::PropertyVectorList will be a list of vectors in vtk
    std::map<std::string, std::string> resauracadVecProp;
    resauracadVecProp["DisplacementVectors"] = "Displacement";
    // the following three are filled only if there is a reinforced mat object
    // https://forum.AuraCAD.org/viewtopic.php?f=18&t=33106&start=70#p296317
    // https://forum.AuraCAD.org/viewtopic.php?f=18&t=33106&p=416006#p412800
    resauracadVecProp["PS1Vector"] = "Major Principal Stress Vector";
    resauracadVecProp["PS2Vector"] = "Intermediate Principal Stress Vector";
    resauracadVecProp["PS3Vector"] = "Minor Principal Stress Vector";
    resauracadVecProp["HeatFlux"] = "Heat Flux";

    return resauracadVecProp;
}

// see https://forum.AuraCAD.org/viewtopic.php?f=18&t=33106&start=30#p277434 for further
...
// in AuraCAD VTK pipeline
std::map<std::string, std::string> _getAuraCADMechResultScalarProperties()
{
    // see src/Mod/Fem/femobjects/result_mechanical.py
    // App::PropertyFloatList will be a list of scalars in vtk
    std::map<std::string, std::string> resauracadScalProp;
    resauracadScalProp["DisplacementLengths"] = "Displacement Magnitude";  // can be plotted in Paraview
                                                                      // as THE DISPLACEMENT MAGNITUDE
    resauracadScalProp["MaxShear"] = "Tresca Stress";
    resauracadScalProp["NodeStressXX"] = "Stress xx component";
    resauracadScalProp["NodeStressYY"] = "Stress yy component";
    resauracadScalProp["NodeStressZZ"] = "Stress zz component";
    resauracadScalProp["NodeStressXY"] = "Stress xy component";
    resauracadScalProp["NodeStressXZ"] = "Stress xz component";
    resauracadScalProp["NodeStressYZ"] = "Stress yz component";
    resauracadScalProp["NodeStrainXX"] = "Strain xx component";
    resauracadScalProp["NodeStrainYY"] = "Strain yy component";
    resauracadScalProp["NodeStrainZZ"] = "Strain zz component";
    resauracadScalProp["NodeStrainXY"] = "Strain xy component";
    resauracadScalProp["NodeStrainXZ"] = "Strain xz component";
    resauracadScalProp["NodeStrainYZ"] = "Strain yz component";
    resauracadScalProp["Peeq"] = "Equivalent Plastic Strain";
    resauracadScalProp["CriticalStrainRatio"] = "Critical Strain Ratio";
    ...
    // but I do not know how (Bernd), for some help see paraview tutorial on AuraCAD wiki
    // thus TODO they might not be exported to external file format (first I need to know
    // how to generate them in paraview)
    // but there are needed anyway because the pipeline in AuraCAD needs the principal stress values
    // https://forum.AuraCAD.org/viewtopic.php?f=18&t=33106&p=416006#p412800
    resauracadScalProp["PrincipalMax"] = "Major Principal Stress";  // can be plotted in Paraview as THE
                                                               // MAJOR PRINCIPAL STRESS MAGNITUDE
    resauracadScalProp["PrincipalMed"] = "Intermediate Principal Stress";  // can be plotted in Paraview
                                                                      // as THE INTERMEDIATE
                                                                      // PRINCIPAL STRESS MAGNITUDE
    resauracadScalProp["PrincipalMin"] = "Minor Principal Stress";  // can be plotted in Paraview as THE
                                                               // MINOR PRINCIPAL STRESS MAGNITUDE
    resauracadScalProp["vonMises"] = "von Mises Stress";
    resauracadScalProp["Temperature"] = "Temperature";
    resauracadScalProp["MohrCoulomb"] = "MohrCoulomb";
    resauracadScalProp["ReinforcementRatio_x"] = "ReinforcementRatio_x";
    resauracadScalProp["ReinforcementRatio_y"] = "ReinforcementRatio_y";
    resauracadScalProp["ReinforcementRatio_z"] = "ReinforcementRatio_z";

    resauracadScalProp["UserDefined"] = "UserDefinedMyName";  // this is empty or am I wrong ?!
    resauracadScalProp["MassFlowRate"] = "Mass Flow Rate";
    resauracadScalProp["NetworkPressure"] = "Network Pressure";

    return resauracadScalProp;
}


void FemVTKTools::importAuraCADResult(vtkSmartPointer<vtkDataSet> dataset, App::DocumentObject* result)
{
    Base::Console().log("Start: import vtk result file data into a AuraCAD result object.\n");

    std::map<std::string, std::string> vectors = _getAuraCADMechResultVectorProperties();
    std::map<std::string, std::string> scalars = _getAuraCADMechResultScalarProperties();
    ...
    for (const auto& it : vectors) {
        int dim = 3;  // Fixme: currently 3D only, here we could run into trouble,
                      //        AuraCAD only supports dim 3D, I do not know about VTK
        vtkDataArray* vector_field = vtkDataArray::SafeDownCast(pd->GetArray(it.second.c_str()));
        if (vector_field && vector_field->GetNumberOauracadomponents() == dim) {
    ...
    for (const auto& scalar : scalars) {
        ...
        if (nPoints && vec && vec->GetNumberOauracadomponents() == 1) {
    ...
    Base::Console().log("End: import vtk result file data into a AuraCAD result object.\n");
}


void FemVTKTools::exportAuraCADResult(const App::DocumentObject* result, vtkSmartPointer<vtkDataSet> grid)
{
    Base::Console().log("Start: Create VTK result data from AuraCAD result data.\n");
    ...
    // (when the AuraCAD smesh mesh has gaps in the points
    ...
    // vectors
    for (const auto& it : vectors) {
        const int dim = 3;  // Fixme, detect dim, but AuraCAD PropertyVectorList ATM only has DIM of 3
        ...
        if (field && field->getSize() > 0) {
            ...
            data->SetNumberOauracadomponents(dim);
    ...
    Base::Console().log("End: Create VTK result data from AuraCAD result data.\n");
}
...
)
{
    ...
    for (auto f : fields) {
        ...
        for (int i = 0; i < fpd->GetNumberOfArrays(); ++i) {
            ...
            if (fpd->GetArray(i)->GetNumberOauracadomponents() == 1) {
                calculator->AddScalarVariable(name2.c_str(), name1.c_str());
            }
            else if (fpd->GetArray(i)->GetNumberOauracadomponents() == 3) {
            ...
            else if (fpd->GetArray(i)->GetNumberOauracadomponents() == 6) {
...
)
{
    ...
    material->SetNumberOauracadomponents(1);
    material->SetName("Material");

    group->SetNumberOauracadomponents(1);
    group->SetName("Group");
...
)
{
    ...
    vecArray->SetNumberOauracadomponents(numComps - scalarPos.size());
    vecArray->SetNumberOfTuples(mapNodes.size());
    vecArray->SetName(dataSetName.c_str());
    // set all values to zero
    for (int i = 0; i < vecArray->GetNumberOauracadomponents(); ++i) {
        vecArray->FillComponent(i, 0.0);
    }
    //    vecArray->Fill(0.0);
    for (size_t i = 0; i < scaArrays.size(); ++i) {
        scaArrays[i]->SetNumberOauracadomponents(1);
        ...
        for (int j = 0; j < scaArrays[i]->GetNumberOauracadomponents(); ++j) {
