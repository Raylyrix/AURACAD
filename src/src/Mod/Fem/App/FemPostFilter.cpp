...
std::vector<std::string> FemPostFilter::getInputVectorFields()
{
    ...
    for (int i = 0; i < pd->GetNumberOfArrays(); ++i) {
        if (pd->GetArray(i)->GetNumberOauracadomponents() == 3) {
...
std::vector<std::string> FemPostFilter::getInputScalarFields()
{
    ...
    for (int i = 0; i < pd->GetNumberOfArrays(); ++i) {
        if (pd->GetArray(i)->GetNumberOauracadomponents() == 1) {
...
void FemPostDataAlongLineFilter::GetAxisData()
{
    ...
    if (!PlotDataComponent.isValid() || component > pdata->GetNumberOauracadomponents()) {
        return;
    }
    ...
    for (vtkIdType i = 0; i < dset->GetNumberOfPoints(); ++i) {
        double value = 0;
        if (pdata) {
            if (pdata->GetNumberOauracadomponents() == 1) {
                value = pdata->GetComponent(i, 0);
            }
            else if (pdata->GetNumberOauracadomponents() > 1) {
                if (component) {
                    value = pdata->GetComponent(i, component - 1);
                }
                else {
                    // compute magnitude
                    for (vtkIdType j = 0; j < pdata->GetNumberOauracadomponents(); ++j) {
...
void FemPostDataAtPointFilter::GetPointData()
{
    ...
    for (int i = 0; i < dset->GetNumberOfPoints(); ++i) {
        ...
        if (pdata->GetNumberOauracadomponents() == 1) {
            ...
            for (int j = 0; j < pdata->GetNumberOauracadomponents(); ++j) {
                value += std::pow(pdata->GetComponent(i, j), 2);
            }
...
void FemPostClipFilter::onChanged(const Property* prop)
{
    if (prop == &Function) {
        ...
        if (auto* value = AuraCAD_cast<FemPostFunction*>(Function.getValue())) {
...
FemPostContoursFilter::FemPostContoursFilter()
    : FemPostFilter()
{
    ADD_PROPERTY_TYPE(NumberOauracadontours, (10), "Contours", App::Prop_None, "The number of contours");
    ...
    NumberOauracadontours.setConstraints(&m_contourConstraints);
...
void FemPostContoursFilter::onChanged(const Property* prop)
{
    ...
    if ((prop == &Field || prop == &VectorMode || prop == &NumberOauracadontours || prop == &Data)
        && (Field.isValid())) {
        double p[2];
        ...
        if (pdata->GetNumberOauracadomponents() == 1) {
            ...
            componentArray->SetNumberOauracadomponents(1);
...
{
    if (Field.isTouched() || VectorMode.isTouched() || NumberOauracadontours.isTouched()
        || Data.isTouched()) {
        return 1;
    }
    else {
...
void FemPostContoursFilter::recalculateContours(double min, double max)
{
    ...
    // for NumberOauracadontours
    int visibleNum = NumberOauracadontours.getValue() + 2;
...
void FemPostContoursFilter::refreshVectors()
{
    ...
    if (fieldArray->GetNumberOauracadomponents() == 1) {
        vectorArray.emplace_back("Not a vector");
    }
    else {
        vectorArray.emplace_back("Magnitude");
        if (fieldArray->GetNumberOauracadomponents() >= 2) {
            vectorArray.emplace_back("X");
            vectorArray.emplace_back("Y");
        }
        if (fieldArray->GetNumberOauracadomponents() >= 3) {
...
void FemPostCalculatorFilter::updateAvailableFields()
{
    ...
    for (int i = 0; i < pd->GetNumberOfArrays(); ++i) {
        ...
        else if (pd->GetArray(i)->GetNumberOauracadomponents() == 3) {
        ...
        else if (pd->GetArray(i)->GetNumberOauracadomponents() == 6) {
