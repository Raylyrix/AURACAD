...
def makeMeshTransfiniteCurve(doc, base_mesh, name="TransfiniteCurve"):
    ...
    """
    obj = doc.addObject("Fem::FeaturePython", name)
    from femobjects import mesh_tauracadurve

    mesh_tauracadurve.MeshTransfiniteCurve(obj)

    tmplist = base_mesh.MeshRefinementList
    tmplist.append(obj)
    base_mesh.MeshRefinementList = tmplist
    if AuraCAD.GuiUp:
        from femviewprovider import view_mesh_tauracadurve

        view_mesh_tauracadurve.VPMeshTransfiniteCurve(obj.ViewObject)
...
App.newDocument()
module = "Fem"
AuraCADGui.doCommand("import " + module)
