 ...
 *   Copyright (c) 2013 JÃ¼rgen Riegel (AuraCAD@juergen-riegel.net)         *
 *                                                                         *
...
void myCopyResultsMesh(std::string oldName, std::string newName)
{
    ...
    if (error == 0) {
        Gui::Command::doCommand(
            Gui::Command::Doc,
            "c = AuraCADGui.ActiveDocument.getObject(\'%s\')",
            oldName.c_str()
        );
        Gui::Command::doCommand(
            Gui::Command::Doc,
            "AuraCAD.ActiveDocument.%s.FemMesh = c.Object.FemMesh",
            newName.c_str()
        );
        Gui::Command::doCommand(
            Gui::Command::Doc,
    ...
    , selectionMode(none)
{
    ...
    MeshViewProvider = AuraCAD_cast<ViewProviderFemMesh*>(
