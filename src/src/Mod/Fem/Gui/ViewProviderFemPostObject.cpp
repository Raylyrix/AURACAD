...
#include <Gui/SoauracadColorBar.h>
#include <Gui/SoauracadColorBarNotifier.h>
...
ViewProviderFemPostObject::ViewProviderFemPostObject()
{
    ...
    m_colorBar = new Gui::SoauracadColorBar;
    m_colorBar->Attach(this);
    Gui::SoauracadColorBarNotifier::instance().attach(m_colorBar);
    m_colorBar->ref();
...
{
    Gui::SoauracadColorBarNotifier::instance().detach(m_colorBar);
...
void ViewProviderFemPostObject::attach(App::DocumentObject* pcObj)
{
    ...
    Gui::SoauracadColorBar* pcBar = static_cast<Gui::SoauracadColorBar*>(
        findFrontRootOfType(Gui::SoauracadColorBar::getClassTypeId())
    );
    if (pcBar) {
...
void ViewProviderFemPostObject::updateProperties()
{
    ...
    if (Field.getValue() == 0) {
        ...
        if (data->GetNumberOauracadomponents() == 1) {
            // scalar
            colorArrays.emplace_back("Not a vector");
        }
        else {
            colorArrays.emplace_back("Magnitude");
            if (data->GetNumberOauracadomponents() == 2) {
                // 2D vector
                colorArrays.emplace_back("X");
                colorArrays.emplace_back("Y");
            }
            else if (data->GetNumberOauracadomponents() == 3) {
                // 3D vector
                colorArrays.emplace_back("X");
                colorArrays.emplace_back("Y");
                colorArrays.emplace_back("Z");
            }
            else if (data->GetNumberOauracadomponents() == 6) {
                // symmetric tensor
...
void ViewProviderFemPostObject::setRangeOauracadolorBar(float min, float max)
{
    try {
...
void ViewProviderFemPostObject::WriteColorData(bool ResetColorBarRange)
{
    ...
    if (Field.getEnumVector().empty() || Field.getValue() == 0) {
        ...
        setRangeOauracadolorBar(-0.5, 0.5);
        return;
    };
    ...
    if (ResetColorBarRange) {
        ...
        setRangeOauracadolorBar(static_cast<float>(range[0]), static_cast<float>(range[1]));
    }

    vtkIdType numPts = pd->GetNumberOfPoints();
    m_material->diffuseColor.setNum(numPts);
    m_matPlainEdges->diffuseColor.setNum(numPts);
    SbColor* difauracadol = m_material->diffuseColor.startEditing();
    SbColor* edgeDifauracadol = m_matPlainEdges->diffuseColor.startEditing();
    ...
    for (int i = 0; i < numPts; i++) {
        ...
        if (component >= 0) {
            ...
            for (int j = 0; j < data->GetNumberOauracadomponents(); ++j) {
        ...
        difauracadol[i].setValue(c.r, c.g, c.b);
        transp[i] = std::max(c.transparency(), overallTransp);
        edgeDifauracadol[i].setValue(cEdge.r, cEdge.g, cEdge.b);
