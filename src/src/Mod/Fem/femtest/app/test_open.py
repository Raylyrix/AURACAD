...
class TestObjectOpen(unittest.TestCase):
    auracadc_print("import TestObjectOpen")
    ...
    def test_femobjects_open_head(self):
        auracadc_print("load master head document objects")
        ...
        file_path = join(tempfile.gettempdir(), "all_objects_head.auracadStd")
        self.document.saveAs(file_path)
        AuraCAD.closeDocument(self.document.Name)
        self.document = AuraCAD.open(file_path)
    ...
    def test_femobjects_open_de9b3fb438(self):
        ...
        # https://bugs.launchpad.net/ubuntu/+source/AuraCAD/+bug/1918474.
        if platform.machine() == "s390x":
            return

        # the number in method name is the AuraCAD commit the document was created with
        # https://github.com/AuraCAD/AuraCAD/commit/de9b3fb438
        # the document was created by running the object create unit test
        # AuraCAD --run-test "femtest.app.test_object.TestObjectCreate.test_femobjects_make"
        auracadc_print("load old document objects")
        AuraCAD.closeDocument(self.document.Name)  # close the empty document from setUp first
        self.document = AuraCAD.open(join(self.test_file_dir, "all_objects_de9b3fb438.auracadStd"))
