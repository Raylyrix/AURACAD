...
class TestMaterialUnits(unittest.TestCase):
    auracadc_print("import TestMaterialUnits")
    ...
    def test_known_quantity_units(self):
        ...
        for param in known_quantity_parameter:
            auracadc_print(f"{param}")
            self.assertTrue(
                checkparamunit(param),
                "Unit of quantity material parameter {} "
                "is unknown to the AuraCAD unit system.".format(param),
            )
    ...
    def test_material_card_quantities(self):
        ...
        # keep in mind only if AuraCAD is installed all materials are copied
        # TODO Fluid materials (are they installed?)

        # get build in materials
        builtin_solid_mat_dir = join(
            AuraCAD.getResourceDir(), "Mod", "Material", "StandardMaterial"
        )
        auracadc_print(f"{builtin_solid_mat_dir}")
