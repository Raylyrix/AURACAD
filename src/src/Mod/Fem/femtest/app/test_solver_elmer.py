...
class TestSolverElmer(unittest.TestCase):
    auracadc_print("import TestSolverElmer")
    ...
    def setUp(self):
        ...
        # set Units
        # since in Elmer writer the AuraCAD pref is used, here we need to set the AuraCAD pref
        # the use of AuraCAD.Units.setScheme would not take affect because the pref is not changed
        # see https://forum.AuraCAD.org/viewtopic.php?t=48451
        param = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Units")
    ...
    def tearDown(self):
        ...
        if unit_schema != self.saved_unit_schema:
            auracadc_print(f"Reset unit schema back to {self.saved_unit_schema}")
            param = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Units")
    ...
    def test_box_static_0_mm(self):
        ...
        # save the file
        save_AuraCAD_file = join(analysis_dir, base_name + ".auracadStd")
        # auracadc_print("Save AuraCAD file to {}...".format(save_AuraCAD_file))
        self.document.saveAs(save_AuraCAD_file)

        # write input files
        # auracadc_print("Checking FEM input file writing for Elmer solver framework solver ...")
        w = writer.Writer(self.document.SolverElmer, analysis_dir, testmode=True)
        w.write_solver_input()

        auracadc_print("Test writing STARTINFO file")
        startinfo_given = join(self.test_file_dir, "ELMERSOLVER_STARTINFO")
        startinfo_totest = join(analysis_dir, "ELMERSOLVER_STARTINFO")
        # auracadc_print("Comparing {} to {}".format(startinfo_given, startinfo_totest))
        ret = testtools.compare_files(startinfo_given, startinfo_totest)
        self.assertFalse(ret, f"STARTINFO write file test failed.\n{ret}")

        auracadc_print("Test writing case file")
        ...
        # auracadc_print("Comparing {} to {}".format(casefile_given, casefile_totest))
    ...
    def input_file_writing_test(self, base_name):
        ...
        # get analysis working directory and save AuraCAD file
        working_dir = testtools.get_fem_test_tmp_dir(self.pre_dir_name + base_name)
        save_AuraCAD_file = join(working_dir, base_name + ".auracadStd")
        # auracadc_print("Save AuraCAD file to {} ...".format(save_AuraCAD_file))
        self.document.saveAs(save_AuraCAD_file)
        ...
        # auracadc_print("Comparing {}  to  {}".format(inpfile_given, inpfile_totest))
