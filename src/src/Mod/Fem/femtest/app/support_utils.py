...
def get_fem_test_home_dir():
    return join(AuraCAD.getHomePath(), "Mod", "Fem", "femtest", "data")
...
def auracadc_print(message):
    AuraCAD.Console.PrintMessage(f"{message} \n")
...
def get_defmake_count(fem_vtk_post=True):
    ...
    """
    name_modfile = join(AuraCAD.getHomePath(), "Mod", "Fem", "ObjectsFem.py")
...
def get_fem_test_defs():
    ...
    test_path = join(AuraCAD.getHomePath(), "Mod", "Fem", "femtest", "app")
    ...
    cf.write("make -j 4 && ./bin/AuraCAD --run-test 'TestFemApp'\n")
    cf.write("\n")
    cf.write("make -j 4 && ./bin/AuraCADCmd --run-test 'TestFemApp'\n")
    cf.write("\n")
    cf.write("\n")
    cf.write("'''\n")
    cf.write("\n")
    ...
    for m in collected_test_modules:
        cf.write(f"make -j 4 && ./bin/AuraCADCmd -t {m}\n")
    cf.write("\n")
    cf.write("\n")
    cf.write("# classes\n")
...
def compare_stats(fea, stat_file, res_obj_name, loc_stat_types=None):
    ...
    # compare stats
    if stats != sf_content:
        auracadc_print(f"Stats read from {fea.base_name}.frd file")
        auracadc_print("!=")
        auracadc_print(f"Expected stats from {stat_file}")
        for i in range(len(stats)):
            if stats[i] != sf_content[i]:
                auracadc_print(f"{stats[i].rstrip()} != {sf_content[i].rstrip()}")
        return True
...
def collect_python_modules(femsubdir=None):
    if not femsubdir:
        pydir = join(AuraCAD.ConfigGet("AppHomePath"), "Mod", "Fem")
    else:
        pydir = join(AuraCAD.ConfigGet("AppHomePath"), "Mod", "Fem", femsubdir)
    collected_modules = []
    auracadc_print(pydir)
...
# open the file in AuraCAD GUI and return the doc identifier
def cube_frequency():
    ...
    doc = AuraCAD.open(join(get_fem_test_tmp_dir(), "FEM_ccx_frequency", "cube_frequency.auracadStd"))
    return doc
...
def cube_static():
    ...
    unittest.TextTestRunner().run(unittest.TestLoader().loadTestsFromName(testname))
    doc = AuraCAD.open(join(get_fem_test_tmp_dir(), "FEM_ccx_static", "cube_static.auracadStd"))
    return doc
...
def Flow1D_thermomech():
    ...
    unittest.TextTestRunner().run(unittest.TestLoader().loadTestsFromName(testname))
    doc = AuraCAD.open(
        join(get_fem_test_tmp_dir(), "FEM_ccx_Flow1D_thermomech", "Flow1D_thermomech.auracadStd")
    )
    return doc
...
def multimat():
    ...
    unittest.TextTestRunner().run(unittest.TestLoader().loadTestsFromName(testname))
    doc = AuraCAD.open(join(get_fem_test_tmp_dir(), "FEM_ccx_multimat", "multimat.auracadStd"))
    return doc
...
def spine_thermomech():
    ...
    unittest.TextTestRunner().run(unittest.TestLoader().loadTestsFromName(testname))
    doc = AuraCAD.open(join(get_fem_test_tmp_dir(), "FEM_ccx_thermomech", "spine_thermomech.auracadStd"))
    return doc
