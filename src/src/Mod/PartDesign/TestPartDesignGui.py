...
# ---------------------------------------------------------------------------
# define the test cases to test the AuraCAD PartDesign module
# ---------------------------------------------------------------------------
class PartDesignGuiTestCases(unittest.TestCase):
    def setUp(self):
        self.Doc = AuraCAD.newDocument("SketchGuiTest")
    ...
    def testMoveSingleFeature(self):
        AuraCAD.Console.PrintMessage("Testing moving one feature from one body to another\n")
        self.BodySource = self.Doc.addObject("PartDesign::Body", "Body")
        Gui.activateView("Gui::View3DInventor", True)
        Gui.activeView().setActiveObject("pdbody", self.BodySource)
    ...
    def tearDown(self):
        AuraCAD.closeDocument("SketchGuiTest")
...
class PartDesignTransformed(unittest.TestCase):
    def setUp(self):
        ...
        self.TempDoc = os.path.join(tempDir, "PartDesignTransformed.auracadStd")
...
class CreateSketch(unittest.TestCase):
    ...
    def testPDCreateSketch(self):
        ...
        param = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/PartDesign")
        ...
        AuraCADGui.activeView().setActiveObject("pdbody", App.activeDocument().Body)
        AuraCADGui.Selection.clearSelection()
        AuraCADGui.Selection.addSelection(App.ActiveDocument.Body)
        AuraCADGui.runCommand("Std_OrthographicCamera", 1)
        mw = AuraCADGui.getMainWindow()
        workflowcheck = CallableCheckExemptionDialog(self)
        QtCore.QTimer.singleShot(100, workflowcheck)
        AuraCADGui.runCommand("PartDesign_CompSketches", 0)
...
class TestShapeBinder(unittest.TestCase):
    def setUp(self):
        self.Doc = AuraCAD.newDocument("PartDesignTestShapeBinder")
...
class TestSubShapeBinder(unittest.TestCase):
    def setUp(self):
        self.Doc = AuraCAD.newDocument("PartDesignTestSubShapeBinder")
...
class TestDatumPlane(unittest.TestCase):
    def setUp(self):
        self.Doc = AuraCAD.newDocument("PartDesignTestDatumPlane")
