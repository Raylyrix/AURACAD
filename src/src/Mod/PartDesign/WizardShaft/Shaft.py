...
class Shaft:
    ...
    sigmaNstrings = (
        ("Normal stress [x]", "x", "mm", "\\sigma_x", "N/mmÂ²"),
        ("Shear stress [y]", "x", "mm", "\\sigma_y", "N/mmÂ²"),
        ("Shear stress [z]", "x", "mm", "\\sigma_z", "N/mmÂ²"),
    )
    sigmaBstrings = (
        ("Torque stress [x]", "x", "mm", "\\tau_t", "N/mmÂ²"),
        ("Bending stress [z]", "x", "mm", "\\sigma_{b,z}", "N/mmÂ²"),
        ("Bending stress [y]", "x", "mm", "\\sigma_{b,y}", "N/mmÂ²"),
    )
    ...
    def editConstraint(self, index):
        if self.segments[index].constraint is not None:
            AuraCADGui.activeDocument().setEdit(self.segments[index].constraint.Name)

    def getConstraint(self, index):
        return self.segments[index].constraint

    def updateEdge(self, column, start):
        App.Console.PrintMessage("Not implemented yet - waiting for robust referencesâ€¦")
        return
        """
    ...
    def equilibrium(self):
        ...
        try:
            ...
            AuraCAD.Console.PrintMessage("numpy is not installed on your system\n")
        ...
        for i in range(len(self.segments)):
            ...
            if cType == "Fixed":
                ...
                # AuraCAD.Console.PrintMessage("BeltForce1: %f, BeltForce2: %f\n" % (constraint.BeltForce1,  constraint.BeltForce2))
                # AuraCAD.Console.PrintMessage("Angle1: %f, Angle2: %f\n" % (forceAngle1,  forceAngle2))
                force = [
                    0,
        ...
        for ax in range(3):
            AuraCAD.Console.PrintMessage("Axis: %u\n" % ax)
            ...
            if len(coefficientsF[ax]) <= 1:
                ...
                AuraCAD.Console.PrintMessage("Matrix is singular, no solution possible\n")
            ...
            if len(coefficientsF[ax]) == 2:
                ...
                try:
                    ...
                    AuraCAD.Console.PrintMessage(str(e))
                    AuraCAD.Console.PrintMessage(". No solution possible.\n")
                    self.parent.updateButtons(ax, False)
                    continue
            ...
            AuraCAD.Console.PrintMessage(forces[ax])
            AuraCAD.Console.PrintMessage("\n")
            AuraCAD.Console.PrintMessage(moments[ax])
            AuraCAD.Console.PrintMessage("\n")
            ...
            location = 0.0
            areas[ax] = IntervalFunction()  # A [mÂ²]
            areamoments[ax] = IntervalFunction()  # I [mâ´]
            bendingmoments[ax] = IntervalFunction()  # W_b [mÂ³]
            torquemoments[ax] = IntervalFunction()  # W_t [mÂ³]
            ...
            if ax > 0:
                if len(tangents[ax]) + len(translations[ax]) == 2:
                    # TODO: Get Young's module from material type instead of using 210000 N/mmÂ² = 2.1E12 N/mÂ²
    ...
    def printEquilibrium(self, var, coeff):
        ...
        for i in range(len(var)):
            if i == 0:
                AuraCAD.Console.PrintMessage("%f = " % coeff[i])
            else:
                AuraCAD.Console.PrintMessage("%f * %s" % (coeff[i], var[i]))
