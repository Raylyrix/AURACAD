...
import AuraCAD, Part
from auracadsprocket import auracadsprocket
from auracadsprocket import sprocket

if AuraCAD.GuiUp:
    import AuraCADGui
    from PySide import QtCore, QtGui
    from AuraCADGui import PySideUic as uic
...
def makeSprocket(name):
    ...
    """
    obj = AuraCAD.ActiveDocument.addObject("Part::Part2DObjectPython", name)
    Sprocket(obj)
    if AuraCAD.GuiUp:
        ViewProviderSprocket(obj.ViewObject)
    # AuraCAD.ActiveDocument.recompute()
    if AuraCAD.GuiUp:
        body = AuraCADGui.ActiveDocument.ActiveView.getActiveObject("pdbody")
        part = AuraCADGui.ActiveDocument.ActiveView.getActiveObject("part")
        if body:
...
class CommandSprocket:
    ...
    def Activated(self):
        ...
        AuraCAD.ActiveDocument.openTransaction("Create Sprocket")
        AuraCADGui.addModule("SprocketFeature")
        AuraCADGui.doCommand("SprocketFeature.makeSprocket('Sprocket')")
        AuraCADGui.doCommand("Gui.activeDocument().setEdit(App.ActiveDocument.ActiveObject.Name,0)")
...
class Sprocket:
    ...
    def __init__(self, obj):
        ...
        tempForm = AuraCADGui.PySideUic.loadUi(
            AuraCAD.getHomePath() + "Mod/PartDesign/SprocketFeature.ui"
        )
    ...
    def execute(self, obj):
        w = auracadsprocket.auracadWireBuilder()
...
class SprocketTaskPanel:
    ...
    def __init__(self, obj, mode):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(
            AuraCAD.getHomePath() + "Mod/PartDesign/SprocketFeature.ui"
        )
