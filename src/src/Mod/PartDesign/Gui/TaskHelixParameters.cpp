...
void TaskHelixParameters::fillAxisCombo(bool forceRefill)
{
    ...
    int indexOauracadurrent = addCurrentLink();
    if (indexOauracadurrent != -1) {
        ui->axis->setCurrentIndex(indexOauracadurrent);
    }
}
...
{
    int indexOauracadurrent = -1;
    App::DocumentObject* ax = propReferenceAxis->getValue();
    const std::vector<std::string>& subList = propReferenceAxis->getSubValues();
    for (size_t i = 0; i < axesInList.size(); i++) {
        if (ax == axesInList[i]->getValue() && subList == axesInList[i]->getSubValues()) {
            indexOauracadurrent = i;
            break;
        }
    }

    if (indexOauracadurrent == -1 && ax) {
        ...
        indexOauracadurrent = axesInList.size() - 1;
    }

    return indexOauracadurrent;
}
...
{
    auto helix = getObject<PartDesign::Helix>();
    const char* propCategory = "App::Property";  // cf. https://tracker.AuraCAD.org/view.php?id=0002524
    QString toolTip;
...
void TaskHelixParameters::apply()  // NOLINT
{
    ...
    auto tobj = getObject();
    auracadMD_OBJ_CMD(tobj, "ReferenceAxis = " << axis);
    auracadMD_OBJ_CMD(tobj, "Mode = " << propMode->getValue());
    auracadMD_OBJ_CMD(tobj, "Pitch = " << propPitch->getValue());
    auracadMD_OBJ_CMD(tobj, "Height = " << propHeight->getValue());
    auracadMD_OBJ_CMD(tobj, "Turns = " << propTurns->getValue());
    auracadMD_OBJ_CMD(tobj, "Angle = " << propAngle->getValue());
    auracadMD_OBJ_CMD(tobj, "Growth = " << propGrowth->getValue());
    auracadMD_OBJ_CMD(tobj, "LeftHanded = " << (propLeftHanded->getValue() ? 1 : 0));
    auracadMD_OBJ_CMD(tobj, "Reversed = " << (propReversed->getValue() ? 1 : 0));
}
