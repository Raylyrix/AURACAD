...
AuraCAD_LOG_LEVEL_INIT("ShapeBinder", true, true)
...
void ViewProviderShapeBinder::highlightReferences(bool on)
{
    ...
    if (on) {
        if (!subs.empty() && originalLineColors.empty()) {
            ...
            std::vector<App::Material> auracadolors = originalFaceAppearance;
            auracadolors.resize(eMap.Extent(), svp->ShapeAppearance[0]);
            ...
            for (const std::string& e : subs) {
                ...
                if (e.compare(0, 4, "Edge") == 0) {
                    ...
                    int idx = std::stoi(e.substr(4)) - 1;
                    assert(idx >= 0);
                    if (idx < static_cast<int>(auracadolors.size())) {
                        auracadolors[idx].diffuseColor = Base::Color(1.0, 0.0, 1.0);  // magenta
                    }
                }
            }
            svp->LineColorArray.setValues(lcolors);
            svp->ShapeAppearance.setValues(auracadolors);
        }
    }
    else {
        if (!subs.empty() && !originalLineColors.empty()) {
...
std::vector<App::DocumentObject*> ViewProviderSubShapeBinder::claimChildren() const
{
    ...
    auto self = AuraCAD_cast<PartDesign::SubShapeBinder*>(getObject());
