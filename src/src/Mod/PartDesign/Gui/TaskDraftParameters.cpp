...
/***************************************************************************
 *   Copyright (c) 2012 Jan RheinlÃ¤nder                                    *
...
bool TaskDlgDraftParameters::accept()
{
    ...
    auracadMD_OBJ_CMD(tobj, "Angle = " << draftparameter->getAngle());
    auracadMD_OBJ_CMD(tobj, "Reversed = " << draftparameter->getReversed());
    if (neutralPlane.empty()) {
        neutralPlane = "None";
    }
    auracadMD_OBJ_CMD(tobj, "NeutralPlane = " << neutralPlane);
    if (pullDirection.empty()) {
        pullDirection = "None";
    }
    auracadMD_OBJ_CMD(tobj, "PullDirection = " << pullDirection);
