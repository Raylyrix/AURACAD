...
public:
    ...
    void handleSelectedBody(PartDesign::Body* activeBody)
    {
        ...
        // https://forum.AuraCAD.org/viewtopic.php?f=3&t=37448
...
public:
    ...
    void createSketchOnSupport(const std::string& supportString)
    {
        ...
        auracadMD_OBJ_CMD(activeBody, "newObject('Sketcher::SketchObject','" << FeatName << "')");
        auto Feat = activeBody->getDocument()->getObject(FeatName.c_str());
        auracadMD_OBJ_CMD(Feat, "AttachmentSupport = " << supportString);
...
private:
    ...
    )
    {
        ...
        auracadMD_OBJ_CMD(partDesignBody, "newObject('Sketcher::SketchObject','" << FeatName << "')");
...
std::tuple<Gui::SelectionFilter, Gui::SelectionFilter, Gui::SelectionFilter> SketchWorkflow::getFilters() const
{
    ...
    // a new sketch.
    // See https://forum.AuraCAD.org/viewtopic.php?f=3&t=44070
