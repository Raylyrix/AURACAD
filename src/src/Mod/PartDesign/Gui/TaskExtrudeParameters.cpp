...
void TaskExtrudeParameters::fillDirectionCombo()
{
    ...
    if (indexOauracadurrent != -1 && !hasCustom) {
        ui->directionCB->setCurrentIndex(indexOauracadurrent);
        updateDirectionEdits();
        setDirectionMode(indexOauracadurrent);
    }
    if (hasCustom) {
...
void TaskExtrudeParameters::applyParameters()
{
    ...
    ui->taperEdit->apply();
    ui->taperEdit2->apply();
    auracadMD_OBJ_CMD(obj, "UseCustomVector = " << (getCustom() ? 1 : 0));
    auracadMD_OBJ_CMD(
        obj,
        "Direction = (" << getXDirection() << ", " << getYDirection() << ", " << getZDirection() << ")"
    );
    auracadMD_OBJ_CMD(obj, "ReferenceAxis = " << getReferenceAxis());
    auracadMD_OBJ_CMD(obj, "AlongSketchNormal = " << (getAlongSketchNormal() ? 1 : 0));
    auracadMD_OBJ_CMD(obj, "SideType = " << getSidesMode());
    auracadMD_OBJ_CMD(obj, "Type = " << type1);
    auracadMD_OBJ_CMD(obj, "Type2 = " << type2);
    auracadMD_OBJ_CMD(obj, "UpToFace = " << facename.toUtf8().data());
    auracadMD_OBJ_CMD(obj, "UpToFace2 = " << facename2.toUtf8().data());
    auracadMD_OBJ_CMD(obj, "Reversed = " << (getReversed() ? 1 : 0));
    auracadMD_OBJ_CMD(obj, "Offset = " << getOffset());
    auracadMD_OBJ_CMD(obj, "Offset2 = " << getOffset2());
}
