...
AuraCAD_LOG_LEVEL_INIT("PartDesignGui", true, true)
...
bool setEdit(App::DocumentObject* obj, PartDesign::Body* body)
{
    if (!obj || !obj->isAttachedToDocument()) {
        AuraCAD_ERR("invalid object");
        return false;
    }
    if (!body) {
        body = getBodyFor(obj, false);
        if (!body) {
            AuraCAD_ERR("no body found");
            return false;
        }
    }
...
void fixSketchSupport(Sketcher::SketchObject* sketch)
{
    ...
    if (fabs(offset) < Precision::Confusion()) {
        ...
        auracadMD_OBJ_CMD(sketch, "AttachmentSupport = (" << Gui::Command::getObjectCmd(plane) << ",[''])");
        auracadMD_OBJ_CMD(sketch, "MapReversed = " << (reverseSketch ? "True" : "False"));
        auracadMD_OBJ_CMD(
            sketch,
        ...
        auracadMD_DOC_CMD(doc, "addObject('PartDesign::Plane','" << Datum << "')");
        auto obj = doc->getObject(Datum.c_str());
        auracadMD_OBJ_CMD(obj, "AttachmentSupport = [(" << Gui::Command::getObjectCmd(plane) << ",'')]");
        auracadMD_OBJ_CMD(obj, "MapMode = '" << AttachEngine::getModeName(Attacher::mmFlatFace) << "'");
        auracadMD_OBJ_CMD(obj, "AttachmentOffset.Base.z = " << offset);
        auracadMD_OBJ_CMD(
            body,
            "insertObject(" << Gui::Command::getObjectCmd(obj) << ','
                            << Gui::Command::getObjectCmd(sketch) << ")"
        );
        auracadMD_OBJ_CMD(sketch, "AttachmentSupport = (" << Gui::Command::getObjectCmd(obj) << ",[''])");
        auracadMD_OBJ_CMD(sketch, "MapReversed = " << (reverseSketch ? "True" : "False"));
        auracadMD_OBJ_CMD(
            sketch,
            "MapMode = '" << Attacher::AttachEngine::getModeName(Attacher::mmFlatFace) << "'"
        ...
        if (!isPartDesignAwareObjecta(obj)) {
            ...
            for (auto prop : properties) {
                ...
                if (!valueStr.empty() && prop->hasName()) {
                    auracadMD_OBJ_CMD(obj, prop->getName() << '=' << valueStr);
                }
            }
        }
    }
