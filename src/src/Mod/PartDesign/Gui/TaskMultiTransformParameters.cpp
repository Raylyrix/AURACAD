...
void TaskMultiTransformParameters::onTransformAddMirrored()
{
    ...
    auracadMD_OBJ_CMD(pcBody, "newObject('PartDesign::Mirrored','" << newFeatName << "')");
    ...
    if (sketch) {
        auracadMD_OBJ_CMD(Feat, "MirrorPlane = (" << Gui::Command::getObjectCmd(sketch) << ",['V_Axis'])");
    }
    else {
        App::Origin* orig = pcBody->getOrigin();
        auracadMD_OBJ_CMD(Feat, "MirrorPlane = (" << Gui::Command::getObjectCmd(orig->getXY()) << ",[''])");
    }
...
void TaskMultiTransformParameters::onTransformAddLinearPattern()
{
    ...
    auracadMD_OBJ_CMD(pcBody, "newObject('PartDesign::LinearPattern','" << newFeatName << "')");
    auto Feat = pcBody->getDocument()->getObject(newFeatName.c_str());
    if (!Feat) {
        return;
    }
    ...
    App::DocumentObject* sketch = getSketchObject();
    if (sketch) {
        auracadMD_OBJ_CMD(Feat, "Direction = (" << Gui::Command::getObjectCmd(sketch) << ",['H_Axis'])");
    }
    else {
...
void TaskMultiTransformParameters::onTransformAddPolarPattern()
{
    ...
    auracadMD_OBJ_CMD(pcBody, "newObject('PartDesign::PolarPattern','" << newFeatName << "')");
    auto Feat = pcBody->getDocument()->getObject(newFeatName.c_str());
    if (!Feat) {
        return;
    }
    ...
    if (sketch) {
        ...
        App::Origin* orig = pcBody->getOrigin();
        auracadMD_OBJ_CMD(Feat, "Axis = (" << Gui::Command::getObjectCmd(orig->getX()) << ",[''])");
    }
...
void TaskMultiTransformParameters::onTransformAddScaled()
{
    ...
    auracadMD_OBJ_CMD(pcBody, "newObject('PartDesign::Scaled','" << newFeatName << "')");
    auto Feat = pcBody->getDocument()->getObject(newFeatName.c_str());
    if (!Feat) {
        return;
    }
