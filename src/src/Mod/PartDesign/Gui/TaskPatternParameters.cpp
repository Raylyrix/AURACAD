...
void TaskPatternParameters::apply()
{
    ...
    auracadMD_OBJ_CMD(pattern, propName << direction.c_str());
    auracadMD_OBJ_CMD(pattern, "Reversed = " << parametersWidget->getReverse());
    auracadMD_OBJ_CMD(pattern, "Mode = " << parametersWidget->getMode());
    parametersWidget->applyQuantitySpinboxes();

    auracadMD_OBJ_CMD(pattern, "SpacingPattern = " << parametersWidget->getSpacingPatternsAsString());

    if (parametersWidget2) {
        parametersWidget2->getAxis(obj, dirs);
        direction = buildLinkSingleSubPythonStr(obj, dirs);

        auracadMD_OBJ_CMD(pattern, "Direction2 = " << direction.c_str());
        auracadMD_OBJ_CMD(pattern, "Reversed2 = " << parametersWidget2->getReverse());
        auracadMD_OBJ_CMD(pattern, "Mode2 = " << parametersWidget2->getMode());
        parametersWidget2->applyQuantitySpinboxes();

        auracadMD_OBJ_CMD(pattern, "SpacingPattern2 = " << parametersWidget2->getSpacingPatternsAsString());
    }
