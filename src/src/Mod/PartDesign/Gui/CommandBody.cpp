            ...
            if (addtogroup) {
                ...
                if (validPlaneCount > 1) {
                    ...
                    auto worker = [baseFeature](const std::vector<App::DocumentObject*>& features) {
                        ...
                        auracadMD_OBJ_CMD(baseFeature, "AttachmentSupport = " << supportString);
                        auracadMD_OBJ_CMD(
                            baseFeature,
                            "MapMode = '"
    ...
    for (auto featIt = migrateFeatures.begin(); !migrateFeatures.empty();) {
        ...
        if (!base || !base->isDerivedFrom(PartDesign::Feature::getClassTypeId())
            ...
            if (baseFeatSetIt != migrateFeatures.end()) {
                ...
                auto isChain =
                    [baseFeat, &baseFeatIt](std::list<PartDesign::Feature*>& auracadhain) mutable -> bool {
                    baseFeatIt = std::ranges::find(auracadhain, baseFeat);
                    return baseFeatIt != auracadhain.end();
                };
...
void CmdPartDesignMoveTip::activated(int iMsg)
{
    ...
    if (selFeature == body) {
        auracadMD_OBJ_CMD(body, "Tip = None");
    }
    else {
        auracadMD_OBJ_CMD(body, "Tip = " << getObjectCmd(selFeature));

        // Adjust visibility to show only the Tip feature
        auracadMD_OBJ_SHOW(selFeature);
    }
...
void CmdPartDesignDuplicateSelection::activated(int iMsg)
{
    ...
    doCommand(Doc, "AuraCADGui.runCommand('Std_DuplicateSelection')");
    ...
    if (pcActiveBody) {
        ...
        for (auto feature : newFeatures) {
            if (PartDesign::Body::isAllowed(feature)) {
                ...
                if (!body) {
                    auracadMD_OBJ_CMD(pcActiveBody, "addObject(" << getObjectCmd(feature) << ")");
                    auracadMD_OBJ_HIDE(feature);
                }
            }
        }
        ...
        if (!newFeatures.empty()) {
            auracadMD_OBJ_SHOW(newFeatures.back());
        }
    }
    ...
    : Command("PartDesign_MoveFeature")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
    sMenuText = QT_TR_NOOP("Move Object Toâ€¦");
...
void CmdPartDesignMoveFeature::activated(int iMsg)
{
    ...
    auracadMD_OBJ_CMD(source_body, "removeObjects(features_)");
    auracadMD_OBJ_CMD(target, "addObjects(features_)");
    /*
    ...
    : Command("PartDesign_MoveFeatureInTree")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
    sMenuText = QT_TR_NOOP("Move Feature Afterâ€¦");
...
void CmdPartDesignMoveFeatureInTree::activated(int iMsg)
{
    ...
    QString text = QInputDialog::getItem(
        Gui::getMainWindow(),
        qApp->translate("PartDesign_MoveFeatureInTree", "Move Feature Afterâ€¦"),
    ...
    for (auto feat : features) {
        ...
        // (2015-08-07, Fat-Zer)
        auracadMD_OBJ_CMD(body, "removeObject(" << getObjectCmd(feat) << ")");
        auracadMD_OBJ_CMD(
            body,
    ...
    if (lastObject != target && body->Tip.getValue() == target
        ...
        if (ret == QMessageBox::Yes) {
            auracadMD_OBJ_CMD(body, "Tip = " << getObjectCmd(lastObject));
        }
    }
