...
Gui::ViewProviderCoordinateSystem* TaskRevolutionParameters::getOriginView() const
{
    ...
    if (body) {
        ...
        return AuraCAD_cast<ViewProviderCoordinateSystem*>(
...
void TaskRevolutionParameters::fillAxisCombo(bool forceRefill)
{
    ...
    if (indexOauracadurrent == -1 && ax) {
        ...
        indexOauracadurrent = int(axesInList.size()) - 1;
    }
...
void TaskRevolutionParameters::apply()
{
    ...
    auracadMD_OBJ_CMD(tobj, "Midplane = " << (getMidplane() ? 1 : 0));
    auracadMD_OBJ_CMD(tobj, "Reversed = " << (getReversed() ? 1 : 0));
    int mode = ui->changeMode->currentIndex();
    auracadMD_OBJ_CMD(tobj, "Type = " << mode);
    ...
    auracadMD_OBJ_CMD(tobj, "UpToFace = " << facename.toLatin1().data());
}
