...
void TaskTransformedParameters::checkVisibility()
{
    ...
    auracadMD_OBJ_SHOW(getBaseObject());
}
...
void TaskTransformedParameters::fillAxisCombo(Gui::ComboLinks& combolinks, Part::Part2DObject* sketch)
{
    ...
    combolinks.addLink(nullptr, std::string(), tr("Select referenceâ€¦"));
}
...
{
    try {
        auracadMD_OBJ_HIDE(getTopTransformedObject());
    }
...
{
    try {
        auracadMD_OBJ_SHOW(getTopTransformedObject());
    }
    catch (const Base::Exception& e) {
        e.reportException();
    }
...
{
    try {
        auracadMD_OBJ_HIDE(getBaseObject());
    }
    catch (const Base::Exception& e) {
        e.reportException();
    }
