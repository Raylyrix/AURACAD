...
AuraCAD_LOG_LEVEL_INIT("PartDesign", true, true)
...
void UnifiedDatumCommand(Gui::Command& cmd, Base::Type type, std::string name)
{
    try {
        ...
        if (bEditSelected) {
            ...
            auracadMD_OBJ_CMD(pcActiveBody, "newObject('" << fullTypeName << "','" << FeatName << "')");
            ...
            if (support.getSize() > 0) {
                ...
                if (sugr.message == Attacher::SuggestResult::srOK) {
                    ...
                    auracadMD_OBJ_CMD(Feat, "AttachmentSupport = " << support.getPyReprString());
                    auracadMD_OBJ_CMD(
                        Feat,
...
void CmdPartDesignShapeBinder::activated(int iMsg)
{
    ...
    if (bEditSelected) {
        ...
        auracadMD_OBJ_CMD(pcActiveBody, "newObject('PartDesign::ShapeBinder','" << FeatName << "')");
        ...
        // test if current selection fits a mode.
        if (support.getSize() > 0) {
            auracadMD_OBJ_CMD(Feat, "Support = " << support.getPyReprString());
        }
        updateActive();
        PartDesignGui::setEdit(Feat, pcActiveBody);
    }
...
void CmdPartDesignSubShapeBinder::activated(int iMsg)
{
    ...
    try {
        ...
        if (pcActiveBody) {
            auracadMD_OBJ_CMD(pcActiveBody, "newObject('PartDesign::SubShapeBinder','" << FeatName << "')");
...
void CmdPartDesignClone::activated(int iMsg)
{
    ...
    if (objs.size() == 1) {
        // As suggested in https://forum.AuraCAD.org/viewtopic.php?f=3&t=25265&p=198547#p207336
...
)
{
    ...
    if (hidePrevSolid && prevSolidFeature) {
        auracadMD_OBJ_HIDE(prevSolidFeature);
    }
...
bool importExternalElements(App::PropertyLinkSub& prop, std::vector<App::SubObjectT> _sobjs)
{
    if (!prop.getName() || !prop.getName()[0]) {
        AuraCAD_THROWM(Base::RuntimeError, "Invalid property");
    }
    auto editObj = AuraCAD_cast<App::DocumentObject*>(prop.getContainer());
    if (!editObj) {
        AuraCAD_THROWM(Base::RuntimeError, "Editing object not found");
    }
    auto body = PartDesign::Body::findBodyOf(editObj);
    if (!body) {
        AuraCAD_THROWM(Base::RuntimeError, "No body for editing object: " << editObj->getNameInDocument());
    }
    ...
    for (auto sobjT : _sobjs) {
        ...
        if (!sobj) {
            AuraCAD_THROWM(Base::RuntimeError, "Object not found: " << sobjT.getSubObjectFullName(docName));
        }
...
)
{
    auto base_worker = [=](App::DocumentObject* feature, const std::vector<std::string>& subs) {
        ...
        auracadMD_OBJ_CMD(pcActiveBody, "newObject('PartDesign::" << which << "','" << FeatName << "')");
        auto Feat = pcActiveBody->getDocument()->getObject(FeatName.c_str());
        ...
        auto ProfileFeature = AuraCAD_cast<PartDesign::ProfileBased*>(Feat);
        ...
        auto runProfileCmd = [=]() {
            auracadMD_OBJ_CMD(Feat, "Profile = " << objCmd);
        };
        ...
        auto runProfileCmdWithSubs = [=]() {
            ...
            auracadMD_OBJ_CMD(Feat, "Profile = (" << objCmd << ", [" << ss.str() << "])");
        };
        ...
        if (which.compare("AdditiveLoft") == 0 || which.compare("SubtractiveLoft") == 0) {
            ...
            if (selection.size() > 1) {  // treat additional selected objects as sections
                for (std::vector<Gui::SelectionObject>::size_type ii = 1; ii < selection.size();
                    ...
                    auracadMD_OBJ_CMD(Feat, "Sections += [(" << objCmdSection << ", [" << ss.str() << "])]");
                }
            }
        }
            ...
            if (selection.size() == 2) {  // treat additional selected object as spine
                ...
                if (selection[1].getObject()->isDerivedFrom<Part::Part2DObject>()
                    && subnames.empty()) {
                    auracadMD_OBJ_CMD(Feat, "Spine = " << objCmdSpine);
                }
                else {
                    std::ostringstream ss;
                    for (auto& s : subnames) {
                    ...
                    auracadMD_OBJ_CMD(Feat, "Spine = (" << objCmdSpine << ", [" << ss.str() << "])");
                }
            }
        }
        else {
    ...
    if (pcActiveBody && !bNoSketchWasSelected && extReference) {
        ...
        // For more details see: https://forum.AuraCAD.org/viewtopic.php?f=19&t=32164
...
void prepareProfileBased(Gui::Command* cmd, const std::string& which, double length)
{
    ...
    auto worker = [cmd, length](Part::Feature* profile, App::DocumentObject* Feat) {
        ...
        auracadMD_OBJ_CMD(Feat, "Length = " << length);
...
void CmdPartDesignRevolution::activated(int iMsg)
{
    ...
    auto worker = [cmd, &pcActiveBody](Part::Feature* sketch, App::DocumentObject* Feat) {
        ...
        if (sketch->isDerivedFrom<Part::Part2DObject>()) {
            auracadMD_OBJ_CMD(Feat, "ReferenceAxis = (" << getObjectCmd(sketch) << ",['V_Axis'])");
        }
        else {
            auracadMD_OBJ_CMD(
                Feat,
...
)
{
    ...
    cmd->openCommand(std::string("Make ") + which);
    auracadMD_OBJ_CMD(body, "newObject('PartDesign::" << which << "','" << FeatName << "')");
    auto Feat = body->getDocument()->getObject(FeatName.c_str());
    auracadMD_OBJ_CMD(Feat, "Base = " << str.str());
    if (useAllEdges && (which.compare("Fillet") == 0 || which.compare("Chamfer") == 0)) {
        auracadMD_OBJ_CMD(Feat, "UseAllEdges = True");
    }
...
)
{
    ...
    auto worker = [=](std::vector<App::DocumentObject*> features) {
        ...
        if (features.empty()) {
            auracadMD_OBJ_CMD(Feat, "TransformMode = \"Whole shape\"");
        }
        else {
            ...
            str << "]";
            auracadMD_OBJ_CMD(Feat, str.str().c_str());
        }
        ...
        auracadMD_OBJ_CMD(pcActiveBody, "Tip = " << Gui::Command::getObjectCmd(Feat));
        Gui::Command::updateActive();
    };
...
void CmdPartDesignMirrored::activated(int iMsg)
{
    ...
    auto worker = [cmd,
        ...
        if (!features.empty() && features.front()->isDerivedFrom<PartDesign::ProfileBased>()) {
            ...
            if (sketch) {
                auracadMD_OBJ_CMD(Feat, "MirrorPlane = (" << getObjectCmd(sketch) << ", ['V_Axis'])");
                direction = true;
            }
        }
        if (!direction) {
...
void CmdPartDesignLinearPattern::activated(int iMsg)
{
    ...
    auto worker =
        [cmd, pcActiveBody](App::DocumentObject* Feat, std::vector<App::DocumentObject*> features) {
            ...
            auracadMD_OBJ_CMD(Feat, "Length = 100");
            auracadMD_OBJ_CMD(Feat, "Occurrences = 2");
    ...
    auto worker = [cmd,
        ...
        if (!features.empty() && features.front()->isDerivedFrom<PartDesign::ProfileBased>()) {
            Part::Part2DObject* sketch = (static_cast<PartDesign::ProfileBased*>(features.front()))
                                             ->getVerifiedSketch(/* silent =*/true);
            if (sketch) {
                auracadMD_OBJ_CMD(Feat, "Axis = (" << Gui::Command::getObjectCmd(sketch) << ",['N_Axis'])");
                direction = true;
            }
        }
        if (!direction) {
        ...
        auracadMD_OBJ_CMD(Feat, "Angle = 360");
        auracadMD_OBJ_CMD(Feat, "Occurrences = 2");
...
void CmdPartDesignScaled::activated(int iMsg)
{
    ...
    auto worker = [cmd](App::DocumentObject* Feat, std::vector<App::DocumentObject*> /*features*/) {
        auracadMD_OBJ_CMD(Feat, "Factor = 2");
        auracadMD_OBJ_CMD(Feat, "Occurrences = 2");
...
void CmdPartDesignMultiTransform::activated(int iMsg)
{
    ...
    if (!features.empty()) {
        ...
        if (pcActiveBody)
            auracadMD_OBJ_CMD(pcActiveBody,"removeObject("<<getObjectCmd(trFeat)<<")");
#endif
        ...
        auracadMD_OBJ_CMD(pcActiveBody, "newObject('PartDesign::MultiTransform','" << FeatName << "')");
        auto Feat = pcActiveBody->getDocument()->getObject(FeatName.c_str());
        auto objCmd = getObjectCmd(trFeat);
        auracadMD_OBJ_CMD(Feat, "Originals = " << objCmd << ".Originals");
        auracadMD_OBJ_CMD(Feat, "TransformMode = " << objCmd << ".TransformMode");
        auracadMD_OBJ_CMD(Feat, "BaseFeature = " << objCmd << ".BaseFeature");
        auracadMD_OBJ_CMD(Feat, "Transformations = [" << objCmd << "]");

        auracadMD_OBJ_CMD(trFeat, "Originals = []");
            ...
            [cmd,
             pcActiveBody](App::DocumentObject* Feat, std::vector<App::DocumentObject*> /*features*/) {
                ...
                if (prevSolid) {
                    ...
                    auracadMD_OBJ_CMD(Feat, "Shape = " << getObjectCmd(feat) << ".Shape");
                }
                finishFeature(cmd, Feat);
            };
...
void CmdPartDesignBoolean::activated(int iMsg)
{
    ...
    auracadMD_OBJ_CMD(pcActiveBody, "newObject('PartDesign::Boolean','" << FeatName << "')");
    auto Feat = pcActiveBody->getDocument()->getObject(FeatName.c_str());
    ...
    if (BodyFilter.match() && !BodyFilter.Result.empty()) {
        ...
        if (!bodies.empty()) {
            ...
            auracadMD_OBJ_CMD(Feat, "addObjects(" << bodyString << ")");
        }
    }
