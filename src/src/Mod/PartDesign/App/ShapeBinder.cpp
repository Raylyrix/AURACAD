    ...
    if (!init) {
        ...
        if (!errMsg.empty()) {
            if (!(options & UpdateInit)) {
                AuraCAD_THROWM(Base::RuntimeError, errMsg);
            }
        ...
        if (!forced) {
            bool hit = true;
            for (auto& v : mats) {
                auto prop = AuraCAD_cast<App::PropertyMatrix*>(
...
)
{
    ...
    for (auto& v : values) {
        ...
        if (inSet.find(v.first) != inSet.end()) {
            AuraCAD_THROWM(Base::ValueError, "Cyclic reference to " << v.first->getFullName());
        }
        ...
        for (auto& whole : wholeSubs) {
            for (auto it = v.second.begin(); it != v.second.end();) {
                ...
                if (!boost::starts_with(sub, whole) || sub.size() == whole.size()) {
                    ...
                    AuraCAD_LOG("Remove subname " << sub << " because of whole selection " << whole);
                ...
                for (auto it = subs.begin(); it != subs.end();) {
                    if (sub[sub.size() - 1] == '.') {
                        if (boost::starts_with(*it, sub)) {
                            AuraCAD_LOG("Remove subname " << *it << " because of whole selection " << sub);
                            it = subs.erase(it);
                        }
                        else {
                            ++it;
                        ...
                        AuraCAD_LOG("Remove whole subname " << *it << " because of " << sub);
                        it = subs.erase(it);
                    }
                    else {
                        ++it;
