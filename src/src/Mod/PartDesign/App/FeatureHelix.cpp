...
App::DocumentObjectExecReturn* Helix::execute()
{
    ...
    try {
        ...
        if (getAddSubType() == FeatureAddSub::Additive) {
            ...
            auracadBRepAlgoAPI_Fuse mkFuse(base.getShape(), result);
            if (!mkFuse.IsDone()) {
                return new App::DocumentObjectExecReturn(
            ...
            if (Outside.getValue()) {  // are we subtracting the inside or the outside of the profile.
                auracadBRepAlgoAPI_Common mkCom(result, base.getShape());
                if (!mkCom.IsDone()) {
                    return new App::DocumentObjectExecReturn(
                ...
                auracadBRepAlgoAPI_Cut mkCut(base.getShape(), result);
                if (!mkCut.IsDone()) {
                    return new App::DocumentObjectExecReturn(
...
TopoDS_Shape Helix::generateHelixPath(double breakAtTurn)
{
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=10&t=65048
