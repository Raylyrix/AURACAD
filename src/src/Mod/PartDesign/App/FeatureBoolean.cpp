...
/******************************************************************************
 *   Copyright (c) 2013 Jan RheinlÃ¤nder <jrheinlaender@users.sourceforge.net> *
 *                                                                            *
...
App::DocumentObjectExecReturn* Boolean::execute()
{
    ...
    if (!tools.empty()) {
        ...
        try {
            ...
            AuraCAD_ERR("Boolean operation failed: " << e.GetMessageString());
            return new App::DocumentObjectExecReturn(
...
void Boolean::updatePreviewShape()
{
    if (strcmp(Type.getValueAsString(), "Cut") == 0) {
        ...
        try {
            ...
            AuraCAD_ERR("Boolean preview failed: " << e.GetMessageString());
            PreviewShape.setValue(base);
        }
        catch (Base::Exception& e) {
            AuraCAD_ERR("Boolean preview failed: " << e.what());
            PreviewShape.setValue(base);
        }
        return;
    }
