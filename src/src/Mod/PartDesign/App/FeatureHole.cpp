...
void Hole::updateHoleCutParams()
{
    ...
    if (threadTypeStr == "ISOMetricProfile" || threadTypeStr == "ISOMetricFineProfile") {
        ...
        // Tag: MIGRATION
        // handle since AuraCAD 0.18 deprecated types that were
        // removed after AuraCAD 0.20
...
void Hole::findClosestDesignation()
{
    ...
    if (targetPitch == 0.0) {
        ...
        // Scan all entries to find the minimal (Î”diameter, Î”pitch) Euclidean distance
        ...
        try {
            ...
            AuraCAD_WARN(
                getFullName() << ": boolean operation with compound failed ("
                              << e.GetMessageString() << "), retryâ€¦"
            );
        }
        catch (Base::Exception& e) {
            AuraCAD_WARN(
                getFullName() << ": boolean operation with compound failed (" << e.what() << "), retryâ€¦"
            );
        }
...
TopoDS_Shape Hole::makeThread(const gp_Vec& xDir, const gp_Vec& zDir, double length)
{
    ...
    if (threadTypeStr == "BSP" || threadTypeStr == "BSW" || threadTypeStr == "BSF") {
        ...
        Handle(Geom_TrimmedCurve) arc1 = GC_MakeArcOauracadircle(p2, crest, p3).Value();
