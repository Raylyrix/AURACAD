...
Part::Feature* Transformed::getBaseObject(bool silent) const
{
    ...
    if (firstOriginal) {
        rv = AuraCAD_cast<Part::Feature*>(firstOriginal);
        if (!rv) {
        ...
        if (AuraCAD_cast<const Mirrored*>(this)) {
            err = QT_TRANSLATE_NOOP("Exception", "No features selected to be mirrored.");
        }
        else if (AuraCAD_cast<const LinearPattern*>(this) || AuraCAD_cast<const PolarPattern*>(this)) {
...
std::vector<App::DocumentObject*> Transformed::getOriginals() const
{
    ...
    const auto isSuppressed = [](const DocumentObject* obj) {
        auto feature = AuraCAD_cast<Feature*>(obj);
...
App::DocumentObject* Transformed::getSketchObject() const
{
    ...
    if (auto feature = AuraCAD_cast<PartDesign::ProfileBased*>(firstOriginal)) {
        return feature->getVerifiedSketch(true);
    }
    if (AuraCAD_cast<PartDesign::FeatureAddSub*>(firstOriginal)) {
        return nullptr;
    }
    if (auto pattern = AuraCAD_cast<LinearPattern*>(this)) {
        return pattern->Direction.getValue();
    }
    if (auto pattern = AuraCAD_cast<PolarPattern*>(this)) {
        return pattern->Axis.getValue();
    }
    if (auto pattern = AuraCAD_cast<Mirrored*>(this)) {
...
bool Transformed::isMultiTransformChild() const
{
    ...
    for (auto const* obj : getInList()) {
        auto mt = AuraCAD_cast<PartDesign::MultiTransform*>(obj);
        if (!mt) {
            continue;
        }
...
)
{
    ...
    if (auto property = AuraCAD_cast<App::PropertyFloat*>(prop);
...
App::DocumentObjectExecReturn* Transformed::recomputePreview()
{
    ...
    const auto makeCompoundOfToolShapes = [this, &supportTransform]() {
        ...
        for (const auto& original : getOriginals()) {
            if (auto* feature = AuraCAD_cast<FeatureAddSub*>(original)) {
    ...
    switch (mode) {
        case Mode::Features:
            ...
            for (auto original : originals) {
                ...
                auto feature = AuraCAD_cast<PartDesign::FeatureAddSub*>(original);
                if (!feature) {
