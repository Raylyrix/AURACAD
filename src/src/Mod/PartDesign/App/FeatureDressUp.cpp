...
/***************************************************************************
 *   Copyright (c) 2010 Juergen Riegel <AuraCAD@juergen-riegel.net>        *
 *                                                                         *
...
std::vector<TopoShape> DressUp::getContinuousEdges(const TopoShape& shape)
{
    ...
    auto addEdge = [&](const TopoDS_Shape& subshape, const std::string& ref) {
        ...
        if (faces.size() != 2) {
            AuraCAD_WARN(getFullName() << ": skip edge " << ref << " with less two attaching faces");
            return;
        }
        ...
        if (cont != GeomAbs_C0) {
            AuraCAD_WARN(getFullName() << ": skip edge " << ref << " that is not C0 continuous");
            return;
        }
    ...
    for (const auto& v : Base.getShadowSubs()) {
        ...
        if (subshape.IsNull()) {
            AuraCAD_THROWM(Base::CADKernelError, "Invalid edge link: " << ref);
        }
...
std::vector<TopoShape> DressUp::getFaces(const TopoShape& shape)
{
    ...
    for (auto& val : vals) {
        ...
        if (subshape.isNull()) {
            AuraCAD_ERR(getFullName() << ": invalid face reference '" << ref << "'");
        ...
        try {
            ...
            if (SupportTransform.getValue()) {
                ...
                for (Feature* current = this;; current = static_cast<DressUp*>(base)) {
                    base = AuraCAD_cast<FeatureAddSub*>(current->getBaseObject(true));
                    if (!base) {
                        AuraCAD_THROWM(
                            Base::CADKernelError,
...
void DressUp::updatePreviewShape()
{
    ...
    auto baseFeature = AuraCAD_cast<Feature*>(BaseFeature.getValue());
