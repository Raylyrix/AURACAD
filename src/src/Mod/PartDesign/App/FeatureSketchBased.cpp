...
) const
{
    ...
    try {
        ...
        if (count > 1) {
            ...
            AuraCAD_WARN("Found more than one face from profile");
        }
        if (!openshape.isNull()) {
            return shape;
        }
...
) const
{
    ...
    if (pcReferenceAxis->isDerivedFrom<Part::Feature>()) {
        ...
        TopoDS_Shape ref;
        try {
            // if an exception is raised then convert it into a AuraCAD-specific exception
...
Base::Vector3d ProfileBased::getProfileNormal() const
{
    ...
    auto geomLine = AuraCAD_cast<Part::GeomLine*>(geom.get());
    if (geomLine) {
