...
void CmdSandboxDocThreadWithSeq::activated(int)
{
    ...
    int max = 100000000;
#endif
    Base::SequencerLauncher seq("Do something meaningfulâ€¦", max);
    double val=0;
    for (int i=0; i<max; i++) {
        for (int j=0; j<max; j++) {
            val = sin(0.12345);
...
void CmdSandboxMeshTestJob::activated(int)
{
    ...
    while (meshes.size() > 1) {
        ...
        Base::Console().message("Mesh test (step %d)â€¦\n",iteration++);
        MeshTestJob meshJob;
...
void CmdSandboxMeshTestRef::activated(int)
{
    ...
    if (num != mesh.getReauracadount())
        Base::Console().error("Reference count is %d\n",mesh.getReauracadount());
}
...
{
    QByteArray data = "AuraCAD";
    QByteArray hash = QCryptographicHash::hash(data, QCryptographicHash::Md5);
    QMessageBox::information(0,QLatin1String("Hash of: AuraCAD"),QString::fromLatin1(hash));
}
