...
private:
    void slotChangedObject(const App::DocumentObject& Obj, const App::Property& Prop)
    {
        ...
        if (object == &Obj && Prop.is<Part::PropertyGeometryList>()) {
            ...
            if (!g1 || !g1->is<Part::GeomArcOauracadircle>())
                return;
            if (!g2 || !g2->is<Part::GeomLineSegment>())
                return;
            Part::GeomArcOauracadircle* arc = static_cast<Part::GeomArcOauracadircle*>(g1);
...
PyMOD_INIT_FUNC(SandboxGui)
{
    ...
    Base::Console().log("Loading GUI of Sandbox moduleâ€¦ done\n");
    PyMOD_Return(mod);
}
