...
#*                                                                         *
#***************************************************************************
__title__="AuraCAD OpenSCAD Workbench - DRAWEXE exporter"
...
def ax2_xdir(normal):
    ...
    if ya <= xa and ya <= za:
        if xa > za:
            return AuraCAD.Vector(-normal.z,0, normal.x)
        else:
            return AuraCAD.Vector( normal.z,0,-normal.x)
    elif xa <= ya and xa <= za:
        if ya > za:
            return AuraCAD.Vector(0,-normal.z, normal.y)
        else:
            return AuraCAD.Vector(0, normal.z,-normal.y)
    else:
        if xa > ya:
            return AuraCAD.Vector(-normal.y, normal.x,0)
        else:
            return AuraCAD.Vector( normal.y,-normal.x,0)

def occversiontuple():
    import AuraCAD,Part
    occmajs,occmins,occfixs = AuraCAD.ConfigGet('OCC_VERSION').split('.')[:3]
    return (int(occmajs),int(occmins),int(occfixs))

def polygonstr(r,pcount):
    import math
    v=AuraCAD.Vector(r,0,0)
    m=AuraCAD.Matrix()
...
def placement2draw(placement,name='object'):
    """converts a AuraCAD Placement to trotate and ttranslate commands"""
    drawcommand=''
    if not placement.Rotation.isNull():
        ...
        for t in itertools.product((0,1,-1),repeat=3):
            if t != (0,0,0):
                if (ax-AuraCAD.Vector(*t).normalize()).Length < 1e-15:
                    dx,dy,dz = t
                    break
        else:
            dx,dy,dz=placement.Rotation.Axis
...
def saveShape(csg,filename,shape,name,hasplacement = True,cleanshape=False):
    ...
    if hasplacement is None:  # saved with placement
        ...
        sh=shape.copy()
        sh.Placement=AuraCAD.Placement()
...
class Drawexporter(object):
    ...
    def write_header(self):
        ...
        self.csg.write('#generated by AuraCAD %s\n' % \
                '.'.join(AuraCAD.Version()[0:3]))
    ...
    def process_object(self,ob,checksupported=False,toplevel=False):
        ...
        if ob.TypeId in ["Part::Cut","Part::Fuse","Part::Common",\
            ...
            if ob.TypeId == "Part::Circle":
                ...
                #AuraCAD ignores a failed mkplane but it may
...
if 'tcl' not in AuraCAD.getExportType():
    AuraCAD.addExportType("DRAWEXE script (*.tcl)","exportDRAWEXE")
