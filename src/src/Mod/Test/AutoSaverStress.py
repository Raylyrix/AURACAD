...
This module is intentionally not registered in AuraCAD.__unit_test__, so it is
only run when requested explicitly.

Examples:
    AuraCAD src/Mod/Test/RunAutoSaverStress.py
    AuraCAD src/Mod/Test/RunAutoSaverStress.py documents=3 objects=150
    AuraCAD src/Mod/Test/RunAutoSaverStress.py iterations=25 burst=10 timeout=5.0
"""
...
class TestAutoSaverStress(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for index in range(self.document_count):
            document = AuraCAD.newDocument(f"AutoSaverStressDoc{index}")
            self.assertIsNotNone(AuraCADGui.getDocument(document.Name))
            objects = [
    ...
    def tearDown(self):
        for document, _objects in reversed(self.documents):
            if AuraCAD.getDocument(document.Name):
                AuraCAD.closeDocument(document.Name)
    ...
    def _recoveryArchive(self, document):
        return os.path.join(document.TransientDir, "AuraCAD_recovery_file.auracadstd")
