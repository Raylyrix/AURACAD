...
The test is intended to be executed via AuraCAD's test runner:

  AuraCAD -t TestCoinNodeSnapshots

Environment variables:
  - AuraCAD_VISUAL_OUT_DIR: output directory (writes actual/expected/diff)
  - AuraCAD_VISUAL_BASELINE_DIR: baseline directory override (default: tests/visual/baselines/coin-nodes)
  - AuraCAD_VISUAL_UPDATE_BASELINE: if truthy, overwrite baselines with actual renders
  - AuraCAD_VISUAL_WIDTH / AuraCAD_VISUAL_HEIGHT: render size (default: 512x512)
  - AuraCAD_VISUAL_TOLERANCE: per-channel tolerance (default: 8)
  - AuraCAD_VISUAL_MAX_MISMATCH_PCT: mismatch threshold percent (default: 0.20)
  - AuraCAD_VISUAL_IGNORE_ALPHA: ignore alpha differences (default: 1)
  - AuraCAD_VISUAL_NODES: optional comma-separated node list to run
"""

# This file is intentionally tolerant of missing optional bindings and older
# AuraCAD/Coin/Pivy environments; it is a test harness rather than library code.
...
def _baseline_dir(create: bool = False) -> Path:
    baseline_dir_env = os.environ.get("AuraCAD_VISUAL_BASELINE_DIR", "").strip()
    if baseline_dir_env:
    ...
    raise FileNotFoundError(
        f"baseline directory not found (expected {_BASELINE_REL}; set AuraCAD_VISUAL_BASELINE_DIR to override)"
    )
...
def _configure_fontconfig_for_visual_fonts(font_dir: Path):
    ...
    tmp = Path(tempfile.mkdtemp(prefix="AuraCAD-visual-fontconfig-"))
    conf = tmp / "fonts.conf"
...
def _register_visual_fonts():
    ...
    # Register for native font backends (important on Windows, where AuraCAD forces Coin
...
def _configure_software_gl_for_snapshots():
    ...
    force = os.environ.get("AuraCAD_VISUAL_FORCE_SOFTWARE_GL", "1").strip().lower()
...
def _require_gui():
    try:
        import AuraCAD  # type: ignore
    except Exception as exc:  # pragma: no cover
        raise RuntimeError("AuraCAD module not available") from exc

    _configure_software_gl_for_snapshots()

    # `AuraCADCmd -t 0` runs the base test suite without X11/Wayland. Creating a Qt application
    ...
    try:
        import AuraCADGui  # type: ignore
    except Exception as exc:  # pragma: no cover
        raise unittest.SkipTest("AuraCADGui not available in this build") from exc
    ...
    setup_without_gui = getattr(AuraCADGui, "setupWithoutGUI", None)
    ...
    return AuraCAD, AuraCADGui, coin
...
def _make_scene_for_node(coin, type_name: str):
    ...
    if type_name == "SoauracadBackgroundGradient":
        grad = _instantiate(coin, "SoauracadBackgroundGradient")
    ...
    if type_name in ("SoNaviCube", "SoNaviCubeTranslucent", "SoNaviCubeHiliteFront"):
        ...
        width = float(int(os.environ.get("AuraCAD_VISUAL_WIDTH", "512")))
        height = float(int(os.environ.get("AuraCAD_VISUAL_HEIGHT", "512")))
    ...
    if type_name in (
        ...
        "SoauracadIndexedFaceSetPerFaceColor",
        "SoauracadIndexedFaceSetPerVertexColor",
        "SoauracadIndexedFaceSetTranslucent",
    ):
    ...
    if type_name in (
        ...
        # SoauracadIndexedFaceSet test geometry does not provide normals. Without normals, lighting
        ...
        if type_name == "SoauracadIndexedFaceSetTranslucent":
            # Provide visible background so translucency can be verified.
            grad = _instantiate(coin, "SoauracadBackgroundGradient")
            if hasattr(grad, "setColorGradient"):
                top = coin.SbColor(0.15, 0.15, 0.20)
        ...
        faces = _instantiate(coin, "SoauracadIndexedFaceSet")
        # fmt: off
        ...
        # fmt: on
        material = coin.SoMaterial()
        if type_name == "SoauracadIndexedFaceSetPerFaceColor":
            # 12 faces (triangles).
            ...
            root.addChild(bind)
        elif type_name == "SoauracadIndexedFaceSetPerVertexColor":
            material.diffuseColor.setValues(
                0,
                8,
                [
    ...
    if type_name == "SoauracadControlPoints":
        ...
        cp = _instantiate(coin, "SoauracadControlPoints")
...
def _render_png(AuraCADGui, coin, root, out_path: Path, width: int, height: int) -> None:
    ...
    off = AuraCADGui.SoQtOffscreenRenderer(width, height)
...
class CoinNodeSnapshotTestCase(unittest.TestCase):
    ...
    def test_coin_node_snapshots(self):
        ...
        _, AuraCADGui, coin = _require_gui()

        nodes_env = os.environ.get("AuraCAD_VISUAL_NODES", "")
        if nodes_env.strip():
        ...
        width = int(os.environ.get("AuraCAD_VISUAL_WIDTH", "512"))
        height = int(os.environ.get("AuraCAD_VISUAL_HEIGHT", "512"))

        out_dir = Path(
            os.environ.get(
                "AuraCAD_VISUAL_OUT_DIR",
                os.path.join(tempfile.gettempdir(), "AuraCADTesting", "CoinNodeSnapshots"),
            )
        )

        update_baseline = os.environ.get("AuraCAD_VISUAL_UPDATE_BASELINE", "").strip() not in (
            "",
            "0",
            "false",
            "False",
        ...
        if not baseline_dir.is_dir():
            self.fail(
                ...
                "(set AuraCAD_VISUAL_BASELINE_DIR or run with AuraCAD_VISUAL_UPDATE_BASELINE=1)"
            )

        tolerance = int(os.environ.get("AuraCAD_VISUAL_TOLERANCE", "8"))
        tolerance = max(0, min(tolerance, 255))
        ignore_alpha = os.environ.get("AuraCAD_VISUAL_IGNORE_ALPHA", "1").strip() not in (
            "",
            "0",
            "false",
            "False",
        )
        max_mismatch_pct = float(os.environ.get("AuraCAD_VISUAL_MAX_MISMATCH_PCT", "0.20"))
        ...
        for type_name in node_types:
            with self.subTest(node=type_name):
                ...
                _render_png(AuraCADGui, coin, root, actual_path, width, height)
                ...
                if not baseline_path.exists():
                    if smoke_mode:
                        continue
                    self.fail(
                        f"missing baseline: {baseline_path} (run with AuraCAD_VISUAL_UPDATE_BASELINE=1)"
                    )
