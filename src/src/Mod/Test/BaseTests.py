...
class ConsoleTestCase(unittest.TestCase):
    ...
    def testPrint(self):
        AuraCAD.Console.PrintMessage("   Printing message\n")
        AuraCAD.Console.PrintError("   Printing error\n")
        AuraCAD.Console.PrintWarning("   Printing warning\n")
        AuraCAD.Console.PrintLog("   Printing Log\n")
    ...
    def testSynchronPrintFromThread(self):
        ...
        def adder():
            ...
            AuraCAD.Console.PrintMessage("Call from Python thread: count=" + str(self.count) + "\n")
            lock.release()
        ...
        AuraCAD.Console.PrintMessage(str(self.count) + "\n")
...
class ParameterTestCase(unittest.TestCase):
    def setUp(self):
        self.TestPar = AuraCAD.ParamGet("System parameter:Test")

    def testGroup(self):
        # AuraCAD.Console.PrintLog("Base::ParameterTestCase::testGroup\n")
    ...
    def testExportImport(self):
        ...
        TempPath = tempfile.gettempdir() + os.sep + "ExportTest.auracadExport"
    ...
    def tearDown(self):
        # remove all
        TestPar = AuraCAD.ParamGet("System parameter:Test")
        TestPar.Clear()
...
class AlgebraTestCase(unittest.TestCase):
    ...
    def testRotationFromMatrix(self):
        rot = AuraCAD.Rotation(45, 30, 0)
        m_r = rot.toMatrix()
        m_r.move(5, 6, 7)
        m_s = AuraCAD.Matrix()
        m_s.scale(1, 1, 3)
        m_s.move(7, 3, 2)
        target_rot = AuraCAD.Rotation(45, 60, 0)
        err = "Non uniform scale has wrong affect non orthogonal rotation"
        self.assertTrue(AuraCAD.Rotation(m_s * m_r).isSame(target_rot, 1e-12), err)
        err = "Right multiplication with non uniform scale must not affect rotation"
        self.assertTrue(AuraCAD.Rotation(m_r * m_s).isSame(rot, 1e-12), err)
        m_r.scale(-2)
        err = "Uniform scale must not affect rotation"
        self.assertTrue(AuraCAD.Rotation(m_r).isSame(rot, 1e-12), err)

    def testRotation(self):
        r = AuraCAD.Rotation(1, 0, 0, 0)  # 180 deg around (1,0,0)
        self.assertEqual(r.Axis, AuraCAD.Vector(1, 0, 0))
        self.assertAlmostEqual(math.fabs(r.Angle), math.fabs(math.pi))

        r = r.multiply(r)  # identity
        self.assertEqual(r.Axis, AuraCAD.Vector(0, 0, 1))
        self.assertAlmostEqual(r.Angle, 0)

        r = AuraCAD.Rotation(1, 0, 0, 0)
        r.Q = (0, 0, 0, 1)  # update axis and angle
        s = AuraCAD.Rotation(0, 0, 0, 1)
        self.assertEqual(r.Axis, s.Axis)
        self.assertAlmostEqual(r.Angle, s.Angle)
        self.assertTrue(r.isSame(s))

        r = AuraCAD.Rotation(1, 0, 0, 0)
        r.Matrix = AuraCAD.Matrix()  # update axis and angle
        s = AuraCAD.Rotation(0, 0, 0, 1)
        self.assertEqual(r.Axis, s.Axis)
        self.assertAlmostEqual(r.Angle, s.Angle)
        self.assertTrue(r.isSame(s))
        ...
        r = AuraCAD.Rotation(1, 0, 0, 0)
        r.Axes = (AuraCAD.Vector(0, 0, 1), AuraCAD.Vector(0, 0, 1))  # update axis and angle
        s = AuraCAD.Rotation(0, 0, 0, 1)
        self.assertEqual(r.Axis, s.Axis)
        self.assertAlmostEqual(r.Angle, s.Angle)
        self.assertTrue(r.isSame(s))
        ...
        r = AuraCAD.Rotation(AuraCAD.Vector(1, 0, 0), 270)
        s = AuraCAD.Rotation(AuraCAD.Vector(1, 0, 0), 270 + 360)
        self.assertEqual(r.Axis, s.Axis)
        # self.assertAlmostEqual(r.Angle, s.Angle + 2*math.pi)
        self.assertTrue(r.isSame(s))

        # subtract 360 deg from angle using Euler angles
        r = AuraCAD.Rotation(0, 0, 180)
        r.invert()
        s = AuraCAD.Rotation(0, 0, -180)
        self.assertTrue(r.isSame(s))

        # subtract 360 deg from angle using quaternion
        r = AuraCAD.Rotation(1, 0, 0, 0)
        s = AuraCAD.Rotation(-1, 0, 0, 0)
    ...
    def testDecomposeScale(self):
        mat = AuraCAD.Matrix()
        mat.scale(AuraCAD.Vector(2, 3, 4))
        res = mat.decompose()
    ...
    def testMatrixDecompose(self):
        ...
        # scaling
        self.assertEqual(res[1].hasScale(), AuraCAD.ScaleType.NonUniformLeft)

        # rotation
        rot1 = AuraCAD.Rotation(res[2])
        rot2 = AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), 45)
        self.assertTrue(rot1.isSame(rot2, 1e-12))

        # translation
        plm = AuraCAD.Placement(res[3])
        self.assertEqual(plm.Base, AuraCAD.Vector(1, 2, 3))
    ...
    def testYawPitchRoll(self):
        ...
        def getYPR2(yaw, pitch, roll):
            ...
            rx.Axis = AuraCAD.Vector(1, 0, 0)
            ry.Axis = AuraCAD.Vector(0, 1, 0)
            rz.Axis = AuraCAD.Vector(0, 0, 1)
    ...
    def testBounding(self):
        ...
        self.assertEqual(b.Center, AuraCAD.Vector(0, 0, 0), "Center is not at (0,0,0)")
        self.assertTrue(b.isInside(b.Center), "Center is not inside Bbox")
        b.add(2, 2, 2)
        self.assertTrue(
            b.isInside(b.getIntersectionPoint(b.Center, AuraCAD.Vector(0, 1, 0))),
            "Intersection point is not inside Bbox",
        )
        self.assertTrue(b.intersect(b), "Bbox doesn't intersect with itself")
        self.assertFalse(
            b.intersected(AuraCAD.BoundBox(4, 4, 4, 6, 6, 6)).isValid(),
            "Bbox should not intersect with Bbox outside",
        )
        self.assertEqual(
            b.intersected(AuraCAD.BoundBox(-2, -2, -2, 2, 2, 2)).Center,
            b.Center,
...
class MatrixTestCase(unittest.TestCase):
    def setUp(self):
        self.mat = AuraCAD.Matrix()

    def testOrder(self):
        self.mat = AuraCAD.Matrix(1.0, 2.0, 3.0, 4.0)
    ...
    def testRotation(self):
        rot = AuraCAD.Rotation()
        res = self.mat * rot
        self.assertEqual(type(res), AuraCAD.Matrix)
    ...
    def testMatrixPlacementMatrix(self):
        # Example taken from https://forum.AuraCAD.org/viewtopic.php?f=3&t=61000
        mat = AuraCAD.Matrix(
            -0.470847778020266,
            0.8150598976807029,
            0.3376088628746235,
        ...
        plm = AuraCAD.Placement(mat)
        mat = plm.toMatrix()
        self.assertEqual(mat.hasScale(), AuraCAD.ScaleType.NoScaling)
    ...
    def testUnity(self):
        mat = AuraCAD.Matrix(2, 0, 0, 0, 0, 1, 0, 0, 0, 0, 2, 0, 0, 0, 0, -1)
    ...
    def testScale(self):
        ...
        self.assertEqual(self.mat.hasScale(), AuraCAD.ScaleType.NonUniformLeft)
        self.mat.unity()
        self.assertEqual(self.mat.hasScale(), AuraCAD.ScaleType.NoScaling)
        self.mat.scale(2.0, 2.0, 2.0)
        self.assertEqual(self.mat.hasScale(), AuraCAD.ScaleType.Uniform)
        ...
        self.mat.scale(1.0, 2.0, 3.0)
        self.mat.rotateX(1.0)
        self.assertEqual(self.mat.hasScale(), AuraCAD.ScaleType.NonUniformRight)
        self.mat.unity()
        self.mat.setCol(0, AuraCAD.Vector(1, 2, 3))
        self.mat.setCol(1, AuraCAD.Vector(1, 2, 3))
        self.mat.setCol(2, AuraCAD.Vector(1, 2, 3))
        self.assertEqual(self.mat.hasScale(), AuraCAD.ScaleType.Other)
        self.mat.unity()
        self.mat.setRow(0, AuraCAD.Vector(1, 2, 3))
        self.mat.setRow(1, AuraCAD.Vector(1, 2, 3))
        self.mat.setRow(2, AuraCAD.Vector(1, 2, 3))
        self.assertEqual(self.mat.hasScale(), AuraCAD.ScaleType.Other)
        self.mat.unity()
        self.mat.scale(-1.0)
        self.assertEqual(self.mat.hasScale(), AuraCAD.ScaleType.Uniform)
    ...
    def testShearing(self):
        self.mat.setRow(1, AuraCAD.Vector(0, 1, 1))
        self.assertEqual(self.mat.hasScale(), AuraCAD.ScaleType.Other)
    ...
    def testMultLeftOrRight(self):
        ...
        self.assertEqual((mat1 * mat2).hasScale(), AuraCAD.ScaleType.NonUniformRight)
        self.assertEqual((mat2 * mat1).hasScale(), AuraCAD.ScaleType.NonUniformLeft)
    ...
    def testColRow(self):
        ...
        with self.assertRaises(TypeError):
            self.mat.row("string")
        self.assertEqual(type(self.mat.col(0)), AuraCAD.Vector)
        self.assertEqual(type(self.mat.row(0)), AuraCAD.Vector)
        self.mat.setCol(0, AuraCAD.Vector(1, 0, 0))
        self.mat.setRow(0, AuraCAD.Vector(1, 0, 0))

    def testDiagonal(self):
        self.mat.scale(2.0, 2.0, 2.0)
        self.assertEqual(self.mat.diagonal(), AuraCAD.Vector(2.0, 2.0, 2.0))
