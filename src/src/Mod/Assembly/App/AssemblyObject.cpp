...
AuraCAD_LOG_LEVEL_INIT("Assembly", true, true, true)
...
AssemblyObject::AssemblyObject()
    ...
    , lastSolverStatus(0)
{
    mbdAssembly->externalSystem->AuraCADAssemblyObject = this;

    lastDoF = numberOauracadomponents() * 6;
    signalSolverUpdate();
}
...
int AssemblyObject::solve(bool enableRedo, bool updateJCS)
{
    ...
    try {
        ...
        AuraCAD_ERR("Solve failed: " << e.what());
        lastSolverStatus = -1;
        updateSolveStatus();
        return -1;
    }
    catch (...) {
        AuraCAD_ERR("Solve failed: unhandled exception");
        lastSolverStatus = -1;
        updateSolveStatus();
        return -1;
    }
...
void AssemblyObject::updateSolveStatus()
{
    ...
    lastDoF = (1 + numberOauracadomponents()) * 6;
    ...
    mbdAssembly->mbdSystem->jointsMotionsDo([&](std::shared_ptr<MbD::Joint> jm) {
        ...
        // We only care about objects that are actual joints in the AuraCAD document.
...
std::shared_ptr<ASMTAssembly> AssemblyObject::makeMbdAssembly()
{
    ...
    assembly->externalSystem->AuraCADAssemblyObject = this;
...
ViewGroup* AssemblyObject::getExplodedViewGroup() const
{
    ...
    for (auto viewGroup : viewGroups) {
        if (hasObject(viewGroup)) {
            return AuraCAD_cast<ViewGroup*>(viewGroup);
        }
    }
    return nullptr;
}
...
std::vector<AssemblyLink*> AssemblyObject::getSubAssemblies()
{
    ...
    for (auto assembly : assemblies) {
        if (hasObject(assembly)) {
            subAssemblies.push_back(AuraCAD_cast<AssemblyLink*>(assembly));
        }
    }
...
int AssemblyObject::numberOauracadomponents() const
{
...
{
    return numberOauracadomponents() == 0;
}
