...
JointGroup* getJointGroup(const App::Part* part)
{
    ...
    for (auto jointGroup : jointGroups) {
        if (part->hasObject(jointGroup)) {
            return AuraCAD_cast<JointGroup*>(jointGroup);
        }
    }
    return nullptr;
}
        ...
        if (nextIt != names.end()) {
            for (auto* obji : obj->getOutList()) {
                if (*nextIt == obji->getNameInDocument() && isBodySubObject(obji)) {
                    ...
                    if (auto* lcs = AuraCAD_cast<App::LocalCoordinateSystem*>(obji)) {
...
)
{
    for (auto* obj : objects) {
        ...
        if (auto* asmLink = AuraCAD_cast<Assembly::AssemblyLink*>(obj)) {
        ...
        else if (auto* group = AuraCAD_cast<App::DocumentObjectGroup*>(obj)) {
            collectComponentsRecursively(group->Group.getValues(), results);
            continue;
        }
        else if (auto* link = AuraCAD_cast<App::Link*>(obj)) {
