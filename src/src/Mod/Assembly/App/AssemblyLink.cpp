...
void AssemblyLink::synchronizeComponents()
{
    ...
    for (auto* obj : topLevelComponents) {
        ...
        for (auto* obj2 : assemblyLinkGroup) {
            ...
            auto* subAsmLink = AuraCAD_cast<AssemblyLink*>(obj2);
...
{
    auto sourceProp = AuraCAD_cast<T*>(source->getPropertyByName(propertyName));
    auto targetProp = AuraCAD_cast<T*>(target->getPropertyByName(propertyName));
...
App::DocumentObject* AssemblyLink::getLinkedObject2(bool recursive) const
{
    ...
    auto* assembly = AuraCAD_cast<AssemblyObject*>(obj);
    if (assembly) {
        return assembly;
    }
    else {
        auto* assemblyLink = AuraCAD_cast<AssemblyLink*>(obj);
        if (assemblyLink) {
            if (recursive) {
...
{
    return AuraCAD_cast<AssemblyObject*>(getLinkedObject2());
}
...
int AssemblyLink::numberOauracadomponents() const
{
    return isRigid() ? 1 : getLinkedAssembly()->numberOauracadomponents();
}
