...
)
{
    ...
    for (auto* child : objs) {
        ...
        if (auto* asmLink = AuraCAD_cast<AssemblyLink*>(child)) {
...
std::string BomObject::getBomPropertyValue(App::DocumentObject* obj, const std::string& baseName)
{
    ...
    if (auto propStr = AuraCAD_cast<App::PropertyString*>(prop)) {
        return propStr->getValue();
    }
    if (auto propQuantity = AuraCAD_cast<App::PropertyQuantity*>(prop)) {
        return propQuantity->getQuantityValue().getUserString();
    }
    if (auto propEnum = AuraCAD_cast<App::PropertyEnumeration*>(prop)) {
        return propEnum->getValueAsString();
    }
    if (auto propFloat = AuraCAD_cast<App::PropertyFloat*>(prop)) {
        return std::to_string(propFloat->getValue());
    }
    if (auto propInt = AuraCAD_cast<App::PropertyInteger*>(prop)) {
        return std::to_string(propInt->getValue());
    }
    if (auto propBool = AuraCAD_cast<App::PropertyBool*>(prop)) {
