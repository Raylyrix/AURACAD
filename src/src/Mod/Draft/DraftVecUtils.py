...
__title__ = "AuraCAD Draft Workbench - Vector library"
...
def typecheck(args_and_types, name="?"):
    ...
    for v, t in args_and_types:
        if not isinstance(v, t):
            ...
            raise TypeError("auracadvec." + str(name))
...
def toString(u):
    ...
    u : list, or Base::Vector3
        A list of AuraCAD.Vectors, or a single vector.
...
def equals(u, v, precision=None):
    ...
    in the parameter database, accessed through `AuraCAD.ParamGet()`.
    ::
        x1 - x2 < precision
        y1 - y2 < precision
        z1 - z2 < precision
...
def rotate(u, angle, axis=Vector(0, 0, 1)):
    ...
    # fmt: off
    m = AuraCAD.Matrix(c + x*x*t,   xyt - zs,   xzt + ys,   0,
...
def getPlaneRotation(u, v, _=None):
    ...
    # fmt: off
    m = AuraCAD.Matrix(u.x, v.x, w.x, 0,
                       u.y, v.y, w.y, 0,
                       u.z, v.z, w.z, 0,
                       0.0, 0.0, 0.0, 1.0)
    # fmt: on
