...
__title__ = "AuraCAD Draft Workbench - OCA importer/exporter"
__author__ = "Yorik van Havre <yorik@uncreated.net>"
__url__ = "https://www.AuraCAD.org"

import AuraCAD, os, Part, DraftVecUtils, DraftGeomUtils
...
def getpoint(data):
    """Turn an OCA point definition into a AuraCAD.Vector.
    ...
    """
    auracadC.PrintMessage("found point %s \n" % data)
    if len(data) == 3:
...
def getarea(data):
    """Turn an OCA area definition into a AuraCAD Part.Wire.
    ...
    """
    auracadC.PrintMessage("found area %s \n" % data)
    if data[0] == "S":
        if data[1] == "POL":
            pts = data[2:]
            verts = []
...
def getarc(data):
    """Turn an OCA arc definition into a AuraCAD Part.Edge.
    ...
    """
    auracadC.PrintMessage("found arc %s \n" % data)
    c = None
    if data[0] == "ARC":
        # 3-points arc
        pts = data[1:]
...
def getline(data):
    """Turns an OCA line definition into a AuraCAD Part.Edge.
    ...
    Part.Edge
        An edge object from the points in `data`.
    """
    auracadC.PrintMessage("found line %s \n" % data)
    verts = []
    for p in range(len(data)):
        if data[p] == "P":
            verts.append(getpoint(data[p : p + 4]))
...
def gettranslation(data):
    ...
    """
    auracadC.PrintMessage("found translation %s \n" % data)
    if data[0] == "Z":
...
def writepoint(vector):
    """Write a AuraCAD.Vector in OCA format.
...
def createobject(oid, doc):
    ...
    doc : App::Document
        A AuraCAD document to which the object is added.
    Returns
    -------
    None
    """
...
def export(exportList, filename):
    ...
    if not (edges or faces):
        auracadC.PrintMessage(translate("importOCA", "OCA: found no data to export") + "\n")
        return

    # writing file
    oca = pyopen(filename, "w")
    oca.write("#oca file generated from AuraCAD\r\n")
    oca.write("# edges\r\n")
    count = 1
    for e in edges:
    ...
    # closing
    oca.close()
    auracadC.PrintMessage(translate("importOCA", "successfully exported") + " " + filename + "\n")
