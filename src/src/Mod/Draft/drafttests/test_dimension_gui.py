...
# ***************************************************************************
# *   Copyright (c) 2026 AuraCAD Project Association                        *
# *                                                                         *
...
class DraftGuiDimension(test_base.DraftTestCaseDoc):
    ...
    AuraCAD_NAMESPACE = "https://www.AuraCAD.org/ns"
    ...
    def extractOvershootLines(self, svg_text):
        ...
        root = ET.fromstring(f'<svg xmlns:AuraCAD="{self.AuraCAD_NAMESPACE}">{svg_text}</svg>')
        lines = []
        for element in root.iter():
            ...
            skip_value = next(
                (
                    ...
                    if key == "AuraCAD:skip" or key == "skip" or key.endswith("}skip")
                ),
                None,
            )
            if skip_value != "1":
    ...
    def test_angular_dimension_draws_overshoots_after_restore(self):
        ...
        temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=".auracadStd")
        temp_file.close()
        reopened = None
        try:
