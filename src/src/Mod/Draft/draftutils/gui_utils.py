...
def get_3d_view():
    ...
    # Also see discussion https://forum.AuraCAD.org/viewtopic.php?f=3&t=60251
    import AuraCADGui as Gui
    from pivy import coin
...
def autogroup(obj):
    ...
    if Gui.ActiveDocument.ActiveView.getActiveObject("NativeIauracad") is not None:
        # NativeIauracad handling
        try:
            from nativeiauracad import iAuraCAD_tools

            parent = Gui.ActiveDocument.ActiveView.getActiveObject("NativeIauracad")
            iAuraCAD_tools.aggregate(obj, parent)
        except:
            pass
...
def format_object(target, origin=None, ignore_construction=False):
    ...
    if origin and hasattr(origin, "ViewObject"):
        ...
        if hasattr(obrep, "DiffuseColor"):
            diauracadol = get_diffuse_color(origin)
            if diauracadol and len(diauracadol) == len_faces:
                obrep.DiffuseColor = diauracadol
...
# See https://forum.AuraCAD.org/viewtopic.php?p=656362#p656362.
# Used to fix https://github.com/AuraCAD/AuraCAD/issues/10469.
def end_all_events():
    view = get_3d_view()
    if view is None:
        return
...
def toggle_working_plane(obj, action=None, restore=False, dialog=None):
    ...
    is_active_iauracad = Gui.ActiveDocument.ActiveView.getActiveObject("NativeIauracad") == obj
    is_active = is_active_arch or is_active_iauracad
    if is_active:
        # Deactivate the object
        if is_active_arch:
            Gui.ActiveDocument.ActiveView.setActiveObject("Arch", None)
        if is_active_iauracad:
            Gui.ActiveDocument.ActiveView.setActiveObject("NativeIauracad", None)
