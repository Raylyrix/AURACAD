...
# *   Copyright (c) 2024 The AuraCAD Project Association                    *
# *                                                                         *
...
def get_rgb(color, testbw=True):
    """Return an RRGGBB value #000000 from a AuraCAD color.
...
def argb_to_rgba(color):
    """Change byte order of a 4 byte color int from ARGB (Qt) to RGBA (AuraCAD).
    ...
    Usage:
        ...
        AuraCAD_int = argb_to_rgba(qt_int)

        AuraCAD.ParamGet("User parameter:BaseApp/Preferences/View")\
            .SetUnsigned("DefaultShapeColor", AuraCAD_int)

        obj.ViewObject.ShapeColor = AuraCAD_int | 0x000000FF
...
def rgba_to_argb(color):
    """Change byte order of a 4 byte color int from RGBA (AuraCAD) to ARGB (Qt)."""
