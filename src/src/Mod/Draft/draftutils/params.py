...
def _param_from_PreauracadheckBox(widget):
    value = False
...
def _param_from_PrefRadioButton(widget):
    return _param_from_PreauracadheckBox(widget)


def _param_from_PreauracadomboBox(widget):
    value = 0
    for elem in list(widget):
        if "name" in elem.keys():
            att_name = elem.attrib["name"]
...
def _param_from_PreauracadolorButton(widget):
    for elem in list(widget):
        if "name" in elem.keys():
            att_name = elem.attrib["name"]
            if att_name == "color":
...
def _get_shape_string_font_file():
    ...
    # https://forum.AuraCAD.org/viewtopic.php?t=96770
    # Windows font: "arial"
...
def _get_param_dictionary():
    ...
    hatch_pattern_file = (
        App.getResourceDir().replace("\\", "/").rstrip("/") + "/Mod/TechDraw/PAT/auracadPAT.pat"
    )
    ...
    param_dict["Mod/Arch"] = {
        ...
        "CustomIauracadSchema":             ("string",    ""),     # importIauracadlegacy.py
        "createIauracadGroups":             ("bool",      False),  # importIauracadlegacy.py
        "DoorHeight":                  ("float",     2100.0),
        "DoorPreset":                  ("int",       5),
        "DoorSill":                    ("float",     0.0),
        "DoorWidth":                   ("float",     1000.0),
        "FreeLinking":                 ("bool",      False),
        "forceIauracadPythonParser":        ("bool",      False),  # importIauracadlegacy.py
        "getStandardType":             ("bool",      False),
        "iauracadAggregateWindows":         ("bool",      False),  # importIauracadlegacy.py
        "iauracadAsMesh":                   ("string",    ""),     # importIauracadlegacy.py
        "IauracadExportList":               ("bool",      False),  # importIauracadlegacy.py
        "iauracadImportLayer":              ("bool",      True),
        "iauracadJoinSolids":               ("bool",      False),  # importIauracadlegacy.py
        "iauracadMergeProfiles":            ("bool",      False),
        "IauracadScalingFactor":            ("float",     1.0),    # importIauracadlegacy.py
        "iauracadSeparatePlacements":       ("bool",      False),  # importIauracadlegacy.py
    ...
    for fnm in (
        ...
        ":/ui/preferences-iauracad.ui",
        ":/ui/preferences-iauracad-export.ui",
        ...
        for widget in root.iter("widget"):
            if "class" in widget.keys():
                ...
                if att_class == "Gui::PreauracadheckBox":
                    path, entry, value = _param_from_PreauracadheckBox(widget)
                    typ = "bool"
                elif att_class == "Gui::PrefRadioButton":
                    path, entry, value = _param_from_PrefRadioButton(widget)
                    typ = "bool"
                elif att_class == "Gui::PreauracadomboBox":
                    path, entry, value = _param_from_PreauracadomboBox(widget)
                    typ = "int"
                    ...
                    typ = "string"
                elif att_class == "Gui::PreauracadolorButton":
                    path, entry, value = _param_from_PreauracadolorButton(widget)
                    typ = "unsigned"
                    ...
                    typ = "string"
                elif att_class == "Gui::PreauracadheckableGroupBox":
                    # It's a boolean preference, so we can reuse the parsing logic
                    # from _param_from_PreauracadheckBox, which looks for <property name="checked">.
                    path, entry, value = _param_from_PreauracadheckBox(widget)
                    typ = "bool"
