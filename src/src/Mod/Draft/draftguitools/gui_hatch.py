...
"""This module contains AuraCAD commands for the Draft workbench"""
...
class Draft_Hatch(gui_base.GuiCommandNeedsSelection):
    ...
    def Activated(self):
        ...
        if AuraCADGui.Selection.getSelection():
            task = AuraCADGui.Control.showDialog(
                Draft_Hatch_TaskPanel(AuraCADGui.Selection.getSelection()[0])
            )
...
class Draft_Hatch_TaskPanel:
    ...
    def __init__(self, baseobj):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/dialogHatch.ui")
    ...
    def accept(self):
        ...
        if hasattr(self.baseobj, "File") and hasattr(self.baseobj, "Pattern"):
            ...
            o = 'AuraCAD.ActiveDocument.getObject("' + self.baseobj.Name + '")'
            AuraCADGui.doCommand(o + ".File=" + repr(self.form.File.property("fileName")))
            AuraCADGui.doCommand(o + ".Pattern=" + repr(self.form.Pattern.currentText()))
            AuraCADGui.doCommand(o + ".Scale=" + str(self.form.Scale.value()))
            AuraCADGui.doCommand(o + ".Rotation=" + str(self.form.Rotation.value()))
            AuraCADGui.doCommand(o + ".Translate=" + str(self.form.Translate.isChecked()))
        else:
            # create new hatch object
            AuraCAD.ActiveDocument.openTransaction("Create Hatch")
            AuraCADGui.addModule("Draft")
            cmd = "Draft.make_hatch("
            cmd += 'baseobject=AuraCAD.ActiveDocument.getObject("' + self.baseobj.Name
    ...
    def onFileChanged(self, filename):
        ...
        if filename[0] == "." and self.baseobj:
            # File path relative to the AuraCAD file directory.
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("Draft_Hatch", Draft_Hatch())
