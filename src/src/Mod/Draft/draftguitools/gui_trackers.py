...
__title__ = "AuraCAD Draft Trackers"
...
class Tracker:
    ...
    def setColor(self, color=None):
        ...
        if color is not None:
            ...
            hasattr(AuraCAD, "activeDraftCommand")
            and AuraCAD.activeDraftCommand is not None
            and hasattr(AuraCAD.activeDraftCommand, "featureName")
            and AuraCAD.activeDraftCommand.featureName in ("Dimension", "Label", "Text")
        ):
    ...
    def _get_wp(self):
        return AuraCAD.DraftWorkingPlane
...
class snapTracker(Tracker):
    ...
    def __init__(self):
        ...
        self.marker.markerIndex = AuraCADGui.getMarkerIndex(
...
class arcTracker(Tracker):
    ...
    def setBy3Points(self, p1, p2, p3):
        ...
        try:
            arc = Part.ArcOauracadircle(p1, p2, p3)
        except Exception:
            return
        e = arc.toShape()
...
class ghostTracker(Tracker):
    ...
    def getMatrix(self):
        ...
        r = (
            AuraCADGui.ActiveDocument.ActiveView.getViewer()
            .getSoRenderManager()
            .getViewportRegion()
        )
        ...
        if m:
            m = m.getValue()
            # fmt: off
            return AuraCAD.Matrix(m[0][0], m[0][1], m[0][2], m[0][3],
                                  m[1][0], m[1][1], m[1][2], m[1][3],
                                  m[2][0], m[2][1], m[2][2], m[2][3],
                                  m[3][0], m[3][1], m[3][2], m[3][3])
            # fmt: on
        else:
            return AuraCAD.Matrix()
    ...
    def _do_flip(self, root):
        ...
        # Code by wmayer:
        # https://forum.AuraCAD.org/viewtopic.php?p=702640#p702640
...
class editTracker(Tracker):
    ...
    def __init__(
        ...
        if inactive:
            ...
            self.selnode = coin.SoType.fromName("SoauracadSelection").createInstance()
            if name:
                self.selnode.useNewSelection = False
                self.selnode.documentName.setValue(AuraCAD.ActiveDocument.Name)
...
class gridTracker(Tracker):
    ...
    def update(self):
        ...
        if self.space == 0:
            ...
            self.pts = []
            AuraCAD.Console.PrintWarning("Draft Grid: Spacing value is zero\n")
            return
        if self.mainlines == 0:
            self.lines1.numVertices.deleteValues(0)
            self.lines2.numVertices.deleteValues(0)
        ...
        if pts != self.pts:
            ...
            if params.get_param("gridBorder"):
                ...
                txt = AuraCAD.Units.Quantity(
                    self.space * self.mainlines, AuraCAD.Units.Length
                ).UserString
    ...
    def getGridColors(self):
        ...
        if params.get_param("coloredGridAxes"):
            vp = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/View")
    ...
    def displayHumanFigure(self, wp):
        ...
        if (
            ...
            and wp.axis.getAngle(AuraCAD.Vector(0, 0, 1)) < 0.001
        ):
            loc = AuraCAD.Vector(-bound + self.space / 2, -bound + self.space / 2, 0)
    ...
    def setAxesColor(self, wp):
        ...
        cols = [0, 0]
        if params.get_param("coloredGridAxes"):
            if round(wp.u.getAngle(AuraCAD.Vector(1, 0, 0)), 2) in (0, 3.14):
                cols[0] = 1
            elif round(wp.u.getAngle(AuraCAD.Vector(0, 1, 0)), 2) in (0, 3.14):
                cols[0] = 2
            elif round(wp.u.getAngle(AuraCAD.Vector(0, 0, 1)), 2) in (0, 3.14):
                cols[0] = 3
            if round(wp.v.getAngle(AuraCAD.Vector(1, 0, 0)), 2) in (0, 3.14):
                cols[1] = 1
            elif round(wp.v.getAngle(AuraCAD.Vector(0, 1, 0)), 2) in (0, 3.14):
                cols[1] = 2
            elif round(wp.v.getAngle(AuraCAD.Vector(0, 0, 1)), 2) in (0, 3.14):
                cols[1] = 3
    ...
    def reset(self):
        ...
        try:
            self.space = AuraCAD.Units.Quantity(params.get_param("gridSpacing")).Value
        except ValueError:
            self.space = 1
    ...
    def update(self, line=None, normal=None):
        ...
        if lvec.Length > tol and normal.Length > tol:
            ...
            if not lvec.isEqual(normal, tol) and not lvec.isEqual(normal.negative(), tol):
                rot = AuraCAD.Rotation(lvec, AuraCAD.Vector(), normal, "XZY")
...
class radiusTracker(Tracker):
    ...
    def __init__(self, position=AuraCAD.Vector(0, 0, 0), radius=1):
        self.trans = coin.SoTransform()
...
class archDimTracker(Tracker):
    ...
    def __init__(self, p1=AuraCAD.Vector(0, 0, 0), p2=AuraCAD.Vector(1, 0, 0), mode=1):
        import SketcherGui
    ...
    def setString(self, text=None):
        ...
        text = AuraCAD.Units.Quantity(self.Distance, AuraCAD.Units.Length).UserString
