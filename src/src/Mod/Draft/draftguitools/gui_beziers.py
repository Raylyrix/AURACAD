...
In particular, a cubic B̩zier curve is defined, as it is one of the most
...
class BezCurve(gui_lines.Line):
    """Gui command for the B̩zier Curve tool."""
    ...
    def GetResources(self):
        ...
        return {
            ...
            "Accel": "B, Z",
            "MenuText": QT_TRANSLATE_NOOP("Draft_BezCurve", "B̩zier Curve"),
            "ToolTip": QT_TRANSLATE_NOOP(
                "Draft_BezCurve",
                "Creates an n-degree B̩zier curve. The more points, the higher the degree.",
            ),
        }
    ...
    def Activated(self):
        ...
        Activate the specific B̩zier curve tracker.
        """
        super().Activated(
            name="BezCurve", icon="Draft_BezCurve", task_title=translate("draft", "B̩zier Curve")
        )
        if self.doc:
    ...
    def finish(self, cont=False, closed=False):
        ...
        if len(self.node) > 1:
            ...
            try:
                ...
                self.commit(translate("draft", "Create B̩zier Curve"), _cmd_list)
            except Exception:
...
class CubicBezCurve(gui_lines.Line):
    """Gui command for the 3rd degree B̩zier Curve tool.
    ...
    See: https://github.com/AuraCAD/AuraCAD/issues/6452
    """
    ...
    def GetResources(self):
        ...
        return {
            ...
            # "Accel": "B, Z",
            "MenuText": QT_TRANSLATE_NOOP("Draft_CubicBezCurve", "Cubic B̩zier Curve"),
            "ToolTip": QT_TRANSLATE_NOOP(
                "Draft_CubicBezCurve",
                "Creates a B̩zier curve made of 2nd degree (quadratic) and 3rd degree (cubic) segments. Clicking and dragging allows to define segments.\nControl points and properties of each knot can be edited after creation.",
            ),
        }
    ...
    def Activated(self):
        ...
        super().Activated(
            ...
            task_title=translate("draft", "Cubic B̩zier Curve"),
        )
        if self.doc:
            self.bezcurvetrack = trackers.bezcurveTracker()
...
class BezierGroup:
    """Gui Command group for the B̩zier curve tools."""

    def GetResources(self):
        """Set icon, menu and tooltip."""
        return {
            "MenuText": QT_TRANSLATE_NOOP("Draft_BezierTools", "B̩zier Tools"),
            "ToolTip": QT_TRANSLATE_NOOP(
                "Draft_BezierTools", "Tools to create various types of B̩zier curves"
            ),
        }
