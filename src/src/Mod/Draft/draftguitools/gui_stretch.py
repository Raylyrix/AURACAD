            ...
            if self.displacement.Length > 0:
                ...
                for ops in self.ops:
                    ...
                    if tp in ["Wire", "BSpline", "BezCurve"]:
                        ...
                        pts = str(pts).replace("Vector ", "AuraCAD.Vector")
                        ...
                        points_str = (
                            "["
                            + ", ".join([f"AuraCAD.Vector({p.x}, {p.y}, {p.z})" for p in npts])
                            + "]"
                        )

                        commitops.append("import AuraCAD")
                        commitops.append(
                            f"wall_obj = AuraCAD.ActiveDocument.getObject('{ops[0].Name}')"
                        )
                        commitops.append(
                        ...
                        commitops.append(_pl)
        if commitops:
            commitops.append("AuraCAD.ActiveDocument.recompute()")
            Gui.addModule("Draft")
