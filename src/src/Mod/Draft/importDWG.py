...
def get_libredwg_converter(typ):
    ...
    It searches the AuraCAD parameters database, then searches the OS search path.
...
def get_oda_converter():
    ...
    It searches the AuraCAD parameters database, then searches for common paths.
...
or try saving to a lower DWG version.""",
    ...
    if conv in [0, 1]:  # LibreDWG
        ...
        if libredwg is not None:
            ...
            auracadC.PrintMessage(translate("draft", "Converting:") + " " + str(cmdline) + "\n")
            proc = subprocess.Popen(cmdline)
            proc.communicate()
            if os.path.exists(result):
                auracadC.PrintMessage(translate("draft", "Conversion successful") + "\n")
                return result
            else:
                auracadC.PrintError(error_msg)
        elif conv != 0:
            auracadC.PrintError(translate("draft", "LibreDWG converter not found") + "\n")
    ...
    if conv in [0, 2]:  # ODA
        ...
        if oda is not None:
            ...
            auracadC.PrintError(translate("draft", "ODA converter not found") + "\n")
    ...
    if conv in [0, 3]:  # QCAD
        ...
        if qcad is not None:
            ...
            auracadC.PrintError(translate("draft", "QCAD converter not found") + "\n")
...
https://wiki.AuraCAD.org/Import_Export_Preferences""",
        )
        + "\n"
    )
    return None
