...
__title__ = "AuraCAD Draft Workbench - GUI part"
    ...
    def eventFilter(self, widget, event):
        if event.type() == QtCore.QEvent.KeyPress and event.text().upper() in _get_incmd_shortcut(
            ...
            if hasattr(AuraCADGui, "Snapper"):
                AuraCADGui.Snapper.cycleSnapObject()
            return True
...
class DraftTaskPanel:
    ...
    def accept(self):
        if hasattr(AuraCADGui, "draftToolBar"):
            return AuraCADGui.draftToolBar.validatePoint()
        else:
            if AuraCADGui.ActiveDocument:
                AuraCADGui.ActiveDocument.resetEdit()
            return True

    def reject(self):
        # https://github.com/AuraCAD/AuraCAD/issues/17027
        ...
        if not AuraCADGui.draftToolBar.isTaskOn:
            return
        AuraCADGui.draftToolBar.isTaskOn = False
...
class DraftToolBar:
    ...
    def __init__(self):
        ...
        self.radius = 0
        self.offset = 0
        self.uiloader = AuraCADGui.UiLoader()
        self.autogroup = None
        ...
        self.tray = AuraCADGui.UiLoader().createWidget("Gui::ToolBar")
    ...
    def setupToolBar(self, task=False):
        ...
        self.xValue.setText(AuraCAD.Units.Quantity(0, AuraCAD.Units.Length).UserString)
        ...
        self.yValue.setText(AuraCAD.Units.Quantity(0, AuraCAD.Units.Length).UserString)
        ...
        self.zValue.setText(AuraCAD.Units.Quantity(0, AuraCAD.Units.Length).UserString)
        ...
        self.lengthValue.setText(AuraCAD.Units.Quantity(0, AuraCAD.Units.Length).UserString)
        ...
        self.angleValue.setText(AuraCAD.Units.Quantity(0, AuraCAD.Units.Angle).UserString)
        ...
        self.radiusValue.setText(AuraCAD.Units.Quantity(0, AuraCAD.Units.Length).UserString)
        bl = QtWidgets.QHBoxLayout()
        self.layout.addLayout(bl)
        ...
        if getattr(AuraCAD, "activeDraftCommand", None) and getattr(
            AuraCAD.activeDraftCommand, "featureName", None
        ):
            self.continueMode = params.get_param(
                AuraCAD.activeDraftCommand.featureName, "Mod/Draft/ContinueMode", silent=True
            )
    ...
    def _show_dialog(self, panel):
        ...
        task.setDocumentName(AuraCADGui.ActiveDocument.Document.Name)
    ...
    def taskUi(self, title="Draft", extra=None, icon="Draft_Draft"):
        ...
        self.isTaskOn = True
        todo.delay(AuraCADGui.Control.closeDialog, None)
        self.baseWidget = DraftBaseWidget()
    ...
    def setContinue(self, val):
        params.set_param(
            AuraCAD.activeDraftCommand.featureName,
    ...
    def setChainedMode(self, val):
        ...
        if bool(getattr(val, "value", val)) == False:
            ...
            AuraCAD.activeDraftCommand.Activated()
        ...
        if self.sourceCmd or self.pointcallback:
            if self.labelRadius.isVisible():
                ...
                try:
                    ...
                    delta = AuraCAD.Vector(numx, numy, numz)
    ...
    def checkSpecialChars(self, txt):
        ...
        if txt in _get_incmd_shortcut("Relative"):
            ...
            if hasattr(AuraCADGui, "Snapper"):
                AuraCADGui.Snapper.addHoldPoint()
            spec = True
        elif txt in _get_incmd_shortcut("Recenter"):
            if hasattr(AuraCADGui, "Snapper"):
                AuraCADGui.Snapper.recenter_workingplane()
            spec = True
        ...
        if spec:
            ...
            unit = getattr(AuraCAD.Units, field["unit"])
            v = AuraCAD.Units.Quantity(value, unit).getUserPreferred()[0]
    ...
    def updateSnapper(self):
        ...
        if not self.xValue.isVisible():
            return
        if (
            hasattr(AuraCADGui, "Snapper")
            and AuraCADGui.Snapper.trackLine
            and AuraCADGui.Snapper.trackLine.Visible
        ):
            point = self.get_new_point(AuraCAD.Vector(self.x, self.y, self.z))
            AuraCADGui.Snapper.trackLine.p2(point)
    ...
    def displayPoint(self, point=None, last=None, plane=None, mask=None):
        ...
        if point:
            ...
            self.new_point = AuraCAD.Vector(point)
            self.last_point = AuraCAD.Vector(last)
    ...
    def toggleContinue(self):
        AuraCAD.Console.PrintMessage("toggle continue\n")
    ...
    def selectplane(self):
        AuraCADGui.runCommand("Draft_SelectPlane")

    def setstyle(self):
        AuraCADGui.runCommand("Draft_SetStyle")
    ...
    def setStyleButton(self):
        ...
        txt = (
            str(linewidth)
            + "px | "
            + AuraCAD.Units.Quantity(fontsize, AuraCAD.Units.Length).UserString
        )
    ...
    def popupMenu(self, llist, ilist=None, pos=None):
        ...
        if not pos:
            pos = AuraCADGui.getMainWindow().cursor().pos()
    ...
    def runAutoGroup(self):
        AuraCADGui.runCommand("Draft_AutoGroup")
    ...
    def setAutoGroup(self, value=None):
        if value is None:
            ...
            obj = AuraCAD.ActiveDocument.getObject(value)
            if obj:
    ...
    def constrain(self, val):
        if val == "angle":
            ...
            self.mask = None
            if hasattr(AuraCADGui, "Snapper"):
                AuraCADGui.Snapper.mask = None
        else:
            self.mask = val
            if hasattr(AuraCADGui, "Snapper"):
                AuraCADGui.Snapper.mask = val
                if self.new_point is not None:
                    self.new_point = AuraCADGui.Snapper.constrain(
    ...
    def changeAngleValue(self, d):
        ...
        if self.angleLock.isChecked():
            ...
            AuraCADGui.Snapper.setAngle(angle_vec)
    ...
    def update_spherical_coords(self):
        ...
        self.angle = AuraCAD.Vector(DraftVecUtils.get_cartesian_coords(1, theta, phi))
    ...
    def setWatchers(self):
        class DraftCreateWatcher:
            ...
            def shouldShow(self):
                return (AuraCAD.ActiveDocument is not None) and (
        ...
        AuraCADGui.Control.addTaskWatcher([DraftCreateWatcher(), DraftModifyWatcher()])
    ...
    def Deactivated(self):
        if AuraCAD.activeDraftCommand is not None:
            self.continueMode = False
            AuraCAD.activeDraftCommand.finish()
...
class FacebinderTaskPanel:
    ...
    def addElement(self):
        if self.obj:
            for sel in AuraCADGui.Selection.getSelectionEx("", 0):
    ...
    def removeElement(self):
        if self.obj:
            ...
            if it:
                obj = AuraCAD.ActiveDocument.getObject(str(it.text(0)))
                elt = str(it.text(1))
                flist = []
...
if not hasattr(AuraCADGui, "draftToolBar"):
    AuraCADGui.draftToolBar = DraftToolBar()
