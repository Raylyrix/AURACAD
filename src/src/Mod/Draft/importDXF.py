...
__title__ = "AuraCAD Draft Workbench - DXF importer/exporter"
...
import WorkingPlane
from AuraCAD import Vector
from AuraCAD import Console as auracadC
...
if gui:
    ...
    try:
        draftui = AuraCADGui.draftToolBar
...
def errorDXFLib(gui):
    ...
    if dxfAllowDownload:
        files = ["dxauracadolorMap.py", "dxfImportObjects.py", "dxfLibrary.py", "dxfReader.py"]
...
from menu Tools â†’ Addon Manager""",
                    )
                    QtWidgets.QMessageBox.information(None, "", message)
                else:
                    auracadC.PrintWarning(
                        "The DXF import/export libraries needed by AuraCAD to handle the DXF format are not installed.\n"
                    )
                    auracadC.PrintWarning(
                        "Please install the DXF Library addon from Tools â†’ Addon Manager\n"
                    )
                break
        progressbar.stop()
        sys.path.append(AuraCAD.ConfigGet("UserAppData"))
    else:
        if gui:
            message = translate(
                "draft",
                """The DXF import/export libraries needed by AuraCAD to handle
...
To enabled AuraCAD to download these libraries, answer Yes.""",
            )
...
def getDXFlibs():
    ...
    `dxfLibrary`, `dxauracadolorMap`, `dxfReader`,
    ...
    """
    try:
        if AuraCAD.ConfigGet("UserAppData") not in sys.path:
            sys.path.append(AuraCAD.ConfigGet("UserAppData"))
        global dxfLibrary, dxauracadolorMap, dxfReader
        import dxfLibrary
        import dxauracadolorMap
        ...
        libsok = False
        auracadC.PrintWarning("DXF libraries not found. Trying to downloadâ€¦\n")
    else:
        if float(dxfLibrary.__version__[1:5]) >= CURRENTDXFLIB:
            libsok = True
        else:
            auracadC.PrintWarning("DXF libraries need to be updated. " "Trying to downloadâ€¦\n")
            libsok = False
    if not libsok:
        errorDXFLib(gui)
        try:
            import dxauracadolorMap, dxfLibrary, dxfReader
            import importlib

            importlib.reload(dxauracadolorMap)
            importlib.reload(dxfLibrary)
            importlib.reload(dxfReader)
        except Exception:
            dxfReader = None
            dxfLibrary = None
            auracadC.PrintWarning("DXF libraries not available. Aborting.\n")
...
def getGroup(ob):
    ...
    Use local variables, not global variables.
    """
    all_objs = AuraCAD.ActiveDocument.Objects
    if dxfUseDraftVisGroups:
...
def getACI(ob, text=False):
    ...
    if not gui:
        ...
        for i in range(255, -1, -1):
            ref = dxauracadolorMap.color_map[i]
...
def getGroupColor(dxfobj, index=False):
    ...
    `dxauracadolorMap.color_map` dictionary.
    ...
    list of 3 floats
        ...
        or the mapped color `dxauracadolorMap.color_map[color]`.
                    ...
                    if index:
                        ...
                        if (l.color == 7) and dxfBrightBackground:
                            ...
                            if isinstance(l.color, int):
                                if l.color > 0:
                                    return dxauracadolorMap.color_map[l.color]
    return [0.0, 0.0, 0.0]
...
def formatObject(obj, dxfobj=None):
    ...
    `dxauracadolorMap.color_map[dxfobj.color_index]`.
    ...
    if dxfGetColors and dxfobj and hasattr(dxfobj, "color_index"):
        if hasattr(obj.ViewObject, "TextColor"):
            if dxfobj.color_index == 256:
                ...
                cm = dxauracadolorMap.color_map[dxfobj.color_index]
...
def placementFromDXFOCS(ent):
    ...
    if draftWPlane.axis == AuraCAD.Vector(1.0, 0.0, 0.0):
        draftWPlane.u = AuraCAD.Vector(0.0, 1.0, 0.0)
        draftWPlane.v = AuraCAD.Vector(0.0, 0.0, 1.0)
    elif draftWPlane.axis == AuraCAD.Vector(-1.0, 0.0, 0.0):
        draftWPlane.u = AuraCAD.Vector(0.0, -1.0, 0.0)
        draftWPlane.v = AuraCAD.Vector(0.0, 0.0, 1.0)
    else:
        if (abs(ent.extrusion[0]) < (1.0 / 64.0)) and (abs(ent.extrusion[1]) < (1.0 / 64.0)):
            draftWPlane.u = AuraCAD.Vector(0.0, 1.0, 0.0).cross(draftWPlane.axis)
        else:
            draftWPlane.u = AuraCAD.Vector(0.0, 0.0, 1.0).cross(draftWPlane.axis)
...
def drawLine(line, forceShape=False):
    ...
    Part::Feature or Part::TopoShape ('Edge')
        ...
        variables `dxauracadreateDraft` or `dxauracadreateSketch` are set,
    ...
    if len(line.points) > 1:
        ...
        if not DraftVecUtils.equals(v1, v2):
            try:
                if (dxauracadreateDraft or dxauracadreateSketch) and (not forceShape):
...
def drawPolyline(polyline, forceShape=False, num=None):
    ...
    `dxauracadreateDraft` or `dxauracadreateSketch` are set, and `forceShape` is `False`
            ...
            try:
                ...
                if width and dxfRenderPolylineWidth:
                    ...
                    if polyline.closed:
                        ...
                        return Part.Face(w1)
                elif (dxauracadreateDraft or dxauracadreateSketch) and (not curves) and (not forceShape):
...
def drawArc(arc, forceShape=False):
    ...
    Part::Part2DObject or Part::TopoShape ('Edge')
        ...
        if the global variables `dxauracadreateDraft` or `dxauracadreateSketch` are set,
        and `forceShape` is `False`.
        Otherwise it produces a `Part.Edge`.
...
def drawMesh(mesh, forceShape=False):
    ...
    if mesh.flags == 16:
        ...
        #       "points and", len(auracads), "facets")
        for f in auracads:
    ...
    try:
        ...
        if forceShape:
            ...
            s = s.removeSplitter()
            m = s
    except AuraCAD.Base.AuraCADError:
        warn(mesh)
    else:
        return m
    return None
...
def drawInsert(insert, num=None, clone=False):
    ...
    if clone:
        if insert.block in blockobjects:
            ...
            sc = insert.scale
            sc = vec([sc[0], sc[1], 0])
            newob.Placement = AuraCAD.Placement(tsf)
            newob.Scale = sc
            return newob
        else:
            shape = None
...
def drawLayerBlock(objlist, name="LayerBlock"):
    ...
    for o in objlist:
        if isinstance(o, Part.Shape):
            isObj = False
    obj = None
    if (dxauracadreateDraft or dxauracadreateSketch) and isObj:
        try:
...
def processdxf(document, filename, getShapes=False, reComputeFlag=True):
    ...
    if not dxfReader:
        getDXFlibs()
        readPreferences()
    auracadC.PrintMessage("opening " + filename + "...\n")
    ...
    layers = [o for o in AuraCAD.ActiveDocument.Objects if utils.get_type(o) == typ]
    global doc
    doc = document
    global blockshapes
    blockshapes = {}
    ...
    if hasattr(drawing, "tables"):
        for table in drawing.tables.get_type("table"):
            for layer in table.get_type("layer"):
                name = layer.name
                color = tuple(dxauracadolorMap.color_map[abs(layer.color)])
                drawstyle = "Solid"
                lt = rawValue(layer, 6)
    ...
    if lines:
        auracadC.PrintMessage("drawing " + str(len(lines)) + " lines...\n")
    for line in lines:
        if dxfImportLayouts or (not rawValue(line, 67)):
            shape = drawLine(line)
            if shape:
                if dxauracadreateSketch:
    ...
    for p in pls:
        if hasattr(p, "flags"):
            ...
            polylines.append(p)
    if polylines:
        auracadC.PrintMessage("drawing " + str(len(polylines)) + " polylines...\n")
    num = 0
    for polyline in polylines:
        if dxfImportLayouts or (not rawValue(polyline, 67)):
            ...
            if shape:
                if dxauracadreateSketch:
                    if isinstance(shape, Part.Shape):
                        t = AuraCAD.ActiveDocument.addObject("Part::Feature", "Shape")
                        t.Shape = shape
                        shape = t
                    AuraCAD.ActiveDocument.recompute()
                    if dxfMakeBlocks or dxfJoin:
    ...
    if arcs:
        auracadC.PrintMessage("drawing " + str(len(arcs)) + " arcs...\n")
    for arc in arcs:
    ...
    if dxfJoin and shapes:
        auracadC.PrintMessage("Joining geometry...\n")
        edges = []
        for s in shapes:
            edges.extend(s.Edges)
        if len(edges) > (100):
            auracadC.PrintMessage(str(len(edges)) + " edges to join\n")
            if gui:
    ...
    if circles:
        auracadC.PrintMessage("drawing " + str(len(circles)) + " circles...\n")
    ...
    if solids:
        auracadC.PrintMessage("drawing " + str(len(solids)) + " solids...\n")
    for solid in solids:
    ...
    if splines:
        auracadC.PrintMessage("drawing " + str(len(splines)) + " splines...\n")
    ...
    if ellipses:
        auracadC.PrintMessage("drawing " + str(len(ellipses)) + " ellipses...\n")
    ...
    if dxfImportTexts:
        ...
        if texts:
            auracadC.PrintMessage("drawing " + str(len(texts)) + " texts...\n")
        for text in texts:
            if dxfImportLayouts or (not rawValue(text, 67)):
                addText(text)
    else:
        auracadC.PrintMessage("skipping texts...\n")

    # Draw 3D objects
    faces3d = drawing.entities.get_type("3dface")
    if faces3d:
        auracadC.PrintMessage("drawing " + str(len(faces3d)) + " 3dfaces...\n")
    ...
    if meshes:
        auracadC.PrintMessage("drawing " + str(len(meshes)) + " 3dmeshes...\n")
    for mesh in meshes:
        me = drawMesh(mesh)
        if me:
    ...
    if dxfImportTexts:
        ...
        auracadC.PrintMessage("drawing " + str(len(dims)) + " dimensions...\n")
        for dim in dims:
        ...
        auracadC.PrintMessage("skipping dimensions...\n")

    # Draw points
    if dxfImportPoints:
        points = drawing.entities.get_type("point")
        if points:
            auracadC.PrintMessage("drawing " + str(len(points)) + " points...\n")
        for point in points:
        ...
        auracadC.PrintMessage("skipping points...\n")

    # Draw leaders
    if dxfImportTexts:
        leaders = drawing.entities.get_type("leader")
        if leaders:
            auracadC.PrintMessage("drawing " + str(len(leaders)) + " leaders...\n")
        ...
        auracadC.PrintMessage("skipping leaders...\n")

    # Draw hatches
    if dxfImportHatches:
        hatches = drawing.entities.get_type("hatch")
        if hatches:
            auracadC.PrintMessage("drawing " + str(len(hatches)) + " hatches...\n")
        for hatch in hatches:
            if dxfImportLayouts or (not rawValue(hatch, 67)):
                ...
                if len(points) > 1:
                    ...
                    points = points[:-1]
                    newob = None
                    if dxauracadreatePart or dxfMakeBlocks:
        ...
        auracadC.PrintMessage("skipping hatches...\n")

    # Draw blocks
    inserts = drawing.entities.get_type("insert")
    if not dxfStarBlocks:
        auracadC.PrintMessage("skipping *blocks...\n")
        newinserts = []
        for i in inserts:
        ...
        inserts = newinserts
    if inserts:
        auracadC.PrintMessage("drawing " + str(len(inserts)) + " blocks...\n")
        blockrefs = drawing.blocks.data
        for ref in blockrefs:
            if dxauracadreateDraft or dxauracadreateSketch:
                drawBlock(ref, createObject=True)
            else:
                drawBlock(ref, createObject=False)
        num = 0
        for insert in inserts:
            if (dxauracadreateDraft or dxauracadreateSketch) and not dxfMakeBlocks:
    ...
    auracadC.PrintMessage("successfully imported " + filename + "\n")
    if badobjects:
...
def _import_dxf_file(filename, doc_name=None):
    ...
    """
    hGrp = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft")
    ...
    if doc_name:  # INSERT operation
        try:
            doc = AuraCAD.getDocument(doc_name)
        except NameError:
            doc = AuraCAD.newDocument(doc_name)
        AuraCAD.setActiveDocument(doc_name)
    else:  # OPEN operation
            ...
            if lwPoly:
                if hasattr(dxfLibrary, "LwPolyLine"):
                    ...
                    auracadC.PrintWarning(
                        ...
                        "from your AuraCAD user directory "
...
def export(objectslist, filename, nospline=False, lwPoly=False):
    ...
    if dxfLibrary:
        ...
        if (len(exportList) == 1) and (utils.get_type(exportList[0]) == "ArchSectionView"):
            ...
            for ob in exportList:
                ...
                if obtype == "PanelSheet":
                    ...
                    p2 = AuraCAD.Vector(_v)
                    lspc = AuraCAD.Vector(_h)
                    ...
                    if t2:
                        ofs = AuraCAD.Vector(0, -lspc.Length, 0)
                        if rotation:
                            Z = AuraCAD.Vector(0, 0, 1)
                            ofs = AuraCAD.Rotation(Z, rotation).multVec(ofs)
                        dxf.append(
                            dxfLibrary.Text(
                                t2,
                                p1.add(ofs),
                    ...
                    if gui:
                        ...
                        for text in vobj.Proxy.getTextData():
                            pos = text[1].add(AuraCAD.Vector(0, -h / 2, 0))
                            dxf.append(
                                dxfLibrary.Text(
                                    text[0],
                                    pos,
                    ...
                    if ob.isDerivedFrom("Sketcher::SketchObject"):
                        ...
                        _view = AuraCADGui.ActiveDocument.ActiveView
        ...
        auracadC.PrintMessage("successfully exported" + " " + filename + "\n")

    else:
        errorDXFLib(gui)


class dxauracadounter:
...
def exportPage(page, filename):
    ...
    It also increments the counter by using the `dxauracadounter` class.
    ...
    if os.path.exists(template):
        ...
        svgtemplate = f.read()
        f.close()
        editables = re.findall(r"AuraCAD:editable=\"(.*?)\"", svgtemplate)
        ...
        template = "999\nAuraCAD DXF exporter v" + _version + "\n"
        template += "0\nSECTION\n2\nHEADER\n9\n$ACADVER\n1\nAC1009\n0\nENDSEC\n"
        template += "0\nSECTION\n2\nBLOCKS\n999\n$blocks\n0\nENDSEC\n"
        template += "0\nSECTION\n2\nENTITIES\n999\n$entities\n0\nENDSEC\n"
        template += "0\nEOF"
...
def readPreferences():
    ...
    `dxauracadreatePart`, `dxauracadreateDraft`, `dxauracadreateSketch`,
    ...
    Use local variables, not global variables.
    """
    global dxauracadreatePart, dxauracadreateDraft, dxauracadreateSketch
    ...
    if dxfUseLegacyImporter:
        ...
        dxauracadreateSketch = hGrp.GetBool("dxauracadreateSketch", False)

        if dxauracadreateSketch:  # dxauracadreateSketch overrides the import mode for the legacy importer
            dxauracadreatePart = False
            dxauracadreateDraft = False
            dxfMakeBlocks = False
...
class DxfImportReporter:
    ...
    def to_console_string(self):
        ...
        lines.append(f"AuraCAD objects created: {self.stats.get('totalEntitiesCreated', 0)}")
    ...
    def report_to_console(self):
        """
        Prints the formatted statistics string to the AuraCAD console.
        """
        output_string = self.to_console_string()
        auracadC.PrintMessage(output_string)
...
class DxfDraftPostProcessor:
    ...
    def _create_draft_object_from_part(self, part_obj):
        ...
        if part_obj.isDerivedFrom("Part::Line"):
            ...
            start_point = AuraCAD.Vector(part_obj.X1.Value, part_obj.Y1.Value, part_obj.Z1.Value)
            end_point = AuraCAD.Vector(part_obj.X2.Value, part_obj.Y2.Value, part_obj.Z2.Value)
            ...
            if hasattr(part_obj, "Radius"):
                new_obj.Radius = AuraCAD.Units.Quantity(part_obj.Radius.Value, "mm")
            ...
            if hasattr(part_obj, "Angle1") and hasattr(part_obj, "Angle2"):
                ...
                new_obj.FirstAngle = AuraCAD.Units.Quantity(start_angle, "deg")
                new_obj.LastAngle = AuraCAD.Units.Quantity(end_angle, "deg")
            ...
            if hasattr(part_obj, "Placement"):
                ...
                new_obj.Placement = AuraCAD.Placement()
            ...
            if is_full_ellipse:
                ...
                center_at_origin = AuraCAD.Vector(0, 0, 0)
                geom = Part.Ellipse(
                ...
                if placeholder.DxfEntityType == "DIMENSION":
                    ...
                    if hasattr(placeholder, "DxfDimensionType"):
                        ...
                        if dim_type == 0 and hasattr(placeholder, "DxfRotation"):
                            ...
                            direction_vector = AuraCAD.Vector(math.cos(angle), math.sin(angle), 0)
        ...
        for obj in all_objects_to_style:
            if obj.isValid() and hasattr(obj, "ViewObject") and hasattr(obj, "Proxy"):
                try:
                    ...
                    auracadC.PrintWarning(f"Failed to set ViewProvider for {obj.Name}: {e}\n")
    ...
    def _get_canonical_angles(self, start_angle_deg, end_angle_deg, radius_mm):
        ...
        AuraCAD's App::PropertyAngle, which constrains values to [-360, 360].
    ...
    def run(self):
        ...
        try:
            ...
            auracadC.PrintError(f"Aborting DXF post-processing due to an error: {e}\n")
            import traceback
        ...
        self.doc.recompute()
        auracadC.PrintMessage("--- Draft post-processing finished. ---\n")
