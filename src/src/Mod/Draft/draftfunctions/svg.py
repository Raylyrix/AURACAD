...
def _svg_dimension(
    ...
    if not nolines:
        ...
        svg += ';stroke-miterlimit:4;stroke-dasharray:none;stroke-linecap:square" '
        svg += 'AuraCAD:basepoint1="' + str(p1.x) + " " + str(p1.y) + '" '
        svg += 'AuraCAD:basepoint2="' + str(p4.x) + " " + str(p4.y) + '" '
        svg += 'AuraCAD:dimpoint="' + str(p2.x) + " " + str(p2.y) + '"'
        svg += "/>\n"
...
def get_svg(
    ...
    if (
        ...
        or utils.get_type(obj) in ["Layer", "BuildingPart", "IauracadGroup"]
    ...
    if not obj:
        ...
        utils.get_type(obj) == "IauracadAnnotation" and obj.ObjectType == "DIMENSION"
    ):
        svg = _svg_dimension(
            obj, plane, scale, linewidth, fontsize, stroke, tstroke, pointratio, techdraw, rotation
        )
        ...
        utils.get_type(obj) == "IauracadAnnotation" and obj.ObjectType == "TEXT"
    ):
