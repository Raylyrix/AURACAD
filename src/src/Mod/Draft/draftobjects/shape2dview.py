...
class Shape2DView(DraftObject):
    ...
    def execute(self, obj):
        ...
        pl = obj.Placement
        if obj.Base:
            if utils.get_type(obj.Base) in ["BuildingPart", "SectionPlane", "IauracadAnnotation"]:
                objs = []
                if utils.get_type(obj.Base) == "SectionPlane":
                    objs = self.excludeNames(obj, obj.Base.Objects)
                    cutplane = obj.Base.Shape
                elif utils.get_type(obj.Base) == "IauracadAnnotation":
                    # this is a NativeIauracad section plane
                        ...
                        for sh in shapes:
                            ...
                            if (obj.ProjectionMode == "Cutfaces") and (sh.ShapeType == "Solid"):
                                ...
                                if hasattr(obj, "InPlace"):
                                    if obj.InPlace:
                                        faces = facesOrg
                                    # Alternative approach in https://forum.AuraCAD.org/viewtopic.php?p=807314#p807314, not adopted
                                    else:
