...
in AuraCAD and a couple of utility functions.
...
AuraCADGui = lz.LazyLoader("AuraCADGui", globals(), "AuraCADGui")

__title__ = "AuraCAD Working Plane utility"
...
class PlaneBase:
    ...
    def align_to_straight_edge(self, edge, offset=0):
        ...
        rot = AuraCAD.Rotation(u, self.v, self.axis, "XZY")
    ...
    def align_to_point_and_axis(self, point, axis, offset=0, upvec=Vector(1, 0, 0)):
        ...
        If the axis and up-vector are parallel the AuraCAD.Rotation algorithm
        ...
        rot = AuraCAD.Rotation(Vector(), upvec, axis, "ZYX")
    ...
    def get_global_coords(self, point, as_vector=False):
        ...
        mtx = AuraCAD.Matrix(self.u, self.v, self.axis, pos)
    ...
    def get_placement(self):
        ...
        return AuraCAD.Placement(self.position, AuraCAD.Rotation(self.u, self.v, self.axis, "ZYX"))
    ...
    def is_ortho(self):
        ...
        rot = AuraCAD.Rotation(self.u, self.v, self.axis, "ZYX")
...
class Plane(PlaneBase):
    ...
    def alignToSelection(self, offset=0):
        ...
        `True`/`False`
            `True` if successful.
        """
        sel_ex = AuraCADGui.Selection.getSelectionEx()
        if not sel_ex:
            return False
        ...
        for obj in sel_ex:
            ...
            geom_is_shape = False
            if isinstance(obj.Object, AuraCAD.GeoFeature):
    ...
    def setup(self, direction=None, point=None, upvec=None, force=False):
        ...
        This method only works when `AuraCAD.GuiUp` is `True`,
            ...
            if direction is not None and point is not None:
                ...
                try:
                    ...
                    if view is not None:
                        ...
                        rot = AuraCAD.Rotation(*cam.getField("orientation").getValue().getValue())
    ...
    def setTop(self):
        ...
        if AuraCAD.GuiUp:
            ...
            if hasattr(AuraCADGui, "draftToolBar"):
                AuraCADGui.draftToolBar.wplabel.setText(translate("draft", "Top"))
    ...
    def setFront(self):
        ...
        if AuraCAD.GuiUp:
            ...
            if hasattr(AuraCADGui, "Snapper"):
                AuraCADGui.Snapper.setGrid()
            if hasattr(AuraCADGui, "draftToolBar"):
                AuraCADGui.draftToolBar.wplabel.setText(translate("draft", "Front"))
    ...
    def setSide(self):
        ...
        if AuraCAD.GuiUp:
            ...
            if hasattr(AuraCADGui, "Snapper"):
                AuraCADGui.Snapper.setGrid()
            if hasattr(AuraCADGui, "draftToolBar"):
                AuraCADGui.draftToolBar.wplabel.setText(translate("draft", "Side"))

    def getRotation(self):
        """Return a placement describing the WP orientation only."""
        return AuraCAD.Placement(Vector(), AuraCAD.Rotation(self.u, self.v, self.axis, "ZYX"))
...
class PlaneGui(PlaneBase):
    ...
    def align_to_selection(self, offset=0, _hist_add=True):
        ...
        sels = AuraCADGui.Selection.getSelectionEx("", 0)
        if not sels:
            return False
        ...
        elif typ == "IauracadBuildingStorey":
            pl = AuraCAD.Placement(obj.Placement)
            pl.move(AuraCAD.Vector(0, 0, obj.Elevation.Value))
    ...
    def align_to_wp_proxy(self, obj, offset=0, place=None, _hist_add=True):
        ...
        if (
            ...
            for name, vis in vobj.VisibilityMap.items():
                obj = AuraCADGui.ActiveDocument.getObject(name)
                if obj:
    ...
    def _format_coord(self, coord):
        return AuraCAD.Units.Quantity(coord, AuraCAD.Units.Length).UserString
    ...
    def _update_old_plane(self):
        ...
        """
        if not hasattr(AuraCAD, "DraftWorkingPlane"):
            AuraCAD.DraftWorkingPlane = Plane()
        for prop in ["u", "v", "axis", "position"]:
            setattr(AuraCAD.DraftWorkingPlane, prop, self._copy_value(getattr(self, prop)))
        AuraCAD.DraftWorkingPlane.weak = self.auto

    def _update_grid(self):
        # Check for draftToolBar because the trackers (grid) depend on it for its colors.
        if (
            AuraCAD.GuiUp
            and hasattr(AuraCADGui, "draftToolBar")
            and hasattr(AuraCADGui, "Snapper")
            and self._view is not None
        ):
            AuraCADGui.Snapper.setGrid()
            AuraCADGui.Snapper.restack()  # Required??

    def _update_gui(self):
        if AuraCAD.GuiUp and hasattr(AuraCADGui, "draftToolBar") and self._view is not None:
            from PySide import QtGui

            button = AuraCADGui.draftToolBar.wplabel
...
def get_working_plane(update=True):
    ...
    if not hasattr(AuraCAD, "draft_working_planes"):
        AuraCAD.draft_working_planes = [[], []]

    view = gui_utils.get_3d_view()

    if view is not None and view in AuraCAD.draft_working_planes[0]:
        i = AuraCAD.draft_working_planes[0].index(view)
        wp = AuraCAD.draft_working_planes[1][i]
        if update is False:
    ...
    if AuraCAD.GuiUp:
        ...
        wp.set_to_default()
        if view is not None:
            AuraCAD.draft_working_planes[0].append(view)
            AuraCAD.draft_working_planes[1].append(wp)
...
if AuraCAD.GuiUp:
    ...
    def _update_gui():
        try:
            ...
            if not hasattr(AuraCAD, "draft_working_planes"):
                AuraCAD.draft_working_planes = [[], []]
            if view in AuraCAD.draft_working_planes[0]:
                i = AuraCAD.draft_working_planes[0].index(view)
                wp = AuraCAD.draft_working_planes[1][i]
                wp._update_gui()
            else:
