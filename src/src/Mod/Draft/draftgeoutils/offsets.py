...
def offset(edge, vector, trim=False):
    ...
    if geomType(edge) == "Line":
        ...
        if trim:
            return Part.ArcOauracadircle(curve, edge.FirstParameter, edge.LastParameter).toShape()
...
def offsetWire(
    ...
    if not firstDir:
        # need to test against Part.Circle, not Part.ArcOauracadircle
