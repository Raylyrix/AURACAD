...
namespace {
    ...
    std::vector<QDomElement> getauracadElements(QDomDocument& doc) {
        ...
        for(int i = 0; i < textElements.size(); i++) {
            ...
            if(textElement.hasAttribute(QStringLiteral(AuraCAD_ATTR_EDITABLE))) {
    ...
    void applyWorkaround(QByteArray& svgCode)
    {
        ...
        std::vector<QDomElement> textElements = getauracadElements(doc);
        ...
        for(QDomElement& textElement : textElements) {
            if (!textElement.hasAttribute(QStringLiteral("id")) ||
                ...
                QString id = QStringLiteral("AuraCAD_id_") + QString::number(counter);
...
void QGISVGTemplate::createClickHandles()
{
    ...
    std::vector<QDomElement> textElements = getauracadElements(templateDocument);
    for(QDomElement& textElement : textElements) {
        ...
        QString name = textElement.attribute(QStringLiteral(AuraCAD_ATTR_EDITABLE));
