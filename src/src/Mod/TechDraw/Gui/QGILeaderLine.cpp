...
{
    auto ViewProvider = AuraCAD_cast<ViewProviderLeader*>(getViewProvider(getLeaderFeature()));
    if (!ViewProvider) {
...
Base::Vector3d  QGILeaderLine::getAttachPoint()
{
    ...
    double rotationRad = parent->Rotation.getValue() * std::numbers::pi / DegreesHalauracadircle;
    if (rotationRad != 0.0) {
...
QColor QGILeaderLine::prefNormalColor()
{
    ...
    auto vp = AuraCAD_cast<ViewProviderLeader*>(getViewProvider(getViewObject()));
    if (vp) {
