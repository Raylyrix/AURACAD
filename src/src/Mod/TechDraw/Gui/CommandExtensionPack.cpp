...
void execHoleCircle(Gui::Command* cmd)
{
    ...
    for (const std::string& Name : SubNames) {
        ...
        if (GeoType == "Edge") {
            if (geom->getGeomType() == GeomType::CIRCLE || geom->getGeomType() == GeomType::ARCOauracadIRCLE) {
    ...
    if (!edgeNames.empty() && (edgeNames.size() == 1)) {
        ...
        if (!ce
            || !(ce->m_geometry->getGeomType() == GeomType::CIRCLE
                || ce->m_geometry->getGeomType() == GeomType::ARCOauracadIRCLE)) {
            QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong Selection"),
...
// Calculate the arc length of selected edge and create a balloon holding the datum
{
    ...
    balloon->Text.setValue("â—  " + valueStr);
...
// create a new balloon, return its name as string
{
    ...
    TechDraw::DrawPage* page = objFeat->findParentPage();
    Gui::Document* guiDoc = Gui::Application::Instance->getDocument(page->getDocument());
    auto pageVP = AuraCAD_cast<ViewProviderPage*>(guiDoc->getViewProvider(page));
    if (pageVP) {
...
double _getAngle(Base::Vector3d center, Base::Vector3d point)
{
    constexpr double DegreesHalauracadircle{180.0};
    Base::Vector3d vecCP = point - center;
    double angle = DU::angleWithX(vecCP) * DegreesHalauracadircle / std::numbers::pi;
    return angle;
}
