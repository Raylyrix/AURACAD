...
{
    return AuraCAD_cast<DrawTemplate*>(getDrawPage()->Template.getValue());
}
...
QGITemplate* ViewProviderPage::getQTemplate() const
{
    ...
    if (guiDoc) {
        ...
        auto vpTemplate = AuraCAD_cast<ViewProviderTemplate*>(vp);
        if (vpTemplate) {
...
void ViewProviderPage::setTemplateMarkers(bool state) const
{
    ...
    auto* vpt = AuraCAD_cast<ViewProviderTemplate*>(vp);
    if (vpt) {
