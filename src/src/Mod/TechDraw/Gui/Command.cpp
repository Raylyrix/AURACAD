...
void CmdTechDrawView::activated(int iMsg)
{
    ...
    auto* vpp = AuraCAD_cast<ViewProviderPage*>
        (Gui::Application::Instance->getViewProvider(page));
    if (vpp) {
        vpp->show();
    }
    ...
    doCommand(Doc,
              "App.activeDocument().%s.Direction = AuraCAD.Vector(%.12f, %.12f, %.12f)",
              FeatName.c_str(), dirs.first.x, dirs.first.y, dirs.first.z);
    doCommand(Doc,
              "App.activeDocument().%s.RotationVector = AuraCAD.Vector(%.12f, %.12f, %.12f)",
              FeatName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
    doCommand(Doc,
              "App.activeDocument().%s.XDirection = AuraCAD.Vector(%.12f, %.12f, %.12f)",
              FeatName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
...
void CmdTechDrawBrokenView::activated(int iMsg)
{
    ...
    doCommand(Doc, "App.activeDocument().%s.Direction = AuraCAD.Vector(%.6f,%.6f,%.6f)",
              FeatName.c_str(), projDir.x, projDir.y, projDir.z);
    doCommand(Doc, "App.activeDocument().%s.XDirection = AuraCAD.Vector(%.6f,%.6f,%.6f)",
                  FeatName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
    getDocument()->setStatus(App::Document::Status::SkipRecompute, true);
...
void CmdTechDrawProjectionGroup::activated(int iMsg)
{
    ...
    getDocument()->setStatus(App::Document::Status::SkipRecompute, true);
    doCommand(Doc,
        "App.activeDocument().%s.Anchor.Direction = AuraCAD.Vector(%.12f, %.12f, %.12f)",
        multiViewName.c_str(), dirs.first.x, dirs.first.y, dirs.first.z);
    doCommand(Doc,
        "App.activeDocument().%s.Anchor.RotationVector = AuraCAD.Vector(%.12f, %.12f, %.12f)",
        multiViewName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
    doCommand(Doc,
        "App.activeDocument().%s.Anchor.XDirection = AuraCAD.Vector(%.12f, %.12f, %.12f)",
        multiViewName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
    getDocument()->setStatus(App::Document::Status::SkipRecompute, false);
...
void CmdTechDrawBalloon::activated(int iMsg)
{
    ...
    ViewProviderPage* pageVP = AuraCAD_cast<ViewProviderPage*>(guiDoc->getViewProvider(page));
    ViewProviderDrawingView* viewVP =
        AuraCAD_cast<ViewProviderDrawingView*>(guiDoc->getViewProvider(objFeat));
...
void CmdTechDrawDraftView::activated(int iMsg)
{
    ...
    for (auto* obj : objects) {
        ...
        doCommand(Doc, "App.activeDocument().%s.addView(App.activeDocument().%s)", PageName.c_str(),
                  FeatName.c_str());
        doCommand(Doc, "App.activeDocument().%s.Direction = AuraCAD.Vector(%.12f, %.12f, %.12f)",
                  FeatName.c_str(), dirs.first.x, dirs.first.y, dirs.first.z);
        updateActive();
        commitCommand();
    }
...
void CmdTechDrawExportPageSVG::activated(int iMsg)
{
    ...
    ViewProviderPage* vpPage = AuraCAD_cast<ViewProviderPage*>(vp);
    ...
    if (vpPage) {
        ...
        QMessageBox::warning(Gui::getMainWindow(),
                             ...
                             QObject::tr("AuraCAD could not find a page to export"));
        return;
    }
}
