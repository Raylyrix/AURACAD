...
void QGIView::snapPosition(QPointF& newPosition)
{
    ...
    auto* dvp = AuraCAD_cast<DrawViewPart*>(feature);
    if (dvp  &&
        !dvp->hasGeometry()) {
    ...
    for (auto* view : views) {
        ...
        auto viewDvp = AuraCAD_cast<DrawViewPart*>(viewFeature);
                              ...
                              QPointF& newPosition)
{
    ...
    auto* vpdv = AuraCAD_cast<ViewProviderDrawingView*>(getViewProvider(baseView));
    if (!vpdv) {
        return;
    }
...
ViewProviderPage* QGIView::getViewProviderPage(TechDraw::DrawView* dView)
{
    ...
    return AuraCAD_cast<ViewProviderPage*>(activeGui->getViewProvider(page));
}
...
bool QGIView::isExporting() const
{
    ...
    auto* view = AuraCAD_cast<TechDraw::DrawView*>(obj);
    if (!view) {
        return false;
    }
