    ...
    m_subs(subs)
{
    ...
    connect(ui->auracadFile, &FileChooser::fileNameSelected, this, &TaskHatch::onFileChanged);
...
void TaskHatch::setUiPrimary()
{
    ...
    ui->auracadFile->setFileName(QString::fromStdString(DrawHatch::prefSvgHatch()));
    ui->auracadFile->setFilter(QStringLiteral(
...
void TaskHatch::setUiEdit()
{
    ...
    ui->auracadFile->setFileName(QString::fromStdString(m_saveFile));
    ui->auracadFile->setFilter(QStringLiteral(
            "SVG files (*.svg *.SVG);;Bitmap files(*.jpg *.jpeg *.png *.bmp);;All files (*)"));
...
{
    m_file = ui->auracadFile->fileName().toStdString();
    apply();
}
...
//    Base::Console().message("TH::createHatch()\n");
    ...
    auto filespec = ui->auracadFile->fileName().toStdString();
