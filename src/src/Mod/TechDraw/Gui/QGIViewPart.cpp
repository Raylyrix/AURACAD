...
void QGIViewPart::drawAllFaces(void)
{
    ...
    for (auto& face : faceGeoms) {
        ...
        if (fGeom) {
            ...
            ViewProviderGeomHatch* geomVp = AuraCAD_cast<ViewProviderGeomHatch*>(gvp);
            if (geomVp) {
            ...
            ViewProviderHatch* hatchVp = AuraCAD_cast<ViewProviderHatch*>(gvp);
            if (hatchVp) {
...
void QGIViewPart::highlightMoved(QGIHighlight* highlight, QPointF newPos)
{
    ...
    auto detail = AuraCAD_cast<DrawViewDetail*>(docObj);
    auto baseView = AuraCAD_cast<DrawViewPart*>(getViewObject());
    if (detail && baseView) {
