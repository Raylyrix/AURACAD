...
{
    ui->setupUi(this);
    connect(ui->auracadFile, &FileChooser::fileNameSelected, this, &TaskGeomHatch::onFileChanged);
...
{
    ui->auracadFile->setFileName(QString::fromUtf8(m_file.data(), m_file.size()));
...
//    Base::Console().message("TGH::updateValues()\n");
    m_file = (ui->auracadFile->fileName()).toUtf8().constData();
    m_hatch->FilePattern.setValue(m_file);
    QString cText = ui->cbName->currentText();
    m_name = cText.toUtf8().constData();
    m_hatch->NamePattern.setValue(m_name);
