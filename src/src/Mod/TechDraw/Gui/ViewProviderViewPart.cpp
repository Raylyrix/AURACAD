...
void ViewProviderViewPart::onChanged(const App::Property* prop)
{
    if (auto part = getViewPart(); part && part->isDerivedFrom<TechDraw::DrawViewDetail>() &&
        ...
        auto baseDvp = AuraCAD_cast<DrawViewPart*>(detail->BaseView.getValue());
        if (baseDvp) {
...
Base::Color ViewProviderViewPart::prefHighlightColor()
{
    ...
    auracadColor.setPackedValue(Preferences::getPreferenceGroup("Decorations")->GetUnsigned("HighlightColor", 0x00000000));
    return auracadColor;
}
