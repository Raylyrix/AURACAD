...
void QGIBalloonLabel::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    auto ViewProvider = AuraCAD_cast<ViewProviderBalloon*>(
...
void QGIViewBalloon::setViewPartFeature(TechDraw::DrawViewBalloon* balloonFeat)
{
    ...
    if (docObj) {
        balloonParent = AuraCAD_cast<DrawView*>(docObj);
        if (balloonParent) {
...
void QGIViewBalloon::placeBalloon(QPointF pos)
{
    ...
    DrawView* balloonParent = AuraCAD_cast<DrawView*>(balloon->SourceView.getValue());
    if (!balloonParent) {
        return;
    }
    ...
    auto partVP = AuraCAD_cast<ViewProviderViewPart*>(objVp);
    if (partVP) {
...
QColor QGIViewBalloon::prefNormalColor()
{
    ...
    if (vp) {
        vpBalloon = AuraCAD_cast<ViewProviderBalloon*>(vp);
        if (vpBalloon) {
            Base::Color auracadColor = Preferences::getAccessibleColor(vpBalloon->Color.getValue());
            setNormalColor(auracadColor.asValue<QColor>());
        }
    }
...
DrawView* QGIViewBalloon::getSourceView() const
{
    ...
    DrawViewBalloon* dvb = AuraCAD_cast<DrawViewBalloon*>(docObj);
    if (dvb) {
        balloonParent = AuraCAD_cast<DrawView*>(dvb->SourceView.getValue());
    }
