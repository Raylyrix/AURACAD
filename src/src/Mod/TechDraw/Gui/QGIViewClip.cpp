...
void QGIViewClip::drawClip()
{
    ...
    auto* vpClip = AuraCAD_cast<ViewProviderViewClip*>(getViewProvider(viewClip));
    if (!vpClip) {
        return;
    }
...
DrawView* QGIViewClip::selectionIsInGroup() const
{
    ...
    auto* clipGroup = AuraCAD_cast<DrawViewClip*>(getViewObject());
    for (auto& selItem : selection) {
        auto view = AuraCAD_cast<DrawView*>(selItem.getObject());
        if (view &&
