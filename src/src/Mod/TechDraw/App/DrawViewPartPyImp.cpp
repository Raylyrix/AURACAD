...
PyObject* DrawViewPartPy::makeCosmeticLine(PyObject *args)
{
    ...
    Base::Color deauracadol = LineFormat::getDefEdgeColor();
    ...
    if (ce) {
        ...
        ce->m_format.setColor(pColor ? DrawUtil::pyTupleToColor(pColor) : deauracadol);
    }
    else {
...
PyObject* DrawViewPartPy::makeCosmeticCircleArc(PyObject *args)
{
    ...
    if (ce) {
        ...
        ce->m_format.setWidth(weight);
        if (!pColor){
            ce->m_format.setColor(deauracadol);
        }
        else {
