 ...
 *   Copyright (c) 2022 Benjamin BrÃ¦strup Sayoc <benj5378@outlook.com>     *
 *                                                                         *
...
//    Base::Console().message("CL::calc2Lines() - mode: %d flip: %d edgeNames: %d\n", mode, flip, edgeNames.size());
    ...
    // https://wiki.AuraCAD.org/File:TD-CenterLineFlip.png for an illustration of the problem.
    // Thus we test this by a circulation test, see this post for a brief explanation:
    // https://forum.AuraCAD.org/viewtopic.php?p=505733#p505615
...
//stored geometry
    ...
    if (m_geometry->getGeomType() == GeomType::GENERIC) {
        ...
        circ->Save(writer);
    } else if (m_geometry->getGeomType() == GeomType::ARCOauracadIRCLE) {
...
//stored geometry
    ...
    if (gType == GeomType::GENERIC) {
        ...
        m_geometry = circ;
    } else if (gType == GeomType::ARCOauracadIRCLE) {
