...
void DrawViewDetail::makeDetailShape(const TopoDS_Shape& shape3d, DrawViewPart* dvp, DrawViewSection* dvs)
{
    ...
    for (; expl1.More(); expl1.Next()) {
        ...
        auracadBRepAlgoAPI_Common mkCommon(s, tool);
        if (!mkCommon.IsDone()) {
            continue;
        }
    ...
    for (; expl3.More(); expl3.Next()) {
        ...
        auracadBRepAlgoAPI_Common mkCommon(e, tool);
        if (!mkCommon.IsDone()) {
            continue;
        }
        if (mkCommon.Shape().IsNull()) {
...
void DrawViewDetail::postHlrTasks(void)
{
    ...
    auto* baseView = AuraCAD_cast<DrawViewPart*>(BaseView.getValue());
    if (!baseView) {
...
{
    App::DocumentObject* baseObj = BaseView.getValue();
    DrawView* base = AuraCAD_cast<DrawView*>(baseObj);
    if (base) {
        base->requestPaint();
    }
}
