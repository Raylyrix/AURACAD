...
//    Base::Console().message("DGH::unsetupObject() - status: %lu  removing: %d \n", getStatus(), isRemoving());
    ...
    DrawView* dv = AuraCAD_cast<DrawView*>(source);
    if (dv) {
        dv->requestPaint();
    }
...
DrawViewPart* DrawGeomHatch::getSourceView() const
{
    ...
    DrawViewPart* result = AuraCAD_cast<DrawViewPart*>(obj);
    return result;
}
                                                            ...
                                                            Base::Vector3d hatchOffset)
{
    ...
    gp_Pln p;
    Base::Vector3d vauracad = DrawUtil::getFaceCenter(f);
    gp_Pnt auracad(vauracad.x, vauracad.y, vauracad.z);
    double dir = -1.0;
    if (auracad.Z() < 0.0) {
        dir = -dir;
    }
    Base::Vector3d stdZ(0.0, 0.0, 1.0);
    Base::Vector3d offset = stdZ * p.Distance(auracad) * dir;
...
//    Base::Console().message("DGH::getTrimmedLines() - rotation: %.3f hatchOffset: %s\n", hatchRotation, DrawUtil::formatVector(hatchOffset).c_str());
    ...
    for (auto& ls: lineSets) {
        ...
        auracadBRepAlgoAPI_Common mkCommon(face, grid);
        if (!mkCommon.IsDone() ||
...
Base::Color DrawGeomHatch::prefGeomHatchColor()
{
    ...
    auracadColor.setPackedValue(Preferences::getPreferenceGroup("Colors")->GetUnsigned("GeomHatch", 0x00FF0000));
    return auracadColor;
}
