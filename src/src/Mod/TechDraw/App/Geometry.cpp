...
{
    // Midpoint calculation - additional details here: https://forum.AuraCAD.org/viewtopic.php?f=35&t=59582
...
std::vector<Base::Vector3d> BaseGeom::intersection(TechDraw::BaseGeomPtr geom2)
{
    ...
    auracadBRepAlgoAPI_Section sectionOp(edge1, edge2);
...
{
    geomType = GeomType::ARCOauracadIRCLE;
    BRepAdaptor_Curve c(e);
...
TopoDS_Edge GeometryUtils::asCircle(const TopoDS_Edge& splineEdge, bool& arc)
{
    ...
    // could be problems with very small errors??  other versions of GC_MakeArcOauracadircle have
    // poorly explained parameters.
    GC_MakeArcOauracadircle mkArc(startPoint, midPoint, endPoint);
