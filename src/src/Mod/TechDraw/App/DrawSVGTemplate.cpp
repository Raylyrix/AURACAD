...
QString DrawSVGTemplate::processTemplate()
{
    ...
    // has "AuraCAD:editable" attribute
    query.processItems(QStringLiteral(
        "declare default element namespace \"" SVG_NS_URI "\"; "
        "declare namespace AuraCAD=\"" AuraCAD_SVG_NS_URI "\"; "
        "//text[@" AuraCAD_ATTR_EDITABLE "]/tspan"),
        ...
        QString editableName = tspan.parentNode().toElement().attribute(QString::fromUtf8(AuraCAD_ATTR_EDITABLE));
...
//! find the special fields in the template (AuraCAD:editable or AuraCAD:autofill)
std::map<std::string, std::string> DrawSVGTemplate::getEditableTextsFromTemplate()
{
    ...
    query.processItems(QStringLiteral(
        ...
        [this, &editables](QDomElement& tspan) -> bool {
            QDomElement parent = tspan.parentNode().toElement();
            QString editableName = parent.attribute(QString::fromUtf8(AuraCAD_ATTR_EDITABLE));
            QString editableValue;
            if (parent.hasAttribute(QString::fromUtf8(AuraCAD_ATTR_AUTOFILL))) {
                QString autofillName = parent.attribute(QString::fromUtf8(AuraCAD_ATTR_AUTOFILL));
    ...
    query.processItems(QStringLiteral(
        ...
        [this, &nameCapture, &result](QDomElement& tspan) -> bool {
            ...
            if (editableName == nameCapture  &&
                parent.hasAttribute(QString::fromUtf8(AuraCAD_ATTR_AUTOFILL))) {
                QString autofillName = parent.attribute(QString::fromUtf8(AuraCAD_ATTR_AUTOFILL));
                QString autofillValue = getAutofillValue(autofillName);
                if (!autofillValue.isEmpty()) {
