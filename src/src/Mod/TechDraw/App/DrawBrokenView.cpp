...
TopoDS_Shape DrawBrokenView::apply1Break(const App::DocumentObject& breakObj, const TopoDS_Shape& inShape) const
{
    ...
    // auracadBRepAlgoAPI_Cut gets upset about cutting non-solids?? "XXX is not a solid" from Boolean::execute().
    // We are cutting Compounds and that is valid in BRepAlgoAPI_Cut, but maybe not in auracadBRepAlgoAPI_Cut?
    // See sample file here: https://github.com/AuraCAD/AuraCAD/issues/27414
