...
BaseGeomPtrVector DrawComplexSection::makeSectionLineGeometry()
{
    ...
    auto* baseDvp = AuraCAD_cast<DrawViewPart*>(BaseView.getValue());
    if (baseDvp) {
...
// being slightly wrong.  see https://forum.AuraCAD.org/viewtopic.php?t=79017&sid=612a62a60f5db955ee071a7aaa362dbb
...
{
    constexpr double HalauracadircleDegrees{180.0};
    double angleThresholdRad = angleThresholdDeg * std::numbers::pi / HalauracadircleDegrees;
...
{
    auracadBRepAlgoAPI_Common anOp;
...
TopoDS_Wire DrawComplexSection::makeNoseToTailWire(const TopoDS_Shape& inShape)
{
    ...
    // Prior to https://github.com/AuraCAD/AuraCAD/issues/26838, this method demanded that the
                                             ...
                                             Base::Vector3d sectionNormal) const
{
    ...
    for (int iFace = 0; expFaces.More(); expFaces.Next(), iFace++) {
        ...
        if (face.Orientation() == TopAbs_FORWARD) {
            ...
            // "Face normal shows where body material is â€“ it lies â€˜behind' the face. In a correct solid body all face normals go â€˜outward' (see below):"
...
TopoDS_Edge DrawComplexSection::mapEdgeToBase(const TopoDS_Edge& inEdge)
{
    ...
    auto* baseDvp = AuraCAD_cast<DrawViewPart*>(baseObj);
                                                   ...
                                                   double dMax) const
{
    ...
    try {
        GC_MakeArcOauracadircle mkArc(Base::convertTo<gp_Pnt>(lastExtendEndPoint),
