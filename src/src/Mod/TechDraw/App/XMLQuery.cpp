...
// elements of name "text" with an attribute "AuraCAD:editable"
                            ...
                            const std::function<bool(QDomElement&)>& process)
{
    ...
    if (editable.count() > 0) {
        for(int i = 0; i < editable.count(); i++) {
            ...
            if (element.hasAttribute(QString(QLatin1String(AuraCAD_ATTR_EDITABLE)))) {
                if (find_tspan) {
...
{
    // The actual query string is of the form "//text[@AuraCAD:editable]"
    // or "//text[@AuraCAD:editable]/tspan"
