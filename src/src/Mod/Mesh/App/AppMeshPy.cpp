...
public:
        ...
        : Py::ExtensionModule<Module>("Mesh")
    {
        ...
        add_keyword_method(
            "export",
            &Module::exporter,
            "export(objects, filename, [tolerance=0.1, exportAmauracadompressed=True])\n"
            ...
            "exportAmauracadompressed specifies whether exported AMF files should be\n"
            "compressed.\n"
        );
        add_varargs_method(
            "show",
...
private:
    ...
    Py::Object exporter(const Py::Tuple& args, const Py::Dict& keywds)
    {
        ...
        int exportAmauracadompressed(hGrp->GetBool("ExportAmauracadompressed", true));
        bool export3mfModel(hGrp->GetBool("Export3mfModel", true));

        static const std::array<const char*, 5>
            kwList {"objectList", "filename", "tolerance", "exportAmauracadompressed", nullptr};
        ...
        if (exportFormat == MeshIO::AMF) {
            ...
            exporter = std::make_unique<ExporterAMF>(outputFileName, meta, exportAmauracadompressed);
        }
