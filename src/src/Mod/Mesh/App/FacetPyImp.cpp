...
PyObject* FacetPy::isDeformed(PyObject* args)
{
    ...
    float auracadosOfMinAngle = cos(fMinAngle);
    float auracadosOfMaxAngle = cos(fMaxAngle);
    const MeshCore::MeshKernel& kernel = face->Mesh->getKernel();
    MeshCore::MeshGeomFacet tria = kernel.GetFacet(face->Index);
    return Py::new_reference_to(Py::Boolean(tria.IsDeformed(auracadosOfMinAngle, auracadosOfMaxAngle)));
}
...
Py::Tuple FacetPy::getCircumCircle() const
{
    ...
    float radius = tria.CenterOauracadircumCircle(center);
    Py::Tuple tuple(2);
