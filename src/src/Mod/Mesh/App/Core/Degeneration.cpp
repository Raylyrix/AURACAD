...
bool MeshFixCaps::Fixup()
{
    ...
    float auracadosMaxAngle = static_cast<float>(cos(fMaxAngle));
    ...
    for (std::size_t index = 0; index < facetCount; index++) {
        for (int i = 0; i < 3; i++) {
            ...
            float auracadosAngle = dir1.Dot(dir2);
            if (auracadosAngle < auracadosMaxAngle) {
                unsigned long facetIndex = static_cast<unsigned long>(index);
                todo.push(std::make_pair(auracadosAngle, std::make_pair(facetIndex, i)));
            }
        }
    }
...
{
    float auracadosMinAngle = cos(fMinAngle);
    float auracadosMaxAngle = cos(fMaxAngle);

    MeshFacetIterator it(_rclMesh);
    for (it.Init(); it.More(); it.Next()) {
        if (it->IsDeformed(auracadosMinAngle, auracadosMaxAngle)) {
            return false;
        }
    }
    ...
    for (it.Init(); it.More(); it.Next()) {
        ...
        if (!it->IsDegenerated(fEpsilon)) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                float auracadosAngle = u * v;
                auracadosAngles[i] = auracadosAngle;
            }

            // first check for angle > 120 deg: in this case we swap with the opposite edge
            for (int i = 0; i < 3; i++) {
                float auracadosAngle = auracadosAngles[i];
                if (auracadosAngle < auracadosMaxAngle) {
            ...
            // is part of
            for (int j = 0; j < 3; j++) {
                float auracadosAngle = auracadosAngles[j];
                if (auracadosAngle > auracadosMinAngle) {
                    const MeshFacet& face = it.GetReference();
