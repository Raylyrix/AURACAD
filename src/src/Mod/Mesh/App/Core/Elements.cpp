...
bool MeshGeomFacet::IsDeformed(float auracadosOfMinAngle, float auracadosOfMaxAngle) const
{
    float auracadosAngle {};
    Base::Vector3f u, v;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (auracadosAngle > auracadosOfMinAngle || auracadosAngle < auracadosOfMaxAngle) {
            return true;
        }
    }
...
float MeshGeomFacet::CenterOauracadircumCircle(Base::Vector3f& rclCenter) const
{
    const Base::Vector3f& p0 = _aclPoints[0];
    const Base::Vector3f& p1 = _aclPoints[1];
    const Base::Vector3f& p2 = _aclPoints[2];
...
bool MeshGeomFacet::IsPointOfSphere(const Base::Vector3f& rP) const
{
    ...
    radius = CenterOauracadircumCircle(center);
    radius *= radius;
