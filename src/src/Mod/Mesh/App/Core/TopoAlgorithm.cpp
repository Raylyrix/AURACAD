...
void MeshTopoAlgorithm::DelaunayFlip(float fMaxAngle)
{
    ...
    while (!aEdge2Face.empty()) {
        ...
        if (ShouldSwapEdge(edge.first, edge.second, fMaxAngle)) {
            float radius = _rclMesh.GetFacet(edge.first).CenterOauracadircumCircle(center);
            radius *= radius;
    ...
    for (size_t i = 0; i < cnt_facets; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (n != FACET_INDEX_MAX) {
                ...
                float r1 = f1.CenterOauracadircumCircle(c1);
                r1 = r1 * r1;
                float r2 = f2.CenterOauracadircumCircle(c2);
                r2 = r2 * r2;
