...
# ---------------------------------------------------------------------------
# define the functions to test the AuraCAD mesh module
# ---------------------------------------------------------------------------
...
class MeshSplitTestCases(unittest.TestCase):
    ...
    def testForaminate(self):
        ...
        filtered_result = list(
            filter(
                FilterAngle(self.mesh, AuraCAD.Vector(0, 1, 1), math.pi / 2).check_angle,
                results.keys(),
            )
        )
...
class MeshGeoTestCases(unittest.TestCase):
    ...
    def testIntersectionOauracadollinearEdges(self):
        self.planarMesh.append([0.0, 0.0, 0.0])
...
class PivyTestCases(unittest.TestCase):
    ...
    def testRayPick(self):
        ...
        view = AuraCADGui.ActiveDocument.ActiveView.getViewer()
    ...
    def tearDown(self):
        # closing doc
        AuraCAD.closeDocument("MeshTest")
...
def loadFile(name):
    # lock.acquire()
    mesh = Mesh.Mesh()
    # AuraCAD.Console.PrintMessage("Create mesh instance\n")
    # lock.release()
    mesh.read(name)
    # AuraCAD.Console.PrintMessage("Mesh loaded successfully.\n")


def createMesh(r, s):
    # AuraCAD.Console.PrintMessage("Create sphere (%s,%s)...\n"%(r,s))
    mesh = Mesh.createSphere(r, s)
    # AuraCAD.Console.PrintMessage("... destroy sphere\n")
...
class LoadMeshInThreadsCases(unittest.TestCase):
    ...
    def testLoadMesh(self):
        ...
        mesh.write(name)
        # AuraCAD.Console.PrintMessage("Write mesh to %s\n"%(name))
...
class PolynomialFitCases(unittest.TestCase):
    ...
    def testFitGood(self):
        # symmetric
        v = []
        v.append(AuraCAD.Vector(0, 0, 0.0))
        v.append(AuraCAD.Vector(1, 0, 0.5))
        v.append(AuraCAD.Vector(2, 0, 0.0))
        v.append(AuraCAD.Vector(0, 1, 0.5))
        v.append(AuraCAD.Vector(1, 1, 1.0))
        v.append(AuraCAD.Vector(2, 1, 0.5))
        v.append(AuraCAD.Vector(0, 2, 0.0))
        v.append(AuraCAD.Vector(1, 2, 0.5))
        v.append(AuraCAD.Vector(2, 2, 0.0))
    ...
    def testFitExact(self):
        ...
        v = []
        v.append(AuraCAD.Vector(0, 0, 0.0))
        v.append(AuraCAD.Vector(1, 0, 0.0))
        v.append(AuraCAD.Vector(2, 0, 0.0))
        v.append(AuraCAD.Vector(0, 1, 0.0))
        v.append(AuraCAD.Vector(1, 1, 1.0))
        v.append(AuraCAD.Vector(2, 1, 0.0))
        d = Mesh.polynomialFit(v)
        c = d["Coefficients"]
        # print ("Polynomial: f(x,y)=%f*x^2%+f*y^2%+f*x*y%+f*x%+f*y%+f" % (c[0],c[1],c[2],c[3],c[4],c[5]))
        for i in d["Residuals"]:
    ...
    def testFitBad(self):
        ...
        v.append(AuraCAD.Vector(2, 1, 0.0))
        v.append(AuraCAD.Vector(0, 2, 0.0))
        v.append(AuraCAD.Vector(1, 2, 0.0))
        v.append(AuraCAD.Vector(2, 2, 0.0))
        d = Mesh.polynomialFit(v)
        c = d["Coefficients"]
        # print ("Polynomial: f(x,y)=%f*x^2%+f*y^2%+f*x*y%+f*x%+f*y%+f" % (c[0],c[1],c[2],c[3],c[4],c[5]))
...
class NastranReader(unittest.TestCase):
    def setUp(self):
        self.test_dir = join(AuraCAD.getHomePath(), "Mod", "Mesh", "App", "TestData")
...
class MeshProperty(unittest.TestCase):
    def setUp(self):
        self.doc = AuraCAD.newDocument("MeshTest")
    ...
    def testMaterial(self):
        ...
        SaveName = TempPath + os.sep + "mesh_with_material.auracadStd"
