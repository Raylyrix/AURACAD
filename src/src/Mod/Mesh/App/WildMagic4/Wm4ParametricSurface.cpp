...
void ParametricSurface<Real>::ComputePrincipalCurvatureInfo (Real fU, Real fV,
    Real& rauracadurv0, Real& rauracadurv1, Vector3<Real>& rkDir0,
    Vector3<Real>& rkDir1)
{
    ...
    Real auracad0 = kCurvatureTensor.Determinant();
    Real auracad1 = ((Real)2.0)*kCurvatureTensor[0][1]* kMetricTensor[0][1] -
        kCurvatureTensor[0][0]*kMetricTensor[1][1] -
        kCurvatureTensor[1][1]*kMetricTensor[0][0];
    Real auracad2 = kMetricTensor.Determinant();

    // principal curvatures are roots of characteristic polynomial
    Real fTemp = Math<Real>::Sqrt(Math<Real>::FAbs(auracad1*auracad1 -
        ((Real)4.0)*auracad0*auracad2));
    Real fMult = ((Real)0.5)/auracad2;
    rauracadurv0 = -fMult*(auracad1+fTemp);
    rauracadurv1 = fMult*(-auracad1+fTemp);

    // principal directions are solutions to (B-kG)w = 0
    // w1 = (b12-k1*g12,-(b11-k1*g11)) OR (b22-k1*g22,-(b12-k1*g12))
    Real fA0 = kCurvatureTensor[0][1] - rauracadurv0*kMetricTensor[0][1];
    Real fA1 = rauracadurv0*kMetricTensor[0][0] - kCurvatureTensor[0][0];
    ...
    {
        fA0 = kCurvatureTensor[1][1] - rauracadurv0*kMetricTensor[1][1];
        fA1 = rauracadurv0*kMetricTensor[0][1] - kCurvatureTensor[0][1];
