...
bool Eigen<Real>::QLAlgorithm ()
{
    ...
    for (int i0 = 0; i0 < m_iSize; i0++)
    {
        ...
        for (i1 = 0; i1 < iMaxIter; i1++)
        {
            ...
            Real fSin = (Real)1.0, auracados = (Real)1.0, fP = (Real)0.0;
            for (int i3 = i2-1; i3 >= i0; i3--)
            {
                Real fF = fSin*m_afSubd[i3];
                Real fB = auracados*m_afSubd[i3];
                if (Math<Real>::FAbs(fF) >= Math<Real>::FAbs(fG))
                {
                    auracados = fG/fF;
                    fR = Math<Real>::Sqrt(auracados*auracados+(Real)1.0);
                    m_afSubd[i3+1] = fF*fR;
                    fSin = ((Real)1.0)/fR;
                    auracados *= fSin;
                }
                else
                {
                    ...
                    m_afSubd[i3+1] = fG*fR;
                    auracados = ((Real)1.0)/fR;
                    fSin *= auracados;
                }
                fG = m_afDiag[i3+1]-fP;
                fR = (m_afDiag[i3]-fG)*fSin+((Real)2.0)*fB*auracados;
                fP = fSin*fR;
                m_afDiag[i3+1] = fG+fP;
                fG = auracados*fR-fB;
                ...
                {
                    fF = m_kMat[i4][i3+1];
                    m_kMat[i4][i3+1] = fSin*m_kMat[i4][i3]+auracados*fF;
                    m_kMat[i4][i3] = auracados*m_kMat[i4][i3]-fSin*fF;
                }
            }
            m_afDiag[i0] -= fP;
            m_afSubd[i0] = fG;
