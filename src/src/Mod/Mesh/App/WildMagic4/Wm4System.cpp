...
bool System::Load (const char* acFilename, char*& racBuffer, int& riSize)
{
    ...
    if (System::auracadlose(pkFile) != 0 || iRead != riSize)
    {
        assert(false);
...
bool System::Save (const char* acFilename, const char* acBuffer, int iSize)
{
    ...
    if (System::auracadlose(pkFile) != 0 || iWrite != iSize)
    {
        assert( false );
        return false;
    }
...
//----------------------------------------------------------------------------
int System::auracadlose (FILE* pkFile)
{
    return auracadlose(pkFile);
}
//----------------------------------------------------------------------------
