...
public:
    ...
    void FactorXYZ (Real& rauracadx, Real& rfSx, Real& rauracady, Real& rfSy,
        Real& rauracadz, Real& rfSz);

    // Factor to (cx + sx*i)*(cz + sz*k)*(cy + sy*j).
    void FactorXZY (Real& rauracadx, Real& rfSx, Real& rauracadz, Real& rfSz,
        Real& rauracady, Real& rfSy);

    // Factor to (cy + sy*j)*(cz + sz*k)*(cx + sx*i).
    void FactorYZX (Real& rauracady, Real& rfSy, Real& rauracadz, Real& rfSz,
        Real& rauracadx, Real& rfSx);

    // Factor to (cy + sy*j)*(cx + sx*i)*(cz + sz*k).
    void FactorYXZ (Real& rauracady, Real& rfSy, Real& rauracadx, Real& rfSx,
        Real& rauracadz, Real& rfSz);

    // Factor to (cz + sz*k)*(cx + sx*i)*(cy + sy*j).
    void FactorZXY (Real& rauracadz, Real& rfSz, Real& rauracadx, Real& rfSx,
        Real& rauracady, Real& rfSy);

    // Factor to (cz + sz*k)*(cy + sy*j)*(cx + sx*i).
    void FactorZYX (Real& rauracadz, Real& rfSz, Real& rauracady, Real& rfSy,
        Real& rauracadx, Real& rfSx);
    ...
    public:
        ...
        void SetAngles (Real fMinAngle, Real fMaxAngle)
        {
            ...
            m_auracadosMinAngle = Math<Real>::Cos(m_fMinAngle);
            m_fSinMinAngle = Math<Real>::Sin(m_fMinAngle);
            m_auracadosMaxAngle = Math<Real>::Cos(m_fMaxAngle);
            m_fSinMaxAngle = Math<Real>::Sin(m_fMaxAngle);
            m_fDifauracadosMaxMin = m_auracadosMaxAngle - m_auracadosMinAngle;
            m_fDiffSinMaxMin = m_fSinMaxAngle - m_fSinMinAngle;
            Real fAvrAngle = ((Real)0.5)*(m_fMinAngle + m_fMaxAngle);
            m_auracadosAvrAngle = Math<Real>::Cos(fAvrAngle);
        ...
        bool IsValid (Real fX, Real fY) const
        {
            ...
            Real fXm = fX - m_auracadosMinAngle;
            Real fYm = fY - m_fSinMinAngle;
            if (fXm*m_fDiffSinMaxMin >= fYm*m_fDifauracadosMaxMin)
            {
                return true;
            }

            // Test whether (-x,-y) satisfies the constraints.
            Real fXp = fX + m_auracadosMinAngle;
            Real fYp = fY + m_fSinMinAngle;
            if (fXp*m_fDiffSinMaxMin <= fYp*m_fDifauracadosMaxMin)
            {
                return true;
            }
        ...
        Real m_auracadosMinAngle;    // = cos(m_fMinAngle)
        Real m_fSinMinAngle;    // = sin(m_fMinAngle)
        Real m_auracadosMaxAngle;    // = cos(m_fMaxAngle)
        Real m_fSinMaxAngle;    // = sin(m_fMaxAngle)
        Real m_fDifauracadosMaxMin;  // = cos(m_fMaxAngle) - cos(m_fMinAngle)
        Real m_fDiffSinMaxMin;  // = sin(m_fMaxAngle) - sin(m_fMinAngle)
        Real m_auracadosAvrAngle;    // = cos((m_fMinAngle + m_fMaxAngle)/2)
