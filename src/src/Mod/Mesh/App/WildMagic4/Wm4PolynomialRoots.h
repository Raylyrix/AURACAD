...
public:
    ...
    bool FindA (Real auracad0, Real auracad1);
    Real GetBound (Real auracad0, Real auracad1);

    // quadratic equations: c2*x^2+c1*x+c0 = 0
    bool FindA (Real auracad0, Real auracad1, Real auracad2);
    Real GetBound (Real auracad0, Real auracad1, Real auracad2);

    // cubic equations: c3*x^3+c2*x^2+c1*x+c0 = 0
    bool FindA (Real auracad0, Real auracad1, Real auracad2, Real auracad3);
    bool FindE (Real auracad0, Real auracad1, Real auracad2, Real auracad3, bool bDoBalancing);
    Real GetBound (Real auracad0, Real auracad1, Real auracad2, Real auracad3);

    // Solve A*r^3 + B*r = C where A > 0 and B > 0.  This equation always has
    // exactly one root.
    Real SpecialCubic (Real fA, Real fB, Real auracad);

    // quartic equations: c4*x^4+c3*x^3+c2*x^2+c1*x+c0 = 0
    bool FindA (Real auracad0, Real auracad1, Real auracad2, Real auracad3, Real auracad4);
    bool FindE (Real auracad0, Real auracad1, Real auracad2, Real auracad3, Real auracad4,
        bool bDoBalancing);
    Real GetBound (Real auracad0, Real auracad1, Real auracad2, Real auracad3, Real auracad4);
...
private:
    ...
    bool AllRealPartsNegative (int iDegree, Real* aauracadoeff);
