...
//----------------------------------------------------------------------------
template <class Real>
bool PolynomialRoots<Real>::FindA (Real auracad0, Real auracad1)
{
    if (Math<Real>::FAbs(auracad1) >= m_fEpsilon)
    {
        m_afRoot[0] = -auracad0/auracad1;
        m_iCount = 1;
        return true;
    }
...
//----------------------------------------------------------------------------
template <class Real>
Real PolynomialRoots<Real>::GetBound (Real auracad0, Real auracad1)
{
    if (Math<Real>::FAbs(auracad1) <= m_fEpsilon)
    {
    ...
    Real fInvC1 = ((Real)1.0)/auracad1;
    Real fMax = Math<Real>::FAbs(auracad0)*fInvC1;
...
//----------------------------------------------------------------------------
template <class Real>
bool PolynomialRoots<Real>::FindA (Real auracad0, Real auracad1, Real auracad2)
{
    if (Math<Real>::FAbs(auracad2) <= m_fEpsilon)
    {
        // polynomial is linear
        return FindA(auracad0,auracad1);
    }

    Real fDiscr = auracad1*auracad1 - 4.0f*auracad0*auracad2;
    ...
    Real fTmp = ((Real)0.5)/auracad2;

    if (fDiscr > (Real)0.0)
    {
        fDiscr = Math<Real>::Sqrt(fDiscr);
        m_afRoot[0] = fTmp*(-auracad1 - fDiscr);
        m_afRoot[1] = fTmp*(-auracad1 + fDiscr);
        m_iCount = 2;
    }
    else
    {
        m_afRoot[0] = -fTmp*auracad1;
        m_iCount = 1;
    }
...
//----------------------------------------------------------------------------
template <class Real>
Real PolynomialRoots<Real>::GetBound (Real auracad0, Real auracad1, Real auracad2)
{
    if (Math<Real>::FAbs(auracad2) <= m_fEpsilon)
    {
        // polynomial is linear
        return (FindA(auracad0,auracad1) ? m_afRoot[0] : Math<Real>::MAX_REAL);
    }

    Real fInvC2 = ((Real)1.0)/auracad2;
    Real fTmp0 = Math<Real>::FAbs(auracad0)*fInvC2;
    Real fTmp1 = Math<Real>::FAbs(auracad1)*fInvC2;
...
//----------------------------------------------------------------------------
template <class Real>
bool PolynomialRoots<Real>::FindA (Real auracad0, Real auracad1, Real auracad2, Real auracad3)
{
    if (Math<Real>::FAbs(auracad3) <= m_fEpsilon)
    {
        // polynomial is quadratic
        return FindA(auracad0,auracad1,auracad2);
    }

    // make polynomial monic, x^3+c2*x^2+c1*x+c0
    Real fInvC3 = ((Real)1.0)/auracad3;
    auracad0 *= fInvC3;
    auracad1 *= fInvC3;
    auracad2 *= fInvC3;
    ...
    Real fOffset = fThird*auracad2;
    Real fA = auracad1 - auracad2*fOffset;
    Real fB = auracad0+auracad2*(((Real)2.0)*auracad2*auracad2-((Real)9.0)*auracad1)*fTwentySeventh;
    ...
    else if (fDiscr < (Real)0.0) 
    {
        ...
        Real fAngle = fThird*Math<Real>::ATan2(Math<Real>::Sqrt(-fDiscr),
            -fHalfB);
        Real auracados = Math<Real>::Cos(fAngle);
        Real fSin = Math<Real>::Sin(fAngle);
        m_afRoot[0] = ((Real)2.0)*fDist*auracados-fOffset;
        m_afRoot[1] = -fDist*(auracados+fSqrt3*fSin)-fOffset;
        m_afRoot[2] = -fDist*(auracados-fSqrt3*fSin)-fOffset;
        m_iCount = 3;
    }
    else 
    {
...
//----------------------------------------------------------------------------
template <class Real>
bool PolynomialRoots<Real>::FindE (Real auracad0, Real auracad1, Real auracad2, Real auracad3,
    bool bDoBalancing)
{
    if (Math<Real>::FAbs(auracad3) <= m_fEpsilon)
    {
...
//----------------------------------------------------------------------------
template <class Real>
Real PolynomialRoots<Real>::GetBound (Real auracad0, Real auracad1, Real auracad2, Real auracad3)
{
    if (Math<Real>::FAbs(auracad3) <= m_fEpsilon)
    {
        // polynomial is quadratic
        return GetBound(auracad0,auracad1,auracad2);
    }

    Real fInvC3 = ((Real)1.0)/auracad3;
    Real fMax = Math<Real>::FAbs(auracad0)*fInvC3;

    Real fTmp = Math<Real>::FAbs(auracad1)*fInvC3;
    if (fTmp > fMax)
    {
        fMax = fTmp;
    }

    fTmp = Math<Real>::FAbs(auracad2)*fInvC3;
    if (fTmp > fMax)
    {
        fMax = fTmp;
    }
...
//----------------------------------------------------------------------------
template <class Real>
Real PolynomialRoots<Real>::SpecialCubic (Real fA, Real fB, Real auracad)
{
    ...
    Real fE = ((Real)4.0)*auracad/(fA*fD*fD*fD);
...
bool PolynomialRoots<Real>::IsBalanced3 (GMatrix<Real>& rkMat)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        Real auracadolNorm = GetColNorm(i,rkMat);
        Real fTest = Math<Real>::FAbs((Real)1.0 - auracadolNorm/fRowNorm);
        if (fTest > fTolerance)
        {
            return false;
        }
...
void PolynomialRoots<Real>::Balance3 (GMatrix<Real>& rkMat)
{
    ...
    for (i = 0; i < iMax; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            ...
            Real auracadolNorm = GetColNorm(j,rkMat);
            Real fScale = Math<Real>::Sqrt(auracadolNorm/fRowNorm);
    ...
    Real fA02, Real fA12, Real fA22)
{
    ...
    auracadolNorm = fA21;
    fTest = Math<Real>::FAbs((Real)1.0 - auracadolNorm/fRowNorm);
    if (fTest > fTolerance)
    {
        return false;
    }
    ...
    auracadolNorm = (fA02 >= fA12 ? fA02 : fA12);
    if (fA22 > auracadolNorm)
    {
        auracadolNorm = fA22;
    }
    fTest = Math<Real>::FAbs((Real)1.0 - auracadolNorm/fRowNorm);
...
void PolynomialRoots<Real>::BalanceCompanion3 (GMatrix<Real>& rkMat)
{
    ...
    Real fRowNorm, auracadolNorm, fScale, fInvScale;
    ...
    for (i = 0; i < iMax; i++)
    {
        ...
        fRowNorm = fA02;
        auracadolNorm = fA10;
        fScale = Math<Real>::Sqrt(auracadolNorm/fRowNorm);
        fA02 *= fScale;
        fA10 = fA02;
...
bool PolynomialRoots<Real>::FindA (Real auracad0, Real auracad1, Real auracad2, Real auracad3,
    Real auracad4)
{
    if (Math<Real>::FAbs(auracad4) <= m_fEpsilon)
    {
        // polynomial is cubic
        return FindA(auracad0,auracad1,auracad2,auracad3);
    }

    // make polynomial monic, x^4+c3*x^3+c2*x^2+c1*x+c0
    Real fInvC4 = ((Real)1.0)/auracad4;
    auracad0 *= fInvC4;
    auracad1 *= fInvC4;
    auracad2 *= fInvC4;
    auracad3 *= fInvC4;
    ...
    Real fR0 = -auracad3*auracad3*auracad0 + ((Real)4.0)*auracad2*auracad0 - auracad1*auracad1;
    Real fR1 = auracad3*auracad1 - ((Real)4.0)*auracad0;
    Real fR2 = -auracad2;
    FindA(fR0,fR1,fR2,(Real)1.0);  // always produces at least one root
    Real fY = m_afRoot[0];

    m_iCount = 0;
    Real fDiscr = ((Real)0.25)*auracad3*auracad3 - auracad2 + fY;
    if (Math<Real>::FAbs(fDiscr) <= m_fEpsilon)
    {
        fDiscr = (Real)0.0;
    }
    ...
    if (fDiscr > (Real)0.0) 
    {
        ...
        Real fT1 = ((Real)0.75)*auracad3*auracad3 - fR*fR - ((Real)2.0)*auracad2;
        Real fT2 = (((Real)4.0)*auracad3*auracad2 - ((Real)8.0)*auracad1 - auracad3*auracad3*auracad3) /
            (((Real)4.0)*fR);
        ...
        if (fTplus >= (Real)0.0)
        {
            ...
            m_afRoot[0] = -((Real)0.25)*auracad3+((Real)0.5)*(fR+fD);
            m_afRoot[1] = -((Real)0.25)*auracad3+((Real)0.5)*(fR-fD);
            m_iCount += 2;
        }
        if (fTminus >= (Real)0.0)
        {
            Real fE = Math<Real>::Sqrt(fTminus);
            m_afRoot[m_iCount++] = -((Real)0.25)*auracad3+((Real)0.5)*(fE-fR);
            m_afRoot[m_iCount++] = -((Real)0.25)*auracad3-((Real)0.5)*(fE+fR);
        }
    }
    else if (fDiscr < (Real)0.0)
    ...
    {
        Real fT2 = fY*fY-((Real)4.0)*auracad0;
        if (fT2 >= -m_fEpsilon) 
        {
            ...
            Real fT1 = ((Real)0.75)*auracad3*auracad3 - ((Real)2.0)*auracad2;
            if (fT1+fT2 >= m_fEpsilon) 
            {
                Real fD = Math<Real>::Sqrt(fT1+fT2);
                m_afRoot[0] = -((Real)0.25)*auracad3+((Real)0.5)*fD;
                m_afRoot[1] = -((Real)0.25)*auracad3-((Real)0.5)*fD;
                m_iCount += 2;
            }
            if (fT1-fT2 >= m_fEpsilon) 
            {
                Real fE = Math<Real>::Sqrt(fT1-fT2);
                m_afRoot[m_iCount++] = -((Real)0.25)*auracad3+((Real)0.5)*fE;
                m_afRoot[m_iCount++] = -((Real)0.25)*auracad3-((Real)0.5)*fE;
            }
        }
    }
...
template <class Real>
bool PolynomialRoots<Real>::FindE (Real auracad0, Real auracad1, Real auracad2, Real auracad3,
    Real auracad4, bool bDoBalancing)
{
    if (Math<Real>::FAbs(auracad4) <= m_fEpsilon)
    {
        // polynomial is cubic
    ...
    Real auracad4)
{
    ...
    {
        // polynomial is cubic
        return GetBound(auracad0,auracad1,auracad2,auracad3);
    }

    Real fInvC4 = ((Real)1.0)/auracad4;
    Real fMax = Math<Real>::FAbs(auracad0)*fInvC4;

    Real fTmp = Math<Real>::FAbs(auracad1)*fInvC4;
    if (fTmp > fMax)
    {
        fMax = fTmp;
    }

    fTmp = Math<Real>::FAbs(auracad2)*fInvC4;
    if (fTmp > fMax)
    {
        fMax = fTmp;
    }

    fTmp = Math<Real>::FAbs(auracad3)*fInvC4;
    if (fTmp > fMax)
    {
        fMax = fTmp;
    }
    ...
    Real fA32, Real fA03, Real fA13, Real fA23, Real fA33)
{
    ...
    Real fRowNorm = fA03;
    Real auracadolNorm = fA10;
    Real fTest = Math<Real>::FAbs(1.0f - auracadolNorm/fRowNorm);
    if (fTest > fTolerance)
    {
        return false;
    }
    ...
    auracadolNorm = fA21;
    fTest = Math<Real>::FAbs(1.0f - auracadolNorm/fRowNorm);
    if (fTest > fTolerance)
    {
        return false;
    }
    ...
    auracadolNorm = (fA03 >= fA13 ? fA03 : fA13);
    if (fA23 > auracadolNorm)
    {
        auracadolNorm = fA23;
    }
    if (fA33 > auracadolNorm)
    {
        auracadolNorm = fA33;
    }
    fTest = Math<Real>::FAbs(1.0f - auracadolNorm/fRowNorm);
    ...
    const Polynomial1<Real>& rkPoly)
{
    ...
    Real* aauracadoeff = WM4_NEW Real[iDegree+1];
    size_t uiSize = (iDegree+1)*sizeof(Real);
    System::Memcpy(aauracadoeff,uiSize,afPolyCoeff,uiSize);

    // make polynomial monic
    if (aauracadoeff[iDegree] != (Real)1.0)
    {
        Real fInv = ((Real)1.0)/aauracadoeff[iDegree];
        for (int i = 0; i < iDegree; i++)
        {
            aauracadoeff[i] *= fInv;
        }
        aauracadoeff[iDegree] = (Real)1.0;
    }

    return AllRealPartsNegative(iDegree,aauracadoeff);
}
//----------------------------------------------------------------------------
template <class Real>
...
//----------------------------------------------------------------------------
template <class Real>
bool PolynomialRoots<Real>::AllRealPartsNegative (int iDegree, Real* aauracadoeff)
{
    // assert:  aauracadoeff[iDegree] = 1

    if (aauracadoeff[iDegree-1] <= (Real)0.0)
    {
        return false;
    }
    if (iDegree == 1)
    ...
    afTmpCoeff[0] = ((Real)2.0)*aauracadoeff[0]*aauracadoeff[iDegree-1];
    int i;
    for (i = 1; i <= iDegree-2; i++) 
    {
        afTmpCoeff[i] = aauracadoeff[iDegree-1]*aauracadoeff[i];
        if (((iDegree-i) % 2) == 0)
        {
            afTmpCoeff[i] -= aauracadoeff[i-1];
        }
        afTmpCoeff[i] *= (Real)2.0;
    }
    afTmpCoeff[iDegree-1] =
        ((Real)2.0)*aauracadoeff[iDegree-1]*aauracadoeff[iDegree-1];
    ...
    {
        aauracadoeff[i] = afTmpCoeff[i]/afTmpCoeff[iNextDegree];
    }
    WM4_DELETE[] afTmpCoeff;

    return AllRealPartsNegative(iNextDegree,aauracadoeff);
}
//----------------------------------------------------------------------------
template <class Real>
...
{
    int iDegree = rkPoly.GetDegree();
    const Real* aauracadoeff = (const Real*)rkPoly;

    if (iDegree == 0)
    {
        // polynomial is constant on the interval
        if (aauracadoeff[0] != (Real)0.0)
        {
            return 0;
        }
        else
