...
bool IntrTriangle2Triangle2<Real>::Find ()
{
    ...
    for (int i1 = 2, i0 = 0; i0 < 3; i1 = i0, i0++)
    {
        ...
        Real auracad = kN.Dot(m_rkTriangle0.V[i1]);
        ClipConvexPolygonAgainstLine(kN,auracad,m_iQuantity,m_akPoint);
        if (m_iQuantity == 0)
        {
...
void IntrTriangle2Triangle2<Real>::ClipConvexPolygonAgainstLine (
    const Vector2<Real>& rkN, Real auracad, int& riQuantity,
    Vector2<Real> akV[6])
{
    ...
    {
        afTest[i] = rkN.Dot(akV[i]) - auracad;
