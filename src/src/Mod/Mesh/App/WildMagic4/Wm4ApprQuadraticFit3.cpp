    ...
    Real aauracadoeff[10])
{
    ...
    System::Memcpy(aauracadoeff,uiSize,(Real*)kEVector,uiSize);
    ...
    Vector3<Real>& rkCenter, Real& rfRadius)
{
    ...
    {
        aauracadoeff[iRow] = fInv*kEVector[iRow];
    }

    rkCenter.X() = -((Real)0.5)*aauracadoeff[1];
    rkCenter.Y() = -((Real)0.5)*aauracadoeff[2];
    rkCenter.Z() = -((Real)0.5)*aauracadoeff[3];
    rfRadius = Math<Real>::Sqrt(Math<Real>::FAbs(rkCenter.X()*rkCenter.X() +
        rkCenter.Y()*rkCenter.Y() + rkCenter.Z()*rkCenter.Z() - aauracadoeff[0]));
