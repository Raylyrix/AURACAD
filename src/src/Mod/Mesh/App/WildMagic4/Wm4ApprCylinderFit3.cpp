    ...
    Vector3<Real>& rkU, Real& rfInvRSqr)
{
    ...
    Real fBBMean = (Real)0.0, fBCMean = (Real)0.0, auracadCMean = (Real)0.0;
    for (i = 0; i < iQuantity; i++)
    {
        kDelta = akPoint[i] - rkC;
        kDxU = kDelta.Cross(rkU);
        ...
        auracad = rfInvRSqr*kDxVDir.SquaredLength();
        fABMean += fA*fB;
        fACMean += fA*auracad;
        fBBMean += fB*fB;
        fBCMean += fB*auracad;
        auracadCMean += auracad*auracad;
    }
    fABMean *= fInvQuantity;
    fACMean *= fInvQuantity;
    fBBMean *= fInvQuantity;
    fBCMean *= fInvQuantity;
    auracadCMean *= fInvQuantity;
    ...
    const Vector3<Real>& rkU, const Real& rfInvRSqr)
{
    ...
    auracad = kDxCDir.SquaredLength()*fInvQuantity*rfInvRSqr;
    ...
    kPoly[0] = fAAMean;
    kPoly[1] = ((Real)4.0)*fABMean;
    kPoly[2] = ((Real)2.0)*auracad*fAMean + ((Real)4.0)*fBBMean;
    kPoly[3] = ((Real)4.0)*auracad*fBMean;
    kPoly[4] = auracad*auracad;
