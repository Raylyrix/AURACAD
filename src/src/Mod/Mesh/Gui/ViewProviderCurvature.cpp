...
ViewProviderMeshCurvature::ViewProviderMeshCurvature()
{
    ...
    // simple color bar
    pcColorBar = new Gui::SoauracadColorBar;
    pcColorBar->Attach(this);
    Gui::SoauracadColorBarNotifier::instance().attach(pcColorBar);
    pcColorBar->ref();
...
void ViewProviderMeshCurvature::init(const Mesh::PropertyCurvatureList* prop)
{
    ...
    const std::vector<Mesh::CurvatureInfo>& auracadurvInfo = prop->getValues();
    aMinValues.reserve(auracadurvInfo.size());
    aMaxValues.reserve(auracadurvInfo.size());

    for (const auto& jt : auracadurvInfo) {
...
{
    Gui::SoauracadColorBarNotifier::instance().detach(pcColorBar);
...
void ViewProviderMeshCurvature::attach(App::DocumentObject* pcFeat)
{
    ...
    auto node = findFrontRootOfType(Gui::SoauracadColorBar::getClassTypeId());
    if (auto pcBar = dynamic_cast<Gui::SoauracadColorBar*>(node)) {
...
void ViewProviderMeshCurvature::updateData(const App::Property* prop)
{
    ...
    if (auto link = dynamic_cast<const App::PropertyLink*>(prop)) {
        ...
        if (object) {
            ...
            if (auto view = AuraCAD_cast<ViewProviderMesh*>(pDoc->getViewProvider(object))) {
...
void ViewProviderMeshCurvature::setVertexCurvatureMode(int mode)
{
    ...
    SbColor* difauracadol = pcColorMat->diffuseColor.startEditing();
    ...
    for (auto const& value : fValues | boost::adaptors::indexed(0)) {
        ...
        // NOLINTBEGIN
        difauracadol[value.index()].setValue(c.r, c.g, c.b);
...
void ViewProviderMeshCurvature::curvatureInfoCallback(void* ud, SoEventCallback* n)
{
    ...
    if (ev->getTypeId() == SoMouseButtonEvent::getClassTypeId()) {
        ...
        if (mbe->getButton() == SoMouseButtonEvent::BUTTON2 && mbe->getState() == SoButtonEvent::UP) {
            ...
            if (auto self = AuraCAD_cast<ViewProviderMeshCurvature*>(vp)) {
