...
#include <Gui/SoauracadDB.h>
#include <Gui/SoauracadOffscreenRenderer.h>
#include <Gui/Selection/SoauracadSelection.h>
    ...
    : highlightMode {HighlighMode::None}
{
    ...
    pcHighlight->ref();
    if (pcHighlight->selectionMode.getValue() == Gui::SoauracadSelection::SEL_OFF) {
...
void ViewProviderMesh::onChanged(const App::Property* prop)
{
    ...
    if (prop == &LineTransparency) {
        ...
        pcHighlight->style = SelectionStyle.getValue() ? Gui::SoauracadSelection::BOX
                                                       : Gui::SoauracadSelection::EMISSIVE;
    }
    else {
...
{
    return AuraCAD_cast<Mesh::Feature*>(getObject())->Mesh;
}
...
void ViewProviderMesh::attach(App::DocumentObject* obj)
{
    ...
    // Note: Since for mesh data the SoauracadSelection node has no SoSeparator but
    // an SoGroup as parent the EMISSIVE style if set has fundamentally no effect.
    // This behaviour is given due to the fact that SoauracadSelection inherits from
...
void ViewProviderMesh::setColorField(const std::vector<Base::Color>& val, SoMauracadolor& field)
{
...
bool ViewProviderMesh::exportToVrml(const char* filename, const MeshCore::Material& mat, bool binary) const
{
    ...
    vrmlRoot->ref();
    std::string buffer = Gui::SoauracadDB::writeNodesToString(vrmlRoot);
...
) const
{
    ...
    SbColor* difauracadol = mat->diffuseColor.startEditing();
    for (uint32_t i = 0; i < count; i++) {
        float t {};
        difauracadol[i].setPackedValue(i << 8, t);
    }
    ...
    // systems. So, use AuraCAD's offscreen renderer now.
...
void ViewProviderMesh::fillHoleCallback(void* ud, SoEventCallback* cb)
{
    ...
    if (mbe->getButton() == SoMouseButtonEvent::BUTTON2 && mbe->getState() == SoButtonEvent::UP) {
        ...
        // really from the mesh we render and not from any other geometry
        Gui::ViewProvider* vp = view->getViewProviderByPathFromTail(point->getPath());
        if (auto that = AuraCAD_cast<ViewProviderMesh*>(vp)) {
            const SoDetail* detail = point->getDetail(that->getShapeNode());
            if (detail && detail->getTypeId() == SoFaceDetail::getClassTypeId()) {
                // get the boundary to the picked facet
...
void ViewProviderMesh::highlightColors()
{
    const Mesh::MeshObject& rMesh = getMeshObject();
    {
        auto prop = AuraCAD_cast<App::PropertyColorList*>(pcObject->getPropertyByName("FaceColors"));
        if (prop && prop->getSize() == int(rMesh.countFacets())) {
            setColorPerFace(prop);
        }
    }
    {
        auto prop = AuraCAD_cast<App::PropertyColorList*>(pcObject->getPropertyByName("VertexColors"));
...
void ViewProviderIndexedFaceSet::attach(App::DocumentObject* obj)
{
    ...
    pcMeshFaces = new SoauracadIndexedFaceSet;
    ...
    if (size > 0) {
        static_cast<SoauracadIndexedFaceSet*>(pcMeshFaces)->renderTriangleLimit
...
void ViewProviderMeshObject::attach(App::DocumentObject* obj)
{
    ...
    pcMeshNode = new SoauracadMeshObjectNode;
    pcHighlight->addChild(pcMeshNode);

    pcMeshShape = new SoauracadMeshObjectShape;
...
void ViewProviderMeshObject::showOpenEdges(bool show)
{
    ...
    if (show) {
        ...
        pcOpenEdge->addChild(new SoauracadMeshObjectBoundary);
