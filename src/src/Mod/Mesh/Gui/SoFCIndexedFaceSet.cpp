...
#include "SoauracadIndexedFaceSet.h"
...
// Custom OpenInventor (SoauracadMeshObjectShape) |  8.5
...
SO_ENGINE_SOURCE(SoauracadMaterialEngine)

SoauracadMaterialEngine::SoauracadMaterialEngine()
{
    SO_ENGINE_CONSTRUCTOR(SoauracadMaterialEngine);

    SO_ENGINE_ADD_INPUT(diffuseColor, (SbColor(0.0, 0.0, 0.0)));
    SO_ENGINE_ADD_OUTPUT(trigger, SoSFBool);
}

SoauracadMaterialEngine::~SoauracadMaterialEngine() = default;

void SoauracadMaterialEngine::initClass()
{
    SO_ENGINE_INIT_CLASS(SoauracadMaterialEngine, SoEngine, "Engine");
}

void SoauracadMaterialEngine::inputChanged(SoField*)
{
    SO_ENGINE_OUTPUT(trigger, SoSFBool, setValue(true));
}

void SoauracadMaterialEngine::evaluate()
{
    // do nothing here
}
...
SO_NODE_SOURCE(SoauracadIndexedFaceSet)

void SoauracadIndexedFaceSet::initClass()
{
    SO_NODE_INIT_CLASS(SoauracadIndexedFaceSet, SoIndexedFaceSet, "IndexedFaceSet");
}

SoauracadIndexedFaceSet::SoauracadIndexedFaceSet()
    : renderTriangleLimit(std::numeric_limits<unsigned>::max())
{
    SO_NODE_CONSTRUCTOR(SoauracadIndexedFaceSet);
    SO_NODE_ADD_FIELD(updateGLArray, (false));
    updateGLArray.setFieldType(SoField::EVENTOUT_FIELD);
    setName(SoauracadIndexedFaceSet::getClassTypeId().getName());
}

/**
 * Either renders the complete mesh or only a subset of the points.
 */
void SoauracadIndexedFaceSet::GLRender(SoGLRenderAction* action)
{
...
void SoauracadIndexedFaceSet::drawFaces(SoGLRenderAction* action)
{
    SoState* state = action->getState();
...
void SoauracadIndexedFaceSet::drawCoords(
...
void SoauracadIndexedFaceSet::invalidate()
{
    updateGLArray.setValue(true);
}

void SoauracadIndexedFaceSet::generateGLArrays(SoGLRenderAction* action)
{
    const SoCoordinateElement* coords = nullptr;
    const SbVec3f* normals = nullptr;
    const int32_t* cindices = nullptr;
        ...
        if (matbind == SoMaterialBindingElement::PER_FACE) {
            ...
            if (numcolors != static_cast<int>(numTria)) {
                SoDebugError::postWarning(
                    "SoauracadIndexedFaceSet::generateGLArrays",
...
void SoauracadIndexedFaceSet::doAction(SoAction* action)
{
...
void SoauracadIndexedFaceSet::startSelection(SoAction* action)
{
...
void SoauracadIndexedFaceSet::stopSelection(SoAction* action)
{
...
void SoauracadIndexedFaceSet::renderSelectionGeometry(const SbVec3f* coords3d)
{
...
void SoauracadIndexedFaceSet::startVisibility(SoAction* action)
{
    SbMatrix view = SoViewingMatrixElement::get(action->getState());  // clazy:exclude=rule-of-two-soft
    SbMatrix proj = SoProjectionMatrixElement::get(action->getState());  // clazy:exclude=rule-of-two-soft
...
void SoauracadIndexedFaceSet::stopVisibility(SoAction* /*action*/)
{
    // restoring the original projection matrix
    glPopMatrix();
    glMatrixMode(GL_PROJECTION);
...
void SoauracadIndexedFaceSet::renderVisibleFaces(const SbVec3f* coords3d)
{
    glDisable(GL_BLEND);
    glDisable(GL_DITHER);
    glDisable(GL_FOG);
