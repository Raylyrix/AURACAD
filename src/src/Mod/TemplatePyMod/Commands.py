...
# AuraCAD TemplatePyMod module  
# (c) 2007 Juergen Riegel LGPL

#


# import AuraCAD modules
import AuraCAD, AuraCADGui,inspect
...
def addCommand(name,cmdObject):
	...
	for i in range(len(list)-1):
		source += list[i+1][pos:]
	AuraCADGui.addCommand(name,cmdObject,source)
...
class TemplatePyMod_Cmd3:
    ...
    def Activated(self):
        ...
        AuraCADGui.activateWorkbench("PythonQtWorkbench")
...
class SphereCreator:
	def __init__(self):
		...
		self.pt = Part
		self.mode = False
		AuraCAD.Console.PrintMessage("Create instance of SphereCreator\n")

	def __del__(self):
		AuraCAD.Console.PrintMessage("Delete instance of SphereCreator\n")

	def enter(self):
		if (self.mode):
			return
		AuraCAD.Console.PrintMessage("Enter sphere creation mode\n")
		self.av = AuraCADGui.ActiveDocument.ActiveView
	...
	def leave(self):
		if (not self.mode):
			return
		AuraCAD.Console.PrintMessage("Leave sphere creation mode\n")
	...
	def create(self, info):
		...
		if (down):
			...
			AuraCAD.Console.PrintMessage("Clicked on position: ("+str(pos[0])+", "+str(pos[0])+")")
			msg = " -> (%f,%f,%f)\n" % (pnt.x, pnt.y, pnt.z)
...
class TemplatePyMod_Cmd4:
	...
	def __del__(self):
		AuraCAD.Console.PrintError('TemplatePyMod_Cmd4 was destroyed\n')

	def Activated(self):
		if AuraCADGui.ActiveDocument is not None:
			self.sc.enter()
		else:
			AuraCAD.Console.PrintWarning('A 3d view must be created\n')
...
class TemplatePyMod_Cmd5:
	...
	def Activated(self):
		...
		if myRenderArea is None:
			...
			myRenderArea = sogui.SoGuiRenderArea(AuraCADGui.getMainWindow())
...
class TemplatePyGrp_1:
    def Activated(self):
        import AuraCAD
        AuraCAD.Console.PrintMessage("TemplatePyGrp_1\n")
...
class TemplatePyGrp_2:
    def Activated(self):
        import AuraCAD
        AuraCAD.Console.PrintMessage("TemplatePyGrp_2\n")
...
class TemplatePyGrp_3:
    def Activated(self):
        import AuraCAD
        AuraCAD.Console.PrintMessage("TemplatePyGrp_3\n")
...
#---------------------------------------------------------------------------
# Adds the commands to the AuraCAD command manager
#---------------------------------------------------------------------------
...
AuraCADGui.addCommand('TemplatePyMod_Cmd4',TemplatePyMod_Cmd4())
AuraCADGui.addCommand('TemplatePyMod_Cmd5',TemplatePyMod_Cmd5())
AuraCADGui.addCommand('TemplatePyMod_Cmd6',TemplatePyMod_Cmd6())
AuraCADGui.addCommand('TemplatePyGrp_1',TemplatePyGrp_1())
AuraCADGui.addCommand('TemplatePyGrp_2',TemplatePyGrp_2())
AuraCADGui.addCommand('TemplatePyGrp_3',TemplatePyGrp_3())
AuraCADGui.addCommand('TemplatePyGroup',TemplatePyGroup())
AuraCADGui.addCommand('TemplatePyCheckable',TemplatePyCheckable())
