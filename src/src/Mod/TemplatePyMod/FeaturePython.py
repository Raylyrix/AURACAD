...
class Box(PartFeature):
    ...
    def onChanged(self, fp, prop):
        ...
        AuraCAD.Console.PrintMessage("Change property: " + str(prop) + "\n")

    def execute(self, fp):
        ''' Print a short message when doing a recomputation, this method is mandatory '''
        AuraCAD.Console.PrintMessage("Recompute Python Box feature\n")
...
def makeBox():
    ...
    a=AuraCAD.ActiveDocument.addObject("Part::FeaturePython","Box")
    Box(a)
...
class Line:
    def __init__(self, obj):
        ...
        obj.addProperty("App::PropertyVector","p2","Line","End point", locked=True).p2=AuraCAD.Vector(1,0,0)
        obj.Proxy = self
...
def makeLine():
    doc=AuraCAD.newDocument()
    a=AuraCAD.ActiveDocument.addObject("Part::FeaturePython","Line")
    Line(a)
...
class Octahedron:
    ...
    def execute(self, fp):
        ...
        v2 = AuraCAD.Vector(fp.Length,0,0)
        v3 = AuraCAD.Vector(0,fp.Width,0)
        v4 = AuraCAD.Vector(fp.Length,fp.Width,0)
        v5 = AuraCAD.Vector(fp.Length/2,fp.Width/2,fp.Height/2)
        v6 = AuraCAD.Vector(fp.Length/2,fp.Width/2,-fp.Height/2)
...
def makeOctahedron():
    doc=AuraCAD.newDocument()
    a=AuraCAD.ActiveDocument.addObject("App::FeaturePython","Octahedron")
    Octahedron(a)
...
def makePoints():
    ...
    a=AuraCAD.ActiveDocument.addObject("Points::FeaturePython","Points")
    a.Points=p
    PointFeature(a)
...
def makeMesh():
    ...
    a=AuraCAD.ActiveDocument.addObject("Mesh::FeaturePython","Mesh")
...
class Molecule:
    def __init__(self, obj):
        ''' Add two point properties '''
        obj.addProperty("App::PropertyVector","p1","Line","Start point", locked=True)
        obj.addProperty("App::PropertyVector","p2","Line","End point", locked=True).p2=AuraCAD.Vector(5,0,0)
...
def makeMolecule():
    doc=AuraCAD.newDocument()
    a=AuraCAD.ActiveDocument.addObject("Part::FeaturePython","Molecule")
    Molecule(a)
...
def makeCircleSet():
    ...
    doc=AuraCAD.newDocument()
    a=AuraCAD.ActiveDocument.addObject("App::FeaturePython","Circles")
    CircleSet(a)
...
def makeEnumTest():
    AuraCAD.newDocument()
    a=AuraCAD.ActiveDocument.addObject("Part::FeaturePython","Enum")
    EnumTest(a)
...
def makeDistanceBolt():
    doc=AuraCAD.newDocument()
    bolt=AuraCAD.ActiveDocument.addObject("Part::FeaturePython","Distance_Bolt")
