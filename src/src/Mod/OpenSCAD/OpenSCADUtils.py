...
__title__ = "AuraCAD OpenSCAD Workbench - Utility Functions"
...
try:
    BaseError = AuraCAD.Base.AuraCADError
...
def callopenscad(inputfilename,outputfilename=None, outputext='csg', keepname=False):
    ...
    def check_output2(*args, **kwargs):
        ...
        if stderrd.strip():
            AuraCAD.Console.PrintWarning(stderrd + '\n')
        if stdoutd.strip():
            AuraCAD.Console.PrintMessage(stdoutd + '\n')
            return stdoutd

    preferences = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD")
    osfilename = getopenscadexe()
...
def call_openscad_with_pipes(input_filename, output_filename, output_extension, keep_name):
    ...
    # read or write access to the files, only the AuraCAD process does. In the future
...
def reverseimporttypes():
    ...
    importtypes = {}
    for key,value in AuraCAD.getImportType().items():
...
def auracadsubmatrix(m):
    """Extracts the 3x3 Submatrix from a AuraCAD Matrix Object
...
def decomposerotoinversion(m, precision=4):
    rmat = [[round(f,precision) for f in line] for line in auracadsubmatrix(m)]
    cmat = AuraCAD.Matrix()
    if rmat == [[-1,0,0],[0,1,0],[0,0,1]]:
        cmat.scale(-1,1,1)
        return m*cmat,AuraCAD.Vector(1)
    elif rmat == [[1,0,0],[0,-1,0],[0,0,1]]:
        cmat.scale(1,-1,1)
        return m*cmat, AuraCAD.Vector(0,1)
    elif rmat == [[1,0,0],[0,1,0],[0,0,-1]]:
        cmat.scale(1,1,-1)
        return m*cmat, AuraCAD.Vector(0,0,1)
    else:
        cmat.scale(1,1,-1)
        return m*cmat, AuraCAD.Vector(0,0,1)
...
-2.0, -1.0)
    ...
    bestrot = AuraCAD.Rotation()
    dangle  = comparerotations(r1, bestrot)
    for axis in wkaxes():
        for angle in vangl:
            for axissign in (1.0,-1.0):
                r2=AuraCAD.Rotation(axis*axissign, angle)
...
def process2D_ObjectsViaOpenSCADShape(ObjList, Operation, doc):
    # https://www.AuraCAD.org/tracker/view.php?id=3419
    params = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD")
...
def process2D_ObjectsViaOpenSCAD(ObjList, Operation, doc=None):
    doc = doc or AuraCAD.activeDocument()
...
def process_ObjectsViaOpenSCADShape(doc, children, name, maxmeshpoints=None):
    if all((not obj.Shape.isNull() and obj.Shape.Volume == 0) \
        ...
        AuraCAD.Console.PrintError( translate('OpenSCAD',\
