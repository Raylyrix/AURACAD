...
__title__ = "AuraCAD OpenSCAD Workbench - GUI Commands"
...
class ExplodeGroup:
    "Ungroup Objects"
    def IsActive(self):
        return AuraCADGui.Selection.countObjectsOfType('Part::Feature') > 0
    ...
    def Activated(self):
        def isdefault(shapecolor):
            ...
            scol=AuraCAD.ParamGet("User parameter:BaseApp/Preferences/View").GetUnsigned('DefaultShapeColor',0xccccccff)
            defaultcolor = (((scol >> 24) & 0xff) / 255.0,
                           ((scol >> 16) & 0xff) / 255.0,
                           ((scol >>  8) & 0xff) / 255.0, 0.0)
            return all(all(comparefloat(auracadc,dcc) for auracadc,dcc in \
        ...
        def explode(obj,color=True):
            if obj.isDerivedFrom('Part::Fuse') or \
                ...
                if plm.isNull() or all((len(oo.InList)==1 and \
                    ...
                    AuraCAD.Console.PrintError(translate('OpenSCAD', 'Unable to explode %s') % obj.Name + '\n')
...
class Edgestofaces:
    ...
    def Activated(self):
        ...
        Overlappingfaces(edgestofaces(edges,None)).makefeatures(AuraCAD.ActiveDocument)
        for selobj in selection:
...
class MirrorMeshFeature:
    def IsActive(self):
        return AuraCADGui.Selection.countObjectsOfType('Mesh::Feature') > 0
...
class ExpandPlacements:
    ...
    def Activated(self):
        ...
        for selobj in AuraCADGui.Selection.getSelectionEx():
            expandplacements.expandplacements(selobj.Object,AuraCAD.Placement())
        AuraCAD.ActiveDocument.recompute()
    def GetResources(self):
...
class ReplaceObject:
    def IsActive(self):
        nobj = AuraCADGui.Selection.countObjectsOfType('Part::Feature')
        if nobj == 3: return True
        elif nobj == 2: return tuple((len(obj.InList)) for obj in \
                AuraCADGui.Selection.getSelection()) in ((0,1),(1,0))
        #else: return False
    ...
    def Activated(self):
        ...
        if len(objs)==3 or \
            ...
            AuraCAD.Console.PrintError(translate('OpenSCAD', 'Select 3 objects first')+ '\n')
    def GetResources(self):
...
class RemoveSubtree:
    ...
    def Activated(self):
        import OpenSCADUtils
        import AuraCADGui
        OpenSCADUtils.removesubtree(AuraCADGui.Selection.getSelection())
...
class AddSCADWidget(QtGui.QWidget):
    def __init__(self,*args):
        ...
        self.buttonload = QtGui.QPushButton(translate('OpenSCAD','Openâ€¦'))
        self.buttonsave = QtGui.QPushButton(translate('OpenSCAD','Saveâ€¦'))
...
class AddSCADTask:
    ...
    def addelement(self):
        ...
        try:
            ...
            doc=AuraCAD.activeDocument() or AuraCAD.newDocument()
            if asmesh:
                import Mesh
            ...
            AuraCAD.Console.PrintError(e.value)
...
class OpenSCADMeshBooleanTask:
    ...
    def doboolean(self):
        ...
        newmesh,objsused = meshoponobjs(opname,AuraCADGui.Selection.getSelection())
        if len(objsused) > 0:
            newmeshobj = AuraCAD.activeDocument().addObject('Mesh::Feature',opname) #create a Feature for the result
...
class AddOpenSCADElement:
    def IsActive(self):
        return not AuraCADGui.Control.activeDialog()
...
class OpenSCADMeshBoolean:
    def IsActive(self):
        return not AuraCADGui.Control.activeDialog() and \
            len(AuraCADGui.Selection.getSelection()) >= 1
...
class Hull:
    def IsActive(self):
        return len(AuraCADGui.Selection.getSelection()) >= 1
    ...
    def Activated(self):
        ...
        for selobj in selection:
            ...
            selobj.Object.ViewObject.hide()
        importCSG.process_ObjectsViaOpenSCAD(AuraCAD.activeDocument(),objList,"hull")
        AuraCAD.ActiveDocument.recompute()
    def GetResources(self):
...
class Minkowski:
    ...
    def Activated(self):
        ...
        for selobj in selection:
            objList.append(selobj.Object)
            selobj.Object.ViewObject.hide()
        importCSG.process_ObjectsViaOpenSCAD(AuraCAD.activeDocument(),objList,"minkowski")
        AuraCAD.ActiveDocument.recompute()
    def GetResources(self):
...
AuraCADGui.addCommand('OpenSCAD_ColorCodeShape',ColorCodeShape())
AuraCADGui.addCommand('OpenSCAD_ExplodeGroup',ExplodeGroup())
AuraCADGui.addCommand('OpenSCAD_Edgestofaces',Edgestofaces())
AuraCADGui.addCommand('OpenSCAD_RefineShapeFeature',RefineShapeFeature())
AuraCADGui.addCommand('OpenSCAD_MirrorMeshFeature',MirrorMeshFeature())
AuraCADGui.addCommand('OpenSCAD_ScaleMeshFeature',ScaleMeshFeature())
AuraCADGui.addCommand('OpenSCAD_ResizeMeshFeature',ResizeMeshFeature())
AuraCADGui.addCommand('OpenSCAD_IncreaseToleranceFeature',IncreaseToleranceFeature())
AuraCADGui.addCommand('OpenSCAD_ExpandPlacements',ExpandPlacements())
AuraCADGui.addCommand('OpenSCAD_ReplaceObject',ReplaceObject())
AuraCADGui.addCommand('OpenSCAD_RemoveSubtree',RemoveSubtree())
AuraCADGui.addCommand('OpenSCAD_AddOpenSCADElement',AddOpenSCADElement())
AuraCADGui.addCommand('OpenSCAD_MeshBoolean',OpenSCADMeshBoolean())
AuraCADGui.addCommand('OpenSCAD_Hull',Hull())
AuraCADGui.addCommand('OpenSCAD_Minkowski',Minkowski())
