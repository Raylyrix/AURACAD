...
class OpenSCADWorkbench(Workbench):
    ...
    def __init__(self):
        ...
        self.__class__.ToolTip = (
            ...
            "AuraCAD utizes OpenSCAD's capability as a script-only based modeller that uses its own description language\n"
    ...
    def Initialize(self):
        ...
        param = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD")
        ...
        if not openscadfilename:
            ...
            if openscadfilename:  # automatic search was successful
                AuraCAD.addImportType("OpenSCAD Format (*.scad *.SCAD)", "importCSG")
                param.SetString(
        ...
        if openscadfilename:
            ...
            AuraCAD.Console.PrintWarning("OpenSCAD executable not found\n")
        ...
        if openscadfilename and transferMechanism == 0:
            ...
            if "snap" in openscadfilename:
                AuraCAD.Console.PrintMessage(
                    ...
                    + translate(
                        "OpenSCAD",
                        "If OpenSCAD execution fails to load the temporary file, use AuraCAD's OpenSCAD Workbench Preferences to change the transfer mechanism.",
                    )
                    + "\n"
                )
                ...
                AuraCAD.Console.PrintMessage(
                    translate(
                        "OpenSCAD",
                        "It looks like you may be using a sandboxed version of AuraCAD.",
                    )
                    + " "
                    + translate(
                        "OpenSCAD",
        ...
        AuraCADGui.addPreferencePage(":/ui/openscadprefs-base.ui", "OpenSCAD")
...
AuraCAD.__unit_test__ += ["TestOpenSCADGui"]
