...
__title__ = "AuraCAD OpenSCAD Workbench - 2D helper functions"
    ...
    for path,debug in zip(*findConnectedEdges(edgelist,eps=eps,debug=True)):
        ...
        if maxd < eps*2 and maxd > 0.000001: # OCC won't like it if maxd > 0.02:
            ...
            if True:
                ...
                #from draftlibs import auracadvec, auracadgeo
                w2=superWireReverse(debug,tobeclosed)
                wirelist.append(w2)
            else:#this locks up AuraCAD
...
def superWireReverse(debuglist,closed=False):
    ...
    try:
        ...
        from draftlibs.auracadgeo import findMidpoint #workaround for Version 0.12
    import Part
    print(debuglist)
    newedges = []
