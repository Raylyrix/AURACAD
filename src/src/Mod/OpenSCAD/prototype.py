...
class Node:
    ...
    def addtoAuraCAD(self,doc=None,auracadpar=None):
        ...
        if namel in multifeature:
            if len(self.children)>1:
                ...
                subobjs = [child.addtoAuraCAD(doc,obj) for child in self.children]
                obj.Shapes = subobjs
                for subobj in subobjs:
                    subobj.ViewObject.hide()
            elif len(self.children) == 1:
                obj = self.children[0].addtoAuraCAD(doc,auracadpar or True)
            else:
                obj = auracadpar
            ...
            if len(self.children) == 1:
                ...
                base = self.children[0].addtoAuraCAD(doc,obj)

                if len(self.children) == 2:
                    tool = self.children[1].addtoAuraCAD(doc,obj)
                else:
                    tool = Node(name='imp_union',\
                        children=self.children[1:]).addtoAuraCAD(doc,obj)
                obj.Base = base
                obj.Tool = tool
            ...
            and self.arguments['$fn']<=Node.fnmin: # polygonal
                if r1 == r2: # prismatic
                    ...
                    if self.arguments['center']:
                        p1.Placement = AuraCAD.Placement(\
                        AuraCAD.Vector(0.0,0.0,-h/2.0),AuraCAD.Rotation())
                        p2.Placement = AuraCAD.Placement(\
                        AuraCAD.Vector(0.0,0.0,h/2.0), AuraCAD.Rotation())
                    else:
                        p2.Placement = AuraCAD.Placement(\
                        AuraCAD.Vector(0.0,0.0,h),AuraCAD.Rotation())
...
#                solid.complement()
            ...
            if len(self.children) == 1:
                ...
                obj = Node(name='imp_union',\
                        children=self.children).addtoAuraCAD(doc,auracadpar or True)
            ...
            m1 = AuraCAD.Matrix(*tuple(m1l)) #That's the original matrix
            if isspecialorthogonalpython(auracadsubmatrix(m1)): #a Placement can represent the transformation
                if len(self.children) == 1:
                    obj = self.children[0].addtoAuraCAD(doc,auracadpar or True)
                else:
                    obj = Node(name='imp_union',\
                            children = self.children).addtoAuraCAD(doc,auracadpar or True)
                    #AuraCAD.Console.PrintMessage('obj %s\nmat %s/n' % (obj.Placement,m1))
                obj.Placement=AuraCAD.Placement(m1).multiply(obj.Placement)
                ...
                if len(self.children) == 1:
                    child = self.children[0].addtoAuraCAD(doc,obj)
                else:
                    child = Node(name='imp_union',\
                            children=self.children).addtoAuraCAD(doc,obj)
            ...
            if len(self.children)==0:
                base = Node('import',self.arguments).addtoAuraCAD(doc,obj)
            elif len(self.children)==1:
                base = self.children[0].addtoAuraCAD(doc,obj)
            else:
            ...
            obj.Angle = 360.00
            base.ViewObject.hide()
            obj.Placement=AuraCAD.Placement(AuraCAD.Vector(),AuraCAD.Rotation(0,0,90))
            ...
            if self.arguments['cut']:
                ...
                if not plane:
                    ...
                    -Node.planedim,-Node.planedim,0),AuraCAD.Rotation(0,0,0,1))
            ...
            if filename:
                ...
                if extension.lower() in reverseimporttypes()['Mesh']:
                    ...
                    import importDXF
                    global dxauracadache
                    layers = dxauracadache.get(id(doc),[])
                    if layers:
                    ...
                    try:
                        ...
                        AuraCAD.Console.PrintError('processing of dxf import failed\nRework \'%s\' manually\n' % layera)
                    ...
                    AuraCAD.Console.ErrorMessage('Filetype of %s not supported\n' % (filename))
                    raise(NotImplementedError)
                if obj: #handle origin and scale
                    if scale is not None and scale !=1:
                        ...
                        placement = AuraCAD.Placement(AuraCAD.Vector(*[-c for c in origin]),AuraCAD.Rotation())
                        obj.Placement = placement.multiply(obj.Placement)
                else:
                    AuraCAD.Console.ErrorMessage('Import of %s failed\n' % (filename))
            ...
            if len(self.children) == 2 and \
                ...
                if len(nonsphere) == 1:
                    child = nonsphere[0].addtoAuraCAD(doc,obj)
                else:
                    child = Node(name='imp_union',\
                        children=nonsphere).addtoAuraCAD(doc,obj)
                OffsetShape(obj,child,offset)
                ViewProviderTree(obj.ViewObject)
            elif False:
                raise(NotImplementedError)
                pass # handle rotated cylinders and select edges that
                     #radius = radius0 * m1.multiply(AuraCAD.Vector(0,0,1)).dot(edge.Curve.tangent(0)[0])
            else:
                raise(NotImplementedError)
        elif namel == 'surface':
            ...
            if lenchld == 1:
                AuraCAD.Console.PrintMessage('Not recognized %s\n' % (self))
                obj = self.children[0].addtoAuraCAD(doc,auracadpar)
            elif lenchld >1:
                obj = Node(name='imp_union',\
                        children=self.children).addtoAuraCAD(doc,auracadpar or True)
            else:
                obj = doc.addObject("Part::Feature",'Not_Impl_%s'%namel)
        if auracadpar == True: #We are the last real object, our parent is not rendered.
            return obj
...
def parseexpression(e):
    ...
    if el == 'true': return True
        ...
        try:
            ...
            import AuraCAD
            AuraCAD.Console.PrintMessage('%s\n' % (el))
            return 1.0
...
def open(filename):
    ...
    readfile(filename).addtoAuraCAD(doc)
    #doc.recompute()
    return doc
