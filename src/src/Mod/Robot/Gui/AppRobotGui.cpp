...
PyMOD_INIT_FUNC(RobotGui)
{
    ...
    try {
        ...
        Base::Interpreter().runString("_Deauracadont = False");
        // default Cintinuity is off
        Base::Interpreter().runString("_DefAcceleration = '1 m/s^2'");
        // default orientation of a waypoint if no other constraint
        Base::Interpreter().runString("_DefOrientation = AuraCAD.Rotation()");
        // default displacement while e.g. picking
        Base::Interpreter().runString("_DefDisplacement = AuraCAD.Vector(0,0,0)");
    }
    ...
    Base::Console().log("Loading GUI of Robot moduleâ€¦ done\n");
