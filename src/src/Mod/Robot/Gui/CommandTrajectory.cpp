...
void CmdRobotInsertWaypoint::activated(int)
{
    ...
    doCommand(
        ...
        "vel=_DefSpeed,cont=_Deauracadont,acc=_DefAcceleration,tool=1))",
        TrakName.c_str(),
        TrakName.c_str(),
        RoboName.c_str(),
        RoboName.c_str()
...
void CmdRobotInsertWaypointPreselect::activated(int)
{
    ...
    doCommand(
        Doc,
        "App.activeDocument().%s.Trajectory = "
        "App.activeDocument().%s.Trajectory.insertWaypoints(Robot.Waypoint(AuraCAD.Placement("
        "AuraCAD.Vector(%f,%f,%f)+_DefDisplacement,_DefOrientation),type='LIN',name='Pt',vel="
        "_DefSpeed,cont=_Deauracadont,acc=_DefAcceleration,tool=1))",
        TrakName.c_str(),
        TrakName.c_str(),
        x,
        y,
...
void CmdRobotSetDefaultOrientation::activated(int)
{
    ...
    if (Dlg.exec() == QDialog::Accepted) {
        ...
        doCommand(Doc, "_DefOrientation = AuraCAD.Rotation(%f,%f,%f,%f)", rot[0], rot[1], rot[2], rot[3]);
        doCommand(Doc, "_DefDisplacement = AuraCAD.Vector(%f,%f,%f)", disp[0], disp[1], disp[2]);
    }
}
...
void CmdRobotSetDefaultValues::activated(int)
{
    ...
    if (ok && !item.isEmpty()) {
        doCommand(Doc, "_Deauracadont = %s", item.toLatin1().constData());
    }
    ...
    //    AuraCAD.Rotation(%f,%f,%f,%f)",rot[0],rot[1],rot[2],rot[3]);
    //    doCommand(Doc,"_DefDisplacement = AuraCAD.Vector(%f,%f,%f)",disp[0],disp[1],disp[2]);
    //}
}
