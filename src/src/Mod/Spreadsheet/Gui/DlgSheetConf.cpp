...
)
{
    ...
    if (from.col() >= to.col()) {
        AuraCAD_THROWM(Base::RuntimeError, "Invalid cell range");
    }
    ...
    if (!init) {
        ...
        try {
            ...
            e.reportException();
            AuraCAD_THROWM(Base::RuntimeError, "Failed to parse expression for property");
        }
        if (expr->hasComponent() || !expr->isDerivedFrom<App::VariableExpression>()) {
            AuraCAD_THROWM(Base::RuntimeError, "Invalid property expression: " << expr->toString());
        }

        path = static_cast<App::VariableExpression*>(expr.get())->getPath();
        auto obj = path.getDocumentObject();
        if (!obj) {
            AuraCAD_THROWM(Base::RuntimeError, "Invalid object referenced in: " << expr->toString());
        }
        ...
        if (pseudoType
            ...
            AuraCAD_THROWM(Base::RuntimeError, "Invalid property referenced in: " << expr->toString());
        }
        return prop;
    }
    ...
    if (cell && cell->getExpression()) {
        ...
        if (expr->isDerivedFrom<FunctionExpression>()) {
            auto fexpr = AuraCAD_cast<FunctionExpression*>(cell->getExpression());
            if (fexpr
        ...
        auto vexpr = AuraCAD_cast<VariableExpression*>(expr);
        if (vexpr) {
            auto prop = AuraCAD_cast<PropertyEnumeration*>(vexpr->getPath().getProperty());
            if (prop) {
                auto obj = AuraCAD_cast<DocumentObject*>(prop->getContainer());
                if (obj && prop->hasName()) {
