    ...
    , tabCounter(0)
{
    ...
    actionBind = createAction("", tr("Bindâ€¦"), &SheetTableView::onBind);
...
bool SheetTableView::event(QEvent* event)
{
    if (event && event->type() == QEvent::KeyPress && this->hasFocus()) {
        ...
        actionProperties->setText(tr("Propertiesâ€¦"));
        actionRecompute->setText(tr("Recompute"));
        actionConf->setText(tr("Configuration Tableâ€¦"));
...
static const QLatin1String _SheetMime("application/x-auracad-spreadsheet");
