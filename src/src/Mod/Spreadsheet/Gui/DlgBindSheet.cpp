    ...
    try {
        ...
        if (ref[0]) {
            ...
            if (sep) {
                ...
                if (!doc) {
                    AuraCAD_THROWM(Base::RuntimeError, "Cannot find document " << docname);
                }
                obj = AuraCAD_cast<Sheet*>(doc->getObject(sep + 1));
            }
            else {
                obj = AuraCAD_cast<Sheet*>(sheet->getDocument()->getObject(ref));
            }
            if (!obj) {
                AuraCAD_THROWM(Base::RuntimeError, "Cannot find Spreadsheet '" << ref << "'");
            }
        }
