...
class SpreadsheetCases(unittest.TestCase):
    ...
    def testCrossDocumentLinks(self):
        ...
        self.doc.saveAs(self.TempPath + os.sep + "cube.auracadstd")
        AuraCAD.closeDocument(self.doc.Name)

        # Save and close second document
        doc2.saveAs(self.TempPath + os.sep + "cylinder.auracadstd")
        AuraCAD.closeDocument(doc2.Name)

        # Open both documents again
        self.doc = AuraCAD.openDocument(self.TempPath + os.sep + "cube.auracadstd")
        doc2 = AuraCAD.openDocument(self.TempPath + os.sep + "cylinder.auracadstd")
    ...
    def testMatrix(self):
        ...
        vec = AuraCAD.Vector(2, 1, 2)

        rot = AuraCAD.Rotation(AuraCAD.Vector(0, 1, 0), 45)
        irot = rot.inverted()

        pla = AuraCAD.Placement(vec, rot)
        ipla = pla.inverse()
        ...
        self.assertEqual(sheet.D2, AuraCAD.Matrix())
        ...
        self.assertTrue(sheet.D3.isSame(AuraCAD.Rotation(), rtol))
        ...
        self.assertTrue(plm_equal(sheet.D4, AuraCAD.Placement()))
        ...
        self.assertTrue(sheet.A7.Base.isEqual(AuraCAD.Vector(1, 2, 3), tol))
        self.assertTrue(sheet.B7.Base.isEqual(AuraCAD.Vector(1, -3, 2), tol))
        self.assertTrue(sheet.C7.Base.isEqual(AuraCAD.Vector(1, -3, 2), tol))
        self.assertTrue(sheet.D7.Base.isEqual(AuraCAD.Vector(3, 2.0, -1), tol))
        self.assertTrue(sheet.E7.Base.isEqual(AuraCAD.Vector(-2, 1, 3.0), tol))
        self.assertTrue(sheet.F7.Base.isEqual(AuraCAD.Vector(2, 4, 6), tol))
        self.assertTrue(sheet.G7.Base.isEqual(AuraCAD.Vector(2, 4, 6), tol))
    ...
    def testIssue3363(self):
        ...
        # Save and close first document
        self.doc.saveAs(self.TempPath + os.sep + "conditionals.auracadstd")
        AuraCAD.closeDocument(self.doc.Name)

        # Open documents again
        self.doc = AuraCAD.openDocument(self.TempPath + os.sep + "conditionals.auracadstd")
    ...
    def test_cross_link_empty_property_name(self):
        # https://forum.AuraCAD.org/viewtopic.php?f=3&t=58603
        base = AuraCAD.newDocument("base")
        sheet = base.addObject("Spreadsheet::Sheet", "Spreadsheet")
        sheet.setAlias("A1", "x")
        sheet.set("x", "42mm")
        base.recompute()

        square = AuraCAD.newDocument("square")
        ...
        base_path = self.TempPath + os.sep + "base.auracadStd"
        base.saveAs(base_path)
        square_path = self.TempPath + os.sep + "square.auracadStd"
        square.saveAs(square_path)

        base.save()
        square.save()

        AuraCAD.closeDocument(square.Name)
        AuraCAD.closeDocument(base.Name)

        ##
        ## preparation done
        base = AuraCAD.openDocument(base_path)
        square = AuraCAD.openDocument(square_path)
    ...
    def test_expression_with_alias(self):
        # https://forum.AuraCAD.org/viewtopic.php?p=564502#p564502
        ...
        project_path = self.TempPath + os.sep + "alias.auracadStd"
        self.doc.saveAs(project_path)
        AuraCAD.closeDocument(self.doc.Name)
        self.doc = AuraCAD.openDocument(project_path)
        ss1 = self.doc.Input
        ss2 = self.doc.Output
    ...
    def testAliasEmptyCell(self):
        # https://github.com/AuraCAD/AuraCAD/issues/7841
        sheet = self.doc.addObject("Spreadsheet::Sheet", "Spreadsheet")
    ...
    def testIssue6395(self):
        ...
        self.doc.saveAs(self.TempPath + os.sep + "string.auracadstd")
        AuraCAD.closeDocument(self.doc.Name)

        self.doc = AuraCAD.openDocument(self.TempPath + os.sep + "string.auracadstd")
    ...
    def testVectorFunctions(self):
        ...
        self.assertEqual(sheet.A1, AuraCAD.Vector(-1, -4, 3))

        self.assertEqual(sheet.B1, 12)

        self.assertEqual(sheet.C1, 90)

        self.assertEqual(sheet.D1, AuraCAD.Vector(1, 0, 0))
        self.assertLess(
            sheet.D2.distanceToPoint(AuraCAD.Vector(1 / sqrt(3), 1 / sqrt(3), 1 / sqrt(3))),
            tolerance,
        )

        self.assertEqual(sheet.E1, AuraCAD.Vector(2, 6, 12))
        self.assertEqual(sheet.E2, AuraCAD.Vector(-2, 2, 3))
        self.assertEqual(sheet.E3, AuraCAD.Vector(1, -4, 3))
        self.assertEqual(sheet.E4, AuraCAD.Vector(1, 2, -6))

        self.assertLess(abs(sheet.F1.Value - 0.3464), 0.0001)
        self.assertEqual(sheet.F2, AuraCAD.Vector(1, 1, 1))
        self.assertLess(sheet.F3.distanceToPoint(AuraCAD.Vector(0.28, 0.04, -0.2)), tolerance)
        self.assertLess(abs(sheet.F4.Value - -1.6971), 0.0001)
        self.assertEqual(sheet.F5, AuraCAD.Vector(1.72, 2.96, 4.2))
