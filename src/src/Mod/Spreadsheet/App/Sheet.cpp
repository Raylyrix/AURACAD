...
AuraCAD_LOG_LEVEL_INIT("Spreadsheet", true, true)
...
bool Sheet::exportToFile(const std::string& filename, char delimiter, char quoteChar, char escapeChar) const
{
    ...
    while (i != usedCells.end()) {
        ...
        if (auto p = AuraCAD_cast<PropertyQuantity*>(prop)) {
            field << p->getValue();
        }
        else if (auto p = AuraCAD_cast<PropertyFloat*>(prop)) {
            field << p->getValue();
        }
        else if (auto p = AuraCAD_cast<PropertyInteger*>(prop)) {
            field << p->getValue();
        }
        else if (auto p = AuraCAD_cast<PropertyString*>(prop)) {
            field << p->getValue();
        }
        else {
            assert(0);
...
Property* Sheet::setFloatProperty(CellAddress key, double value)
{
    ...
    if (!prop || !prop->is<PropertyFloat>()) {
        ...
        floatProp = AuraCAD_cast<PropertyFloat*>(addDynamicProperty(
            "App::PropertyFloat",
            name.c_str(),
            nullptr,
            nullptr,
...
Property* Sheet::setIntegerProperty(CellAddress key, long value)
{
    ...
    if (!prop || !prop->is<PropertyInteger>()) {
        ...
        intProp = AuraCAD_cast<PropertyInteger*>(addDynamicProperty(
            "App::PropertyInteger",
            name.c_str(),
            nullptr,
            nullptr,
...
Property* Sheet::setQuantityProperty(CellAddress key, double value, const Base::Unit& unit)
{
    ...
    if (!prop || !prop->is<PropertySpreadsheetQuantity>()) {
        ...
        quantityProp = AuraCAD_cast<PropertySpreadsheetQuantity*>(p);
    }
    else {
...
Property* Sheet::setStringProperty(CellAddress key, const std::string& value)
{
    std::string name = key.toString(CellAddress::Cell::ShowRowColumn);
    Property* prop = props.getDynamicPropertyByName(name.c_str());
    PropertyString* stringProp = AuraCAD_cast<PropertyString*>(prop);
    ...
    if (!stringProp) {
        ...
        stringProp = AuraCAD_cast<PropertyString*>(addDynamicProperty(
            "App::PropertyString",
            name.c_str(),
            nullptr,
            nullptr,
...
Property* Sheet::setObjectProperty(CellAddress key, Py::Object object)
{
    std::string name = key.toString(CellAddress::Cell::ShowRowColumn);
    Property* prop = props.getDynamicPropertyByName(name.c_str());
    PropertyPythonObject* pyProp = AuraCAD_cast<PropertyPythonObject*>(prop);
    ...
    if (!pyProp) {
        ...
        pyProp = AuraCAD_cast<PropertyPythonObject*>(addDynamicProperty(
...
void Sheet::updateProperty(CellAddress key)
{
    ...
    if (cell) {
        ...
        auto number = AuraCAD_cast<NumberExpression*>(output.get());
        if (number) {
            long l;
            auto constant = AuraCAD_cast<ConstantExpression*>(output.get());
            ...
            auto str_expr = AuraCAD_cast<StringExpression*>(output.get());
            if (str_expr) {
                setStringProperty(key, str_expr->getText().c_str());
            }
            else {
                Base::PyGILStateLocker lock;
                auto py_expr = AuraCAD_cast<PyObjectExpression*>(output.get());
                if (py_expr) {
...
DocumentObjectExecReturn* Sheet::execute()
{
    ...
    try {
        ...
        // Recompute cells
        AuraCAD_LOG("recomputing " << getFullName());
        for (auto& pos : make_order) {
            const auto& addr = VertexIndexList[pos];
            AuraCAD_TRACE(addr.toString());
            recomputeCell(addr);
        }
    }
    catch (std::exception&) {
...
void Sheet::onDocumentRestored()
{
    ...
    if (ret != DocumentObject::StdReturn) {
        AuraCAD_ERR("Failed to restore " << getFullName() << ": " << ret->Why);
        delete ret;
    }
}
