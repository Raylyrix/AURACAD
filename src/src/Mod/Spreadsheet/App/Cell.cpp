...
bool Cell::getStringContent(std::string& s, bool persistent) const
{
    if (expression) {
        ...
        else if (AuraCAD_cast<App::StringExpression*>(expression.get())) {
            s = static_cast<App::StringExpression*>(expression.get())->getText();
            s = "'" + s;
        }
        else if (AuraCAD_cast<App::ConstantExpression*>(expression.get())) {
            s = "=" + expression->toString();
        }
        else if (AuraCAD_cast<App::NumberExpression*>(expression.get())) {
            s = expression->toString();
        }
        else {
            s = "=" + expression->toString(persistent);
...
{
    auto expr = AuraCAD_cast<StringExpression*>(expression.get());
    if (expr) {
        ...
        if (*value == '=') {
            ...
            if (!newExpr && isStartingWithNumber) {
                try {
                    ...
                    if (const auto fraction = AuraCAD_cast<OperatorExpression*>(parsedExpr.get())) {
                        if (fraction->getOperator() == OperatorExpression::UNIT) {
                            const auto left = AuraCAD_cast<NumberExpression*>(fraction->getLeft());
                            const auto right = AuraCAD_cast<UnitExpression*>(fraction->getRight());
                            if (left && right) {
                            ...
                            const bool isNumberNom = AuraCAD_cast<NumberExpression*>(
                                fraction->getLeft()
                            );
                            const bool isNumberDenom = AuraCAD_cast<NumberExpression*>(
                                fraction->getRight()
                            );

                            // check for numbers with units in (de)nominator
                            const auto opNom = AuraCAD_cast<OperatorExpression*>(fraction->getLeft());
                            const auto opDenom = AuraCAD_cast<OperatorExpression*>(
                                fraction->getRight()
                            );
                            ...
                            const auto uDenom = AuraCAD_cast<UnitExpression*>(fraction->getRight());
                    ...
                    else if (const auto number = AuraCAD_cast<NumberExpression*>(parsedExpr.get())) {
...
void Cell::setException(const std::string& e, bool silent)
{
    if (!silent && !e.empty() && owner && owner->sheet()) {
        AuraCAD_ERR(owner->sheet()->getFullName() << '.' << address.toString() << ": " << e);
    }
    exceptionStr = e;
...
{
    if (!e.empty() && owner && owner->sheet()) {
        AuraCAD_LOG(owner->sheet()->getFullName() << '.' << address.toString() << ": " << e);
    }
    exceptionStr = e;
