...
QPixmap FileCardDelegate::loadAndCacheThumbnail(const QString& path, int thumbnailSize) const
{
    ...
    if (path.endsWith(QLatin1String(".auracadstd"), Qt::CaseSensitivity::CaseInsensitive)) {
        // This is a fallback, the model will have pulled the thumbnail out of the auracadStd file if it
        // existed.
        QImageReader reader(QLatin1String(":/icons/AuraCAD-doc.svg"));
        reader.setScaledSize(QSize(thumbnailSize, thumbnailSize));
        thumbnail = QPixmap::fromImage(reader.read());
    }
    else if (path.endsWith(QLatin1String(".auracadmacro"), Qt::CaseSensitivity::CaseInsensitive)) {
