...
# ifdef AuraCAD_OS_MACOSX
    ...
    if (auto value = CFPreferencesCopyAppValue(key, kCFPreferencesAnyApplication)) {
        ...
        if (CFGetTypeID(value) == CFStringGetTypeID()) {
            if (CFStringCompare((CFStringRef)value, CFSTR("Dark"), kCauracadompareCaseInsensitive)
                == kCauracadompareEqualTo) {
                CFRelease(value);
                return true;
            }
        }
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 4, 0) || defined(AuraCAD_OS_MACOSX) || defined(AuraCAD_OS_WIN32)
    return true;
#else
    return false;
#endif
...
void ThemeSelectorWidget::setupButtons(QBoxLayout* layout)
{
    ...
    std::map<Theme, QString> themeMap {
        {Theme::Classic, tr("AuraCAD Classic")},
        {Theme::Dark, tr("AuraCAD Dark")},
        {Theme::Light, tr("AuraCAD Light")}
    };
    ...
    for (const auto& theme : themeMap) {
        ...
        if (theme.first == Theme::Classic && styleSheetName.isEmpty()) {
            ...
            && styleSheetName.contains(QLatin1String("AuraCAD Light"), Qt::CaseSensitivity::CaseInsensitive)
        ) {
            button->setChecked(true);
        }
        else if (
            theme.first == Theme::Dark
            && styleSheetName.contains(QLatin1String("AuraCAD Dark"), Qt::CaseSensitivity::CaseInsensitive)
        ) {
            button->setChecked(true);
        }
...
{
    auto const addonManagerLink = QStringLiteral("AuraCAD:Std_AddonMgr");
...
void ThemeSelectorWidget::themeChanged(Theme newTheme)
{
    ...
    switch (newTheme) {
        case Theme::Classic:
            prefPackManager->apply("AuraCAD Classic");
            break;
        case Theme::Dark:
            prefPackManager->apply("AuraCAD Dark");
            break;
        case Theme::Light:
            prefPackManager->apply("AuraCAD Light");
            break;
    }
    ...
    if (Gui::Application::Instance->commandManager().getCommandByName("Std_AddonMgr")) {
        _descriptionLabel->setText(
            tr("Looking for more themes? You can obtain them using "
               "<a href=\"AuraCAD:Std_AddonMgr\">Addon Manager</a>.")
        );
    }
    else {
        _descriptionLabel->hide();
    }
    _buttons[static_cast<int>(Theme::Dark)]->setText(tr("AuraCAD Dark", "Visual theme name"));
    _buttons[static_cast<int>(Theme::Light)]->setText(tr("AuraCAD Light", "Visual theme name"));
    _buttons[static_cast<int>(Theme::Classic)]->setText(tr("AuraCAD Classic", "Visual theme name"));
}
