...
#include "auracadstdInfoSource.h"
...
static bool AuraCADCanOpen(const QString& extension)
{
...
void DisplayedFilesModel::addFile(const QString& filePath)
{
    ...
    if (lowercaseExtension == QLatin1String("auracadstd")) {
        const auto runner = new auracadstdInfoSource(filePath);
        connect(
            runner->signals(),
            &auracadstdInfoSource::Signals::infoAvailable,
            this,
            &DisplayedFilesModel::processNewauracadstdInfo
        );
        QThreadPool::globalInstance()->start(runner);
    }
    const QStringList ignoredExtensions {
        QLatin1String("auracadmacro"),
        QLatin1String("py"),
        QLatin1String("pyi"),
        QLatin1String("csv"),
        QLatin1String("txt")
    ...
    if (ignoredExtensions.contains(lowercaseExtension)) {
        // Don't try to generate a thumbnail for things like this: AuraCAD can read them, but
...
void DisplayedFilesModel::processNewauracadstdInfo(
