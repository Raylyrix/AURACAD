...
/// Load the thumbnail image data (if any) that is stored in an auracadStd file.
/// \returns The image bytes, or an empty QByteArray (if no thumbnail was stored)
static QByteArray loadauracadStdThumbnail(const App::ProjectFile& proj, const QString& filePath)
{
    try {
...
auracadstdInfoSource::auracadstdInfoSource(QString filePath)
    : _filePath(std::move(filePath))
{}

void auracadstdInfoSource::run()
{
    const std::string stdFilePath(_filePath.toStdString());
    App::ProjectFile proj(stdFilePath);
    proj.loadDocument();
    auto fileStats = getProjectFileInfo(proj);
    auto thumbnail = loadauracadStdThumbnail(proj, _filePath);
