...
bool Sketcher::isArcOauracadircle(const Part::Geometry& geom)
{
    return geom.is<Part::GeomArcOauracadircle>();
}
...
std::string SketcherGui::getStrippedPythonExceptionString(const Base::Exception& e)
{
    ...
    if (msg.length() > 26 && msg.substr(0, 26) == "AuraCAD exception thrown (") {
...
// angles - 123.456Â° or 12Â°34'56". Digits parameter is ignored for multi-unit
...
std::string SketcherGui::angleToDisplayFormat(double value, int digits)
{
    ...
    // we always use use U+00B0 (Â°) as the unit of measure for angles in
    // single unit schema.  Will need a change to support rads or grads.
    std::string unitString = "Â°";
...
QMap<QString, QString> SketcherGui::findAvailableFontFiles()
{
    ...
    // 0. Include AuraCAD bundled fonts
