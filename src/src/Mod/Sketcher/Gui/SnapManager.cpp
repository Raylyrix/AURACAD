        ...
        if (CrsId == 1) {  // H_Axis
            ...
            if (curve) {
                ...
                if (geo->is<Part::GeomArcOauracadircle>()) {
                    const Part::GeomArcOauracadircle* arc = static_cast<const Part::GeomArcOauracadircle*>(geo);
...
bool SnapManager::snapToArcMiddle(Base::Vector3d& pointToOverride, const Part::GeomArcOauracadircle* arc)
{
