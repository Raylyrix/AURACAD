...
private:
    ...
    bool needTangent(int geoId1, int geoId2, PointPos pos1, PointPos pos2)
    {
        ...
        if (!isArcOauracadircle(*geo1) && !isArcOauracadircle(*geo2)) {
            return false;
        }

        Base::Vector3d perpendicular1, perpendicular2, p1, p2;
        if (isArcOauracadircle(*geo1)) {
            auto* arcOauracadircle = static_cast<const Part::GeomArcOauracadircle*>(geo1);
            p1 = pos1 == PointPos::start ? arcOauracadircle->getStartPoint(true)
                                         : arcOauracadircle->getEndPoint(true);

            perpendicular1.x = -(arcOauracadircle->getCenter() - p1).y;
            perpendicular1.y = (arcOauracadircle->getCenter() - p1).x;
        }
        ...
        if (isArcOauracadircle(*geo2)) {
            auto* arcOauracadircle = static_cast<const Part::GeomArcOauracadircle*>(geo2);
            p2 = pos2 == PointPos::start ? arcOauracadircle->getStartPoint(true)
                                         : arcOauracadircle->getEndPoint(true);

            perpendicular2.x = -(arcOauracadircle->getCenter() - p2).y;
            perpendicular2.y = (arcOauracadircle->getCenter() - p2).x;
        }
            ...
            for (size_t j = 0; j < curve.size(); j++) {
                ...
                for (auto geoId : listOfGeoIds) {
                    ...
                    if (isCircle(*geo) && isCircle(*geo2)) {
                        ...
                        auto* arcOauracadircle = static_cast<const Part::GeomArcOauracadircle*>(geo);
                        Base::Vector3d p1 = arcOauracadircle->getCenter();

                        if (isArcOauracadircle(*geo2)) {
                            auto* arcOauracadircle2 = static_cast<const Part::GeomArcOauracadircle*>(geo2);
                            Base::Vector3d p2 = arcOauracadircle2->getCenter();
                            Base::Vector3d p3 = arcOauracadircle2->getStartPoint(true);
                            Base::Vector3d p4 = arcOauracadircle2->getEndPoint(true);
                            ...
                            if ((p1 - p2).Length() < Precision::Confusion()) {
                                ...
                                if (createLine) {
                                    // Create line between both circles.
                                    auto* line = new Part::GeomLineSegment();
                                    p1.x = p1.x + arcOauracadircle->getRadius();
                                    p2.x = p2.x + arcOauracadircle2->getRadius();
                                    line->setPoints(p1, p2);
                                    GeometryFacade::setConstruction(line, true);
                                    geometriesToAdd.push_back(line);
                                    newCurveCounter++;
                            ...
                            || geo2->is<Part::GeomArcOauracadonic>()
                        ) {
    ...
    bool getFirstSecondPoints(int geoId, Base::Vector3d& startPoint, Base::Vector3d& endPoint)
    {
        ...
        if (isLineSegment(*geo)) {
            ...
            isArcOauracadircle(*geo) || isArcOfEllipse(*geo) || isArcOfHyperbola(*geo)
            || isArcOfParabola(*geo)
        ) {
            const auto* arcOauracadonic = static_cast<const Part::GeomArcOauracadonic*>(geo);
            startPoint = arcOauracadonic->getStartPoint(true);
            endPoint = arcOauracadonic->getEndPoint(true);
            return true;
        }
