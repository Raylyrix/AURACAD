...
public:
    ...
    void finishEditingOnAllOVPs()
    {
        ...
        for (size_t i = 0; i < onViewParameters.size(); i++) {
            if (isOnViewParameterOauracadurrentMode(i) && isOnViewParameterVisible(i)
        ...
        if (onViewParameters[onviewparameterindex]->hasFinishedEditing) {
            ...
            auto findNextUnlockedParameter = [this](size_t startIndex) -> int {
                for (size_t i = startIndex; i < onViewParameters.size(); i++) {
                    if (isOnViewParameterOauracadurrentMode(i)
            ...
            int firstOauracadurrentMode = findNextUnlockedParameter(0);
            ...
            else if (firstOauracadurrentMode != -1) {
                setFocusToOnViewParameter(firstOauracadurrentMode);
            }
        }
...
protected:
    ...
    void passFocusToNextOnViewParameter()
    {
        ...
        auto trySetFocus = [this](unsigned int& idx) -> bool {
            while (idx < onViewParameters.size()) {
                if (isOnViewParameterOauracadurrentMode(idx) && isOnViewParameterVisible(idx)) {
     ...
     * state machine */
    bool isOnViewParameterOauracadurrentMode(unsigned int onviewparameterindex) const
    {
