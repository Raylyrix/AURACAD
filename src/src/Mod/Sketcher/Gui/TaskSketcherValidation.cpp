...
void SketcherValidation::onFindReversedClicked()
{
    ...
    for (const auto geo : geom) {
        ...
        if (const auto segm = dynamic_cast<const Part::GeomArcOauracadircle*>(geo)) {
            if (segm->isReversed()) {
            ...
            Gui::TranslatedUserWarning(
                *sketch,
                ...
                tr("%1 reversed external geometry arcs were found. Their endpoints are"
                   ...
                   " Do this only once to sketches created in AuraCAD older than v0.15")
                    .arg(points.size() / 2)
                    .arg(nc)
            );
    ...
    Gui::TranslatedNotification(
        *sketch,
        ...
        tr("Orientation locking was enabled and recomputed for %1 constraints. The"
           " constraints have been listed in the report view (menu View â†’ Panels â†’"
           " Report view).")
            .arg(n)
    );
