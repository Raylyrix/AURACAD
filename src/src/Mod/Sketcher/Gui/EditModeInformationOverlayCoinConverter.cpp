...
void EditModeInformationOverlayCoinConverter::convert(const Part::Geometry* geometry, int geoid)
{
    ...
    else if (geometry->is<Part::GeomArcOauracadircle>()) {
        // at this point all calculations relate to ArcOauracadircle
...
)
{
    ...
    if constexpr (calculation == CalculationType::ArcCircleHelper) {
        const Part::GeomArcOauracadircle* arc = static_cast<const Part::GeomArcOauracadircle*>(geometry);
        ...
        if constexpr (calculation == CalculationType::BSplineDegree) {
            ...
            // https://github.com/tomate44/CurvesWB/blob/master/AuraCAD/Curves/ParametricComb.py
            // by AuraCAD user Chris_G
