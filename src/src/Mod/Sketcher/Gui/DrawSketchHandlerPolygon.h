...
private:
    unsigned int numberOauracadorners;
    ...
    void createShape(bool onlyeditoutline) override
    {
        ...
        double angleOfSeparation = 2.0 * std::numbers::pi
            / static_cast<double>(numberOauracadorners);  // NOLINT
        ...
        for (int i = 1; i <= static_cast<int>(numberOauracadorners); i++) {
...
void DSHPolygonController::configureToolWidget()
{
    ...
    toolWidget->setParameter(WParameter::First,
                             handler->numberOauracadorners);  // unconditionally set
...
void DSHPolygonController::adaptDrawingToParameterChange(int parameterindex, double value)
{
    switch (parameterindex) {
        case WParameter::First:
            handler->numberOauracadorners = std::max(3, static_cast<int>(value));
            break;
    }
}
