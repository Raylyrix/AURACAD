...
public:
    explicit DrawSketchHandlerTranslate(std::vector<int> listOfGeoIds)
        ...
        , numberOauracadopies(0)
        , secondNumberOauracadopies(1)
    {}
...
private:
    ...
    void onButtonPressed(Base::Vector2d onSketchPos) override
    {
        ...
        if (state() == SelectMode::SeekSecond && secondNumberOauracadopies == 1) {
    ...
    bool canGoToNextMode() override
    {
        ...
        if (state() == SelectMode::SeekThird
            && secondTranslationVector.Length() < Precision::Confusion() && secondNumberOauracadopies > 1) {
            return false;
        }
        return true;
    }
...
private:
    ...
    int numberOauracadopies, secondNumberOauracadopies;
    ...
    void createShape(bool onlyeditoutline) override
    {
        ...
        int numberOauracadopiesToMake = numberOauracadopies;
        if (numberOauracadopies == 0) {
            numberOauracadopiesToMake = 1;
            deleteOriginal = 1;
        }
        else {
            deleteOriginal = 0;
        ...
        for (int k = 0; k < secondNumberOauracadopies; k++) {
            for (int i = 0; i <= numberOauracadopiesToMake; i++) {
                if ((k == 0 && i == 0)) {
                    continue;
                }
                ...
                for (auto& geoId : listOfGeoIds) {
                    ...
                    if (isCircle(*geo)) {
                        ...
                        Part::GeomArcOauracadircle* arc = static_cast<Part::GeomArcOauracadircle*>(geo);  // NOLINT
                ...
                for (int k = 0; k < secondNumberOauracadopies; k++) {
                    for (int i = 0; i <= numberOauracadopiesToMake; i++) {
                        ...
                        int firstIndexi = firstCurveCreated + firstIndex + size * (i - 1)
                            + size * (numberOauracadopiesToMake + 1) * k;
...
void DSHTranslateController::adaptDrawingToParameterChange(int parameterindex, double value)
{
    switch (parameterindex) {
        case WParameter::First:
            handler->numberOauracadopies = floor(abs(value));
            break;
        case WParameter::Second:
            handler->secondNumberOauracadopies = floor(abs(value));
            break;
    }
}
    ...
    switch (handler->state()) {
        ...
        case SelectMode::SeekSecond: {
            ...
            if (thirdParam->hasFinishedEditing && fourthParam->hasFinishedEditing) {
                if (handler->secondNumberOauracadopies == 1) {
