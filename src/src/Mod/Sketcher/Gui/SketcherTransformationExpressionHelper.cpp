...
void SketcherTransformationExpressionHelper::copyExpressionsToNewConstraints(
    ...
    int numberOauracadopies,
    int secondNumberOauracadopies
)
{
    ...
    CopyCalculationParams params
        = calculateCopyParams(sketchObject, listOfGeoIds, shapeGeometrySize, numberOauracadopies);
    for (size_t i = 0; i < vals.size(); i++) {
        const auto& cstr = vals[i];
        if (!cstr->isDriving || !cstr->isDimensional()) {
            continue;
...
) const
{
    ...
    params.numberOauracadopiesToMake = numberOauracadopies == 0 ? 1 : numberOauracadopies;
    return params;
}
...
) const
{
    ...
    for (int k = 0; k < secondNumberOauracadopies; k++) {
        for (int copy = 1; copy <= params.numberOauracadopiesToMake; copy++) {
            int expectedNewGeoId = params.firstCurveCreated + originalIndex
                + params.size * (copy - 1) + params.size * params.numberOauracadopiesToMake * k;
