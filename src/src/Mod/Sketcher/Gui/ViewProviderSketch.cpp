...
// #include <Gui/Inventor/SoauracadSwitch.h>
...
#include <Mod/Part/Gui/SoauracadShapeObject.h>
...
{
    SO_NODE_INIT_CLASS(SoSketchFaces, SoauracadShape, "auracadShape");
}
    ...
    switch (key) {
        case SoKeyboardEvent::ESCAPE: {
            ...
            if (isInEditMode()) {
                ...
                // https://forum.AuraCAD.org/viewtopic.php?f=3&t=42207
...
void ViewProviderSketch::initDragging(int geoId, Sketcher::PointPos pos, Gui::View3DInventorViewer* viewer)
{
    ...
    for (auto geoIdi : selection.SelCurvSet) {
        ...
        if (geoIdi == geoId) {
            ...
            bool isArcMidDrag = (pos == Sketcher::PointPos::mid) && isArcOauracadircle(*geo);
...
void ViewProviderSketch::moveAngleConstraint(Sketcher::Constraint* constr, int constNum, const Base::Vector2d& toPos)
{
    ...
    if (constr->Second != GeoEnum::GeoUndef) {// line to line angle
        ...
        if (isLineSegment(*geo)) {
            ...
            const auto* arc = static_cast<const Part::GeomArcOauracadircle*>(geo);
            Base::Vector3d center = arc->getCenter();
            double startangle, endangle;
            arc->getRange(startangle, endangle, /*emulateCCW=*/true);
            double arcAngle = (startangle + endangle) / 2.;
                                        ...
                                        const Gui::View3DInventorViewer* viewer)
{
    ...
    for (std::vector<Part::Geometry*>::const_iterator it = geomlist.begin();
        ...
        else if ((*it)->isDerivedFrom<Part::GeomArcOauracadonic>()) {
            // Check if arc lies inside box selection
            const Part::GeomArcOauracadonic* aoc = static_cast<const Part::GeomArcOauracadonic*>(*it);
...
void SketcherGui::ViewProviderSketch::finishRestoring()
{
    ...
    // if AutoColor was not touched it means that the document is from older version of AuraCAD
    // that meaans that we need to run migration strategy and come up with a proper value
    if (!AutoColor.isTouched()) {
        // white is the normally provided default for AuraCAD sketch colors
