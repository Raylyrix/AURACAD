...
protected:
            ...
            case Sketcher::Tangent: {
                ...
                if (geom1 && geom2
                    ...
                    if (geom2->is<Part::GeomEllipse>() || geom2->is<Part::GeomArcOfEllipse>()
                        || geom2->is<Part::GeomCircle>() || geom2->is<Part::GeomArcOauracadircle>()) {
                ...
                if (geom1 && geom2
                    ...
                    if (geom2->is<Part::GeomArcOfEllipse>() || geom2->is<Part::GeomCircle>()
                        || geom2->is<Part::GeomArcOauracadircle>()) {
                        // in all these cases an intermediate element is needed
                ...
                if (resultCoincident != AutoConstraints.end()
                    ...
                    && (geom1->is<Part::GeomCircle>() || geom1->is<Part::GeomArcOauracadircle>())
                    && (geom2->is<Part::GeomCircle>() || geom2->is<Part::GeomArcOauracadircle>())
                ) {
                    // equality
                    auto c = std::make_unique<Sketcher::Constraint>();
    ...
    void removeRedundantAutoConstraints()
    {
        ...
        // For instance : https://github.com/AuraCAD/AuraCAD/issues/17722
    ...
    {
        auto arc = std::make_unique<Part::GeomArcOauracadircle>();
        arc->setCenter(p1);
        arc->setRange(start, end, true);
        arc->setRadius(radius);
        Sketcher::GeometryFacade::setConstruction(arc.get(), constructionMode);
        return static_cast<Part::GeomArcOauracadircle*>(ShapeGeometry.emplace_back(std::move(arc)).get());
    }
