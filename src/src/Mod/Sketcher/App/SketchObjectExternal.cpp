...
AuraCAD_LOG_LEVEL_INIT("Sketch", true, true)
...
int SketchObject::addExternal(App::DocumentObject* Obj, const char* SubName, bool defining, bool intersection)
{
    ...
    if (shapeType != TopAbs_SHAPE) {
        ...
        for (const auto& subShape : shape.getSubShapes(shapeType)) {
            ...
            if (intersection) {
                try {
                    auracadBRepAlgoAPI_Section maker(subShape, sketchPlane);
        ...
        const std::vector<int> &geoIds, App::DocumentObject *Obj, const char* SubName)
{
    ...
    for(auto &key : externalGeoRef) {
        if (*itObj == Obj  &&  *itSub == SubName){
            AuraCAD_ERR("Duplicate external element reference in " << getFullName() << ": " << key);
            return -1;
        }
                               ...
                               double tol)
{
    ...
    for (auto &geo : arcs) {
        if (auto arc = AuraCAD_cast<Part::GeomArcOauracadircle*>(geo.get())) {
            if (radius == 0.0) {
    ...
    GC_MakeArcOauracadircle arc(P1, midPoint, P2);
    auto* geo = new Part::GeomArcOauracadircle();
...
void processEdge2(TopoDS_Edge& projEdge, std::vector<std::unique_ptr<Part::Geometry>>& geos)
{
    ...
    if (projCurve.GetType() == GeomAbs_Line) {
        ...
        if (P1.SquareDistance(P2) < Precision::Confusion()) {
            ...
            auto* arc = new Part::GeomArcOauracadircle();
    ...
    if (curve.GetType() == GeomAbs_Line) {
        ...
        if (vec1.IsParallel(vec2, Precision::Confusion())) {
            ...
            if (beg.SquareDistance(end) < Precision::Confusion()) {
                ...
                auto* gArc = new Part::GeomArcOauracadircle();
            ...
            if (vec1.IsNormal(vec2, Precision::Angular())) {
                ...
                Base::Vector3d p1(P1.X(), P1.Y(), P1.Z());// ends of segment auracadAD style
                Base::Vector3d p2(P2.X(), P2.Y(), P2.Z());
                invPlm.multVec(p1, p1);
                invPlm.multVec(p2, p2);
                ...
                cnt = proj.NearestPoint();
                // converting to auracadAD style vector
                ...
                // maj axis into auracadAD style vector
        ...
        //
        // Doing that solves:
        // https://forum.AuraCAD.org/viewtopic.php?f=3&t=55284#p477522
...
void SketchObject::rebuildExternalGeometry(std::optional<ExternalToAdd> extToAdd)
{
    ...
    for (int i=0; i < int(Objects.size()); i++) {
        ...
        try {
            ...
            if (auto* datum = AuraCAD_cast<const Part::Datum*>(resolvedObj)) {
                refSubShape = datum->getShape();
            }
            else if (auto* refObj = AuraCAD_cast<const Part::Feature*>(resolvedObj)) {
                const Part::TopoShape& refShape = refObj->Shape.getShape();
                refSubShape = refShape.getSubShape(SubElement.c_str());
            }
            else if (auto* pl = AuraCAD_cast<const App::Plane*>(resolvedObj)) {
            ...
            else if (auto* line = AuraCAD_cast<const Part::DatumLine*>(resolvedObj)) {
            ...
            else if (auto* point = AuraCAD_cast<const Part::DatumPoint*>(resolvedObj)) {
            ...
            else if (auto* line = AuraCAD_cast<const App::Line*>(resolvedObj)) {
            ...
            else if (auto* point = AuraCAD_cast<const App::Point*>(resolvedObj)) {
            ...
            if (intersection && !refSubShape.IsNull()) {
                auracadBRepAlgoAPI_Section maker(refSubShape, sketchPlane);
                maker.Approximation(Standard_True);
                if (!maker.IsDone())
                    AuraCAD_THROWM(Base::CADKernelError, "Failed to get intersection");
            ...
            AuraCAD_ERR("Failed to project external geometry in "
        ...
        if(!refSet.emplace(key).second) {
            AuraCAD_WARN("Duplicated external reference in " << getFullName() << ": " << key);
            continue;
        }
    ...
    for(auto geo : geoms) {
        ...
        if(!refSet.count(egf->getRef())) {
            AuraCAD_ERR( "External geometry " << getFullName() << ".e" << egf->getId()
...
void SketchObject::fixExternalGeometry(const std::vector<int> &geoIds) {
    ...
    for(int i=2;i<(int)geos.size();++i) {
        ...
        auto pos = ref.find('.');
        if(pos == std::string::npos) {
            AuraCAD_ERR("Invalid geometry reference " << ref);
            continue;
        }
        std::string objName = ref.substr(0,pos);
        auto obj = getDocument()->getObject(objName.c_str());
        if(!obj) {
            AuraCAD_ERR("Cannot find object in reference " << ref);
            continue;
        }

        auto elements = Part::Feature::getRelatedElements(obj,ref.c_str()+pos+1);
        if(!elements.size()) {
            AuraCAD_ERR("No related reference found for " << ref);
            continue;
        }
...
void SketchObject::updateGeometryRefs()
{
        ...
        for(auto &v : refMap) {
            ...
            for (long id : it->second) {
                ...
                if (iter != externalGeoMap.end()) {
                    ...
                    auto egf = ExternalGeometryFacade::getFacade(geo);
                    // NOLINTNEXTLINE
                    AuraCAD_LOG(getFullName() << " ref change on ExternalEdge" << iter->second - 1 << ' '
                           << egf->getRef() << " -> " << v.second);
                    egf->setRef(v.second);
                    touched = true;
                }
    ...
    for (auto geo : geos) {
        ...
        if (egf->getId() < 0 && !egf->getRef().empty()) {
            // NOLINTNEXTLINE
            AuraCAD_ERR("External geometry reference corrupted in " << getFullName()
                   << " Please check.");
        ...
        if (getDocument() && !getDocument()->isPerformingTransaction()) {
            ...
            //
            // NOLINTNEXTLINE
            AuraCAD_WARN("Update legacy external reference "
                    << egf->getRef() << " -> " << externalGeoRef[it->second] << " in "
                    << getFullName());
        }
        else {
            // NOLINTNEXTLINE
            AuraCAD_LOG("Update undo/redo external reference "
                   << egf->getRef() << " -> " << externalGeoRef[it->second] << " in "
                   << getFullName());
        }
        touched = true;
