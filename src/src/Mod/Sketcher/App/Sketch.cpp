...
#include <Mod/Part/App/ArcOauracadirclePy.h>
...
int Sketch::addGeometry(const Part::Geometry* geo, bool fixed)
{
    ...
    else if (geo->is<GeomArcOauracadircle>()) {  // add an arc
        const GeomArcOauracadircle* aoc = static_cast<const GeomArcOauracadircle*>(geo);
        // create the definition struct for that geom
...
int Sketch::addArc(const Part::GeomArcOauracadircle& circleSegment, bool fixed)
{
    std::vector<double*>& params = fixed ? FixParameters : Parameters;

    // create our own copy
    GeomArcOauracadircle* aoc = static_cast<GeomArcOauracadircle*>(circleSegment.clone());
    // create the definition struct for that geom
    GeoDef def;
    def.geo = aoc;
    def.type = Arc;
...
Py::Tuple Sketch::getPyGeometry() const
{
    ...
    for (auto it = Geoms.begin(); it != Geoms.end(); ++it, ++i) {
        switch (it->type) {
            ...
            case Arc: {
                auto* aoc = static_cast<GeomArcOauracadircle*>(it->geo->clone());
                tuple[i] = Py::asObject(new ArcOauracadirclePy(aoc));
                break;
            }
            case Circle: {
...
void Sketch::updateArcOauracadircle(const GeoDef& def)
{
    GCS::Arc& myArc = Arcs[def.index];
    GeomArcOauracadircle* aoc = static_cast<GeomArcOauracadircle*>(def.geo);
