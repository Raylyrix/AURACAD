...
void SketchObject::buildShape()
{
    ...
    auto addEdge = [this, &shapes](auto geo, auto indexedName) {
        ...
        if (checkSmallEdge(shapes.back())) {
            AuraCAD_WARN("Edge too small: " << indexedName);
        }
    };
...
Part::TopoShape SketchObject::buildInternals(const Part::TopoShape &edges) const {
    ...
    try {
        ...
        AuraCAD_WARN("Failed to make face for sketch: " << e.what());
    } catch (Standard_Failure &e) {
        AuraCAD_WARN("Failed to make face for sketch: " << e.GetMessageString());
    }
    return Part::TopoShape();
}
...
public:
    ...
    void update(const Base::Vector3d &pt, long id) {
        AuraCAD_TRACE("update " << id << ", " << AuraCAD_xyz(pt));
        auto it = find(pt);
        if(it==adjlist.end()) {
...
void SketchObject::generateId(const Part::Geometry* geo)
{
    ...
    AuraCAD_TRACE("found " << found.front());
...
void SketchObject::onGeometryChanged()
{
    ...
    for (long i = 0; i < (long)vals.size(); ++i) {
        ...
        while (!geoMap.insert(std::make_pair(gf->getId(), i)).second) {
            AuraCAD_WARN("duplicate geometry id " << gf->getId() << " -> "
                    << geoLastId + 1);  // NOLINT
            gf->setId(++geoLastId);
        }
    }
...
void SketchObject::onExternalGeoChanged()
{
    ...
    for(int i=0; i<ExternalGeo.getSize(); ++i) {
        ...
        if (!externalGeoMap.emplace(egf->getId(), i).second) {
            AuraCAD_WARN("duplicate geometry id " << egf->getId() << " -> "
                    << geoLastId + 1);  // NOLINT
            egf->setId(++geoLastId);
...
void SketchObject::onExpressionEngineChanged()
{
    ...
    if (!isRestoring() && doc && !doc->isPerformingTransaction() && noRecomputes
        ...
        try {
            ...
            if (res) {
                AuraCAD_ERR("Failed to recompute " << ExpressionEngine.getFullName() << ": "
                       << res->Why);  // NOLINT
                delete res;
            }
        } catch (Base::Exception &e) {
    ...
    try {
        ...
        if(ExternalGeo.getSize()<=2) {
            ...
            if(ExternalGeometry.getSize()+2!=ExternalGeo.getSize())
                AuraCAD_WARN("Failed to restore some external geometry in " << getFullName());
        }else
            acceptGeometry();
        ...
        for (auto &v : ExpressionEngine.getExpressions()) {
            ...
            if (!cstr || !cstr->isDimensional()) {
                AuraCAD_WARN((cstr ? "Invalid" : "Orphan")
        ...
        e.reportException();
        AuraCAD_ERR("Error while restoring " << getFullName());
    } catch (...) {
    }
}
...
SketchObject::getHigherElements(const char *element, bool silent) const
{
    ...
    // see https://github.com/AuraCAD/AuraCAD/issues/20753
                                 ...
                                 std::shared_ptr<App::Expression> expr)
{
    ...
    if (noRecomputes) {
        ...
        try {
            ...
            e.reportException();
            AuraCAD_ERR("Failed to recompute " << ExpressionEngine.getFullName() << ": " << e.what());
        }
        solve();
    }
}
...
Data::IndexedName SketchObject::checkSubName(const char *subname) const
{
    ...
    if (!valid) {
        AuraCAD_ERR("invalid subname " << subname);
    ...
    if (geo && GeometryFacade::getId(geo) == id) {
        ...
        if ((iss >> sep >> posId) && sep == 'v') {
            ...
            // see https://github.com/AuraCAD/AuraCAD/issues/25089
    ...
    AuraCAD_ERR("cannot find subname " << subname);
