...
void SketchObject::reverseAngleConstraintToSupplementary(Constraint* constr, int constNum)
{
    ...
    if (expr) {
        ...
        auto op = AuraCAD_cast<App::OperatorExpression*>(expr.get());
        if (op && op->getOperator() == App::OperatorExpression::SUB) {
            auto leftNum = AuraCAD_cast<App::NumberExpression*>(op->getLeft());
            if (leftNum && leftNum->getQuantity() == Quantity(180)) {
                newExpr = op->getRight()->copy();
            }
            auto leftOp = AuraCAD_cast<App::OperatorExpression*>(op->getLeft());
            auto leftOpNum = leftOp ? AuraCAD_cast<App::NumberExpression*>(leftOp->getLeft()) : nullptr;
            auto leftOpUnit = leftOp ? AuraCAD_cast<App::UnitExpression*>(leftOp->getRight()) : nullptr;
        ...
        if (!newExpr) {
            ...
            auto* number = AuraCAD_cast<App::NumberExpression*>(result.get());
            ...
            if (!value.isDimensionless()) {
                valueExpr = new App::OperatorExpression(expr->getOwner(),
                    valueExpr, App::OperatorExpression::UNIT, new App::UnitExpression(expr->getOwner(), Base::Quantity::Degree, "Â°"));
            }
        ...
        auto* newNumber = AuraCAD_cast<App::NumberExpression*>(newResult.get());
...
int SketchObject::addCopyOauracadonstraints(const SketchObject& orig)
{
    // no need to check input data validity as this is an sketchobject managed operation.
    Base::StateLocker lock(managedoperation, true);
...
{
    if (auto* asCirc = AuraCAD_cast<const Part::GeomCircle*>(geo)) {
    ...
    if (auto* asArcOauracadirc = AuraCAD_cast<const Part::GeomArcOauracadircle*>(geo)) {
        auto loc = asArcOauracadirc->getLocation();
        return gp_Circ(gp_Ax2(gp_Pnt(loc.x, loc.y, loc.z), gp_Dir(1, 0, 0), gp_Dir(0, 1, 0)), std::max(asArcOauracadirc->getRadius(), 0.0));
    }
    return std::nullopt;
}
void SketchObject::setOrientationDistance(Constraint* constr)
{
    ...
    if (constr->FirstPos != PointPos::none && constr->Second != GeoEnum::GeoUndef) {
        auto* geo1AsLine = AuraCAD_cast<const Part::GeomLineSegment*>(getGeometry(constr->Second));
        if (geo1AsLine) {
    ...
    if (constr->FirstPos == PointPos::none && constr->SecondPos == PointPos::none && constr->Second != GeoEnum::GeoUndef) {
        ...
        auto* geo2AsLine = AuraCAD_cast<const Part::GeomLineSegment*>(secGeo);
...
) const
{
    ...
    switch (con->Type) {
        ...
        case Equal: {
            ...
            if (geo->isDerivedFrom<Part::GeomConic>() || geo->isDerivedFrom<Part::GeomArcOauracadonic>()) {
                Constraint* trans = con->copy();
                                             ...
                                             std::shared_ptr<const App::Expression> expr)
{
    ...
    for(long i=0; i<(long)vals.size(); ++i) {
        ...
        while(!geoMap.insert(std::make_pair(gf->getId(),i)).second) {
            AuraCAD_WARN("duplicate geometry id " << gf->getId() << " -> " << geoLastId+1);
            gf->setId(++geoLastId);
        }
    }
    updateGeoHistory();
    ...
    for (std::size_t ic = 0; ic < newVals.size(); ic++) {// ic = index of constraint
        ...
        for (int ig = 1; ig <= 3; ig++) {
            ...
            if (geoId <= GeoEnum::RefExt
                ...
                if (g->is<Part::GeomArcOauracadircle>()) {
                    const auto* segm = static_cast<const Part::GeomArcOauracadircle*>(g);
                    if (segm->isReversed()) {
