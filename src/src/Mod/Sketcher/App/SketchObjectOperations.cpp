                         ...
                         const Base::Vector3d& refPnt2, double radius, bool trim, bool createCorner, bool chamfer)
{
    ...
    int pos2 = 0;
    bool reverse = false;
    std::unique_ptr<Part::GeomArcOauracadircle> arc(createFilletGeometry(geo1, geo2, refPnt1, refPnt2, radius, pos1, pos2, reverse));
    if (!arc) {
        return -1;
    }
...
int SketchObject::extend(int GeoId, double increment, PointPos endpoint)
{
    ...
    else if (geom->is<Part::GeomArcOauracadircle>()) {
        auto* arc = static_cast<Part::GeomArcOauracadircle*>(geom);
...
int SketchObject::trim(int GeoId, const Base::Vector3d& point)
{
    ...
    bool geomHasMid = geoAsCurve->isDerivedFrom<Part::GeomConic>()
        || geoAsCurve->isDerivedFrom<Part::GeomArcOauracadonic>();
    if (geomHasMid) {
...
int SketchObject::split(int GeoId, const Base::Vector3d& point)
{
    ...
    if (geoAsCurve->is<Part::GeomArcOauracadircle>()) {
        auto* joint = new Constraint();
        joint->Type = Coincident;
        joint->First = newIds.front();
        joint->FirstPos = PointPos::mid;
...
)
{
    ...
    // see https://github.com/AuraCAD/AuraCAD/issues/13551
    // This does not happen with other arcs types.
    if (addSymmetryConstraints) {
        for (auto* geo : symgeos) {
            if (auto* arc = dynamic_cast<Part::GeomArcOauracadircle*>(geo)) {
                double start, end;
...

    {
        ...
        for (auto geoIdPair : geoIdMap) {
            ...
            if (geo->is<Part::GeomLineSegment>()) {
                ...
                geo->is<Part::GeomArcOauracadircle>()        //
...
)
{
    ...
    if (refIsLine) {
        ...
        for (auto geoId : geoIdList) {
            ...
            else if (geosym->is<Part::GeomArcOauracadircle>()) {
                auto* geoaoc = static_cast<Part::GeomArcOauracadircle*>(geosym);
...
)
{
    ...
    auto makeCopyAtRowColumn = [&](int x, int y) {
        ...
        for (auto it = newgeoIdList.cbegin(); it != newgeoIdList.cend(); ++it, ++index) {
            ...
            else if (geocopy->is<Part::GeomArcOauracadircle>()) {
                auto* geoaoc = static_cast<Part::GeomArcOauracadircle*>(geocopy);
                Base::Vector3d cp = geoaoc->getCenter();
                Base::Vector3d scp = cp + double(x) * displacement
                    + double(y) * perpendicularDisplacement;
...
bool SketchObject::convertToNURBS(int GeoId)
{
    ...
    try {
        ...
        if (geo1->isDerivedFrom<Part::GeomArcOauracadonic>()) {
            const auto* geoaoc = static_cast<const Part::GeomArcOauracadonic*>(geo1);
...
bool SketchObject::insertBSplineKnot(int GeoId, double param, int multiplicity)
{
    ...
    if (!delGeoId.empty()) {
        ...
        // https://forum.AuraCAD.org/viewtopic.php?f=19&t=64962&sid=10272db50a635c633260517b14ecad37.
