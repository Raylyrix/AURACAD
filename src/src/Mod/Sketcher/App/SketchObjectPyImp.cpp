...
PyObject* SketchObjectPy::addGeometry(PyObject* args)
{
    ...
    if (PyObject_TypeCheck(pcObj, &(Part::GeometryPy::Type))) {
        ...
        int ret;
        // An arc created with Part.Arc will be converted into a Part.ArcOauracadircle
        if (geo->is<Part::GeomTrimmedCurve>()) {
            ...
            || geo->is<Part::GeomArcOauracadircle>() || geo->is<Part::GeomArcOfEllipse>()
        ...
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            if (PyObject_TypeCheck((*it).ptr(), &(Part::GeometryPy::Type))) {
                ...
                if (geo->is<Part::GeomTrimmedCurve>()) {
                    ...
                    if (!circle.IsNull()) {
                        // create the definition struct for that geom
                        std::shared_ptr<Part::GeomArcOauracadircle> aoc(new Part::GeomArcOauracadircle());
                        aoc->setHandle(trim);
                    ...
                    || geo->is<Part::GeomEllipse>() || geo->is<Part::GeomArcOauracadircle>()
