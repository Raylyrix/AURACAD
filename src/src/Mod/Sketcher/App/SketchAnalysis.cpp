...
struct PointConstraints
{
    void addGeometry(const Part::Geometry* geo, int index)
    {
        ...
        else if (const auto* segm = dynamic_cast<const Part::GeomArcOauracadircle*>(geo)) {
            addArcOauracadircle(segm, index);
        }
    ...
    void addArcOauracadircle(const Part::GeomArcOauracadircle* segm, int index)
    {
        VertexIds id;
        id.GeoId = index;
        id.PosId = Sketcher::PointPos::start;
