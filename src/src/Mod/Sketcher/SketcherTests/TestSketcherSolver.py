...
# **************************************************************************
#   Copyright (c) 2011 Juergen Riegel <AuraCAD@juergen-riegel.net>        *
#                                                                         *
...
import AuraCAD, Part, Sketcher
from Part import Precision

App = AuraCAD

xy_normal = AuraCAD.Vector(0, 0, 1)


def vec(x, y):
    """Shorthand to create a vector in the XY-plane"""
    return AuraCAD.Vector(x, y, 0)
...
def CreateRectangleSketch(SketchFeature, corner, lengths):
    ...
    SketchFeature.addGeometry(
        Part.LineSegment(AuraCAD.Vector(hmin, vmax), AuraCAD.Vector(hmax, vmax, 0))
    )
    SketchFeature.addGeometry(
        Part.LineSegment(AuraCAD.Vector(hmax, vmax, 0), AuraCAD.Vector(hmax, vmin, 0))
    )
    SketchFeature.addGeometry(
        Part.LineSegment(AuraCAD.Vector(hmax, vmin, 0), AuraCAD.Vector(hmin, vmin, 0))
    )
    SketchFeature.addGeometry(
        Part.LineSegment(AuraCAD.Vector(hmin, vmin, 0), AuraCAD.Vector(hmin, vmax, 0))
    )
...
def CreateBoxSketchSet(SketchFeature):
    SketchFeature.addGeometry(
        Part.LineSegment(
            AuraCAD.Vector(-99.230339, 36.960674, 0),
            AuraCAD.Vector(69.432587, 36.960674, 0),
        )
    )
    SketchFeature.addGeometry(
        Part.LineSegment(
            AuraCAD.Vector(69.432587, 36.960674, 0),
            AuraCAD.Vector(69.432587, -53.196629, 0),
        )
    )
    SketchFeature.addGeometry(
        Part.LineSegment(
            AuraCAD.Vector(69.432587, -53.196629, 0),
            AuraCAD.Vector(-99.230339, -53.196629, 0),
        )
    )
    SketchFeature.addGeometry(
        Part.LineSegment(
...
# ---------------------------------------------------------------------------
# define the test cases to test the AuraCAD Sketcher module
# ---------------------------------------------------------------------------


class TestSketcherSolver(unittest.TestCase):
    def setUp(self):
        self.Doc = AuraCAD.newDocument("SketchSolverTest")
    ...
    def testIssue3245(self):
        self.Doc2 = AuraCAD.newDocument("Issue3245")
        ...
        AuraCAD.closeDocument("Issue3245")
    ...
    def testBlockConstraintEllipse(self):
        self.Doc3 = AuraCAD.newDocument("BlockConstraintTests")
        ...
        ActiveSketch.solve()
        self.assertTrue(status == 0)  # no redundants/conflicts/convergence issues
        AuraCAD.closeDocument(self.Doc3.Name)
    ...
    def testCircleLineTangentOriented(self):
        # Test to ratchet issue https://github.com/AuraCAD/AuraCAD/issues/29007
    ...
    def testRemovedExternalGeometryReference(self):
        if "BUILD_PARTDESIGN" in AuraCAD.__cmake__:
    ...
    def testSaveLoadWithExternalGeometryReference(self):
        if "BUILD_PARTDESIGN" in AuraCAD.__cmake__:
            ...
            filename = tempfile.gettempdir() + os.sep + self.Doc.Name + ".auracadStd"
            self.Doc.saveAs(filename)
            AuraCAD.closeDocument(self.Doc.Name)
            self.Doc = AuraCAD.openDocument(filename)
    ...
    def testConstructionToggleTNP(self):
        ...
        if "BUILD_PARTDESIGN" in AuraCAD.__cmake__:
            ...
            sketch.addGeometry(
                Part.LineSegment(AuraCAD.Vector(0, 0, 0), AuraCAD.Vector(-10, 10, 0)), True
            )
            sketch.addGeometry(
                Part.LineSegment(AuraCAD.Vector(-10, 10, 0), AuraCAD.Vector(-5, 20, 0)), True
            )
            sketch.addGeometry(
                Part.LineSegment(AuraCAD.Vector(-5, 20, 0), AuraCAD.Vector(-10, 25, 0)), True
            )
            sketch.addGeometry(
                Part.LineSegment(AuraCAD.Vector(-10, 25, 0), AuraCAD.Vector(0, 30, 0)), True
            )
    ...
    def tearDown(self):
        # closing doc
        AuraCAD.closeDocument("SketchSolverTest")
