...
"""
Test module for AuraCAD material cards and APIs
"""
...
class MaterialCreationTestCases(unittest.TestCase):
    """
    Test class for AuraCAD material creation and APIs
    """
    ...
    def tearDown(self):
        try:
            material = self.MaterialManager.getMaterialByPath("Example/Frakenstein.auracadMat", "User")
            os.remove(material.LibraryRoot + "/Example/Frakenstein.auracadMat")
            try:
    ...
    def testCreateMaterial(self):
        ...
        #
        self.MaterialManager.save("User", material, "Example/Frakenstein.auracadMat", overwrite=True)

        # Now the UUID is valid
        self.assertEqual(len(material.UUID), 36)
        self.assertEqual(material.UUID, uuid)
        self.assertIn(uuid, self.MaterialManager.Materials)
        self.assertIsNotNone(self.MaterialManager.getMaterialByPath("Example/Frakenstein.auracadMat", "User"))
