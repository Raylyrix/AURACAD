...
https://forum.AuraCAD.org/viewtopic.php?f=38&t=16714
'''
...
def get_material_preferred_directory(category=None):
    """
        ...
        3. system folder
    """
    mat_prefs = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material/Resources")
    ...
    if use_built_in_materials:
        if category == 'Fluid':
            preferred = join(
                AuraCAD.getResourceDir(), "Mod", "Material", "Resources", "Materials", "FluidMaterial"
            )

        elif category == 'Solid':
            preferred = join(
                AuraCAD.getResourceDir(), "Mod", "Material", "Resources", "Materials", "StandardMaterial"
            )

        else:
            preferred = join(
                AuraCAD.getResourceDir(), "Mod", "Material"
            )

    if use_mat_from_config_dir:
        user = join(
            AuraCAD.ConfigGet("UserAppData"), "Material"
        )
...
def get_material_resources(category='Solid'):
    ...
    if use_built_in_materials:
        ...
        resources[builtin_mat_dir] = ":/icons/AuraCAD.svg"

    if use_mat_from_modules:
        module_prefs = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material/Resources/Modules")
...
def get_material_libraries():
    ...
    if use_built_in_materials:
        builtin_mat_dir = join(
            AuraCAD.getResourceDir(), "Mod", "Material", "Resources", "Materials"
        )
        resources["System"] = (builtin_mat_dir, ":/icons/AuraCAD.svg")
...
def output_resources(resources):
    AuraCAD.Console.PrintMessage('Directories in which we will look for material cards:\n')
...
def add_cards_from_a_dir(materials, cards, icons, mat_dir, icon, template=False):
    ...
    import glob
    from importauracadMat import read
    dir_path_list = glob.glob(mat_dir + '/*' + ".auracadMat")
    mat_prefs = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material/Cards")
...
def output_material_param(mat_dict):
    ...
    if not mat_dict:
        AuraCAD.Console.PrintMessage('  empty matdict\n')
    else:
        for p in mat_dict:
            AuraCAD.Console.PrintMessage('   {} --> {}\n'.format(p, mat_dict[p]))
    AuraCAD.Console.PrintMessage('\n')
...
def get_material_template(withSpaces=False):
    # material properties
    # see the following resources in the AuraCAD wiki for more
    # information about the material specific properties:
    # https://www.AuraCAD.org/wiki/Material_data_model
    # https://www.AuraCAD.org/wiki/Material

    print("Call to get_material_template() successful")

    import yaml
    template_data = yaml.safe_load(
        open(join(AuraCAD.ConfigGet('AppHomePath'), 'Mod/Material/Templatematerial.yml'))
    )
    if withSpaces:
...
def create_mat_template_card(write_group_section=True):
    template_card = join(get_source_path(), 'src/Mod/Material/StandardMaterial/TEMPLATE.auracadMat')
    if not os.path.isfile(template_card):
        ...
        return
    rev = "{}.{}.{}".format(
        AuraCAD.ConfigGet("BuildVersionMajor"),
        AuraCAD.ConfigGet("BuildVersionMinor"),
        AuraCAD.ConfigGet("BuildRevision")
    )
    template_data = get_material_template()
    ...
    f.write('; https://www.AuraCAD.org/wiki/Material\n')
    f.write(': this template card was created by AuraCAD ' + rev + '\n\n')
    ...
    f.write('; find unit information in src/App/AuraCADInit.py')
    # write sections
    # write standard auracadMat section if write group section parameter is set to False
...
def get_source_path():
    ...
    source_dir = ''
    make_file = join(AuraCAD.ConfigGet('AppHomePath'), 'Makefile')
    f = open(make_file, 'r')
    lines = f.readlines()
    f.close()
    for line in lines:
...
def read_cards_from_path(cards_path):
    ...
    # to make sure all file lower and upper and mixed endings are found, use upper and .auracadMAT
    mat_files = [f for f in only_files if basename(splitext(f)[1]).upper() == '.auracadMAT']
    # print(mat_files)
    mat_cards = []
...
def write_cards_to_path(cards_path, cards_data, write_group_section=True, write_template=False):
    from importauracadMat import write
    from os.path import join
    for card_data in cards_data:
        if (card_data['CardName'] == 'TEMPLATE') and (write_template is False):
            continue
        else:
            card_path = join(cards_path, (card_data['CardName'] + '.auracadMat'))
            # print(card_path)
            if write_group_section is True:
...
def check_parm_unit(param):
    # check if this parameter is known to AuraCAD unit system
...
def check_value_unit(param, value):
    ...
    # AuraCAD.Console.PrintMessage('{} --> {}\n'.format(param, value))
    if hasattr(Units, param):
        # get unit and other information known by AuraCAD for this parameter
        ...
        try:
            ...
            try:
                ...
                AuraCAD.Console.PrintError(
                    '{} Unit {} is known by AuraCAD, but wrong for parameter {}.\n'
        ...
        AuraCAD.Console.PrintError(
            'Parameter {} is unknown to the AuraCAD unit system.\n'
            .format(param)
        )
    return False
...
def output_parm_unit_info(param):
    ...
    if hasattr(Units, param):
        AuraCAD.Console.PrintMessage(
            '\nParameter {} is known to AuraCAD unit system.'
            .format(param)
        )
...
def output_value_unit_info(param, value):
    ...
    from AuraCAD import Units
    some_text = "Parameter: {} --> value: {} -->".format(param, value)
    AuraCAD.Console.PrintMessage('{} unit information:'.format(some_text))
    if hasattr(Units, param):
...
def check_mat_units(mat):
    ...
    for param, value in mat.items():
        if param in known_quantities:
            ...
            pass
            # print('{} is not a known AuraCAD quantity.'.format(param))
    # print('')
    return units_ok
...
checkvalueunit('ThermalConductivity', '25.83eâˆ’3 W/m/K')
checkvalueunit('ThermalConductivity', '25.83e-3 W/m/K')
from AuraCAD import Units
Units.Quantity('25.83eâˆ’3 W/m/K')
