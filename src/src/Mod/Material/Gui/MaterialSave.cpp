    ...
    , _deleteAction(this)
{
    ...
    if (_material->getName().length() > 0) {
        ui->editFilename->setText(_material->getName() + QStringLiteral(".auracadMat"));
    }
    else {
        ui->editFilename->setText(QStringLiteral("NewMaterial.auracadMat"));
    }
...
void MaterialSave::onOk(bool checked)
{
    ...
    QString name = _filename.remove(QStringLiteral(".auracadMat"), Qt::CaseInsensitive);
