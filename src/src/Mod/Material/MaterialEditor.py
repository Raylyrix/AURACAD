...
__title__ = "AuraCAD material editor"
...
class MaterialEditor:
    ...
    def __init__(self, obj=None, prop=None, material=None, card_path="", category="Solid"):
        ...
        if self.directory is None:
            self.directory = AuraCAD.getResourceDir() + "Mod/Material"
        self.materials = {}
        self.cards = {}
        self.icons = {}
        self.initialIndex = -1
        ...
        self.widget = AuraCADGui.PySideUic.loadUi(":/ui/materials-editor.ui")
        # remove unused Help button
        self.widget.setWindowFlags(self.widget.windowFlags()
                                   & ~QtCore.Qt.WindowContextHelpButtonHint)

        # restore size and position
        param = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material")
        ...
        if self.prop and self.obj:
            matProperty = AuraCAD.ActiveDocument.getObject(self.obj).getPropertyByName(self.prop)
        elif self.material:
    ...
    def implementModel(self):
        ...
        p = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material")
        widget = self.widget
        treeView = widget.treeView
    ...
    def chooseMaterial(self, index):
        ...
        if os.path.isfile(self.card_path):
            ...
            AuraCAD.Console.PrintError("Material card not found: {}\n".format(self.card_path))

    def verifyMaterial(self):
        if self.edited:
            reply = QtGui.QMessageBox.question(self.widget, #AuraCADGui.getMainWindow(),
    ...
    def updatePreviews(self, mat=None):
        ...
        if difauracadol or highlightcol:
            ...
            if fd.open(QtCore.QIODevice.ReadOnly | QtCore.QIODevice.Text):
                ...
                fd.close()
                if difauracadol:
                    svg = svg.replace("#d3d7cf", self.getColorHash(difauracadol, val=255))
                    svg = svg.replace("#555753", self.getColorHash(difauracadol, val=125))
                if highlightcol:
    ...
    def openfile(self):
        ...
        if os.path.isfile(filetuple[0]):
            ...
            AuraCAD.Console.PrintError("Error: Invalid path to the material file\n")
            return
        ...
        # D:/AuraCAD-build/data/Mod\Material\FluidMaterial\Air.auracadMat
    ...
    def savefile(self):
        ...
        filetuple = QtGui.QFileDialog.getSaveFileName(
            QtGui.QApplication.activeWindow(),
            "Save AuraCAD material file",
            self.save_directory + "/" + name + ".auracadMat",
            "*.auracadMat"
        )
...
class MaterialsDelegate(QtGui.QStyledItemDelegate):
    ...
    def setEditorData(self, editor, index):
        ...
        if Type == "Color":
            ...
            if not hasattr(AuraCAD.Units, self.matproperty):
            ...
            if not self.Value:
                ...
                unit = getattr(AuraCAD.Units, self.matproperty)
                quantity = AuraCAD.Units.Quantity(1, unit)
...
def matProperWidget(parent=None, matproperty=None, Type="String", Value=None,
    ...
    # https://forum.AuraCAD.org/viewtopic.php?f=18&t=56912&start=20#p516811
    # set precision to 12
    ...
    if Type == "String":
        ...
        widget = ui.createWidget("Gui::PreauracadomboBox")
        ...
        widget = ui.createWidget("Gui::PreauracadolorButton")
        if Value:
...
def openEditor(obj=None, prop=None):
    ...
    # param = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material/Cards")
    # legacy = param.GetBool("LegacyEditor", True)
    # if legacy:
    #     editor = MaterialEditor(obj, prop)
    #     editor.exec_()
    # else:
    AuraCADGui.runCommand('Material_Edit',0)
...
doc = AuraCAD.open(
    AuraCAD.ConfigGet("AppHomePath") + "data/examples/FEMExample.auracadStd"
)
import MaterialEditor
