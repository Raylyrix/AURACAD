...
#define AREA_LOG AuraCAD_LOG
#define AREA_WARN AuraCAD_WARN
#define AREA_ERR AuraCAD_ERR
#define AREA_TRACE AuraCAD_TRACE
#define AREA_XYZ AuraCAD_XYZ
#define AREA_XY AREA_XY

#ifdef AuraCAD_DEBUG
# define AREA_DBG AuraCAD_WARN
#else
# define AREA_DBG(...) \
     do { \
     } while (0)
...
AuraCAD_LOG_LEVEL_INIT("Path.Area", true, true)
...
static std::vector<gp_Pnt> discretize(const TopoDS_Edge& edge, double deflection)
{
    ...
    // https://forum.AuraCAD.org/viewtopic.php?f=15&t=42628
    //
...
// See https://github.com/realthunder/AuraCAD/blob/LinkStable/src/Mod/Part/App/WireJoiner.h for the
// original implementation of the class and https://github.com/AuraCAD/AuraCAD/pull/12535 for the
// import conversation.
struct WireJoiner
{
    ...
    int findClosedWires(double tol = Precision::Confusion())
    {
        ...
        for (int iteration = 1; !edgesToVisit.empty(); ++iteration) {
            ...
            while (true) {
                ...
                if (!BRep_Tool::IsClosed(wire)) {
                    AuraCAD_WARN("failed to close some projection wire");
    ...
    if (_heights.empty()) {
        ...
        for (double z : _heights) {
            ...
            if (z - zMin < myParams.SectionTolerance) {
                ...
                if (_heights.empty() && AuraCAD_LOG_INSTANCE.isEnabled(AuraCAD_LOGLEVEL_LOG)) {
            ...
            for (auto it = myShapes.begin(); it != myShapes.end(); ++it) {
                ...
                for (TopExp_Explorer xp(s.shape.Moved(loc), TopAbs_SOLID); xp.More(); xp.Next()) {
                    ...
                    Part::FuzzyHelper::withBooleanFuzzy(.0, [&]() {
                        // Workaround for https://github.com/AuraCAD/AuraCAD/issues/17748
