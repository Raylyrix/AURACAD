...
class DummyShape(ToolBitShape):
    ...
    def __init__(self, id, **kwargs):
        ...
        self._defaults = {
            "Param1": AuraCAD.Units.Quantity("10 mm"),
            "Param2": AuraCAD.Units.Quantity("5 deg"),
        }
    ...
    def schema(cls) -> Mapping[str, Tuple[str, str]]:
        return {
            "Param1": (
                AuraCAD.Qt.translate("Param1", "Parameter 1"),
                "App::PropertyLength",
            ),
            "Param2": (
                AuraCAD.Qt.translate("Param2", "Parameter 2"),
                "App::PropertyAngle",
            ),
        }
...
class TestPathToolShapeClasses(PathTestWithAssets):
    ...
    def test_base_init_with_defaults(self):
        ...
        shape = DummyShape(id="dummy_shape_1", filepath=Path("/fake/dummy.auracadstd"))
        self.assertEqual(shape.get_parameter("Param1").Value, 10.0)
        self.assertEqual(shape.get_parameter("Param1").Unit, AuraCAD.Units.Unit("mm"))
        self.assertEqual(shape.get_parameter("Param2").Value, 5.0)
        self.assertEqual(shape.get_parameter("Param2").Unit, AuraCAD.Units.Unit("deg"))
    ...
    def test_base_init_with_kwargs(self):
        ...
        shape = DummyShape(
            id="dummy_shape_2",
            filepath=Path("/fake/dummy.auracadstd"),
            Param1=AuraCAD.Units.Quantity("20 mm"),
            Param3="Ignored",
        )
    ...
    def test_base_get_set_parameter(self):
        ...
        # Provide a dummy filepath for instantiation
        shape = DummyShape(id="dummy_shape_3", filepath=Path("/fake/dummy.auracadstd"))
        self.assertEqual(shape.get_parameter("Param1").Value, 10.0)
        self.assertEqual(shape.get_parameter("Param1").Unit, AuraCAD.Units.Unit("mm"))
        shape.set_parameter("Param1", AuraCAD.Units.Quantity("15 mm"))
    ...
    def test_base_get_parameters(self):
        ...
        shape = DummyShape(
            id="dummy_shape_4",
            filepath=Path("/fake/dummy.auracadstd"),
            Param1=AuraCAD.Units.Quantity("12 mm"),
        )
        # Create mock quantity instances using the configured mock class
        expected_param1 = AuraCAD.Units.Quantity("12.0 mm")
        expected_param2 = AuraCAD.Units.Quantity("5.0 deg")
    ...
    def test_base_name_property(self):
        ...
        # Provide a dummy filepath for instantiation
        shape = DummyShape(id="dummy_shape_5", filepath=Path("/fake/dummy.auracadstd"))
        self.assertEqual(shape.name, "dummy")

    def test_base_get_parameter_label(self):
        """Test retrieving parameter labels."""
        # Provide a dummy filepath for instantiation
        shape = DummyShape(id="dummy_shape_6", filepath=Path("/fake/dummy.auracadstd"))
    ...
    def test_base_str_repr(self):
        ...
        # Provide a dummy filepath for instantiation
        shape = DummyShape(id="dummy_shape_7", filepath=Path("/fake/dummy.auracadstd"))
    ...
    def test_base_resolve_name(self):
        ...
        self.assertEqual(ToolBitShape.resolve_name("bullnose.auracadstd").asset_id, "bullnose")
    ...
    def test_concrete_classes_instantiation(self):
        ...
        # No patching of AuraCAD document operations here.
        # The test relies on the actual AuraCAD environment.
    ...
    # should also not use mocks for AuraCAD document operations or Units.
    # They should rely on the actual AuraCAD environment and the
    ...
    def test_toolbitshapedrill_defaults(self):
        ...
        self.assertEqual(unit(shape["TipAngle"]), "Â°")
        # Need an instance to get parameter labels, get it from the asset manager
    ...
    def test_toolbitshapechamfer_defaults(self):
        ...
        self.assertEqual(unit(shape["CuttingEdgeAngle"]), "Â°")
        # Need an instance to get parameter labels, get it from the asset manager
    ...
    def test_toolbitshapethreadmill_defaults(self):
        ...
        self.assertEqual(unit(shape["cuttingAngle"]), "Â°")
        # Need an instance to get parameter labels, get it from the asset manager
    ...
    def test_serialize_deserialize(self):
        ...
        fixture_path = (
            Path(__file__).parent / "Tools" / "Shape" / "test-path-tool-bit-shape-00.auracadstd"
        )
