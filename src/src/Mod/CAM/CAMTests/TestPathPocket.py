...
class TestPathPocket(PathTestBase):
    ...
    def setUp(self):
        ...
        self.doc = AuraCAD.newDocument("TestPocket")
    ...
    def test_pocket_square_offset(self):
        ...
        inner = Part.makeBox(
            ...
            pocket_size,
            pocket_depth_amount,
            AuraCAD.Vector(pocket_offset_xy, pocket_offset_xy, pocket_bottom_z),
        )
        pocket_solid = outer.cut(inner)

        part_obj = AuraCAD.ActiveDocument.addObject("Part::Feature", "PocketPart")
    ...
    def test_pocket_pointy_triangle_offset(self):
        ...
        v1 = AuraCAD.Vector(center_x - triangle_base / 2.0, base_y, pocket_bottom_z)
        v2 = AuraCAD.Vector(center_x + triangle_base / 2.0, base_y, pocket_bottom_z)
        v3 = AuraCAD.Vector(center_x, apex_y, pocket_bottom_z)

        # Create triangle wire and extrude
        triangle_wire = Part.makePolygon([v1, v2, v3, v1])
        triangle_face = Part.Face(triangle_wire)
        inner = triangle_face.extrude(AuraCAD.Vector(0, 0, pocket_depth_amount))

        pocket_solid = outer.cut(inner)

        part_obj = AuraCAD.ActiveDocument.addObject("Part::Feature", "TrianglePart")
        part_obj.Shape = pocket_solid
