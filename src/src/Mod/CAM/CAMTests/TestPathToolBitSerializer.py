...
class _BaseToolBitSerializerTestCase(PathTestWithAssets):
    ...
    def setUp(self):
        ...
        self.test_tool_bit.set_diameter(AuraCAD.Units.Quantity("4.12 mm"))
        self.test_tool_bit.set_length(AuraCAD.Units.Quantity("15.0 mm"))
...
class TestCamoticsToolBitSerializer(_BaseToolBitSerializerTestCase):
    ...
    def test_deserialize(self):
        ...
        self.assertEqual(
            deserialized_bit.get_diameter(), AuraCAD.Units.Quantity(4.12, AuraCAD.Units.Length)
        )
        self.assertEqual(
            deserialized_bit.get_length(), AuraCAD.Units.Quantity(15.0, AuraCAD.Units.Length)
        )
        self.assertEqual(deserialized_bit.get_shape_name(), "Endmill")


class TestauracadTBSerializer(_BaseToolBitSerializerTestCase):
    serializer_class = auracadTBSerializer

    def test_serialize(self):
        super().test_serialize()
        serialized_data = self.serializer_class.serialize(self.test_tool_bit)
        # auracadTB specific assertions (JSON format)
        data = json.loads(serialized_data.decode("utf-8"))
        self.assertEqual(data.get("name"), "Test Tool")
        self.assertEqual(data.get("shape"), "endmill.auracadstd")
        self.assertEqual(
            AuraCAD.Units.Quantity(data.get("parameter", {}).get("Diameter")),
            AuraCAD.Units.Quantity(4.12, AuraCAD.Units.Length),
        )
        self.assertEqual(
            AuraCAD.Units.Quantity(data.get("parameter", {}).get("Length")),
            AuraCAD.Units.Quantity(15.0, AuraCAD.Units.Length),
        )

    def test_extract_dependencies(self):
        """Test dependency extraction for auracadTB."""
        auracadtb_data = (
            b'{"name": "Test Tool", "pocket": null, "shape": "endmill", '
            b'"parameter": {"Diameter": "4.12 mm", "Length": "15.0 mm"}, "attribute": {}}'
        )
        dependencies = self.serializer_class.extract_dependencies(auracadtb_data)
        self.assertIsInstance(dependencies, list)
        self.assertEqual(len(dependencies), 1)
        self.assertEqual(dependencies[0], AssetUri.build("toolbitshape", "endmill"))

    def test_deserialize(self):
        # Create a known serialized data string based on the auracadTB format
        auracadtb_data = (
            b'{"name": "Test Tool", "pocket": null, "shape": "endmill", '
            b'"parameter": {"Diameter": "4.12 mm", "Length": "15.0 mm"}, "attribute": {}}'
        )
        ...
        deserialized_bit = cast(
            ToolBitEndmill,
            self.serializer_class.deserialize(auracadtb_data, id="test_id", dependencies=dependencies),
        )
