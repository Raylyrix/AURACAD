...
class TestPathUtil(PathTestBase):
    def setUp(self):
        self.doc = AuraCAD.newDocument("TestPathUtils")

    def tearDown(self):
        AuraCAD.closeDocument("TestPathUtils")
    ...
    def test02(self):
        ...
        box.Width = 10
        box.Height = 1
        box.Placement = AuraCAD.Placement(
            AuraCAD.Vector(-5, -5, 0), AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), 0)
        )
        cyl = self.doc.addObject("Part::Cylinder", "Cylinder")
        cyl.Radius = 1
        cyl.Height = 10
        box.Placement = AuraCAD.Placement(
            AuraCAD.Vector(0, 0, -5), AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), 0)
        )
