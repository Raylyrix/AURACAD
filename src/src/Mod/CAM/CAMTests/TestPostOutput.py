...
class TestFileNameGenerator(unittest.TestCase):
    ...
    Assume:
    active document: self.assertTrue(filename, f"{home}/testdoc.auracadstd
    user macro: ~/.local/share/AuraCAD/Macro
    Job:  MainJob
    Operations:
        OutsideProfile
        DrillAllHoles
    ...
    def setUpClass(cls):
        ...
        cls.doc = AuraCAD.newDocument("TestFileNaming")
        cls.testfilename = cls.doc.Name
        cls.testfilepath = os.getcwd()
        cls.macro = AuraCAD.getUserMacroDir()
    ...
    def test000(self):
        ...
        AuraCAD.setActiveDocument(self.doc.Label)
        teststring = ""
...
class TestExport2Integration(unittest.TestCase):
    ...
    def setUpClass(cls):
        AuraCAD.ConfigSet("SuppressRecomputeRequiredDialog", "True")
        cls.doc = AuraCAD.newDocument("export2_test")
    ...
    def test110_tool_length_offset_enabled(self):
        ...
        controller postable's path.  M6 belongs in the tool_controller postable â€” not
        in operation paths â€” so the test builds a postlist directly rather than using
    ...
    def test128_duplicate_parameters_suppressed(self):
        ...
        Expected:
            ...
            AFTER:  G0 X10 Y10 Z5 F60000
                    G1 X20              (Y, Z, F suppressed â€” unchanged)
        """
        machine = self._create_machine(
    ...
    def test129_duplicate_commands_suppressed(self):
        ...
        Expected:
            ...
            AFTER:  G1 X10 Y10
                    X20 Y10      (G1 suppressed â€” same as previous)
                    X20 Y20
        """
        with self._modify_operation_path(
            [
    ...
    def test141_rotary_blocks_insertion(self):
        ...
        Expected: 2 rotary groups â†’ 2 pre-rotary + 2 post-rotary blocks.
        """
        config = self._get_full_machine_config()
        machine = Machine.from_dict(config)
    ...
    def test154_schema_defaults_applied_for_missing_properties(self):
        ...
        Real .auracadm files only store properties the user explicitly changed.
        ...
        """
        config = self._get_full_machine_config()
        # Simulate a sparse .auracadm: only file_extension is stored
