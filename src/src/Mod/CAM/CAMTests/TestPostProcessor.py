...
class TestResolvingPostProcessorName(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        AuraCAD.ConfigSet("SuppressRecomputeRequiredDialog", "True")
        # Create a new document instead of opening external file
        cls.doc = AuraCAD.newDocument("boxtest")
    ...
    def tearDownClass(cls):
        AuraCAD.closeDocument(cls.doc.Name)
        AuraCAD.ConfigSet("SuppressRecomputeRequiredDialog", "")

    def setUp(self):
        pref = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/CAM")
...
class TestHeaderBuilder(unittest.TestCase):
    ...
    def test020_add_methods(self):
        ...
        builder.add_cam_file("test.auracadstd")
        builder.add_project_file("/path/to/project.auracadStd")
        ...
        self.assertEqual(builder._cam_file, "test.auracadstd")
        self.assertEqual(builder._project_file, "/path/to/project.auracadStd")
    ...
    def test040_path_property_complete(self):
        ...
        builder.add_exporter_info("AuraCAD")
        builder.add_machine_info("CNC Router")
        builder.add_post_processor("linuxcnc")
        builder.add_cam_file("project.auracadstd")
        builder.add_project_file("/home/user/myproject.auracadStd")
        builder.add_output_units("Metric - mm")
        ...
        expected_commands = [
            ...
            "(Cam File: project.auracadstd)",
            "(Project File: /home/user/myproject.auracadStd)",
    ...
    def test050_path_property_partial(self):
        ...
        expected_commands = ["(Exported by AuraCAD)", "(T5=Drill)", "(Note: Partial test)"]
        ...
        # converted
        expected_gcode = "(Exported by AuraCAD)\n(T5=Drill)\n(Note: Partial test)\n"
        gcode = path.toGCode()
...
class TestConfigurationBundle(unittest.TestCase):
    ...
    def _make_postprocessor(self, machine_props=None, job_overrides=None, schema=None):
        ...
        class BundleTestPP(PostProcessor):
            def __init__(self):
                # Skip super().__init__() â€” we only need the bundle methods
                self.values = {}
                self._machine = None
                self._job = None
    ...
    # ------------------------------------------------------------------
    # build_configuration_bundle â€” pure function tests
    # ------------------------------------------------------------------
    ...
    def test140_build_bundle_explicit_overrides_win_over_job(self):
        ...
        # Explicit override wins â€” job overrides are never read
    ...
    # ------------------------------------------------------------------
    # apply_configuration_bundle â€” side-effecting tests
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # _read_job_overrides â€” parsing tests
    # ------------------------------------------------------------------
