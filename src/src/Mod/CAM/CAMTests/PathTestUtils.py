...
from Path.Tool.library.serializers import auracadTLSerializer
from Path.Tool.toolbit.serializers import auracadTBSerializer
...
class PathTestBase(unittest.TestCase):
    ...
    def assertEqualLocale(self, s1, s2):
        ...
        q1 = AuraCAD.Units.Quantity(s1)
        q2 = AuraCAD.Units.Quantity(s2)
...
class PathTestWithAssets(PathTestBase):
    ...
    def setUp(self):
        ...
        self.assets.register_asset(Library, auracadTLSerializer)
        self.assets.register_asset(ToolBit, auracadTBSerializer)
        ...
        for path in pathlib.Path(self.__tool_dir / "Bit").glob("*.auracadtb"):
            self.assets.add_file("toolbit", path)
        for path in pathlib.Path(self.__tool_dir / "Shape").glob("*.auracadstd"):
