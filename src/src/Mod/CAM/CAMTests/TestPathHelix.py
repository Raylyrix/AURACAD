...
class TestPathHelix(PathTestUtils.PathTestBase):
    ...
    def setUp(self):
        self.clone = None
        self.doc = AuraCAD.open(AuraCAD.getHomePath() + "Mod/CAM/CAMTests/test_holes00.auracadstd")
    ...
    def test02(self):
        ...
        for deg in range(self.RotateBy, 360, self.RotateBy):
            model.Placement.Rotation = AuraCAD.Rotation(deg, 0, 0)
            for base in op.Base:
                model = base[0]
                for sub in base[1]:
                    pos = proxy.holePosition(model, sub)
    ...
    def test03(self):
        ...
        for deg in range(self.RotateBy, 360, self.RotateBy):
            self.tearDown()
            self.doc = AuraCAD.open(AuraCAD.getHomePath() + "Mod/CAM/CAMTests/test_holes00.auracadstd")
            self.doc.Body.Placement.Rotation = AuraCAD.Rotation(deg, 0, 0)
    ...
    def test04(self):
        ...
        for deg in range(self.RotateBy, 360, self.RotateBy):
            ...
            self.clone.Placement.Rotation = AuraCAD.Rotation(deg, 0, 0)
    ...
    def testRecomputeHelixFromV021(self):
        """Verify that we can still open and recompute a Helix created with older AuraCAD"""
        self.tearDown()
        self.doc = AuraCAD.openDocument(str(FIXTURE_PATH / "OpHelix_v0-21.auracadStd"))
