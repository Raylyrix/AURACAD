...
class TestBuildPostList(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.doc = AuraCAD.newDocument("test_filenaming")
    ...
    def test095_operation_postable_tool_controller_is_postable(self):
        ...
        not the raw AuraCAD document object.

        When legacy post scripts or other code accesses item.ToolController on an
        operation Postable, they must receive a Postable wrapperâ€”not a live reference
        ...
        Then: The returned value is a PostList.Postable instance (not a AuraCAD document object)
    ...
    def test096_operation_postable_tool_controller_is_isolated(self):
        ...
        a copy, so mutations do not propagate back to the real AuraCAD TC document object.
        ...
        Then: The original AuraCAD TC document object's Path is unchanged

        Example:
            original_tc_path = op_item.source.ToolController.Path
            op_item.ToolController.path = Path.Path([])    # mutate the copy
            op_item.source.ToolController.Path == original_tc_path  # True â€” unaffected
        """
        self.job.SplitOutput = False
        self.job.OrderOutputBy = "Operation"
        postlist = self.pp._buildPostList()
    ...
    def test097_buildpostlist_does_not_touch_operations(self):
        ...
        operation state without side-effecting the AuraCAD document.  When operations are
        ...
        self.assertGreater(
            len(clean_before),
            0,
            "No operations were clean after recompute â€” test cannot validate the invariant",
        )
        ...
        for op in ops:
            if op.Name in clean_before:
                self.assertNotIn(
                    "Touched",
                    op.State,
                    f"Operation {op.Label!r} was Touched by _buildPostList() â€” "
...
class TestJobPropertyOverrides(unittest.TestCase):
    ...
    def setUpClass(cls):
        AuraCAD.ConfigSet("SuppressRecomputeRequiredDialog", "True")
        cls.doc = AuraCAD.newDocument("job_override_test")
