...
class VbitTool:
    ...
    def __init__(self, dia, angle, tipDia):
        self.Diameter = AuraCAD.Units.Quantity(dia, AuraCAD.Units.Length)
        self.CuttingEdgeAngle = AuraCAD.Units.Quantity(angle, AuraCAD.Units.Angle)
        self.TipDiameter = AuraCAD.Units.Quantity(tipDia, AuraCAD.Units.Length)
...
class TestPathVcarve(PathTestWithAssets):
    ...
    def testFinishingPass(self):
        ...
        part1 = AuraCAD.ActiveDocument.addObject("Part::Feature", "TestShape")
        part2 = AuraCAD.ActiveDocument.addObject("Part::Feature", "TestShape")
    ...
    def test17(self):
        ...
        positionHistory = [
            AuraCAD.Base.Vector(0, 0, 0),  # previous position
            AuraCAD.Base.Vector(0, 1, 5),  # current position
        ]

        newPosition = AuraCAD.Base.Vector(0, 1.4, 3)
        assert PathVcarve.canSkipRepositioning(positionHistory, newPosition, 0.01)

        newPosition = AuraCAD.Base.Vector(0, 1.7, 3)
    ...
    def test18(self):
        ...
        newPosition = AuraCAD.Base.Vector(0, 0, 0)
        ...
        newPosition = AuraCAD.Base.Vector(0, 0.1, 0)
    ...
    def test19(self):
        ...
        newEdge = Part.Edge(
            Part.LineSegment(AuraCAD.Base.Vector(1, 2, 3), AuraCAD.Base.Vector(0, 1, 5))
        )
        ...
        newEdge = Part.Edge(
            Part.LineSegment(AuraCAD.Base.Vector(1, 2, 3), AuraCAD.Base.Vector(0, 0, 0))
        )
