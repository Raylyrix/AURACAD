...
class TestPathStock(PathTestBase):
    def setUp(self):
        self.doc = AuraCAD.newDocument("TestPathStock")
        ...
        model = AuraCAD.ActiveDocument.addObject("App::DocumentObjectGroup", "Model")
        model.addObject(self.base)
        self.job.Model = model
        self.job.Proxy = FakeJobProxy()

    def tearDown(self):
        AuraCAD.closeDocument("TestPathStock")
    ...
    def test00(self):
        ...
        extent = AuraCAD.Vector(17, 13, 77)
        ...
        placement = AuraCAD.Placement(AuraCAD.Vector(-3, 88, 4), AuraCAD.Vector(0, 0, 1), 180)
    ...
    def test01(self):
        ...
        placement = AuraCAD.Placement(AuraCAD.Vector(3, 8, -4), AuraCAD.Vector(0, 0, 1), -90)
    ...
    def test10(self):
        ...
        self.assertEqual(orig.Width, stock.Width)
        self.assertEqual(orig.Height, stock.Height)
        self.assertPlacement(AuraCAD.Placement(), stock.Placement)

    def test11(self):
        """Verify FromTemplate cylinder creation."""
        radius = 7
        height = 12
        placement = AuraCAD.Placement(AuraCAD.Vector(99, 88, 77), AuraCAD.Vector(1, 1, 1), 123)
        ...
        self.assertPlacement(
            AuraCAD.Placement(AuraCAD.Vector(50, 100, 0), AuraCAD.Rotation()),
            stock.Placement,
        )

    def test12(self):
        """Verify FromTemplate from Base creation."""
        neg = AuraCAD.Vector(1, 2, 3)
        pos = AuraCAD.Vector(9, 8, 7)
