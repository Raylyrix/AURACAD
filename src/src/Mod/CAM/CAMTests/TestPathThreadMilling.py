...
class TestObject(object):
    def __init__(self, orientation, direction, zTop, zBottom):
        ...
        self.StartDepth = AuraCAD.Units.Quantity(zTop, AuraCAD.Units.Length)
        self.FinalDepth = AuraCAD.Units.Quantity(zBottom, AuraCAD.Units.Length)
...
class TestPathThreadMilling(PathTestBase):
    ...
    def test02(self):
        """Verify crest compensation uses cutting angle, not hardcoded 60Â°.

        Bug: threadRadii() hardcodes SQRT_3_DIVIDED_BY_2 (the crest-to-tip
        factor for a 60Â° profile).  For a non-60Â° profile the factor should
        be 1 / (2 * tan(cuttingAngle / 2)).

        For 60Â° ISO:      factor = 1/(2*tan(30Â°)) = âˆš3/2 â‰ˆ 0.86603
        For 55Â° Whitworth: factor = 1/(2*tan(27.5Â°))    â‰ˆ 0.96050

        With major=20, minor=18, toolDia=2, crest=0.1 (internal):
          H = 1.6,  outerTip = 10.2
          60Â° -> toolTip = 10.2 - 0.1*0.86603 = 10.11340 -> max_r = 9.11340
          55Â° -> toolTip = 10.2 - 0.1*0.96050 = 10.10395 -> max_r = 9.10395
        ...
        # Expected max radius for a 55Â° profile
    ...
    def test03(self):
        """Verify crest compensation for external thread with non-60Â° angle.

        Same bug as test02 but for external threads.

        With major=20, minor=18, toolDia=2, crest=0.1 (external):
          H = 1.6,  innerTip = 8.6
          60Â° -> toolTip = 8.6 + 0.1*0.86603 = 8.68660 -> min_r = 9.68660
          55Â° -> toolTip = 8.6 + 0.1*0.96050 = 8.69605 -> min_r = 9.69605
