...
class TestMachineDataclass(PathTestUtils.PathTestBase):
    ...
    def test_default_initialization(self):
        ...
        machine.add_linear_axis("X", AuraCAD.Vector(1, 0, 0))
        machine.add_linear_axis("Y", AuraCAD.Vector(0, 1, 0))
        self.assertEqual(machine.machine_type, "custom")  # Still missing Z axis

        machine.add_linear_axis("Z", AuraCAD.Vector(0, 0, 1))
        self.assertEqual(machine.machine_type, "xyz")  # Now has XYZ axes

        # Add rotary axes and verify machine type updates
        machine.add_rotary_axis("A", AuraCAD.Vector(1, 0, 0), -120, 120)
        self.assertEqual(machine.machine_type, "xyza")

        machine.add_rotary_axis("C", AuraCAD.Vector(0, 0, 1), -360, 360)
        ...
        self.assertIsNotNone(machine.AuraCAD_version)
...
class TestMachineFactory(PathTestUtils.PathTestBase):
    ...
    def test_save_and_load_configuration(self):
        ...
        # Save configuration
        filepath = MachineFactory.save_configuration(machine, "test_machine.auracadm")
        self.assertTrue(filepath.exists())

        # Load configuration
        loaded_machine = MachineFactory.load_configuration("test_machine.auracadm")
    ...
    def test_save_configuration_auto_filename(self):
        ...
        self.assertTrue(filepath.exists())
        self.assertEqual(filepath.name, "My_Test_Machine.auracadm")

    def test_load_nonexistent_file(self):
        """Test loading a file that doesn't exist"""
        with self.assertRaises(FileNotFoundError):
            MachineFactory.load_configuration("nonexistent.auracadm")
    ...
    def test_list_configuration_files(self):
        ...
        MachineFactory.save_configuration(machine1, "machine1.auracadm")
        MachineFactory.save_configuration(machine2, "machine2.auracadm")
    ...
    def test_list_configurations(self):
        ...
        MachineFactory.save_configuration(machine, "test.auracadm")
    ...
    def test_delete_configuration(self):
        ...
        filepath = MachineFactory.save_configuration(machine, "delete_me.auracadm")

        self.assertTrue(filepath.exists())

        # Delete the configuration
        result = MachineFactory.delete_configuration("delete_me.auracadm")
    ...
    def test_serialization_roundtrip(self):
        ...
        # Save and load
        filepath = MachineFactory.save_configuration(machine, "complex.auracadm")
    ...
    def test_register_addon_machine_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            MachineFactory.register_addon_machine_dir(p)  # second call â€” no duplicate
            self.assertEqual(len(MachineFactory._addon_machine_dirs), before + 1)
    ...
    def test_list_addon_templates(self):
        ...
        Given: a registered addon directory containing one .auracadm file with name "Addon Machine"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            p = pathlib.Path(tmpdir)
            auracadm = {"machine": {"name": "Addon Machine", "axes": {}, "spindles": []}}
            (p / "addon_machine.auracadm").write_text(json.dumps(auracadm))
            MachineFactory.register_addon_machine_dir(p)
    ...
    def test_get_machine_from_addon(self):
        ...
        Given: a registered addon directory containing a valid .auracadm file
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (p / "addon_test.auracadm").write_text(json.dumps(machine.to_dict()))
            MachineFactory.register_addon_machine_dir(p)
    ...
    def test_list_configurations_includes_addon(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            p = pathlib.Path(tmpdir)
            auracadm = {"machine": {"name": "Community Mill", "axes": {}, "spindles": []}}
            (p / "community_mill.auracadm").write_text(json.dumps(auracadm))
            MachineFactory.register_addon_machine_dir(p)
            configs = MachineFactory.list_configurations()
