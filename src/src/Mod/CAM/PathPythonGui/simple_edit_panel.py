...
class SimpleEditPanel:
    """A simple property edit panel for a AuraCAD object
    ...
    def __init__(self, obj, view):
        ...
        AuraCAD.ActiveDocument.openTransaction(self._transaction_name)
        self.form = AuraCADGui.PySideUic.loadUi(self._ui_file)
        self._auracad = {}
        self.setupUi()

    def getFields(self):
        for prop_name, (get_field, set_field) in self._auracad.items():
            val = get_field()
    ...
    def connectWidget(self, prop_name, widget, custom_lbls=None):
        ...
        if prop_type == "App::PropertyEnumeration" and widget_type == "QComboBox":
            ...
            self._auracad[prop_name] = _getter, _setter
        elif prop_type == "App::PropertyBool" and widget_type == "QCheckBox":
            self._auracad[prop_name] = widget.isChecked, widget.setChecked
        elif prop_type in PROP_TYPE_NUMERIC and widget_type == "QDoubleSpinBox":
            self._auracad[prop_name] = widget.value, widget.setValue
        elif prop_type in PROP_TYPE_QTYES and widget_type == "QLineEdit":
            self._auracad[prop_name] = widget.text, lambda v: widget.setText(v.UserString)
        else:
            raise ValueError(
