...
class Perauracadounter
{
public:
    Perauracadounter(string p_name)
    {
        name = p_name;
        count = 0;
        running = false;
    ...
    inline void Start()
    {
        ...
        start_ticks = clock();
        if (running) {
            cerr << "Perauracadounter already running:" << name << endl;
        }
        running = true;
#endif
    }
...
#ifdef DEV_MODE
        if (!running) {
            cerr << "Perauracadounter not running:" << name << endl;
        }
...
Perauracadounter Perf_ProcessPolyNode("ProcessPolyNode");
Perauracadounter Perf_CalcCutAreaCirc("CalcCutArea");
Perauracadounter Perf_NextEngagePoint("NextEngagePoint");
Perauracadounter Perf_PointIterations("PointIterations");
Perauracadounter Perf_ExpandCleared("ExpandCleared");
Perauracadounter Perf_DistanceToBoundary("DistanceToBoundary");
Perauracadounter Perf_AppendToolPath("AppendToolPath");
Perauracadounter Perf_IsAllowedToCutTrough("IsAllowedToCutTrough");
Perauracadounter Perf_IsClearPath("IsClearPath");
...
)
{
    ...
    double ofauracadlearance = 2 * stepOverScaled;
    if (ofauracadlearance > directDistance / 2) {
        ofauracadlearance = directDistance / 2;
        clearance = 0;
    }
    ...
    while (!queue.empty()) {
        ...
        if (pointPair.first == startPoint) {
            checkPath.push_back(IntPoint(
                pointPair.first.X + ofauracadlearance * direction.X,
                pointPair.first.Y + ofauracadlearance * direction.Y
            ));
        }
        else {
        ...
        if (pointPair.second == endPoint) {
            checkPath.push_back(IntPoint(
                pointPair.second.X - ofauracadlearance * direction.X,
                pointPair.second.Y - ofauracadlearance * direction.Y
            ));
        }
        else {
