...
void CmdPathArea::activated(int iMsg)
{
    ...
    for (const Gui::SelectionObject& selObj :
        ...
        if (subnames.empty()) {
            ...
            sources << "AuraCAD.activeDocument()." << pcObj->getNameInDocument() << ",";
            continue;
        }
        for (const std::string& name : subnames) {
            ...
            cmd << "AuraCAD.activeDocument().addObject('Part::Feature','" << sub_fname
                << "').Shape = PathCommands.findShape(AuraCAD.activeDocument()."
            ...
            sources << "AuraCAD.activeDocument()." << sub_fname << ",";
        }
    }
    if (addView && !areaName.empty()) {
        ...
        doCommand(
            Doc,
            "AuraCAD.activeDocument().addObject('Path::FeatureAreaView','%s')",
            FeatName.c_str()
        );
        doCommand(
            Doc,
            "AuraCAD.activeDocument().%s.Source = AuraCAD.activeDocument().%s",
            FeatName.c_str(),
            areaName.c_str()
        );
        commitCommand();
    ...
    doCommand(Doc, "AuraCAD.activeDocument().addObject('Path::FeatureArea','%s')", FeatName.c_str());
    doCommand(
        Doc,
        "AuraCAD.activeDocument().%s.Sources = [ %s ]",
        FeatName.c_str(),
        sources.str().c_str()
    );
    commitCommand();
...
void CmdPathAreaWorkplane::activated(int iMsg)
{
    ...
    doCommand(
        Doc,
        "AuraCAD.activeDocument().%s.WorkPlane = PathCommands.findShape("
        "AuraCAD.activeDocument().%s)",
        areaName.c_str(),
        planeSubname.c_str()
    );
    doCommand(Doc, "AuraCAD.activeDocument().%s.ViewObject.Visibility = True", areaName.c_str());
    commitCommand();
    updateActive();
}
    ...
    if (!Sel.empty()) {
        ...
        for (std::vector<Gui::SelectionSingleton::SelObj>::const_iterator it = Sel.begin();
            ...
            if ((*it).pObject->isDerivedFrom<Path::Feature>()) {
                ...
                cmd << "AuraCAD.activeDocument()." << pcPathObject->getNameInDocument() << ",";
            }
            else {
                Base::Console().error(
        ...
        doCommand(
            Doc,
            "AuraCAD.activeDocument().addObject('Path::FeatureCompound','%s')",
            FeatName.c_str()
        );
        doCommand(Doc, "AuraCAD.activeDocument().%s.Group = %s", FeatName.c_str(), cmd.str().c_str());
        commitCommand();
        updateActive();
    }
    else {
...
void CmdPathShape::activated(int iMsg)
{
    ...
    doCommand(Doc, "AuraCAD.activeDocument().addObject('Path::FeatureShape','%s')", FeatName.c_str());
    doCommand(
        Doc,
        "AuraCAD.activeDocument().%s.Sources = [ %s ]",
        FeatName.c_str(),
