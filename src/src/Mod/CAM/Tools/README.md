...
an accurate thumbnail. On the other hand, storing each tool as a `*.auracadstd`
...
practically impossible for `*.auracadstd` files.
...
with AuraCAD should be installed analogous to TechDraw's templates.
...
1.  Create a new AuraCAD document.
...
1.  Save the document as a new file in the Shape directory.
    * Before saving the document, make sure you have _Save Thumbnail_
        selected, and _Add program logo_ deselected in AuraCAD's
        preferences.
