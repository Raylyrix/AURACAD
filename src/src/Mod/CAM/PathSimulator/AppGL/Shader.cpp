...
const char* FragShaderSSAOLighting = R"(
    ...
    void main()
    {
        ...
        vec4 DifauracadolorA = texture2D(ColorTex, texCoord);
        vec3 difauracadolor = DifauracadolorA.rgb;
        float aoVal = ssaoActive ? texture2D( AoTex, texCoord).r : 1.0;

        vec3 col = ambAndDiffuse(pos, norm, difauracadolor, aoVal);
        col = pow(col, vec3(1.0/2.2));

        gl_FragColor = vec4( col, DifauracadolorA.a );
    }
)";
