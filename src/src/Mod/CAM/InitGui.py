...
if AuraCAD.GuiUp:
    import AuraCADGui
    from AuraCADGui import Workbench
else:
...
AuraCAD.__unit_test__ += ["TestCAMGui"]
...
class CAMWorkbench(Workbench):
    ...
    def __init__(self):
        self.__class__.Icon = AuraCAD.getResourceDir() + "Mod/CAM/Resources/icons/CAMWorkbench.svg"
        ...
        if Path.Preferences.advancedOCLFeaturesEnabled():
            ...
            try:
                ...
                if not Path.Preferences.suppressOpenCamLibWarning():
                    AuraCAD.Console.PrintError("OpenCamLib is not working!\n")
...
AuraCADGui.addWorkbench(CAMWorkbench())
