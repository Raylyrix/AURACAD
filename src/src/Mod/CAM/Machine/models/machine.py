...
refAxis = RefAxes(
    AuraCAD.Vector(1, 0, 0),  # x: linear direction
    AuraCAD.Vector(0, 1, 0),  # y: linear direction
    AuraCAD.Vector(0, 0, 1),  # z: linear direction
)

RefRotAxes = namedtuple("RefRotAxes", ["a", "b", "c"])
refRotAxis = RefRotAxes(
    AuraCAD.Vector(1, 0, 0),  # a: rotational direction
    AuraCAD.Vector(0, 1, 0),  # b: rotational direction
    AuraCAD.Vector(0, 0, 1),  # c: rotational direction
)
...
class LinearAxis:
    ...
    name: str
    direction_vector: AuraCAD.Vector
    min_limit: float = 0
    ...
    def __post_init__(self):
        ...
        if self.direction_vector is None or self.direction_vector.Length < 1e-6:
            ...
            self.direction_vector = AuraCAD.Vector(1, 0, 0)
        else:
...
class RotaryAxis:
    ...
    name: str
    rotation_vector: AuraCAD.Vector
    min_limit: float = -360
    max_limit: float = 360
    ...
    def __post_init__(self):
        ...
        if self.rotation_vector is None or self.rotation_vector.Length < 1e-6:
            ...
            self.rotation_vector = AuraCAD.Vector(0, 0, 1)
        else:
...
class Machine:
    ...
    version: int = 1
    AuraCAD_version: str = field(init=False)
    ...
    def __post_init__(self):
        ...
        self.AuraCAD_version = ".".join(AuraCAD.Version()[0:3])
    ...
    def create_AC_table_config(cls, a_limits=(-120, 120), c_limits=(-360, 360)):
        ...
        config.add_linear_axis("X", AuraCAD.Vector(1, 0, 0))
        config.add_linear_axis("Y", AuraCAD.Vector(0, 1, 0))
        config.add_linear_axis("Z", AuraCAD.Vector(0, 0, 1))
        config.linear_axes["Z"].role = AxisRole.HEAD_LINEAR
        config.add_rotary_axis("C", AuraCAD.Vector(0, 0, 1), c_limits[0], c_limits[1])
        config.rotary_axes["C"].role = AxisRole.TABLE_ROTARY
        config.rotary_axes["C"].sequence = 0
        config.add_rotary_axis("A", AuraCAD.Vector(1, 0, 0), a_limits[0], a_limits[1])
    ...
    def create_BC_head_config(cls, b_limits=(-120, 120), c_limits=(-360, 360)):
        ...
        config.add_linear_axis("Z", AuraCAD.Vector(0, 0, 1))
        config.linear_axes["Z"].role = AxisRole.HEAD_LINEAR
        config.add_rotary_axis("B", AuraCAD.Vector(0, 1, 0), b_limits[0], b_limits[1])
    ...
    def to_dict(self):
        ...
        data = {
            "AuraCAD_version": self.AuraCAD_version,
            "machine": {
                "name": self.name,
    ...
    def _initialize_3axis_config(self) -> None:
        ...
        self.linear_axes = {
            "X": LinearAxis("X", AuraCAD.Vector(1, 0, 0), role=AxisRole.TABLE_LINEAR),
            "Y": LinearAxis("Y", AuraCAD.Vector(0, 1, 0), role=AxisRole.TABLE_LINEAR),
            "Z": LinearAxis("Z", AuraCAD.Vector(0, 0, 1), role=AxisRole.HEAD_LINEAR),
        }
        self.rotary_axes = {}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Machine":
        ...
        for axis_name, axis_data in axes.items():
            ...
            if isinstance(joint, dict) and "origin" in joint and "axis" in joint:
                ...
                vec0 = (
                    AuraCAD.Vector(joint[0][0], joint[0][1], joint[0][2])
                    if len(joint[0]) >= 3
                    else AuraCAD.Vector(0, 0, 0)
                )
                vec1 = (
                    AuraCAD.Vector(joint[1][0], joint[1][1], joint[1][2])
                    if len(joint[1]) >= 3
                    else AuraCAD.Vector(0, 0, 0)
                )
            ...
            if axis_type == "linear":
                ...
                direction_vec = AuraCAD.Vector(axis_vector[0], axis_vector[1], axis_vector[2])
                ...
                rotation_vec = AuraCAD.Vector(axis_vector[0], axis_vector[1], axis_vector[2])
...
class MachineFactory:
    ...
    def register_addon_machine_dir(cls, path, namespace=None) -> None:
        ...
        The directory should contain .auracadm files.  Machines are available
        for direct use and as templates â€” without copying to the user
        asset directory.  Duplicate registrations are silently ignored.

        Args:
            path: Directory path (str or pathlib.Path) containing .auracadm files.
    ...
    def get_config_directory(cls):
        ...
        if cls._config_dir is None:
            # Use AuraCAD user data directory + CAM/Machines
            try:
    ...
    def save_configuration(cls, config, filename=None):
        ...
        if filename is None:
            ...
            filename = config.name.replace(" ", "_").replace("/", "_") + ".auracadm"
    ...
    def list_builtin_templates(cls) -> list[tuple[str, str]]:
        ...
        Scans the Machine/machines subdirectory for .auracadm template files
        ...
        templates = []
        try:
            # Get the built-in machines directory from AuraCAD installation
            machines_dir = (
                pathlib.Path(AuraCAD.getHomePath()) / "Mod" / "CAM" / "Machine" / "machines"
            )

            if machines_dir.exists():
                for machine_file in sorted(machines_dir.glob("*.auracadm")):
    ...
    def list_addon_templates(cls) -> list:
        ...
        for namespace, addon_dir in cls._addon_machine_dirs:
            if not addon_dir.exists():
                continue
            for machine_file in sorted(addon_dir.glob("*.auracadm")):
    ...
    def _scan_installed_addon_dirs(cls) -> None:
        """Scan AuraCAD Mod directories for machine addons via package.xml content.
        ...
        Name-agnostic â€” does not rely on the addon folder name or on Init.py
        execution order.
        Supplements the Init.py push mechanism; duplicate registrations are ignored.

        Sentinel files:
          - ``Mod/ALL_ADDONS_DISABLED`` â€” skip the entire Mod tree.
          - ``<addon>/ADDON_DISABLED``  â€” skip a single addon.
        """
        for get_dir in (AuraCAD.getUserAppDataDir, AuraCAD.getHomePath):
            try:
                ...
                for entry in sorted(mod_root.iterdir(), key=lambda e: e.name.lower()):
                    ...
                    if not pkg_xml.exists():
                        continue
                    try:
                        meta = AuraCAD.Metadata(str(pkg_xml))
                    except Exception:
    ...
    def get_addon_machine_tree(cls) -> list:
        ...
        Each subtree is a list whose elements are either:
          ('file', display_name, full_path_str)  â€” a .auracadm machine file
          ('dir',  dir_name,     [sub_items])    â€” a subdirectory (n-depth)
    ...
    def _scan_machine_dir(cls, directory) -> list:
        """Recursively scan a directory for .auracadm files and subdirectories.
        ...
        try:
            ...
            for entry in entries:
                if entry.is_file() and entry.suffix == ".auracadm":
    ...
    def list_configuration_files(cls) -> list[tuple[str, pathlib.Path]]:
        ...
        Scans the Machine subdirectory of the asset path for .auracadm files
        ...
        try:
            ...
            if asset_base.exists():
                for p in sorted(asset_base.glob("*.auracadm")):
    ...
    def create_standard_configs(cls):
        ...
        for name, config in configs.items():
            try:
                filepath = cls.save_configuration(config, f"{name}.auracadm")
    ...
    def get_machine(cls, machine_name):
        ...
        Args:
            machine_name: Name of the machine to load (without .auracadm extension)
    ...
    def _read_machine_name_from_path(cls, filepath) -> str:
        """Read the machine display name from a full .auracadm file path.
