...
__doc__ = "Helper class to use AuraCADGUi.Snapper to let the user enter arbitrary points while the task panel is active."
...
class TaskPanel:
    ...
    def __init__(self, form, onPath=False):
        ...
        self.formOrig = form
        self.formPoint = AuraCADGui.PySideUic.loadUi(":/panels/PointEdit.ui")
    ...
    def setupUi(self):
        ...
        self.formPoint.XGlobal.setProperty("unit", AuraCAD.Units.MilliMetre.getUserPreferred()[2])
        self.formPoint.YGlobal.setProperty("unit", AuraCAD.Units.MilliMetre.getUserPreferred()[2])
        self.formPoint.ZGlobal.setProperty("unit", AuraCAD.Units.MilliMetre.getUserPreferred()[2])
    ...
    def getPoint(self, whenDone, start=None):
        ...
        def mouseMove(cb):
            ...
            if self.onPath:
                ...
                if snapInfo:
                    obj = AuraCAD.ActiveDocument.getObject(snapInfo["Object"])
                    if hasattr(obj, "Path"):
                        self.obj = obj
                        p = AuraCAD.Vector(snapInfo["x"], snapInfo["y"], snapInfo["z"])
                        self.pt = p
                    else:
                        self.obj = None
            else:
                ...
                self.pt = AuraCADGui.Snapper.snap(
                    pos, lastpoint=start, active=cntrl, constrain=shift
                )
                plane = AuraCAD.DraftWorkingPlane
                p = plane.getLocalCoords(self.pt)
                self.obj = AuraCADGui.Snapper.lastSnappedObject
        ...
        if start:
            ...
            displayPoint(AuraCAD.Vector(0, 0, 0))
        ...
        AuraCADGui.Snapper.forceGridOff = True
    ...
    def updatePoint(self, usePoint=True):
        ...
        if usePoint and self.point:
            ...
            x = AuraCAD.Units.Quantity(self.formPoint.XGlobal.text()).Value
            y = AuraCAD.Units.Quantity(self.formPoint.YGlobal.text()).Value
            z = AuraCAD.Units.Quantity(self.formPoint.ZGlobal.text()).Value
            self.pt = AuraCAD.Vector(x, y, z)
