        ...
        if hasattr(obj, "ExpressionEngine"):
            ...
            for prp, expr in obj.ExpressionEngine:
                if prp == prop:
                    ...
                    value = AuraCAD.Units.Quantity(obj.evalExpression(expr)).Value
                    if not Path.Geom.isRoughly(attrValue, value):
                        isDiff = True
                    break
            if exprSet:
...
class QuantitySpinBox(QtCore.QObject):
    ...
    def updateWidget(self, quantity=None):
        ...
        if self.valid:
            ...
            if quantity is None:
                if expr:
                    quantity = AuraCAD.Units.Quantity(self.obj.evalExpression(expr))
                else:
...
def getDocNode():
    doc = AuraCADGui.ActiveDocument.Document.Name
    tws = AuraCADGui.getMainWindow().findChildren(QtGui.QTreeWidget)
