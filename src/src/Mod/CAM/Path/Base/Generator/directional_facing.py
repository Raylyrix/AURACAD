...
def directional(
    ...
    if kept_segments == 0:
        ...
        for t in t_candidates:
            ...
            sp = (
                AuraCAD.Vector(origin)
                .add(AuraCAD.Vector(primary_vec).multiply(p_start))
                .add(AuraCAD.Vector(step_vec).multiply(t))
            )
            ep = (
                AuraCAD.Vector(origin)
                .add(AuraCAD.Vector(primary_vec).multiply(p_end))
                .add(AuraCAD.Vector(step_vec).multiply(t))
            )
            sp.z = z
            ep.z = z
