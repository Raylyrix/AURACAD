...
# ***************************************************************************
# *                                                                         *
# *   Copyright (c) 2025 sliptonic sliptonic@AuraCAD.org                    *
# *                                                                         *
...
def _create_link(
    ...
    # Natural semicircle radius for 180Â° arc
    r0 = chord_length / 2.0
    ...
    # Normal to chord (rotate chord by 90Â°)
    ...
    # For vertical chords (dy != 0, dx â‰ˆ 0), normal is horizontal
    # For horizontal chords (dx != 0, dy â‰ˆ 0), normal is vertical

    # Calculate both possible normals (90Â° rotations of chord)
    ...
    #
    # For r = r0 (minimum possible radius): offset = 0 (semicircle, 180Â° arc)
    ...
    # chord Ã— normal = (dx, dy, 0) Ã— (nx, ny, 0) = (0, 0, dx*ny - dy*nx)
    z_cross = dx * ny - dy * nx
...
def zigzag(
    ...
    primary_vec = AuraCAD.Vector(primary_vec).normalize()
    step_vec = AuraCAD.Vector(step_vec).normalize()
    ...
    # Guarantee full coverage at high stepover â€“ identical to bidirectional/directional
    if stepover_percent >= 99.9 and step_positions:
        ...
        if min_covered > min_t + 1e-4:
            ...
            added = True
        if added:
            Path.Log.info("Zigzag: Added extra pass(es) for full coverage at â‰¥100% stepover")
    ...
    base_negative = (
        milling_direction == "climb"
    ) ^ reverse  # True â†’ negative primary for first pass
