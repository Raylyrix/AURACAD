...
# SPDX-FileCopyrightText: 2025 sliptonic sliptonic@AuraCAD.org
...
def ensure_polygon(wire, num_segments=64):
    ...
    if points[0].distanceToPoint(points[-1]) > 1e-6:
        points.append(AuraCAD.Vector(points[0]))
...
def select_starting_corner(corners, primary_vec, step_vec, milling_direction):
    ...
    Args:
        ...
        primary_vec (AuraCAD.Vector): Primary direction vector (normalized)
        step_vec (AuraCAD.Vector): Step direction vector (normalized)
        milling_direction (str): "climb" or "conventional"

    Returns:
        AuraCAD.Vector: The selected starting corner point
    """
    if len(corners) < 4:
        return corners[0]
...
def get_angled_polygon(wire, angle):
    ...
    rotation_axis = AuraCAD.Vector(0, 0, 1)  # Z-axis
    ...
    corners = [
        AuraCAD.Vector(bounding_box.XMin, bounding_box.YMin, bounding_box.ZMin),
        AuraCAD.Vector(bounding_box.XMax, bounding_box.YMin, bounding_box.ZMin),
        AuraCAD.Vector(bounding_box.XMax, bounding_box.YMax, bounding_box.ZMin),
        AuraCAD.Vector(bounding_box.XMin, bounding_box.YMax, bounding_box.ZMin),
    ]
...
def validate_inputs(
    ...
    if start_point is not None:
        if not hasattr(start_point, "x") or not hasattr(start_point, "y"):
            raise ValueError("Start point must be a AuraCAD.Vector with x and y coordinates")
...
def align_edges_to_angle(primary_vec, step_vec, primary_length, step_length, angle_degrees):
    ...
    intended cut direction. This prevents sudden flips around angles like 46Â°/90Â°.
    """
    ...
    dir_vec = AuraCAD.Vector(math.cos(rad), math.sin(rad), 0)
...
def unit_vectors_from_angle(angle_degrees):
    ...
    primary_vec points in the angle direction. step_vec is +90Â° rotation (left normal).
    """
    import math

    rad = math.radians(angle_degrees)
    p = AuraCAD.Vector(math.cos(rad), math.sin(rad), 0)
    # +90Â° rotation
    s = AuraCAD.Vector(-math.sin(rad), math.cos(rad), 0)
    return p, s
...
def slice_wire_segments(wire, primary_vec, step_vec, t, origin):
    ...
    # Iterate edges
    for edge in wire.Edges:
        A = AuraCAD.Vector(edge.Vertexes[0].Point)
        B = AuraCAD.Vector(edge.Vertexes[1].Point)
