...
class refAxis(Enum):
    x = AuraCAD.Vector(1, 0, 0)
    y = AuraCAD.Vector(0, 1, 0)
...
def relAngle(vec, ref):
    ...
    norm.projectToPlane(AuraCAD.Vector(0, 0, 0), plane)

    ref = ref.value
    rot = AuraCAD.Rotation(norm, ref)
...
def generate(normalVector, aMin=-360, aMax=360, cMin=-360, cMax=360, compound=False):
    ...
    for result in cResults:
        ...
        rot = AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), result)
