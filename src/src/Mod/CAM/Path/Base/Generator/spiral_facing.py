...
def generate_spiral_corners(
    ...
    inward_primary = AuraCAD.Vector(primary_vec).multiply(inward_offset)
    inward_step = AuraCAD.Vector(step_vec).multiply(inward_offset)

    # The actual starting corner for this layer is offset inward
    layer_start_corner = AuraCAD.Vector(start_corner).add(inward_primary).add(inward_step)

    # Build rectangle from the offset starting corner with reduced dimensions
    corner1 = AuraCAD.Vector(layer_start_corner)
    corner2 = AuraCAD.Vector(corner1).add(
        AuraCAD.Vector(primary_vec).multiply(adjusted_primary_length)
    )
    corner3 = AuraCAD.Vector(corner2).add(AuraCAD.Vector(step_vec).multiply(adjusted_step_length))
    corner4 = AuraCAD.Vector(corner3).add(
        AuraCAD.Vector(primary_vec).multiply(-adjusted_primary_length)
    )
