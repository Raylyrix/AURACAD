...
def bidirectional(
    ...
    # Coverage guarantee at â‰¥100% stepover (exact same fix as zigzag/directional)
    if stepover_percent >= 99.9 and step_positions:
        ...
        if added:
            Path.Log.info(
                "Bidirectional facing: Added extra pass(es) for full coverage at â‰¥100% stepover"
            )

    center = (min_t + max_t) / 2.0

    # Split into bottom (â‰¤ center) and top (> center)
    bottom_positions = [t for t in step_positions if t <= center]  # ascending = outer â†’ inner
    top_positions = [t for t in step_positions if t > center][::-1]  # descending = outer â†’ inner
    ...
    for side, t in all_passes:
        # Same direction for all passes on the same side â†’ short outside rapids
        if side == "bottom":
            if milling_direction == "climb":
                p_start, p_end = end_s, start_s  # right â†’ left
            else:
                p_start, p_end = start_s, end_s  # left â†’ right
        else:  # top
            if milling_direction == "climb":
                p_start, p_end = start_s, end_s  # left â†’ right
            else:
        ...
        if commands:
            ...
            # First pass â€“ include Z for preamble replacement
