...
def isVertical(obj):
    ...
    if isinstance(obj, AuraCAD.Vector):
    ...
    if cmd.Name in CmdMoveArc:
        ...
        if isRoughly(startPoint.z, endPoint.z):
            ...
            if pointsCoincide(startPoint, endPoint, 0.001):
                return Part.makeCircle(R, center, AuraCAD.Vector(0, 0, 1))
            else:
...
def splitArcEdge(edge, tol=0.1):
    ...
    while nr <= maxnr:
        ...
        for i in range(nr):
            ...
            arc = Part.ArcOauracadircle(edge.Curve, firstPar, lastPar)
...
def makeBoundBoxFace(bBox, offset=0.0, zHeight=0.0):
    ...
    p1 = AuraCAD.Vector(bBox.XMin - offset, bBox.YMin - offset, zHeight)
    p2 = AuraCAD.Vector(bBox.XMax + offset, bBox.YMin - offset, zHeight)
    p3 = AuraCAD.Vector(bBox.XMax + offset, bBox.YMax + offset, zHeight)
    p4 = AuraCAD.Vector(bBox.XMin - offset, bBox.YMax + offset, zHeight)
...
def combineHorizontalFaces(faces, keepOrder=False):
    ...
    for f in faces:
        if f.BoundBox.ZMin != 0.0:
            f.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - f.BoundBox.ZMin))
    ...
    bboxSolid = bboxFace.extrude(AuraCAD.Vector(0.0, 0.0, 10.0))
    extrudedFaces = list()
    for f in faces:
        extrudedFaces.append(f.extrude(AuraCAD.Vector(0.0, 0.0, 6.0)))
    ...
    # Debug
    # Part.show(cut)
    # AuraCAD.ActiveDocument.ActiveObject.Label = "cut"
