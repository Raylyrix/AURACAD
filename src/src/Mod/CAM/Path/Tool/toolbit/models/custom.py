...
class ToolBitCustom(ToolBit):
    ...
    def summary(self) -> str:
        return AuraCAD.Qt.translate("CAM", "Unknown custom toolbit type")

    # Connor: Adding in getters and setters for diameter and length
    def get_diameter(self) -> AuraCAD.Units.Quantity:
        """
        Get the diameter of the rotary tool bit from the shape.
        """
        return self.obj.Diameter

    def set_diameter(self, diameter: AuraCAD.Units.Quantity):
        """
        Set the diameter of the rotary tool bit on the shape.
        """
        if not isinstance(diameter, AuraCAD.Units.Quantity):
            raise ValueError("Diameter must be a AuraCAD Units.Quantity")
        self.obj.Diameter = diameter

    def get_length(self) -> AuraCAD.Units.Quantity:
        """
        Get the length of the rotary tool bit from the shape.
        """
        return self.obj.Length

    def set_length(self, length: AuraCAD.Units.Quantity):
        """
        Set the length of the rotary tool bit on the shape.
        """
        if not isinstance(length, AuraCAD.Units.Quantity):
            raise ValueError("Length must be a AuraCAD Units.Quantity")
