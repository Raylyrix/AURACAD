...
class ToolBitPropertiesWidget(QtGui.QWidget):
    ...
    def __init__(
        ...
        properties_group_box = QtGui.QGroupBox(AuraCAD.Qt.translate("CAM", "Properties"))
    ...
    def load_toolbit(self, toolbit: ToolBit):
        ...
        elif AuraCAD.ActiveDocument is None:
            setToolBitSchema()
        ...
        has_file = (
            ...
            and os.path.isfile(os.path.join(shape_dir, f"{current_type}.auracadstd"))
        )

        editable_types = [base.lower()]
        for subtype in subtypes:
            if not os.path.isfile(os.path.join(shape_dir, f"{subtype}.auracadstd")):
...
class ToolBitEditor(QtGui.QWidget):
    ...
    def __init__(
        ...
        super().__init__(parent)
        self.form = AuraCADGui.PySideUic.loadUi(":/panels/ToolBitEditor.ui")
        ...
        self._original_schema = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Units").GetInt(
            "UserSchema", 6
        )
    ...
    def _refresh_property_editor(self):
        ...
        AuraCAD.Units.setSchema(self._original_schema)
