...
class auracadTBSerializer(AssetSerializer):
    for_class = ToolBit
    mime_type = "application/x-AuraCAD-toolbit"
    extensions = (".auracadtb",)

    @classmethod
    def get_label(cls) -> str:
        return AuraCAD.Qt.translate("CAM", "AuraCAD Tool")

    @classmethod
    def extract_dependencies(cls, data: bytes) -> List[AssetUri]:
        """Extracts URIs of dependencies from serialized data."""
        Path.Log.debug(f"auracadTBSerializer.extract_dependencies: raw data = {data!r}")
    ...
    def deserialize(
        ...
        if dependencies is None:
            ...
            Path.Log.debug(f"auracadTBSerializer.deserialize: shallow. id = {id!r}, attrs = {attrs!r}")
