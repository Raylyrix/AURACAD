...
def units_from_json(params):
    ...
    Args:
        params: Dictionary of parameters from JSON (before conversion to AuraCAD.Units.Quantity)
...
def format_value(
    value: AuraCAD.Units.Quantity | int | float | None,
    ...
    This function uses the ToolBitSchema (via setToolBitSchema) to ensure that units are formatted according to the correct schema (Metric or Imperial) when a AuraCAD.Units.Quantity is provided. The schema is temporarily set for formatting and then restored.
    ...
    if value is None:
        ...
        if precision is not None:
            ...
            if user_unit in ("deg", "Â°", "degree", "degrees"):
...
def setToolBitSchema(schema=None):
    """
    Set the AuraCAD units schema. If passed 'Metric' or 'Imperial', set accordingly (case-insensitive).
    Otherwise, if a document is open, set to its schema. If no document, fallback to user preference or provided schema.
    """
    units_schema_map = {
        "metric": 6,  # 6 = Metric schema in AuraCAD
        "imperial": 3,  # 3 = Imperial schema in AuraCAD
    }
    if isinstance(schema, str) and schema.lower() in units_schema_map:
        AuraCAD.Units.setSchema(units_schema_map[schema.lower()])
        return
    if AuraCAD.ActiveDocument is not None:
        try:
            doc_schema = AuraCAD.ActiveDocument.getSchema()
            AuraCAD.Units.setSchema(doc_schema)
            return
        except Exception:
            pass
    # Fallback to user preference or provided schema
    if schema is None:
        schema = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Units").GetInt(
            "UserSchema", 6
        )
    AuraCAD.Units.setSchema(schema)
