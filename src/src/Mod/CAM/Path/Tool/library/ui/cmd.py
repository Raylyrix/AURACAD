...
class CommandToolBitLibraryDockOpen:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP("CAM_ToolBitSelection", "Add toolbitâ€¦"),
...
class CommandLibraryEditorOpen:
    ...
    def Activated(self):
        library = LibraryEditor(parent=AuraCADGui.getMainWindow())
        library.open()


if AuraCAD.GuiUp:
    AuraCADGui.addCommand("CAM_ToolBitLibraryOpen", CommandLibraryEditorOpen())
    AuraCADGui.addCommand("CAM_ToolBitDock", CommandToolBitLibraryDockOpen())

BarList = ["CAM_ToolBitDock"]
MenuList = ["CAM_ToolBitLibraryOpen", "CAM_ToolBitDock"]

AuraCAD.Console.PrintLog("Loading PathToolBitLibraryCmdâ€¦ done\n")
