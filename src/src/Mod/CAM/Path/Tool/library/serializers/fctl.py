...
class auracadTLSerializer(AssetSerializer):
    for_class = Library
    extensions = (".auracadtl",)
    mime_type = "application/x-AuraCAD-toolbit-library"

    @classmethod
    def get_label(cls) -> str:
        return AuraCAD.Qt.translate("CAM", "AuraCAD Tool Library")
    ...
    def deserialize(
        ...
        if dependencies is None:
            Path.Log.debug(
                f"auracadTLSerializer.deserialize: Shallow load for library '{library.label}' (id: {id}). Tools not populated."
            )
    ...
    def deep_deserialize(cls, data: bytes) -> Library:
        ...
        Path.Log.info(
            f"auracadTL DEEP_DESERIALIZE: Starting deep deserialization for library id='{library_id}'"
        )

        # Extract dependency URIs from the library data
        dependency_uris = cls.extract_dependencies(data)
        Path.Log.info(
            f"auracadTL DEEP_DESERIALIZE: Found {len(dependency_uris)} toolbit dependencies: {[uri.asset_id for uri in dependency_uris]}"
        )
        ...
        for dep_uri in dependency_uris:
            try:
                Path.Log.info(
                    f"auracadTL DEEP_DESERIALIZE: Fetching toolbit '{dep_uri.asset_id}' from stores ['local', 'builtin']"
                )
                ...
                Path.Log.info(
                    f"auracadTL DEEP_DESERIALIZE: Toolbit '{dep_uri.asset_id}' exists - local: {exists_local}, builtin: {exists_builtin}"
                )

                toolbit = cam_assets.get(dep_uri, store=["local", "builtin"], depth=0)
                resolved_dependencies[dep_uri] = toolbit
                Path.Log.info(
                    f"auracadTL DEEP_DESERIALIZE: Successfully fetched toolbit '{dep_uri.asset_id}'"
                )
            except Exception as e:
                Path.Log.warning(
                    f"auracadTL DEEP_DESERIALIZE: Failed to fetch toolbit '{dep_uri.asset_id}': {e}"
                )
                ...
                try:
                    ...
                    Path.Log.info(
                        f"auracadTL DEBUG: Local store has {len(local_ids)} toolbits: {local_ids[:10]}{'...' if len(local_ids) > 10 else ''}"
                    )

                    if dep_uri.asset_id in local_ids:
                        Path.Log.warning(
                            f"auracadTL DEBUG: Toolbit '{dep_uri.asset_id}' IS in local store list but get() failed!"
                        )
                except Exception as list_error:
                    Path.Log.error(f"auracadTL DEBUG: Failed to list local toolbits: {list_error}")

        Path.Log.info(
            f"auracadTL DEEP_DESERIALIZE: Resolved {len(resolved_dependencies)} of {len(dependency_uris)} dependencies"
        )
    ...
    def deep_deserialize_with_context(cls, data: bytes, file_path: "pathlib.Path"):
        ...
        Path.Log.info(
            f"auracadTL DEEP_DESERIALIZE_WITH_CONTEXT: Starting deep deserialization for library from {file_path}"
        )

        # Extract dependency URIs from the library data
        dependency_uris = cls.extract_dependencies(data)
        Path.Log.info(
            f"auracadTL DEEP_DESERIALIZE_WITH_CONTEXT: Found {len(dependency_uris)} toolbit dependencies: {[uri.asset_id for uri in dependency_uris]}"
        )
        ...
        for dep_uri in dependency_uris:
            try:
                ...
                Path.Log.info(
                    f"auracadTL EXTERNAL: Trying to fetch toolbit '{dep_uri.asset_id}' from stores ['local', 'builtin']"
                )
                toolbit = cam_assets.get(dep_uri, store=["local", "builtin"], depth=0)
                resolved_dependencies[dep_uri] = toolbit
                Path.Log.info(
                    f"auracadTL EXTERNAL: Successfully fetched toolbit '{dep_uri.asset_id}' from stores"
                )
            except Exception as e:
                # If not in stores, try to load from parallel Bit directory
                Path.Log.info(
                    f"auracadTL EXTERNAL: Toolbit '{dep_uri.asset_id}' not in stores, trying external file: {e}"
                )
                ...
                if bit_dir.exists():
                    possible_extensions = [".auracadtb", ".json", ".yaml", ".yml"]
                        ...
                        if toolbit_file.exists():
                            try:
                                ...
                                if serializer_class:
                                    ...
                                    toolbit_loaded = True
                                    Path.Log.info(
                                        f"auracadTL EXTERNAL: Successfully loaded toolbit '{dep_uri.asset_id}' from {toolbit_file}"
                                    )
                                    break
                            except Exception as load_error:
                                Path.Log.warning(
                                    f"auracadTL EXTERNAL: Failed to load toolbit from {toolbit_file}: {load_error}"
                                )
                                continue

                if not toolbit_loaded:
                    Path.Log.warning(
                        f"auracadTL EXTERNAL: Could not load toolbit '{dep_uri.asset_id}' from external files"
                    )

        Path.Log.info(
            f"auracadTL EXTERNAL: Resolved {len(resolved_dependencies)} of {len(dependency_uris)} dependencies"
        )
