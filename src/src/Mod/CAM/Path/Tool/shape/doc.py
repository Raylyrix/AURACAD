...
def find_shape_object(doc: "AuraCAD.Document") -> Optional["AuraCAD.DocumentObject"]:
    """
...
def get_unset_value_for(attribute_type: str):
    if attribute_type == "App::PropertyLength":
        return AuraCAD.Units.Quantity(0)
...
def get_object_properties(
    ...
    """
    Extract properties from a AuraCAD PropertyBag, including their types.
    ...
    Returns:
        Dict[str, Tuple[Any, str]]: A dictionary mapping property names to a tuple
                                    (value, type_id). Values are AuraCAD native types.
...
def update_shape_object_properties(
    obj: "AuraCAD.DocumentObject", parameters: Dict[str, Any]
) -> None:
    """
    Update properties of a AuraCAD PropertyBag based on a dictionary of parameters.

    Args:
        obj (AuraCAD.DocumentObject): The PropertyBag to update properties on.
...
def get_doc_state() -> Any:
    """
    Used to make a "snapshot" of the current state of AuraCAD, to allow
    for restoring the ActiveDocument and selection state later.
    """
    doc_name = AuraCAD.ActiveDocument.Name if AuraCAD.ActiveDocument else None
    if AuraCAD.GuiUp:
        import AuraCADGui
...
def restore_doc_state(state):
    ...
    if AuraCAD.GuiUp:
        ...
        for sel in selection:
            AuraCADGui.Selection.addSelection(doc_name, sel.Name)


class ShapeDocFromBytes:
    """
    Context manager to create and manage a temporary AuraCAD document,
    ...
    def __enter__(self) -> "AuraCAD.Document":
        """Creates a new temporary AuraCAD document or loads cache if provided."""
        # Create a temporary file and write the cache content to it
        with tempfile.NamedTemporaryFile(suffix=".auracadStd", delete=False) as tmp_file:
            tmp_file.write(self._content)
            self._temp_file = tmp_file.name

        # When we open a new document, AuraCAD loses the state, of the active
