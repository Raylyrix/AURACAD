...
class ToolBitShape(Asset):
    ...
    #      "Endmill", then by default the file is "endmill.auracadstd".
    #   2. to identify the shape class from a shape.auracadstd file.
    #      Upon loading a shape, the name of the body in the shape
    #      file is read. It MUST match one of the names.
    name: str

    # Aliases exist for backward compatibility. If an existing .auracadtb file
    # references a shape such as "v-bit.auracadtb", and that shape file cannot
    ...
    def __init__(self, id: str, **kwargs: Any):
        ...
        Args:
            ...
            **kwargs: Keyword arguments for shape parameters (e.g., Diameter).
                      Values should be AuraCAD.Units.Quantity where applicable.
        """
        # _params will be populated with default values after loading
        self._params: Dict[str, Any] = {}

        # Stores default parameter values loaded from the auracadStd file
        self._defaults: Dict[str, Any] = {}

        # Stores the AuraCAD property types for each parameter
        self._param_types: Dict[str, str] = {}

        # Keeps the loaded AuraCAD document content for this instance
    ...
    @classmethod
    def _get_shape_class_from_doc(cls, doc: "AuraCAD.Document") -> Type["ToolBitShape"]:
    ...
    def get_shape_class_from_bytes(cls, data: bytes) -> Type["ToolBitShape"]:
        """
        Identifies the ToolBitShape subclass from the raw bytes of an auracadStd file
        by parsing the XML content to find the Body label.

        Args:
            data (bytes): The raw bytes of the .auracadStd file.

        Returns:
            Type[ToolBitShape]: The appropriate ToolBitShape subclass.

        Raises:
            ValueError: If the data is not a valid auracadStd file, Document.xml is
                        missing, no Body object is found, or the Body label
                        does not match a known shape name.
        """
        try:
            # auracadStd files are zip archives
            ...
            raise ValueError("Invalid auracadStd file data (not a valid zip archive)")
        except KeyError:
            raise ValueError("Invalid auracadStd file data (Document.xml not found)")
        except ET.ParseError:
            raise ValueError("Error parsing Document.xml")
        except Exception as e:
            # Catch any other unexpected errors during parsing
            raise ValueError(f"Error processing auracadStd data: {e}")

    @classmethod
    def _find_property_object(cls, doc: "AuraCAD.Document") -> Optional["AuraCAD.DocumentObject"]:
        """
        Find the PropertyBag object named "Attributes" in a document.

        Args:
            doc (AuraCAD.Document): The document to search within.

        Returns:
            Optional[AuraCAD.DocumentObject]: The found object or None.
        """
    ...
    def extract_dependencies(cls, data: bytes, serializer: Type[AssetSerializer]) -> List[AssetUri]:
        """
        Extracts URIs of dependencies from the raw bytes of an auracadStd file.
        ...
        # We need to extract the shape ID from the auracadStd data.
        try:
    ...
    def from_bytes(
        ...
        """
        Create a ToolBitShape instance from the raw bytes of an auracadStd file.
        ...
        Raises:
            ValueError: If the data cannot be opened, no Body or PropertyBag
                        ...
                        shape name.
            Exception: For other potential AuraCAD errors during loading.
        """
    ...
    def to_bytes(self, serializer: Type[AssetSerializer]) -> bytes:
        """
        Serializes a ToolBitShape object to bytes (e.g., an auracadstd file).
        ...
        try:
            ...
            doc = AuraCAD.newDocument("TemporaryShapeDoc", hidden=True)
            ...
            with tempfile.TemporaryDirectory() as thedir:
                temp_file_path = pathlib.Path(thedir, "temp.auracadStd")
    ...
    def from_file(cls, filepath: pathlib.Path, **kwargs: Any) -> "ToolBitShape":
        """
        Create a ToolBitShape instance from an auracadStd file.

        Reads the file bytes and delegates to from_bytes().

        Args:
            filepath (pathlib.Path): Path to the .auracadStd file.
    ...
    def resolve_name(cls, identifier: str) -> AssetUri:
        ...
        if pathlib.Path(identifier).suffix.lower() == ".auracadstd":
    ...
    def schema(cls) -> Mapping[str, Tuple[str, str]]:
        ...
        translations and AuraCAD property type strings (e.g.,
        'App::PropertyLength').
    ...
    def get_schema_property_type(cls, param_name: str) -> str:
        """
        Get the AuraCAD property type string for a given parameter name.
        """
    ...
    def get_parameter(self, name: str) -> Any:
        ...
        Returns:
            The value of the parameter (often a AuraCAD.Units.Quantity).
    ...
    def set_parameter(self, name: str, value: Any):
        ...
        Args:
            ...
            value: The new value for the parameter. Should be compatible
                   with the expected type (e.g., AuraCAD.Units.Quantity).
    ...
    def make_body(self, doc: "AuraCAD.Document"):
        """
        ...
        with ShapeDocFromBytes(self._data) as tmp_doc:
            ...
            # Temporarily disable duplicate labels to let AuraCAD automatically
            ...
            try:
                ...
                # Copy the body - AuraCAD will now automatically make labels unique
    ...
    def get_parameter_type(self, name: str) -> str:
        """
        Get the AuraCAD property type string for a given parameter name,
        as loaded from the auracadStd file.
        """
