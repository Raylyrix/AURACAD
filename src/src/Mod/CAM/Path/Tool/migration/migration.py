...
Handles migration of CAM assets during AuraCAD version upgrades.
"""
...
class CAMAssetMigrator:
    ...
    def check_asset_location(self):
        ...
        not been offered for the current AuraCAD version.
        """
        Path.Log.debug("Starting CAM asset migration check")

        try:
            # Get current directories
            user_app_data_dir = AuraCAD.getUserAppDataDir()
            ...
            if AuraCAD.ApplicationDirectories.isVersionedPath(str(current_asset_path)):
                # Check if we're already using the current version
                if AuraCAD.ApplicationDirectories.usingCurrentVersionConfig(
    ...
    def check_tool_library_workdir(self):
        ...
        if workdir and not migrated_dir:
            ...
            if os.path.isdir(workdir):
                libraries = [f for f in glob.glob(workdir + os.path.sep + "*.auracadtl")]
                libraries.sort()
                if len(libraries):
    ...
    def _offer_asset_relocation(self):
        ...
        major = int(AuraCAD.ConfigGet("BuildVersionMajor"))
        minor = int(AuraCAD.ConfigGet("BuildVersionMinor"))
        current_version = AuraCAD.ApplicationDirectories.versionStringForPath(major, minor)
        ...
        msg = (
            f"AuraCAD has been upgraded to version {current_version}.\n\n"
        ...
        pref_group = AuraCAD.ParamGet(self.pref_group_path)
    ...
    def _migrate_assets(self, source_path):
        ...
        try:
            AuraCAD.ApplicationDirectories.migrateAllPaths([source_path])
            Path.Log.info(
    ...
    def has_migration_been_offered(self):
        ...
        current_version_string = AuraCAD.ApplicationDirectories.versionStringForPath(major, minor)
        offered_versions = pref_group.GetString("OfferedToMigrateCAMAssets", "")
        known_versions = set(offered_versions.split(",")) if offered_versions else set()
