...
updating, deleting, and receiving assets for the AuraCAD CAM workbench.
...
Assets are arbitrary data, such as AuraCAD models, Tools, and many more.
...
and a ToolBit requires a ToolBitShape (which is a AuraCAD model).
...
- **Manages storage** while existing AuraCAD tool library file structures retained
- **Manages dependencies** including detection of cyclic dependencies, deep vs. shallow fetching
- **Manages threading** for asynchronous storage, while AuraCAD objects are assembled in the main UI thread
...
- Adding a GitStore, to connect to things like the [AuraCAD library](https://github.com/AuraCAD/AuraCAD-library).
