...
class ObjectOp(PathOp.ObjectOp):
    ...
    def _buildPathArea(self, obj, baseobject, isHole, start, getsim):
        ...
        areaParams["SectionTolerance"] = AuraCAD.Base.Precision.confusion() * 10  # basically 1e-06
        ...
        if getsim:
            ...
            simobj = sec.extrude(AuraCAD.Vector(0, 0, baseobject.BoundBox.ZMax))
    ...
    def _buildProfileOpenEdges(self, obj, edgeList, isHole, start, getsim):
        ...
        for i in range(0, len(heights)):
            for baseShape in edgeList:
                ...
                hWire.translate(AuraCAD.Vector(0, 0, heights[i] - hWire.BoundBox.ZMin))
                ...
                if Path.Geom.isRoughly(y, 0.0):
                    y = 0.00001
                pathParams["start"] = AuraCAD.Vector(x, y, verts[0].Z)
    ...
    def opExecute(self, obj, getsim=False):
        ...
        for shp in self.areaOpShapes(obj):
            if len(shp) == 2:
                auracad, iH = shp
                #     auracad, iH,  sub or description
                tup = auracad, iH, "otherOp"
                shapes.append(tup)
            else:
                shapes.append(shp)
