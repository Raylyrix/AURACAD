...
def _sortVoronoiWires(wires, start=AuraCAD.Vector(0, 0, 0)):
...
def canSkipRepositioning(positionHistory, newPosition, tolerance):
    ...
    v0 = AuraCAD.Base.Vector(currentPosition.x, currentPosition.y)
    v1 = AuraCAD.Base.Vector(newPosition.x, newPosition.y)
...
class ObjectVcarve(PathEngraveBase.ObjectOp):
    ...
    def buildMedialWires(self, obj, faces):
        ...
        def is_exterior(vertex, face):
            vector = AuraCAD.Vector(vertex.toPoint(face.BoundBox.ZMin))
        ...
        def insert_many_wires(vd, wires):
            for wire in wires:
                ...
                ptv = [AuraCAD.Vector(p.x, p.y) for p in pts]
                ...
                if len(ptv) > 0:
                    ...
                    if dist < AuraCAD.Base.Precision.confusion():
                        Path.Log.debug(
    ...
    def debugVoronoiMedial(self, obj):
        ...
        vPart = AuraCAD.activeDocument().addObject("App::Part", f"{obj.Name}-VoronoiDebugMedial")
    ...
    def debugVoronoiEdges(self, obj):
        ...
        vPart = AuraCAD.activeDocument().addObject("App::Part", f"{obj.Name}-VoronoiDebugEdge")

        edgesToShow = []

        for face, edges in self.voronoiDebugEdgeCache.items():
            for edge in edges:  # those are voronoi Edge objects, not auracad Edge
                currentEdge = edge.toShape()
