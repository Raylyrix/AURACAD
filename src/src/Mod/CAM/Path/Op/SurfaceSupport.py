...
class PathGeometryGenerator:
    ...
    def __init__(self, obj, shape, pattern):
        ...
        if shape.BoundBox.ZMin != 0.0:
            shape.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - shape.BoundBox.ZMin))
    ...
    def _prepareConstants(self):
        ...
        if self.pattern in ["Circular", "CircularZigZag", "Spiral"]:
            if self.obj.PatternCenterAt == "CenterOfMass":
                auracadnt = 0
                totArea = 0.0
                zeroCOM = AuraCAD.Vector(0.0, 0.0, 0.0)
                for F in self.shape.Faces:
                    comF = F.CenterOfMass
                    areaF = F.Area
                    totArea += areaF
                    auracadnt += 1
                    zeroCOM = zeroCOM.add(AuraCAD.Vector(comF.x, comF.y, 0.0).multiply(areaF))
                if auracadnt == 0:
                    ...
                    AuraCAD.Console.PrintError(msg)
                    bbC = self.shape.BoundBox.Center
                    zeroCOM = AuraCAD.Vector(bbC.x, bbC.y, 0.0)
                else:
                    avgArea = totArea / auracadnt
                    zeroCOM.multiply(1 / auracadnt)
                    zeroCOM.multiply(1 / avgArea)
                self.centerOfMass = AuraCAD.Vector(zeroCOM.x, zeroCOM.y, 0.0)
            self.centerOfPattern = self._getPatternCenter()
        else:
            bbC = self.shape.BoundBox.Center
            self.centerOfPattern = AuraCAD.Vector(bbC.x, bbC.y, 0.0)
    ...
    def generatePathGeometry(self):
        ...
        if self.pattern in ["Line", "ZigZag"]:
            if self.obj.CutPatternAngle != 0.0:
                geomShape.Placement.Rotation = AuraCAD.Rotation(
                    AuraCAD.Vector(0, 0, 1), self.obj.CutPatternAngle
                )
            bbC = self.shape.BoundBox.Center
            geomShape.Placement.Base = AuraCAD.Vector(bbC.x, bbC.y, 0.0 - geomShape.BoundBox.ZMin)

        if self.debugObjectsGroup:
            F = AuraCAD.ActiveDocument.addObject("Part::Feature", "tmpGeometrySet")
            F.Shape = geomShape
            F.purgeTouched()
        ...
        if self.debugObjectsGroup:
            F = AuraCAD.ActiveDocument.addObject("Part::Feature", "tmpPathGeometry")
            F.Shape = cmnShape
            F.purgeTouched()
            self.debugObjectsGroup.addObject(F)
    ...
    def _Line(self):
        GeoSet = []
        centRot = AuraCAD.Vector(0.0, 0.0, 0.0)  # Bottom left corner of face/selection/model
        ...
        for lc in range((-1 * (self.halfPasses - 1)), self.halfPasses + 1):
            ...
            # y2 = y1
            p1 = AuraCAD.Vector(x1, y1, 0.0)
            p2 = AuraCAD.Vector(x2, y1, 0.0)
    ...
    def _Spiral(self):
        ...
        lastPoint = AuraCAD.Vector(0.0, 0.0, 0.0)
    ...
    def _getPatternCenter(self):
        ...
        if centerAt == "CenterOfMass":
            cntrPnt = AuraCAD.Vector(self.centerOfMass.x, self.centerOfMass.y, 0.0)
        elif centerAt == "CenterOfBoundBox":
            cent = self.shape.BoundBox.Center
            cntrPnt = AuraCAD.Vector(cent.x, cent.y, 0.0)
        elif centerAt == "XminYmin":
            cntrPnt = AuraCAD.Vector(self.shape.BoundBox.XMin, self.shape.BoundBox.YMin, 0.0)
        elif centerAt == "Custom":
            cntrPnt = AuraCAD.Vector(
    ...
    def _getRadialPasses(self):
        ...
        if self.obj.PatternCenterAt != "CenterOfBoundBox":
            ...
            CORNERS = [
                AuraCAD.Vector(EBB.XMin, EBB.YMin, 0.0),
                AuraCAD.Vector(EBB.XMin, EBB.YMax, 0.0),
                AuraCAD.Vector(EBB.XMax, EBB.YMax, 0.0),
                AuraCAD.Vector(EBB.XMax, EBB.YMin, 0.0),
            ]
            dMax = 0.0
            for c in range(0, 4):
    ...
    def _makeRegSpiralPnt(self, move, b, radAng):
        ...
        return AuraCAD.Vector(x, y, 0.0).add(move)

    def _makeOppSpiralPnt(self, move, b, radAng):
        x = b * radAng * math.cos(radAng)
        y = b * radAng * math.sin(radAng)
        return AuraCAD.Vector(-1 * x, y, 0.0).add(move)
...
class ProcessSelectedFaces:
        ...
        if self.obj.Base:
            if len(self.obj.Base) > 0:
                ...
                if self.obj.ScanType == "Rotational":
                    self.checkBase = False
                    AuraCAD.Console.PrintWarning(self.msgNoFaces)
        ...
        for m in range(0, lenGRP):
            if self.modelSTLs[m] and not fShapes[m]:
                ...
                if pPEB is False:
                    ...
                    auracadShp, prflShp = pPEB
                    if auracadShp:
                        if auracadShp is True:
                            Path.Log.debug(" -auracadShp is True.")
                            fShapes[m] = True
                        else:
                            fShapes[m] = [auracadShp]
                    if prflShp:
                        if auracadShp:
    ...
    def _preProcessFacesAndVoids(self, base, auracadS, VDS):
        mFS = False
        mVS = False
        mPS = False
        mIFS = []
            ...
            if self.obj.HandleMultipleFeatures == "Collectively":
                ...
                if cont:
                    if self.showDebugObjects:
                        T = AuraCAD.ActiveDocument.addObject("Part::Feature", "tmpCollectiveShape")
                        T.Shape = cfsL
                        T.purgeTouched()
                ...
                if cont:
                    ...
                    if not self.obj.InternalFeaturesCut:
                        if lenIfL == 0:
                            ...
                            if self.showDebugObjects:
                                C = AuraCAD.ActiveDocument.addObject(
                ...
                for auracadshp, auracadIdx in auracadS:
                    cont = True
                    fNum = auracadIdx + 1
                    outerFace = False

                    gUR, ifL = self.findUnifiedRegions(auracadS)
                    if len(gUR) > 0:
                        outerFace = gUR[0]
        ...
        if VDS:
            ...
            outauracadS, intFEAT = self.findUnifiedRegions(VDS)
            if self.obj.InternalFeaturesCut:
                intFEAT = []

            lenOtauracads = len(outauracadS)
            if lenOtauracads == 0:
                cont = False
            else:
                if lenOtauracads == 1:
                    ...
                    avoid = Part.makeCompound(outauracadS)

                if self.showDebugObjects:
                    P = AuraCAD.ActiveDocument.addObject("Part::Feature", "tmpVoidEnvelope")
                    P.Shape = avoid
                    P.purgeTouched()
            ...
            if cont:
                if self.showDebugObjects:
                    P = AuraCAD.ActiveDocument.addObject("Part::Feature", "tmpVoidCompound")
                    P.Shape = avoid
                    P.purgeTouched()
                    self.tempGroup.addObject(P)
            ...
            if cont:
                ...
                if not self.obj.AvoidLastX_InternalFeatures and len(intFEAT) > 0:
                    if len(intFEAT) > 1:
                        iauracad = Part.makeCompound(intFEAT)
                    else:
                        iauracad = intFEAT[0]
                    ofstVal = self._calculateOffsetValue(isHole=True)
                    ifOfstShp = PathUtils.getOffsetArea(iauracad, ofstVal, plane=self.wpc)
        ...
        if cont:
            ...
            if faceOffsetShape is False:
                ...
                faceOffsetShape.translate(
                    AuraCAD.Vector(0.0, 0.0, 0.0 - faceOffsetShape.BoundBox.ZMin)
                )
...
def getExtrudedShape(wire):
    ...
    try:
        shell = wire.extrude(AuraCAD.Vector(0.0, 0.0, extFwd))
    except Exception as ee:
...
def getShapeSlice(shape):
    ...
    ymin = bb.YMin - 1.0
    ymax = bb.YMax + 1.0
    p1 = AuraCAD.Vector(xmin, ymin, mid)
    p2 = AuraCAD.Vector(xmax, ymin, mid)
    p3 = AuraCAD.Vector(xmax, ymax, mid)
    p4 = AuraCAD.Vector(xmin, ymax, mid)
    ...
    if slcArea < midArea:
        ...
        if len(slcShp.Wires) == 1:
            wire = slcShp.Wires[0]
            slc = Part.Face(wire)
            slc.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - slc.BoundBox.ZMin))
            return slc
        else:
            fL = []
            for W in slcShp.Wires:
...
def getProjectedFace(tempGroup, wire):
    ...
    F = AuraCAD.ActiveDocument.addObject("Part::Feature", "tmpProjectionWire")
    F.Shape = wire
    F.purgeTouched()
    tempGroup.addObject(F)
    try:
        prj = Draft.makeShape2DView(F, AuraCAD.Vector(0, 0, 1))
        prj.recompute()
        prj.purgeTouched()
...
def getCrossSection(shape):
    ...
    for i in shape.slice(AuraCAD.Vector(0, 0, 1), mid):
        wires.append(i)

    if len(wires) > 0:
        comp = Part.Compound(wires)  # produces correct cross-section wire !
        comp.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - comp.BoundBox.ZMin))
        csWire = comp.Wires[0]
        if csWire.isClosed() is False:
            Path.Log.debug(" -comp.Wires[0] is not closed")
            return False
        CS = Part.Face(csWire)
        CS.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - CS.BoundBox.ZMin))
        return CS
    else:
...
def getShapeEnvelope(shape):
    ...
    try:
        ...
        AuraCAD.Console.PrintError("PathUtils.getEnvelope() failed.\n" + str(ee) + "\n")
        return False
    else:
        return env
...
def getSliceFromEnvelope(env):
    ...
    tf.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - tf.BoundBox.ZMin))
...
def _makeSafeSTL(self, JOB, obj, mdlIdx, faceShapes, voidShapes, ocl):
    ...
    if obj.BoundBox == "BaseBoundBox":
        ...
        bH = 1.0
        crnr = AuraCAD.Vector(xMin, yMin, zMin - 1.0)
        B = Part.makeBox(bL, bW, bH, crnr, AuraCAD.Vector(0, 0, 1))
        fuseShapes.append(B)
    ...
    if self.showDebugObjects:
        T = AuraCAD.ActiveDocument.addObject("Part::Feature", "safeSTLShape")
        T.Shape = fused
        T.purgeTouched()
        self.tempGroup.addObject(T)
...
def pathGeomToLinesPointSet(self, obj, compGeoShp):
    ...
    if self.CutClimb is True:
        tup = (p2, p1)
        lst = AuraCAD.Vector(p1[0], p1[1], 0.0)
    else:
        tup = (p1, p2)
        lst = AuraCAD.Vector(p2[0], p2[1], 0.0)
    inLine.append(tup)
    sp = AuraCAD.Vector(p1[0], p1[1], 0.0)  # start point
    ...
    for ei in range(1, ec):
        ...
        ep = AuraCAD.Vector(v2[0], v2[1], 0.0)  # end point
        cp = AuraCAD.Vector(v1[0], v1[1], 0.0)  # check point (first / middle point)
        ...
        if iC is True:
            ...
            if self.CutClimb is True:
                sp = cp  # AuraCAD.Vector(v1[0], v1[1], 0.0)
            else:
                sp = ep
...
def pathGeomToZigzagPointSet(self, obj, compGeoShp):
    ...
    if dirFlg == 1:
        ...
        tup = (p2, p1)
        lst = AuraCAD.Vector(p1[0], p1[1], 0.0)
        sp = AuraCAD.Vector(p2[0], p2[1], 0.0)  # start point
    inLine.append(tup)
    ...
    for ei in range(1, ec):
        ...
        cp = AuraCAD.Vector(v1[0], v1[1], 0.0)  # check point (start point of segment)
        ep = AuraCAD.Vector(v2[0], v2[1], 0.0)  # end point
        iC = cp.isOnLineSegment(sp, ep)
        if iC:
            inLine.append("BRK")
...
def pathGeomToCircularPointSet(self, obj, compGeoShp):
    ...
    def dist_to_cent(item):
        ...
        s = item[2][0][0]
        p1 = AuraCAD.Vector(s[0], s[1], 0.0)
        e = item[2][0][2]
        p2 = AuraCAD.Vector(e[0], e[1], 0.0)
    ...
    for ei in range(0, ec):
        ...
        if edg.Closed is True:
            ...
            if isSame is False:
                segEI.append(ei)
                isSame = True
                pnt = AuraCAD.Vector(edg.Vertexes[0].X, edg.Vertexes[0].Y, 0.0)
    ...
    for so in range(0, len(stpOvrEI)):
        ...
        if SO[0] == "L":  # L = Loop/Ring/Circle
            ...
            # p1 = AuraCAD.Vector(v1.X, v1.Y, v1.Z)
            p1 = AuraCAD.Vector(v1.X, v1.Y, 0.0)  # z=0.0 for waterline; z=v1.Z for 3D Surface
...
def pathGeomToSpiralPointSet(obj, compGeoShp):
    ...
    p1 = AuraCAD.Vector(edg1.Vertexes[0].X, edg1.Vertexes[0].Y, 0.0)
    p2 = AuraCAD.Vector(edg1.Vertexes[1].X, edg1.Vertexes[1].Y, 0.0)
    ...
    for ei in range(start, ec):  # Skipped first edge, started with second edge above as edg1
        ...
        ep = AuraCAD.Vector(edg.Vertexes[1].X, edg.Vertexes[1].Y, 0.0)  # end point
...
def pathGeomToOffsetPointSet(obj, compGeoShp):
    ...
    for ei in range(0, ofstCnt):
        ...
        fp = AuraCAD.Vector(OS[0].x, OS[0].y, OS[0].z)
        OS.append(fp)
...
class FindUnifiedRegions:
    ...
    def _showShape(self, shape, name):
        if self.tempGroup:
            S = AuraCAD.ActiveDocument.addObject("Part::Feature", "tmp" + name)
            S.Shape = shape
            S.purgeTouched()
            self.tempGroup.addObject(S)

    def _extractTopFaces(self):
        for F, auracadIdx in self.FACES:  # format is tuple (faceShape, faceIndex_on_base)
            cont = True
            fNum = auracadIdx + 1
            # Extrude face
            fBB = F.BoundBox
            extFwd = math.floor(2.0 * fBB.ZLength) + 10.0
            ef = F.extrude(AuraCAD.Vector(0.0, 0.0, extFwd))
            ...
            cutBox.translate(AuraCAD.Vector(efBB.XMin - 1.0, efBB.YMin - 1.0, zHght))
            base = ef.cut(cutBox)
            ...
            if cont:
                topFace.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - zMin))
                self.topFaces.append((topFace, auracadIdx))

    def _fuseTopFaces(self):
        one, baseauracadIdx = self.topFaces.pop(0)
        base = one
        for face, auracadIdx in self.topFaces:
            base = base.fuse(face)
        self.topFaces.insert(0, (one, baseauracadIdx))
        ...
        if numLoops > 0:
            for li in range(0, numLoops):
                ...
                if wire.isClosed():
                    ...
                    extWire = wire.extrude(AuraCAD.Vector(0.0, 0.0, 2.0))
    ...
    def _identifyInternalFeatures(self):
        ...
        for top, auracadIdx in self.topFaces:
    ...
    def getUnifiedRegions(self):
        ...
        if lenFaces == 1:
            ...
            if wCS:
                ...
                faceShp, auracadIdx = self.FACES[0]
                msg = translate(
                    "PathSurfaceSupport",
        ...
        if self.noSharedEdges:
            ...
            allTopFaces = []
            for topFace, auracadIdx in self.topFaces:
...
class OCL_Tool:
    """The OCL_Tool class is designed to translate a AuraCAD standard ToolBit shape
    ...
    """
        #### AuraCAD Legacy tool shape properties per tool type
        shape = EndMill
        Diameter
        CuttingEdgeHeight
        LengthOffset
        ...
        #### AuraCAD packaged ToolBit named constraints per shape files
        shape = endmill
    ...
    def getOclTool(self):
        ...
        if self.toolMethod == "None":
            ...
            AuraCAD.Console.PrintError(err + "\n")
            return False
        else:
...
def makeExtendedBoundBox(wBB, bbBfr, zDep):
    ...
    p1 = AuraCAD.Vector(wBB.XMin - bbBfr, wBB.YMin - bbBfr, zDep)
    p2 = AuraCAD.Vector(wBB.XMax + bbBfr, wBB.YMin - bbBfr, zDep)
    p3 = AuraCAD.Vector(wBB.XMax + bbBfr, wBB.YMax + bbBfr, zDep)
    p4 = AuraCAD.Vector(wBB.XMin - bbBfr, wBB.YMax + bbBfr, zDep)
