...
class ObjectProfile(PathAreaOp.ObjectOp):
    ...
    def initAreaOpProperties(self, obj, warn=False):
        ...
        if len(self.addNewProps) > 0:
            ...
            if warn:
                ...
                AuraCAD.Console.PrintWarning(newPropMsg)
    ...
    def setOpEditorProperties(self, obj):
        ...
        obj.setEditorMode("HandleMultipleFeatures", auracad)
        obj.setEditorMode("processCircles", auracad)
        obj.setEditorMode("processHoles", auracad)
        obj.setEditorMode("processPerimeter", auracad)
    ...
    def areaOpShapes(self, obj):
        ...
        if self.isDebug:
            for grpNm in ["tmpDebugGrp", "tmpDebugGrp001"]:
                if hasattr(AuraCAD.ActiveDocument, grpNm):
                    for go in AuraCAD.ActiveDocument.getObject(grpNm).Group:
                        AuraCAD.ActiveDocument.removeObject(go.Name)
                    AuraCAD.ActiveDocument.removeObject(grpNm)
            self.tmpGrp = AuraCAD.ActiveDocument.addObject(
        ...
        if self.isDebug:
            if AuraCAD.GuiUp:
                ...
                AuraCADGui.ActiveDocument.getObject(tmpGrpNm).Visibility = False
                ...
                if wire.isClosed():
                    ...
                    if self.JOB.GeometryTolerance.Value == 0.0:
                        ...
                        if flattened:
                            ...
                            flatWire.translate(AuraCAD.Vector(0, 0, diffDepth))
    ...
    def _getOpenProfileDiffDepth(self, base, edges):
        ...
        diffDepth = 2 * tol
        offset = AuraCAD.Vector(0, 0, diffDepth)
    ...
    def _flattenWire(self, obj, wire, trgtDep):
        ...
        if not Path.Geom.isRoughly(wBB.ZLength, 0):
            ...
            mbbEXT = wire.extrude(AuraCAD.Vector(0, 0, extFwdLen))
            ...
            srtWire.translate(AuraCAD.Vector(0, 0, trgtDep - srtWire.BoundBox.ZMin))
    ...
    def _getCutAreaCrossSection(self, obj, base, origWire, flatWire):
        ...
        pb = AuraCAD.Vector(Vb.X, Vb.Y, fdv)
        pe = AuraCAD.Vector(Ve.X, Ve.Y, fdv)
        ...
        if blen > 0.1:
            ...
            bcp = AuraCAD.Vector(begE.Vertexes[1].X, begE.Vertexes[1].Y, fdv)
        if elen > 0.1:
            ecp = endE.valueAt(
                endE.getParameterByLength(elen - 0.1)
            )  # point returned 0.1 mm along edge
        else:
            ecp = AuraCAD.Vector(endE.Vertexes[1].X, endE.Vertexes[1].Y, fdv)
        ...
        extBndboxEXT = extBndbox.extrude(AuraCAD.Vector(0, 0, 1))
        ...
        for f in range(0, len(cutArea.Faces)):
            auracadBB = cutArea.Faces[f].BoundBox
            if abs(auracadBB.ZMax - bbZMax) < tolerance and abs(auracadBB.ZMin - bbZMax) < tolerance:
                topauracad.append(f)
            if abs(auracadBB.ZMax - bbZMin) < tolerance and abs(auracadBB.ZMin - bbZMin) < tolerance:
                botauracad.append(f)
        if len(topauracad) == 0:
            Path.Log.error("Failed to identify top faces of cut area.")
            return False
        topComp = Part.makeCompound([cutArea.Faces[f] for f in topauracad])
        topComp.translate(
            AuraCAD.Vector(0, 0, fdv - topComp.BoundBox.ZMin)
        )  # Translate face to final depth
        if len(botauracad) > 1:
            # Path.Log.debug('len(botauracad) > 1')
            ...
            botComp = Part.makeCompound(
                [cutArea.Faces[f] for f in botauracad]
            )  # Part.makeCompound([CA.Shape.Faces[f] for f in botauracad])
        botComp.translate(
            AuraCAD.Vector(0, 0, fdv - botComp.BoundBox.ZMin)
        )  # Translate face to final depth

        # Make common of the two
        comauracad = topComp.common(botComp)

        # Determine with which set of intersection tags the model intersects
        cmnIntArea, cmnExtArea = self._checkTagIntersection(iTAG, eTAG, "QRY", comauracad)
        ...
        pathStops.translate(AuraCAD.Vector(0, 0, fdv - pathStops.BoundBox.ZMin))
        ...
        if lenWS < 3:
            ...
            auracadShp = Part.Face(nWire)
            auracadShp.translate(AuraCAD.Vector(0, 0, fdv - workShp.BoundBox.ZMin))
        # Eif
        ...
        if wi > 0:  # and isInterior is False:
            ...
            testArea = auracadShp.cut(base.Shape)
            ...
            if isReady is False:
                ...
                pauracadShp = Part.Face(pWire)
                pauracadShp.translate(AuraCAD.Vector(0, 0, fdv - workShp.BoundBox.ZMin))
                workShp = pauracadShp.cut(auracadShp)
    ...
    def _extractPathWire(self, obj, base, flatWire, cutShp):
        ...
        cent0 = AuraCAD.Vector(frstVrt.X, frstVrt.Y, fdv)
        cent1 = AuraCAD.Vector(lstVrt.X, lstVrt.Y, fdv)
        ...
        if numOSWires > 1:
            ...
            for T in self.cutSideTags.Faces:
                tcom = T.CenterOfMass
                tv = AuraCAD.Vector(tcom.x, tcom.y, 0.0)
    ...
    def _getOffsetArea(self, obj, auracadShape, isHole):
        ...
        return PathUtils.getOffsetArea(auracadShape, offset, plane=auracadShape, tolerance=tolerance)

    def _findNearestVertex(self, shape, point):
        Path.Log.debug("_findNearestVertex()")
        PT = AuraCAD.Vector(point.x, point.y, 0.0)
    ...
    def _separateWireAtVertexes(self, wire, VV1, VV2):
        ...
        grps = [[], []]
        wireIdxs = [[], []]
        V1 = AuraCAD.Vector(VV1.X, VV1.Y, VV1.Z)
        V2 = AuraCAD.Vector(VV2.X, VV2.Y, VV2.Z)
        ...
        for e in range(0, edgeCount):
            v = 0
            E = wire.Edges[e]
            fv0 = AuraCAD.Vector(E.Vertexes[0].X, E.Vertexes[0].Y, E.Vertexes[0].Z)
            fv1 = AuraCAD.Vector(E.Vertexes[1].X, E.Vertexes[1].Y, E.Vertexes[1].Z)
    ...
    def _makeCrossSection(self, shape, sliceZ, zHghtTrgt=False):
        ...
        # Create cross-section of shape and translate
        wires = []
        slcs = shape.slice(AuraCAD.Vector(0, 0, 1), sliceZ)
        if len(slcs) > 0:
            for i in slcs:
                wires.append(i)
            comp = Part.Compound(wires)
            if zHghtTrgt is not False:
                comp.translate(AuraCAD.Vector(0, 0, zHghtTrgt - comp.BoundBox.ZMin))
            return comp
    ...
    def _makeOffsetCircleTag(self, p1, p2, cutterRad, depth, lbl, reverse=False):
        ...
        pb = AuraCAD.Vector(p1.x, p1.y, 0.0)
        pe = AuraCAD.Vector(p2.x, p2.y, 0.0)
        ...
        perpE = AuraCAD.Vector(-1 * toMid.y, toMid.x, 0.0).multiply(-1 * cutFactor)  # exterior tag
        extPnt = pb.add(toMid.add(perpE))

        # make exterior tag
        eCntr = extPnt.add(AuraCAD.Vector(0, 0, depth))
        ecw = Part.Wire(Part.makeCircle((cutterRad / 2), eCntr).Edges[0])
        extTag = Part.Face(ecw)

        # make interior tag
        perpI = AuraCAD.Vector(-1 * toMid.y, toMid.x, 0.0).multiply(cutFactor)  # interior tag
        intPnt = pb.add(toMid.add(perpI))
        iCntr = intPnt.add(AuraCAD.Vector(0, 0, depth))
    ...
    def _makeStop(self, sType, pA, pB, lbl):
        ...
        E = AuraCAD.Vector(pB.x, pB.y, 0)  # endpoint
        C = AuraCAD.Vector(pA.x, pA.y, 0)  # checkpoint
    ...
    def _makePerp2DVector(self, v1, v2, dist):
        p1 = AuraCAD.Vector(v1.x, v1.y, 0.0)
        p2 = AuraCAD.Vector(v2.x, v2.y, 0.0)
        toEnd = p2.sub(p1)
        factor = dist / toEnd.Length
        perp = AuraCAD.Vector(-1 * toEnd.y, toEnd.x, 0.0).multiply(factor)
        return p1.add(toEnd.add(perp))

    # Method to add temporary debug object
    def _addDebugObject(self, objName, objShape):
        if self.isDebug:
            newDocObj = AuraCAD.ActiveDocument.addObject("Part::Feature", "tmp_" + objName)
...
def Create(name, obj=None, parentJob=None):
    ...
    if obj is None:
        obj = AuraCAD.ActiveDocument.addObject("Path::FeaturePython", name)
