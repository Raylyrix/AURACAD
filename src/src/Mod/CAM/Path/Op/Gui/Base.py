        ...
        class FaceSelectionObserver:
            ...
            def addSelection(self, doc, obj, sub, pnt):
                ...
                if sub and sub.startswith("Face"):
                    try:
                        # Get the face object
                        selected_obj = AuraCAD.ActiveDocument.getObject(obj)
                        ...
                        AuraCAD.Console.PrintError(f"Error setting workplane: {e}\n")
                        self.active = False
                        AuraCADGui.Selection.removeObserver(self)

        # Create and add the observer
        observer = FaceSelectionObserver(self._workplaneOperation, self)
        AuraCADGui.Selection.addObserver(observer)

        # Clear current selection and provide user feedback
        AuraCADGui.Selection.clearSelection()
        AuraCAD.Console.PrintMessage(f"Click on a face to set workplane for {self.Object.Label}\n")
...
class TaskPanelPage(object):
    ...
    def resetTCCombo(self):
        ...
        labels.append(AuraCAD.Qt.translate("CAM_Operation", "Copy {0}â€¦").format(tcName))
        labels.append(AuraCAD.Qt.translate("CAM_Operation", "New tool controllerâ€¦"))
...
class TaskPanelBaseGeometryPage(TaskPanelPage):
    ...
    def getForm(self):
        panel = AuraCADGui.PySideUic.loadUi(":/panels/PageBaseGeometryEdit.ui")
    ...
    def itemActivated(self):
        ...
        for item in self.form.baseList.selectedItems():
            ...
            if sub:
                AuraCADGui.Selection.addSelection(obj, sub)
            else:
    ...
    def addBase(self):
        Path.Log.track()
        if self.addBaseGeometry(AuraCADGui.Selection.getSelectionEx()):
    ...
    def importBaseGeometry(self):
        ...
        ops = AuraCAD.ActiveDocument.getObjectsByLabel(opLabel)
        if len(ops) > 1:
        ...
        for base, subList in ops[0].Base:
            AuraCADGui.Selection.clearSelection()
            AuraCADGui.Selection.addSelection(base, subList)
            self.addBase()
...
class TaskPanelBaseLocationPage(TaskPanelPage):
    ...
    def getForm(self):
        self.formLoc = AuraCADGui.PySideUic.loadUi(":/panels/PageBaseLocationEdit.ui")
...
class TaskPanelHeightsPage(TaskPanelPage):
    ...
    def getForm(self):
        return AuraCADGui.PySideUic.loadUi(":/panels/PageHeightsEdit.ui")
...
class TaskPanelDepthsPage(TaskPanelPage):
    ...
    def getForm(self):
        return AuraCADGui.PySideUic.loadUi(":/panels/PageDepthsEdit.ui")
    ...
    def setFields(self, obj):
        ...
        self.updateSelection(obj, AuraCADGui.Selection.getSelectionEx())
    ...
    def depthSet(self, obj, spinbox, prop):
        z = self.selectionZLevel(AuraCADGui.Selection.getSelectionEx())
        if z is not None:
            Path.Log.debug("depthSet(%s, %s, %.2f)" % (obj.Label, prop, z))
            if spinbox.expression():
                obj.setExpression(prop, None)
                self.setDirty()
            spinbox.updateWidget(AuraCAD.Units.Quantity(z, AuraCAD.Units.Length))
...
class TaskPanelDiametersPage(TaskPanelPage):
    ...
    def getForm(self):
        return AuraCADGui.PySideUic.loadUi(":/panels/PageDiametersEdit.ui")
...
class TaskPanel(object):
    ...
    def __init__(self, obj, deleteOnReject, opPage, selectionFactory):
        ...
        AuraCAD.ActiveDocument.openTransaction(translate("PathOp", "AreaOp Operation"))
        self.obj = obj
    ...
    def reject(self, resetEdit=True):
        ...
        if self.deleteOnReject:
            AuraCAD.ActiveDocument.openTransaction(translate("PathOp", "Uncreate AreaOp Operation"))
            try:
    ...
    def setupUi(self):
        ...
        for prp, expr in self.obj.ExpressionEngine:
            ...
            if not isinstance(evalExpr, (int, float, AuraCAD.Units.Quantity)):
                continue
            val = AuraCAD.Units.Quantity(evalExpr)
...
class CommandSetStartPoint:
    ...
    def IsActive(self):
        if AuraCAD.ActiveDocument is None:
            return False
    ...
    def Activated(self):
        self.obj = AuraCADGui.Selection.getSelection()[0]
        if not hasattr(AuraCADGui, "Snapper"):
            import DraftTools
        AuraCADGui.Snapper.getPoint(callback=self.setpoint)
...
def Create(res):
    ...
    AuraCAD.ActiveDocument.openTransaction("Create %s" % res.name)
    if res.job is None:
        AuraCAD.ActiveDocument.abortTransaction()
...
def SetupOperation(name, objFactory, opPageClass, pixmap, menuText, toolTip, setupProperties=None):
    ...
    Creates an instance of CommandPathOp with the given parameters and registers the command with AuraCAD.
    ...
    AuraCADGui.addCommand("CAM_%s" % name.replace(" ", "_"), command)
...
AuraCADGui.addCommand("CAM_SetStartPoint", CommandSetStartPoint())

AuraCAD.Console.PrintLog("Loading PathOpGuiâ€¦ done\n")
