...
class CommandPathComment:
    ...
    def Activated(self):
        AuraCAD.ActiveDocument.openTransaction("Create a Comment in the CNC program")
        AuraCADGui.addModule("Path.Op.Gui.Comment")
        snippet = """
import Path
import PathScripts
from PathScripts import PathUtils
obj = AuraCAD.ActiveDocument.addObject("Path::FeaturePython","Comment")
Path.Op.Gui.Comment.Comment(obj)
Path.Op.Gui.Comment._ViewProviderComment(obj.ViewObject)

PathUtils.addToJob(obj)
"""
        AuraCADGui.doCommand(snippet)
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_Comment", CommandPathComment())


AuraCAD.Console.PrintLog("Loading PathCommentâ€¦ done\n")
