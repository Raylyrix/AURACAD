...
class CommandPathSimpleCopy:
    ...
    def Activated(self):
        ...
        AuraCAD.ActiveDocument.openTransaction("Simple Copy")
        AuraCADGui.doCommand("selection = AuraCADGui.Selection.getSelection()")
        ...
        AuraCADGui.doCommand("job = PathScripts.PathUtils.findParentJob(selection[0])")
        AuraCADGui.doCommand(
            "paths = [PathScripts.PathUtils.getPathWithPlacement(sel) for sel in selection]"
        )
        AuraCADGui.addModule("Path.Op.Custom")
        AuraCADGui.doCommand("obj = Path.Op.Custom.Create(name, parentJob=job)")
        AuraCADGui.doCommand(
            "obj.ToolController = Path.Base.Util.toolControllerForOp(selection[0])"
        )
        AuraCADGui.doCommand("obj.CoolantMode = Path.Base.Util.coolantModeForOp(selection[0])")
        AuraCADGui.doCommand(
            "obj.ViewObject.Proxy = Path.Op.Gui.SimpleCopy.ViewProvider(obj.ViewObject)"
        )
        AuraCADGui.doCommand("obj.Gcode = [c.toGCode() for path in paths for c in path.Commands]")
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_SimpleCopy", CommandPathSimpleCopy())
