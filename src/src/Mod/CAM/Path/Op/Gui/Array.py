...
__doc__ = """CAM Array object and AuraCAD command"""
...
class PathArray:
    ...
    def getPolarArray(self, commands):
        ...
        for i in range(self.copies):
            ...
            pl = AuraCAD.Placement()
            pl.rotate(self.polarCentre, AuraCAD.Vector(0, 0, 1), ang)
...
class CommandPathArray:
    ...
    def Activated(self):
        ...
        # undo/redo stack
        AuraCAD.ActiveDocument.openTransaction("Create Array")
        AuraCADGui.addModule("Path.Op.Gui.Array")
        AuraCADGui.addModule("PathScripts.PathUtils")

        AuraCADGui.doCommand(
            'obj = AuraCAD.ActiveDocument.addObject("Path::FeaturePython","Array")'
        )

        AuraCADGui.doCommand("Path.Op.Gui.Array.ObjectArray(obj)")

        baseString = "[%s]" % ",".join(
            ["AuraCAD.ActiveDocument.%s" % sel.Name for sel in selection]
        )
        AuraCADGui.doCommand("obj.Base = %s" % baseString)
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_Array", CommandPathArray())
