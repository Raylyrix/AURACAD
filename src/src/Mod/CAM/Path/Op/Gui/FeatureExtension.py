...
class _Extension(object):
    ...
    def createExtensionSoSwitch(self, ext):
        ...
        if numVert:
            ...
            auracade.numVertices.setValues(tuple(numVert))

        sep.addChild(pos)
        sep.addChild(mat)
        sep.addChild(hnt)
        sep.addChild(crd)
        sep.addChild(auracade)
...
class TaskPanelExtensionPage(PathOpGui.TaskPanelPage):
    ...
    def getForm(self):
        form = AuraCADGui.PySideUic.loadUi(":/panels/PageOpPocketExtEdit.ui")
        return form
    ...
    def restoreSelection(self, selection):
        ...
        if 0 == self.model.rowCount():
            ...
            def setSelectionVisuals(item, ext):
                ...
                if selected:
                    AuraCADGui.Selection.addSelection(ext.base, ext.face)
                    ext.select()
                else:
                    ext.deselect()
...
AuraCAD.Console.PrintLog("Loading PathFeatureExtensionsGui... done\n")
