...
def contourselect():
    AuraCADGui.Selection.addSelectionGate(CONTOURGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Contour Select Mode\n")


def eselect():
    AuraCADGui.Selection.addSelectionGate(EGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Edge Select Mode\n")


def drillselect():
    AuraCADGui.Selection.addSelectionGate(DRILLGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Drilling Select Mode\n")


def tapselect():
    AuraCADGui.Selection.addSelectionGate(TAPGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Tapping Select Mode\n")


def engraveselect():
    AuraCADGui.Selection.addSelectionGate(ENGRAVEGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Engraving Select Mode\n")


def fselect():
    AuraCADGui.Selection.addSelectionGate(FACEGate())  # Was PROFILEGate()
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Profiling Select Mode\n")


def chamferselect():
    AuraCADGui.Selection.addSelectionGate(CHAMFERGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Deburr Select Mode\n")


def profileselect():
    AuraCADGui.Selection.addSelectionGate(PROFILEGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Profiling Select Mode\n")


def pocketselect():
    AuraCADGui.Selection.addSelectionGate(POCKETGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Pocketing Select Mode\n")


def adaptiveselect():
    AuraCADGui.Selection.addSelectionGate(ADAPTIVEGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Adaptive Select Mode\n")


def slotselect():
    AuraCADGui.Selection.addSelectionGate(ALLGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Slot Cutter Select Mode\n")
...
def surfaceselect():
    ...
    if MESHGate() or FACEGate():
        gate = True
    AuraCADGui.Selection.addSelectionGate(gate)
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Surfacing Select Mode\n")


def vcarveselect():
    AuraCADGui.Selection.addSelectionGate(VCARVEGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Vcarve Select Mode\n")


def probeselect():
    AuraCADGui.Selection.addSelectionGate(PROBEGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Probe Select Mode\n")


def customselect():
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Custom Select Mode\n")


def turnselect():
    AuraCADGui.Selection.addSelectionGate(TURNGate())
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Turning Select Mode\n")
...
def clear():
    AuraCADGui.Selection.removeSelectionGate()
    if not Path.Preferences.suppressSelectionModeWarning():
        AuraCAD.Console.PrintWarning("Free Select\n")
