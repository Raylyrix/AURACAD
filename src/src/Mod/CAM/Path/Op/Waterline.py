...
class ObjectWaterline(PathOp.ObjectOp):
    ...
    def opPropertyDefaults(self, obj, job):
        ...
        defaults = {
            ...
            "StartPoint": AuraCAD.Vector(0.0, 0.0, obj.ClearanceHeight.Value),
            ...
            "AvoidLastX_Faces": 0,
            "PatternCenterCustom": AuraCAD.Vector(0.0, 0.0, 0.0),
            "GapThreshold": 0.005,
    ...
    def opExecute(self, obj):
        ...
        auracadAD = AuraCAD.ActiveDocument
        ...
        if auracadAD.getObject(tempGroupName):
            for to in auracadAD.getObject(tempGroupName).Group:
                auracadAD.removeObject(to.Name)
            auracadAD.removeObject(tempGroupName)  # remove temp directory if already exists
        if auracadAD.getObject(tempGroupName + "001"):
            for to in auracadAD.getObject(tempGroupName + "001").Group:
                auracadAD.removeObject(to.Name)
            auracadAD.removeObject(tempGroupName + "001")  # remove temp directory if already exists
        tempGroup = auracadAD.addObject("App::DocumentObjectGroup", tempGroupName)
        ...
        if AuraCAD.GuiUp:
            for m in range(0, len(JOB.Model.Group)):
                ...
                modelVisibility.append(AuraCADGui.ActiveDocument.getObject(mNm).Visibility)
        ...
        if AuraCAD.GuiUp:
            AuraCADGui.ActiveDocument.getObject(tempGroupName).Visibility = False
            for m in range(0, len(JOB.Model.Group)):
                M = JOB.Model.Group[m]
        ...
        if deleteTempsFlag is True:
            for to in tempGroup.Group:
                if hasattr(to, "Group"):
                    for go in to.Group:
                        auracadAD.removeObject(go.Name)
                auracadAD.removeObject(to.Name)
            auracadAD.removeObject(tempGroupName)
        else:
    ...
    def _processWaterlineAreas(self, JOB, obj, mdlIdx, auracadS, VDS):
        """_processWaterlineAreas(JOB, obj, mdlIdx, auracadS, VDS)...
        ...
        if obj.HandleMultipleFeatures == "Collectively":
            if auracadS is True:
                ...
                ADD = Part.makeCompound(auracadS)
                if VDS is not False:
            ...
            for fsi in range(0, len(auracadS)):
                fShp = auracadS[fsi]
            ...
            for STEP in PNTSET:
                for SEG in STEP:
                    if SEG == "BRK":
                        ...
                        P1 = AuraCAD.Vector(A[0], A[1], csHght)
                        P2 = AuraCAD.Vector(B[0], B[1], csHght)
                        stpOvr.append((P1, P2))
                SCANS.append(stpOvr)
                stpOvr = list()
        elif cutPattern in ["Circular", "CircularZigZag"]:
            ...
            for so in range(0, len(PNTSET)):
                ...
                for a in range(0, len(ARCS)):
                    ...
                    if Arc == "BRK":
                        ...
                        sp, ep, cp = Arc
                        S = AuraCAD.Vector(sp[0], sp[1], csHght)
                        E = AuraCAD.Vector(ep[0], ep[1], csHght)
                        C = AuraCAD.Vector(cp[0], cp[1], csHght)
                        scan = (S, E, C, cMode)
                        if scan is False:
                            erFlg = True
                        else:
    ...
    def _oclWaterlineOp(self, JOB, obj, mdlIdx, subShp=None):
        ...
        if self.holdPoint is None:
            self.holdPoint = AuraCAD.Vector(0.0, 0.0, 0.0)
        if self.layerEndPnt is None:
            self.layerEndPnt = AuraCAD.Vector(0.0, 0.0, 0.0)
        ...
        if obj.Algorithm == "OCL Adaptive":
            ...
            oclScan = [AuraCAD.Vector(P.x, P.y, P.z + depOfst) for P in oclScan]
    ...
    def _waterlineAdaptiveScan(self, stl, smplInt, minSmplInt, zheights, depOfst):
        ...
        for zh in zheights:
            ...
            for loop in temp_loops:
                # Convert OCL Points to AuraCAD Vectors and apply Z offset
                AuraCAD_loop = [AuraCAD.Vector(P.x, P.y, P.z + depOfst) for P in loop]
                adapt_loops.append(AuraCAD_loop)
    ...
    def _loopToGcode(self, obj, layDep, loop):
        ...
        nxt = AuraCAD.Vector(0.0, 0.0, 0.0)
        ...
        if obj.Algorithm == "OCL Adaptive":
            if obj.CutMode == "Climb":
                ...
                loop.reverse()
            pnt = pnt1 = AuraCAD.Vector(loop[0].x, loop[0].y, loop[0].z)
        else:
            pnt = AuraCAD.Vector(loop[0].x, loop[0].y, layDep)
    ...
    def _experimentalWaterlineOp(self, JOB, obj, mdlIdx, subShp=None):
        ...
        for ca in range(0, caLen):
            ...
            if area.Area > 0.0:
                ...
                data = AuraCAD.Units.Quantity(csHght, AuraCAD.Units.Length).UserString
                Path.Log.debug("Cut area at {} is zero.".format(data))

            # get offset wire(s) based upon cross-section cut area
            if cont:
                area.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - area.BoundBox.ZMin))
            ...
            if cont:
                ...
                clearArea.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - clearArea.BoundBox.ZMin))
        ...
        if clearLastLayer and obj.ClearLastLayer != "Off":
            ...
            lastClearArea.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - lastClearArea.BoundBox.ZMin))
            if clrLyr == "Offset":
        ...
        for dp in range(0, lenDP):
            ...
            if csFaces:
                ...
                if useFaces:
                    ...
                    cutArea.translate(AuraCAD.Vector(0.0, 0.0, csHght - cutArea.BoundBox.ZMin))
    ...
    def _wiresToWaterlinePath(self, obj, ofstPlnrShp, csHght):
        ...
        ofstPlnrShp.translate(AuraCAD.Vector(0.0, 0.0, csHght - ofstPlnrShp.BoundBox.ZMin))
        ...
        for w in range(start, len(ofstPlnrShp.Wires)):
            ...
            if obj.CutMode == "Climb":
                lv = len(V) - 1
                startVect = AuraCAD.Vector(V[lv].X, V[lv].Y, V[lv].Z)
            else:
                startVect = AuraCAD.Vector(V[0].X, V[0].Y, V[0].Z)
    ...
    def _makeCutPatternLayerPaths(self, JOB, obj, clrAreaShp, csHght, cutPattern):
        ...
        clrAreaShp.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - clrAreaShp.BoundBox.ZMin))
        ...
        if cutPattern == "Offset":
            ...
            if not pathGeom:
                ...
                return commands
            pathGeom.translate(AuraCAD.Vector(0.0, 0.0, csHght - pathGeom.BoundBox.ZMin))
    ...
    def _getModelCrossSection(self, shape, csHght):
        ...
        for i in shape.slice(AuraCAD.Vector(0, 0, 1), csHght):
            wires.append(i)
        ...
        if len(wires) > 0:
            ...
            auracadS = list()
            for w in wires:
                w.translate(AuraCAD.Vector(0.0, 0.0, 0.0 - w.BoundBox.ZMin))
                auracadS.append(Part.Face(w))
            auracadS.sort(key=byArea, reverse=True)
            return auracadS
        else:
    ...
    def _makeExtendedBoundBox(self, wBB, bbBfr, zDep):
        ...
        pl.Rotation = AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), 0)
        pl.Base = AuraCAD.Vector(0, 0, 0)
    ...
    def showDebugObject(self, objShape, objName):
        if self.showDebugObjects:
            do = AuraCAD.ActiveDocument.addObject("Part::Feature", "tmp_" + objName)
            do.Shape = objShape
            do.purgeTouched()
