...
def initSceneGraph(z=10):
    ...
    sceneGraph = AuraCADGui.ActiveDocument.ActiveView.getSceneGraph()
    topZ = z
    return True
...
def Execute(op, obj):
    ...
    try:
        ...
        outer_wire = TechDraw.findShapeOutline(op.stock.Shape, 1, AuraCAD.Vector(0, 0, 1))
...
def _getSolidProjection(shp, z):
    ...
    bbCutTop = Part.makeBox(
        ...
        AuraCAD.Vector(bb.XMin, bb.YMin, z),
    )
            ...
            if sub.startswith("Face"):
                if sub not in avoidFeatures:
                    if obj.UseOutline:
                        ...
                        # wire_A = TechDraw.findShapeOutline(face, 1, AuraCAD.Vector(0.0, 0.0, 1.0))
