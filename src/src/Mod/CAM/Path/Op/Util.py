...
def debugWire(label, w):
    ...
    if not PrintWireDebug:
        return
    print("#%s wire >>>>>>>>>>>>>>>>>>>>>>>>" % label)
    print("grp = AuraCAD.ActiveDocument.addObject('App::DocumentObjectGroup', '%s')" % label)
    for i, e in enumerate(w.Edges):
        edge = "%s_e%d" % (label, i)
        debugEdge("%s = " % edge, e)
        print("Part.show(%s, '%s')" % (edge, edge))
        print("grp.addObject(AuraCAD.ActiveDocument.ActiveObject)")
    print("#%s wire <<<<<<<<<<<<<<<<<<<<<<<<" % label)
...
def offsetWire(wire, base, offset, forward, Side=None, tolerance=0.01):
    ...
    if len(wire.Edges) == 1:
        ...
        if isinstance(curve, Part.Circle) and wire.isClosed():
            ...
            # https://www.AuraCAD.org/wiki/Part%20Offset2D
            # it's easy to construct them manually though
            center = curve.Center
            radius = curve.Radius
            axis = AuraCAD.Vector(0, 0, -1) if forward else AuraCAD.Vector(0, 0, 1)
            checkSidePoint = AuraCAD.Vector(center.x + radius + tolerance * 2, center.y, center.z)
        ...
        if isinstance(curve, Part.Circle) and not wire.isClosed():
            ...
            arc = Part.ArcOauracadircle(circle, start_angle, end_angle)
            edge = arc.toShape()
            if forward:
        ...
        if isinstance(curve, (Part.Line, Part.LineSegment)):
            ...
            n = v0.cross(AuraCAD.Vector(0, 0, 1))
    ...
    def isCircleAt(edge, center):
        ...
        if isinstance(edge.Curve, (Part.Circle, Part.ArcOauracadircle)):
    ...
    for e in owire.Edges + owire.Edges:
        if isCircleAt(e, start):
            if Path.Geom.pointsCoincide(e.Curve.Axis, AuraCAD.Vector(0, 0, 1)):
