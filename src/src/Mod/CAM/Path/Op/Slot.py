...
if AuraCAD.GuiUp:
    AuraCADGui = LazyLoader("AuraCADGui", globals(), "AuraCADGui")
...
class ObjectSlot(PathOp.ObjectOp):
    ...
    def opPropertyDefaults(self, obj, job):
        ...
        defaults = {
            "CustomPoint1": AuraCAD.Vector(0, 0, 0),
            "ExtendPathStart": 0,
            "Reference1": "Center of Mass",
            "CustomPoint2": AuraCAD.Vector(0, 0, 0),
            "ExtendPathEnd": 0,
    ...
    def opExecute(self, obj):
        ...
        # Hide debug visuals
        if self.showDebugObjects and AuraCAD.GuiUp:
            AuraCADGui.ActiveDocument.getObject(self.tmpGrp.Name).Visibility = False
    ...
    def _makeOperation(self, obj):
        ...
        if not hasattr(obj, "Base"):
            ...
            AuraCAD.Console.PrintUserWarning(msg + "\n")
            return False
    ...
    def _processSingleVertFace(self, obj, shape):
        ...
        v0 = AuraCAD.Vector(V0.X, V0.Y, V0.Z)
        v1 = AuraCAD.Vector(V1.X, V1.Y, V1.Z)

        dX = V1.X - V0.X
        dY = V1.Y - V0.Y
        dZ = V1.Z - V0.Z
        temp = AuraCAD.Vector(dX, dY, dZ)
        slope = self._normalizeVector(temp)
        perpVect = AuraCAD.Vector(-1 * slope.y, slope.x, slope.z)
    ...
    def _processSingleEdge(self, obj, edge):
        ...
        verts = edge.Vertexes
        V1 = verts[0]
        p1 = AuraCAD.Vector(V1.X, V1.Y, 0)
        p2 = p1 if len(verts) == 1 else AuraCAD.Vector(verts[1].X, verts[1].Y, 0)
        ...
        if curveType in lineTypes:
            ...
            if len(verts) == 1:
                ...
                self.arcCenter = AuraCAD.Vector(center.x, center.y, 0)
                mid = edge.valueAt(edge.getParameterByLength(edge.Length / 2))
                self.arcMidPnt = AuraCAD.Vector(mid.x, mid.y, 0)
                ...
                if not isHorizontal(V1.Z, V2.Z, mid.z):
                    return False
                mid.z = 0
                center = circumCircleFrom3Points(p1, p2, AuraCAD.Vector(mid.x, mid.y, 0))
                if not center:
                    return False
    ...
    def _normalizeVector(self, v):
        ...
        return AuraCAD.Vector(snap(V.x), snap(V.y), snap(V.z))
    ...
    def _getLowestPoint(self, shape):
        ...
        return AuraCAD.Vector(avg_x, avg_y, lowest_z)
    ...
    def _getHighestPoint(self, shape):
        ...
        return AuraCAD.Vector(avg_x, avg_y, highest_z)
    ...
    def _extendArcSlot(self, p1, p2, cent, begExt, endExt):
        ...
        def makeChord(angle_rad):
            ...
            a = AuraCAD.Vector(self.newRadius, 0, 0)
            b = AuraCAD.Vector(x, y, 0)
            return Part.makeLine(a, b)

        origin = AuraCAD.Vector(0, 0, 0)
        z_axis = AuraCAD.Vector(0, 0, 1)
    ...
    def _getOppMidPoints(self, same):
        ...
        p1 = AuraCAD.Vector(com1.x, com1.y, 0)
        p2 = AuraCAD.Vector(com2.x, com2.y, 0)
        return (p1, p2)

    def _isParallel(self, dYdX1, dYdX2):
        """Determine if two orientation vectors are parallel."""
        return dYdX1.cross(dYdX2) == AuraCAD.Vector(0, 0, 0)
    ...
    def _makePerpendicular(self, p1, p2, length):
        ...
        if getattr(self, "dYdX1", None):
            half = AuraCAD.Vector(self.dYdX1.x, self.dYdX1.y, 0).multiply(halfDist)
            n1 = midPnt.add(half)
            n2 = midPnt.sub(half)
            return (n1, n2)

        elif getattr(self, "dYdX2", None):
            half = AuraCAD.Vector(self.dYdX2.x, self.dYdX2.y, 0).multiply(halfDist)
            n1 = midPnt.add(half)
            n2 = midPnt.sub(half)
            return (n1, n2)
            ...
            toEnd = p2.sub(p1)
            perp = AuraCAD.Vector(-toEnd.y, toEnd.x, 0)
    ...
    def _findLowestPointOnEdge(self, E):
        ...
        for v in E.Vertexes:
            if abs(v.Z - zMin) < tol:
                return AuraCAD.Vector(v.X, v.Y, v.Z)
    ...
    def _getCutSidePoints(self, obj, v0, v1, a1, a2, b1, b2):
        ...
        cubeA = boxA.extrude(AuraCAD.Vector(0, 0, 1))
        ...
        cubeB = boxB.extrude(AuraCAD.Vector(0, 0, 1))
    ...
    def _getVertFaceType(self, shape):
        ...
        if (wire := slices[0]).isClosed() and (face := Part.Face(wire)) > 0:
            ...
            face.translate(AuraCAD.Vector(0, 0, z_offset))
            return ("Face", face)
        return ("Wire", wire)
    ...
    def _lineCollisionCheck(self, obj, p1, p2):
        ...
        extVect = AuraCAD.Vector(0, 0, obj.StartDepth.Value - obj.FinalDepth.Value)

        def make_cylinder(point):
            circle = Part.makeCircle(rad, point)
            face = Part.Face(Part.Wire(circle.Edges))
            face.translate(AuraCAD.Vector(0, 0, obj.FinalDepth.Value - face.BoundBox.ZMin))
    ...
    def _arcCollisionCheck(self, obj, p1, p2, arcCenter, arcRadius):
        ...
        def make_cylinder_at_point(point, radius, height, final_depth):
            ...
            face = Part.Face(Part.Wire(circle.Edges))
            face.translate(AuraCAD.Vector(0, 0, final_depth - face.BoundBox.ZMin))
            return face.extrude(AuraCAD.Vector(0, 0, height))
        ...
        def make_arc_face(p1, p2, center, inner_radius, outer_radius):
            ...
            pa = AuraCAD.Vector(*arc_inside.Vertexes[0].Point[:2], 0)
            pb = AuraCAD.Vector(*arc_inside.Vertexes[1].Point[:2], 0)
            pc = AuraCAD.Vector(*arc_outside.Vertexes[1].Point[:2], 0)
            pd = AuraCAD.Vector(*arc_outside.Vertexes[0].Point[:2], 0)
        ...
        if self.isArc == 1:
            ...
            path.translate(AuraCAD.Vector(0, 0, obj.FinalDepth.Value - path.BoundBox.ZMin))
            ...
            rectFace.translate(AuraCAD.Vector(0, 0, obj.FinalDepth.Value - rectFace.BoundBox.ZMin))
