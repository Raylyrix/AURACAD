    ...
    try:
        ...
        for s in sel:
            if s.SubElementNames:
                AuraCADGui.Selection.addSelection(s.Object, s.SubElementNames)
            else:
                AuraCADGui.Selection.addSelection(s.Object)
...
class ViewProvider:
    ...
    def attach(self, vobj):
        ...
        self.axs = coin.SoType.fromName("SoauracadPlacementIndicatorKit").createInstance()
        self.axs.axisLength.setValue(1.2)

        # enum values for SoauracadPlacementIndicatorKit
        parts = AuraCADGui.PlacementIndicatorParts
    ...
    def openTaskPanel(self, activate=None):
        ...
        AuraCADGui.Control.showDialog(self.taskPanel)
...
class MaterialDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.materialTree = AuraCADGui.UiLoader().createWidget("MatGui::MaterialTreeWidget")
...
class StockEdit(object):
    ...
    def setLengthField(self, widget, prop):
        widget.setText(AuraCAD.Units.Quantity(prop.Value, AuraCAD.Units.Length).UserString)
...
class StockFromBaseBoundBoxEdit(StockEdit):
    ...
    def getFieldsStock(self, stock, fields=None):
        ...
        try:
            if "xneg" in fields:
                stock.ExtXneg = AuraCAD.Units.Quantity(self.form.stockExtXneg.text())
            if "xpos" in fields:
                stock.ExtXpos = AuraCAD.Units.Quantity(self.form.stockExtXpos.text())
            if "yneg" in fields:
                stock.ExtYneg = AuraCAD.Units.Quantity(self.form.stockExtYneg.text())
            if "ypos" in fields:
                stock.ExtYpos = AuraCAD.Units.Quantity(self.form.stockExtYpos.text())
            if "zneg" in fields:
                stock.ExtZneg = AuraCAD.Units.Quantity(self.form.stockExtZneg.text())
            if "zpos" in fields:
                stock.ExtZpos = AuraCAD.Units.Quantity(self.form.stockExtZpos.text())
        except Exception:
            pass
...
class StockCreateBoxEdit(StockEdit):
    ...
    def getFields(self, obj, fields=None):
        ...
        try:
            if self.IsStock(obj):
                if "length" in fields:
                    obj.Stock.Length = AuraCAD.Units.Quantity(self.form.stockBoxLength.text())
                if "width" in fields:
                    obj.Stock.Width = AuraCAD.Units.Quantity(self.form.stockBoxWidth.text())
                if "height" in fields:
                    obj.Stock.Height = AuraCAD.Units.Quantity(self.form.stockBoxHeight.text())
            else:
...
class StockCreateCylinderEdit(StockEdit):
    ...
    def getFields(self, obj, fields=None):
        ...
        try:
            if self.IsStock(obj):
                if "radius" in fields:
                    obj.Stock.Radius = AuraCAD.Units.Quantity(self.form.stockCylinderRadius.text())
                if "height" in fields:
                    obj.Stock.Height = AuraCAD.Units.Quantity(self.form.stockCylinderHeight.text())
            else:
...
class TaskPanel:
    ...
    def __init__(self, vobj, deleteOnReject):
        AuraCAD.ActiveDocument.openTransaction("Edit Job")
        self.vobj = vobj
        self.vproxy = vobj.Proxy
        self.obj = vobj.Object
        self.deleteOnReject = deleteOnReject
        self.form = AuraCADGui.PySideUic.loadUi(":/panels/PathEdit.ui")
        self.template = PathJobDlg.JobTemplateExport(self.obj, self.form.jobBox.widget(1))
        self.name = self.obj.Name

        vUnit = AuraCAD.Units.Quantity(1, AuraCAD.Units.Velocity).getUserPreferred()[2]
    ...
    def assignMaterial(self):
        ...
        if result == QtWidgets.QDialog.Accepted:
            AuraCAD.Console.PrintMessage("Material assigned\n")
    ...
    def preCleanup(self):
        Path.Log.track()
        AuraCADGui.Selection.removeObserver(self)
    ...
    def reject(self, resetEdit=True):
        ...
        if self.deleteOnReject and AuraCAD.ActiveDocument.getObject(self.name):
            Path.Log.info("Uncreate Job")
            AuraCAD.ActiveDocument.openTransaction("Uncreate Job")
            ...
            Path.Log.track(
                self.name,
                self.deleteOnReject,
                AuraCAD.ActiveDocument.getObject(self.name),
            )
        self.cleanup(resetEdit)
        return True
    ...
    def toolControllerAdd(self):
        ...
        for toolbit in toolbits:
            toolbit.attach_to_doc(AuraCAD.ActiveDocument)
    ...
    def toolControllerChanged(self, item):
        ...
        if "Label" == prop:
            ...
            vUnit = AuraCAD.Units.Quantity(1, AuraCAD.Units.Velocity).getUserPreferred()[2]
            try:
                val = AuraCAD.Units.Quantity(item.text())
                if AuraCAD.Units.Velocity == val.Unit:
                    setattr(tc, prop, val)
                elif AuraCAD.Units.Unit() == val.Unit:
                    val = AuraCAD.Units.Quantity(item.text() + vUnit)
                    setattr(tc, prop, val)
            except Exception:
                pass
    ...
    def modelSetAxis(self, axis):
            ...
            for sel in selection:
                ...
                for feature in sel.SubElementNames:
                    ...
                    if "Face" == sub.ShapeType:
                        ...
                        elif Path.Geom.pointsCoincide(axis, AuraCAD.Vector() - normal):
                            alignSel(sel, AuraCAD.Vector() - normal, True)
                        else:
                            alignSel(sel, normal)
        ...
        if selObject and selFeature:
            AuraCADGui.Selection.clearSelection()
            AuraCADGui.Selection.addSelection(selObject, selFeature)

    def restoreSelection(self, selection):
        AuraCADGui.Selection.clearSelection()
        for sel in selection:
            AuraCADGui.Selection.addSelection(sel.Object, sel.SubElementNames)
        ...
        with selectionEx() as selection:
            for sel in selection:
                ...
                for name in sel.SubElementNames:
                    ...
                    bb = feature.BoundBox
                    offset = AuraCAD.Vector(axis.x * bb.XMax, axis.y * bb.YMax, axis.z * bb.ZMax)
    ...
    def alignSetOrigin(self):
        ...
        placement = AuraCADGui.ActiveDocument.ActiveView.viewPosition()
        placement.Base = placement.Base + by
        AuraCADGui.ActiveDocument.ActiveView.viewPosition(placement, 0)

    def alignMoveToOrigin(self):
        selObject = None
        selFeature = None
        p = None
        for sel in AuraCADGui.Selection.getSelectionEx():
            selObject = sel.Object
            for feature in sel.SubElementNames:
                selFeature = feature
                sub = sel.Object.Shape.getElement(feature)
                if "Vertex" == sub.ShapeType:
                    p = AuraCAD.Vector() - sub.Point
                if "Edge" == sub.ShapeType:
                    p = AuraCAD.Vector() - sub.Curve.Location
                if "Face" == sub.ShapeType:
                    p = AuraCAD.Vector() - sub.BoundBox.Center
    ...
    def updateSelection(self):
        ...
        if self.obj in AuraCADGui.Selection.getSelection():
            AuraCADGui.Selection.removeSelection(self.obj)
    ...
    def setupUi(self, activate):
        ...
        self.form.modelSetXAxis.clicked.connect(lambda: self.modelSetAxis(AuraCAD.Vector(1, 0, 0)))
        self.form.modelSetYAxis.clicked.connect(lambda: self.modelSetAxis(AuraCAD.Vector(0, 1, 0)))
        self.form.modelSetZAxis.clicked.connect(lambda: self.modelSetAxis(AuraCAD.Vector(0, 0, 1)))
        self.form.modelSetX0.clicked.connect(lambda: self.modelSet0(AuraCAD.Vector(-1, 0, 0)))
        self.form.modelSetY0.clicked.connect(lambda: self.modelSet0(AuraCAD.Vector(0, -1, 0)))
        self.form.modelSetZ0.clicked.connect(lambda: self.modelSet0(AuraCAD.Vector(0, 0, -1)))

        self.form.setOrigin.clicked.connect(self.alignSetOrigin)
        self.form.moveToOrigin.clicked.connect(self.alignMoveToOrigin)

        self.form.modelMoveLeftUp.clicked.connect(lambda: self.modelMove(AuraCAD.Vector(-1, 1, 0)))
        self.form.modelMoveLeft.clicked.connect(lambda: self.modelMove(AuraCAD.Vector(-1, 0, 0)))
        self.form.modelMoveLeftDown.clicked.connect(
            lambda: self.modelMove(AuraCAD.Vector(-1, -1, 0))
        )

        self.form.modelMoveUp.clicked.connect(lambda: self.modelMove(AuraCAD.Vector(0, 1, 0)))
        self.form.modelMoveDown.clicked.connect(lambda: self.modelMove(AuraCAD.Vector(0, -1, 0)))

        self.form.modelMoveRightUp.clicked.connect(lambda: self.modelMove(AuraCAD.Vector(1, 1, 0)))
        self.form.modelMoveRight.clicked.connect(lambda: self.modelMove(AuraCAD.Vector(1, 0, 0)))
        self.form.modelMoveRightDown.clicked.connect(
            lambda: self.modelMove(AuraCAD.Vector(1, -1, 0))
        )

        self.form.modelRotateLeft.clicked.connect(lambda: self.modelRotate(AuraCAD.Vector(0, 0, 1)))
        self.form.modelRotateRight.clicked.connect(
            lambda: self.modelRotate(AuraCAD.Vector(0, 0, -1))
        )
    ...
    def open(self):
        AuraCADGui.Selection.addObserver(self)
...
def Create(base, template=None, openTaskPanel=True):
    ...
    AuraCADGui.addModule("Path.Main.Job")
    AuraCAD.ActiveDocument.openTransaction("Create Job")
    try:
