...
from AuraCAD import Vector, Base
...
class CAMSimTaskUi:
    def __init__(self, parent):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/panels/TaskPathSimulator.ui")
        self.parent = parent
...
class PathSimulation:
    def __init__(self):
        ...
        self.stdrot = AuraCAD.Rotation(Vector(0, 0, 1), 0)
        self.iprogress = 0
        self.numCommands = 0
        self.simperiod = 20
        self.accuracy = 0.1
    ...
    def Activate(self):
        ...
        self.onJobChange()
        AuraCADGui.Control.showDialog(self.taskForm)
    ...
    def _populateJobSelection(self, form):
        ...
        jobList = AuraCAD.ActiveDocument.findObjects("Path::FeaturePython", "Job.*")
        jCnt = len(jobList)

        # Check if user has selected a specific job for simulation
        guiSelection = AuraCADGui.Selection.getSelectionEx()
    ...
    def SetupOperation(self, itool):
        ...
        self.icmd = 0
        self.curpos = AuraCAD.Placement(self.initialPos, self.stdrot)
    ...
    def SimulateMill(self):
        ...
        # Add cut tool
        self.cutTool = AuraCAD.ActiveDocument.addObject("Part::FeaturePython", "CutTool")
        self.cutTool.ViewObject.Proxy = 0
        self.cutTool.ViewObject.hide()

        # Add cut material
        if self.isVoxel:
            self.cutMaterial = AuraCAD.ActiveDocument.addObject(
                "Mesh::FeaturePython", "CutMaterial"
            )
            self.cutMaterialIn = AuraCAD.ActiveDocument.addObject(
        ...
        if self.debug:
            self.cutSolid = AuraCAD.ActiveDocument.addObject("Part::FeaturePython", "CutDebug")
    ...
    def PerformCutBoolean(self):
        ...
        if not self.disableAnim:
            self.cutTool.Placement = AuraCAD.Placement(self.curpos, self.stdrot)
        self.icmd += 1
        self.iprogress += 1
        self.UpdateProgress()
    ...
    def RemoveTool(self):
        if self.cutTool is None:
            return
        AuraCAD.ActiveDocument.removeObject(self.cutTool.Name)
        self.cutTool = None
    ...
    def RemoveInnerMaterial(self):
        if self.cutMaterialIn is not None:
            ...
            AuraCAD.ActiveDocument.removeObject(self.cutMaterialIn.Name)
            self.cutMaterialIn = None

    def RemoveMaterial(self):
        if self.cutMaterial is not None:
            AuraCAD.ActiveDocument.removeObject(self.cutMaterial.Name)
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_Simulator", CommandPathSimulate())
    AuraCAD.Console.PrintLog("Loading PathSimulator Guiâ€¦ done\n")
