...
class CommandJobCreate:
    ...
    def Execute(cls, base, template):
        AuraCADGui.addModule("Path.Main.Gui.Job")
        if template:
            template = "'%s'" % template
        else:
            template = "None"
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_Job", CommandJobCreate())
    AuraCADGui.addCommand("CAM_ExportTemplate", CommandJobTemplateExport())

AuraCAD.Console.PrintLog("Loading PathJobCmdâ€¦ done\n")
