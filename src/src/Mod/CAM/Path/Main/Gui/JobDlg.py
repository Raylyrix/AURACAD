...
class JobCreate:
    ...
    def __init__(self, parent=None, sel=None):
        ...
        self.dialog = AuraCADGui.PySideUic.loadUi(":/panels/DlgJobCreate.ui")
    ...
    def _getMinuteBasedSchemas(self):
        ...
        internal_names = AuraCAD.Units.listSchemas()
        minute_based_schemes = []

        # Create a test velocity quantity
        q = AuraCAD.Units.Quantity(1, AuraCAD.Units.Velocity)

        for i, key in enumerate(internal_names):
            try:
                label = AuraCAD.Units.listSchemas(i)
                r = AuraCAD.Units.schemaTranslate(q, i)
                if "/min" in r[2]:
    ...
    def _warnUserIfNotUsingMinutes(self):
        ...
        current_info.setText(
            ...
            ).format(AuraCAD.ActiveDocument.UnitSystem)
        )
        ...
        info_label.setText(
            translate(
                ...
                "<a href='https://wiki.AuraCAD.org/CAM_Workbench#Units'>Units section</a> "
    ...
    def _applyUnitSchema(self, dialog):
        ...
        if selected_schema_id is not None:
            try:
                AuraCAD.ActiveDocument.UnitSystem = selected_schema_id
    ...
    def setupModel(self, job=None):
        ...
        if job:
            ...
            preSelected = Counter([obj.Label for obj in AuraCADGui.Selection.getSelection()])
            jobResources = []
...
class JobTemplateExport:
    ...
    def __init__(self, job, parent=None):
        self.job = job
        self.dialog = AuraCADGui.PySideUic.loadUi(":/panels/DlgJobTemplateExport.ui")
        if parent:
