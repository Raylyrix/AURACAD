...
class JobPreferencesPage:
    def __init__(self, parent=None):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":preferences/PathJob.ui")
    ...
    def saveStockSettings(self):
        if self.form.stockGroup.isChecked():
            ...
            if typ == PathStock.StockType.CreateBox:
                attrs["length"] = AuraCAD.Units.Quantity(self.form.stockBoxLength.text()).UserString
                attrs["width"] = AuraCAD.Units.Quantity(self.form.stockBoxWidth.text()).UserString
                attrs["height"] = AuraCAD.Units.Quantity(self.form.stockBoxHeight.text()).UserString
            if typ == PathStock.StockType.CreateCylinder:
                attrs["radius"] = AuraCAD.Units.Quantity(
                    self.form.stockCylinderRadius.text()
                ).UserString
                attrs["height"] = AuraCAD.Units.Quantity(
                    self.form.stockCylinderHeight.text()
                ).UserString
            if typ == PathStock.StockType.FromBase:
                attrs["xneg"] = AuraCAD.Units.Quantity(self.form.stockExtXneg.text()).UserString
                attrs["xpos"] = AuraCAD.Units.Quantity(self.form.stockExtXpos.text()).UserString
                attrs["yneg"] = AuraCAD.Units.Quantity(self.form.stockExtYneg.text()).UserString
                attrs["ypos"] = AuraCAD.Units.Quantity(self.form.stockExtYpos.text()).UserString
                attrs["zneg"] = AuraCAD.Units.Quantity(self.form.stockExtZneg.text()).UserString
                attrs["zpos"] = AuraCAD.Units.Quantity(self.form.stockExtZpos.text()).UserString
            if self.form.stockPlacementGroup.isChecked():
                angle = AuraCAD.Units.Quantity(self.form.stockAngle.text()).Value
                axis = AuraCAD.Vector(
                    self.form.stockAxisX.value(),
                    self.form.stockAxisY.value(),
                    self.form.stockAxisZ.value(),
                )
                rot = AuraCAD.Rotation(axis, angle)
                attrs["rotX"] = rot.Q[0]
                attrs["rotY"] = rot.Q[1]
                attrs["rotZ"] = rot.Q[2]
                attrs["rotW"] = rot.Q[3]
                attrs["posX"] = AuraCAD.Units.Quantity(self.form.stockPositionX.text()).Value
                attrs["posY"] = AuraCAD.Units.Quantity(self.form.stockPositionY.text()).Value
                attrs["posZ"] = AuraCAD.Units.Quantity(self.form.stockPositionZ.text()).Value
    ...
    def loadStockSettings(self):
        ...
        if (
            ...
            pos = AuraCAD.Vector(float(posX), float(posY), float(posZ))
            rot = AuraCAD.Rotation(float(rotX), float(rotY), float(rotZ), float(rotW))
            placement = AuraCAD.Placement(pos, rot)
        ...
        self.form.stockAngle.setText(
            AuraCAD.Units.Quantity("%f rad" % placement.Rotation.Angle).UserString
        )
        ...
        self.form.stockPositionX.setText(
            AuraCAD.Units.Quantity(placement.Base.x, AuraCAD.Units.Length).UserString
        )
        self.form.stockPositionY.setText(
            AuraCAD.Units.Quantity(placement.Base.y, AuraCAD.Units.Length).UserString
        )
        self.form.stockPositionZ.setText(
            AuraCAD.Units.Quantity(placement.Base.z, AuraCAD.Units.Length).UserString
        )
