...
class CAMoticsUI:
    def __init__(self, simulation):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/panels/TaskPathCamoticsSim.ui")
    ...
    def reject(self):
        ...
        if self.simulation.simmesh is not None:
            AuraCAD.ActiveDocument.removeObject(self.simulation.simmesh.Name)
        AuraCADGui.Control.closeDialog()
...
class CamoticsSimulation(QtCore.QObject):
    ...
    def addMesh(self, surface):
        ...
        if self.simmesh is None:
            self.simmesh = AuraCAD.ActiveDocument.addObject("Mesh::Feature", "Camotics")
        buffer = io.BytesIO()
    ...
    def Activate(self):
        ...
        self.job = AuraCADGui.Selection.getSelectionEx()[0].Object
        self.SIM.set_metric()
    ...
    def buildproject(self):  # , files=[]):
        ...
        unitstring = "imperial" if AuraCAD.Units.getSchema() in [2, 3, 5, 7] else "metric"
...
class CommandCamoticsSimulate:
    ...
    def IsActive(self):
        if bool(AuraCADGui.Selection.getSelection()) is False:
            return False
        try:
            job = AuraCADGui.Selection.getSelectionEx()[0].Object
...
if AuraCAD.GuiUp:
    AuraCADGui.addCommand("CAM_Camotics", CommandCamoticsSimulate())


AuraCAD.Console.PrintLog("Loading PathCamoticsSimulateGuiâ€¦ done\n")
