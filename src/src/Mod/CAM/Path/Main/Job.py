...
class ObjectJob:
    ...
    def setupOperations(self, obj):
        ...
        # ops = AuraCAD.ActiveDocument.addObject(
        #     "Path::FeatureCompoundPython", "Operations"
        # )
        ops = AuraCAD.ActiveDocument.addObject("App::DocumentObjectGroup", "Operations")
        if ops.ViewObject:
    ...
    def setupToolTable(self, obj):
        ...
        if addTable:
            toolTable = AuraCAD.ActiveDocument.addObject("App::DocumentObjectGroup", "Tools")
    ...
    def fixupOperations(self, obj):
        if getattr(obj.Operations, "ViewObject", None):
            try:
                ...
                ops = AuraCAD.ActiveDocument.addObject("Path::FeatureCompoundPython", "Operations")
                ops.ViewObject.Proxy = 0
    ...
    def baseCandidates(cls):
        ...
        return sorted(
            [obj for obj in AuraCAD.ActiveDocument.Objects if cls.isBaseCandidate(obj)],
...
def Instances():
    ...
    if AuraCAD.ActiveDocument:
        return [
            job
            for job in AuraCAD.ActiveDocument.Objects
...
def Create(name, base, templateFile=None):
    ...
    if isinstance(base[0], str):
        ...
        for baseName in base:
            models.append(AuraCAD.ActiveDocument.getObject(baseName))
    else:
        models = base
