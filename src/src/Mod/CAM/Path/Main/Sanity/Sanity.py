...
"""
This file has utilities for checking and catching common errors in AuraCAD
...
class CAMSanity:
    ...
    def squawk(self, operator, note, date=datetime.now(), squawkType="NOTE"):
        ...
        path = f"{AuraCAD.getHomePath()}Mod/CAM/Path/Main/Sanity/{squawk_icon}.svg"
    ...
    def _runData(self):
        ...
        data["jobMinZ"] = AuraCAD.Units.Quantity(
            obj.Path.BoundBox.ZMin, AuraCAD.Units.Length
        ).UserString
        data["jobMaxZ"] = AuraCAD.Units.Quantity(
            obj.Path.BoundBox.ZMax, AuraCAD.Units.Length
        ).UserString
        data["jobDescription"] = obj.Description
        ...
        for op in obj.Operations.Group:
            ...
            if op.Path.BoundBox.isValid():
                zmin = AuraCAD.Units.Quantity(
                    op.Path.BoundBox.ZMin, AuraCAD.Units.Length
                ).UserString
                zmax = AuraCAD.Units.Quantity(
                    op.Path.BoundBox.ZMax, AuraCAD.Units.Length
                ).UserString
            else:
                zmin = ""
                zmax = ""
    ...
    def _stockData(self):
        ...
        data["xLen"] = AuraCAD.Units.Quantity(bb.XLength, AuraCAD.Units.Length).UserString
        data["yLen"] = AuraCAD.Units.Quantity(bb.YLength, AuraCAD.Units.Length).UserString
        data["zLen"] = AuraCAD.Units.Quantity(bb.ZLength, AuraCAD.Units.Length).UserString
        ...
        if hasattr(obj.Stock, "ShapeMaterial"):
            ...
            if "SurfaceSpeedCarbide" in props:
                data["surfaceSpeedCarbide"] = AuraCAD.Units.Quantity(
                    props["SurfaceSpeedCarbide"]
                ).UserString
            if "SurfaceSpeedHSS" in props:
                data["surfaceSpeedHSS"] = AuraCAD.Units.Quantity(
