...
class StockFromBase(Stock):
    ...
    def __init__(self, obj, base):
        ...
        if bb:
            obj.Placement = AuraCAD.Placement(
                AuraCAD.Vector(bb.XMin, bb.YMin, bb.ZMin), AuraCAD.Rotation()
            )
        else:
    ...
    def execute(self, obj):
        ...
        if bb:
            self.origin = AuraCAD.Vector(-obj.ExtXneg.Value, -obj.ExtYneg.Value, -obj.ExtZneg.Value)
...
def SetupStockObject(obj, stockType):
    ...
    if AuraCAD.GuiUp and obj.ViewObject:
        obj.addProperty(
            "App::PropertyString",
            "StockType",
            "Stock",
...
def CreateFromBase(job, neg=None, pos=None, placement=None):
    ...
    base = _getBase(job)
    obj = AuraCAD.ActiveDocument.addObject("Part::FeaturePython", "Stock")
...
def CreateBox(job, extent=None, placement=None):
    ...
    if placement:
        ...
        bb = shapeBoundBox(base.Group)
        origin = AuraCAD.Vector(bb.XMin, bb.YMin, bb.ZMin)
        obj.Placement = AuraCAD.Placement(origin, AuraCAD.Vector(), 0)
...
def CreateCylinder(job, radius=None, height=None, placement=None):
    ...
    if placement:
        ...
        bb = shapeBoundBox(base.Group)
        origin = AuraCAD.Vector((bb.XMin + bb.XMax) / 2, (bb.YMin + bb.YMax) / 2, bb.ZMin)
        obj.Placement = AuraCAD.Placement(origin, AuraCAD.Vector(), 0)
        ...
        if stockType:
            ...
            if stockType == StockType.FromBase:
                ...
                if (
                    ...
                    neg = AuraCAD.Vector(
                        AuraCAD.Units.Quantity(xneg).Value,
                        AuraCAD.Units.Quantity(yneg).Value,
                        AuraCAD.Units.Quantity(zneg).Value,
                    )
                    pos = AuraCAD.Vector(
                        AuraCAD.Units.Quantity(xpos).Value,
                        AuraCAD.Units.Quantity(ypos).Value,
                        AuraCAD.Units.Quantity(zpos).Value,
                    )
                elif (
                    xneg is not None
                    or xpos is not None
            ...
            if stockType == StockType.CreateBox:
                ...
                if length is not None and width is not None and height is not None:
                    ...
                    extent = AuraCAD.Vector(
                        AuraCAD.Units.Quantity(length).Value,
                        AuraCAD.Units.Quantity(width).Value,
                        AuraCAD.Units.Quantity(height).Value,
                    )
...
AuraCAD.Console.PrintLog("Loading PathStock... done\n")
