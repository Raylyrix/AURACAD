...
def preferences():
    return AuraCAD.ParamGet(PreferencesGroup)


def tool_preferences():
    return AuraCAD.ParamGet(ToolGroup)
...
def pathPostSourcePath():
    return os.path.join(AuraCAD.getHomePath(), "Mod/CAM/Path/Post/")


def getBuiltinAssetPath() -> pathlib.Path:
    home = pathlib.Path(AuraCAD.getHomePath())
...
def getDefaultAssetPath() -> Path:
    data_dir = pathlib.Path(AuraCAD.getUserAppDataDir())
...
def getAssetPath() -> pathlib.Path:
    ...
    if cam_assets_path:
        ...
        most_recent_path = AuraCAD.ApplicationDirectories.mostRecentConfigFromBase(cam_assets_path)
    ...
    if legacy_path:
        ...
        if legacy_path_obj.exists() and legacy_path_obj.is_dir():
            ...
            most_recent_legacy = AuraCAD.ApplicationDirectories.mostRecentConfigFromBase(
                str(legacy_path_obj)
            )
    ...
    most_recent_default = AuraCAD.ApplicationDirectories.mostRecentConfigFromBase(str(default))
...
def macroFilePath():
    grp = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Macro")
    return grp.GetString("MacroPath", AuraCAD.getUserMacroDir())
...
def _scan_addon_post_dirs() -> None:
    """Scan AuraCAD Mod directories for post-processor addons via package.xml content.
...
def addAddonPostPath(path: str) -> None:
    ...
    Called by addon Init.py at AuraCAD startup. Each call adds one
...
def addAddonAssetPath(addon_dir: str) -> None:
    ...
    subdirectory layout of a Machines-style addon and registers each type:
      - ``<addon_dir>/posts/``     â†’ post-processor search path
      - ``<addon_dir>/machines/``  â†’ machine definition templates
