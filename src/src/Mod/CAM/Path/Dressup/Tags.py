...
def debugMarker(vector, label, color=None, radius=0.5):
    if Path.Log.getLevel(Path.Log.thisModule()) == Path.Log.Level.DEBUG:
        obj = AuraCAD.ActiveDocument.addObject("Part::Sphere", label)
        obj.Label = label
        obj.Radius = radius
        obj.Placement = AuraCAD.Placement(vector, AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), 0))
        if color:
...
def debugCylinder(vector, r, height, label, color=None):
    if Path.Log.getLevel(Path.Log.thisModule()) == Path.Log.Level.DEBUG:
        obj = AuraCAD.ActiveDocument.addObject("Part::Cylinder", label)
        obj.Label = label
        obj.Radius = r
        obj.Height = height
        obj.Placement = AuraCAD.Placement(vector, AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), 0))
...
def debugCone(vector, r1, r2, height, label, color=None):
    if Path.Log.getLevel(Path.Log.thisModule()) == Path.Log.Level.DEBUG:
        obj = AuraCAD.ActiveDocument.addObject("Part::Cone", label)
        obj.Label = label
        obj.Radius1 = r1
        obj.Radius2 = r2
        obj.Height = height
...
class Tag:
    def __init__(self, nr, x, y, width, height, angle, radius, enabled=True):
        ...
        self.radius = getattr(
            radius, "Value", AuraCAD.Units.Quantity(radius, AuraCAD.Units.Length).Value
        )
    ...
    def originAt(self, z):
        return AuraCAD.Vector(self.x, self.y, z)
    ...
    def createSolidsAt(self, z, R):
        ...
        if not Path.Geom.isRoughly(0, R):  # testing is easier if the solid is not rotated
            ...
            self.solid.rotate(AuraCAD.Vector(0, 0, 0), AuraCAD.Vector(0, 0, 1), angle)
    ...
    def isPointOnEdge(self, pt, edge):
        ...
        # p1 = edge.Vertexes[0]
        # f1 = edge.Curve.parameter(AuraCAD.Vector(p1.X, p1.Y, p1.Z))
        # p2 = edge.Vertexes[1]
        # f2 = edge.Curve.parameter(AuraCAD.Vector(p2.X, p2.Y, p2.Z))
        return False
...
class MapWireToTag:
    ...
    def cleanupEdges(self, edges):
        ...
        if not self.entryEdges:
            Path.Log.debug("fill entryEdgesâ€¦")
        ...
        if not self.exitEdges:
            Path.Log.debug("fill exitEdgesâ€¦")
    ...
    def shell(self):
        ...
        shell = wire.extrude(AuraCAD.Vector(0, 0, self.tag.height + 1))
...
class PathData:
        ...
        for i, pos in enumerate(fromObj.Positions):
            ...
            if i not in fromObj.Disabled:
                dist = self.baseWire.distToShape(
                    Part.Vertex(AuraCAD.Vector(pos.x, pos.y, self.minZ))
                )
    ...
    def pointAtBottom(self, p):
        return AuraCAD.Vector(p.x, p.y, self.minZ)
...
class ObjectTagDressup:
    ...
    def doExecute(self, obj):
        ...
        try:
            ...
            Path.Log.error("processing tags failed clearing all tagsâ€¦ '%s'" % (e.args[0]))
            obj.Path = PathUtils.getPathWithPlacement(obj.Base)
        ...
        if Path.Log.getLevel(Path.Log.thisModule()) == Path.Log.Level.DEBUG:
            for tag in self.tags:
                ...
                if tag.enabled:
                    ...
                    # debugMarker(AuraCAD.Vector(tag.x, tag.y, self.pathData.minZ), "tag-%02d" % tagID , (1.0, 0.0, 1.0), 0.5)
...
def Create(baseObject, name="DressupTag"):
    """
    Create(basePath, name='DressupTag') â€¦ create tag dressup object for the given base path.
    """
...
Path.Log.notice("Loading CAM_DressupTagâ€¦ done\n")
