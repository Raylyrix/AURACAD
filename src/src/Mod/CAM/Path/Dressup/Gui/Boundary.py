...
class TaskPanel(object):
    def __init__(self, obj, viewProvider):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/panels/DressupPathBoundary.ui")
        if obj.Stock:
...
def Create(base, name="DressupPathBoundary"):
    AuraCAD.ActiveDocument.openTransaction("Create a Boundary dressup")
...
class CommandPathDressupPathBoundary:
    ...
    def Activated(self):
        ...
        # everything ok!
        AuraCAD.ActiveDocument.openTransaction("Create Path Boundary Dress-up")
        AuraCADGui.addModule("Path.Dressup.Gui.Boundary")
        AuraCADGui.doCommand("Path.Dressup.Gui.Boundary.Create(App.ActiveDocument.%s)" % op.Name)
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_DressupPathBoundary", CommandPathDressupPathBoundary())
