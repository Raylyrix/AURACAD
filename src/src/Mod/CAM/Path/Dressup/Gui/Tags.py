...
class PathDressupTagTaskPanel:
    ...
    def __init__(self, obj, viewProvider, jvoVisibility=None):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/panels/HoldingTagsEdit.ui")
        ...
        if jvoVisibility is None:
            AuraCAD.ActiveDocument.openTransaction("Edit HoldingTags Dress-up")
    ...
    def getFields(self):
        width = AuraCAD.Units.Quantity(self.form.ifWidth.text()).Value
        height = AuraCAD.Units.Quantity(self.form.ifHeight.text()).Value
        angle = self.form.dsbAngle.value()
        radius = AuraCAD.Units.Quantity(self.form.ifRadius.text()).Value

        tags = self.getTags(True)
        positions = []
        disabled = []
        for i, (x, y, enabled) in enumerate(tags):
            positions.append(AuraCAD.Vector(x, y, 0))
            if not enabled:
                disabled.append(i)
    ...
    def copyNewTags(self):
        ...
        tags = [o for o in AuraCAD.ActiveDocument.Objects if sel == o.Label]
        if 1 == len(tags):
    ...
    def updateTagsViewWith(self, tags):
        self.tags = tags
        self.Positions = [AuraCAD.Vector(t[0], t[1], 0) for t in tags]
    ...
    def addNewTagAt(self, point, obj):
        if point and obj and self.obj.Proxy.pointIsOnPath(self.obj, point):
            ...
            self.Positions.append(AuraCAD.Vector(point.x, point.y, 0))
            self.updateTagsView()
        else:
    ...
    def editTag(self, item):
        if item:
            ...
            self.getPoint.getPoint(self.editTagAt, AuraCAD.Vector(x, y, z))
    ...
    def setFields(self):
        ...
        self.form.ifHeight.setText(
            AuraCAD.Units.Quantity(self.obj.Height, AuraCAD.Units.Length).UserString
        )
        self.form.ifWidth.setText(
            AuraCAD.Units.Quantity(self.obj.Width, AuraCAD.Units.Length).UserString
        )
        self.form.dsbAngle.setValue(self.obj.Angle)
        self.form.ifRadius.setText(
            AuraCAD.Units.Quantity(self.obj.Radius, AuraCAD.Units.Length).UserString
        )
    ...
    def setupUi(self):
        ...
        enableCopy = False
        for tags in sorted(
            [o.Label for o in AuraCAD.ActiveDocument.Objects if "DressupTag" in o.Name]
        ):
...
class PathDressupTagViewProvider:
    ...
    def setupTaskPanel(self, panel):
        ...
        panel.setupUi()
        AuraCADGui.Selection.addSelectionGate(self)
...
def Create(baseObject, name="DressupTag"):
    ...
    """
    AuraCAD.ActiveDocument.openTransaction("Create a Tag dressup")
...
class CommandPathDressupTag:
    ...
    def Activated(self):
        ...
        # everything ok!
        AuraCAD.ActiveDocument.openTransaction("Create Tag Dress-up")
        AuraCADGui.addModule("Path.Dressup.Gui.Tags")
        AuraCADGui.doCommand("Path.Dressup.Gui.Tags.Create(App.ActiveDocument.%s)" % op.Name)
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_DressupTag", CommandPathDressupTag())
