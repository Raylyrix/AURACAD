...
class TaskPanel(object):
    ...
    def __init__(self, viewProvider, obj):
        ...
        self.obj = obj
        self.form = AuraCADGui.PySideUic.loadUi(":/panels/DogboneEdit.ui")
        self.s = None
        AuraCAD.ActiveDocument.openTransaction("Edit Dogbone Dress-up")
        # self.height = 10 ???
        self.markers = []
    ...
    def reject(self):
        ...
        AuraCADGui.Selection.removeObserver(self.s)
        self.cleanup()
    ...
    def updateBoneList(self):
        ...
        for item in sorted(itemList, key=lambda item: item.data(self.DataState).boneIDs()[0]):
            ...
            markers.append(
                Marker(
                    AuraCAD.Vector(loc.x, loc.y, min(zs)),
                    r,
                    max(1, max(zs) - min(zs)),
                    state.isEnabled(),
                )
    ...
    def open(self):
        ...
        AuraCADGui.Selection.addObserver(self.s)
...
class SelObserver(object):
    ...
    def addSelection(self, doc, obj, sub, pnt):
        AuraCADGui.doCommand("Gui.Selection.addSelection(AuraCAD.ActiveDocument." + obj + ")")
...
def Create(base, name="DressupDogbone"):
    """
    Create(obj, name='DressupDogbone')â€¦ dresses the given Path.Op.Profile object with dogbones.
    """
...
class CommandDressupDogboneII(object):
    ...
    def Activated(self):
        ...
        # everything ok!
        AuraCAD.ActiveDocument.openTransaction("Create Dogbone Dress-up")
        AuraCADGui.addModule("Path.Dressup.Gui.DogboneII")
        AuraCADGui.doCommand(
            "Path.Dressup.Gui.DogboneII.Create(AuraCAD.ActiveDocument.%s)" % op.Name
        )
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_DressupDogbone", CommandDressupDogboneII())

AuraCAD.Console.PrintLog("Loading DressupDogboneIIâ€¦ done\n")
