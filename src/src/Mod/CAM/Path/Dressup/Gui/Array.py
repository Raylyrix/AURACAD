...
class CommandPathDressupArray:
    ...
    def Activated(self):
        ...
        # everything ok!
        AuraCAD.ActiveDocument.openTransaction("Create Path Array Dress-up")
        AuraCADGui.addModule("Path.Dressup.Gui.Array")
        AuraCADGui.doCommand("Path.Dressup.Gui.Array.Create(App.ActiveDocument.%s)" % op.Name)
...
def Create(base, name="DressupPathArray"):
    AuraCAD.ActiveDocument.openTransaction("Create an Array dressup")
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_DressupArray", CommandPathDressupArray())
