...
class ObjectDressup:
    ...
    def _bilinearInterpolate(self, surface, x, y):
        p1 = AuraCAD.Vector(x, y, 100.0)
        p2 = AuraCAD.Vector(x, y, -100.0)
    ...
    def _getinterpSurface(self, obj):
        ...
        for y in yindex:
            ...
            array.append([AuraCAD.Vector(p[0], p[1], p[2]) for p in points])
    ...
    def execute(self, obj):
        ...
        for cmd in path.Commands:
            ...
            if cmd.Name not in Path.Geom.CmdMoveMill:
                ...
                curVec = AuraCAD.Vector(currLocation["X"], currLocation["Y"], currLocation["Z"])
                ...
                for point in pointlist:
                    if not bb.isInside(AuraCAD.Vector(point.x, point.y, 0)):
                        obj.Path = path
...
class TaskPanel:
    def __init__(self, obj):
        self.obj = obj
        self.form = AuraCADGui.PySideUic.loadUi(":/panels/ZCorrectEdit.ui")
        AuraCAD.ActiveDocument.openTransaction("Edit Z Correction Dress-up")
        self.interpshape = AuraCAD.ActiveDocument.addObject("Part::Feature", "InterpolationSurface")
    ...
    def accept(self):
        ...
        AuraCAD.ActiveDocument.removeObject(self.interpshape.Name)
...
class ViewProviderDressup:
    ...
    def setEdit(self, vobj, mode=0):
        if mode == 1:
            AuraCADGui.runCommand("Std_TransformManip")
        elif mode == 0:
            AuraCADGui.Control.closeDialog()
...
class CommandPathDressup:
    ...
    def Activated(self):
        ...
        AuraCADGui.addModule("Path.Dressup.Gui.ZCorrect")
        AuraCADGui.addModule("PathScripts.PathUtils")
        AuraCADGui.doCommand(
            'obj = AuraCAD.ActiveDocument.addObject("Path::FeaturePython", "ZCorrectDressup")'
        )
        AuraCADGui.doCommand("Path.Dressup.Gui.ZCorrect.ObjectDressup(obj)")
        AuraCADGui.doCommand("obj.Base = AuraCAD.ActiveDocument." + op.Name)
        AuraCADGui.doCommand("Path.Dressup.Gui.ZCorrect.ViewProviderDressup(obj.ViewObject)")
        AuraCADGui.doCommand("PathScripts.PathUtils.addToJob(obj)")
        AuraCADGui.doCommand("Gui.ActiveDocument.getObject(obj.Base.Name).Visibility = False")
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_DressupZCorrect", CommandPathDressup())

AuraCAD.Console.PrintLog("Loading PathDressupâ€¦ done\n")
