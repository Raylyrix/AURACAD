...
class HoldingTagPreferences:
    ...
    def __init__(self):
        if AuraCAD.GuiUp:
            ...
            self.form = AuraCADGui.PySideUic.loadUi(":/preferences/PathDressupHoldingTags.ui")
            self.label = translate("CAM_DressupTag", "Holding Tag")

    def loadSettings(self):
        self.form.ifWidth.setText(
            AuraCAD.Units.Quantity(self.defaultWidth(0), AuraCAD.Units.Length).UserString
        )
        self.form.ifHeight.setText(
            AuraCAD.Units.Quantity(self.defaultHeight(0), AuraCAD.Units.Length).UserString
        )
        self.form.dsbAngle.setValue(self.defaultAngle())
        self.form.ifRadius.setText(
            AuraCAD.Units.Quantity(self.defaultRadius(), AuraCAD.Units.Length).UserString
        )
    ...
    def saveSettings(self):
        ...
        pref.SetFloat(
            ...
            AuraCAD.Units.Quantity(self.form.ifWidth.text()).Value,
        )
        pref.SetFloat(
            self.DefaultHoldingTagHeight,
            AuraCAD.Units.Quantity(self.form.ifHeight.text()).Value,
        )
        pref.SetFloat(self.DefaultHoldingTagAngle, self.form.dsbAngle.value())
        pref.SetFloat(
            self.DefaultHoldingTagRadius,
            AuraCAD.Units.Quantity(self.form.ifRadius.text()),
        )
