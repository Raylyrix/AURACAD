...
__doc__ = """Dragknife Dressup object and AuraCAD command"""
...
class ObjectDressup:
    ...
    def shortcut(self, queue):
        ...
        if queue[1].Name in arccommands:
            ...
            v1 = radvector.cross(AuraCAD.Vector(0, 0, 1))
        else:
        ...
        if queue[0].Name in arccommands:
            ...
            v2 = radvector.cross(AuraCAD.Vector(0, 0, 1))
        else:
    ...
    def segmentAngleXY(self, prevCommand, currCommand, endpos=False, currentZ=0):
        ...
        myAngle = D.angle(v1, AuraCAD.Base.Vector(1, 0, 0), AuraCAD.Base.Vector(0, 0, -1))
        return myAngle
    ...
    def arcExtension(self, obj, queue):
        ...
        C = AuraCAD.Base.Vector(queue[2].x + queue[1].I, queue[2].y + queue[1].J, currLocation["Z"])
        ...
        # endpoint = AuraCAD.Base.Vector(Bx, By, currLocation["Z"])
    ...
    def arcTwist(self, obj, queue, lastXY, twistCW=False):
        ...
        segAngle = D.angle(v1, AuraCAD.Base.Vector(1, 0, 0), AuraCAD.Base.Vector(0, 0, -1))
        ...
        endpointvector = AuraCAD.Base.Vector(Bx, By, currLocation["Z"])
    ...
    def lineTwist(self, obj, queue, lastXY, twistCW=False):
        ...
        segAngle = D.angle(v2, AuraCAD.Base.Vector(1, 0, 0), AuraCAD.Base.Vector(0, 0, -1))
        xoffset = math.cos(segAngle) * offset
        yoffset = math.sin(segAngle) * offset
                    ...
                    if changedXYFlag and (len(queue) == 3):
                        ...
                        if abs(incident_angle) >= obj.filterAngle:
                            ...
                            if queue[1].Name in ["G01", "G1"]:
                                ...
                                AuraCAD.Console.PrintWarning("I don't know what's up")
                            #
                            #  DO THE TWIST
                            #
...
class TaskPanel:
    def __init__(self, obj):
        self.obj = obj
        self.form = AuraCADGui.PySideUic.loadUi(":/panels/DragKnifeEdit.ui")
        ...
        AuraCAD.ActiveDocument.openTransaction("Edit Dragknife Dress-up")
...
class ViewProviderDressup:
        ...
        if self.Object and self.Object.Base:
            for i in self.Object.Base.InList:
                if hasattr(i, "Group"):
                    ...
                    i.Group = group
                    print(i.Group)
            # AuraCADGui.ActiveDocument.getObject(obj.Base.Name).Visibility = False
    ...
    def onDelete(self, arg1=None, arg2=None):
        if arg1.Object and arg1.Object.Base:
            AuraCADGui.ActiveDocument.getObject(arg1.Object.Base.Name).Visibility = True
...
class CommandDressupDragknife:
    ...
    def Activated(self):
        ...
        # everything ok!
        AuraCAD.ActiveDocument.openTransaction("Create Dress-up")
        AuraCADGui.addModule("Path.Dressup.Gui.Dragknife")
        AuraCADGui.addModule("PathScripts.PathUtils")
        AuraCADGui.doCommand(
            'obj = AuraCAD.ActiveDocument.addObject("Path::FeaturePython","DragknifeDressup")'
        )
        AuraCADGui.doCommand("Path.Dressup.Gui.Dragknife.ObjectDressup(obj)")
        AuraCADGui.doCommand("base = AuraCAD.ActiveDocument." + op.Name)
        AuraCADGui.doCommand("job = PathScripts.PathUtils.findParentJob(base)")
        AuraCADGui.doCommand("obj.Base = base")
        AuraCADGui.doCommand("job.Proxy.addOperation(obj, base)")
        AuraCADGui.doCommand(
            "obj.ViewObject.Proxy = Path.Dressup.Gui.Dragknife.ViewProviderDressup(obj.ViewObject)"
        )
        AuraCADGui.doCommand("Gui.ActiveDocument.getObject(base.Name).Visibility = False")
        AuraCADGui.doCommand("obj.filterAngle = 20")
        AuraCADGui.doCommand("obj.offset = 2")
        AuraCADGui.doCommand("obj.pivotheight = 4")
        AuraCADGui.doCommand("obj.ViewObject.Document.setEdit(obj.ViewObject, 0)")

        # AuraCAD.ActiveDocument.commitTransaction()  # Final `commitTransaction()` called via TaskPanel.accept()
        AuraCAD.ActiveDocument.recompute()


if AuraCAD.GuiUp:
    # register the AuraCAD command
    AuraCADGui.addCommand("CAM_DressupDragKnife", CommandDressupDragknife())

AuraCAD.Console.PrintLog("Loading CAM_DressupDragKnifeâ€¦ done\n")
