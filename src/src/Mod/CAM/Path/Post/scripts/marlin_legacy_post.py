...
# *****************************************************************************
# *   Due to the fundamentals of the AuraCAD pre-processor,                   *
...
# * Therefore, check AuraCAD gcodes for conflicts with Marlin.                *
...
def export(objectslist, filename, argstring):
    ...
    # Write header:
    if OUTPUT_HEADER:
        gcode += linenumber() + "(Exported by AuraCAD)\n"
    ...
    if AuraCAD.GuiUp and SHOW_EDITOR:
        ...
        for c in PathUtils.getPathWithPlacement(pathobj).Commands:
                ...
                if param in c.Parameters:
                    if param == "F":
                        if command not in RAPID_MOVES:
                            feedRate = Units.Quantity(c.Parameters["F"], AuraCAD.Units.Velocity)
                    ...
                    # [X, Y, Z, U, V, W, I, J, K, R, Q]
                        ...
                        pos = Units.Quantity(c.Parameters[param], AuraCAD.Units.Length)
                        outlist.append(
            ...
            if command in MOTION_COMMANDS:
                if "X" in c.Parameters:
                    CURRENT_X = Units.Quantity(c.Parameters["X"], AuraCAD.Units.Length)
                if "Y" in c.Parameters:
                    CURRENT_Y = Units.Quantity(c.Parameters["Y"], AuraCAD.Units.Length)
                if "Z" in c.Parameters:
                    CURRENT_Z = Units.Quantity(c.Parameters["Z"], AuraCAD.Units.Length)
...
# * The following code converts AuraCAD's canned drill cycles into            *
...
def drill_translate(outlist, cmd, params):
    ...
    drill_X = Units.Quantity(params["X"], AuraCAD.Units.Length)
    drill_Y = Units.Quantity(params["Y"], AuraCAD.Units.Length)
    drill_Z = Units.Quantity(params["Z"], AuraCAD.Units.Length)
    drill_R = Units.Quantity(params["R"], AuraCAD.Units.Length)
    drill_F = Units.Quantity(params["F"], AuraCAD.Units.Velocity)
    if cmd == "G82":
        drill_DwellTime = params["P"]
    elif cmd == "G83":
        drill_Step = Units.Quantity(params["Q"], AuraCAD.Units.Length)
