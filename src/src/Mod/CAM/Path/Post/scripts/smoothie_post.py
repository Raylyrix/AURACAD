...
M2"""
    ...
    def _send_to_smoothie(self, ip: str, gcode: str, fname: str) -> None:
        ...
        AuraCAD.Console.PrintMessage(f"Sending to SmoothieBoard: {fname}\n")
        ...
        try:
            ...
            if not ln.startswith("+"):
                AuraCAD.Console.PrintError(f"Failed to connect with SFTP: {ln}\n")
                return
            ...
            ln = tn.readline()
            if not ln.startswith("+"):
                AuraCAD.Console.PrintError(f"Failed to create file: {ln}\n")
                return
            ...
            ln = tn.readline()
            if not ln.startswith("+"):
                AuraCAD.Console.PrintError(f"Failed: {ln}\n")
                return
            ...
            ln = tn.readline()
            if not ln.startswith("+"):
                AuraCAD.Console.PrintError(f"Failed to save file: {ln}\n")
                return
            ...
            AuraCAD.Console.PrintError(f"Connection timeout while connecting to {ip}:115\n")
        except ConnectionRefusedError:
            AuraCAD.Console.PrintError(
                f"Connection refused by {ip}:115. Is the SmoothieBoard running?\n"
            )
        except Exception as e:
            AuraCAD.Console.PrintError(f"Error sending to SmoothieBoard: {str(e)}\n")
