...
# post processor with Generic_Estlcam.auracadm machine configuration file.
...
used directly from inside AuraCAD, via the GUI importer or via python scripts
with:
                ...
                if param in c.Parameters:
                    if param == "F":
                        if command not in RAPID_MOVES:
                            speed = Units.Quantity(c.Parameters["F"], AuraCAD.Units.Velocity)
...
def drill_translate(outstring, cmd, params):
    ...
    RETRACT_Z = Units.Quantity(params["R"], AuraCAD.Units.Length)
    ...
    drill_feedrate = Units.Quantity(params["F"], AuraCAD.Units.Velocity)
    if cmd == "G83":
