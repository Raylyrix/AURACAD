...
TOOLTIP = """
AuraCAD Path post-processor to output code for the Roland Modela MDX-## machines.
...
def xyarc(args, state):
    ...
    lastPoint = AuraCAD.Vector(state["X"], state["Y"])
    newPoint = AuraCAD.Vector(float(args["X"]), float(args["Y"]))
    centerOffset = AuraCAD.Vector(float(args["I"]), float(args["J"]))
    center = lastPoint + centerOffset
    radius = (center - lastPoint).Length
    xyNormal = AuraCAD.Vector(0, 0, 1)
    circle = Part.Circle(center, xyNormal, radius)
    p0 = circle.parameter(lastPoint)
    p1 = circle.parameter(newPoint)
    arc = Part.ArcOauracadircle(circle, p0, p1)
    steps = 64  # specify max error instead?
    points = arc.discretize(steps)
    # consider direction?
    # print('p = Part.ArcOauracadircle(Part.Circle(AuraCAD.Vector(%f, %f), AuraCAD.Vector(0, 0, 1), %f), %f, %f)' % (center.x, center.y, radius, p0, p1))
    for p in points:
