...
# post processor with Generic_UCCNC.auracadm machine configuration file.
# Use the generic post processor instead.

# See: https://wiki.AuraCAD.org/Path_Post
#      https://wiki.AuraCAD.org/Path_Postprocessor_Customization
...
Path/Tool folder, can be used directly from inside AuraCAD,
...
It was tested on AuraCAD v0.17, v0.18 and v0.19
...
HEADER = """(Exported by AuraCAD for {})
...
def export(objectslist, filename, argstring):
    ...
    if OUTPUT_HEADER:
        for line in HEADER.format(
            ...
            __name__,
            VERSION,
            AuraCAD.ActiveDocument.FileName,
            str(now),
        ).splitlines(False):
            if line:
                gcode += append(line + "\n")
