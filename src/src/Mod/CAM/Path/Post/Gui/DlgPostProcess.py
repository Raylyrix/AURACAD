...
class PostProcessDialog:
    ...
    def __init__(self, job):
        self.job = job
        self.dialog = AuraCADGui.PySideUic.loadUi(":/panels/DlgPostProcess.ui")
        if self.dialog is None:
            raise RuntimeError(
                "Failed to load DlgPostProcess.ui â€” rebuild may be needed to update Qt resources"
            )
        ...
        # Original (subpart, gcode) sections â€” used to regenerate filenames
    ...
    def _populate(self):
        ...
        self._populate_warnings()  # must be last â€” updates tab badge
    ...
    def _populate_warnings(self, use_dialog_values=False):
        ...
        if n_critical:
            dlg.labelWarningsStatus.setText(
                translate("CAM_Post", "{} critical issue(s) â€” review before machining.").format(
                    n_critical
                )
            )
    ...
    def _save_output_files(self):
        ...
        for fname, gcode in self._generated_outputs.items():
            try:
                ...
                AuraCAD.Console.PrintMessage(f"File written to {save_path}\n")
            except Exception as e:
