...
def filter_inefficient_moves(gcode: List[str]) -> List[str]:
    ...
    def collapse_rapid_chain(chain):
        ...
        # If only one axis changes â†’ keep only the last command
        if len(axes_changed) == 1:
            return [chain[-1]["original"]]

        # If changes are only within linear or rotary groups â†’ keep only last
        lin = {"X", "Y", "Z"}
        rot = {"A", "B", "C"}
        ...
        # Mixed changes â†’ can't collapse, keep all
