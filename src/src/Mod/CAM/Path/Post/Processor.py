...
class _HeaderBuilder:
    ...
    def add_exporter_info(self, exporter: str = "AuraCAD"):
...
class PostProcessor:
    ...
    def _apply_schema_defaults(self):
        ...
        The .auracadm file only stores properties explicitly changed by the user.
    ...
    def build_configuration_bundle(self, overrides=None):
        ...
        Pure computation with no side effects â€” suitable for testing.

        Merges in priority order:
          1. Machine postprocessor_properties (from .auracadm file)
        ...
        """
        # Stage 1 â€” machine postprocessor_properties
        bundle = {}
        if self._machine:
            bundle.update(self._machine.postprocessor_properties)

        # Stage 2 â€” schema defaults for missing keys
        schema = self.get_full_property_schema()
        for prop in schema:
            name = prop.get("name", "")
        ...
        # Stage 3 â€” job configuration elements (defaults from the document/job)
        ...
        # Stage 4 â€” caller / dialog overrides (highest priority)
    ...
    def apply_configuration_bundle(self, overrides=None):
        ...
        Path.Log.debug(f"Configuration bundle applied â€” " f"bundle: {bundle}")
    ...
    def _build_header(self, postables):
        ...
        # Tool/fixture lists are part of the header block â€” gate on header_enabled,
    ...
    def get_sanity_checks(self, job):
        ...
        Args:
            job: AuraCAD CAM job object to validate
    ...
    def _create_squawk(self, squawk_type, note):
        ...
        return {
            ...
            "squawkIcon": f"{AuraCAD.getHomePath()}Mod/CAM/Path/Main/Sanity/{icon_map.get(squawk_type, 'Sanity_Note')}.svg",
        }
