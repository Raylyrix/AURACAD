...
class DlgSelectPostProcessor:
    ...
    def __init__(self):
        self.dialog = AuraCADGui.PySideUic.loadUi(":/panels/DlgSelectPostProcessor.ui")
        firstItem = None
...
class CommandPathPost:
    ...
    def _write_file(self, filename, gcode, policy):
        ...
        AuraCAD.Console.PrintMessage(f"File written to {filename}\n")
    ...
    def Activated(self):
        ...
        if use_new_flow and AuraCAD.GuiUp:
            ...
            AuraCAD.ActiveDocument.openTransaction("Post Process the Selected Job")
        ...
        if use_new_flow:
            ...
            try:
                ...
                AuraCAD.Console.PrintError(f"Machine configuration error: {e}\n")
                AuraCAD.ActiveDocument.abortTransaction()
                return
        else:
        ...
        for item in post_data:
            ...
            # the system where AuraCAD is running.  In the old-style postprocessors the
            ...
            if gcode is not None:
                ...
                if use_new_flow and AuraCAD.GuiUp and Path.Preferences.showEditorOnPostProcess():
                    if len(gcode) > 100000:
                        ...
                        if editor_result == 1:  # User clicked OK
                            ...
                            AuraCAD.Console.PrintMessage(f"Post-processing cancelled for {fname}\n")
                            continue
...
class CommandPathPostSelected(CommandPathPost):
    ...
    def Activated(self):
        ...
        on user selection and document context.
        """
        AuraCAD.ActiveDocument.openTransaction("Post Process the Selected operations")
        ...
        if (
            ...
            baseOp = AuraCAD.ActiveDocument.getObject(selection[0].Base[0])
        ...
        for item in post_data:
            ...
            if gcode is not None:
                ...
                final_gcode = gcode
                if AuraCAD.GuiUp and Path.Preferences.showEditorOnPostProcess():
                    if len(gcode) > 100000:
                        AuraCAD.Console.PrintWarning(
                            "Skipping editor since output is greater than 100kb\n"
                        )
...
if AuraCAD.GuiUp:
    ...
    AuraCADGui.addCommand("CAM_Post", CommandPathPost())
    AuraCADGui.addCommand("CAM_PostSelected", CommandPathPostSelected())

AuraCAD.Console.PrintLog("Loading PathPostâ€¦ done\n")
