...
def filterArcs(arcEdge):
    ...
    if isinstance(arcEdge.Curve, Part.Circle):
        ...
        if goodarc:
            ...
            arcseg1 = Part.ArcOauracadircle(arcstpt, arcquad1, arcmid)
            arcseg2 = Part.ArcOauracadircle(arcmid, arcquad2, arcendpt)
...
def getEnvelope(partshape, subshape=None, depthparams=None):
    ...
    newPlace = AuraCAD.Placement(Vector(0, 0, zShift), sec.Placement.Rotation)
    ...
    if Path.Log.getLevel(Path.Log.thisModule()) == Path.Log.Level.DEBUG:
        removalshape = AuraCAD.ActiveDocument.addObject("Part::Feature", "Envelope")
...
def getOffsetArea(
    ...
    https://github.com/sliptonic/AuraCAD/wiki/PathArea-notes."""
...
def reverseEdge(e):
    if DraftGeomUtils.geomType(e) == "Circle":
        ...
        arcoauracadirc = Part.ArcOauracadircle(arcendpt, arcmid, arcstpt)
        newedge = arcoauracadirc.toShape()
...
def drillTipLength(tool):
    ...
    if angle <= 0 or angle >= 180:
        Path.Log.error(
            translate("Path", "Invalid Cutting Edge Angle %.2f, must be >0Â° and <=180Â°") % angle
        )
        return 0.0
...
def applyPlacementToPath(placement, path):
    ...
    for cmd in path.Commands:
        if cmd.Name in Path.Geom.CmdMoveAll:
            ...
            x, y, z = placement.Rotation.multVec(AuraCAD.Vector(x, y, z))
            ...
            if cmd.Name in Path.Geom.CmdMoveArc:
                ...
                i, j, k = placement.Rotation.multVec(AuraCAD.Vector(i, j, 0))
