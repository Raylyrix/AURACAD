...
__title__ = "AuraCAD Arch Component"
...
class Component(ArchIauracad.IauracadProduct):
    ...
    Components here: https://wiki.AuraCAD.org/Arch_Component
    ...
    def setProperties(self, obj):
        ...
        https://wiki.AuraCAD.org/property
        """

        ArchIauracad.IauracadProduct.setProperties(self, obj)
    ...
    def execute(self, obj):
        ...
        if hasattr(obj.Base, "Shape") and not obj.Base.Shape.isNull():
            ...
            base_shape.Placement = AuraCAD.Placement()
    ...
    def onBeforeChange(self, obj, prop):
        ...
        """
        if prop == "Placement":
            self.oldPlacement = AuraCAD.Placement(obj.Placement)
    ...
    def onChanged(self, obj, prop):
        ...
        Also call ArchIauracad.IauracadProduct.onChanged().
        ...
        ArchIauracad.IauracadProduct.onChanged(self, obj, prop)
        ...
        if obj.Base:
            ...
            if (
                ...
                if obj.Base.Base:
                    if obj.Placement.Rotation.Angle < 0.0001:
                        # if the final obj is rotated, this will screw all our Iauracad orientation. Better leave it like that then...
            ...
            # the base is a Part Extrusion
                ...
                if obj.Base.Base and len(obj.Base.Base.Shape.Wires) == 1:
                    ...
                    extrusion = AuraCAD.Vector(obj.Base.Dir).normalize()
                    if extrusion.Length == 0:
                        extrusion = AuraCAD.Vector(0, 0, 1)
                    else:
                ...
                for sub in obj.Base.Shapes:
                    if sub.isDerivedFrom("Part::Extrusion"):
                        if sub.Base:
                            ...
                            extrusion = AuraCAD.Vector(sub.Dir).normalize()
                            if extrusion.Length == 0:
                                extrusion = AuraCAD.Vector(0, 0, 1)
                            else:
                                extrusion = placement.inverse().Rotation.multVec(extrusion)
    ...
    def rebase(self, shape, hint=None):
        ...
        r = AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), n)
        ...
        for s in shape:
            ...
            s.rotate(AuraCAD.Vector(0, 0, 0), r.Axis, math.degrees(-r.Angle))
            shapes.append(s)
        p = AuraCAD.Placement()
        p.Base = v
        p.Rotation = r
    ...
    def processSubShapes(self, obj, base, placement=None):
        ...
        if placement:
            if self.isIdentity(placement):
                ...
                placement = AuraCAD.Placement(placement)
        ...
        for o in obj.Additions:
            ...
            if not base or base.isNull():
                if hasattr(o, "Shape"):
                    ...
                    if placement:
                        # see https://forum.AuraCAD.org/viewtopic.php?p=579754#p579754
    ...
    def isStandardCase(self, obj):
        """Determine if the component is a standard case of its Iauracad type.

        Not all Iauracad types have a standard case.
        ...
        All objects whose IauracadType is suffixed with the string " Sandard Case"
        ...
        if obj.IauracadType.endswith("Standard Case"):
            return True
        # Try to guess
        if obj.IauracadType + " Standard Case" in ArchIauracad.IauracadTypes:
            # this type has a standard case
            if obj.Additions or obj.Subtractions:
                return False
            if obj.Placement.Rotation.Axis.getAngle(AuraCAD.Vector(0, 0, 1)) > 0.01:
                # reject rotated objects
                return False
            if obj.CloneOf:
                return obj.CloneOf.Proxy.isStandardCase(obj.CloneOf)
            if obj.IauracadType == "Wall":
                # rules:
                # - vertically extruded
                # - single baseline or no baseline
                if (not obj.Base) or (len(obj.Base.Shape.Edges) == 1):
                    if hasattr(obj, "Normal"):
                        if obj.Normal in [AuraCAD.Vector(0, 0, 0), AuraCAD.Vector(0, 0, 1)]:
                            return True
            elif obj.IauracadType in ["Beam", "Column", "Slab"]:
                # rules:
                ...
                if hasattr(obj, "Tool") and obj.Tool:
                    return False
                if obj.Normal == AuraCAD.Vector(0, 0, 0):
                    return True
            ...
            # - must have an IauracadWindowType and IauracadRelFillsElement (to be implemented in Iauracad exporter)
            return False
        ...
        if getattr(obj, "Base", None):
            if obj.Base.isDerivedFrom("Part::Feature"):
                ...
                if isinstance(getattr(obj.Base, "Shape", None), Part.Shape):
                    ...
                    AuraCAD.Console.PrintError(obj.Label + ": " + t + "\n")
                    return False
    ...
    def _isInternalLinkgroup(self, obj):
        ...
        # https://forum.AuraCAD.org/viewtopic.php?p=820487#p820428
...
class AreaCalculator:
    ...
    def isFaceVertical(self, face, face_index=None):
        ...
        https://github.com/AuraCAD/AuraCAD/issues/14687.
        """
        import Part
        import DraftGeomUtils
        import TechDraw
        ...
        if face.Surface.TypeId == "Part::GeomCylinder":
            angle = face.Surface.Axis.getAngle(AuraCAD.Vector(0, 0, 1))
            return self.isZeroAngle(angle)
        elif face.Surface.TypeId == "Part::GeomSurfaceOfExtrusion":
            angle = face.Surface.Direction.getAngle(AuraCAD.Vector(0, 0, 1))
            return self.isZeroAngle(angle)
            ...
            try:
                edges = TechDraw.project(face, AuraCAD.Vector(0, 0, 1))[0].Edges
        ...
        try:
            angle = face.normalAt(0, 0).getAngle(AuraCAD.Vector(0, 0, 1))
    ...
    def _computeHorizontalAreaAndPerimeter(self, horizontalAreaFaces):
        ...
        param_grp = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/TechDraw/debug")
        ...
        direction = AuraCAD.Vector(0, 0, 1)
        projectedFaces = []
...
class ViewProviderComponent:
    ...
    def setEdit(self, vobj, mode):
        ...
        taskd.update()
        AuraCADGui.Control.showDialog(taskd)
        return True
    ...
    def edit(self):
        AuraCADGui.ActiveDocument.setEdit(self.Object, 0)

    def toggleSubcomponents(self):
        AuraCADGui.runCommand("Arch_ToggleSubs")
...
class ArchSelectionObserver:
    ...
    def addSelection(self, document, object, element, position):
        ...
        if not self.watched:
            AuraCADGui.Selection.removeObserver(AuraCAD.ArchObserver)
            if self.nextCommand:
                AuraCADGui.runCommand(self.nextCommand)
            del AuraCAD.ArchObserver
        elif object == self.watched.Name:
            if not element:
                AuraCAD.Console.PrintMessage(translate("Arch", "Closing Sketch edit"))
                if self.hide:
                    if self.origin:
                ...
                if hasattr(AuraCAD, "ArchObserver"):
                    AuraCADGui.Selection.removeObserver(AuraCAD.ArchObserver)
                    del AuraCAD.ArchObserver
                if self.nextCommand:
                    AuraCADGui.Selection.clearSelection()
                    AuraCADGui.Selection.addSelection(self.watched)
                    AuraCADGui.runCommand(self.nextCommand)
...
class ComponentTaskPanel:
    ...
    def __init__(self):
        ...
        self.iauracadButton = QtGui.QPushButton(self.baseform)
        self.iauracadButton.setObjectName("iauracadButton")
        self.iauracadButton.setIcon(QtGui.QIcon(":/icons/Iauracad.svg"))
        self.grid.addWidget(self.iauracadButton, 4, 0, 1, 2)
        self.iauracadButton.hide()
        ...
        try:
            ...
            if not "BIM_Classification" in AuraCADGui.listCommands():
                AuraCADGui.activateWorkbench("BIMWorkbench")
        ...
        QtCore.QObject.connect(self.iauracadButton, QtCore.SIGNAL("clicked()"), self.editIauracadProperties)
    ...
    def update(self):
        ...
        if self.obj:
            ...
            if hasattr(self.obj, "IauracadProperties"):
                if isinstance(self.obj.IauracadProperties, dict):
                    self.iauracadButton.show()
    ...
    def removeElement(self):
        ...
        element_to_remove = AuraCAD.ActiveDocument.getObject(str(element_selected.toolTip(0)))
    ...
    def editObject(self, wid, col):
        ...
        if wid.parent():
            obj = AuraCAD.ActiveDocument.getObject(str(wid.toolTip(0)))
            if obj:
                ...
                AuraCAD.ArchObserver = ArchSelectionObserver(self.obj, obj)
                AuraCADGui.Selection.addObserver(AuraCAD.ArchObserver)
                AuraCADGui.ActiveDocument.setEdit(obj.Name, 0)
    ...
    def retranslateUi(self, TaskPanel):
        ...
        self.iauracadButton.setText(QtGui.QApplication.translate("Arch", "Edit Iauracad Properties", None))
        self.classButton.setText(QtGui.QApplication.translate("Arch", "Edit Standard Code", None))

    def editIauracadProperties(self):
        """Open the Iauracad editor dialog box.

        This is the method that runs as a callback when the Edit Iauracad properties
        ...
        if hasattr(self, "iauracadEditor"):
            if self.iauracadEditor:
                self.iauracadEditor.hide()
            del self.iauracadEditor
        if not self.obj:
            return
        if not hasattr(self.obj, "IauracadProperties"):
            return
        if not isinstance(self.obj.IauracadProperties, dict):
            return
        import csv
        import os
        import Arch_rc
        ...
        # get presets
        self.ptypes = list(ArchIauracadSchema.IauracadTypes)
        self.plabels = [
            "".join(map(lambda x: x if x.islower() else " " + x, t[3:]))[1:] for t in self.ptypes
        ]
        self.psetdefs = {}
        psetspath = os.path.join(
            AuraCAD.getResourceDir(), "Mod", "Arch", "Presets", "pset_definitions.csv"
        )
        ...
        self.psetkeys.sort()
        self.iauracadEditor = AuraCADGui.PySideUic.loadUi(":/ui/dialogIauracadPropertiesRedux.ui")

        # center the dialog over AuraCAD window
        mw = AuraCADGui.getMainWindow()
        self.iauracadEditor.move(
            mw.frameGeometry().topLeft() + mw.rect().center() - self.iauracadEditor.rect().center()
        )
        self.iauracadModel = QtGui.QStandardItemModel()
        self.iauracadEditor.treeProperties.setModel(self.iauracadModel)
        # self.iauracadEditor.treeProperties.setDragDropMode(QtGui.QAbstractItemView.InternalMove)
        self.iauracadEditor.treeProperties.setUniformRowHeights(True)
        self.iauracadEditor.treeProperties.setItemDelegate(
            IauracadEditorDelegate(dialog=self, ptypes=self.ptypes, plabels=self.plabels)
        )
        self.iauracadModel.setHorizontalHeaderLabels(
            [
        ...
        # set combos
        self.iauracadEditor.comboProperty.addItems(
            [QtGui.QApplication.translate("Arch", "Add property", None)] + self.plabels
        )
        self.iauracadEditor.comboPset.addItems(
            [
        ...
        # set UUID
        if "IauracadUID" in self.obj.IauracadData:
            self.iauracadEditor.labelUUID.setText(self.obj.IauracadData["IauracadUID"])

        # fill the tree
        psets = {}
        for pname, value in self.obj.IauracadProperties.items():
            # properties in IauracadProperties dict are stored as "key":"pset;;type;;value" or "key":"type;;value"
            value = value.split(";;")
            if len(value) == 3:
                pset = value[0]
                ptype = value[1]
        ...
        for pset, plists in psets.items():
            ...
            self.iauracadModel.appendRow([top, QtGui.QStandardItem(), QtGui.QStandardItem()])
            for plist in plists:
        ...
        # span top levels
        idx = self.iauracadModel.invisibleRootItem().index()
        for i in range(self.iauracadModel.rowCount()):
            if self.iauracadModel.item(i, 0).hasChildren():
                self.iauracadEditor.treeProperties.setFirstColumnSpanned(i, idx, True)
        self.iauracadEditor.treeProperties.expandAll()

        # Add callbacks
        QtCore.QObject.connect(
            self.iauracadEditor.buttonBox, QtCore.SIGNAL("accepted()"), self.acceptIauracadProperties
        )
        QtCore.QObject.connect(
            self.iauracadEditor.comboProperty,
            QtCore.SIGNAL("currentIndexChanged(int)"),
            self.addIauracadProperty,
        )
        QtCore.QObject.connect(
            self.iauracadEditor.comboPset, QtCore.SIGNAL("currentIndexChanged(int)"), self.addIauracadPset
        )
        QtCore.QObject.connect(
            self.iauracadEditor.buttonDelete, QtCore.SIGNAL("clicked()"), self.removeIauracadProperty
        )
        self.iauracadEditor.treeProperties.setSortingEnabled(True)

        # set checkboxes
        if "FlagForceBrep" in self.obj.IauracadData:
            self.iauracadEditor.checkBrep.setChecked(self.obj.IauracadData["FlagForceBrep"] == "True")
        if "FlagParametric" in self.obj.IauracadData:
            self.iauracadEditor.checkParametric.setChecked(self.obj.IauracadData["FlagParametric"] == "True")
        self.iauracadEditor.show()

    def acceptIauracadProperties(self):
        """This method runs as a callback when the user selects the ok button in the Iauracad editor.

        Scrape through the rows of the Iauracad editor's items, and compare them to
        the object being edited's .IauracadData. If the two are different, change
        the object's .IauracadData to match the editor's items.
        """

        if hasattr(self, "iauracadEditor") and self.iauracadEditor:
            self.iauracadEditor.hide()
            iauracaddict = {}
            for row in range(self.iauracadModel.rowCount()):
                pset = self.iauracadModel.item(row, 0).text()
                if self.iauracadModel.item(row, 0).hasChildren():
                    for childrow in range(self.iauracadModel.item(row, 0).rowCount()):
                        prop = self.iauracadModel.item(row, 0).child(childrow, 0).text()
                        ptype = self.iauracadModel.item(row, 0).child(childrow, 1).text()
                        if not ptype.startswith("Iauracad"):
                            ptype = self.ptypes[self.plabels.index(ptype)]
                        pvalue = self.iauracadModel.item(row, 0).child(childrow, 2).text()
                        iauracaddict[prop] = pset + ";;" + ptype + ";;" + pvalue
            iauracadData = self.obj.IauracadData
            iauracadData["IauracadUID"] = self.iauracadEditor.labelUUID.text()
            iauracadData["FlagForceBrep"] = str(self.iauracadEditor.checkBrep.isChecked())
            iauracadData["FlagParametric"] = str(self.iauracadEditor.checkParametric.isChecked())
            # The transaction context is implicitly opened by the C++ layer when entering edit mode.
            if iauracaddict != self.obj.IauracadProperties:
                self.obj.IauracadProperties = iauracaddict
            if iauracadData != self.obj.IauracadData:
                self.obj.IauracadData = iauracadData
            del self.iauracadEditor

    def addIauracadProperty(self, idx=0, pset=None, prop=None, ptype=None):
        """Add an Iauracad property to the object, within the Iauracad editor.

        This method runs as a callback when the user selects an option in the
        Add Property dropdown box in the Iauracad editor. When used via the
        ...
        pset: str, optional
            ...
            been selected within the Iauracad editor widget.
        prop: str, optional
        ...
        if hasattr(self, "iauracadEditor") and self.iauracadEditor:
            if not pset:
                sel = self.iauracadEditor.treeProperties.selectedIndexes()
                if sel:
                    item = self.iauracadModel.itemFromIndex(sel[0])
            ...
            if idx != 0:
                self.iauracadEditor.comboProperty.setCurrentIndex(0)

    def addIauracadPset(self, idx=0):
        """Add an Iauracad property set to the object, within the Iauracad editor.
            ...
            if idx != 0:
                ...
                for i in range(self.iauracadModel.rowCount()):
                    if self.iauracadModel.item(i, 0).hasChildren():
                        self.iauracadEditor.treeProperties.setFirstColumnSpanned(i, idx, True)
                self.iauracadEditor.treeProperties.expandAll()
                self.iauracadEditor.comboPset.setCurrentIndex(0)

    def removeIauracadProperty(self):
        """Remove an Iauracad property or property set from the object, within the Iauracad editor.

        This method runs as a callback when the user selects the Delete
        selected property/set button within the Iauracad editor.
        ...
        if hasattr(self, "iauracadEditor") and self.iauracadEditor:
            sel = self.iauracadEditor.treeProperties.selectedIndexes()
            if sel:
                if self.iauracadModel.itemFromIndex(sel[0]).toolTip() == "PropertySet":
                    self.iauracadModel.takeRow(sel[0].row())
                else:
                    pset = self.iauracadModel.itemFromIndex(sel[0].parent())
    ...
    def editClass(self):
        ...
        in the Iauracad editor. It relies on the presence of the BIM module.
        """

        AuraCADGui.Selection.clearSelection()
        AuraCADGui.Selection.addSelection(self.obj)
        AuraCADGui.runCommand("BIM_Classification")
...
class ComponentOptionsTaskPanel(ComponentTaskPanel):
    ...
    def __init__(self, obj, property_definitions):
        """
        obj: The AuraCAD object being edited.
        ...
        for item in self.property_definitions:
            ...
            if prop_type_id in [
                ...
                AuraCADGui.ExpressionBinding(widget).bind(target_obj, prop_name)
            ...
            # Float
                ...
                widget.setDecimals(
                    AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Units").GetInt(
                        "Decimals", 2
                    )
                )
    ...
    def accept(self):
        ...
        for prop_name, data in self.property_widgets.items():
            ...
            try:
                ...
                AuraCAD.Console.PrintError(f"{msg} {prop_name}: {str(e)}\n")
...
if AuraCAD.GuiUp:
    ...
    class IauracadEditorDelegate(QtGui.QStyledItemDelegate):
        """This class manages the editing of the individual table cells in the Iauracad editor.
        ...
        ptypes: list of str
            A list of the names of Iauracad property types.
        plables: list of str
            A list of the human readable names of Iauracad property types.
        """
        ...
        def createEditor(self, parent, option, index):
            ...
            index: <PySide2.QtCore.QModelIndex>
                The index object of the table of the Iauracad editor.
            ...
            if index.column() == 0:  # property name
                ...
                if "Integer" in ptype:
                    ...
                    editor = AuraCADGui.UiLoader().createWidget("Gui::InputField")
        ...
        def setModelData(self, editor, model, index):
            """Write the data in the editor to the Iauracad editor's table.
            ...
            editor: <pyside2.qtwidgets.qwidget>
                The editor widget.
            model:
                The table object of the Iauracad editor.
            index: <PySide2.QtCore.QModelIndex>
                The index object of the table, of the Iauracad editor
            """
