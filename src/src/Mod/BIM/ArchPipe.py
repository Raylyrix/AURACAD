...
class _ArchPipe(ArchComponent.Component):
    ...
    def __init__(self, obj):
        ...
        # IauracadPipeSegment is new in Iauracad4
        from ArchIauracad import IauracadTypes

        if "Pipe Segment" in IauracadTypes:
            obj.IauracadType = "Pipe Segment"
        else:
            # Iauracad2x3 does not know a Pipe Segment
            obj.IauracadType = "Building Element Proxy"
    ...
    def onDocumentRestored(self, obj):
        ...
        if (
            ...
            and AuraCAD.ActiveDocument.getProgramVersion().split()[0] <= "1.1R42474"
        ):
    ...
    def onChanged(self, obj, prop):
        if prop == "IauracadType":
            root = ArchIauracad.IauracadProduct()
            root.setupIauracadAttributes(obj)
            root.setupIauracadComplexAttributes(obj)
    ...
    def execute(self, obj):
        ...
        w = self.getWire(obj)
        if not w:
            AuraCAD.Console.PrintError(translate("Arch", "Unable to build the base path") + "\n")
            return
        ...
        if not p:
            AuraCAD.Console.PrintError(translate("Arch", "Unable to build the profile") + "\n")
            return
        ...
        # rot = AuraCAD.Rotation(v2,v1)
        # rotate keeping up vector
        if v1.getAngle(AuraCAD.Vector(0, 0, 1)) > 0.01:
            up = AuraCAD.Vector(0, 0, 1)
        else:
            up = AuraCAD.Vector(0, 1, 0)
        v1y = up.cross(v1)
        v1x = v1.cross(v1y)
        rot = AuraCAD.Rotation(v1x, v1y, v1, "ZYX")
        ...
        try:
            ...
            AuraCAD.Console.PrintError(translate("Arch", "Unable to build the pipe") + "\n")
        else:
            if len(shapes) == 0:
                return
            elif len(shapes) == 1:
    ...
    def getWire(self, obj):
        ...
        if obj.Base:
            ...
            if obj.Base.Shape.Wires[0].isClosed():
                AuraCAD.Console.PrintError(translate("Arch", "The base wire is closed") + "\n")
                return
            ...
            w = Part.Wire(
                [
                    Part.LineSegment(
                        AuraCAD.Vector(0, 0, 0), AuraCAD.Vector(0, 0, obj.Length.Value)
                    ).toShape()
                ]
            )
        return w
    ...
    def getProfile(self, obj):
        ...
        if obj.Profile:
            if not obj.Profile.getLinkedObject().isDerivedFrom("Part::Part2DObject"):
                AuraCAD.Console.PrintError(translate("Arch", "The profile is not a 2D Part") + "\n")
                return
            if not obj.Profile.Shape.Wires[0].isClosed():
                AuraCAD.Console.PrintError(translate("Arch", "The profile is not closed") + "\n")
                return
            ...
            if obj.ProfileType == "Square":
                ...
                p = Part.makePlane(
                    obj.Width.Value,
                    obj.Width.Value,
                    AuraCAD.Vector(-obj.Width.Value / 2, -obj.Width.Value / 2, 0),
                )
                ...
                p = Part.makePlane(
                    obj.Width.Value,
                    obj.Height.Value,
                    AuraCAD.Vector(-obj.Width.Value / 2, -obj.Height.Value / 2, 0),
                )
                if (
                ...
                p = Part.Wire(
                    [
                        Part.Circle(
                            AuraCAD.Vector(0, 0, 0), AuraCAD.Vector(0, 0, 1), obj.Diameter.Value / 2
                        ).toShape()
                    ]
                )
...
class _ArchPipeConnector(ArchComponent.Component):
    ...
    def execute(self, obj):
        ...
        if wires[0].Vertexes[0].Point.sub(wires[1].Vertexes[0].Point).Length <= ptol:
            ...
            AuraCAD.Console.PrintError(translate("Arch", "Common vertex not found") + "\n")
            return
        if order[0] == "start":
        ...
        if len(obj.Pipes) == 2:
            ...
            if round(v1.getAngle(v2), tol) in [0, round(math.pi, tol)]:
                AuraCAD.Console.PrintError(translate("Arch", "Pipes are already aligned") + "\n")
                return
            normal = v2.cross(v1)
            ...
            if round(v1.getAngle(v2), tol) in [0, round(math.pi, tol)]:
                ...
                AuraCAD.Console.PrintError(translate("Arch", "At least 2 pipes must align") + "\n")
                return
            ...
            p.translate(delta)
            vp = DraftGeomUtils.getNormal(p)
            rot = AuraCAD.Rotation(vp, pair[0])
            p.rotate(p.CenterOfMass, rot.Axis, math.degrees(rot.Angle))
            ...
            p.translate(delta)
            vp = DraftGeomUtils.getNormal(p)
            rot = AuraCAD.Rotation(vp, pair[2])
            p.rotate(p.CenterOfMass, rot.Axis, math.degrees(rot.Angle))
