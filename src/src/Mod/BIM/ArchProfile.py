...
profilefiles = [
    os.path.join(AuraCAD.getResourceDir(), "Mod", "BIM", "Presets", "profiles.csv"),
    os.path.join(os.path.dirname(__file__), "Presets", "profiles.csv"),
    os.path.join(AuraCAD.getUserAppDataDir(), "BIM", "profiles.csv"),
]
...
class _ProfileTSLOT(_Profile):
    ...
    def execute(self, obj):
        ...
        hole = Part.makeCircle(obj.HoleDiameter / 2, AuraCAD.Vector(0, 0, 0))
    ...
    def changeCategory(self, text):
        ...
        for pre in self.presets:
            if pre[1] == text:
                ...
                f = AuraCAD.Units.Quantity(pre[4], AuraCAD.Units.Length).getUserPreferred()
