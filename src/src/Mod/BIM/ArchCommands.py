...
__title__ = "AuraCAD Arch Commands"
...
def getStringList(objects):
    ...
    for o in objects:
        if len(result) > 1:
            result += ","
        result += "AuraCAD.ActiveDocument." + o.Name
    result += "]"
    return result
...
def makeComponent(baseobj=None, name=None, delete=False):
    ...
    if not AuraCAD.ActiveDocument:
        AuraCAD.Console.PrintError("No active document. Aborting\n")
        return
    obj = AuraCAD.ActiveDocument.addObject("Part::FeaturePython", "Component")
    ...
    if baseobj:
        ...
        if hasattr(baseobj, "Shape"):
            ...
            if delete:
                AuraCAD.ActiveDocument.removeObject(baseobj.Name)
            else:
                obj.Base = baseobj
                if AuraCAD.GuiUp:
...
def cloneComponent(obj):
    ...
    if hasattr(obj, "IauracadAttributes"):
        if obj.IauracadAttributes:
            c.IauracadAttributes = obj.IauracadAttributes
    Draft.select(c)
    return c
...
def splitMesh(obj, mark=True):
    ...
    if comps:
        basename = obj.Name
        AuraCAD.ActiveDocument.removeObject(basename)
        for c in comps:
            newobj = AuraCAD.ActiveDocument.addObject("Mesh::Feature", basename)
            newobj.Mesh = c
...
def getCutVolume(cutplane, shapes, clip=False, depth=None):
    ...
    try:
        ...
        AuraCAD.Console.PrintMessage(translate("Arch", "Invalid cut plane") + "\n")
        return None, None, None
    ce = p.CenterOfMass
    ax = p.normalAt(0, 0)
    ...
    if not bb.isCutPlane(ce, ax):
        # AuraCAD.Console.PrintMessage(translate("Arch","No objects are cut by the plane)+"\n")
        return None, None, None
    else:
        corners = [
            AuraCAD.Vector(bb.XMin, bb.YMin, bb.ZMin),
            AuraCAD.Vector(bb.XMin, bb.YMax, bb.ZMin),
            AuraCAD.Vector(bb.XMax, bb.YMin, bb.ZMin),
            AuraCAD.Vector(bb.XMax, bb.YMax, bb.ZMin),
            AuraCAD.Vector(bb.XMin, bb.YMin, bb.ZMax),
            AuraCAD.Vector(bb.XMin, bb.YMax, bb.ZMax),
            AuraCAD.Vector(bb.XMax, bb.YMin, bb.ZMax),
            AuraCAD.Vector(bb.XMax, bb.YMax, bb.ZMax),
        ]
        for c in corners:
            dv = c.sub(ce)
...
def getShapeFromMesh(mesh, fast=True, tolerance=0.001, flat=False, cut=True):
    ...
    if mesh.isSolid() and (mesh.countComponents() == 1) and fast:
        ...
        for f in mesh.Facets:
            ...
            pts = []
            for pp in p:
                pts.append(AuraCAD.Vector(pp[0], pp[1], pp[2]))
            try:
...
def meshToShape(obj, mark=True, fast=True, tol=0.001, flat=False, cut=True):
    ...
    if "Mesh" in obj.PropertiesList:
        ...
        if solid:
            ...
            newobj = AuraCAD.ActiveDocument.addObject("Part::Feature", name)
            newobj.Shape = solid
    ...
    for o in objectslist:
        ...
        for c in o.Components:
            if not c in l:
                l.append(c)
        base.Components = l
        AuraCAD.ActiveDocument.removeObject(o.Name)
...
def download(url, force=False):
    ...
    name = url.split("/")[-1]
    macropath = AuraCAD.getUserMacroDir(True)
    filepath = os.path.join(macropath, name)
    if os.path.exists(filepath) and not (force):
        return filepath
    try:
        AuraCAD.Console.PrintMessage("Downloading " + url + " â€¦\n")
...
def pruneIncluded(objectslist, strict=False, silent=False):
    ...
    for obj in objectslist:
        ...
        if toplevel:
            ...
            AuraCAD.Console.PrintWarning("pruning " + obj.Label + "\n")
    return newlist
...
def survey(callback=False):
    ...
    if not callback:
        if hasattr(AuraCAD, "SurveyObserver"):
            for label in AuraCAD.SurveyObserver.labels:
                AuraCAD.ActiveDocument.removeObject(label)
            AuraCADGui.Selection.removeObserver(AuraCAD.SurveyObserver)
            del AuraCAD.SurveyObserver
            AuraCADGui.Control.closeDialog()
            if hasattr(AuraCAD, "SurveyDialog"):
                del AuraCAD.SurveyDialog
        else:
            AuraCAD.SurveyObserver = _SurveyObserver(callback=survey)
            AuraCADGui.Selection.addObserver(AuraCAD.SurveyObserver)
            AuraCAD.SurveyDialog = SurveyTaskPanel()
            AuraCADGui.Control.showDialog(AuraCAD.SurveyDialog)
    else:
        sel = AuraCADGui.Selection.getSelectionEx()
        if hasattr(AuraCAD, "SurveyObserver"):
            if not sel:
                if AuraCAD.SurveyObserver.labels:
                    for label in AuraCAD.SurveyObserver.labels:
                        AuraCAD.ActiveDocument.removeObject(label)
                    tl = AuraCAD.SurveyObserver.totalLength
                    ta = AuraCAD.SurveyObserver.totalArea
                    AuraCAD.SurveyObserver.labels = []
                    AuraCAD.SurveyObserver.selection = []
                    AuraCAD.SurveyObserver.totalLength = 0
                    AuraCAD.SurveyObserver.totalArea = 0
                    AuraCAD.SurveyObserver.totalVolume = 0
                    if not AuraCAD.SurveyObserver.cancellable:
                        AuraCAD.Console.PrintMessage("\n---- Reset ----\n\n")
                        AuraCAD.SurveyObserver.cancellable = True
                        if hasattr(AuraCAD, "SurveyDialog"):
                            AuraCAD.SurveyDialog.newline(tl, ta)
                    else:
                        AuraCADGui.Selection.removeObserver(AuraCAD.SurveyObserver)
                        del AuraCAD.SurveyObserver
                        AuraCADGui.Control.closeDialog()
                ...
                AuraCAD.SurveyObserver.cancellable = False
                basesel = AuraCAD.SurveyObserver.selection
                newsels = []
                for o in sel:
                    found = False
                    for eo in basesel:
                ...
                if newsels:
                    for o in newsels:
                        if hasattr(o.Object, "Shape"):
                            ...
                            if not o.HasSubObjects:
                                ...
                                AuraCAD.SurveyObserver.labels.append(anno.Name)
                                if o.Object.Shape.Solids:
                                    u = AuraCAD.Units.Quantity(
                                        o.Object.Shape.Volume, AuraCAD.Units.Volume
                                    )
                                    ...
                                    AuraCAD.SurveyObserver.totalVolume += u.Value
                                elif o.Object.Shape.Faces:
                                    u = AuraCAD.Units.Quantity(
                                        o.Object.Shape.Area, AuraCAD.Units.Area
                                    )
                                    t = u.getUserPreferred()[0]
                                    t = t.replace("^2", "Â²")
                                    anno.LabelText = "a " + t
                                    ...
                                    AuraCAD.SurveyObserver.totalArea += u.Value
                                    if hasattr(AuraCAD, "SurveyDialog"):
                                        AuraCAD.SurveyDialog.update(2, t)
                                else:
                                    u = AuraCAD.Units.Quantity(
                                        o.Object.Shape.Length, AuraCAD.Units.Length
                                    )
                                    t = u.getUserPreferred()[0]
                                    anno.LabelText = "l " + t
                                    AuraCAD.Console.PrintMessage(
                                    ...
                                    AuraCAD.SurveyObserver.totalLength += u.Value
                                    if hasattr(AuraCAD, "SurveyDialog"):
                                        AuraCAD.SurveyDialog.update(1, t)
                                if AuraCAD.GuiUp and t:
                                    if showUnit:
                                ...
                                for el in o.SubElementNames:
                                    ...
                                    AuraCAD.SurveyObserver.labels.append(anno.Name)
                                    if "Face" in el:
                                        u = AuraCAD.Units.Quantity(e.Area, AuraCAD.Units.Area)
                                        t = u.getUserPreferred()[0]
                                        t = t.replace("^2", "Â²")
                                        anno.LabelText = "a " + t
                                        AuraCAD.Console.PrintMessage(
                                        ...
                                        u = AuraCAD.Units.Quantity(e.Length, AuraCAD.Units.Length)
                                        t = u.getUserPreferred()[0]
                                        anno.LabelText = "l " + t
                                        AuraCAD.Console.PrintMessage(
                                            "Object: "
                                        ...
                                        u = AuraCAD.Units.Quantity(e.Z, AuraCAD.Units.Length)
                                        t = u.getUserPreferred()[0]
                                        anno.LabelText = "z " + t
                                        AuraCAD.Console.PrintMessage(
                                            "Object: "
                    ...
                    AuraCAD.SurveyObserver.selection.extend(newsels)
            if hasattr(AuraCAD, "SurveyObserver"):
                if (
                    AuraCAD.SurveyObserver.totalLength
                    or AuraCAD.SurveyObserver.totalArea
                    or AuraCAD.SurveyObserver.totalVolume
                ):
                    msg = " Total:"
                    if AuraCAD.SurveyObserver.totalLength:
                        u = AuraCAD.Units.Quantity(
                            AuraCAD.SurveyObserver.totalLength, AuraCAD.Units.Length
                        )
                        t = u.getUserPreferred()[0]
                        msg += " Length: " + t
                    if AuraCAD.SurveyObserver.totalArea:
                        u = AuraCAD.Units.Quantity(
                            AuraCAD.SurveyObserver.totalArea, AuraCAD.Units.Area
                        )
                        t = u.getUserPreferred()[0]
                        t = t.replace("^2", "Â²")
                        msg += " Area: " + t
                    if AuraCAD.SurveyObserver.totalVolume:
                        u = AuraCAD.Units.Quantity(
                            AuraCAD.SurveyObserver.totalVolume, AuraCAD.Units.Volume
                        )
                        t = u.getUserPreferred()[0]
                        t = t.replace("^3", "Â³")
                        msg += " Volume: " + t
...
class SurveyTaskPanel:
    ...
    def clipLength(self):
        if hasattr(AuraCAD, "SurveyObserver"):
            u = AuraCAD.Units.Quantity(AuraCAD.SurveyObserver.totalLength, AuraCAD.Units.Length)
            t = u.getUserPreferred()[0]
    ...
    def clipArea(self):
        if hasattr(AuraCAD, "SurveyObserver"):
            u = AuraCAD.Units.Quantity(AuraCAD.SurveyObserver.totalArea, AuraCAD.Units.Area)
            t = u.getUserPreferred()[0]
            t = t.replace("^2", "Â²")
            if params.get_param_arch("surveyUnits"):
                QtGui.QApplication.clipboard().setText(t)
    ...
    def newline(self, length=0, area=0):
        ...
        self.tree.setCurrentItem(item)
        if length:
            u = AuraCAD.Units.Quantity(length, AuraCAD.Units.Length)
            t = u.getUserPreferred()[0]
            item.setText(1, t)
        if area:
            u = AuraCAD.Units.Quantity(area, AuraCAD.Units.Area)
            t = u.getUserPreferred()[0]
            t = t.replace("^2", "Â²")
            item.setText(2, t)
        if length or area:
            ...
            if filename:
                with open(filename[0].encode("utf8"), "w") as csvfile:
                    ...
                    for i in range(rows):
                        ...
                        if item.text(1):
                            u = AuraCAD.Units.Quantity(item.text(1))
                            ...
                            row.extend(["", ""])
                        if item.text(2):
                            t = item.text(2).replace("Â²", "^2")
                            u = AuraCAD.Units.Quantity(t)
                            if item.toolTip(0) == "total":
...
def toggleIauracadBrepFlag(obj):
    """toggleIauracadBrepFlag(obj): toggles the Iauracad brep flag of the given object, forcing it
    to be exported as brep geometry or not."""
    if not hasattr(obj, "IauracadData"):
        AuraCAD.Console.PrintMessage(
            translate("Arch", "Object does not have settable Iauracad attributes")
        )
    else:
        d = obj.IauracadData
...
def makeCompoundFromSelected(objects=None):
    ...
    so = []
    if not objects:
        objects = AuraCADGui.Selection.getSelectionEx()
...
def rebuildArchShape(objects=None):
    ...
    if not objects and AuraCAD.GuiUp:
        objects = AuraCADGui.Selection.getSelection()
    if not isinstance(objects, list):
        objects = [objects]
    for obj in objects:
...
def makeIauracadSpreadsheet(archobj=None):
    iAuraCAD_container = None
    for obj in AuraCAD.ActiveDocument.Objects:
        if obj.Name == "IauracadPropertiesContainer":
            iAuraCAD_container = obj
    if not iAuraCAD_container:
        iAuraCAD_container = AuraCAD.ActiveDocument.addObject(
            "App::DocumentObjectGroup", "IauracadPropertiesContainer"
        )
    import Spreadsheet

    iAuraCAD_spreadsheet = AuraCAD.ActiveDocument.addObject("Spreadsheet::Sheet", "IauracadProperties")
    iAuraCAD_spreadsheet.set("A1", translate("Arch", "Category"))
    iAuraCAD_spreadsheet.set("B1", translate("Arch", "Key"))
    iAuraCAD_spreadsheet.set("C1", translate("Arch", "Type"))
    iAuraCAD_spreadsheet.set("D1", translate("Arch", "Value"))
    iAuraCAD_spreadsheet.set("E1", translate("Arch", "Unit"))
    iAuraCAD_container.addObject(iAuraCAD_spreadsheet)
    if archobj:
        if hasattr(obj, "IauracadProperties"):
            archobj.IauracadProperties = iAuraCAD_spreadsheet
            return iAuraCAD_spreadsheet
        else:
            AuraCAD.Console.PrintWarning(
                translate(
                    "Arch",
                    "The object does not have an IauracadProperties attribute. Cancel spreadsheet creation for object:",
                )
                + " "
                + archobj.Label
            )
            AuraCAD.ActiveDocument.removeObject(iAuraCAD_spreadsheet)
    else:
        return iAuraCAD_spreadsheet
...
def override_link_properties(link_obj, prop_names):
    ...
    See: https://wiki.AuraCAD.org/Std_LinkMake#Copy_on_Change
