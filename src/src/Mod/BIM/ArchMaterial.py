...
class _ViewProviderArchMaterialContainer:
    ...
    def doubleClicked(self, vobj):
        ...
        is not handled).
        """
        AuraCADGui.runCommand("BIM_Material")
        return True
    ...
    def mergeByName(self):
        if hasattr(self, "Object"):
            ...
            for tod in todelete:
                if not tod.InList:
                    AuraCAD.Console.PrintMessage("Merging duplicate material " + tod.Label + "\n")
...
class _ViewProviderArchMaterial:
    ...
    def setEdit(self, vobj, mode):
        ...
        AuraCADGui.Control.showDialog(self.taskd)
...
class _ArchMaterialTaskPanel:
    ...
    def __init__(self, obj=None):
        ...
        self.obj = obj
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/ArchMaterial.ui")
    ...
    def chooseMat(self, card):
        ...
        if card in self.cards:
            import importauracadMat

            self.material = importauracadMat.read(self.cards[card])
            self.setFields()
    ...
    def fillMaterialCombo(self):
        "fills the combo with the existing auracadMat cards"
        ...
        user_mat_path = os.path.join(AuraCAD.ConfigGet("UserAppData"), "Material")
...
class _ArchMultiMaterialTaskPanel:
    ...
    def __init__(self, obj=None):
        self.obj = obj
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/ArchMultiMaterial.ui")
        self.model = QtGui.QStandardItemModel()
        unit_str = AuraCAD.Units.Quantity(1, AuraCAD.Units.Length).getUserPreferred()[2]
            ...
            for i in range(len(obj.Names)):
                ...
                item3 = QtGui.QStandardItem(
                    AuraCAD.Units.Quantity(
                        obj.Thicknesses[i], AuraCAD.Units.Length
                    ).getUserPreferred()[0]
                )
    ...
    def _parse_thickness(text):
        ...
        pref_factor = AuraCAD.Units.Quantity(1, AuraCAD.Units.Length).getUserPreferred()[1]
        try:
            return float(text) * pref_factor
        except ValueError:
            return AuraCAD.Units.Quantity(text).Value
    ...
    def recalcThickness(self, item=None):
        ...
        val = AuraCAD.Units.Quantity(th, AuraCAD.Units.Length).UserString
