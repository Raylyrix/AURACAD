...
class ChunkMetaclass(type):
    ...
    def __init__(self, name, bases, clsdict):
        ...
        if hasattr(self, "keyframe"):
            ...
            slots = ["frameno", "flags", "tension", "continuity", "bias", "ease_to", "ease_from"]
...
class ErrorChunk(UndefinedChunk):
    ...
    def __init__(self, intended_tag=None, intended_label=None, error_msg=None):
        super().__init__(
            intended_tag=intended_tag, intended_label=intended_label, error_msg=error_msg
        )
...
class OneColorChunk(ChunkBase):
    single = "COLOR_F color, COLOR_24 color," "LIN_COLOR_F lincolor, LIN_COLOR_24 lincolor"
    ...
    def dump_array(self, flo, indent, flags):
        ...
        for i in range(4):
            flo.write(
                ...
                % (indent, self.array[i, 0], self.array[i, 1], self.array[i, 2], self.array[i, 3])
            )
...
class V_GRADIENT(ChunkBase):
    ...
    multiple = "COLOR_F color, LIN_COLOR_F lincolor," "COLOR_24 color, LIN_COLOR_24 lincolor"
...
class FOG(ChunkBase):
    tag = 0x2200
    struct = "float near_plane, float near_density," "float far_plane, float far_density"
...
class MESH_TEXTURE_INFO(MatrixChunk):
    ...
    struct = (
        "float x_tiling, float y_tiling, float icon_x," "float icon_y, float icon_z, float scaling"
    )
...
class MSH_BOXMAP(ChunkBase):
    tag = 0x4190
    struct = "string front, string back, string left," "string right, string top, string bottom"
    swallow = True
...
class DL_SPOTLIGHT(ChunkBase):
    tag = 0x4610
    struct = "float spot_x, float spot_y, float spot_z," "float hotspot_angle, float falloff_angle"
    single = (
...
class KFDATA(ChunkBase):
    tag = 0xB000
    single = "KFHDR kfhdr, KFSEG kfseg, KauracadURTIME kauracadurtime"
    multiple = (
...
class KauracadURTIME(ChunkBase):
    tag = 0xB009
...
class LIGHT_NODE_TAG(ChunkBase):
    tag = 0xB005
    single = (
        "NODE_ID node_id," "NODE_HDR node_hdr," "POS_TRACK_TAG pos_track," "COL_TRACK_TAG col_track"
    )
...
class BOUNDBOX(ChunkBase):
    tag = 0xB014
    struct = "float min_x, float min_y, float min_z," "float max_x, float max_y, float max_z"
