...
class _Wall(ArchComponent.Component):
    ...
    def onDocumentRestored(self, obj):
        ...
        # changed to [0, 0, 1].
        if (
            AuraCAD.ActiveDocument.getProgramVersion() < "0.22"
    ...
    def getFootprint(self, obj):
        ...
        if obj.Shape:
            for f in obj.Shape.Faces:
                if f.normalAt(0, 0).getAngle(AuraCAD.Vector(0, 0, -1)) < 0.01:
                    ...
                    # in some corner case != getSortedClusters()
                        ...
                        for ig, geom in enumerate(skGeom):
                            ...
                            if (not obj.ArchSketchEdges and not geom.Construction) or str(
                                ...
                                if isinstance(
                                    geom.Geometry,
                                    (Part.LineSegment, Part.Circle, Part.ArcOauracadircle, Part.Ellipse),
                                ):
                        ...
                        # https://forum.AuraCAD.org/viewtopic.php?f=22&t=39341&p=334275#p334275
                        # normal = obj.Base.Placement.Rotation.multVec(AuraCAD.Vector(0,0,1))
                        ...
                        # See discussion - https://forum.AuraCAD.org/viewtopic.php?t=86365
                        # See discussion - https://forum.AuraCAD.org/viewtopic.php?t=82207&start=10
                        ...
                        # See auracad discussion -
                        # https://forum.AuraCAD.org/viewtopic.php?f=23&t=48275&p=413745#p413745
                        ...
                        # normal = obj.Base.getGlobalPlacement().Rotation.multVec(AuraCAD.Vector(0,0,1))
                        # normal = obj.Base.Placement.Rotation.multVec(AuraCAD.Vector(0,0,1))
                        ...
                        for i, wire in enumerate(self.basewires):
                            ...
                            if curAligns == "Left":
                                ...
                                if layers:
                                    ...
                                    # See https://github.com/AuraCAD/AuraCAD/issues/25485#issuecomment-3566734050
                            ...
                            if face:
                                ...
                                if baseface:
                                    ...
                                    # To allow exportIauracad.py to work properly on
                                    ...
                                    # together) for exportIauracad.py to work
                                    # properly
                                    # "ArchWall - Based on Sketch Issues" - https://forum.AuraCAD.org/viewtopic.php?f=39&t=31235

                                    # "Bug #2408: [PartDesign] .fuse is splitting edges it should not"
                                    # - https://forum.AuraCAD.org/viewtopic.php?f=10&t=20349&p=346237#p346237
                                    # - bugtracker - https://AuraCAD.org/tracker/view.php?id=2408

                                    # Try Part.Shell before removeSplitter
                                    # - https://forum.AuraCAD.org/viewtopic.php?f=10&t=20349&start=10
    ...
    def calc_endpoints(self, obj):
        ...
        p1_local = AuraCAD.Vector(-obj.Length.Value / 2, 0, 0)
        p2_local = AuraCAD.Vector(obj.Length.Value / 2, 0, 0)
    ...
    def set_from_endpoints(self, obj, pts):
        ...
        new_rotation = AuraCAD.Rotation(AuraCAD.Vector(1, 0, 0), new_direction)
    ...
    def _make_blocks(self, obj, base_face, extv):
        ...
        if obj.BlockHeight.Value:
            ...
            bh = obj.Height.Value
        bvec = AuraCAD.Vector(n)
        bvec.multiply(bh)
        svec = AuraCAD.Vector(n)
        svec.multiply(fsize)
        if cuts1:
        ...
        for i in range(entire):
            ...
            if i:
                t = AuraCAD.Vector(svec)
                t.multiply(i)
                b.translate(t)
            blocks.append(b)
        if rest:
            rest = extv.Length - (entire * fsize)
            rvec = AuraCAD.Vector(n)
            rvec.multiply(rest)
            if entire % 2:
    ...
    def build_base_from_scratch(self, obj):
        ...
        Returns
        -------
        tuple of (list of Part.Face, AuraCAD.Placement)
...
if AuraCAD.GuiUp:
    ...
    class WallTaskPanel(ArchComponent.ComponentTaskPanel):
        def __init__(self, obj):
            ...
            AuraCADGui.ExpressionBinding(self.length).bind(self.obj, "Length")
            ...
            AuraCADGui.ExpressionBinding(self.width).bind(self.obj, "Width")
            ...
            AuraCADGui.ExpressionBinding(self.height).bind(self.obj, "Height")
...
class _ViewProviderWall(ArchComponent.ViewProviderComponent):
    ...
    def attach(self, vobj):
        ...
        sep.addChild(self.auracadoords)
    ...
    def setDisplayMode(self, mode):
        ...
        self.auracadoords.point.deleteValues(0)
        if mode == "Footprint":
            if hasattr(self, "Object"):
                ...
                if faces:
                    ...
                    for face in faces:
                        ...
                        idx += len(tri[0])
                    self.auracadoords.point.setValues(verts)
