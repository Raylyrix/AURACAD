...
__title__ = "AuraCAD Arch External Reference"
...
class ArchReference:
    ...
    def execute(self, obj):
        ...
        if filename and self.reload and obj.ReferenceMode in ["Normal", "Transient"]:
            ...
            if self.parts:
                if filename.lower().endswith(".auracadstd"):
                    ...
                    if zdoc:
                        ...
                        obj.Placement = pl
                elif filename.lower().endswith(".iauracad"):
                    iauracadfile = self.getIauracadFile(filename)
                    if not iauracadfile:
                        return
                    try:
                        from nativeiauracad import iAuraCAD_tools
                        from nativeiauracad import iAuraCAD_generator
                    except:
                        t = translate(
                            "Arch", "NativeIauracad not available - unable to process Iauracad files"
                        )
                        AuraCAD.Console.PrintError(t + "\n")
                        return
                    elements = self.getIauracadElements(obj, iauracadfile)
                    shape, colors = iAuraCAD_generator.generate_shape(iauracadfile, elements, cached=True)
                    if shape:
                        placement = shape.Placement
                        obj.Shape = shape
                        obj.Placement = placement
                        if colors:
                            iAuraCAD_tools.set_colors(obj, colors)
                ...
                hGrp = AuraCAD.ParamGet(loc).GetGroup("RefDxfImport")
    ...
    def getIauracadElements(self, obj, iauracadfile):
        """returns Iauracad elements for this object"""

        try:
            from nativeiauracad import iAuraCAD_generator
        except:
            t = translate("Arch", "NativeIauracad not available - unable to process Iauracad files")
            AuraCAD.Console.PrintError(t + "\n")
            return
        if obj.Part:
            element = iauracadfile[int(obj.Part)]
        else:
            element = iauracadfile.by_type("IauracadProject")[0]
        elements = iAuraCAD_generator.get_decomposed_elements(element)
        elements = iAuraCAD_generator.filter_types(elements)
        return elements
    ...
    def getFile(self, obj, filename=None):
        ...
        if not filename.lower().endswith(".auracadstd"):
            if not filename.lower().endswith(".iauracad"):
    ...
    def getPartsList(self, obj, filename=None):
        ...
        if not filename:
            return None
        if filename.lower().endswith(".auracadstd"):
            return self.getPartsListauracadSTD(obj, filename)
        elif filename.lower().endswith(".iauracad"):
            return self.getPartsListIauracad(obj, filename)
        elif filename.lower().endswith(".dxf"):
    ...
    def getPartsListIauracad(self, obj, filename):
        """returns a list of Part-based objects in a Iauracad file"""

        iauracadfile = self.getIauracadFile(filename)
        if not iauracadfile:
            return None
        structs = iauracadfile.by_type("IauracadSpatialElement")
        res = {}
        for s in structs:
            n = s.Name
            if not n:
    ...
    def getPartsListauracadSTD(self, obj, filename):
        """returns a list of Part-based objects in a auracadStd file"""
    ...
    def getIauracadFile(self, filename):
        """Gets an IauracadOpenShell object"""
        ...
        try:
            ...
            AuraCAD.Console.PrintError(t + "\n")
            return None
        if not getattr(self, "iauracadfile", None):
            self.iauracadfile = iauracadopenshell.open(filename)
        return self.iauracadfile
    ...
    def _getColorsPart(self, filename, part):
        ...
        if writemode2:
            ...
            # File format ShapeAppearance files in auracadStd file:
...
class ViewProviderArchReference:
    ...
    def onOpen(self):
        ...
        if hasattr(self, "Object") and self.Object:
            if self.Object.File:
                AuraCAD.openDocument(self.Object.File)
    ...
    def loadInventor(self, obj):
        ...
        if not filename:
            return None
        if filename.lower().endswith(".iauracad"):
            self.setIauracadNode(obj, filename)
            return
        ...
        if not ivstring:
            ...
            AuraCAD.Console.PrintWarning(t + " " + obj.Label + "\n")
            return
        ...
        if not isinstance(lwnode, coin.SoSeparator):
            ...
            AuraCAD.Console.PrintError(t + " " + obj.Label + "\n")
            return
    ...
    def setIauracadNode(self, obj, filename):
        """Sets the coin node of this object from an Iauracad file"""
        ...
        try:
            ...
            AuraCAD.Console.PrintError(t + "\n")
            return
        iauracadfile = obj.Proxy.getIauracadFile(filename)
        elements = obj.Proxy.getIauracadElements(obj, iauracadfile)
        node, placement = iAuraCAD_generator.generate_coin(iauracadfile, elements, cached=True)
        if node:
            iAuraCAD_generator.set_representation(obj.ViewObject, node)
            colors = node[0]
            if colors:
                iAuraCAD_tools.set_colors(obj, colors)
        else:
            iAuraCAD_generator.set_representation(obj.ViewObject, None)
        if placement:
            obj.Placement = placement
...
class ArchReferenceTaskPanel:
    ...
    def accept(self):
        ...
        if self.partCombo.currentText():
            ...
            if i >= 1:
                ...
                self.obj.Part = ""
            QtCore.QTimer.singleShot(0, AuraCAD.ActiveDocument.recompute)
    ...
    def chooseFile(self):
        ...
        filters = "*.auracadStd *.dxf"
        # enable Iauracad support if NativeIauracad is present
        try:
            from nativeiauracad import iAuraCAD_tools
        except:
            pass
    ...
    def openFile(self):
        ...
        if self.obj.File:
            if self.obj.File.lower().endswith(".auracadstd"):
                AuraCAD.openDocument(self.obj.File)
            else:
                AuraCAD.loadFile(self.obj.File)
            AuraCADGui.Control.closeDialog()
            AuraCADGui.ActiveDocument.resetEdit()
