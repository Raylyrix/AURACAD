...
# AuraCAD-Reporting workbench (https://github.com/furti/AuraCAD-Reporting).
...
def _get_property(obj, prop_name):
    """Gets a property from a AuraCAD object, including sub-properties.
    ...
    if not is_nested_property(prop_name):
        ...
        if hasattr(obj, prop_name):
            ...
            if isinstance(value, AuraCAD.DocumentObject):
                return value.Label
            return value
        return None
    else:
...
def _map_results_to_objects(headers, data_rows):
    """
    Maps the raw data rows from a query result back to AuraCAD DocumentObjects.
    ...
    Returns
    -------
    list of AuraCAD.DocumentObject
        A list of unique AuraCAD DocumentObject instances that correspond to the
    ...
    objects_by_name = {o.Name: o for o in AuraCAD.ActiveDocument.Objects}
    ...
    if "Name" in headers:
        ...
        objects_by_label = {o.Label: o for o in AuraCAD.ActiveDocument.Objects}
...
def _get_bim_type(obj):
    """
    Gets the most architecturally significant type for a AuraCAD object.

    This is a specialized utility for BIM reporting. It prioritizes explicit
    BIM properties (.IauracadType) to correctly distinguish between different Arch
    ...
    # 1. Prioritize the explicit IauracadType for architectural objects.
    #    This correctly handles Door vs. Window and returns the raw value.
    if hasattr(obj, "IauracadType"):
        if obj.IauracadType and obj.IauracadType != "Undefined":
            return obj.IauracadType

    # 2. Check for legacy .Class property from old Iauracad imports.
    if hasattr(obj, "Class") and "Iauracad" in str(obj.Class):
        return obj.Class
...
def _get_direct_children(obj, discover_hosted_elements, include_components_from_additions):
    ...
    Encapsulates the different ways an object can be a "child" in AuraCAD's BIM context, checking
...
@register_select_function(
    ...
    snippet="SELECT COUNT(*) FROM document WHERE IauracadType = 'Space'",
)
...
@register_select_function(
    ...
    snippet="SELECT SUM(Area) FROM document WHERE IauracadType = 'Space'",
)
...
@register_select_function(
    ...
    snippet="SELECT MIN(Length) FROM document WHERE IauracadType = 'Wall'",
)
...
@register_select_function(
    ...
    snippet="SELECT MAX(Height) FROM document WHERE IauracadType = 'Wall'",
)
...
@register_select_function(
    ...
    snippet="SELECT Label FROM document WHERE UPPER(IauracadType) = 'WALL'",
)
...
@register_select_function(
    ...
    snippet="SELECT CONCAT(Label, ': ', IauracadType) FROM document",
)
...
class ConvertFunction(FunctionBase):
    ...
    def get_value(self, obj):
        ...
        if not isinstance(input_value, AuraCAD.Units.Quantity):
            raise SqlEngineError(
...
class FromFunctionBase:
    ...
    def _get_parent_objects(self, source_objects=None):
        ...
        into a list of AuraCAD document objects.
        """
        ...
        headers, rows = self.args.execute(AuraCAD.ActiveDocument.Objects)
...
class ChildrenRecursiveFromFunction(FromFunctionBase):
    ...
    def _get_parent_objects_from_subquery(self, substatement, source_objects=None):
        ...
        if source_objects:
            ...
            headers, rows = substatement.execute(AuraCAD.ActiveDocument.Objects)
...
@register_select_function(
    ...
    snippet="SELECT Label, PARENT(*).Label AS Floor FROM document WHERE IauracadType = 'Space'",
)
        ...
        if self.order_by_clause:
            ...
            def sort_key(row):
                ...
                for index in sort_column_indices:
                    ...
                    elif isinstance(value, (int, float, AuraCAD.Units.Quantity)):
                        num_val = (
                            value.Value if isinstance(value, AuraCAD.Units.Quantity) else value
                        )
                    ...
                    if extractor.function_name == "count":
                        ...
                        for obj in object_list:
                            ...
                            if prop_val is not None:
                                # Handle AuraCAD.Quantity by using its value
                                if isinstance(prop_val, AuraCAD.Units.Quantity):
...
class BooleanComparison:
    ...
    def evaluate(self, obj):
        ...
        if isinstance(left_val, AuraCAD.Units.Quantity):
            left_val = left_val.Value
        if isinstance(right_val, AuraCAD.Units.Quantity):
...
class ReferenceExtractor:
    ...
    def get_objects(self, source_objects=None):
        ...
        if self.value == "document" and not self.base:
            found_objects = AuraCAD.ActiveDocument.Objects
            return found_objects
        return []
...
try:
    ...
    AuraCAD.Console.PrintError(f"BIM SQL engine failed to initialize: {e}\n")
...
def _run_query(query_string: str, mode: str, source_objects: Optional[List] = None):
    ...
    source_objects : list of AuraCAD.DocumentObject, optional
...
class ReportStatement:
    ...
    allowing it to be persisted within a AuraCAD document object.
...
def selectObjectsFromPipeline(statements: list) -> list:
    """
    Executes a multi-statement pipeline and returns a final list of AuraCAD objects.
    ...
    -------
    list of AuraCAD.DocumentObject
        A list of the final AuraCAD.DocumentObject instances that result from
        the pipeline.
    """
...
def selectObjects(query_string: str) -> List["AuraCAD.DocumentObject"]:
    """
    ...
    query_string : str
        ...
        'SELECT * FROM document WHERE IauracadType = "Wall" AND Label LIKE "%exterior%"'

    Returns
    -------
    list of App::DocumentObject
        A list of the AuraCAD document objects that match the query.
        Returns an empty list if the query is invalid or finds no objects.
    """
    if not AuraCAD.ActiveDocument:
        AuraCAD.Console.PrintError("Arch.selectObjects() requires an active document.\n")
        return []
    ...
    try:
        ...
        AuraCAD.Console.PrintError(f"Arch.selectObjects query failed: {e}\n")
        return []
