...
def add_geom_properties(obj):
    """Adds geometry properties to a AuraCAD object"""

    element = iAuraCAD_tools.get_iAuraCAD_element(obj)
    if not iAuraCAD_tools.has_representation(element):
        return
    iauracadfile = iAuraCAD_tools.get_iauracadfile(obj)
    scaling = iauracadopenshell.util.unit.calculate_unit_scale(iauracadfile)
    scaling = scaling * 1000  # given scale is for m, we work in mm
    for rep in element.Representation.Representations:
        if rep.RepresentationIdentifier == "Body":
            if len(rep.Items) == 1:
                # Extrusions
                if rep.Items[0].is_a("IauracadExtrudedAreaSolid"):
                    ext = rep.Items[0]
                    ...
                    obj.ExtrusionDirection = AuraCAD.Vector(ext.ExtrudedDirection.DirectionRatios)

                    # Extrusion of a rectangle
                    if ext.SweptArea.is_a("IauracadRectangleProfileDef"):
                    ...
                    elif ext.SweptArea.is_a("IauracadArbitraryClosedProfileDef"):
                        if ext.SweptArea.OuterCurve.is_a("IauracadPolyline"):
                            ...
                            points = [AuraCAD.Vector(p).multiply(scaling) for p in points]
                            obj.PolylinePoints = points

                    # I profile
                    elif ext.SweptArea.is_a("IauracadIShapeProfileDef"):
                        for p in [
                            "FilletRadius",
                            "FlangeEdgeRadius",
                            "FlangeSlope",
        ...
        # below is disabled for now... Don't know if it's useful to expose to the user
            ...
            if len(rep.Items) == 1:
                if rep.Items[0].is_a("IauracadCompositeCurve"):
                    if len(rep.Items[0].Segments) == 1:
                        if rep.Items[0].Segments[0].is_a("IauracadCompositeCurveSegment"):
                            if rep.Items[0].Segments[0].ParentCurve.is_a("IauracadPolyline"):
                                ...
                                if len(pol.Points) == 2:
                                    ...
                                    obj.AxisEnd = AuraCAD.Vector(
                                        pol.Points[1].Coordinates
                                    ).multiply(scaling)


def set_attribute(iauracadfile, element, prop, value):
...
def set_geom_property(obj, prop):
    """Updates the internal Iauracad file with the given value"""
    ...
    if prop == "ExtrusionDepth":
            ...
            if rep.RepresentationIdentifier == "Body":
                if len(rep.Items) == 1:
                    if rep.Items[0].is_a("IauracadExtrudedAreaSolid"):
                        ...
                        changed = set_attribute(iauracadfile, elem, "Depth", value)
            ...
            if rep.RepresentationIdentifier == "Body":
                if len(rep.Items) == 1:
                    if rep.Items[0].is_a("IauracadExtrudedAreaSolid"):
                        ...
                        changed = set_attribute(iauracadfile, elem, "DirectionRatios", value)
            ...
            if rep.RepresentationIdentifier == "Body":
                if len(rep.Items) == 1:
                    if rep.Items[0].is_a("IauracadExtrudedAreaSolid"):
                        ...
                        if elem.is_a("IauracadRectangleProfileDef"):
                            value = getattr(obj, prop).Value * scaling
                            changed = set_attribute(iauracadfile, elem, "XDim", value)
            ...
            if rep.RepresentationIdentifier == "Body":
                if len(rep.Items) == 1:
                    if rep.Items[0].is_a("IauracadExtrudedAreaSolid"):
                        ...
                        if elem.is_a("IauracadRectangleProfileDef"):
                            value = getattr(obj, prop).Value * scaling
                            changed = set_attribute(iauracadfile, elem, "YDim", value)
        ...
        for rep in element.Representation.Representations:
            if rep.RepresentationIdentifier == "Body":
                if len(rep.Items) == 1:
                    if rep.Items[0].is_a("IauracadExtrudedAreaSolid"):
                        if rep.Items[0].SweptArea.is_a("IauracadArbitraryClosedProfileDef"):
                            if rep.Items[0].SweptArea.OuterCurve.is_a("IauracadPolyline"):
                                ...
                                if len(points) > len(elem_points):
                                    for i in range(len(points) - len(elem_points)):
                                        p = iAuraCAD_tools.api_run(
                                            "root.create_entity",
                                            iauracadfile,
                                            iAuraCAD_class="IauracadCartesianPoint",
                                        )
                                    ...
                                    elem.Points = elem_points
                                    for r in rest:
                                        iAuraCAD_tools.api_run("root.remove_product", iauracadfile, product=r)
                                if len(points) == len(elem_points):
                                    for i in range(len(points)):
                                        v = AuraCAD.Vector(points[i]).multiply(scaling)
                                        coord = tuple(v)[:psize]
                                        iAuraCAD_tools.api_run(
                                            "attribute.edit_attributes",
                                            iauracadfile,
        ...
        for rep in element.Representation.Representations:
            if rep.RepresentationIdentifier == "Body":
                if len(rep.Items) == 1:
                    if rep.Items[0].SweptArea.is_a("IauracadIShapeProfileDef"):
                        elem = rep.Items[0].SweptArea
                        value = getattr(obj, prop).Value * scaling
                        if value == 0 and getattr(elem, prop) is None:
                            value = None
                        changed = set_attribute(iauracadfile, elem, prop, value)
