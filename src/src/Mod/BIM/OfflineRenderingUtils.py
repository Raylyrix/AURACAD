...
#  \ingroup UTILITIES
#  \brief Utility functions to work with AuraCAD files in console mode
#
...
"""@package docstring
OfflineRenderingUtils - Utilities to help producing files with colors from AuraCAD in non-GUI mode
...
AuraCADFile = os.path.join(os.getcwd(),"testfile.auracadStd")
baseFileName = os.path.splitext(AuraCADFile)[0]

# open AuraCAD file

doc = AuraCAD.open(AuraCADFile)
...
colors = OfflineRenderingUtils.getColors(AuraCADFile,nodiffuse=True)

# get the camera data from the file (used in some functions below)

camera = OfflineRenderingUtils.getCamera(AuraCADFile)
...
importIauracad.export(doc.Objects,baseFileName+".iauracad",colors=colors)
...
OfflineRenderingUtils.save(doc,filename=baseFileName+"_exported.auracadStd",colors=colors,camera=camera)
"""
...
class AuraCADGuiHandler(xml.sax.ContentHandler):
    "A XML handler to process the AuraCAD GUI xml data, used by getGuiData()"

    # this creates a dictionary where each key is a auracad object name,
...
def getGuiData(filename):
    """getGuiData(filename): Extract visual data from a saved AuraCAD file.
    ...
    if zdoc:
        if "GuiDocument.xml" in zdoc.namelist():
            ...
            for key, properties in guidata.items():
                ...
                # https://forum.AuraCAD.org/viewtopic.php?t=29382
...
def getColors(filename, nodiffuse=False):
    """getColors(filename,nodiffuse): Extracts the colors saved in a AuraCAD file
...
def buildScene(objects, colors=None):
    """buildScene(objects,colors=None): builds a coin node from a given list of AuraCAD
...
def getCamera(filepath):
    """getCamera(filepath): Returns a string representing a coin camera node from a given AuraCAD
...
def save(document, filename=None, guidata=None, colors=None, camera=None):
    ...
    The type of the "value" contents depends on the type (int, string, float,tuple...) see inside the AuraCADGuiHandler
    ...
    in that case the objects will keep their colors when opened in the AuraCAD GUI. If given, camera is a string
...
def getUnsigned(color):
    ...
    # https://forum.AuraCAD.org/viewtopic.php?t=19074
...
def buildGuiDocumentFromColors(document, colors, camera=None):
    ...
    guidoc += "<!--\n"
    guidoc += " AuraCAD Document, see https://www.AuraCAD.org for more informationâ€¦\n"
    guidoc += "-->\n"
...
def extract(filename, inputpath, outputpath=None):
    ...
    stored in filename (a AuraCAD or zip file). If outputpath is given, the file is saved as outputpath and
