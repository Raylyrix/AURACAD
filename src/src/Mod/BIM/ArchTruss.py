...
class Truss(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        if not "Normal" in pl:
            ...
            obj.Normal = AuraCAD.Vector(0, 0, 1)
    ...
    def execute(self, obj):
        ...
        if not obj.StrutWidth.Value or not obj.StrutHeight.Value:
            AuraCAD.Console.PrintLog(obj.Label + ": Invalid strut size\n")
            return
        if not obj.HeightStart.Value or not obj.HeightEnd.Value:
            AuraCAD.Console.PrintLog(obj.Label + ": Invalid truss heights\n")
            return
        if not obj.RodSections or not obj.RodSize.Value:
            AuraCAD.Console.PrintLog(obj.Label + ": Invalid rod config\n")
            return
        if not obj.Base:
            AuraCAD.Console.PrintLog(obj.Label + ": No base\n")
            return
        if (not hasattr(obj.Base, "Shape")) or (len(obj.Base.Shape.Vertexes) > 2):
            AuraCAD.Console.PrintLog(obj.Label + ": No base edge\n")
            return
    ...
    def makeTruss(self, obj, v0, v1):
        ...
        if not sidedir.Length:
            AuraCAD.Console.PrintLog(obj.Label + ": normal and base are parallel\n")
            return
        sidedir.normalize()
        p0 = v0.add(AuraCAD.Vector(sidedir).negative().multiply(obj.StrutWidth.Value / 2))
        p1 = p0.add(AuraCAD.Vector(sidedir).multiply(obj.StrutWidth.Value / 2).multiply(2))
        p2 = p1.add(AuraCAD.Vector(normal).multiply(obj.StrutHeight))
        p3 = p0.add(AuraCAD.Vector(normal).multiply(obj.StrutHeight))
        ...
        # create top strut
        v2 = v0.add(AuraCAD.Vector(normal).multiply(obj.HeightStart.Value))
        v3 = v1.add(AuraCAD.Vector(normal).multiply(obj.HeightEnd.Value))
        topdir = v3.sub(v2)
        ...
        topstrut.translate(AuraCAD.Vector(normal).multiply(-obj.StrutHeight.Value))
        ...
        if obj.RodType == "Round":
            ...
            rodprofile = Part.Face(
                Part.makePlane(
                    obj.RodSize, obj.RodSize, AuraCAD.Vector(-obj.RodSize / 2, -obj.RodSize / 2, 0)
                )
            )

        # create rods
        rods = []
        bottomrodstart = v0.add(AuraCAD.Vector(normal).multiply(obj.StrutHeight.Value / 2))
        toprodstart = v2.add(AuraCAD.Vector(normal).multiply(obj.StrutHeight.Value / 2).negative())
        bottomrodvec = AuraCAD.Vector(maindir).multiply(1 / obj.RodSections)
        toprodvec = AuraCAD.Vector(topdir).multiply(1 / obj.RodSections)
    ...
    def makeRod(self, profile, p1, p2):
        ...
        rot = AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), p2.sub(p1))
