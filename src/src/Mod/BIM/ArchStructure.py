...
class CommandStructuresFromSelection:
    ...
    def Activated(self):
        selex = AuraCADGui.Selection.getSelectionEx()
        if len(selex) >= 2:
            ...
            for selexi in selex[
                ...
                for sub in subelement_names:
                    AuraCADGui.doCommand(
                        "structure = Arch.makeStructure(AuraCAD.ActiveDocument." + base.Name + ")"
                    )
                    AuraCADGui.doCommand(
                        "structure.Tool = (AuraCAD.ActiveDocument."
                        + selexi.Object.Name
                        + ", '"
                        + sub
                        + "')"
                    )
                    AuraCADGui.doCommand("structure.BasePerpendicularToTool = True")
                    AuraCADGui.doCommand("Draft.autogroup(structure)")
...
class _CommandStructure:
    ...
    def Activated(self):
        ...
        if sel:
            ...
            ax = Draft.getObjectsOfType(sel, "Axis")
            if ax:
                AuraCADGui.runCommand("Arch_StructuralSystem")
                return
                ...
                for obj in sel:
                    AuraCADGui.doCommand(
                        "obj = Arch.makeStructure(AuraCAD.ActiveDocument." + obj.Name + ")"
                    )
                    AuraCADGui.addModule("Draft")
                    AuraCADGui.doCommand("Draft.autogroup(obj)")
        ...
        AuraCADGui.draftToolBar.continueCmd.show()
    ...
    def getPoint(self, point=None, obj=None):
        ...
        if self.Profile is not None:
            ...
            if ("Precast" in self.Profile) and self.precastvalues:
                ...
                if self.mode == StructureMode.BEAM:
                    delta = AuraCAD.Vector(0, 0 - self.Width / 2, 0)
                else:
                    delta = AuraCAD.Vector(-self.Length / 2, -self.Width / 2, 0)
                ...
                AuraCADGui.addModule("ArchPrecast")
                AuraCADGui.doCommand("s = ArchPrecast.makePrecast(" + argstring + ")")
            else:
                # metal profile
                AuraCADGui.doCommand("p = Arch.makeProfile(" + str(self.Profile) + ")")
                if self.mode == StructureMode.BEAM:
                    # horizontal
                    AuraCADGui.doCommand(
                ...
                AuraCADGui.doCommand("s.Profile = " + repr(self.Profile[2]))
        else:
            AuraCADGui.doCommand(
        ...
        if self.mode == StructureMode.BEAM and self.bpoint:
            ...
            AuraCADGui.doCommand("s.Placement.Base = " + DraftVecUtils.toString(point))
        ...
        AuraCADGui.addModule("Draft")
        AuraCADGui.doCommand("Draft.autogroup(s)")
        self.doc.commitTransaction()
        self.doc.recompute()
        # gui_utils.end_all_events()  # Causes a crash on Linux.
        self.tracker.finalize()
        if AuraCADGui.draftToolBar.continueMode:
            self.Activated()
    ...
    def _createItemlist(self, baselist):
        ...
        ilist = []
        for p in baselist:
            f = AuraCAD.Units.Quantity(p[4], AuraCAD.Units.Length).getUserPreferred()
    ...
    def taskbox(self):
        ...
        self.vLength.setText(AuraCAD.Units.Quantity(self.Length, AuraCAD.Units.Length).UserString)
        ...
        self.vWidth.setText(AuraCAD.Units.Quantity(self.Width, AuraCAD.Units.Length).UserString)
        ...
        self.vHeight.setText(AuraCAD.Units.Quantity(self.Height, AuraCAD.Units.Length).UserString)
    ...
    def update(self, point, info):
        ...
        if AuraCADGui.Control.activeDialog():
            try:  # try to update latest precast values - fails if dialog has been destroyed already
                self.precastvalues = self.precast.getValues()
            except Exception:
                pass
            ...
            if self.mode == StructureMode.COLUMN:
                ...
                if self.bpoint:
                    ...
                    self.vLength.setText(AuraCAD.Units.Quantity(l, AuraCAD.Units.Length).UserString)
                else:
                    self.tracker.off()
            ...
            if elt in self.precast.PrecastTypes:
                ...
                if self.mode == StructureMode.BEAM:
                    self.vHeight.setText(
                        AuraCAD.Units.Quantity(depth, AuraCAD.Units.Length).UserString
                    )
                ...
                self.vWidth.setText(AuraCAD.Units.Quantity(width, AuraCAD.Units.Length).UserString)
                self.setWidth(width)
                self.Profile = elt
    ...
    def switchLH(self, beam_toggled):
        ...
        self.tracker.length(self.Length)
        if self.mode == StructureMode.COLUMN:
            self.tracker.setRotation(AuraCAD.Rotation())
...
class _Structure(ArchComponent.Component):
    ...
    def execute(self, obj):
        ...
        if extdata:
            ...
            for i in range(len(sh)):
                ...
                if i < len(ev):
                    ...
                    evi = ev[-1]
                    if isinstance(evi, AuraCAD.Vector):
                        evi = AuraCAD.Vector(evi)
                    else:
                        evi = evi.copy()
                if i < len(pla):
                    pli = pla[i]
            ...
            obj.ComputedLength = AuraCAD.Units.Quantity(extrusion_length, AuraCAD.Units.Length)
        if obj.Base:
        ...
        if (not base) and (not obj.Additions):
            # AuraCAD.Console.PrintError(translate("Arch","Error: Invalid base object")+"\n")
            return
    ...
    def getExtrusionData(self, obj):
        ...
        if obj.Base:
            ...
            if (length > height) and (IauracadType in ["Beam", "Column"]):
                h2 = height / 2 or 0.5
                w2 = width / 2 or 0.5
        ...
        if baseface:
                ...
                if len(edges) > 0:
                    ...
                    if hasattr(obj, "BasePerpendicularToTool") and obj.BasePerpendicularToTool:
                        ...
                        if hasattr(obj, "BaseRotation"):
                            pl.rotate(
                                AuraCAD.Vector(0, 0, 0), AuraCAD.Vector(0, 0, 1), -obj.BaseRotation
                            )
                        if hasattr(obj, "BaseOffsetX") and hasattr(obj, "BaseOffsetY"):
                            pl.translate(AuraCAD.Vector(obj.BaseOffsetX, obj.BaseOffsetY, 0))
                        if hasattr(obj, "BaseMirror") and obj.BaseMirror:
                            pl.rotate(AuraCAD.Vector(0, 0, 0), AuraCAD.Vector(0, 1, 0), 180)
            ...
            if extrusion:
                ...
                if isinstance(extrusion, AuraCAD.Vector):
    ...
    def onChanged(self, obj, prop):
        ...
        if AuraCAD.ActiveDocument.Restoring:
            return
        ...
        if prop in ["Shape", "ResetNodes", "NodesOffset"]:
            ...
            if extdata and not isinstance(extdata[0], list):
                nodes = extdata[0]
                if IauracadType in ["Beam", "Column"]:
                    if not isinstance(extdata[1], AuraCAD.Vector):
                        nodes = extdata[1]
                ...
                if isinstance(extdata[1], AuraCAD.Vector):
                    nodes.Placement = nodes.Placement.multiply(extdata[2])
            offset = AuraCAD.Vector()
            if hasattr(obj, "NodesOffset"):
                offset = AuraCAD.Vector(0, 0, obj.NodesOffset.Value)
...
class _ViewProviderStructure(ArchComponent.ViewProviderComponent):
    ...
    def updateData(self, obj, prop):
        ...
        elif prop in ["IauracadType"]:
            if hasattr(obj.ViewObject, "NodeType"):
                if hasattr(obj, "IauracadType"):
                    IauracadType = obj.IauracadType
                else:
                    IauracadType = None
                if IauracadType == "Slab":
...
class StructureTaskPanel(ArchComponent.ComponentOptionsTaskPanel):
    ...
    def __init__(self, obj):
        # Define properties based on the IauracadType
        if getattr(obj, "IauracadType", "Beam") == "Slab":
    ...
    def editNodes(self):
        ...
        AuraCADGui.runCommand("Draft_Edit")
    ...
    def extendNodes(self, other=None):
        ...
        if not other:
            ...
            AuraCADGui.Selection.addObserver(self.observer)
            AuraCAD.Console.PrintMessage(translate("Arch", "Choose another Structure object:"))
        else:
            AuraCADGui.Selection.removeObserver(self.observer)
            self.observer = None
                ...
                if not other.Nodes:
                    ...
                    if (len(self.Object.Nodes) != 2) or (len(other.Nodes) != 2):
                        ...
                        if not intersect:
                            ...
                            AuraCAD.Console.PrintMessage(translate("Arch", "Intersection found.\n"))
    ...
    def setSelectionFromTool(self):
        ...
        if hasattr(self.Object, "Tool"):
            ...
            if hasattr(tool, "Shape") and tool.Shape:
                AuraCADGui.Selection.addSelection(tool)
            else:
                if not isinstance(tool, list):
                    tool = [tool]
                for o, subs in tool:
                    AuraCADGui.Selection.addSelection(o, subs)
...
class StructSelectionObserver:
    ...
    def addSelection(self, docName, objName, sub, pos):
        print("got ", objName)
        obj = AuraCAD.getDocument(docName).getObject(objName)
        self.callback(obj)
...
if AuraCAD.GuiUp:
    AuraCADGui.addCommand("Arch_StructuralSystem", CommandStructuralSystem())
    AuraCADGui.addCommand("Arch_StructuresFromSelection", CommandStructuresFromSelection())
    ...
    AuraCADGui.addCommand("Arch_StructureTools", _ArchStructureGroupCommand())
