...
class BIM_IauracadProperties:

    def GetResources(self):
        return {
            "Pixmap": "BIM_IauracadProperties",
            "MenuText": QT_TRANSLATE_NOOP("BIM_IauracadProperties", "Manage Iauracad Properties"),
            "ToolTip": QT_TRANSLATE_NOOP(
                "BIM_IauracadProperties",
                "Manages the different Iauracad properties of the BIM objects",
            ),
        }
    ...
    def Activated(self):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/dialogIauracadProperties.ui")
        self.form.setWindowIcon(QtGui.QIcon(":/icons/BIM_IauracadProperties.svg"))
        ...
        self.form.onlySelected.setChecked(PARAMS.GetInt("IauracadPropertiesSelectedState", 0))
        self.form.onlyVisible.setChecked(PARAMS.GetInt("IauracadPropertiesVisibleState", 0))
        w = PARAMS.GetInt("BimIauracadPropertiesDialogWidth", 1200)
        h = PARAMS.GetInt("BimIauracadPropertiesDialogHeight", 608)
        ...
        try:
            ...
            self.ptypes = list(ArchIauracadSchema.IauracadTypes.keys())
        except (ImportError, AttributeError):
            import ArchComponent
        ...
        self.form.buttonIauracadPropertiesDelete.clicked.connect(self.removeProperty)
    ...
    def rebuildObjectsList(self):
        ...
        if self.form.onlySelected.isChecked():
            ...
            objects = AuraCAD.ActiveDocument.Objects
        for obj in objects:
            ...
            if role:
                if hasattr(obj, "IauracadProperties") and isinstance(obj.IauracadProperties, dict):
                    ...
                    props = self.getNativeIauracadProperties(obj)
                else:
                    props = {}
    ...
    def accept(self):
        PARAMS.SetInt("BimIauracadPropertiesDialogWidth", self.form.width())
        PARAMS.SetInt("BimIauracadPropertiesDialogHeight", self.form.height())
        self.form.hide()
        ...
        for key, values in self.objectslist.items():
            ...
            if obj:
                if hasattr(obj, "IauracadProperties"):
                    if not isinstance(obj.IauracadProperties, dict):
                        AuraCAD.Console.PrintWarning(
                            translate(
                                "BIM",
                                "Warning: object %1 has old-styled IauracadProperties and cannot be updated",
                            ).replace("%1", obj.Label)
                            + "\n"
                        )
                        continue
                ...
                if values[1] != props:
                    if not changed:
                        AuraCAD.ActiveDocument.openTransaction("Change properties")
                        changed = True
                    if hasattr(obj, "IauracadClass"):
                        print("props:", props)
                    ...
                    elif not hasattr(obj, "IauracadProperties"):
                        obj.addProperty(
                            "App::PropertyMap",
                            "IauracadPRoperties",
                            "Iauracad",
                    ...
                    if hasattr(obj, "IauracadProperties"):
                        obj.IauracadProperties = values[1]
        if changed:
    ...
    def getNativeIauracadProperties(self, obj):
        props = {}
        for p in obj.PropertiesList:
            ...
            if ":" in ttip:
                ...
                if pset not in ["Base", "Iauracad", "Geometry"]:
    ...
    def onSelected(self, index):
        PARAMS.SetInt("IauracadPropertiesSelectedState", getattr(index, "value", index))
        self.objectslist, searchterms = self.rebuildObjectsList()
    ...
    def onVisible(self, index):
        PARAMS.SetInt("IauracadPropertiesVisibleState", getattr(index, "value", index))
        self.update()
...
AuraCADGui.addCommand("BIM_IauracadProperties", BIM_IauracadProperties())
