...
class BIM_Preflight:
    def GetResources(self):
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "BIM_Preflight",
                "Checks several characteristics of this model before exporting to Iauracad",
            ),
        }
    ...
    def Activated(self):
        AuraCADGui.BIMPreflightDone = False
        AuraCADGui.Control.showDialog(BIM_Preflight_TaskPanel())
...
class BIM_Preflight_TaskPanel:
    ...
    def __init__(self):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/dialogPreflight.ui")
        ...
        # setup custom tests
        self.customTests = {}
        customModulePath = os.path.join(AuraCAD.getUserAppDataDir(), "BIM", "Preflight")
        ...
        if (test in self.results) and self.results[test]:
            if (test in self.culprits) and self.culprits[test]:
                ...
                for c in self.culprits[test]:
                    AuraCADGui.Selection.addSelection(c)
            if not self.rform:
                self.rform = AuraCADGui.PySideUic.loadUi(":/ui/dialogPreflightResults.ui")
    ...
    def toReport(self):
        ...
        if self.rform and hasattr(self.rform, "test") and self.rform.test:
            if self.results[self.rform.test]:
                AuraCAD.Console.PrintMessage(self.results[self.rform.test] + "\n")
    ...
    def getObjects(self):
        ...
        if self.form.getAll.isChecked():
            ...
            objs = [o for o in AuraCAD.ActiveDocument.Objects if o.ViewObject.Visibility == True]
        else:
    ...
    def testAll(self):
        ...
        AuraCADGui.BIMPreflightDone = True

    def testIauracad4(self):
        "tests for Iauracad4 support"
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            try:
                ...
                msg = (
                    translate(
                        "BIM",
                        "iauracadopenshell is not installed on the system or not available to AuraCAD. This library is responsible for Iauracad support in AuraCAD, and therefore Iauracad support is currently disabled. Check %1 to obtain more information.",
                    ).replace(
                        "%1", "https://www.AuraCAD.org/wiki/Extra_python_modules#IauracadOpenShell"
                    )
                    + " "
                )
                self.failed(test)
                ...
                if hasattr(
                    iauracadopenshell, "schema_identifier"
                ) and iauracadopenshell.schema_identifier.startswith("Iauracad4"):
                    self.passed(test)
                elif hasattr(iauracadopenshell, "version"):
                    try:
                        from packaging import version

                        if "-" in iauracadopenshell.version:
                            # Prebuild version have a version like 'v0.7.0-<GIT_COMMIT_ID>,
                            # trying to remove the commit id.
                            cur_version = version.parse(iauracadopenshell.version.split("-")[0])
                        else:
                            cur_version = version.parse(iauracadopenshell.version)
                        ...
                        if cur_version >= min_version:
                            ...
                            msg = (
                                translate(
                                    "BIM",
                                    "The version of Iauracadopenshell installed on the system could not be parsed",
                                )
                                + " "
                            )
                            self.failed(test)
                    ...
                    msg += (
                        translate(
                            "BIM",
                            "The version of Iauracadopenshell installed on the system will produce files with this schema version:",
                        )
                        + "\n\n"
                    )
                    if hasattr(iauracadopenshell, "schema_identifier"):
                        msg += iauracadopenshell.schema_identifier + "\n\n"
                    else:
                        msg += "Unable to retrieve schemas information from iauracadopenshell\n\n"
                    self.failed(test)
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            for obj in self.getObjects():
                if (
                    ...
                    or (hasattr(obj, "IauracadRole") and (obj.IauracadRole == "Site"))
                    or (hasattr(obj, "IauracadType") and (obj.IauracadType == "Site"))
                ):
                    sites = True
                elif (
                    (Draft.getType(obj) == "Building")
                    or (hasattr(obj, "IauracadRole") and (obj.IauracadRole == "Building"))
                    or (hasattr(obj, "IauracadType") and (obj.IauracadType == "Building"))
                ):
                    buildings = True
                elif (hasattr(obj, "IauracadRole") and (obj.IauracadRole == "Building Storey")) or (
                    hasattr(obj, "IauracadType") and (obj.IauracadType == "Building Storey")
                ):
                    storeys = True
                ...
                if (
                    ...
                    for parent in obj.InList:
                        if (
                            ...
                            or (hasattr(parent, "IauracadRole") and (parent.IauracadRole == "Site"))
                            or (hasattr(parent, "IauracadType") and (parent.IauracadType == "Site"))
                        ):
    ...
    def testBuildings(self):
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            msg = None
            for obj in self.getObjects():
                if (hasattr(obj, "IauracadRole") and (obj.IauracadRole == "Building Storey")) or (
                    hasattr(obj, "IauracadType") and (obj.IauracadType == "Building Storey")
                ):
                    ok = False
                    for parent in obj.InList:
                        if (hasattr(parent, "IauracadRole") and (parent.IauracadRole == "Building")) or (
                            hasattr(parent, "IauracadType") and (parent.IauracadType == "Building")
                        ):
                            if hasattr(parent, "Group") and parent.Group:
                                if obj in parent.Group:
                                    ok = True
                    ...
                    if not ok:
                        ...
                        if not msg:
                            ...
                            msg += (
                                translate(
                                    "BIM",
                                    'The following building storey (building parts with their Iauracad role set as "building storey") objects have been found to not be included in any building. Resolve the situation by creating a building object, if none is present in the model, and drag and drop the building storey objects into it in the tree view:',
                                )
                                + "\n\n"
                            )
                        msg += obj.Label + "\n"
    ...
    def testStoreys(self):
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            for obj in self.getObjects():
                if (
                    hasattr(obj, "IauracadRole")
                    and (not obj.IauracadRole in ["Building", "Building Storey", "Site"])
                ) or (
                    hasattr(obj, "IauracadType")
                    and (not obj.IauracadType in ["Building", "Building Storey", "Site"])
                ):
                    ok = False
                    ...
                    for parent in ancestors:
                        ...
                        if (
                            hasattr(parent, "IauracadRole")
                            and (parent.IauracadRole in ["Building Storey", "Building"])
                        ) or (
                            hasattr(parent, "IauracadType")
                            and (parent.IauracadType in ["Building Storey", "Building"])
                        ):
                            ok = True
                            break
                    if not ok:
                        ...
                        if not msg:
                            ...
                            msg += (
                                translate(
                                    "BIM",
                                    'The following BIM objects have been found to not be included in any building storey (building parts with their Iauracad role set as "building storey"). Resolve the situation by creating a building storey object, if none is present in the model, and drag and drop these objects into it in the tree view:',
                                )
                                + "\n\n"
                            )
                        msg += obj.Label + "\n"
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            for obj in self.getObjects():
                if hasattr(obj, "IauracadType"):
                    if obj.IauracadType == "Undefined":
                        self.culprits[test].append(obj)
                        undefined.append(obj)
                elif hasattr(obj, "IauracadRole"):
                    if obj.IauracadRole == "Undefined":
                        self.culprits[test].append(obj)
                        undefined.append(obj)
                else:
                    self.culprits[test].append(obj)
    ...
    def testQuantities(self):
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            for obj in self.getObjects():
                if hasattr(obj, "IauracadAttributes") and (Draft.getType(obj) != "BuildingPart"):
                    for prop in ["Length", "Width", "Height"]:
                        if prop in obj.PropertiesList:
                            if (not "Export" + prop in obj.IauracadAttributes) or (
                                obj.IauracadAttributes["Export" + prop] == "False"
                            ):
                                self.culprits[test].append(obj)
                                break
            if self.culprits[test]:
                ...
                msg += (
                    translate(
                        "BIM",
                        "The objects below have length, width or height properties, but these properties won't be explicitly exported to Iauracad. This is not necessarily an issue, unless these quantities are desired to be exported:",
                    )
                    + "\n\n"
                )
                for o in self.culprits[test]:
                ...
                msg += "\n" + translate(
                    "BIM",
                    "To enable exporting of these quantities, use the Iauracad quantities manager tool located under menu Manage -> Manage Iauracad Quantities",
                )
            if msg:
                self.failed(test)
            else:
    ...
    def testCommonPsets(self):
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            for obj in self.getObjects():
                ok = True
                if hasattr(obj, "IauracadProperties") and isinstance(obj.IauracadProperties, dict):
                    r = None
                    if hasattr(obj, "IauracadType"):
                        r = obj.IauracadType
                    if hasattr(obj, "IauracadRole"):
                        r = obj.IauracadRole
                    if r and (r in psets):
                        ok = False
                        if "Pset_" + r.replace(" ", "") + "Common" in ",".join(
                            obj.IauracadProperties.values()
                        ):
                            ok = True
                if not ok:
                    self.culprits[test].append(obj)
            ...
            if self.culprits[test]:
                ...
                msg += (
                    translate(
                        "BIM",
                        "The objects below have a defined Iauracad type but do not have the associated common property set:",
                    )
                    + "\n\n"
                )
                for o in self.culprits[test]:
                ...
                msg += "\n" + translate(
                    "BIM",
                    "To add common property sets to these objects, use the Iauracad properties manager tool located under menu Manage -> Manage Iauracad Properties",
                )
            if msg:
                self.failed(test)
            else:
                ...
                if hasattr(obj, "IauracadProperties") and isinstance(obj.IauracadProperties, dict):
                    ...
                    if r and (r != "Undefined"):
                        ...
                        for pset in psets.keys():
                            for val in obj.IauracadProperties.values():
                                if pset in val:
                                    found = pset
                                    break
                        if found:
                            for i in range(int(len(psets[found]) / 2)):
                                p = psets[found][i * 2]
                                t = psets[found][i * 2 + 1]
                                # print("testing for ",p,t,found," in ",obj.IauracadProperties)
                                if p in obj.IauracadProperties:
                                    if (not found in obj.IauracadProperties[p]) or (
                                        not t in obj.IauracadProperties[p]
                                    ):
                                        ok = False
                                else:
                                    ok = False
            ...
            if self.culprits[test]:
                ...
                msg += (
                    ...
                    + translate(
                        ...
                        "%1",
                        "https://standards.buildingsmart.org/Iauracad/DEV/Iauracad4_2/FINAL/HTML/annex/annex-b/alphabeticalorder_psets.htm",
                    )
                    + "\n\n"
                )
                msg += translate(
                    "BIM",
                    "To fix the property sets of these objects, use the Iauracad properties manager tool located under menu Manage -> Manage Iauracad Properties",
                )
            if msg:
                self.failed(test)
            else:
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            for obj in self.getObjects():
                if hasattr(obj, "Proxy"):
                    if (
                        hasattr(obj, "IauracadAttributes")
                        and ("FlagForceBrep" in obj.IauracadAttributes.keys())
                        and (obj.IauracadAttributes["FlagForceBrep"] == "True")
                    ):
                        self.culprits[test].append(obj)
    ...
    def testTinyLines(self):
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            if edges:
                ...
                result = AuraCAD.ActiveDocument.addObject("Part::Feature", "TinyLinesResult")
    ...
    def testRectangleProfileDef(self):
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            self.culprits[test] = None
            msg = None
            if AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetBool(
                "DisableIauracadRectangleProfileDef", False
            ):
                self.passed(test)
            else:
                msg = self.getToolTip(test)
...
AuraCADGui.addCommand("BIM_Preflight", BIM_Preflight())
