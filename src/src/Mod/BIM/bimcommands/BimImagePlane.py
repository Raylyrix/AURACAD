...
class BIM_ImagePlane:
    ...
    def MoveCallback(self, point, snapinfo):
        ...
        if point and self.basepoint and (point != self.basepoint):
            ...
            self.opposite = (
                AuraCAD.Vector(self.tracker.u)
                .multiply(length)
                .add(AuraCAD.Vector(self.tracker.v).multiply(height))
            )
    ...
    def PointCallback(self, point, snapinfo):
        ...
        if not point:
            ...
            image.Placement = AuraCAD.Placement(midpoint, rotation)
            image.XSize = length
            image.YSize = height
...
AuraCADGui.addCommand("BIM_ImagePlane", BIM_ImagePlane())
