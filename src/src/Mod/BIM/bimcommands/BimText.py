...
class BIM_Text:
    ...
    def IsActive(self):
        act_win = AuraCADGui.getMainWindow().getActiveWindow()
    ...
    def Activated(self):
        ...
        if hasattr(self.view, "getPage") and self.view.getPage():
            ...
            AuraCADGui.draftToolBar.taskUi()
            AuraCADGui.draftToolBar.textUi()
        else:
            AuraCADGui.runCommand("Draft_Text")
    ...
    def createObject(self):
        ...
        if self.text:
            ...
            if not pagescale:
                pagescale = 1
            anno = AuraCAD.ActiveDocument.addObject("TechDraw::DrawViewAnnotation", "Annotation")
            anno.Text = self.text
            page.addView(anno)
            param = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft")
...
AuraCADGui.addCommand("BIM_Text", BIM_Text())
