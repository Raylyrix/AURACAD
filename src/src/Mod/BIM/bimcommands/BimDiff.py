...
class BIM_Diff:
    def GetResources(self):
        return {
            "Pixmap": "BIM_Diff",
            "MenuText": QT_TRANSLATE_NOOP("BIM_Diff", "Iauracad Diff"),
            "ToolTip": QT_TRANSLATE_NOOP(
                "BIM_Diff", "Shows the difference between two Iauracad-based documents"
            ),
        }
    ...
    def Activated(self):
        ...
        documents = AuraCAD.listDocuments()
        ...
        if len(documents) == 2:
            ...
            if reply == QtGui.QMessageBox.Yes:
                activedoc = AuraCAD.ActiveDocument
                if list(documents.keys())[0] == activedoc.Name:
                    otherdoc = list(documents.values())[1]
                else:
                    otherdoc = list(documents.values())[0]

                # build lists of BIM objects with Iauracad ID

                objswithoutid = []  # let's try to match these later on

                activedocids = {}  # main document, the original AuraCAD one
                for obj in activedoc.Objects:
                    if hasattr(obj, "IauracadData") and obj.ViewObject.Visibility:
                        if "IauracadUID" in obj.IauracadData:
                            activedocids[obj.IauracadData["IauracadUID"]] = obj
                ...
                for obj in otherdoc.Objects:
                    if hasattr(obj, "IauracadData") and obj.ViewObject.Visibility:
                        if "IauracadUID" in obj.IauracadData:
                            otherdocids[obj.IauracadData["IauracadUID"]] = obj
                ...
                propertieschanged = {}  # objects whose Iauracad properties are different
                ...
                for id, obj in otherdocids.items():
                    if id in activedocids:
                        ...
                        if obj.IauracadProperties and (obj.IauracadProperties != mainobj.IauracadProperties):
                            # properties have changed
                            propertieschanged[id] = obj.IauracadProperties
                ...
                if toselect:
                    AuraCAD.setActiveDocument(otherdoc.Name)
                    AuraCAD.ActiveDocument = AuraCADGui.getDocument(otherdoc.Name)
                    AuraCADGui.ActiveDocument = AuraCADGui.getDocument(otherdoc.Name)
                ...
                if matchanged:
                    ...
                    if reply == QtGui.QMessageBox.Yes:
                        for obj in matchanged:
                            mat = obj.Material
                            if mat:
                                mainobj = activedocids[obj.IauracadData["IauracadUID"]]
                                if mainobj.Material:
                ...
                if newids:
                    reply = QtGui.QMessageBox.question(
                        ...
                        + translate(
                            "BIM",
                            "objects have no Iauracad ID in the main document, but an identical object with an ID exists in the new document. Transfer these IDs to the original objects?",
                        ),
                        QtGui.QMessageBox.Yes | QtGui.QMessageBox.No,
                        QtGui.QMessageBox.No,
                    )
                ...
                if moved:
                    ...
                    if reply == QtGui.QMessageBox.Yes:
                        for obj in moved:
                            mainobj = activedocids[obj.IauracadData["IauracadUID"]]
                            otherobj = otherdocids[obj.IauracadData["IauracadUID"]]
...
AuraCADGui.addCommand("BIM_Diff", BIM_Diff())
