...
class Arch_Wall:
    ...
    https://wiki.AuraCAD.org/Arch_Wall
    """
                ...
                for selobj in sel:
                    if (
                        ...
                        AuraCADGui.doCommand(
                            "obj = Arch.makeWall(AuraCAD.ActiveDocument."
                            + selobj.Object.Name
                            + ",face="
                            + str(idx)
                            + ")"
                        ...
                        AuraCADGui.doCommand(
                            "obj = Arch.makeWall(AuraCAD.ActiveDocument." + selobj.Object.Name + ")"
                        )
    ...
    def _create_baseless_wall(self, p0, p1):
        ...
        rotation = AuraCAD.Rotation(AuraCAD.Vector(1, 0, 0), direction)

        # This placement is local to the working plane.
        local_placement = AuraCAD.Placement(midpoint, rotation)
        ...
        placement_str = (
            f"AuraCAD.Placement(AuraCAD.Vector({final_placement.Base.x}, {final_placement.Base.y}, {final_placement.Base.z}), "
            f"AuraCAD.Rotation({final_placement.Rotation.Q[0]}, {final_placement.Rotation.Q[1]}, {final_placement.Rotation.Q[2]}, {final_placement.Rotation.Q[3]}))"
        )
        ...
        AuraCADGui.doCommand("import Arch")
        AuraCADGui.doCommand(make_wall_cmd)
        AuraCADGui.doCommand(f"{wall_var}.Placement = {placement_str}")
        if self.MultiMat:
            AuraCADGui.doCommand(
                f"{wall_var}.Material = AuraCAD.ActiveDocument.{self.MultiMat.Name}"
            )
        ...
        AuraCADGui.doCommand("import Draft")
        AuraCADGui.doCommand(f"Draft.autogroup({wall_var})")
    ...
    def _create_baseline_object(self, p0, p1):
        ...
        placement_str = (
            f"AuraCAD.Placement(AuraCAD.Vector({placement.Base.x}, {placement.Base.y}, {placement.Base.z}), "
            f"AuraCAD.Rotation({placement.Rotation.Q[0]}, {placement.Rotation.Q[1]}, {placement.Rotation.Q[2]}, {placement.Rotation.Q[3]}))"
        )

        trace_cmd = (
            f"trace = Part.LineSegment(AuraCAD.Vector({p0.x}, {p0.y}, {p0.z}), "
            f"AuraCAD.Vector({p1.x}, {p1.y}, {p1.z}))"
        )

        AuraCADGui.doCommand("import AuraCAD")
        AuraCADGui.doCommand("import Part")
        AuraCADGui.addModule("WorkingPlane")
        AuraCADGui.doCommand(trace_cmd)

        if self.baseline_mode == WallBaselineMode.DRAFT_LINE:
            AuraCADGui.doCommand("import Draft")

            # Execute creation command. AuraCAD will ensure a unique name.
            AuraCADGui.doCommand("base = Draft.make_line(trace)")

            # Execute placement command.
            AuraCADGui.doCommand(f"base.Placement = {placement_str}")
            ...
            useArchSketch = hasattr(ArchSketchObject, "makeArchSketch") and getattr(
                AuraCAD, "ArchSketchLock", True
            )

            if not useArchSketch:
                # Regular path without SketchArch add-on installed. Execute creation command with a
                # suggested name. AuraCAD will ensure uniqueness.
                AuraCADGui.doCommand(
                    "base = AuraCAD.ActiveDocument.addObject('Sketcher::SketchObject', 'WallTrace')"
                )
                user_label = translate("BimWall", "Wall Trace")
                # Set the user-facing, translated label.
                AuraCADGui.doCommand(f"base.Label = {repr(user_label)}")
            else:
                # Use ArchSketch if SketchArch add-on is present
                AuraCADGui.doCommand("import ArchSketchObject")
                AuraCADGui.doCommand("base = ArchSketchObject.makeArchSketch()")

            # Apply placement and geometry using the correctly identified object name.
            AuraCADGui.doCommand(f"base.Placement = {placement_str}")
            AuraCADGui.doCommand(f"base.addGeometry(trace)")
    ...
    def _create_wall_from_baseline(self, base_obj):
        ...
        make_wall_cmd = (
            f"{wall_var} = Arch.makeWall(AuraCAD.ActiveDocument.{base_obj.Name}, "
            f"width={self.Width}, height={self.Height}, align='{self.Align}')"
        )
        set_normal_cmd = f"{wall_var}.Normal = AuraCAD.{self.wp.axis}"

        # Execute creation and property-setting commands
        AuraCADGui.doCommand("import Arch")
        AuraCADGui.doCommand(make_wall_cmd)
        AuraCADGui.doCommand(set_normal_cmd)
        if self.MultiMat:
            AuraCADGui.doCommand(
                f"{wall_var}.Material = AuraCAD.ActiveDocument.{self.MultiMat.Name}"
            )
        ...
        if wall_obj and self.existing:
            ...
            if wallGrp == oldWallGrp:
                ...
                if (
                    ...
                    AuraCADGui.doCommand(
                        f"Arch.joinWalls([AuraCAD.ActiveDocument.{wall_obj.Name}, AuraCAD.ActiveDocument.{oldWall.Name}], delete=True, deletebase=True)"
                    )
                    joined = True
                ...
                if not joined and AUTOJOIN:
                    if wallGrp:
                        ...
                        AuraCADGui.doCommand(
                            f"AuraCAD.ActiveDocument.{wallGrp.Name}.removeObject(AuraCAD.ActiveDocument.{wall_obj.Name})"
                        )
                    AuraCADGui.doCommand(
                        f"Arch.addComponents(AuraCAD.ActiveDocument.{wall_obj.Name}, AuraCAD.ActiveDocument.{oldWall.Name})"
                    )
    ...
    def update(self, point, info):
        ...
        if AuraCADGui.Control.activeDialog():
            ...
            if self.Length:
                self.Length.setText(
                    AuraCAD.Units.Quantity(bv.Length, AuraCAD.Units.Length).UserString
                )
    ...
    def taskbox(self):
        ...
        if hasattr(AuraCAD, "LastArchMultiMaterial"):
            for i, o in enumerate(self.multimats):
                if o.Name == AuraCAD.LastArchMultiMaterial:
        ...
        inputWidth.setText(AuraCAD.Units.Quantity(self.Width, AuraCAD.Units.Length).UserString)
        ...
        inputHeight.setText(AuraCAD.Units.Quantity(self.Height, AuraCAD.Units.Length).UserString)
        ...
        inputOffset.setText(AuraCAD.Units.Quantity(self.Offset, AuraCAD.Units.Length).UserString)
    ...
    def createFromGUI(self):
        ...
        if self.MultiMat:
            AuraCADGui.doCommand("wall.Material = AuraCAD.ActiveDocument." + self.MultiMat.Name)
        self.doc.commitTransaction()
        self.doc.recompute()
        if hasattr(AuraCADGui, "draftToolBar"):
            AuraCADGui.draftToolBar.escape()


AuraCADGui.addCommand("Arch_Wall", Arch_Wall())
