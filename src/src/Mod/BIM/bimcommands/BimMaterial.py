...
class BIM_Material:
                    ...
                    for par in mat.InList:
                        for prop in par.PropertiesList:
                            if getattr(par, prop) == mat:
                                ...
                                if first:
                                    AuraCAD.ActiveDocument.openTransaction("Merge materials")
                                    first = False
            ...
            for tod in todelete:
                if not tod.InList:
                    ...
                    if first:
                        AuraCAD.ActiveDocument.openTransaction("Merge materials")
                        first = False
                    AuraCAD.ActiveDocument.removeObject(tod.Name)
                ...
                if item:
                    ...
                    if obj:
                        ...
                        if not parents:
                            ...
                            label = obj.Label
                            if first:
                                AuraCAD.ActiveDocument.openTransaction("Delete materials")
                                first = False
                            AuraCAD.Console.PrintMessage(
    ...
    def onDuplicate(self):
        if self.dlg:
            ...
            if item:
                oldmat = AuraCAD.ActiveDocument.getObject(item.toolTip())
                if oldmat:
                    import Arch
    ...
    def onMergeTo(self):
        if self.dlg:
            ...
            if item:
                oldmat = AuraCAD.ActiveDocument.getObject(item.toolTip())
                # load dialog
                form = AuraCADGui.PySideUic.loadUi(":/ui/dialogListWidget.ui")
                ...
                if result:
                    ...
                    if mergeto:
                        mergemat = AuraCAD.ActiveDocument.getObject(mergeto.toolTip())
                        if oldmat and mergemat:
                            ...
                            name = oldmat.Name
                            AuraCAD.ActiveDocument.openTransaction("Merge material")
            ...
            if item:
                ...
                if obj:
                    ...
                    if parents:
                        ...
                        name = obj.Name
                        AuraCAD.ActiveDocument.openTransaction("Delete material")
                        AuraCAD.ActiveDocument.removeObject(name)
                        AuraCAD.ActiveDocument.commitTransaction()
                        AuraCAD.ActiveDocument.recompute()
                        self.rescan()
    ...
    def onAccept(self, item=None):
        if self.dlg:
            ...
            if item and self.dlg.objects:
                mat = AuraCAD.ActiveDocument.getObject(item.toolTip())
                if mat:
                    if self.dlg.objects:
                        AuraCAD.ActiveDocument.openTransaction("Change material")
                        for obj in self.dlg.objects:
                            if hasattr(obj, "StepId"):
                                from nativeiauracad import iAuraCAD_materials

                                iAuraCAD_materials.set_material(mat, obj)
                            else:
                                obj.Material = mat
                        AuraCAD.ActiveDocument.commitTransaction()
                        AuraCAD.ActiveDocument.recompute()
...
class Arch_Material:
    ...
    def Activated(self):
        ...
        AuraCAD.ActiveDocument.openTransaction(translate("Arch", "Create material"))
        AuraCADGui.addModule("Arch")
        AuraCADGui.Control.closeDialog()
        AuraCADGui.doCommand("mat = Arch.makeMaterial()")
        for obj in sel:
            if hasattr(obj, "Material") and hasattr(obj, "MoveWithHost"):  # 'isComponent' check
                AuraCADGui.doCommand(
                    'AuraCAD.ActiveDocument.getObject("' + obj.Name + '").Material = mat'
                )
        AuraCADGui.doCommandGui("mat.ViewObject.Document.setEdit(mat.ViewObject, 0)")
        AuraCAD.ActiveDocument.commitTransaction()
        AuraCAD.ActiveDocument.recompute()
...
class Arch_MultiMaterial:
    ...
    def Activated(self):
        ...
        sel = AuraCADGui.Selection.getSelection()
        AuraCAD.ActiveDocument.openTransaction(translate("Arch", "Create multi-material"))
        AuraCADGui.addModule("Arch")
        AuraCADGui.Control.closeDialog()
        AuraCADGui.doCommand("mat = Arch.makeMultiMaterial()")
        for obj in sel:
            if hasattr(obj, "Material"):
                if not obj.isDerivedFrom("App::MaterialObject"):
                    AuraCADGui.doCommand("AuraCAD.ActiveDocument." + obj.Name + ".Material = mat")
        AuraCADGui.doCommandGui("mat.ViewObject.Document.setEdit(mat.ViewObject, 0)")
        AuraCAD.ActiveDocument.commitTransaction()
        AuraCAD.ActiveDocument.recompute()
...
AuraCADGui.addCommand("BIM_Material", BIM_Material())
AuraCADGui.addCommand("Arch_Material", Arch_Material())
AuraCADGui.addCommand("Arch_MultiMaterial", Arch_MultiMaterial())
AuraCADGui.addCommand("Arch_MaterialTools", Arch_MaterialToolsCommand())
