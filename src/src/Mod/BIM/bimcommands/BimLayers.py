...
class BIM_Layers:
    "The BIM_Layers AuraCAD command"
    ...
    def Activated(self):
        ...
        # create the dialog
        self.dialog = AuraCADGui.PySideUic.loadUi(":/ui/dialogLayersIauracad.ui")

        # store the iauracad icon
        self.iauracadicon = QtGui.QIcon(":/icons/Iauracad.svg")
        ...
        self.dialog.buttonIauracad.setIcon(self.iauracadicon)
        ...
        self.dialog.buttonIauracad.clicked.connect(self.onIauracad)
        ...
        for row in range(self.model.rowCount()):
            ...
            if not obj:
                ...
                if self.model.item(row, 1).icon().isNull():
                    ...
                    active = AuraCADGui.ActiveDocument.ActiveView.getActiveObject("NativeIauracad")
                    project = None
                    if active:
                        project = iAuraCAD_tools.get_project(active)
                    else:
                        projects = [
                            o
                        ...
                        if projects:
                            ...
                            AuraCAD.Console.PrintError(
                                translate("BIM", "There is no Iauracad project in this document") + "\n"
                            )
                    if project:
                        obj = iAuraCAD_tools.create_layer(self.model.item(row, 1).text(), project)
            vobj = obj.ViewObject
    ...
    def reject(self):
        ...
        # save dialog size
        pref = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/BIM")
        pref.SetInt("LayersManagerWidth", self.dialog.width())
        pref.SetInt("LayersManagerHeight", self.dialog.height())

        # wipe to let AuraCAD know the dialog has been closed
        del self.dialog
    ...
    def update(self):
        ...
        # populate
        objs = [obj for obj in AuraCAD.ActiveDocument.Objects if Draft.getType(obj) == "Layer"]
    ...
    def addItem(self, obj=None):
        ...
        if AuraCADGui.ActiveDocument.ActiveView.getActiveObject("NativeIauracad"):
            nameItem.setIcon(self.iauracadicon)
    ...
    def getPref(self, value, default, valuetype="Unsigned"):
        ...
        p = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/View")
    ...
    def onIauracad(self):
        "attributes this layer to an Iauracad project"
        ...
        for index in self.dialog.tree.selectedIndexes():
            if index.column() == 1:
                ...
                if item.icon().isNull():
                    item.setIcon(self.iauracadicon)
                else:
    ...
    def onAssign(self):
        ...
        for index in self.dialog.tree.selectedIndexes():
            if index.column() == 1:
                name = self.model.itemFromIndex(index).toolTip()
                if name:
                    target = AuraCAD.ActiveDocument.getObject(name)
                    if target:
                        selected = AuraCADGui.Selection.getSelection()
                        if selected:
...
AuraCADGui.addCommand("BIM_Layers", BIM_Layers())
