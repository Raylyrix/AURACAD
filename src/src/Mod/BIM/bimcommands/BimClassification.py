...
class BIM_Classification:
    ...
    def Activated(self):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/dialogClassification.ui")
        ...
        self.form.groupMode.setItemIcon(1, QtGui.QIcon(":/icons/Iauracad.svg"))  # Type
        ...
        self.form.labelDownload.setText(
            self.form.labelDownload.text().replace(
                "%s", os.path.join(AuraCAD.getUserAppDataDir(), "BIM", "Classification")
            )
        )

        # hide materials list if we are editing a particular object
        if len(AuraCADGui.Selection.getSelection()) == 1:
            self.isEditing = AuraCADGui.Selection.getSelection()[0]
            pl = self.isEditing.PropertiesList
            if ("StandardCode" in pl) or ("IauracadClass" in pl):
                ...
                if "IauracadClass" in pl:
                    # load existing class if needed
                    from nativeiauracad import iAuraCAD_classification

                    iAuraCAD_classification.show_classification(self.isEditing)
        ...
        p = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetString(
            "DefaultClassificationSystem", ""
        )
        presetdir = os.path.join(AuraCAD.getUserAppDataDir(), "BIM", "Classification")
        if os.path.isdir(presetdir):
            presets = []
            for f in os.listdir(presetdir):
                if f.lower().endswith(".xml") or f.lower().endswith(".iauracad"):
    ...
    def updateByType(self):
        ...
        for name in self.objectslist.keys():
            ...
            if obj and hasattr(obj, "IauracadType"):
                groups.setdefault(obj.IauracadType, []).append(name)
            elif obj and hasattr(obj, "IauracadRole"):
                groups.setdefault(obj.IauracadRole, []).append(name)
            else:
    ...
    def updateByMaterial(self):
        ...
        for name in self.matlist.keys():
            mat = AuraCAD.ActiveDocument.getObject(name)
            if mat:
        ...
        for name, code in d.items():
            ...
            if obj:
                if (
                    ...
                    it.setToolTip(0, name)
                    self.form.treeObjects.addTopLevelItem(it)
                    if obj in AuraCADGui.Selection.getSelection():
    ...
    def updateClasses(self, search=""):
        ...
        # save as default
        AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").SetString(
    ...
    def build(self, system):
        # try to load the Iauracad first
        preset = os.path.join(AuraCAD.getUserAppDataDir(), "BIM", "Classification", system + ".iauracad")
        if os.path.exists(preset):
            return self.build_iauracad(system)
        else:
            preset = os.path.join(
                AuraCAD.getUserAppDataDir(), "BIM", "Classification", system + ".xml"
            )
            if os.path.exists(preset):
                return self.build_xml(system)
            else:
                AuraCAD.Console.PrintError("Unable to find classification file:" + system + "\n")
                return []

    def build_iauracad(self, system):
        # builds from iauracad instead of xml
        ...
        f = iauracadopenshell.open(preset)
        classes = f.by_type("IauracadClassificationReference")
        rootclass = f.by_type("IauracadClassification")
        if rootclass:
    ...
    def build_xml(self, system):
        ...
        preset = os.path.join(AuraCAD.getUserAppDataDir(), "BIM", "Classification", system + ".xml")
        if not os.path.exists(preset):
            return None
        import codecs
        import re
    ...
    def accept(self):
        if not self.isEditing:
                ...
                for item in items:
                    ...
                    if item.toolTip(0):
                        ...
                        if obj:
                            ...
                            if label != obj.Label:
                                if not changed:
                                    AuraCAD.ActiveDocument.openTransaction("Change standard codes")
                                    changed = True
                                obj.Label = label
            if changed:
            ...
            if ("StandardCode" in pl) or ("IauracadClass" in pl):
                ...
                AuraCAD.ActiveDocument.commitTransaction()
                AuraCAD.ActiveDocument.recompute()
        p = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/BIM")
...
AuraCADGui.addCommand("BIM_Classification", BIM_Classification())
