...
class Arch_Panel:
        ...
        if sel:
            ...
            for obj in sel:
                AuraCADGui.doCommand(
                    "obj = Arch.makePanel(AuraCAD.ActiveDocument."
                    + obj.Name
                    + ",thickness="
                    + str(self.Thickness)
                    + ")"
    ...
    def getPoint(self, point=None, obj=None):
        ...
        if self.Profile:
            ...
            AuraCADGui.doCommand("s = Arch.makePanel(p,thickness=" + str(self.Thickness) + ")")
            # AuraCADGui.doCommand('s.Placement.Rotation = AuraCAD.Rotation(-0.5,0.5,-0.5,0.5)')
        else:
            AuraCADGui.doCommand(
        ...
        if self.rotated:
            AuraCADGui.doCommand(
                "s.Placement.Rotation = AuraCAD.Rotation(AuraCAD.Vector(1.00,0.00,0.00),90.00)"
            )
        self.doc.commitTransaction()
        self.doc.recompute()
    ...
    def taskbox(self):
        ...
        self.vHeight.setText(
            AuraCAD.Units.Quantity(self.Thickness, AuraCAD.Units.Length).UserString
        )
    ...
    def setPreset(self, i):
        ...
        if i > 0:
            self.vLength.setText(
                AuraCAD.Units.Quantity(float(Presets[i][1]), AuraCAD.Units.Length).UserString
            )
            self.vWidth.setText(
                AuraCAD.Units.Quantity(float(Presets[i][2]), AuraCAD.Units.Length).UserString
            )
            self.vHeight.setText(
                AuraCAD.Units.Quantity(float(Presets[i][3]), AuraCAD.Units.Length).UserString
            )
...
class Arch_PanelCut:
    ...
    def Activated(self):
        ...
        if AuraCADGui.Selection.getSelection():
            AuraCAD.ActiveDocument.openTransaction(str(translate("Arch", "Create Panel Cut")))
            AuraCADGui.addModule("Arch")
            for obj in AuraCADGui.Selection.getSelection():
                if Draft.getType(obj) == "Panel":
                    AuraCADGui.doCommand(
                        "Arch.makePanelCut(AuraCAD.ActiveDocument." + obj.Name + ")"
                    )
...
class Arch_PanelSheet:
    ...
    def Activated(self):
        ...
        AuraCAD.ActiveDocument.openTransaction(str(translate("Arch", "Create Panel Sheet")))
        AuraCADGui.addModule("Arch")
        if AuraCADGui.Selection.getSelection():
            l = "["
            for obj in AuraCADGui.Selection.getSelection():
                l += "AuraCAD.ActiveDocument." + obj.Name + ","
            l += "]"
            AuraCAD.ActiveDocument.commitTransaction()
            AuraCAD.ActiveDocument.recompute()
            AuraCADGui.doCommand("__objs__ = " + l)
            AuraCADGui.doCommand("Arch.makePanelSheet(__objs__)")
            AuraCADGui.doCommand("del __objs__")
        else:
            AuraCADGui.doCommand("Arch.makePanelSheet()")
        AuraCAD.ActiveDocument.commitTransaction()
        AuraCAD.ActiveDocument.recompute()
...
class Arch_Nest:
    ...
    def Activated(self):
        ...
        AuraCADGui.Control.showDialog(NestTaskPanel())
...
class NestTaskPanel:
    ...
    def __init__(self, obj=None):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/ArchNest.ui")
    ...
    def accept(self):
        ...
        self.clearTemps()
        if self.nester:
            AuraCAD.ActiveDocument.openTransaction("Nesting")
            self.nester.apply()
            AuraCAD.ActiveDocument.commitTransaction()
        return True
    ...
    def clearTemps(self):
        ...
        for t in self.temps:
            if AuraCAD.ActiveDocument.getObject(t.Name):
                AuraCAD.ActiveDocument.removeObject(t.Name)
        self.temps = []
        ...
        if len(s) == 1:
            if hasattr(s[0], "Shape"):
                if len(s[0].Shape.Faces) == 1:
                    ...
                    AuraCAD.Console.PrintError(translate("Arch", "This object has no face"))
    ...
    def removeShapes(self):
        ...
        for i in self.form.Shapes.selectedItems():
            o = AuraCAD.ActiveDocument.getObject(i.toolTip())
            if o:
                if o in self.shapes:
...
AuraCADGui.addCommand("Arch_Panel", Arch_Panel())
AuraCADGui.addCommand("Arch_Panel_Cut", Arch_PanelCut())
AuraCADGui.addCommand("Arch_Panel_Sheet", Arch_PanelSheet())
AuraCADGui.addCommand("Arch_Nest", Arch_Nest())
AuraCADGui.addCommand("Arch_PanelTools", Arch_PanelGroup())
