...
class BIM_IauracadElements:

    def GetResources(self):
        return {
            "Pixmap": "BIM_IauracadElements",
            "MenuText": QT_TRANSLATE_NOOP("BIM_IauracadElements", "Manage Iauracad Elements"),
            "ToolTip": QT_TRANSLATE_NOOP(
                "BIM_IauracadElements",
                "Manages how the different elements of the BIM project will be exported to Iauracad",
            ),
        }
    ...
    def Activated(self):
        ...
        try:
            ...
            self.iauracadtypes = ArchIauracad.IauracadTypes
        except (ImportError, AttributeError):
            import ArchComponent

            self.iauracadtypes = ArchComponent.IauracadRoles
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/dialogIauracadElements.ui")
        self.form.setWindowIcon(QtGui.QIcon(":/icons/BIM_IauracadElements.svg"))
        self.model = QtGui.QStandardItemModel()
        self.form.tree.setModel(self.model)
        self.form.tree.setUniformRowHeights(True)
        self.form.tree.setItemDelegate(IauracadElementsDelegate(dialog=self))
        self.form.globalMode.addItems([" "] + self.iauracadtypes)
        ...
        for group in groups.keys():
            ...
            for name, mat in groups[group]:
                ...
                if obj:
                    ...
                    matlabel = ""
                    if mat:
                        matobj = AuraCAD.ActiveDocument.getObject(mat)
                        if matobj:
    ...
    def updateByMaterial(self):
        ...
        for group in groups.keys():
            grlabel = "Undefined"
            if group != "Undefined":
                matobj = AuraCAD.ActiveDocument.getObject(group)
                if matobj:
    ...
    def onClickTree(self, index=None):
        ...
        for index in sel:
            if index.column() == 0:
                obj = AuraCAD.ActiveDocument.getObject(self.model.itemFromIndex(index).toolTip())
                if obj:
                    AuraCADGui.Selection.addSelection(obj)

        for index in sel:
            if index.column() == 1:
                if index.data() in self.iauracadtypes:
                    if mode:
                        if index.data() != mode:
                            mode = None
                            break
        ...
        for index in sel:
            if index.column() == 2:
                ...
                m = AuraCAD.ActiveDocument.getObject(item.toolTip())
                if mat:
                    if m != mat:
                        mat = None
                        break
                    ...
                    mat = m
        if mode:
            self.form.globalMode.setCurrentIndex(self.iauracadtypes.index(mode) + 1)
        else:
    ...
    def onObjectTypeChanged(self, index=-1):
        changed = False
        if index >= 1:
            role = self.iauracadtypes[index - 1]
            sel = self.form.tree.selectedIndexes()
            for index in sel:
                if index.column() == 1:
                    if role:
    ...
    def onMaterialChanged(self, index=-1):
        changed = False
        if index == 1:
            AuraCADGui.runCommand("Arch_Material")
            QtCore.QTimer.singleShot(1000, self.checkMatChanged)
        elif index == 2:
            AuraCADGui.runCommand("Arch_MultiMaterial")
            QtCore.QTimer.singleShot(1000, self.checkMatChanged)
        elif index >= 3:
            ...
            for index in sel:
                if index.column() == 2:
                    if mat:
                        mobj = AuraCAD.ActiveDocument.getObject(mat)
                        if mobj:
                            item = self.model.itemFromIndex(index)
                            if item.toolTip() != mat:
    ...
    def checkMatChanged(self):
        ...
        if len(mats) != len(self.materials):
            ...
            for m in self.materials:
                o = AuraCAD.ActiveDocument.getObject(m)
                if o:
                    self.form.globalMaterial.addItem(
    ...
    def accept(self):
        ...
        for name, rolemat in self.objectslist.items():
            ...
            obj = AuraCAD.ActiveDocument.getObject(name)
            if obj:
                if hasattr(obj, "IauracadRole") and (obj.IauracadRole != role):
                    if not changed:
                        AuraCAD.ActiveDocument.openTransaction("Change Iauracad role")
                        changed = True
                    obj.IauracadRole = role
                elif hasattr(obj, "IauracadType") and (obj.IauracadType != role):
                    if not changed:
                        AuraCAD.ActiveDocument.openTransaction("Change Iauracad type")
                        changed = True
                    obj.IauracadType = role
                    ...
                    if mobj:
                        if obj.Material:
                            if obj.Material.Name != mat:
                                if not changed:
                                    AuraCAD.ActiveDocument.openTransaction("Change material")
                                    changed = True
                                obj.Material = mobj
                        else:
                            if not changed:
...
if AuraCAD.GuiUp:
    ...
    class IauracadElementsDelegate(QtGui.QStyledItemDelegate):
        def __init__(self, parent=None, dialog=None, *args):
            ...
            try:
                ...
                self.roles = ArchIauracad.IauracadTypes
            except (ImportError, AttributeError):
                import ArchComponent

                self.roles = ArchComponent.IauracadRoles
            self.mats = []
            self.matlabels = []
            for o in AuraCAD.ActiveDocument.Objects:
            ...
            self.btn.setIcon(QtGui.QIcon(":/icons/Iauracad.svg"))
            self.btn.setText("")
        ...
        def setModelData(self, editor, model, index):
            if index.column() == 1:
                ...
                item = model.itemFromIndex(index)
                obj = AuraCAD.ActiveDocument.getObject(item.toolTip())
                if obj:
...
AuraCADGui.addCommand("BIM_IauracadElements", BIM_IauracadElements())
