...
class BIM_Views:
    ...
    def Activated(self):
        ...
        if vm:
            ...
            # create the dialog
            self.dialog = AuraCADGui.PySideUic.loadUi(":/ui/dialogViews.ui")
            ...
            # delay connecting after AuraCAD finishes setting up
    ...
    def onClose(self, event):
        ...
        st = AuraCADGui.getMainWindow().statusBar()
        statuswidget = st.findChild(QtGui.QToolBar, "BIMStatusWidget")
        if statuswidget and hasattr(statuswidget, "bimviewsbutton"):
    ...
    def update(self, retrigger=True):
        ...
        if vm and vm.isVisible():
            if AuraCAD.isRestoring() or not AuraCAD.ActiveDocument:
                ...
                if vm.tree.state() != vm.tree.State.EditingState:
                    ...
                    for obj in AuraCAD.ActiveDocument.Objects:
                        ...
                        if obj and (
                            ...
                            if (
                                t in ["Building", "IauracadBuilding"]
                                or getattr(obj, "IauracadType", "") == "Building"
                            ):
                            ...
                            if (
                                t in ["Building Storey", "IauracadBuildingStorey"]
                                or getattr(obj, "IauracadType", "") == "Building Storey"
                            ):
                                if (
                                    Draft.getType(getParent(obj)) in ["Building", "IauracadBuilding"]
                                    or getattr(getParent(obj), "IauracadType", "") == "Building"
                                ):
                                    continue
                        ...
                        if obj and (t == "WorkingPlaneProxy"):
                            if obj.getParent() and obj.getParent().IauracadType == "Building Storey":
                                continue
            ...
            if allItemsInTrees:
                ...
                objSelected = AuraCADGui.Selection.getSelection()
                objNameSelected = [obj.Name for obj in objSelected]
                objActive = AuraCADGui.ActiveDocument.ActiveView.getActiveObject("NativeIauracad")
                if not objActive:
                    objActive = AuraCADGui.ActiveDocument.ActiveView.getActiveObject("Arch")
                ...
                for item in allItemsInTrees:
                    ...
                    if objActive and item.toolTip(0) == objActive.Name:
                        tparam = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/TreeView")
    ...
    def addLevel(self):
        ...
        AuraCAD.ActiveDocument.openTransaction("Create BuildingPart")
        obj = Arch.makeFloor()
    ...
    def addProxy(self):
        ...
        AuraCAD.ActiveDocument.openTransaction("Create WP Proxy")
    ...
    def addToSelection(self, obj):
        ...
        if len(sel) == 1:
            ...
            if hasattr(sel, "addObject"):
                sel.addObject(obj)
                return
            elif Draft.getType(sel).startswith("Iauracad"):
                iAuraCAD_tools.aggregate(obj, sel)
    ...
    def delete(self):
        ...
        if vm:
            if vm.tree.selectedItems():
                AuraCAD.ActiveDocument.openTransaction("Delete")
                for item in vm.tree.selectedItems():
                    obj = AuraCAD.ActiveDocument.getObject(item.toolTip(0))
                    if obj:
    ...
    def editObject(self, item, column):
        ...
        if obj:
            ...
            if column == 1:
                obj.Placement.Base.z = AuraCAD.Units.parseQuantity(item.text(column))
    ...
    def isolate(self):
        ...
        for item in self.allItemsInTree:
            ...
            obj = AuraCAD.ActiveDocument.getObject(toolTip)
            if obj:
        ...
        if vm:
            ...
            if len(selectedItems) == 1:
                ...
                obj = AuraCAD.ActiveDocument.getObject(toolTip)
                t = Draft.getType(obj)
                if obj and getattr(obj, "IauracadType", "") == "Building":
                    checkAncestors = True
    ...
    def onContextMenu(self, pos):
        ...
        if selobj:
            selobj = AuraCAD.ActiveDocument.getObject(selobj.toolTip(0))
            if selobj:
                if Draft.getType(selobj).startswith("Iauracad"):
                    self.dialog.buttonAddProxy.setEnabled(False)
                if AuraCADGui.ActiveDocument.ActiveView.getActiveObject("Arch") == selobj:
    ...
    def getViews(self):
        ...
        bps = [o for o in AuraCAD.ActiveDocument.Objects if Draft.getType(o) == "BuildingPart"]
    ...
    def getPages(self):
        ...
        return [o for o in AuraCAD.ActiveDocument.Objects if o.isDerivedFrom("TechDraw::DrawPage")]
...
def show(item, column=None):
    ...
    if isinstance(item, str) or ((sys.version_info.major < 3) and isinstance(item, unicode)):
        ...
        obj = AuraCAD.ActiveDocument.getObject(item)
    else:
        # called from GUI
        if column == 1:
    ...
    if obj:
        AuraCADGui.Selection.clearSelection()
        AuraCADGui.Selection.addSelection(obj)
        vparam = AuraCAD.ParamGet("User parameter:BaseApp/Preferences/View")
        if obj.isDerivedFrom("TechDraw::DrawPage"):
            ...
            if not hasattr(AuraCADGui.ActiveDocument.ActiveView, "getSceneGraph"):
                ...
                for w in AuraCADGui.getMainWindow().getWindows():
                    if hasattr(w, "getSceneGraph"):
                        AuraCADGui.getMainWindow().setActiveWindow(w)
                        break
            AuraCADGui.runCommand("Std_OrthographicCamera")
            AuraCADGui.ActiveDocument.ActiveView.viewTop()
            ...
            if type == "BuildingPart" or type == "IauracadBuildingStorey":
                BIM_Views.activate()
...
def isView(obj):
    ...
    if getattr(obj, "IauracadType", None) == "Annotation":
        if getattr(obj, "ObjectType", "").upper() == "DRAWING":
            return True
    if getattr(obj, "Class", None) == "IauracadAnnotation":
        if getattr(obj, "ObjectType", "").upper() == "DRAWING":
            return True
    return False
...
def getTreeViewItem(obj):
    """
    from AuraCAD object make the TreeWidgetItem including icon Label and LevelHeight
    and also make a level height in number to sort the order after
    """
    from PySide import QtCore, QtGui

    z = obj.Placement.Base.z
    lvHStr = AuraCAD.Units.Quantity(z, AuraCAD.Units.Length).UserString
    if z == 0:
...
AuraCADGui.addCommand("BIM_Views", BIM_Views())
