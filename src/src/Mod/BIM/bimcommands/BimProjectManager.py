...
def _parse_vector(text):
    ...
    return AuraCAD.Vector(float(match.group(1)), float(match.group(2)), float(match.group(3)))
...
class BIM_ProjectManager:
    ...
    def Activated(self):
        ...
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/dialogProjectManager.ui")
        self.project = None
        self.site = None
        self.building = None
        ...
        if doc:
            if len(sel) == 1:
                if hasattr(sel[0], "Proxy") and hasattr(sel[0].Proxy, "iauracadfile"):
            ...
            if hasattr(doc, "Proxy"):
                ...
                if hasattr(doc.Proxy, "iauracadfile"):
                    self.project = doc
                    self.form.groupNewProject.setEnabled(False)
            if self.project:
                from nativeiauracad import iAuraCAD_tools

                sites = iAuraCAD_tools.get_children(self.project, iauracadtype="IauracadSite")
                sites = list(filter(None, [iAuraCAD_tools.get_object(s) for s in sites]))
                self.form.projectName.setText(self.project.Label)
            else:
                sites = [o for o in doc.Objects if getattr(o, "IauracadType", "") == "Site"]
            if sites:
                self.site = sites[0]
            ...
            if self.site and self.project:
                ...
                buildings = iAuraCAD_tools.get_children(self.site, iauracadtype="IauracadBuilding")
                buildings = list(filter(None, [iAuraCAD_tools.get_object(b) for b in buildings]))
            if not buildings:
                buildings = [o for o in doc.Objects if getattr(o, "IauracadType", "") == "Building"]
            if buildings:
            ...
            if self.building and self.project:
                ...
                levels = iAuraCAD_tools.get_children(self.building, iauracadtype="IauracadBuildingStorey")
                levels = list(filter(None, [iAuraCAD_tools.get_object(l) for l in levels]))
            if not levels:
                levels = [o for o in doc.Objects if getattr(o, "IauracadType", "") == "Building Storey"]
            if levels:
    ...
    def accept(self):
        ...
        # reading form values
        buildingWidth = AuraCAD.Units.Quantity(self.form.buildingWidth.text()).Value
        buildingLength = AuraCAD.Units.Quantity(self.form.buildingLength.text()).Value
        distVAxes = AuraCAD.Units.Quantity(self.form.distVAxes.text()).Value
        distHAxes = AuraCAD.Units.Quantity(self.form.distHAxes.text()).Value
        levelHeight = AuraCAD.Units.Quantity(self.form.levelHeight.text()).Value
        ...
        if self.form.groupNewProject.isChecked():
            ...
            if self.form.radioNative2.isChecked():
                self.project = iAuraCAD_tools.create_document_object(doc, silent=True)
                if self.form.projectName.text():
                    self.project.Label = self.form.projectName.text()
            elif self.form.radioNative3.isChecked():
                self.project = iAuraCAD_tools.convert_document(doc, silent=True)
        ...
        if self.form.addHumanFigure.isChecked():
            ...
            human = AuraCAD.ActiveDocument.addObject("Part::Feature", "Human")
            human.Shape = Part.makePolygon(pts)
            human.Placement.move(AuraCAD.Vector(500, 500, 0))
        ...
        if self.form.groupSite.isChecked():
            if not self.site:
                ...
                if self.project:
                    ...
                    self.site = iAuraCAD_tools.aggregate(self.site, self.project)
            ...
            elev = AuraCAD.Units.Quantity(self.form.siteElevation.text()).Value
            if hasattr(self.site, "Elevation"):
        ...
        if self.form.groupBuilding.isChecked():
            if not self.building:
                ...
                if self.project:
                    ...
                    if self.site:
                        self.building = iAuraCAD_tools.aggregate(self.building, self.site)
                    else:
                        self.building = iAuraCAD_tools.aggregate(self.building, self.project)
                elif self.site:
            ...
            if self.form.countHAxes.value() and distHAxes:
                ...
                axisH.Placement.Rotation = AuraCAD.Rotation(AuraCAD.Vector(0, 0, 1), 90)
            ...
            if self.form.countLevels.value() and levelHeight:
                ...
                for i in range(self.form.countLevels.value()):
                    ...
                    lev.Placement.move(AuraCAD.Vector(0, 0, h))
                    if self.project and self.building:
                        from nativeiauracad import iAuraCAD_tools

                        lev = iAuraCAD_tools.aggregate(lev, self.building)
                    elif self.building:
                        self.building.addObject(lev)
                    h += levelHeight
                    for group in groups:
                        levGroup = AuraCAD.ActiveDocument.addObject("App::DocumentObjectGroup")
                        levGroup.Label = group
                        if self.project:
                            from nativeiauracad import iAuraCAD_tools

                            iAuraCAD_tools.aggregate(levGroup, lev)
                        else:
    ...
    def savePreset(self):
        ...
        if res[1]:
            name = res[0]
            presetdir = os.path.join(AuraCAD.getUserAppDataDir(), "BIM")
            ...
            preset = f"# AuraCAD BIM Project setup preset { name }\n"
    ...
    def getPreset(self, preset):
        ...
        pfile = os.path.join(AuraCAD.getUserAppDataDir(), "BIM", preset + ".txt")
    ...
    def saveTemplate(self):
        ...
        values["unit"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Units").GetInt("UserSchema", 0)
        )
        values["textsize"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetFloat(
                "textheight", 10
            )
        )
        values["textfont"] = str(
        ...
        values["dimstyle"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetInt("dimsymbol", 0)
        )
        ...
        values["decimals"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Units").GetInt("Decimals", 2)
        )
        values["grid"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetFloat(
                "gridSpacing", 10
        ...
        values["squares"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetInt("gridEvery", 10)
        )
        values["linewidth"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/View").GetInt(
                "DefautShapeLineWidth", 2
            )
        )
        values["colFace"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/View").GetUnsigned(
        ...
        values["colHelp"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetUnsigned(
                "ColorHelpers", 674321151
            )
        )
        values["colConst"] = str(
            AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetUnsigned(
        ...
        filename = QtGui.QFileDialog.getSaveFileName(
            ...
            None,
            "AuraCAD file (*.auracadStd)",
        )
        if filename:
            filename = filename[0]
            if not filename.lower().endswith(".auracadStd"):
                filename += ".auracadStd"
            d.saveCopy(filename)
            AuraCAD.Console.PrintMessage(translate("BIM", "Template saved successfully") + "\n")
            self.reject()
    ...
    def loadTemplate(self):
        ...
        if filename:
            ...
            if AuraCAD.ActiveDocument:
                d = AuraCAD.ActiveDocument
                td = AuraCAD.openDocument(filename, True)  # hidden
                tname = td.Name
                values = td.Meta
                AuraCAD.closeDocument(tname)
                d.mergeProject(filename)
                AuraCADGui.ActiveDocument = AuraCADGui.getDocument(d.Name)  # fix b/c hidden doc
            else:
                d = AuraCAD.openDocument(filename)
                AuraCAD.ActiveDocument = d
                values = d.Meta
            bimunit = 0
            wp = WorkingPlane.get_working_plane()
            for key, attr in [
            ...
            if "unit" in values:
                AuraCAD.ParamGet("User parameter:BaseApp/Preferences/Units").SetInt(
                    "UserSchema", int(values["unit"])
                )
                if hasattr(AuraCAD.Units, "setSchema"):
                    AuraCAD.Units.setSchema(int(values["unit"]))
            ...
            if "textsize" in values:
                ...
                if hasattr(AuraCADGui, "draftToolBar"):
                    AuraCADGui.draftToolBar.fontsizeButton.setValue(float(values["textsize"]))
            ...
            if "linewidth" in values:
                ...
                if hasattr(AuraCADGui, "draftToolBar"):
                    AuraCADGui.draftToolBar.widthButton.setValue(int(values["linewidth"]))
...
AuraCADGui.addCommand("BIM_ProjectManager", BIM_ProjectManager())
