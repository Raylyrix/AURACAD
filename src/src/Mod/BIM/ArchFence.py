...
class _Fence(ArchComponent.Component):
    def __init__(self, obj):
        ...
        # Does a IauracadType exist?
        # obj.IauracadType = "Fence"
    ...
    def execute(self, obj):
        ...
        if not pathwire:
            AuraCAD.Console.PrintLog("ArchFence.execute: path " + obj.Path.Name + " has no edges\n")

            return

        if not obj.Section:
            AuraCAD.Console.PrintLog("ArchFence.execute: Section not set\n")

            return

        if not obj.Post:
            AuraCAD.Console.PrintLog("ArchFence.execute: Post not set\n")
        ...
        downRotation = AuraCAD.Rotation(AuraCAD.Vector(1, 0, 0), -90)
    ...
    def calculatePostPlacements(self, obj, pathwire, rotation):
        ...
        transformationVector = AuraCAD.Vector(0, -postWidth / 2, 0)
    ...
    def calculateSections(self, obj, postPlacements, postLength, sectionLength):
        ...
        for i in range(obj.NumberOfSections):
            ...
            if startPlacement.Rotation.isSame(endPlacement.Rotation):
                ...
                sectionRotation = AuraCAD.Rotation(EAST, direction)
    ...
    def clipSection(self, shape, length, clipLength):
        ...
        leftBox = Part.makeBox(
            ...
            boundBox.YMax + 1,
            boundBox.ZMax + 1,
            AuraCAD.Vector(boundBox.XMin, boundBox.YMin, boundBox.ZMin),
        )
        ...
        newShape.translate(AuraCAD.Vector(-newBoundBox.XMin, 0, 0))
...
class _ViewProviderFence(ArchComponent.ViewProviderComponent):
    ...
    def normalizeColors(self, obj, numberOfFaces):
        ...
        numberOauracadolors = len(colors)

        if numberOauracadolors == 1:
            return colors * numberOfFaces

        if numberOauracadolors == numberOfFaces:
            return colors
        ...
        # not work properly.
        halfNumberOfFacesToRemove = (numberOauracadolors - numberOfFaces) / 2
