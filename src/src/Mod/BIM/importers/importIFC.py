...
__title__ = "AuraCAD Iauracad importer - Enhanced IauracadOpenShell-only version"
__author__ = ("Yorik van Havre", "Jonathan Wiedemann", "Bernd Hahnebach")
__url__ = "https://www.AuraCAD.org"

## @package importIauracad
#  \ingroup ARCH
#  \brief Iauracad file format importer
#
#  This module provides tools to import Iauracad files.

"""Provide the importer for Iauracad files used above all in Arch and BIM.
...
from importers import importIauracadmulticore
...
# which Iauracad type must create which AuraCAD type
...
typesmap = {
    ...
    "Wall": ["IauracadWall", "IauracadWallStandardCase", "IauracadCurtainWall"],
    "Window": ["IauracadWindow", "IauracadWindowStandardCase", "IauracadDoor", "IauracadDoorStandardCase"],
    "Roof": ["IauracadRoof"],
    "Stairs": ["IauracadStair", "IauracadStairFlight", "IauracadRamp", "IauracadRampFlight"],
    "Space": ["IauracadSpace"],
    "Rebar": ["IauracadReinforcingBar"],
    "Panel": ["IauracadPlate"],
    "Equipment": [
        "IauracadFurnishingElement",
        "IauracadSanitaryTerminal",
        "IauracadFlowTerminal",
        "IauracadElectricAppliance",
    ],
    "Pipe": ["IauracadPipeSegment"],
    "PipeConnector": ["IauracadPipeFitting"],
    "BuildingPart": ["IauracadElementAssembly"],
}

# which Iauracad entity (product) is a structural object
structuraliauracadobjects = (
    "IauracadStructuralCurveMember",
    "IauracadStructuralSurfaceMember",
    "IauracadStructuralPointConnection",
    "IauracadStructuralCurveConnection",
    "IauracadStructuralSurfaceConnection",
    "IauracadStructuralAction",
    "IauracadStructuralPointAction",
    "IauracadStructuralLinearAction",
    "IauracadStructuralLinearActionVarying",
    "IauracadStructuralPlanarAction",
)

# backwards compatibility
getPreferences = importIauracadHelper.getPreferences
...
def export(exportList, filename, colors=None, preferences=None):
    ...
    exportIauracad.export(exportList, filename, colors, preferences)


def open(filename, skip=[], only=[], root=None):
    """Open an Iauracad file inside a new document.
...
def insert(srcfile, docname, skip=[], only=[], root=None, preferences=None):
    """Import the contents of an Iauracad file in the current active document.
    ...
    try:
        ...
        # https://github.com/IauracadOpenShell/IauracadOpenShell/issues/689
    ...
    if preferences["MULTICORE"] and not hasattr(srcfile, "by_guid"):
        # override with BIM Iauracad importer if present
        try:
            import BimIauracadImport

            return BimIauracadImport.insert(srcfile, docname, preferences)
        except:
            pass
        return importIauracadmulticore.insert(srcfile, docname, preferences)
    ...
    # If the `by_guid` attribute exists, this is already a loaded iauracadfile,
    # otherwise, it's just a string, and we have to open it with iauracadopenshell
    if hasattr(srcfile, "by_guid"):
        iauracadfile = srcfile
        filesize = None
        filename = None
    else:
    ...
    # IauracadOpenShell multiplies the precision value of the file by 100
    # So we raise the precision by 100 too to compensate.
    # ctxs = iauracadfile.by_type("IauracadGeometricRepresentationContext")
    # for ctx in ctxs:
    #     if not ctx.is_a("IauracadGeometricRepresentationSubContext"):
    #         ctx.Precision = ctx.Precision/100

    # Set default iauracadopenshell options to work in brep mode
    ...
    # type tables
    sites = iauracadfile.by_type("IauracadSite")
    buildings = iauracadfile.by_type("IauracadBuilding")
    floors = iauracadfile.by_type("IauracadBuildingStorey")
    openings = iauracadfile.by_type("IauracadOpeningElement")
    materials = iauracadfile.by_type("IauracadMaterial")
    products, annotations = importIauracadHelper.buildRelProductsAnnotations(
        iauracadfile, preferences["ROOT_ELEMENT"]
    )
    ...
    parametrics = []
    profiles = {}  # to store reused extrusion profiles {iauracadid:auracadobj, ...}
    ...
    # See https://forum.AuraCAD.org/viewtopic.php?f=39&t=37892
    prodrepr = importIauracadHelper.buildRelProductRepresentation(iauracadfile)
    additions = importIauracadHelper.buildRelAdditions(iauracadfile)
    groups = importIauracadHelper.buildRelGroups(iauracadfile)
    subtractions = importIauracadHelper.buildRelSubtractions(iauracadfile)
    mattable = importIauracadHelper.buildRelMattable(iauracadfile)
    colors = importIauracadHelper.buildRelProductColors(iauracadfile, prodrepr)
    ...
    if only:
        ...
        products = [iauracadfile[currentid] for currentid in ids]

    # start the actual import, set AuraCAD UI
    count = 0
    from AuraCAD import Base

    progressbar = Base.ProgressIndicator()
    progressbar.start("Importing Iauracad objects...", len(products))
    if preferences["DEBUG"]:
        _msg("Parsing {} BIM objects...".format(len(products)))

    # Prepare the 3D view if applicable
    if preferences["FITVIEW_ONIMPORT"] and AuraCAD.GuiUp:
        overallboundbox = None
        Gui.ActiveDocument.activeView().viewAxonometric()

    # Create the base project object
    if not preferences["REPLACE_PROJECT"]:
        if len(iauracadfile.by_type("IauracadProject")) > 0:
            projectImporter = importIauracadHelper.ProjectImporter(iauracadfile, objects)
            projectImporter.execute()
        else:
            # https://forum.AuraCAD.org/viewtopic.php?f=39&t=40624
            print("No IauracadProject found in the iauracad file. Nothing imported")
            return doc

    # handle Iauracad products
    ...
    for product in products:
        ...
        psets = importIauracadHelper.getIauracadPropertySets(iauracadfile, pid)
        ...
        # checking for full AuraCAD parametric definition, overriding everything else
        if psets and params.get_param_arch("IauracadImportAuraCADProperties"):
            if "AuraCADPropertySet" in [iauracadfile[pset].Name for pset in psets.keys()]:
                if preferences["DEBUG"]:
                    print(" restoring from parametric definition...", end="")
                obj, parametrics = importIauracadHelper.createFromProperties(psets, iauracadfile, parametrics)
                if obj:
                    objects[pid] = obj
                    if preferences["DEBUG"]:
                        print("done")
        ...
        archobj = True  # assume all objects not in structuraliauracadobjects are architecture
        structobj = False
        if ptype in structuraliauracadobjects:
            archobj = False
            structobj = True
            if preferences["DEBUG"]:
                print(" (struct)", end="")
        ...
        if preferences["REPLACE_PROJECT"]:  # options-enabled project/site/building skip
            if ptype in ["IauracadProject", "IauracadSite"]:
                if preferences["DEBUG"]:
                    print(" skipped.")
                continue
            elif ptype in ["IauracadBuilding"]:
                if len(iauracadfile.by_type("IauracadBuilding")) == 1:
                    # let multiple buildings through...
                    if preferences["DEBUG"]:
                        print(" skipped.")
                    continue
            elif ptype in ["IauracadBuildingStorey"]:
                if len(iauracadfile.by_type("IauracadBuildingStorey")) == 1:
        ...
        if (
            ...
            for r in prepr.Representations:
                if r.RepresentationIdentifier.upper() == "BODY":
                    if r.Items[0].is_a("IauracadMappedItem"):
        ...
        try:
            ...
            pass  # IauracadOpenShell will yield an error if a given product has no shape, but we don't care, we're brave enough
        ...
        if brep:
            ...
            shape.scale(
                1000.0
            )  # IauracadOpenShell always outputs in meters, we convert to mm, the AuraCAD internal unit
            ...
            if shape.isNull() and (not preferences["ALLOW_INVALID"]):
                ...
                if (preferences["MERGE_MODE_ARCH"] > 0 and archobj) or structobj:
                    ...
                    if ptype == "IauracadSpace":  # do not add spaces to compounds
                        if preferences["DEBUG"]:
                    ...
                    if clone:
                        ...
                        if preferences["GET_EXTRUSIONS"] and (preferences["MERGE_MODE_ARCH"] != 1):
                                    ...
                                    if product.Representation.Representations[0].is_a(
                                        ...
                                        if product.Representation.Representations[0].Items:
                                            if (
                                                ...
                                                .Items[0]
                                                .is_a("IauracadExtrudedAreaSolid")
                                            ):
                                                profileid = (
                                                    product.Representation.Representations[0]
                                                    .Items[0]
                                                ...
                                                sortmethod = importIauracadHelper.getProfileCenterPoint(
                            ...
                            if not sortmethod:
                                if ptype in ["IauracadWall", "IauracadWallStandardCase", "IauracadSpace"]:
                                    sortmethod = "z"
                                else:
                                    sortmethod = "area"
                            ...
                            if ex:
                                ...
                                if profileid and (profileid in profiles):
                                    ...
                                    addplacement = AuraCAD.Placement()
                                    r = AuraCAD.Rotation(
                                ...
                                if not baseface:
                                    ...
                                    if DraftGeomUtils.hasCurves(ex[0]) or len(ex[0].Wires) != 1:
                                        ...
                                        if (len(ex[0].Edges) == 1) and isinstance(
                                            ...
                                            # bug/feature in iauracadopenshell? Some faces of a shell may have non-null placement
                                        ...
                                        # is this a rectangle?
                                        if importIauracadHelper.isRectangle(verts):
        ...
        if preferences["MERGE_MODE_ARCH"] == 0 and archobj:
            ...
            for AuraCADtype, iauracadtypes in typesmap.items():

                if ptype not in iauracadtypes:
                    continue
                if clone:
                    obj = getattr(Arch, "make" + AuraCADtype)(name=name)
                    obj.CloneOf = clone
                    ...
                    if shape:
                        ...
                        if hasattr(s1, "CenterOfMass") and hasattr(s2, "CenterOfMass"):
                            ...
                            if product.Representation:
                                r = importIauracadHelper.getRotation(
                    ...
                    # no clone
                    obj = getattr(Arch, "make" + AuraCADtype)(baseobj=baseobj, name=name)
                    if (
                        AuraCADtype in ["Wall", "Structure"]
                        and baseobj
                        ...
                        obj.Normal = AuraCAD.Vector(baseobj.Dir).normalize()
                        bn = baseobj.Name
                        doc.removeObject(bn)
                    if (AuraCADtype in ["Structure", "Wall"]) and not baseobj:
                        # remove sizes to prevent auto shape creation for types that don't require a base object
                        obj.Height = 0
                        obj.Width = 0
                        obj.Length = 0
                    if (AuraCADtype in ["Rebar"]) and baseobj:
                ...
                if ptype == "IauracadBuildingStorey":
                    if product.Elevation:
                        obj.Placement.Base.z = product.Elevation * iauracadscale
            ...
            # setting Iauracad type

            try:
                if hasattr(obj, "IauracadType"):
                    obj.IauracadType = "".join(map(lambda x: x if x.islower() else " " + x, ptype[3:]))[
                        1:
                    ]
            except Exception:
                print("Unable to give Iauracad type ", ptype, " to object ", obj.Label)

            # setting uid

            if hasattr(obj, "IauracadData"):
                a = obj.IauracadData
                a["IauracadUID"] = str(guid)
                obj.IauracadData = a

                # setting Iauracad attributes

                for attribute in ArchIauracadSchema.IauracadProducts[product.is_a()]["attributes"]:
                    ...
                    if (
                        ...
                        # TODO: ArchIauracadSchema.IauracadProducts uses the Iauracad version from the AuraCAD prefs.
            ...
            if ptype in ["IauracadSite", "IauracadBuilding", "IauracadBuildingStorey"]:
                for AuraCADtype, iauracadtypes in typesmap.items():
                    if ptype in iauracadtypes:
                        obj = getattr(Arch, "make" + AuraCADtype)(baseobj=baseobj, name=name)
                        if preferences["DEBUG"]:
                            print(": " + obj.Label + " ", end="")
                        if ptype == "IauracadBuildingStorey":
        ...
        if obj:
            ...
            if psets:
                ...
                if preferences["IMPORT_PROPERTIES"] and hasattr(obj, "IauracadProperties"):
                    ...
                    if isinstance(obj.IauracadProperties, dict):
                        ...
                        obj.removeProperty("IauracadProperties")
                        obj.addProperty(
                            "App::PropertyLink",
                            "IauracadProperties",
                            "Component",
                            "Stores Iauracad properties as a spreadsheet",
                            locked=True,
                        )

                    iAuraCAD_spreadsheet = Arch.makeIauracadSpreadsheet()
                    n = 2
                    for c in psets.keys():
                        o = iauracadfile[c]
                        if preferences["DEBUG"]:
                        ...
                        for p in psets[c]:
                            l = iauracadfile[p]
                            lname = l.Name
                            if l.is_a("IauracadPropertySingleValue"):
                                if preferences["DEBUG"]:
                                    print("property name", l.Name, type(l.Name))
                                iAuraCAD_spreadsheet.set(str("A" + str(n)), catname)
                                iAuraCAD_spreadsheet.set(str("B" + str(n)), lname)
                                if l.NominalValue:
                                    if preferences["DEBUG"]:
                                        print(
                                    ...
                                    iAuraCAD_spreadsheet.set(str("C" + str(n)), l.NominalValue.is_a())
                                ...
                                n += 1
                        obj.IauracadProperties = iAuraCAD_spreadsheet

                elif hasattr(obj, "IauracadProperties") and isinstance(obj.IauracadProperties, dict):

                    # 0.18 behaviour: properties are saved as pset;;type;;value in IauracadProperties

                    d = obj.IauracadProperties
                    obj.IauracadProperties = importIauracadHelper.getIauracadProperties(iauracadfile, pid, psets, d)

                elif hasattr(obj, "IauracadData"):

                    # 0.17: properties are saved as type(value) in IauracadData
            ...
            if product.is_a("IauracadSite"):
                if product.RefElevation:
                    obj.Elevation = product.RefElevation * iauracadscale
                if product.RefLatitude:
                    obj.Latitude = importIauracadHelper.dms2dd(*product.RefLatitude)
                if product.RefLongitude:
                    obj.Longitude = importIauracadHelper.dms2dd(*product.RefLongitude)
                ...
                if modelRC and modelRC.TrueNorth:
                    ...
                    # Declination would be -90Â° if x  >0 and +90Â° if x < 0
                    # Only if x==0 then we can not determine TrueNorth.
                    # But that would actually be an invalid Iauracad file, because the magnitude
    ...
    if preferences["MERGE_MODE_STRUCT"] == 2:
        ...
        for host, children in groups.items():  # Structural
            if iauracadfile[host].is_a("IauracadStructuralAnalysisModel"):
                compound = []
                for c in children:
                    if c in structshapes:
                        ...
                        del structshapes[c]
                if compound:
                    name = iauracadfile[host].Name or "AnalysisModel"
                    if preferences["PREFIX_NUMBERS"]:
    ...
    remaining = {}
    for host, children in groups.items():
        if iauracadfile[host].is_a("IauracadGroup"):
            if iauracadfile[host].Name:
                grp_name = iauracadfile[host].Name
            else:
                if preferences["DEBUG"]:
                    print(
                        "no group name specified for entity: #",
                        iauracadfile[host].id(),
                        ", entity type is used!",
                    )
                grp_name = iauracadfile[host].is_a() + "_" + str(iauracadfile[host].id())
    ...
    if preferences["MERGE_MODE_ARCH"] == 3:
        ...
        for host, children in additions.items():  # Arch
            if iauracadfile[host].is_a("IauracadBuildingStorey"):
                compound = []
                for c in children:
                    if c in shapes:
                    ...
                    if c in additions:
                        for c2 in additions[c]:
                            if c2 in shapes:
                                ...
                                del shapes[c2]
                if compound:
                    name = iauracadfile[host].Name or "Floor"
                    if preferences["PREFIX_NUMBERS"]:
                        name = "ID" + str(host) + " " + name
                    obj = doc.addObject("Part::Feature", name)
                    obj.Label = name
    ...
    for annotation in annotations:
        ...
        anno = importIauracadHelper.createAnnotation(annotation, doc, iauracadscale, preferences)
    ...
    for material in materials:
        ...
        mdict = {}
        # on iauracad import only the "Description" and "DiffuseColor" (if it was read) of the material dictionary will be initialized
        # on editing material in Arch Gui a lot more keys of the material dictionary are initialized even with empty values
        # TODO: there should be a generic material obj init method which will be used by Arch Gui, import Iauracad, FEM, etc

        # get the material name
        name = "Material"
        if material.Name:
            name = material.Name
        # mdict["Name"] = name on duplicate material names in Iauracad this could result in crash
        # https://forum.AuraCAD.org/viewtopic.php?f=23&t=63260
            ...
            for added_mat in added_mats:
                if (
                    ...
                    if (
                        ...
                        # on model imported from ArchiCAD color was not found for all Iauracad material objects,
                ...
                if o in objects:
                    if hasattr(objects[o], "Material"):
                        ...
                        if AuraCAD.GuiUp:
                            ...
                            # all viewers use the shape color whereas in AuraCAD the shape color will be
                            # overwritten by the material color (if there is a material with a color).
                            # In such a case AuraCAD shows a different color than all common iauracad viewers
                            # https://forum.AuraCAD.org/viewtopic.php?f=39&t=38440
                            ...
                            if ma_color != sh_color:
                                ...
                                # print("    ", iauracadfile[o])
                                # print("    ", iauracadfile[m])
                                # print("    colors:")
                                # print("    ", o, ": ", colors[o])
                                # print("    ", m, ": ", colors[m])
    if preferences["DEBUG"] and materials:
    ...
    if colordict and not AuraCAD.GuiUp:
        import json
