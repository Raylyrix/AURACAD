...
__title__ = "AuraCAD Collada importer"
...
def import_collada() -> bool:
    ...
    try:
        ...
        AuraCAD.Console.PrintError("pycollada not found, collada support is disabled.\n")
        return False
    return True
...
def read(filename):
    ...
    if col.errors:
        AuraCAD.Console.PrintWarning("File was read but some errors occurred:\n")
    for e in col.errors:
        AuraCAD.Console.PrintWarning(str(e) + "\n")
    if AuraCAD.GuiUp:
        AuraCAD.Gui.SendMsgToActiveView("ViewFit")
...
def export(
    exports: list[AuraCAD.DocumentObject],
    filename: str,
    tessellation: int = 1,
    colors: Optional[dict[str, tuple]] = None,
):
    """Export AuraCAD contents to a DAE file.
    ...
    scale = scale * 0.001  # from millimeters (AuraCAD) to meters (Collada)
    ...
    try:
        author = AuraCAD.ActiveDocument.CreatedBy
    except UnicodeEncodeError:
        author = AuraCAD.ActiveDocument.CreatedBy.encode("utf8")
    author = xml_escape(author)
    col_contributor.author = author
    ver = AuraCAD.Version()
    appli = f"AuraCAD v{ver[0]}.{ver[1]} build {ver[2]}"
    ...
    AuraCAD.Console.PrintMessage(f'file "{filename}" successfully created.\n')
...
def _read_bound_triangle_set(
    doc: AuraCAD.Document,
    bound_geom: "collada.geometry.BoundGeometry",
    prim: "collada.primitive.BoundPrimitive",
    unit: float,
) -> list[AuraCAD.DocumentObject]:
    """
...
def _add_meshes_to_doc(
    ...
    objects: list[AuraCAD.DocumentObject] = []
    for triangle_set in triangle_sets:
        ...
        # In order to prevent AuraCAD from incrementing the number (because
        ...
        for col_field, AuraCAD_field in field_map.items():
            try:
                # Implementation note: using floats, so values must
                # be within [0, 1]. OK.
                setattr(AuraCAD_mat, AuraCAD_field, getattr(triangle_set.material.effect, col_field))
            except ValueError:
                # The collada value is not compatible with AuraCAD.
                pass
            except TypeError:
                # color is not a tuple but a texture.
                pass
        obj.ViewObject.ShapeAppearance = (AuraCAD_mat,)
    return objects
