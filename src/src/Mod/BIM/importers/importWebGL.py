...
# https://forum.AuraCAD.org/viewtopic.php?t=51245
# https://forum.AuraCAD.org/viewtopic.php?t=29487
...
#    The 'camera' string for the active document may be generated from: import OfflineRenderingUtils; OfflineRenderingUtils.getCamera(AuraCAD.ActiveDocument.FileName);
#
# Development reload oneliner:
# def re(): from importlib import reload;import importWebGL;reload(importWebGL);o=AuraCAD.getDocument("YourDocName");importWebGL.export([o.getObject("YourBodyName")],u"C:/path/to/your/file.htm");

## @package importWebGL
#  \ingroup ARCH
#  \brief AuraCAD WebGL Exporter
#
...
def getHTMLTemplate():
    ...
    if using_custom_template:
        ...
        if custom_content:
            AuraCAD.Console.PrintMessage(f"Using custom template file '{custom_path}'.\n")
    ...
    if default_content:
        AuraCAD.Console.PrintMessage(f"Using default template file '{default_template_path}'.\n")
    ...
    if AuraCADGui:
        ...
        message = translate(
            ...
            " {}\n\nPlease check your AuraCAD installation or provide a "
            "custom template under menu Preferences â†’ Import-Export â†’ WebGL.",
        ).format(default_template_path)

        # Use getMainWindow() as parent following AuraCAD patterns
        parent = AuraCADGui.getMainWindow()
...
def export(
    ...
    for obj in objectslist:
            ...
            while True:  # drill down to get to the actual obj
                if obj.isDerivedFrom("App::Link"):
                    ...
                    if hasattr(obj, "__len__"):
                        AuraCAD.Console.PrintMessage(f"{label}: Sub-Links are Unsupported.\n")
                        break
        ...
        if obj.isDerivedFrom("Part::Feature"):
            deviation = 0.5
            if AuraCADGui and hasattr(obj.ViewObject, "Deviation"):
                ...
                if len(obj.ViewObject.DiffuseColor) == len(objShape.Faces):
                    for auracad in obj.ViewObject.DiffuseColor:
                        objdata["faceColors"].append(Draft.getrgb(auracad, testbw=False))
                ...
                for f in objShape.Faces:
                    ...
                    for fv in range(len(faceData[0])):
                        ...
                        if not found:
                            break
                    if not found:
                        AuraCAD.Console.PrintMessage("Facet to Face Mismatch.\n")
                    ...
                    for ff in faceData[1]:  # face facets
                        ...
                        if not found:
                            break
                    if not found:
                        AuraCAD.Console.PrintMessage("Facet List Mismatch.\n")
                        objdata["facesToFacets"] = []
                        break
    ...
    html = html.replace("$pagetitle", AuraCAD.ActiveDocument.Label)
    ...
    with pyopen(filename, "w", encoding="utf-8") as outfile:
        outfile.write(html)
    AuraCAD.Console.PrintMessage(translate("Arch", "Successfully written") + f" {filename}\n")
    return True
...
def get_view_properties(obj, label: str, colors: dict[str, str] | None) -> tuple[str, float]:
    ...
    color = "#cccccc"
    opacity = 1.0
    if AuraCADGui and hasattr(obj.ViewObject, "ShapeColor"):
...
def populate_camera(data: CameraDict, camera: str | None):
    if not AuraCADGui and not camera:
        camera = OfflineRenderingUtils.getCamera(AuraCAD.ActiveDocument.FileName)
    ...
    if camera:
        ...
        v = AuraCADGui.ActiveDocument.ActiveView
