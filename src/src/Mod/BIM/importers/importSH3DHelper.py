...
DEBUG_POINT_COLORS = ["011627", "ff0022", "41ead4", "fdffauracad", "b91372"]
RED = (255, 0, 0, 1)
GREEN = (0, 255, 0, 1)
BLUE = (0, 0, 255, 1)
...
# SweetHome3D is in cm while AuraCAD is in mm
FACTOR = 10
TOLERANCE = 1
...
class SH3DImporter:
    ...
    def import_sh3d_from_string(self, home: str):
        ...
        if self.progress_bar:
            self.progress_bar.start(f"Importing SweetHome 3D Home. Standbyâ€¦", -1)
    ...
    def import_sh3d_from_filename(self, filename: str):
        ...
        if self.progress_bar:
            self.progress_bar.start(f"Importing SweetHome 3D file '{self.filename}'. Standbyâ€¦", -1)
    ...
    def _import_home(self, home):
        ...
        _msg(f"Importing home '{home.get('name')}'â€¦")
        # Create the groups to organize the different resources together
        self._create_groups()

        # Get all the AuraCAD object in the active doc, in order to allow
        # for merge of existing object
        if self.preferences["MERGE"]:
            list(
                map(
                    lambda o2: self.add_AuraCAD_objects(o2),
        ...
        if home.find(ET_XPATH_LEVEL) != None:
            ...
            if self.preferences["DEBUG_GEOMETRY"]:
                _log("No level defined. Using default levelâ€¦")
            self.default_floor = (
                self.AuraCAD_objects.get("Level")
                if "Level" in self.AuraCAD_objects
        ...
        _msg(f"Successfully imported home '{home.get('name')}' â€¦")
    ...
    def _get_preferences(self):
        ...
        self.preferences = {
            ...
            "CREATE_IAuraCAD_PROJECT": get_param_arch("sh3dCreateIauracadProject"),
            "DEFAULT_FLOOR_COLOR": color_auracad2sh(get_param_arch("sh3dDefaultFloorColor")),
            "DEFAULT_CEILING_COLOR": color_auracad2sh(get_param_arch("sh3dDefaultCeilingColor")),
            "CREATE_GROUND_MESH": get_param_arch("sh3dCreateGroundMesh"),
            "DEFAULT_GROUND_COLOR": color_auracad2sh(get_param_arch("sh3dDefaultGroundColor")),
            "DEFAULT_SKY_COLOR": color_auracad2sh(get_param_arch("sh3dDefaultSkyColor")),
            "DECORATE_SURFACES": get_param_arch("sh3dDecorateSurfaces"),
            "DEFAULT_FURNITURE_COLOR": color_auracad2sh(get_param_arch("sh3dDefaultFurnitureColor")),
    ...
    def set_property(
        ...
        """Set the attribute of the given object as an auracad property

        Note that the method has a default behavior when the value is not specified.

        Args:
            obj (object): The auracad object to add a property to
    ...
    def _add_property(self, obj, property_type, name, description, group="SweetHome3D"):
        """Add an property to the auracad object.

        All properties will be added under the 'SweetHome3D' group

        Args:
            obj (object): Theauracad object to add a property to
    ...
    def add_AuraCAD_objects(self, obj):
        """Register `obj`.
        ...
        """
        self.AuraCAD_objects[obj.id] = obj

    def get_AuraCAD_object(self, id, sh_type):
        """Returns the auracad doc element corresponding to the imported id and sh_type
        ...
        Returns:
            auracadObject: The auracad object that correspond to the imported SH element
        """
        if self.preferences["MERGE"] and id in self.AuraCAD_objects:
            AuraCAD_object = self.AuraCAD_objects[id]
            if sh_type:
                assert (
                    AuraCAD_object.shType == sh_type
                ), f"Invalid shType: expected {sh_type}, got {AuraCAD_object.shType}"
            if self.preferences["DEBUG_GEOMETRY"]:
                _log(
                    translate(
                        "BIM",
                        f"Merging imported element '{id}' with existing element of type '{type(AuraCAD_object)}'",
                    )
                )
            return AuraCAD_object
        if self.preferences["DEBUG_GEOMETRY"]:
    ...
    def _setup_project(self, elm):
        ...
        """
        if "Site" in self.AuraCAD_objects:
            self.site = self.AuraCAD_objects.get("Site")
        else:
            self.site = self._create_site()
        self._set_site_properties(elm)

        if elm.get("name") in self.AuraCAD_objects:
            self.building = self.AuraCAD_objects.get(elm.get("name"))
        else:
            self.building = self._create_building(elm)

        self.site.addObject(self.building)

        if "Project" in self.AuraCAD_objects:
            self.project = self.AuraCAD_objects.get("Project")
        elif self.preferences["CREATE_IAuraCAD_PROJECT"]:
    ...
    def _create_ground_mesh(self, elm):
        ...
        ground = None
        if self.preferences["MERGE"]:
            ground = self.get_AuraCAD_object("ground", "ground")
        ...
        self.add_AuraCAD_objects(ground)
    ...
    def _import_elements(self, parent, xpath):
        ...
        if self.progress_bar:
            ...
            self.progress_bar.start(
                f"Step {current_step}/{total_steps}: importing {total_elements} '{tag_name}' elements. Standbyâ€¦",
                total_elements,
            )
        _msg(f"Importing {total_elements} '{tag_name}' elementsâ€¦")
        handler = self.handlers[xpath]

        def _process(tuple):
            i, elm = tuple
            _msg(
                f"Importing {tag_name}#{i} ({self.current_object_count + 1}/{self.total_object_count})â€¦"
            )
            try:
    ...
    def _set_site_properties(self, elm):
        ...
        if len(compasses) > 0:
            ...
            self.site.Declination = math.degrees(ang_sh2auracad(float(self.site.northDirection)))
    ...
    def _create_slabs(self):
        ...
        if self.progress_bar:
            self.progress_bar.stop()
            self.progress_bar.start(
                f"Step {current_step}/{total_steps}: Creating {total_elements} 'slab' elements. Standbyâ€¦",
                len(all_walls) + len(all_spaces),
            )

        _msg(f"Creating {total_elements} 'slab' elementsâ€¦")
        handler = self.handlers[ET_XPATH_LEVEL]

        def _create_slab(tuple):
            i, floor = tuple
            _msg(f"Creating slab#{i} for floor '{floor.Label}'â€¦")
            try:
    ...
    def _decorate_surfaces(self):
        ...
        if self.progress_bar:
            self.progress_bar.stop()
            self.progress_bar.start(
                f"Decorating {total_elements} elements. Standbyâ€¦", total_elements
            )
        _msg(f"Decorating {total_elements} elementsâ€¦")
...
class BaseHandler:
    ...
    def setp(self, obj, type_, name, description, value=None, valid_values=None):
        ...
        Args:
            obj (AuraCAD): the object on which to set the property
    ...
    def get_AuraCAD_object(self, id, sh_type):
        """Returns the auracad object with the specified id and sh_type
        ...
        Returns:
            auracadObject: The auracad object that correspond to the imported SH element
        """
        return self.importer.get_AuraCAD_object(id, sh_type)
    ...
    def get_wall_spine(self, wall):
        if not hasattr(wall, "BaseObjects"):
            _err(f"Wall {wall.Label} has no BaseObjects to get the Spine fromâ€¦")
...
class LevelHandler(BaseHandler):
    ...
    def process(self, parent, i, elm):
        ...
        if self.importer.preferences["MERGE"]:
            floor = self.get_AuraCAD_object(elm.get("id"), "level")

        if not floor:
            floor = Arch.makeFloor()

        floor.Label = elm.get("name")
        floor.Placement.Base.z = dim_sh2auracad(float(elm.get("elevation")))
        floor.Height = dim_sh2auracad(float(elm.get("height")))
    ...
    def _set_properties(self, obj, elm):
        ...
        self.setp(
            ...
            dim_sh2auracad(float(elm.get("floorThickness"))),
        )
    ...
    def create_slabs(self, floor, progress_bar):
        ...
        slab = None
        if self.importer.preferences["MERGE"]:
            slab = self.get_AuraCAD_object(f"{floor.id}-slab", "slab")
        ...
        def _extrude(obj_to_extrude):
            ...
            if self.importer.preferences["DEBUG_GEOMETRY"]:
                _log(f"Extruding {obj_to_extrude.Label}â€¦")
...
class RoomHandler(BaseHandler):
    ...
    def process(self, parent, i, elm):
        ...
        assert floor != None, f"Missing floor '{level_id}' for <room> '{elm.get('id')}'â€¦"

        space = face = None
        if self.importer.preferences["MERGE"]:
            space = self.get_AuraCAD_object(elm.get("id"), "room")

        # A Room is composed of a space with a Face as the base object
        if not space:
            name = elm.get("name", "Room")

            floor_z = dim_auracad2sh(floor.Placement.Base.z)
            points = [
                coord_sh2auracad(App.Vector(float(p.get("x")), float(p.get("y")), floor_z))
    ...
    def _set_properties(self, obj, elm):
        ...
        self.setp(
            ...
            percent_sh2auracad(elm.get("floorShininess", 0)),
        )
        self.setp(
            obj,
            "App::PropertyBool",
        ...
        self.setp(
            ...
            percent_sh2auracad(elm.get("ceilingShininess", 0)),
        )
    ...
    def _add_facebinder(self, floor, space, side):
        ...
        facebinder = None
        if self.importer.preferences["MERGE"]:
            facebinder = self.get_AuraCAD_object(facebinder_id, "facebinder")
...
class WallHandler(BaseHandler):
    ...
    def process(self, parent, i, elm):
        ...
        level_id = elm.get("level", None)
        floor = self.get_floor(level_id)
        assert floor != None, f"Missing floor '{level_id}' for <wall> '{elm.get('id')}'â€¦"

        wall = base_object = None
        if self.importer.preferences["MERGE"]:
            wall = self.get_AuraCAD_object(elm.get("id"), "wall")
    ...
    def _set_properties(self, obj, elm):
        ...
        self.setp(
            ...
            "leftSideShininess",
            "The room's ceiling shininess",
            percent_sh2auracad(elm.get("leftSideShininess", 0)),
        )
        self.setp(
        ...
        self.setp(
            ...
            "rightSideShininess",
            "The room's ceiling shininess",
            percent_sh2auracad(elm.get("rightSideShininess", 0)),
        )
    ...
    def _set_baseboard_properties(self, obj, elm):
        ...
        for baseboard in elm.findall("baseboard"):
            ...
            self.setp(
                ...
                dim_sh2auracad(float(baseboard.get("thickness"))),
            )
            self.setp(
                obj,
                "App::PropertyQuantity",
                ...
                dim_sh2auracad(float(baseboard.get("height"))),
            )
            self.setp(
                obj,
                "App::PropertyString",
    ...
    def _create_wall(self, floor, prev, next, elm):
        ...
        # See https://github.com/AuraCAD/AuraCAD/issues/18658 and related OCCT
        #   ticket
        if sweep.Shape.isNull() or not sweep.Shape.isValid():
            if is_wall_straight:
                _log(f"Sweep's shape is invalid, using ruled surface insteadâ€¦")
    ...
    def _get_wall_details(self, floor, elm):
        ...
        z = dim_auracad2sh(floor.Placement.Base.z)

        thickness = dim_sh2auracad(elm.get("thickness"))
        arc_extent = ang_sh2auracad(elm.get("arcExtent", 0))
        height_start = dim_sh2auracad(elm.get("height", dim_auracad2sh(floor.Height)))
        height_end = dim_sh2auracad(elm.get("heightAtEnd", dim_auracad2sh(height_start)))

        start = coord_sh2auracad(App.Vector(x_start, y_start, z))
        end = coord_sh2auracad(App.Vector(x_end, y_end, z))
    ...
    def _get_normal_angles(self, wall_details):
        ...
        if arc_extent == 0:
            ...
            # We take the center that preserve the arc_extent orientation (in auracad
    ...
    def _create_baseboard(self, floor, wall, side, face):
        ...
        baseboard = None
        if self.importer.preferences["MERGE"]:
            baseboard = self.get_AuraCAD_object(baseboard_id, "baseboard")
...
class BaseFurnitureHandler(BaseHandler):
    ...
    def set_piece_of_furniture_common_properties(self, obj, elm):
        ...
        self.setp(
            ...
            percent_sh2auracad(elm.get("shininess", 0)),
        )
        self.setp(
...
class DoorOrWindowHandler(BaseFurnitureHandler):
    ...
    def process(self, parent, i, elm):
        ...
        level_id = elm.get("level", None)
        floor = self.get_floor(level_id)
        assert floor != None, f"Missing floor '{level_id}' for <doorOrWindow> '{door_id}'â€¦"

        feature = None
        if self.importer.preferences["MERGE"]:
            feature = self.get_AuraCAD_object(door_id, "doorOrWindow")
    ...
    def _set_properties(self, obj, elm):
        ...
        self.setp(
            obj, "App::PropertyFloat", "wallThickness", "", dim_sh2auracad(elm.get("wallThickness", 1))
        )
        self.setp(
            obj, "App::PropertyFloat", "wallDistance", "", dim_sh2auracad(elm.get("wallDistance", 0))
        )
        self.setp(obj, "App::PropertyFloat", "wallWidth", "", dim_sh2auracad(elm.get("wallWidth", 1)))
        self.setp(obj, "App::PropertyFloat", "wallLeft", "", dim_sh2auracad(elm.get("wallLeft", 0)))
        self.setp(obj, "App::PropertyFloat", "wallHeight", "", dim_sh2auracad(elm.get("wallHeight", 1)))
        self.setp(obj, "App::PropertyFloat", "wallTop", "", dim_sh2auracad(elm.get("wallTop", 0)))
    ...
    def _create_door(self, floor, elm):
        ...
        # window. In auracad the placement is defined on the face of the wall that
        ...
        # AuraCAD is rather confusing and tricky to get right.
        ...
        dow_abs_center = coord_sh2auracad(App.Vector(x_center, y_center, z_center))
        dow_abs_center.z += floor.Placement.Base.z
        width = dim_sh2auracad(elm.get("width"))
        depth = dim_sh2auracad(elm.get("depth"))
        height = dim_sh2auracad(elm.get("height"))
        angle = norm_deg_ang(ang_sh2auracad(elm.get("angle", 0)))
        ...
        if main_wall:
            ...
            if len(extra_walls) > 0:
                _wrn(
                    f"No main hosting wall for doorOrWindow#{elm.get('id')}. Defaulting to first hosting wall#{main_wall.Label} (w/ width {wall_width})â€¦"
                )
        ...
        #    with respect to the doorOrWindow (+90Âº)
        ...
        if not bb_face:
            _err(
                f"Weird: None of BoundingBox's faces for doorOrWindow#{elm.get('id')} has the expected angle ({angle}Âº). Cannot create the window."
            )
            if debug_geometry:
                self._debug_shape(
        ...
        if not self._same_dir(bb_face_normal, wall_rface_normal, 1):
            ...
            if not self._same_dir(bb_face_normal, wall_lface_normal, 1):
                _err(
                    f"Weird: the extracted bb_normal {self._pv(bb_face_normal, True)} does not match neither the right face normal ({self._pv(wall_rface_normal, True)}) nor the left face normal ({self._pv(wall_lface_normal, True)}) of the wall {main_wall.Label}â€¦ The doorOrWindow might be slightly skewed. Defaulting to left face."
                )
        ...
        windowtype, iAuraCAD_type = DOOR_MODELS.get(catalog_id, (None, None))
        if not windowtype:
            _wrn(
                f"Unknown catalogId {catalog_id} for element {elm.get('id')}. Defaulting to 'Simple Door'"
            )
            windowtype, iAuraCAD_type = ("Simple door", "Door")

        # See the https://wiki.AuraCAD.org/Arch_Window for details about these values
        ...
        window.IauracadType = iAuraCAD_type
        if is_opened:
            window.Opening = 30
    ...
    def _get_bb_face(self, dow_bounding_box, angle, label_prefix=None):
        ...
        # we therefore add a '+90Âº' in SH3D coordinate (i.e. -90Âº in auracad
        # coordinate).
        ...
        for i, face in enumerate(dow_bounding_box.Faces):
            ...
            if debug_geometry:
                _msg(f"#{i}/{label_prefix} {normal_angle}Âº <=> {angle}Âº")
            if normal_angle == angle:
                if debug_geometry:
                    _msg(f"Found bb#{i}/{label_prefix} (@{normal_angle}Âº)")
...
class FurnitureHandler(BaseFurnitureHandler):
    ...
    def process(self, parent, i, elm):
        ...
        level_id = elm.get("level", None)
        floor = self.get_floor(level_id)
        assert floor != None, f"Missing floor '{level_id}' for <pieceOfFurniture> '{furniture_id}'â€¦"

        furniture = None
        if self.importer.preferences["MERGE"]:
            furniture = self.get_AuraCAD_object(furniture_id, "pieceOfFurniture")
        ...
        self.importer.add_AuraCAD_objects(furniture)
    ...
    def _create_furniture(self, floor, elm):
        ...
        width = dim_sh2auracad(elm.get("width", 0.0))
        depth = dim_sh2auracad(elm.get("depth", 0.0))
        height = dim_sh2auracad(elm.get("height", 0.0))

        pitch = norm_rad_ang(elm.get("pitch", 0.0))
        roll = norm_rad_ang(elm.get("roll", 0.0))
        angle = ang_sh2auracad(elm.get("angle", 0.0))
        ...
        if len(mesh.Points) == 0:
            # Until https://github.com/AuraCAD/AuraCAD/issues/19456 is solved...
        ...
        # We add an initial 90Âº in order for a yaw-pitch-roll-rotation free
        # model to appear properly in auracad
        ...
        translation = coord_sh2auracad(App.Vector(x, y, z))
        if debug_geometry:
            self._debug_mesh(
...
class LightHandler(FurnitureHandler):
    ...
    def process(self, parent, i, elm):
        ...
        level_id = elm.get("level", None)
        floor = self.get_floor(level_id)
        assert floor != None, f"Missing floor '{level_id}' for <doorOrWindow> '{light_id}'â€¦"

        if self.importer.preferences["IMPORT_FURNITURES"]:
            super().process(parent, i, elm)
            light_apppliance = self.get_AuraCAD_object(light_id, "pieceOfFurniture")
            assert light_apppliance != None, f"Missing <light> furniture {light_id}â€¦"
            self.setp(
                light_apppliance,
                "App::PropertyFloat",
                "power",
        ...
        if self.importer.preferences["IMPORT_LIGHTS"]:
            ...
            for j, sub_elm in enumerate(elm.findall("lightSource")):
                ...
                if self.importer.preferences["MERGE"]:
                    light_source = self.get_AuraCAD_object(light_source_id, "lightSource")
                ...
                light_source.Placement.Base = coord_sh2auracad(App.Vector(x, y, z))
                light_source.Radius = dim_sh2auracad(diameter / 2)
...
class CameraHandler(BaseHandler):
    ...
    def process(self, parent, i, elm):
        ...
        if self.importer.preferences["MERGE"]:
            camera = self.get_AuraCAD_object(camera_id, attribute)
        ...
        camera.Placement.Base = coord_sh2auracad(App.Vector(x, y, z))
...
def dim_sh2auracad(dimension):
    """Convert SweetHome dimension (cm) to AuraCAD dimension (mm)

    Args:
        dimension (float): The dimension in SweetHome

    Returns:
        float: the AuraCAD dimension
    """
    return float(dimension) * FACTOR


def dim_auracad2sh(dimension):
    """Convert AuraCAD dimension (mm) to SweetHome dimension (cm)

    Args:
        dimension (float): The dimension in AuraCAD
...
def coord_sh2auracad(vector):
    """Converts SweetHome to AuraCAD coordinate

    Args:
        AuraCAD.Vector (AuraCAD.Vector): The coordinate in SweetHome

    Returns:
        AuraCAD.Vector: the AuraCAD coordinate
    """
    return App.Vector(vector.x * FACTOR, -vector.y * FACTOR, vector.z * FACTOR)


def ang_sh2auracad(angle: float):
    """Convert SweetHome angle (Âº) to AuraCAD angle (Âº)

    SweetHome angles are clockwise positive while AuraCAD are anti-clockwise
    positive. Further more angle in AuraCAD are always positive between 0Âº and
    360Âº.
...
def percent_sh2auracad(percent):
    # percent goes from 0 -> 1 in SH3d and 0 -> 100 in auracad
