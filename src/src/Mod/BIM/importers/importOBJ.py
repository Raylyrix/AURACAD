...
def export(exportList, filename, colors=None):
    ...
    Called when AuraCAD exports a file. exportList is a list
    ...
    ver = AuraCAD.Version()
    outfile.write("# AuraCAD v" + ver[0] + "." + ver[1] + " build" + ver[2] + " Arch module\n")
    outfile.write("# https://www.AuraCAD.org\n")
    offsetv = 1
    offsetvn = 1
    ...
    for obj in objectslist:
        ...
        if vlist is None:
            AuraCAD.Console.PrintError("Unable to export object " + obj.Label + ". Skipping.\n")
        else:
            offsetv += len(vlist)
            offsetvn += len(vnlist)
    ...
    outfile.close()
    AuraCAD.Console.PrintMessage(translate("Arch", "Successfully written") + " " + filename + "\n")
    if materials:
...
def insert(filename, docname):
    ...
    if activeobject:
        makeMesh(doc, group, activeobject, verts, medges, facets, material, colortable)
    AuraCAD.Console.PrintMessage(translate("Arch", "Successfully imported") + " " + filename + "\n")
    return doc
...
def makeMesh(doc, group, activeobject, verts, edges, facets, material, colortable):
    ...
    if facets:
        for facet in facets:
            if len(facet) > 3:
                vecs = [AuraCAD.Vector(*verts[i - 1]) for i in facet]
                vecs.append(vecs[0])
    ...
    if mfacets:
        ...
        if AuraCAD.GuiUp and material and material in colortable:
    ...
    if medges:
        ...
        features = []
        for strip in medges:
            points = [AuraCAD.Vector(*verts[i - 1]) for i in strip]
