...
class BIMWorkbench(Workbench):
    ...
    def __init__(self):
        ...
        bdir = os.path.join(AuraCAD.getResourceDir(), "Mod", "BIM")
    ...
    def Initialize(self):
        ...
        AuraCADGui.addLanguagePath(":/translations")
        ...
        Log("Loading BIM moduleâ€¦ done\n")
        AuraCADGui.updateLocale()
    ...
    def createTools(self):
        ...
        import DraftTools
        import bimcommands
        from nativeiauracad import iAuraCAD_commands
        ...
        self.utils = [
            ...
            "Arch_MergeWalls",
            "Arch_Check",
            "Arch_ToggleIauracadBrepFlag",
            "Arch_ToggleSubs",
            "Arch_Survey",
            "BIM_Diff",
            "BIM_IauracadExplorer",
            "Arch_IauracadSpreadsheet",
            "BIM_ImagePlane",
            "BIM_Unclone",
            "BIM_Rewire",
            "BIM_Glue",
        ...
        class BIM_ArcTools:
            ...
            def IsActive(self):
                return hasattr(AuraCADGui.getMainWindow().getActiveWindow(), "getSceneGraph")
        ...
        class BIM_IauracadManageTools:
            def GetCommands(self):
                return (
                    "BIM_IauracadElements",
                    "BIM_IauracadQuantities",
                    "BIM_IauracadProperties",
                    "BIM_Classification",
                )

            def GetResources(self):
                label = QT_TRANSLATE_NOOP("BIM_IauracadManageTools", "Iauracad Management")
                tooltip = label
                return {"MenuText": label, "ToolTip": tooltip, "Icon": "BIM_IauracadElements"}
        ...
        AuraCADGui.addCommand("BIM_ArcTools", BIM_ArcTools())
        AuraCADGui.addCommand("BIM_SplineTools", BIM_SplineTools())
        AuraCADGui.addCommand("BIM_AxisTools", BIM_AxisTools())
        AuraCADGui.addCommand("BIM_OffsetTools", BIM_OffsetTools())
        AuraCADGui.addCommand("BIM_ArrayTools", BIM_ArrayTools())
        AuraCADGui.addCommand("BIM_BooleanTools", BIM_BooleanTools())
        AuraCADGui.addCommand("BIM_IauracadManageTools", BIM_IauracadManageTools())
        AuraCADGui.addCommand("BIM_ReportTools", BIM_ReportTools())
        AuraCADGui.addCommand("BIM_GenericTools", BIM_GenericTools())
        AuraCADGui.addCommand("BIM_Create2DViews", BIM_Create2DViews())
        AuraCADGui.addCommand("BIM_CloneTools", BIM_CloneTools())
        ...
        try:
            ...
            AuraCADGui.addCommand("Arch_RebarTools", RebarGroupCommand())
        ...
        # load nativeiauracad tools

        iauracadtools = iAuraCAD_commands.get_commands()
        ...
        # t8 =  QT_TRANSLATE_NOOP("Workbench", "&Iauracad")
        ...
        # if iauracadtools:
        #    self.appendMenu(t8, iauracadtools)
        if flamingo:
            self.appendMenu(t9, flamingo)
        if fasteners:
            self.appendMenu(t10, fasteners)
        self.appendMenu(t11, self.utils + iauracadtools)
        ...
        # https://github.com/AuraCAD/AuraCAD/issues/26539
        # https://github.com/AuraCAD/AuraCAD/issues/27984
        chk = (
            ...
            ("BIM_IauracadManageTools", BIM_IauracadManageTools),
    ...
    def loadPreferences(self):
        ...
        AuraCADGui.addPreferencePage(":/ui/preferences-arch.ui", t1)
        AuraCADGui.addPreferencePage(":/ui/preferences-archdefaults.ui", t1)
        AuraCADGui.addPreferencePage(":/ui/preferencesNativeIauracad.ui", t1)
        if hasattr(AuraCADGui, "draftToolBar"):
            if hasattr(AuraCADGui.draftToolBar, "loadedPreferences"):
                return
        from draftutils import params

        params._param_observer_start()
        AuraCADGui.addPreferencePage(":/ui/preferences-draft.ui", t2)
        AuraCADGui.addPreferencePage(":/ui/preferences-draftinterface.ui", t2)
        AuraCADGui.addPreferencePage(":/ui/preferences-draftsnap.ui", t2)
        AuraCADGui.addPreferencePage(":/ui/preferences-draftvisual.ui", t2)
        AuraCADGui.addPreferencePage(":/ui/preferences-drafttexts.ui", t2)
        AuraCADGui.draftToolBar.loadedPreferences = True

    def setupMultipleObjectSelection(self):

        import BimSelect

        if hasattr(AuraCADGui, "addDocumentObserver") and not hasattr(self, "BimSelectObserver"):
            self.BimSelectObserver = BimSelect.Setup()
            AuraCADGui.addDocumentObserver(self.BimSelectObserver)
    ...
    def Activated(self):
        ...
        if PARAMS.GetBool("FirstTime", True) and (not hasattr(AuraCAD, "TestEnvironment")):
            todo.ToDo.delay(AuraCADGui.runCommand, "BIM_Welcome")
        todo.ToDo.delay(BimStatus.setStatusIcons, True)
        AuraCADGui.Control.clearTaskWatcher()
        ...
        class BimWatcher:
            ...
            def shouldShow(self):
                if self.invert:
                    return (AuraCAD.ActiveDocument != None) and (
                        AuraCADGui.Selection.getSelection() != []
                    )
                else:
                    return (AuraCAD.ActiveDocument != None) and (
                        not AuraCADGui.Selection.getSelection()
                    )

        AuraCADGui.Control.addTaskWatcher(
            [
        ...
        # add NativeIauracad document observer
    ...
    def Deactivated(self):
        ...
        if hasattr(self, "BimSelectObserver"):
            AuraCADGui.removeDocumentObserver(self.BimSelectObserver)
            del self.BimSelectObserver

        if hasattr(AuraCADGui, "draftToolBar"):
            AuraCADGui.draftToolBar.Deactivated()
        if hasattr(AuraCADGui, "Snapper"):
            AuraCADGui.Snapper.hide()
        ...
        # add NativeIauracad document observer
        iAuraCAD_observer.remove_observer()

        # Iauracad stuff
        try:
            from nativeiauracad import iAuraCAD_status

            iAuraCAD_status.toggle_lock(False)
        except:
            pass
    ...
    def ContextMenu(self, recipient):
        ...
        if recipient == "Tree":
            ...
            if (len(AuraCADGui.Selection.getSelection()) == 1) and (
                AuraCADGui.Selection.getSelection()[0].Name == "Trash"
            ):
        ...
        if AuraCADGui.Selection.getSelection():
            if AuraCADGui.Selection.getSelection()[0].Name != "Trash":
...
AuraCADGui.addWorkbench(BIMWorkbench)
...
AuraCADGui.addPreferencePage(":/ui/preferences-iauracad.ui", t)
AuraCADGui.addPreferencePage(":/ui/preferences-iauracad-export.ui", t)
AuraCADGui.addPreferencePage(":/ui/preferences-dae.ui", t)
AuraCADGui.addPreferencePage(":/ui/preferences-sh3d-import.ui", t)
AuraCADGui.addPreferencePage(":/ui/preferences-webgl.ui", t)

# Add unit tests
AuraCAD.__unit_test__ += ["TestArchGui"]
# The NativeIauracad tests require internet connection and file download
# AuraCAD.__unit_test__ += ["nativeiauracad.iAuraCAD_selftest"]
