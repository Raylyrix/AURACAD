...
def makeSolarDiagram(longitude, latitude, scale=1, complete=False, tz=None):
    ...
    with temp_logger_level(logging.WARNING):
        try:
            ...
            # AuraCAD.Console.PrintWarning("Ladybug module not found, using pysolar instead. Warning, this will be deprecated in the future\n")
            ladybug = False
            try:
                import pysolar
            except ImportError:
    ...
    for ad in range(0, 360, 15):
        a = math.radians(ad)
        p1 = AuraCAD.Vector(math.cos(a) * scale, math.sin(a) * scale, 0)
        p2 = AuraCAD.Vector(math.cos(a) * scale * 0.125, math.sin(a) * scale * 0.125, 0)
        p3 = AuraCAD.Vector(math.cos(a) * scale * 1.08, math.sin(a) * scale * 1.08, 0)
    ...
    cube.translate(AuraCAD.Vector(-sz / 2, -sz / 2, -sz))
...
def makeWindRose(epwfile, scale=1, sectors=24):
    ...
    if not epwfile:
        AuraCAD.Console.PrintWarning("No EPW file, unable to generate wind rose.\n")
        return None
    ...
    for divider in dividers:
        l = Part.makeLine(AuraCAD.Vector(), divider)
...
class _Site(ArchIauracad.IauracadProduct):
    """The Site object.
    ...
    def __init__(self, obj):
        ...
        obj.IauracadType = "Site"
        obj.CompositionType = "ELEMENT"

    def setProperties(self, obj):
        """Gives the object properties unique to sites.

        Adds the Iauracad product properties, and sites' unique properties like
        Terrain.
        ...
        if not "ExtrusionVector" in pl:
            ...
            obj.ExtrusionVector = AuraCAD.Vector(0, 0, -100000)
        ...
        if not "IauracadType" in pl:
            ...
            obj.IauracadType = ArchIauracad.IauracadTypes
            obj.IcfType = "Site"
    ...
    def onDocumentRestored(self, obj):
        """AuraCAD hook run after the object is restored from a file.

        This method serves a dual purpose due to AuraCAD framework limitations:
        ...
        1.  **Standard Role:** Restore properties on the data object (`_Site` proxy) itself by
            ...
            properties defined in the current AuraCAD version to an object loaded from an older
            file.
        ...
        if AuraCAD.GuiUp and hasattr(obj, "ViewObject"):
            # Manually ensure the view provider's properties are up-to-date.
            #
            # When loading a document, AuraCAD's C++ document loading mechanism restores Python
            ...
            try:
                ...
                AuraCAD.Console.PrintError(f"ArchSite: proxy.setProperties failed: {e}\n")
            ...
            # the way AuraCAD's GUI and data layers interact.
            #
            # 1. Constraints are not saved in the .auracadStd file, so they must be programmatically
    ...
    def execute(self, obj):
        ...
        pl = AuraCAD.Placement(obj.Placement)
        shape = None
        if (
            ...
            shape.Placement = AuraCAD.Placement()
    ...
    def computeAreas(self, obj):
        ...
        for face in obj.Shape.Faces:
            if face.normalAt(0, 0).getAngle(AuraCAD.Vector(0, 0, 1)) < 1.5707:
                edges.extend(TechDraw.project(face, AuraCAD.Vector(0, 0, 1))[0].Edges)
...
if AuraCAD.GuiUp:
    ...
    class SiteTaskPanel(ArchComponent.ComponentTaskPanel):
        ...
        def __init__(self, vobj):
            ...
            # Hour row: slider (0â€“47, half-hour steps) kept in sync with spinbox
            _hourTip = translate(
                "Arch",
...
class _ViewProviderSite:
    ...
    def setProperties(self, vobj):
        ...
        You can learn more about properties here: https://wiki.AuraCAD.org/property
        """
    ...
    def restoreConstraints(self, vobj):
        ...
        try:
            ...
            # see https://github.com/AuraCAD/AuraCAD/issues/24543
            return
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        actionTransform = QtGui.QAction(
            AuraCADGui.getIcon("Std_TransformManip.svg"),
    ...
    def transform(self):
        AuraCADGui.ActiveDocument.setEdit(self.Object, 1)
    ...
    def addDisplaymodeTerrainSwitches(self, vobj):
        ...
        https://forum.AuraCAD.org/viewtopic.php?t=75658
        https://forum.AuraCAD.org/viewtopic.php?t=75883
        """
    ...
    def addTrueNorthRotation(self):
        ...
        if hasattr(self, "trueNorthRotation") and self.trueNorthRotation is not None:
            return
        if not AuraCADGui.ActiveDocument.ActiveView:
            return
        if not hasattr(AuraCADGui.ActiveDocument.ActiveView, "getSceneGraph"):
            return

        from pivy import coin

        self.trueNorthRotation = coin.SoTransform()
        sg = AuraCADGui.ActiveDocument.ActiveView.getSceneGraph()
    ...
    def _migrate_legacy_solar_diagram_scale(self, vobj):
        ...
        Historically older AuraCAD files (1.0.0 and prior) sometimes stored
        ...
        try:
            ...
            AuraCAD.Console.PrintError(f"ArchSite: Failed during legacy migration: {e}\n")
    ...
    def _on_viewobject_ready(self, vobj):
        ...
        try:
            ...
            AuraCAD.Console.PrintError(f"ArchSite: Failed to set properties during restore: {e}\n")
    ...
    def updateSunPosition(self, vobj):
        ...
        obj = vobj.Object
        declination_rot = AuraCAD.Rotation(
            AuraCAD.Vector(0, 0, 1),
            ...
            if alt > 0:
                ...
                if hasattr(vobj, "ShowHourLabels") and vobj.ShowHourLabels:
                    if vobj.ShowHourLabels and (hour_float in [9.0, 12.0, 15.0]):
                        ...
                        direction = AuraCAD.Vector(point).normalize()
        ...
        sun_pos_local = AuraCAD.Vector(x, y, z)
        ...
        if not ray_object:
            ...
            # is not disrupted.
            saved_selection = AuraCADGui.Selection.getSelection()
            ...
            for sel_obj in saved_selection:
                AuraCADGui.Selection.addSelection(sel_obj)
        ...
        if not hasattr(ray_object, "Altitude"):
            ...
            ray_object.addProperty(
                ...
                "Sun Data",
                QT_TRANSLATE_NOOP(
                    "App::Property", "The compass direction of the sun (0Â° is North)"
                ),
                locked=True,
            )
