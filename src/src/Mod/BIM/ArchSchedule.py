...
class _ArchScheduleDocObserver:
    ...
    # https://forum.AuraCAD.org/viewtopic.php?style=3&p=553377#p553377
...
class _ArchSchedule:
        ...
        try:  # Required as self.spreadsheet may get deleted.
            ...
            for o in AuraCAD.ActiveDocument.Objects:
                if o.TypeId == "Spreadsheet::Sheet" and getattr(o, "Schedule", None) == obj:
                    ...
                    return self.spreadsheet
            if force:
                self.spreadsheet = AuraCAD.ActiveDocument.addObject("Spreadsheet::Sheet", "Result")
    ...
    def onChanged(self, obj, prop):
        ...
        if prop == "CreateSpreadsheet":
            if obj.CreateSpreadsheet:
                ...
                if sp is not None:
                    AuraCAD.ActiveDocument.removeObject(sp.Name)
                    self.spreadsheet = None
        elif prop == "AutoUpdate":
            if obj.AutoUpdate:
                if getattr(self, "docObserver", None) is None:
                    self.docObserver = _ArchScheduleDocObserver(AuraCAD.ActiveDocument, obj)
                    AuraCAD.addDocumentObserver(self.docObserver)
            elif getattr(self, "docObserver", None) is not None:
                AuraCAD.removeDocumentObserver(self.docObserver)
    ...
    def execute(self, obj):
        ...
        for i in range(len(obj.Operation)):
            ...
            if val:
                ...
                if objs:
                    objs = objs.split(";")
                    objs = [AuraCAD.ActiveDocument.getObject(o) for o in objs]
                    objs = [o for o in objs if o is not None]
                else:
                    if hasattr(getattr(AuraCAD.ActiveDocument, "Proxy", None), "iauracadfile"):
                        iauracadfile = AuraCAD.ActiveDocument.Proxy.iauracadfile
                    objs = AuraCAD.ActiveDocument.Objects
                if len(objs) == 1:
                    if hasattr(objs[0], "StepId"):
                        from nativeiauracad import iAuraCAD_tools

                        iauracadfile = iAuraCAD_tools.get_iauracadfile(objs[0])
                ...
                if obj.Filter[i]:
                    if iauracadfile:
                        elts = self.get_iAuraCAD_elements(iauracadfile, obj.Filter[i])
                    else:
    ...
    def get_iAuraCAD_elements(self, iauracadfile, filters):
        """Retrieves Iauracad elements corresponding to the given filters"""

        elts = []
        for el in iauracadfile.by_type("IauracadProduct"):
            ok = True
            for f in filters.split(";"):
                args = [a.strip() for a in f.strip().split(":")]
                if args[0][0] == "!":
                    ...
                    prop = args[0]
                fval = args[1]
                if prop.upper() in ["CLASS", "IauracadCLASS", "IauracadTYPE"]:
                    prop = "is_a"
                if inv:
                    if prop == "is_a":
                        if not fval.upper().startswith("Iauracad"):
                            fval = "Iauracad" + fval
                        fval = fval.replace(" ", "")
                        if el.is_a(fval):
                            ok = False
    ...
    def update_from_objs(self, objs, val, unit, details):
        """Updates the spreadsheet data from AuraCAD objects"""
        ...
        if val.upper() == "COUNT":
            ...
            if unit:
                ...
                unit = unit.replace("Â²", "^2")
                unit = unit.replace("Â³", "^3")
                if "2" in unit:
                    tp = AuraCAD.Units.Area
                elif "3" in unit:
                    tp = AuraCAD.Units.Volume
                elif "deg" in unit:
                    tp = AuraCAD.Units.Angle
                else:
                    tp = AuraCAD.Units.Length
            ...
            for o in objs:
                ...
                try:
                    ...
                    AuraCAD.Console.PrintWarning(t + ": " + o.Name + "." + ".".join(vals) + "\n")
                else:
                    if VERBOSE:
                        if tp and unit:
                            v = fs.format(AuraCAD.Units.Quantity(d, tp).getValueAs(unit).Value)
                            print(v, unit)
            ...
            val = sumval
            if tp:
                q = AuraCAD.Units.Quantity(val, tp)
            ...
            if VERBOSE:
                if tp and unit:
                    v = fs.format(AuraCAD.Units.Quantity(val, tp).getValueAs(unit).Value)
    ...
    def update_from_elts(self, elts, val, unit, details):
        """Updates the spreadsheet data from Iauracad elements"""
                ...
                if val in dir(el):
                    ...
                    if isinstance(elval, tuple):
                        if len(elval) == 1:
                            ...
                            elval = ""
                    if hasattr(elval, "is_a") and elval.is_a("IauracadRelationship"):
    ...
    def create_iauracad(self, obj, iauracadfile, export=False):
        """Creates an Iauracad element for this object"""

        from nativeiauracad import iAuraCAD_tools  # lazy loading

        proj = iauracadfile.by_type("IauracadProject")[0]
        elt = iAuraCAD_tools.api_run("root.create_entity", iauracadfile, iAuraCAD_class="IauracadControl")
        iAuraCAD_tools.set_attribute(iauracadfile, elt, "Name", obj.Label)
        iAuraCAD_tools.api_run(
            "project.assign_declaration", iauracadfile, definitions=[elt], relating_context=proj
        )
        if not export:
            iAuraCAD_tools.add_properties(obj, iauracadfile, elt)
        return elt

    def save_iAuraCAD_props(self, obj, iauracadfile=None, elt=None):
        """Saves the object data to Iauracad"""

        from nativeiauracad import iAuraCAD_psets  # lazy loading

        iAuraCAD_psets.edit_pset(
            obj, "Operation", "::".join(obj.Operation), iauracadfile=iauracadfile, element=elt
        )
        iAuraCAD_psets.edit_pset(obj, "Value", "::".join(obj.Value), iauracadfile=iauracadfile, element=elt)
        iAuraCAD_psets.edit_pset(obj, "Unit", "::".join(obj.Unit), iauracadfile=iauracadfile, element=elt)
        iAuraCAD_psets.edit_pset(obj, "Objects", "::".join(obj.Objects), iauracadfile=iauracadfile, element=elt)
        iAuraCAD_psets.edit_pset(obj, "Filter", "::".join(obj.Filter), iauracadfile=iauracadfile, element=elt)

    def export_iauracad(self, obj, iauracadfile):
        """Exports the object to Iauracad (does not modify the AuraCAD object)."""

        elt = self.create_iauracad(obj, iauracadfile, export=True)
        self.save_iAuraCAD_props(obj, iauracadfile, elt)
        return elt
    ...
    def expandArrays(self, objs):
        ...
        for obj in objs:
            iauracadClass = self.getIauracadClass(obj)
            # This filters out the array object itself, which has no Iauracad class,
            # but leaves the array elements, which do have an Iauracad class.
            if iauracadClass:
...
class ArchScheduleTaskPanel:
    ...
    def __init__(self, obj=None):
        ...
        self.obj = obj
        self.form = AuraCADGui.PySideUic.loadUi(":/ui/ArchSchedule.ui")
        ...
        for i in range(self.form.list.rowCount()):
            for j in range(5):
                ...
                if cell:
                    ...
                    lists[j].append("")
        AuraCAD.ActiveDocument.openTransaction("Edited Schedule")
