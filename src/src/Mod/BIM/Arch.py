...
"""Core API for architectural and Building Information Modeling (BIM) in AuraCAD.
...
- BIM data support (materials, Iauracad properties, classification systems)
- Integration with AuraCAD's core (Part, Draft) and other workbenches
- Object creation utilities for architectural workflows

## Usage
Designed for:
1. Internal API for AuraCAD's built-in BIM commands
...
def makeAxis(num=1, size=1000, name=None):
    ...
    if not AuraCAD.ActiveDocument:
        ...
        return
    obj = AuraCAD.ActiveDocument.addObject("Part::FeaturePython", "Axis")
...
def makeAxisSystem(axes, name=None):
    ...
    if not isinstance(axes, list):
        axes = [axes]
    obj = AuraCAD.ActiveDocument.addObject("App::FeaturePython", "AxisSystem")
...
def makeBuildingPart(objectslist=None, baseobj=None, name=None):
    ...
    obj = AuraCAD.ActiveDocument.addObject("App::GeometryPython", "BuildingPart")
    # obj = AuraCAD.ActiveDocument.addObject("App::FeaturePython","BuildingPart")
...
def makeFloor(objectslist=None, baseobj=None, name=None):
    ...
    obj.IauracadType = "Building Storey"
...
def make2DDrawing(objectslist=None, baseobj=None, name=None):
    ...
    if AuraCAD.GuiUp:
        obj.ViewObject.DisplayOffset = AuraCAD.Placement()
        obj.ViewObject.ShowLevel = False
    return obj
    ...
    for obj in objset:
        if Draft.getType(obj) in ["Floor", "Building"]:
            ...
            if obj.ViewObject:
                ...
                # in case AuraCAD doesn't allow 2 objs with same label
            obj.Label = obj.Label + " to delete"
            nobj.Label = label
    for n in todel:
        from draftutils import todo

        todo.ToDo.delay(AuraCAD.ActiveDocument.removeObject, n)
...
def makeEquipment(baseobj=None, placement=None, name=None):
    ...
    placement : AuraCAD.Placement, optional
...
def makeProfile(profile=[0, "REC", "REC100x100", "R", 100, 100]):
    ...
    profile : list, optional
        ...
        4. dimensionsList: int
            ...
            See https://wiki.AuraCAD.org/Arch_Profile for more details on profile presets.
    ...
    if not AuraCAD.ActiveDocument:
        AuraCAD.Console.PrintError("No active document. Aborting\n")
        return
    obj = AuraCAD.ActiveDocument.addObject("Part::Part2DObjectPython", "Profile")
...
def makeProject(sites=None, name=None):
    ...
    The NativeIauracad project is the new way to create Iauracad projects.
    """
...
def makeRebar(
    baseobj: Optional[AuraCAD.DocumentObject] = None,
    sketch: Optional[AuraCAD.DocumentObject] = None,
    diameter: Optional[float] = None,
    amount: int = 1,
    offset: Optional[float] = None,
    name: Optional[str] = None,
) -> Optional[AuraCAD.DocumentObject]:
    """
    ...
    Parameters
    ----------
    baseobj : AuraCAD.DocumentObject, optional
    ...
    sketch : AuraCAD.DocumentObject, optional
    ...
    Returns
    -------
    AuraCAD.DocumentObject or None
        The created rebar object, or None if creation fails.

    Examples
    --------
    >>> import AuraCAD, Arch, Part, Sketcher
    ...
    >>> rebar_sketch.addGeometry(Part.LineSegment(AuraCAD.Vector(25, 25, 0),
    ...                                         AuraCAD.Vector(1975, 25, 0)), False)
...
def makeStairs(baseobj=None, length=None, width=None, height=None, steps=None, name=None):
    ...
    if baseobj:
        ...
        if lenSelection > 1:
            stair = AuraCAD.ActiveDocument.addObject("Part::FeaturePython", "Stairs")
            stair.Label = label
        ...
        obj = AuraCAD.ActiveDocument.addObject("Part::FeaturePython", "Stairs")
        obj.Label = label
...
def makeWall(
    ...
    if wall.Base and AuraCAD.GuiUp:
...
def joinWalls(walls, delete=False, deletebase=False):
        ...
        if Draft.getType(base.Base) == "ArchSketch":
            ...
            try:
                ...
                if Draft.getType(base.Base) != "Sketcher::SketchObject":
                    newSk = AuraCAD.ActiveDocument.addObject("Sketcher::SketchObject", "WallTrace")
                else:
                    newSk = None
            if newSk:
    ...
    if delete:
        for n in deleteList:
            AuraCAD.ActiveDocument.removeObject(n)
    AuraCAD.ActiveDocument.recompute()
    if base.Base and AuraCAD.GuiUp:
...
def makeWindow(
    ...
    inserted into host objects like Walls, creating openings. The IauracadType of
    ...
    - **Dual purpose (window/door)**: despite its name, this function is the primary
      ...
      The distinction is often made by setting the `IauracadType` property of the
    ...
    >>> # Ex5: Door created from a sketch, with IauracadType manually set (sketch on XY plane)
    ...
    >>> door_ex5_manual.IauracadType = "Door" # Explicitly define as a Door
    >>> doc.recompute()
    ...
    if baseobj and Draft.getType(baseobj) == "Window" and AuraCAD.ActiveDocument:
    ...
    if baseobj:
        ...
        # here.
        # obj.Normal = baseobj.Placement.Rotation.multVec(AuraCAD.Vector(0, 0, -1))
        window.Base = baseobj
    ...
    if window.Base and AuraCAD.GuiUp:
...
def debaseWall(wall):
    ...
    if not is_debasable(wall):
        AuraCAD.Console.PrintWarning(f"Wall '{wall.Label}' is not eligible for debasing.\n")
        return False
    ...
    try:
        ...
        normal = wall.Normal
        if normal.Length == 0:
            normal = base_obj.Placement.Rotation.multVec(AuraCAD.Vector(0, 0, 1))
            if normal.Length == 0:
                normal = AuraCAD.Vector(0, 0, 1)
        ...
        final_rotation = AuraCAD.Rotation(x_axis, y_axis, z_axis)
        ...
        final_placement = AuraCAD.Placement(final_position, final_rotation)
        ...
        AuraCAD.Console.PrintError(f"Error debasing wall '{wall.Label}': {e}\n")
        return False
...
def _initializeArchObject(
    ...
    obj = AuraCAD.ActiveDocument.addObject(objectType, internalName)
    if not obj:
        return None
    ...
    try:
        ...
        AuraCAD.Console.PrintError(f"Failed to import module '{moduleName}': {e}\n")
        return None
...
def makeReport(name=None):
    ...
    if AuraCAD.GuiUp:
        ...
        AuraCADGui.ActiveDocument.setEdit(report_obj.Name, 0)
...
def makeStructure(baseobj=None, length=None, width=None, height=None, name=None):
    ...
    Notes
    -----
    - If ``length > height``, the element is classified as Iauracad type "Beam"; if ``height > length``,
    ...
    # default here would interfere with the Length vs Height comparison that determines IauracadType.
    if height:
...
def placeAlongEdge(p1, p2, horizontal=False):
    ...
    if cross_section_horizontal.Length > 0:
        ...
        if horizontal:
            placement.Rotation = AuraCAD.Rotation(
            ...
            placement.Rotation = AuraCAD.Rotation(
                placement.Rotation.multVec(AuraCAD.Vector(0, 0, 1)), 90
