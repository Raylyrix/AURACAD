...
class TestArchSiteGui(TestArchBaseGui.TestArchBaseGui):
    ...
    def test_new_site_save_and_reopen(self):
        ...
        tf = tempfile.NamedTemporaryFile(delete=False, suffix=".auracadStd")
        tf.close()
        path = tf.name
        try:
            AuraCAD.ActiveDocument.saveAs(path)

            # Open the saved document (this returns a new Document instance)
            reopened = AuraCAD.openDocument(path)
            try:
                ...
                AuraCAD.closeDocument(reopened.Name)
        finally:
            try:
                os.unlink(path)
            except Exception:
    ...
    def test_legacy_site_migration(self):
        ...
        Verifies that loading a legacy (<=1.0.0) AuraCAD file triggers the view provider's restore
        ...
        modern default of 20 m radius (https://github.com/AuraCAD/AuraCAD/pull/22496). The migration
        ...
        fname = os.path.join(fixtures_dir, "AuraCAD_site_simple-102.auracadStd")
