...
class TestArchComponent(TestArchBase.TestArchBase):
    ...
    def testBsplineSlabAreas(self):
        ...
        See https://github.com/AuraCAD/AuraCAD/issues/20989.
        """
        ...
        self.assertAlmostEqual(
            ...
            msg=(
                ...
                f"Exp: {theoreticalHorizontalArea:.3f} mÂ² | "
                f"Got: {actualHorizontalArea:.3f} mÂ²"
            ),
        )
        ...
        self.assertAlmostEqual(
            ...
            msg=(
                ...
                f"Exp: {theoreticalVerticalArea:.3f} mÂ² | "
                f"Got: {actualVerticalArea:.3f} mÂ²"
            ),
        )
    ...
    def testHouseSpaceAreas(self):
        ...
        See https://github.com/AuraCAD/AuraCAD/issues/14687.
        """
        ...
        theoreticalHorizontalArea = (baseLength * baseWidth) / 1e6  # 25 mÂ²

        # Vertical areas
        # Side faces (YZ plane) - two rectangles
        sideFaceArea = (rectangleHeight * baseWidth) / 1e6  # 12.5 mÂ² each
        totalSides = sideFaceArea * 2  # 25 mÂ²

        # Front/back faces (XZ plane)
        rectangularPart = (baseLength * rectangleHeight) / 1e6  # 12.5 mÂ²
        triangularPart = (baseLength * triangleHeight / 2) / 1e6  # 6.25 mÂ²
        totalFrontBack = (rectangularPart + triangularPart) * 2  # 37.5 mÂ²

        theoreticalVerticalArea = totalSides + totalFrontBack  # 62.5 mÂ²
    ...
    def test_remove_single_window_from_wall_host_is_none(self):
        ...
        See https://github.com/AuraCAD/AuraCAD/issues/21551
        """
    ...
    def test_horizontal_area_tilted_cylinders(self):
        ...
        for rot_vec in (App.Vector(1, 0, 0), App.Vector(0, 1, 0)):
            ...
            self.assertAlmostEqual(
                ...
                msg=(
                    "Horizontal area > 0.1% tolerance | "
                    f"Exp: {area_expected:.3f} mÂ² | "
                    f"Got: {area_actual:.3f} mÂ²"
                ),
            )
    ...
    def test_rotated_component_area(self):
        ...
        # AuraCAD sums all vertical faces (front + back + vertical side edges).
