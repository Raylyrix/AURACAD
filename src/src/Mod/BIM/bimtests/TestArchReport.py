...
class TestArchReport(TestArchBase.TestArchBase):
    ...
    def test_select_specific_columns_from_document(self):
        """Test a 'SELECT Label, IauracadType, Height FROM document' query."""
        query_string = 'SELECT Label, IauracadType, Height FROM document WHERE IauracadType = "Wall"'
        headers, results_data = self._run_query_for_objects(query_string)

        self.assertEqual(headers, ["Label", "IauracadType", "Height"])
    ...
    def test_where_equals_string(self):
        _, results_labels = self._run_query_for_objects(
            'SELECT * FROM document WHERE IauracadType = "Wall"'
        )
    ...
    def test_where_is_null(self):
        ...
        _, results_labels = self._run_query_for_objects(
            "SELECT * FROM document WHERE IauracadType IS NULL"
        )
        # This expects only self.part_box as it's the only one in self.test_objects_in_doc with IauracadType=None.
        self.assertEqual(len(results_labels), 1)
        self.assertEqual(results_labels[0], self.part_box.Label)

    def test_where_is_not_null(self):
        _, results_labels = self._run_query_for_objects(
            "SELECT * FROM document WHERE IauracadType IS NOT NULL"
        )
    ...
    def test_null_equality_is_excluded(self):
        ...
        _, results = self._run_query_for_objects("SELECT * FROM document WHERE IauracadType = NULL")
        # '=' with NULL should not match (UNKNOWN -> excluded)
        self.assertEqual(len(results), 0)

    def test_null_inequality_excludes_nulls(self):
        """Strict SQL: IauracadType != 'Wall' should exclude rows where IauracadType is NULL."""
        _, results_labels = self._run_query_for_objects(
            'SELECT * FROM document WHERE IauracadType != "Wall"'
        )
        expected_labels = [self.column.Label, self.beam.Label, self.window.Label]
    ...
    def test_where_boolean_and(self):
        query = 'SELECT * FROM document WHERE IauracadType = "Wall" AND Label LIKE "%Exterior%"'
        _, results_labels = self._run_query_for_objects(query)
        self.assertEqual(len(results_labels), 1)
        self.assertEqual(results_labels[0], self.wall_ext.Label)

    def test_where_boolean_or(self):
        query = 'SELECT * FROM document WHERE IauracadType = "Window" OR IauracadType = "Column"'
        _, results_labels = self._run_query_for_objects(query)
        self.assertEqual(len(results_labels), 2)
    ...
    @patch("AuraCAD.Console.PrintError")
    ...
    def test_group_by_iauracadtype_with_count(self):
        ...
        query = (
            "SELECT IauracadType, COUNT(*) FROM document "
            "WHERE TypeId != 'App::FeaturePython' AND TypeId != 'Spreadsheet::Sheet' "
            "GROUP BY IauracadType"
        )
        headers, results_data = self._run_query_for_objects(query)

        self.assertEqual(headers, ["IauracadType", "COUNT(*)"])

        # Convert results to a dict for easy, order-independent comparison.
        # Handle the case where IauracadType is None, which becomes a key in the dict.
        ...
        expected_counts = {
            ...
            "Beam": 1,
            "Window": 1,
            None: 1,  # The Part::Box has a NULL IauracadType, which forms its own group
        }
        self.assertDictEqual(
            results_dict, expected_counts, "The object counts per IauracadType are incorrect."
        )
    ...
    def test_group_by_with_sum(self):
        ...
        query = (
            "SELECT IauracadType, SUM(Height) FROM document "
            "WHERE IauracadType = 'Wall' OR IauracadType = 'Column' "
            "GROUP BY IauracadType"
        )
        headers, results_data = self._run_query_for_objects(query)

        self.assertEqual(headers, ["IauracadType", "SUM(Height)"])
    ...
    def test_min_and_max_functions(self):
        ...
        query = "SELECT MIN(Length), MAX(Length) FROM document WHERE IauracadType = 'Wall'"
        headers, results_data = self._run_query_for_objects(query)
    ...
    def test_invalid_group_by_raises_error(self):
        ...
        query = "SELECT Label, COUNT(*) FROM document GROUP BY IauracadType"
    ...
    def test_non_grouped_sum_calculates_correctly(self):
        ...
        query = "SELECT SUM(Height) FROM document WHERE IauracadType = 'Wall'"
    ...
    def test_non_grouped_query_with_mixed_extractors(self):
        ...
        """
        query = "SELECT 'Total Height', SUM(Height) FROM document WHERE IauracadType = 'Wall'"
    ...
    def test_sum_of_space_area_is_correct_and_returns_float(self):
        ...
        query = "SELECT SUM(Area) FROM document WHERE IauracadType = 'Space'"
    ...
    def test_count_property_vs_count_star(self):
        ...
        # dependent on AuraCAD's internal naming schemes.
        labels_to_count = [
            self.wall_ext.Label,
            self.wall_int.Label,
            self.column.Label,
    ...
    def test_bundled_query_presets_are_valid(self):
        ...
        self.assertIn("Count by IauracadType", loaded_preset_names)
    ...
    def test_children_function(self):
        ...
        floor.IauracadType = "Building Storey"
    ...
    def test_order_by_label_desc(self):
        ...
        query = "SELECT Label FROM document WHERE IauracadType = 'Wall' ORDER BY Label DESC"
        _, results_data = Arch.select(query)
    ...
    def test_column_aliasing(self):
        ...
        query = "SELECT Label AS 'Wall Name' FROM document WHERE IauracadType = 'Wall' ORDER BY 'Wall Name' ASC"
    ...
    def test_string_functions(self):
        ...
        target_obj_iauracadtype = self.column.IauracadType  # "Column"
        ...
        with self.subTest(description="CONCAT function with properties and literals"):
            query = f"SELECT CONCAT(Label, ': ', IauracadType) FROM document WHERE Name = '{target_obj_name}'"
            _, data = Arch.select(query)
            self.assertEqual(len(data), 1)
            expected_string = f"{target_obj_label}: {target_obj_iauracadtype}"
    ...
    def test_null_as_operand(self):
        ...
        query = "SELECT * FROM document WHERE IauracadType = NULL"
        _, results_data = Arch.select(query)
        self.assertEqual(
    ...
    def test_count_with_group_by_is_correct_and_fast(self):
        ...
        """
        # This query will have 5 input objects with an IauracadType
        # but only 4 output rows/groups (Wall, Column, Beam, Window).
        query = "SELECT IauracadType, COUNT(*) FROM document WHERE IauracadType IS NOT NULL GROUP BY IauracadType"
    ...
    def test_sql_comment_support(self):
        ...
        with self.subTest(description="Single-line comments with --"):
            ...
            query = """
                ...
                FROM document
                WHERE IauracadType = 'Wall' -- Only select walls
                -- ORDER BY Label DESC
            """
            _, data = Arch.select(query)
    ...
    def test_query_with_non_ascii_property_name(self):
        ...
        prop_name_unicode = "FlÃ¤che"  # "Area" in German
    ...
    def test_order_by_multiple_columns(self):
        ...
        # IauracadType alphabetically, and then by their Label alphabetically within
        # each IauracadType group. This requires a multi-column sort to verify.
        query = """
            SELECT Label, IauracadType
            FROM document
            WHERE IauracadType IN ('Wall', 'Column', 'Beam')
            ORDER BY IauracadType, Label ASC
        """
        _, data = Arch.select(query)
        ...
        # The engine should sort by IauracadType first ('Beam' < 'Column' < 'Wall'),
        # and then by Label for the two 'Wall' objects.
        expected_order = [
            [self.beam.Label, self.beam.IauracadType],  # Type: Beam
            [self.column.Label, self.column.IauracadType],  # Type: Column
            [self.wall_ext.Label, self.wall_ext.IauracadType],  # Type: Wall, Label: Exterior...
            [self.wall_int.Label, self.wall_int.IauracadType],  # Type: Wall, Label: Interior...
        ]
    ...
    def test_ppa_and_query_permutations(self):
        ...
        with self.subTest(description="PPA in ORDER BY clause"):
            ...
            query = f"SELECT Label, PARENT(*).Label AS ParentLabel FROM document WHERE IauracadType = 'Space' ORDER BY ParentLabel DESC"
            _, data = Arch.select(query)
        ...
        with self.subTest(description="PPA with sub-property access"):
            ...
            query = f"SELECT Label FROM document WHERE PARENT(*).Placement.Base.z = 0.0 AND IauracadType = 'Space'"
            _, data = Arch.select(query)
            found_labels = sorted([row[0] for row in data])
        ...
        with self.subTest(description="Permutation: GROUP BY on a PPA result"):
            query = "SELECT PARENT(*).Label AS FloorName, COUNT(*) FROM document WHERE IauracadType = 'Space' GROUP BY PARENT(*).Label ORDER BY FloorName"
            _, data = Arch.select(query)
        ...
        with self.subTest(description="Permutation: GROUP BY on a Function result"):
            query = "SELECT TYPE(*) AS BimType, COUNT(*) FROM document WHERE IauracadType IS NOT NULL GROUP BY TYPE(*) ORDER BY BimType"
            _, data = Arch.select(query)
        ...
        with self.subTest(description="Permutation: Filtering by a custom property on a parent"):
            query = "SELECT Label FROM document WHERE PARENT(*).FireRating = '60 minutes' AND IauracadType IN ('Door', 'Window')"
            _, data = Arch.select(query)
            found_labels = sorted([row[0] for row in data])
            expected_labels = sorted([front_door.Label, living_window.Label])
    ...
    def test_group_by_with_function_and_count(self):
        ...
        query = "SELECT TYPE(*) AS BimType, COUNT(*) FROM document WHERE Label LIKE 'Unit Test %' AND IauracadType IS NOT NULL GROUP BY TYPE(*)"
        _, data = Arch.select(query)
    ...
    def test_group_by_chained_parent_function(self):
        ...
        query = """
            ...
            FROM document
            WHERE IauracadType IN ('Door', 'Window')
    ...
    def test_group_by_multiple_mixed_columns(self):
        ...
        # ACT
        query = "SELECT IauracadType, TYPE(*), COUNT(*) FROM document GROUP BY IauracadType, TYPE(*)"
        _, data = Arch.select(query)

        # ASSERT: Find the specific row for IauracadType='Column' and TYPE='Column'
    ...
    def test_invalid_group_by_with_aggregate_raises_error(self):
        ...
        """
        query = "SELECT IauracadType, COUNT(*) FROM document GROUP BY COUNT(*)"
    ...
    def test_core_engine_enhancements_for_pipeline(self):
        ...
        with self.subTest(description="Regression test for Arch.select"):
            _, results_data = Arch.select('SELECT Label FROM document WHERE IauracadType = "Wall"')
        ...
        with self.subTest(description="Regression test for Arch.count"):
            count, error = Arch.count('SELECT * FROM document WHERE IauracadType = "Wall"')
        ...
        with self.subTest(description="Test _run_query with a source_objects list"):
            ...
            # c) The new third return value, `resulting_objects`, should contain the correct AuraCAD objects.
            self.assertEqual(
                len(resulting_objects), 3, "The returned object list has the wrong size."
            )
            self.assertIsInstance(
                resulting_objects[0],
                AuraCAD.DocumentObject,
        ...
        with self.subTest(description="Test _run_query with filtering on a source_objects list"):
            ...
            query = "SELECT Label FROM document WHERE IauracadType = 'Wall'"
    ...
    def test_execute_pipeline_orchestrator(self):
        ...
        stmt1 = ArchSql.ReportStatement(
            query_string="SELECT * FROM document WHERE IauracadType = 'Wall'", is_pipelined=False
        )
        ...
        stmt3 = ArchSql.ReportStatement(
            query_string="SELECT * FROM document WHERE IauracadType = 'Column'", is_pipelined=False
        )
        ...
        stmt4_failing = ArchSql.ReportStatement(
            query_string="SELECT * FROM document WHERE IauracadType = 'NonExistentType'",
            is_pipelined=False,
        )
    ...
    def test_public_api_for_pipelines(self):
        ...
        with self.subTest(description="Test Arch.count with a source_objects list"):
            ...
            query = "SELECT * FROM document WHERE IauracadType = 'Column'"
        ...
        with self.subTest(description="Test Arch.selectObjectsFromPipeline"):
            ...
            self.assertIsInstance(resulting_objects[0], AuraCAD.DocumentObject)
            self.assertEqual(
    ...
    def test_pipeline_with_children_function(self):
        ...
        stmt2 = ArchReport.ReportStatement(
            query_string="SELECT * FROM CHILDREN(SELECT * FROM document) WHERE IauracadType = 'Wall'",
            is_pipelined=True,
        )
    ...
    def test_default_header_uses_internal_units(self):
        ...
        original_schema_index = AuraCAD.Units.getSchema()

        try:
            # Get the list of available schema names.
            schema_names = AuraCAD.Units.listSchemas()
            ...
            AuraCAD.Units.setSchema(meter_schema_index)
            ...
            AuraCAD.Units.setSchema(original_schema_index)
    ...
    def test_numeric_comparisons_on_quantities(self):
        ...
        try:
            ...
            AuraCAD.Units.setSchema(mks_schema_index)
