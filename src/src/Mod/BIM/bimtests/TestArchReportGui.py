"""GUI tests for ArchReport features that require a running AuraCAD GUI.

These tests inherit from the GUI test base `TestArchBaseGui` which skips
the whole class when `AuraCAD.GuiUp` is False.
"""
...
class TestArchReportGui(TestArchBaseGui):
    ...
    def setUp(self):
        ...
        self.wall_ext.Height = AuraCAD.Units.Quantity(3000, "mm")

        self.wall_int = Arch.makeWall(length=500, name="Interior partition wall")
        self.wall_int.IauracadType = "Wall"
        self.wall_int.Height = AuraCAD.Units.Quantity(2500, "mm")
    ...
    def DISABLED_test_preview_pane_toggle_and_refresh(self):
        ...
        query = "SELECT Label, IauracadType FROM document WHERE IauracadType = 'Wall' ORDER BY Label"
        ...
        self.assertEqual(self.panel.table_preview_results.item(1, 1).text(), self.wall_int.IauracadType)
    ...
    def DISABLED_test_preview_pane_displays_errors_gracefully(self):
        ...
        self.assertIn(
            "âŒ", error_item.text(), "The error message should contain a visual error indicator."
        )
