        ...
        if obj.Base.Shape.Solids:
            ...
            if hasattr(obj, "Edges"):
                if obj.Edges == "Vertical edges":
                    rv = obj.Base.Placement.Rotation.multVec(AuraCAD.Vector(0, 1, 0))
                    edges = [
                        e
                        for e in edges
                    ...
                    rv = obj.Base.Placement.Rotation.multVec(AuraCAD.Vector(1, 0, 0))
                    edges = [
                        e
                        for e in edges
                        if round(rv.getAngle(e.tangentAt(e.FirstParameter)), 4) in [0, 3.1416]
            ...
            for e in edges:
                ...
                # Supplying AuraCAD.Rotation() with two parallel vectors and
                # a null vector may seem strange, but the function is perfectly
                # able to handle this. Its algorithm will use default axes in
                # such cases.
                if obj.Align:
                    if normal is None:
                        rot = AuraCAD.Rotation(AuraCAD.Vector(), bvec, bvec, "ZYX")
                    else:
                        rot = AuraCAD.Rotation(AuraCAD.Vector(), normal, bvec, "ZYX")
