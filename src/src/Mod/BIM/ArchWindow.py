...
def recolorize(attr):  # names is [docname,objname]
    ...
    if isinstance(attr, list):
        if attr[0] in AuraCAD.listDocuments():
            doc = AuraCAD.getDocument(attr[0])
...
class _Window(ArchComponent.Component):
    ...
    def onDocumentRestored(self, obj):
        ...
        # Related PRs:
        #     https://github.com/AuraCAD/AuraCAD/pull/21005
        #     https://github.com/AuraCAD/AuraCAD/pull/23014
                        ...
                        if obj.LouvreWidth and obj.LouvreSpacing:
                            ...
                            if step < bb.ZLength:
                                ...
                                for i in range(int(bb.ZLength / step) + 1):
                                    b = box.copy()
                                    b.translate(
                                        AuraCAD.Vector(bb.XMin, bb.YMin, bb.ZMin + i * step)
                                    )
                                    boxes.append(b)
    ...
    def executeSketchArchFeatures(self, obj, linkObj=None, index=None, linkElement=None):
        """
        To execute features in the SketchArch External Add-on  (https://github.com/paullee0/AuraCAD_SketchArch)
        -  import ArchSketchObject module, and
        -  execute features that are common to ArchObjects (including Links) and ArchSketch

        To install SketchArch External Add-on, see https://github.com/paullee0/AuraCAD_SketchArch#iv-install
        """
        ...
        try:
            ...
            # see https://forum.AuraCAD.org/viewtopic.php?f=23&t=50802
    ...
    def getSubVolume(self, obj, plac=None, host=None):
            ...
            if obj.Subvolume:
                if hasattr(obj.Subvolume, "Shape"):
                    if not obj.Subvolume.Shape.isNull():
                        ...
                        pl = AuraCAD.Placement(sh.Placement)
        ...
        if (
            not width
        ):  # TODO Should not happen, it means there is no Base (Sketch or another auracad object) in Clone either
            width = (
...
class _ArchWindowTaskPanel:
    ...
    def selectHole(self):
        ...
        s = AuraCADGui.Selection.getSelectionEx()
        if s and self.obj:
    ...
    def update(self):
        ...
        if self.obj:
            ...
            AuraCADGui.ExpressionBinding(self.widthWidget).bind(self.obj, "Width")
            self.widthWidget.setProperty("value", self.obj.Width)

            AuraCADGui.ExpressionBinding(self.heightWidget).bind(self.obj, "Height")
            self.heightWidget.setProperty("value", self.obj.Height)

            AuraCADGui.ExpressionBinding(self.openingWidget).bind(self.obj, "Opening")
                ...
                if comp in self.obj.WindowParts:
                    ...
                    for i in range(5):
                        ...
                        if i == 1:
                            ...
                            f.setProperty(
                                "text",
                                AuraCAD.Units.Quantity(float(t), AuraCAD.Units.Length).UserString,
                            )
                        else:
                            f.setText(t)
    ...
    def create(self):
        ...
        if ok:
            ...
            AuraCAD.Console.PrintWarning(translate("Arch", "Unable to create component") + "\n")
