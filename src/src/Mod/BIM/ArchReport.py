...
if AuraCAD.GuiUp:
    ICON_STATUS_OK = AuraCADGui.getIcon(":/icons/edit_OK.svg")
    ICON_STATUS_WARN = AuraCADGui.getIcon(":/icons/Warning.svg")
    ICON_STATUS_ERROR = AuraCADGui.getIcon(":/icons/delete.svg")
    ICON_STATUS_INCOMPLETE = AuraCADGui.getIcon(":/icons/button_invalid.svg")
    ICON_EDIT = AuraCADGui.getIcon(":/icons/edit-edit.svg")
    ICON_ADD = AuraCADGui.getIcon(":/icons/list-add.svg")
    ICON_REMOVE = AuraCADGui.getIcon(":/icons/list-remove.svg")
    ICON_DUPLICATE = AuraCADGui.getIcon(":/icons/edit-copy.svg")
...
def _get_preset_paths(preset_type):
    ...
    # Path to the bundled presets installed with AuraCAD
    system_path = os.path.join(
        AuraCAD.getResourceDir(), "Mod", "BIM", "Presets", "ArchReport", subdir
    )
    # Path to the user's custom presets in their AppData directory
    user_path = os.path.join(AuraCAD.getUserAppDataDir(), "BIM", "Presets", "ArchReport", subdir)
...
def _save_preset(preset_type, name, data):
    ...
    try:
        ...
        AuraCAD.Console.PrintError(f"BIM Report: Could not save preset to '{file_path}': {e}\n")
...
def _rename_preset(preset_type, filename, new_name):
    ...
    try:
        ...
        AuraCAD.Console.PrintError(f"BIM Report: Failed to rename preset file '{file_path}': {e}\n")
...
def _delete_preset(preset_type, filename):
    ...
    try:
        ...
        AuraCAD.Console.PrintError(f"BIM Report: Failed to delete preset file '{file_path}': {e}\n")
    ...
    def slotRecomputedDocument(self, doc):
        ...
        try:
            if not self.report.isAttachedToDocument():
                AuraCAD.removeDocumentObserver(self)
                return
        except RuntimeError:
            AuraCAD.removeDocumentObserver(self)
            return
...
class _ArchReport:
    ...
    def getSpreadSheet(self, obj, force=False):
        ...
        if force:
            sheet = AuraCAD.ActiveDocument.addObject("Spreadsheet::Sheet", "ReportResult")
        ...
        if prop == "AutoUpdate":
            if obj.AutoUpdate:
                if getattr(self, "docObserver", None) is None:
                    self.docObserver = _ArchReportDocObserver(AuraCAD.ActiveDocument, obj)
    ...
    def _write_cell(self, spreadsheet, cell_address, value):
        ...
        # Handle AuraCAD Quantity objects by extracting their raw numerical value.
    ...
    def setSpreadsheetData(
        ...
        if data_rows:
            for i, cell_value in enumerate(data_rows[0]):
                if isinstance(cell_value, AuraCAD.Units.Quantity):
...
class ViewProviderReport:
        ...
        if mode == 0:
            if AuraCAD.GuiUp:
                ...
                try:
                    ...
                    AuraCAD.Console.PrintError(f"Could not open Report editor: {e}\n")
                    return False
            return True
        return False
    ...
    def onDelete(self, vobj, subelements):
        ...
        if proxy and getattr(proxy, "docObserver", None) is not None:
            AuraCAD.removeDocumentObserver(proxy.docObserver)
...
class ReportTaskPanel:
    ...
    Exposes `self.form` as a QWidget so it works with AuraCADGui.Control.showDialog(panel).
    ...
    def __init__(self, report_obj):
        # Create two top-level widgets so AuraCAD will wrap each into a TaskBox.
        ...
        self.btn_save_template.setIcon(AuraCADGui.getIcon(":/icons/document-save.svg"))
        ...
        self.btn_toggle_preview.setIcon(AuraCADGui.getIcon(":/icons/Std_ToggleVisibility.svg"))
        ...
        self.btn_show_cheatsheet.setIcon(AuraCADGui.getIcon(":/icons/help-browser.svg"))
        ...
        self.btn_refresh_preview.setIcon(AuraCADGui.getIcon(":/icons/view-refresh.svg"))
        ...
        self.chk_include_column_names.setToolTip(
            translate(
                "Arch",
                "Include the column headers (Label, IauracadType, ...) in the spreadsheet output.",
            )
        )
        ...
        self.btn_save.setIcon(AuraCADGui.getIcon(":/icons/document-save.svg"))
        ...
        self.btn_discard.setIcon(AuraCADGui.getIcon(":/icons/delete.svg"))
        ...
        # Expose form as a list of the two top-level widgets so AuraCAD creates
        ...
        self.icon_show_preview = AuraCADGui.getIcon(":/icons/Std_ToggleVisibility.svg")
        self.icon_hide_preview = AuraCADGui.getIcon(":/icons/Invisible.svg")
    ...
    def _update_editor_status_display(self, statement: ReportStatement):
        ...
        if statement._validation_status == "INCOMPLETE":
            self.sql_query_status_label.setText(f"âš ï¸ {statement._validation_message}")
            self.sql_query_status_label.setStyleSheet("color: orange;")
        elif statement._validation_status == "ERROR":
            self.sql_query_status_label.setText(f"âŒ {statement._validation_message}")
            self.sql_query_status_label.setStyleSheet("color: red;")
        elif statement._validation_status == "0_RESULTS":
            self.sql_query_status_label.setText(f"âš ï¸ {statement._validation_message}")
            self.sql_query_status_label.setStyleSheet("color: orange;")
        else:  # OK or Ready
            self.sql_query_status_label.setText(f"âœ… {statement._validation_message}")
    ...
    def _run_and_display_preview(self):
        ...
        try:
            ...
            error_item = QtWidgets.QTableWidgetItem(f"âŒ {str(e)}")
    ...
    def accept(self):
        ...
        if spreadsheet:
            AuraCADGui.ActiveDocument.setEdit(spreadsheet.Name, 0)

        # Close the task panel.
        try:
            AuraCADGui.Control.closeDialog()
        except Exception as e:
            AuraCAD.Console.PrintLog(f"Could not close Report Task Panel: {e}\n")
        ...
        def _on_edit_source(self):
            ...
            if not QDesktopServices.openUrl(url):
                QtWidgets.QMessageBox.warning(
                    ...
                    translate(
                        "Arch",
                        "AuraCAD could not open the file. Please check if you have a default text editor configured in your operating system.",
                    ),
                )
