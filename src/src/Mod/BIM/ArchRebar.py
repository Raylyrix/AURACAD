...
class _Rebar(ArchComponent.Component):
    ...
    def __init__(self, obj):
        ...
        obj.IauracadType = "Reinforcing Bar"
    ...
    def getRebarData(self, obj):
        ...
        if not axis:
            axis = obj.Base.Placement.Rotation.multVec(AuraCAD.Vector(0, 0, -1))
        if hasattr(obj, "Direction"):
            if not DraftVecUtils.isNull(obj.Direction):
                axis = AuraCAD.Vector(obj.Direction)
                axis.normalize()
        size = 0
        if father:
    ...
    def execute(self, obj):
        ...
        if hasattr(obj, "CustomSpacing"):
            if obj.CustomSpacing:
                spacinglist = strprocessOauracadustomSpacing(obj.CustomSpacing)
                influenceArea = sum(spacinglist) - spacinglist[0] / 2 - spacinglist[-1] / 2
        # Drop this check to solve issue as discussed here: https://github.com/AuraCAD/AuraCAD/pull/2550
        ...
        if spacinglist:
            ...
            if round(influenceArea) > round(reqInfluenceArea):
                ...
                AuraCAD.Console.PrintWarning("Last span is greater that end offset.\n")
    ...
    def setEdit(self, vobj, mode):
        ...
        if hasattr(vobj.Object, "RebarShape"):
            try:
                ...
                AuraCAD.Console.PrintError("Unable to import RebarShape module\n")
                return False
...
def CalculatePlacement(
    ...
    placement = AuraCAD.Placement(barplacement, rotation)
    return placement
...
def strprocessOauracadustomSpacing(span_string):
    """strprocessOauracadustomSpacing(span_string): This function take input
...
def getLengthOfRebar(rebar):
    ...
    # When rebar is derived from Sketch
        ...
        AuraCAD.Console.PrintError("Cannot calculate rebar length from its base object\n")
        return None
